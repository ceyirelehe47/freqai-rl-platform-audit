# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R12:共享 calibration/final orchestration(§12)。
...
R12 修复(§12.4 共享 Orchestrator):
- 正式 calibrate(main/holdout)与 preplan rehearsal 调用同一
  orchestration 函数 orchestrate_calibration_bundle_r12;
- 唯一差异通过 R12ExecutionProfile 表达:namespace 集合与样本量
...
#: R12 supervised 正式 model seeds(§17:3 个全新 seeds;R12 与 R10
#: 的 20261001-3 不重合)。
R12_SUPERVISED_MODEL_SEEDS: tuple[int, ...] = (
    20270111, 20270112, 20270113)
R12_SUPERVISED_GATE: dict[str, Any] = {
    # repair R12(B2):min_seeds_passing 语义 = 每个 gated control
...
CALIBRATION_PAIRS_PER_RUNG_R12 = 10
C2_INDEPENDENT_PAIRS_PER_RUNG_R12 = 20
SEMANTIC_BLOCKS_PER_CORPUS_R12 = 160
EQUIVALENCE_PAIRS_PER_RUNG_R12 = 3


@dataclass(frozen=True)
class R12ExecutionProfile:
    ...
    artifact_suffix: str = ""
    #: repair R12(工作包 C):shadow 路由类(工程 rehearsal;非正式)
    shadow: bool = False
    #: repair R12(工作包 C):supervised 训练配置(仅允许缩短纯训练
...
def formal_main_profile_r12(n_blocks: int) -> R12ExecutionProfile:
    return R12ExecutionProfile(
        name="formal_main", preplan=False,
        c13_eval_namespace="calibration_r12",
        equivalence_namespace="calibration_r12",
        supervised_namespace="supervised_main_r12",
        semantic_namespace="cue_semantic_calibration_r12",
        c2_matched_namespace="calibration_r12",
        c2_independent_namespace="c2_independent_calibration_r12",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R12,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R12,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R12,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R12,
        supervised_train_pair_limit=6,
        supervised_model_seeds=R12_SUPERVISED_MODEL_SEEDS,
        equivalence_pairs_per_rung=EQUIVALENCE_PAIRS_PER_RUNG_R12,
    )


def formal_holdout_profile_r12(n_blocks: int) -> R12ExecutionProfile:
    return R12ExecutionProfile(
        name="formal_holdout", preplan=False,
        c13_eval_namespace="calibration_holdout_r12",
        equivalence_namespace="calibration_holdout_r12",
        supervised_namespace="supervised_holdout_r12",
        semantic_namespace="cue_semantic_holdout_r12",
        c2_matched_namespace="calibration_holdout_r12",
        c2_independent_namespace="c2_independent_holdout_r12",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R12,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R12,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R12,
...
def rehearsal_main_profile_r12() -> R12ExecutionProfile:
    return R12ExecutionProfile(
        name="rehearsal_main", preplan=True,
        c13_eval_namespace="preplan_calibration_main_r12",
        equivalence_namespace="preplan_calibration_main_r12",
        supervised_namespace="preplan_supervised_main_r12",
        semantic_namespace="preplan_semantic_main_r12",
        c2_matched_namespace="preplan_calibration_main_r12",
        c2_independent_namespace="preplan_calibration_main_r12",
...
def rehearsal_holdout_profile_r12() -> R12ExecutionProfile:
    return R12ExecutionProfile(
        name="rehearsal_holdout", preplan=True,
        c13_eval_namespace="preplan_calibration_holdout_r12",
        equivalence_namespace="preplan_calibration_holdout_r12",
        supervised_namespace="preplan_supervised_holdout_r12",
        semantic_namespace="preplan_semantic_validation_r12",
        c2_matched_namespace="preplan_calibration_holdout_r12",
        c2_independent_namespace="preplan_calibration_holdout_r12",
        c13_pairs_per_rung=1,
        c2_blocks=2,
        semantic_blocks=4,
        c2_independent_pairs_per_rung=2,
...
def shadow_main_profile_r12() -> R12ExecutionProfile:
    ...
    return R12ExecutionProfile(
        ...
        c13_eval_namespace="shadow_calibration_main_r12",
        equivalence_namespace="shadow_calibration_main_r12",
        supervised_namespace="shadow_supervised_main_r12",
        semantic_namespace="shadow_semantic_main_r12",
        c2_matched_namespace="shadow_calibration_main_r12",
        c2_independent_namespace="shadow_c2_independent_main_r12",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R12,
        c2_blocks=SHADOW_C2_BLOCKS_R12,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R12,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R12,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R12,
        supervised_train_pair_limit=6,
...
def shadow_holdout_profile_r12() -> R12ExecutionProfile:
    """full-scale shadow holdout(与 shadow_main 对称;独立 namespace)。"""
    return R12ExecutionProfile(
        name="shadow_holdout", preplan=False, shadow=True,
        c13_eval_namespace="shadow_calibration_holdout_r12",
        equivalence_namespace="shadow_calibration_holdout_r12",
        supervised_namespace="shadow_supervised_holdout_r12",
        semantic_namespace="shadow_semantic_validation_r12",
        c2_matched_namespace="shadow_calibration_holdout_r12",
        c2_independent_namespace="shadow_c2_independent_holdout_r12",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R12,
        c2_blocks=SHADOW_C2_BLOCKS_R12,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R12,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R12,
...
def preprocessing_robustness_checks_r12(
        routing_main: R12BundleRouting,
        routing_holdout: R12BundleRouting,
        ...
        pack: dict[str, Any], *,
        profile: R12ExecutionProfile,
        ledger: RoutingLedgerR12,
        profile_holdout: R12ExecutionProfile | None = None) -> dict[str, Any]:
    """§18 Preprocessing V2 重新资格审查(R12 版;结构承 R6 电池)。
    ...
    space_validation = validate_observation_space_v2(
        ...
        context="preprocessing_robustness_r12")
    ...
    return {
        "format": "cur261-r12-preprocessing-robustness-v1",
        "iteration": "r12",
...
def run_full_supervised_release_rehearsal(out_dir: Path) -> dict[str, Any]:
    ...
    训练面:
    - 正式 3 个 model seeds(R12_SUPERVISED_MODEL_SEEDS);
    ...
    参数。使用 preplan namespace(preplan_fit_main_r12 /
    preplan_supervised_main_r12 等)。
    """
    ...
    pack = _shadow_pack()
    override_fn = lambda family, p: r12_override_for(family, p)  # noqa: E731
    roles: dict[str, Any] = {}
    any_nan_inf = False
    alignment_failures: list[str] = []
    for role, fit_ns, sup_ns in (
            ("main", "preplan_fit_main_r12", "preplan_supervised_main_r12"),
            ("holdout", "preplan_fit_holdout_r12",
             "preplan_supervised_holdout_r12")):
        records = generate_fit_bank_r12(fit_ns, pack, pairs_per_rung=2)
        bundle, manifest = fit_preprocessor_v2_from_bank_r12(
            fit_ns, pack, records=records, pairs_per_rung=2,
            parameter_pack_identity=str(pack.get("digest", "")))
        sup = supervised_learnability_run_r12(
            bundle, pack, namespace=sup_ns,
            pairs_per_rung=10, train_pair_limit=6,
            model_seeds=R12_SUPERVISED_MODEL_SEEDS)
        ...
        roles[role] = {
            ...
            "model_seeds": list(R12_SUPERVISED_MODEL_SEEDS),
        ...
        (out_dir / f"full_supervised_rehearsal_{role}.json").write_text(
            ...
            encoding="utf-8")
    result = {
        "format": "cur261-r12-full-supervised-release-rehearsal-v1",
        "development_evidence_only": True,
        "not_used_for_parameter_selection": True,
        "formal_model_seeds": list(R12_SUPERVISED_MODEL_SEEDS),
        "formal_gate_constants": R12_SUPERVISED_GATE,
        "roles": roles,
...
def orchestrate_calibration_stage_r12(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R12BundleRouting,
        routing_holdout: R12BundleRouting,
        records_main: list, records_holdout: list,
        profile_main: R12ExecutionProfile,
        profile_holdout: R12ExecutionProfile,
        override_fn: Any,
    ...
    repair R12(工作包 A):整个编排期间打开 generation invocation
    ...
    with sink:
        return _orchestrate_calibration_stage_inner_r12(
...
def _orchestrate_calibration_stage_inner_r12(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R12BundleRouting,
        routing_holdout: R12BundleRouting,
        ...
        design_digest: str | None = None,
        write_artifacts: bool = True) -> dict[str, Any]:
    """orchestrate_calibration_stage_r12 的执行核心(envelope sink 内)。"""
    from rl_curriculum.curriculum261_r12_calibration import (
    ...
    ledger = RoutingLedgerR12()
    result: dict[str, Any] = {
        "format": "cur261-r12-calibration-orchestration-v1",
        "iteration": "r12",
    ...
    prep_rob = preprocessing_robustness_checks_r12(
    ...
    _write("bundle_routing_validation.json", {
        "contract": "cur261-r12-bundle-routing-contract-v1",
    ...
    for role, routing, profile in (
        ...
        v2_sup = require_eval_routing_r12(
            routing, profile.supervised_namespace,
            context=f"supervised_{role}", ledger=ledger)
        supervised[role] = supervised_learnability_run_r12(
            v2_sup, pack,
    ...
    for role, routing, profile in (
        ...
        c13 = run_calibration_corpus_c13_r12(
        ...
        semantic = run_c2_semantic_corpus_r12(
            ...
            artifact_name=semantic_artifact_filename_r12(
                profile.semantic_namespace))
        v2_i = require_eval_routing_r12(
        ...
        marginal = c2_independent_marginal_guard_r12(
            c2_indep, pack, recall_floor_value)
        matched_conditions = c2_matched_conditions_r12(c2_matched, pack)
