# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R12:版本化 clean matched-ladder 参数包
(CurriculumR12MatchedLadderPack-v1)。

R7 诚实 FAIL(shared cue gate main LCB 0.912 < floor 0.930;plan
loader digest 自引用缺陷;mirror 边界错误)。R12 在全新 seed 空间
重做 clean design(§1/§2):
...
复用 curriculum261_r6_param_pack(R12 不复制不修改,实现零语义漂移
由构造保证;R6 模块 sha256 进入 R12 code identity,并与 R7 baseline
的记录值对拍)。
"""
...
R12_PACK_VERSION = "CurriculumR12MatchedLadderPack-v1"
...
#: 作为 R12 development evidence,R12 必须在新 namespace 重做)。
...
#: evidence。全部结果仅作 R12 development evidence;R12 必须在全新
...
#: §17 预注册 R12 candidate 网格(恰好 3 个;生成任何 R12 design
#: episode 前锁定进 r12_design_plan)。
...
C2_LADDER_CANDIDATES_R12: dict[str, dict[str, dict[str, Any]]] = {
...
def r12_candidate_grid() -> dict[str, Any]:
    """预注册 R12 ladder 网格深拷贝(design plan 锁定用)。"""
    import copy

    return copy.deepcopy(C2_LADDER_CANDIDATES_R12)


def validate_r12_grid_semantics() -> list[str]:
    """§17 网格级校验(数量恰好 3;D3 alpha 边界;单调;键白名单)。"""
    issues: list[str] = []
    ids = list(C2_LADDER_CANDIDATES_R12)
    if len(ids) != 3:
        issues.append(f"candidate 数量必须恰好 3(§17 禁止第四个),"
                      f"收到 {len(ids)}")
    for cid, ladder in C2_LADDER_CANDIDATES_R12.items():
...
#: R12 允许覆盖的 family 白名单(与 R6/R7 一致)。
R12_OVERRIDE_FAMILIES = ("c1_opportunity", "c2_context", "c3_cost")
...
def ladder_pack_payload_r12(
    ...
    """构造 R12 clean matched-ladder pack payload(§27)。
    ...
    - R12 新增绑定(§16):code freeze SHA、policy-visible reference
    ...
    return {
        "format": "cur261-r12-clean-matched-ladder-pack-v1",
        "pack_version": R12_PACK_VERSION,
        "iteration": "r12",
        ...
        "generation_determinism_contract_identity": (
            "cur261-r12-generation-invocation-envelope-v1(Generation"
        ...
        "override_scope": {
            "families": list(R12_OVERRIDE_FAMILIES),
            "c2_rungs": ["D0", "D1", "D2", "D3"],
            "rules": "C1/C3 D3 = R4 选定候选(逐位继承);C2 四档全量"
                     "经 R12 design 选出(只动 alpha_bps/wick_kappa);"
...
def pack_digest_r12(pack: dict[str, Any]) -> str:
    """pack digest(canonical JSON;排除运行时间与 digest 自身字段)。"""
    payload = dict(pack)
    payload.pop("created_utc", None)
    payload.pop("digest", None)
    return "r12pk-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def write_selected_pack_r12(out_dir: Path, pack: dict[str, Any]) -> Path:
    ...
    path = out_dir / "r12_parameter_pack.json"
    path.write_text(json.dumps(pack, indent=2, ensure_ascii=False,
                               default=str), encoding="utf-8")
    (out_dir / "r12_parameter_pack_digest.txt").write_text(
...
def load_selected_pack(out_dir: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise RuntimeError(
            f"R12 parameter pack 不存在: {path}(design + marginal guard "
            "通过并锁定后才允许 calibration/final)")
    pack = json.loads(path.read_text(encoding="utf-8"))
    stored = pack.get("digest")
    if not stored or pack_digest_r12(pack) != stored:
        raise RuntimeError("R12 parameter pack digest 复算不一致(fail closed)")
    digest_path = out_dir / "r12_parameter_pack_digest.txt"
    if digest_path.is_file() and \
            digest_path.read_text(encoding="utf-8").strip() != stored:
        raise RuntimeError("R12 parameter pack digest 文件与 payload 不一致")
    return pack


def r12_override_for(
        family: str,
        pack: dict[str, Any]) -> dict[str, dict[str, Any]] | None:
    return _override_for(family, pack)


def apply_r12_override(
...
def r12_family_rung_params(
        family: str, pack: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    return _family_rung_params(family, pack)


def frozen_parameter_identity_r12() -> dict[str, Any]:
    """R12 冻结参数面身份(C1/C3 D0-D2 + C2 结构键;final 复算比对)。"""
    from rl_curriculum.curriculum261_r6_param_pack import (
        frozen_parameter_identity_r6,
    )

    out = frozen_parameter_identity_r6()
    out["r12_note"] = "R12 不修改任何冻结参数面(与 R6/R7 逐位一致)"
    return out


def verify_r4_inheritance_r12(pack: dict[str, Any]) -> dict[str, Any]:
...
def ladder_distance_from_historical_r12(
        ladder: dict[str, dict[str, Any]]) -> float:
    return ladder_distance_from_historical(ladder)


__all__ = [
    "R12_PACK_VERSION", "R4_PARAMETER_PACK_DIGEST",
    "R5_DESIGN_PLAN_DIGEST", "R6_DESIGN_PLAN_DIGEST",
    "R7_DESIGN_PLAN_DIGEST",
    "C2_LADDER_CANDIDATES_R12", "r12_candidate_grid",
    "validate_r12_grid_semantics", "R12_OVERRIDE_FAMILIES",
    "ladder_pack_payload_r12", "pack_digest_r12", "write_selected_pack_r12",
    "load_selected_pack", "r12_override_for", "apply_r12_override",
    "r12_family_rung_params", "frozen_parameter_identity_r12",
    "verify_r4_inheritance_r12", "ladder_distance_from_historical_r12",
