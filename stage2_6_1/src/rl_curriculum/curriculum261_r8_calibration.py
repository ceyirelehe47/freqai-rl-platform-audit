# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R8:calibration/holdout 语料执行与 C2 R8 条件组装。

fit bank / C1/C3 / C2 matched / C2 independent / supervised / V2
robustness / stress 全部复用 R6 冻结实现(namespace 参数化,R8 传
r8 namespace + R8 pack;R6 模块零修改)。R8 变更:
...
- C2 independent marginal guard 语义修正:R6 冻结实现的 semantics 槽
  ...
  故未暴露)——R8 以 semantics=None 调用统计条件,显式 AND
...
CALIBRATION_PAIRS_PER_RUNG_R8 = 10
C2_INDEPENDENT_PAIRS_PER_RUNG_R8 = 20
#: §28/§29 dedicated semantic corpus 规模(与 design 阶段一致 = 160)。
SEMANTIC_BLOCKS_PER_CORPUS_R8 = 160


def generate_fit_bank_r8(
...
def fit_preprocessor_v2_from_bank_r8(
        namespace: str, pack: dict[str, Any],
...
def run_calibration_corpus_c13_r8(
        preproc_v2: Any, pack: dict[str, Any], namespace: str,
        pairs_per_rung: int = CALIBRATION_PAIRS_PER_RUNG_R8,
) -> dict[str, Any]:
    return _run_c13(preproc_v2, pack, namespace, pairs_per_rung)


def run_c2_matched_corpus_r8(
        preproc_v2: Any, pack: dict[str, Any], namespace: str,
        n_blocks: int,
) -> dict[str, Any]:
...
def run_c2_independent_corpus_r8(
        preproc_v2: Any, pack: dict[str, Any], namespace: str,
        pairs_per_rung: int = C2_INDEPENDENT_PAIRS_PER_RUNG_R8,
) -> dict[str, Any]:
    return _run_c2_independent(preproc_v2, pack, namespace,
                               pairs_per_rung)


def run_c2_semantic_corpus_r8(
        pack: dict[str, Any], namespace: str,
        n_blocks: int = SEMANTIC_BLOCKS_PER_CORPUS_R8,
    ...
    ladder = r8_family_rung_params(FAMILY_C2, pack)
    ...
    result = {
        "format": "cur261-r8-semantic-corpus-v1",
        "namespace": namespace,
    ...
    if out_dir is not None and artifact_name is not None:
        out_dir = Path(out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        dump = dict(result)
        dump["shared"] = {k: v for k, v in shared.items()
                          if k != "event_trace"}
        dump["event_count"] = len(trace_rows)
        (out_dir / artifact_name).write_text(
            __import__("json").dumps(
                dump, indent=2, ensure_ascii=False, default=float),
            encoding="utf-8")
        with open(out_dir / artifact_name.replace(".json", ".jsonl"),
...
def c2_matched_conditions_r8(
    ...
    checks = {
        ...
        "cue_semantics_delegated_note": (
            ...
            "160-block semantic corpus(cue_semantic_*_r8 namespace;"
            "§29)"),
    }
    return {
        "format": "cur261-r8-c2-matched-conditions-v1",
        "statistical": base,
...
def c2_independent_marginal_guard_r8(
    ...
    r8_semantics_ok = bool(
        ...
        and cue["pass"])
    guard = dict(base)
    guard["format"] = "cur261-r8-c2-marginal-guard-v1"
    guard["r8_semantics_rule"] = (
        "local cue ∧ context observability ∧ independent cue semantics"
        "(point recall ≥ 0.90 灾难护栏;§26)")
    guard["r8_semantics_pass"] = r8_semantics_ok
    guard["semantics_pass"] = r8_semantics_ok
    guard["pass"] = bool(base["pass"] and r8_semantics_ok)
    return {
        "format": "cur261-r8-c2-independent-marginal-v1",
...
def supervised_learnability_run_r8(
        preproc_v2: Any, pack: dict[str, Any], namespace: str,
...
def run_generator_stress_r8(pack: dict[str, Any],
                            **kwargs: Any) -> dict[str, Any]:
    return _run_generator_stress(pack, **kwargs)


def run_c2_density_diagnostics_r8(
...
def run_c2_diagnostics_r8(records: list[Any]) -> dict[str, Any]:
    """C2 三语义诊断(local cue/context observability/R6 点阈值
    separation——最后一项仅诊断对照,R8 资格判定用 dedicated semantic
...
__all__ = [
    "generate_fit_bank_r8", "fit_preprocessor_v2_from_bank_r8",
    "run_calibration_corpus_c13_r8", "run_c2_matched_corpus_r8",
    "run_c2_independent_corpus_r8", "run_c2_semantic_corpus_r8",
    "c2_matched_conditions_r8", "c2_independent_marginal_guard_r8",
    "supervised_learnability_run_r8", "run_generator_stress_r8",
    "run_c2_density_diagnostics_r8", "run_c2_diagnostics_r8",
    "load_selected_pack", "SEMANTIC_BLOCKS_PER_CORPUS_R8",
]
