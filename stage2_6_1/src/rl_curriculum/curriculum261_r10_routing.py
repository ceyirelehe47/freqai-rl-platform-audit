# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R10:main/holdout/final bundle routing 合同(§9)。
...
R10 修复(§9):
- 显式 R10BundleRouting 数据类:role / fit namespace / 三层 bundle hash,
...
- RoutingLedgerR10 落盘完整路由矩阵(§9.4):corpus → expected fit
...
R10_ROLE_FIT_NAMESPACE: dict[str, str] = {
...
R10_PREPLAN_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "preplan_fit_main_r10",
    "holdout": "preplan_fit_holdout_r10",
    "final": "preplan_fit_main_r10",  # rehearsal final-like 复用 main fit
    "diagnostic": "preplan_fit_main_r10",
}

#: 评估 namespace → 期望 role(§9.1;正式 + preplan)。
R10_EVAL_NAMESPACE_ROLE: dict[str, str] = {
    # ---- 正式 ----
    "calibration_r10": "main",
    "calibration_holdout_r10": "holdout",
    "qualification_r10": "final",
    "c2_independent_calibration_r10": "main",
    "c2_independent_holdout_r10": "holdout",
    "c2_independent_qualification_r10": "final",
    "supervised_main_r10": "main",
    "supervised_holdout_r10": "holdout",
    "cue_semantic_calibration_r10": "main",
    "cue_semantic_holdout_r10": "holdout",
    "cue_semantic_qualification_r10": "final",
    # ---- preplan / rehearsal / diagnostic(非正式)----
    "reference_diagnostic_main_r10": "diagnostic",
    "reference_diagnostic_holdout_r10": "diagnostic",
    "preplan_smoke_r10": "diagnostic",
    "preplan_candidate_eval_r10": "diagnostic",
    "preplan_supervised_main_r10": "main",
    "preplan_supervised_holdout_r10": "holdout",
    "preplan_calibration_main_r10": "main",
    "preplan_calibration_holdout_r10": "holdout",
    "preplan_final_r10": "final",
}
...
class R10BundleRouting:
    ...
    def bundle(self, *, expected_role: str | None = None,
               ...
               context: str = "",
               ledger: "RoutingLedgerR10 | None" = None) -> Any:
...
def build_routing_r10(
        role: str, v2: Any, *,
        preplan: bool = False,
        expected_bundle_hash: str | None = None) -> R10BundleRouting:
    ...
    table = (R10_PREPLAN_ROLE_FIT_NAMESPACE if preplan
             else R10_ROLE_FIT_NAMESPACE)
    ...
    if v2.namespace not in (list(R10_ROLE_FIT_NAMESPACE.values())
                            + list(R10_PREPLAN_ROLE_FIT_NAMESPACE.values())):
        raise RoutingContractError(
            f"bundle fit namespace {v2.namespace} 不属于任何 R10 路由表"
            f"(fail closed)")
    if expected_bundle_hash is not None \
...
def require_eval_routing_r10(
        routing: R10BundleRouting, eval_namespace: str, *,
        context: str = "",
        ledger: "RoutingLedgerR10 | None" = None) -> Any:
    ...
    拒绝(§9.3)。
    """
    expected_role = R10_EVAL_NAMESPACE_ROLE.get(eval_namespace)
    if expected_role is None:
        raise RoutingContractError(
            f"评估 namespace {eval_namespace} 不在 R10 权威路由映射中"
            f"(fail closed;context={context})")
    table = (R10_PREPLAN_ROLE_FIT_NAMESPACE if routing.preplan
             else R10_ROLE_FIT_NAMESPACE)
    ...
    if routing.preplan and not eval_namespace.startswith(
            ("preplan_", "reference_diagnostic")):
        raise RoutingContractError(
            f"preplan routing 不得服务正式评估 namespace "
            f"{eval_namespace}(fail closed)")
    if not routing.preplan and eval_namespace.startswith(
            ("preplan_", "reference_diagnostic")):
        raise RoutingContractError(
            f"正式 routing 不得服务 preplan 评估 namespace "
            f"{eval_namespace}(fail closed)")
...
def bundle_routing_contract_digest() -> str:
    ...
    return "r10rt-" + hashlib.sha256(json.dumps(
...
def bundle_routing_contract_payload() -> dict[str, Any]:
    ...
    return {
        "format": "cur261-r10-bundle-routing-contract-v1",
        "iteration": "r10",
        "formal_role_fit_namespace": dict(R10_ROLE_FIT_NAMESPACE),
        "preplan_role_fit_namespace": dict(R10_PREPLAN_ROLE_FIT_NAMESPACE),
        "eval_namespace_role": dict(R10_EVAL_NAMESPACE_ROLE),
        "fail_closed_rules": [
