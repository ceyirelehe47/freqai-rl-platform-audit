# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R14:Dependence-Aware Global Cue-K 审计(C2MirrorCountGlobalAudit-v1)。
...
全局 null 与机械判定(§10):
  ...
  cue_k_global_null_r14)逐 replicate 重采样全部 unit gap,保持依赖结构;
...
GLOBAL_K_NULL_STREAM_NAMESPACE = "cue_k_global_null_r14"
...
def global_k_audit_contract_payload() -> dict[str, Any]:
    """预注册合同身份载荷(任何 R14 正式 cue event 生成前锁定)。"""
    return {
        ...
        "legacy_diagnostic": {
            ...
            "note": "R11 t=226 的 |z|=4.000504 属于该 legacy 检查;R14 "
                    "起不再决定 PASS/FAIL",
        },
        "tail_definition": {
...
def global_k_audit_contract_digest() -> str:
    blob = json.dumps(global_k_audit_contract_payload(), sort_keys=True,
                      ensure_ascii=False)
    return "r14gk-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
...
class CellSpec:
    ...
    R14 早期草稿的 w_u×Bernoulli(1/9) 权重式在聚合 cell 上错误)。
...
def run_global_k_audit(
    ...
    base = {
        "format": "cur261-r14-global-k-result-v1",
