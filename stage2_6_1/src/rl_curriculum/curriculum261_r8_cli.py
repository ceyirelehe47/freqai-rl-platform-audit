# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R8 CLI(audit → cue-audit → preplan-smoke →
...
digest 自引用缺陷修复的正式验收);design plan 在第一条 R8 design/
...
BASELINE_COMMIT_R8 = "11951f6d9b2f5fa63b17e3857aba92b330da029e"
...
def _pack(out_dir: Path) -> dict:
    from rl_curriculum.curriculum261_r8_param_pack import load_selected_pack
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    _write_json(out, "baseline_integrity.json", {
        "expected_baseline": BASELINE_COMMIT_R8,
        "release_repo_head": head,
        "baseline_matches": bool(head == BASELINE_COMMIT_R8),
        "note": "发布仓库 HEAD 应等于 R7 诚实 FAIL checkpoint"
                "(或其明确后继;组装 R8 产物前)",
    })
    ...
    _dump_txt(out, "preprocessing_v2_contract_digest.txt",
              preprocessing_v2_contract_digest())
    records = generate_fit_bank("preplan_smoke_r8", args.fit_pairs)
...
def cmd_preplan_smoke(args: argparse.Namespace) -> int:
    ...
    不参与参数选择;只用 preplan_smoke_r8,不用 design/calibration/
    ...
    blocks = [generate_matched_block_with_attempts(
        sentinel, namespace="preplan_smoke_r8", block_index=i)
        for i in range(3)]
    ...
    mini_gate = semantic_cue_gate(
        ...
        min_unique_positive_cues=1, label="preplan_smoke_r8")
    identity = {
        "sentinel": {r: {k: sentinel[r][k]
                         for k in ("alpha_bps", "wick_kappa")}
                     for r in sentinel},
        "sentinel_digest": "r8smoke-" + _hashlib.sha256(json.dumps(
            {r: sorted(sentinel[r].items()) for r in sorted(sentinel)},
            default=str).encode("utf-8")).hexdigest(),
        "n_blocks": len(blocks),
        "namespace": "preplan_smoke_r8",
    ...
    _write_json(out, "preplan_engineering_smoke.json", {
        "format": "cur261-r8-preplan-smoke-v1",
        "identity": identity,
...
_SUBPROC_LOADER_DESIGN = (
    ...
    "(design_plan_digest_r8, load_locked_design_plan_r8);"
    "import json,sys;"
    "plan,digest=load_locked_design_plan_r8(sys.argv[1]);"
    "recomputed=design_plan_digest_r8(plan);"
    "print(json.dumps({"
...
_SUBPROC_LOADER_QUAL = (
    "import os,sys;"
    "os.environ['CURRICULUM261_R8_LOCK_DIR']=sys.argv[2];"
    "from rl_curriculum.curriculum261_r8_plan import "
    "(load_locked_plan_r8, plan_digest_r8);"
    "import json;"
    "plan,digest=load_locked_plan_r8();"
    "print(json.dumps({"
    "'loaded_digest':digest,"
    "'recomputed':plan_digest_r8(plan),"
    "'iteration':plan['iteration'],"
...
def _no_alternate_loader_check() -> dict:
    ...
    权威模块(r8_design 的 lock/load;r8_namespaces 的路径常量与本
    扫描自身);其余模块不得直接读 plan JSON。"""
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    allowed_design = {"curriculum261_r8_design.py",
                      "curriculum261_r8_namespaces.py",
                      "curriculum261_r8_cli.py"}
    allowed_qual = {"curriculum261_r8_namespaces.py",
                    "curriculum261_r8_plan.py",
                    "curriculum261_r8_cli.py"}
    design_hits: list[str] = []
    qual_hits: list[str] = []
    for f in sorted(root.glob("curriculum261_r8_*.py")):
        text = f.read_text(encoding="utf-8")
        if "r8_design_plan.json" in text and f.name not in allowed_design:
            design_hits.append(f.name)
        if ("qualification_plan_r8.json" in text
                and f.name not in allowed_qual):
            qual_hits.append(f.name)
    return {
        "design_plan_readers": ["curriculum261_r8_design.py"],
        "qualification_plan_readers": ["curriculum261_r8_namespaces.py"
                                       " (path fn) + "
                                       "curriculum261_r8_plan.py (loader)"],
...
def cmd_plan_roundtrip(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td)
        plan = design_plan_payload_r8(
            baseline_commit=BASELINE_COMMIT_R8,
        ...
        pre_digest = design_plan_digest_r8(plan)
        path, digest = lock_design_plan_r8(tmp, plan)
        ...
        checks["payload_bit_identical"] = bool(
            design_plan_digest_r8(reloaded) == pre_digest)
        # 新进程 load(真实生产路径)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        synth_pack = {
            "digest": "r8pk-" + "0" * 64,
            "pack_version": "CurriculumR8MatchedLadderPack-v1",
            "selected_c2_candidate": "c2l_midpoint",
            "selected_block_count": 15,
            "c2_ladder": {r: dict(v) for r, v in
                          C2_LADDER_CANDIDATES_R8["c2l_midpoint"].items()},
            ...
            "recall_floor": 0.93,
            "p_contract": 0.95,
            "cue_contract_audit_digest": "r8ca-" + "0" * 64,
        ...
        if qual_plan is not None:
            ...
            old_dir = os.environ.get("CURRICULUM261_R8_LOCK_DIR")
            os.environ["CURRICULUM261_R8_LOCK_DIR"] = str(tmp)
            try:
                qpath, qdigest = lock_plan_r8(qual_plan)
                checks["qualification_lock_ok"] = bool(
                    qpath.is_file()
                    and qdigest == plan_digest_r8(qual_plan))
                ...
                if old_dir is None:
                    os.environ.pop("CURRICULUM261_R8_LOCK_DIR", None)
                else:
                    os.environ["CURRICULUM261_R8_LOCK_DIR"] = old_dir
    ...
    from rl_curriculum.curriculum261_r8_design import (
        load_locked_design_plan_r8 as _unused_loader,  # noqa: F401
    )
    ...
    result = {
        "format": "cur261-r8-plan-roundtrip-validation-v1",
        "iteration": "r8",
        "checks": checks,
...
def cmd_design_plan_lock(args: argparse.Namespace) -> int:
    ...
    path, digest = lock_design_plan_r8(out, plan)
...
def cmd_design(args: argparse.Namespace) -> int:
    ...
    out = Path(args.out_dir)
    plan, digest = load_locked_design_plan_r8(out)
    selection = run_design_stage_r8(out, plan, digest,
                                    baseline_commit=BASELINE_COMMIT_R8)
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r8_param_pack import (
        frozen_parameter_identity_r8,
        r8_family_rung_params,
        r8_override_for,
        verify_r4_inheritance_r8,
    )

    require_r8_iteration_active()
    out = Path(args.out_dir)
    pack = _pack(out)
    inheritance = verify_r4_inheritance_r8(pack)
    if not inheritance:
        print("[calibrate] R4 inheritance 验证失败;fail closed")
        return 1
    n_blocks = int(pack["selected_block_count"])
    design_digest = (out / "r8_design_plan_digest.txt").read_text(
        encoding="utf-8").strip()
    recall_floor_value = float(pack["recall_floor"])

    print("[calibrate] fitting main preprocessor "
          "(preprocess_fit_calibration_r8)...")
    records_main = generate_fit_bank_r8(
        "preprocess_fit_calibration_r8", pack)
    v2_main, manifest_main = fit_preprocessor_v2_from_bank_r8(
        "preprocess_fit_calibration_r8", pack, records=records_main,
        parameter_pack_identity=pack["digest"])
    print("[calibrate] fitting holdout preprocessor "
          "(preprocess_fit_holdout_r8)...")
    records_hold = generate_fit_bank_r8(
        "preprocess_fit_holdout_r8", pack)
    v2_hold, manifest_hold = fit_preprocessor_v2_from_bank_r8(
        "preprocess_fit_holdout_r8", pack, records=records_hold,
        parameter_pack_identity=pack["digest"])
    ...
    _write_json(out, "fit_eval_isolation.json", {
        ...
        "holdout": {k: manifest_hold[k] for k in (
            "namespace", "pairs_per_rung", "n_pairs", "n_episodes",
            "n_rows", "integrity_all_ok", "multiset_hash")},
        "eval_namespaces": ["calibration_r8", "calibration_holdout_r8"],
        "c2_matched_namespaces": ["calibration_r8",
                                  "calibration_holdout_r8"],
        "c2_semantic_namespaces": ["cue_semantic_calibration_r8",
                                   "cue_semantic_holdout_r8"],
        "c2_independent_namespaces": [
            "c2_independent_calibration_r8",
            "c2_independent_holdout_r8"],
    ...
    eval_records = [
        generate_pair(f, r, 0, namespace="calibration_r8",
                      rung_params_override=r8_override_for(f, pack))
        for f in CURRICULUM261_FAMILIES for r in CURRICULUM261_RUNGS]
    equiv_records = [
        generate_pair(f, r, i, namespace="calibration_r8",
                      rung_params_override=r8_override_for(f, pack))
        for f in CURRICULUM261_FAMILIES for r in CURRICULUM261_RUNGS
        for i in range(3)]
    ...
    supervised = supervised_learnability_run_r8(v2_main, pack)
    _write_json(out, "supervised_learnability.json", supervised)

    print("[calibrate] C1/C3 calibration_r8 + calibration_holdout_r8...")
    calib_main_c13 = run_calibration_corpus_c13_r8(
        v2_main, pack, "calibration_r8")
    calib_hold_c13 = run_calibration_corpus_c13_r8(
        v2_main, pack, "calibration_holdout_r8")
    ...
    semantic_main = run_c2_semantic_corpus_r8(
        pack, "cue_semantic_calibration_r8",
        out_dir=out, artifact_name="cue_semantic_calibration.json")
    semantic_hold = run_c2_semantic_corpus_r8(
        pack, "cue_semantic_holdout_r8",
    ...
    c2_matched_main = run_c2_matched_corpus_r8(
        v2_main, pack, "calibration_r8", n_blocks)
    c2_matched_hold = run_c2_matched_corpus_r8(
        v2_main, pack, "calibration_holdout_r8", n_blocks)
    ...
    c2_conditions_main = c2_matched_conditions_r8(c2_matched_main, pack)
    c2_conditions_hold = c2_matched_conditions_r8(
    ...
    c2_indep_main = run_c2_independent_corpus_r8(
        v2_main, pack, "c2_independent_calibration_r8")
    c2_indep_hold = run_c2_independent_corpus_r8(
        v2_main, pack, "c2_independent_holdout_r8")
    marginal_main = c2_independent_marginal_guard_r8(
        c2_indep_main, pack, recall_floor_value)
    marginal_hold = c2_independent_marginal_guard_r8(
    ...
    c2_diag = run_c2_diagnostics_r8(c2_sem_records)
    ...
    preprocessing_gate = {
        "format": "cur261-r8-preprocessing-robustness-gate-v1",
    ...
    # R8 curriculum gate(strict per-corpus AND + dedicated semantic
    # corpus ×2 + C2 matched(R8 条件)+ independent marginal guard)
    fam_conditions = {}
    for family in ("c1_opportunity", "c3_cost"):
        fam_conditions[family] = {
            "calibration_r8_conditions_strict": corpus_conditions_r6_pair(
                calib_main_c13["families"][family],
                ROBUSTNESS_KAPPA_R6),
            "calibration_holdout_r8_conditions_strict":
                corpus_conditions_r6_pair(
                    calib_hold_c13["families"][family],
                    ROBUSTNESS_KAPPA_R6),
        }
    curriculum_gate = {
        "format": "cur261-r8-curriculum-robustness-gate-v1",
        "iteration": "r8",
        ...
        "c2_matched": {
            "calibration_r8_conditions": c2_conditions_main,
            "calibration_holdout_r8_conditions": c2_conditions_hold,
        },
        "c2_marginal": {
    ...
    curriculum_gate["pass"] = bool(
        all(cond_["calibration_r8_conditions_strict"]["pass"]
            and cond_["calibration_holdout_r8_conditions_strict"]["pass"]
    ...
    overall = {
        "format": "cur261-r8-robustness-gate-v1",
        "iteration": "r8",
        "preprocessing_gate": {
    ...
    _write_json(out, "calibration_evidence.json", {
        ...
        "rung_params_by_family": {
            f: r8_family_rung_params(f, pack)
            for f in ("c1_opportunity", "c2_context", "c3_cost")},
        "frozen_parameter_identity": frozen_parameter_identity_r8(),
        "reference_thresholds_by_family": {
            f: dict(family_specs()[f].reference_defaults)
    ...
    _write_json(out, "calibration_summary.json", {
        "format": "cur261-r8-calibration-summary-v1",
        "c1_c3_main": {f: {
            "difficulty_ladder":
                calib_main_c13["families"][f]["difficulty_ladder"],
            "conditions": fam_conditions[f][
                "calibration_r8_conditions_strict"]}
            for f in ("c1_opportunity", "c3_cost")},
    ...
    _write_json(out, "calibration_holdout_summary.json", {
        "format": "cur261-r8-calibration-holdout-summary-v1",
        "c1_c3_holdout": {f: {
            "difficulty_ladder":
    ...
    if not overall["pass"]:
        print("[calibrate] robustness gate FAIL——禁止 lock plan(§29);"
              "R8 = FAIL,修复须 R8.1/R9 + 全新 namespaces")
        return 1
    return 0
...
def cmd_preflight_static(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r8_preflight import (
        run_prelock_static_preflight_r8,
    )

    result = run_prelock_static_preflight_r8(Path(args.out_dir),
                                             VENDOR_PIN)
...
def cmd_preflight_sealed(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r8_preflight import (
        run_postlock_sealed_preflight_r8,
    )

    att = run_postlock_sealed_preflight_r8(Path(args.out_dir),
                                           VENDOR_PIN)
...
def cmd_qualify(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r8_final import (
        run_final_qualification_r8,
    )

    result = run_final_qualification_r8(Path(args.out_dir))
...
def cmd_smoke(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r8_smoke import run_ppo_smoke_r8
    ...
    try:
        ...
        pass
    smoke = run_ppo_smoke_r8(pack=pack)
...
def cmd_namespace_integrity(args: argparse.Namespace) -> int:
    ...
    print(f"[namespace-integrity] pass={ns.get('pass')} "
          f"namespaces={len(ns.get('r8_namespaces', []))}")
    return 0 if ns.get("pass") else 1


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="r8-cli")
