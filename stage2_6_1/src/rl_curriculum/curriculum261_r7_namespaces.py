# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R7:iteration/seed namespace、守卫与 exposure 硬合同。

§15 新 R7 seed 空间(与 R0-R6 的 Stage 2.6.1 namespace 及全部
Stage 2.6.2 official/diagnostic namespace 完全不相交):
- cue_contract_audit_r7:cue 检测合同审计专用(p_contract Monte Carlo;
  不读 candidate ladder、不读任何 R7 design 结果);
- preplan_smoke_r7:plan 锁定前工程 smoke(固定 sentinel ladder;
  不参与任何参数选择);
- design_r7_matched_main / design_r7_matched_validation:
  matched-ladder candidate 开发语料(两份独立,均为开发数据,
  不得称为 holdout);
- design_r7_independent_marginal:选定 ladder 的独立-rung
  marginal guard 语料(§21);
- preprocess_fit_calibration_r7 / preprocess_fit_holdout_r7 /
  preprocess_fit_qualification_r7(preprocessing fit bank 专用);
- calibration_r7 / calibration_holdout_r7 / qualification_r7;
- c2_independent_calibration_r7 / c2_independent_holdout_r7 /
  c2_independent_qualification_r7(C2 独立-rung marginal guard 三阶段);
- stress_r7 / fresh_holdout_r7 / training_r7 / ppo_smoke_r7。

qualification 三 namespace 解锁守卫(六要素,沿用 R5/R6 治理;R7 相对
R6 补强:c2_independent_qualification_r7 一并纳入守卫):
1. plan JSON 存在(iteration == r7);
2. plan digest 文件存在;
3. digest 重算与锁定值一致;
...
- iteration aborted marker(O_CREAT|O_EXCL)——design data 已生成后
  任何代码/评估器缺陷导致的中止,必须写 aborted 记录并永久结束 R7;
  任何 R7 阶段入口(design/calibration/final)发现 aborted 即拒绝执行;
...
- append-only ledger(qualification_exposure_ledger_r7.jsonl)先于 marker
...
#: R7 全部 seed namespace(单一来源定义在 curriculum261_api.py 白名单;
#: 本模块 re-export 供守卫/验证使用)。
from rl_curriculum.curriculum261_api import (  # noqa: E402
    CURRICULUM261_R7_NAMESPACES,
)

#: R7 artifacts 目录(WSL 项目侧;发布时映射到 stage2_6_1/artifacts/repair7)。
_REPAIR7_ARTIFACTS = Path(__file__).resolve().parents[2] / "artifacts" / (
    "route_c_stage2_6_1_repair7")

#: design plan / parameter pack / qualification plan / exposure 文件名。
R7_DESIGN_PLAN_FILENAME = "r7_design_plan.json"
R7_DESIGN_PLAN_DIGEST_FILENAME = "r7_design_plan_digest.txt"
R7_PARAMETER_PACK_FILENAME = "r7_parameter_pack.json"
R7_DESIGN_DECISION_FILENAME = "r7_design_decision.json"
R7_PLAN_FILENAME = "qualification_plan_r7.json"
R7_PLAN_DIGEST_FILENAME = "qualification_plan_digest_r7.txt"
R7_EXPOSURE_MARKER_NAME = "qualification_exposure_r7.json"
R7_EXPOSURE_LEDGER_NAME = "qualification_exposure_ledger_r7.jsonl"
R7_SEALED_PREFLIGHT_FILENAME = "sealed_final_preflight.json"
R7_SEALED_PREFLIGHT_DIGEST_FILENAME = "sealed_final_preflight_digest.txt"
R7_STATIC_PREFLIGHT_FILENAME = "prelock_static_preflight.json"
R7_FINAL_LOCK_NAME = "qualification_r7.lock"

#: §16.2 iteration 终止记录(design data 已生成后的 crash/代码缺陷)。
R7_ITERATION_LEDGER_NAME = "r7_iteration_events.jsonl"
R7_ABORTED_MARKER_NAME = "r7_iteration_aborted.json"

#: marker 允许状态机:running -> {completed, failed, crashed}(单向一次)。
R7_EXPOSURE_TERMINAL_STATUSES = ("completed", "failed", "crashed")


def _env_dir(env_key: str) -> Path:
    env_val = os.environ.get(env_key)
    return Path(env_val) if env_val else _REPAIR7_ARTIFACTS


def qualification_r7_lock_dir() -> Path:
    return _env_dir("CURRICULUM261_R7_LOCK_DIR")


def r7_design_plan_path() -> Path:
    return qualification_r7_lock_dir() / R7_DESIGN_PLAN_FILENAME


def r7_design_plan_digest_path() -> Path:
    return qualification_r7_lock_dir() / R7_DESIGN_PLAN_DIGEST_FILENAME


def r7_design_decision_path() -> Path:
    return qualification_r7_lock_dir() / R7_DESIGN_DECISION_FILENAME


def r7_parameter_pack_path() -> Path:
    return qualification_r7_lock_dir() / R7_PARAMETER_PACK_FILENAME


def qualification_r7_plan_path() -> Path:
    return qualification_r7_lock_dir() / R7_PLAN_FILENAME


def qualification_r7_digest_path() -> Path:
    return qualification_r7_lock_dir() / R7_PLAN_DIGEST_FILENAME


def qualification_r7_exposure_marker() -> Path:
    return qualification_r7_lock_dir() / R7_EXPOSURE_MARKER_NAME


def qualification_r7_exposure_ledger() -> Path:
    return qualification_r7_lock_dir() / R7_EXPOSURE_LEDGER_NAME


def sealed_preflight_path() -> Path:
    return qualification_r7_lock_dir() / R7_SEALED_PREFLIGHT_FILENAME


def sealed_preflight_digest_path() -> Path:
    return qualification_r7_lock_dir() / R7_SEALED_PREFLIGHT_DIGEST_FILENAME


def r7_iteration_ledger_path() -> Path:
    return qualification_r7_lock_dir() / R7_ITERATION_LEDGER_NAME


def r7_aborted_marker_path() -> Path:
    return qualification_r7_lock_dir() / R7_ABORTED_MARKER_NAME


# ------------------------------------------------- §16.2 iteration 事件账本
def _iteration_ledger_append(event: str, note: str = "") -> None:
    """append-only iteration 事件账本(design-data-started / aborted)。"""
    path = r7_iteration_ledger_path()
...
def mark_design_data_started() -> None:
    ...
    """
    if r7_iteration_aborted():
        raise RuntimeError(
            "R7 iteration 已 aborted(design data 生成后发生代码/评估器"
...
def write_r7_iteration_aborted(reason: str) -> None:
    """§16.2:写 aborted marker(O_CREAT|O_EXCL;已存在即拒绝)。"""
    path = r7_aborted_marker_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "iteration": CURRICULUM261_ITERATION_ID_R7,
        ...
        "contract": "design data 已生成后发生代码/评估器/统计缺陷;"
                    "按 §16.2 R7 iteration 永久结束:原 plan 保留不得删除,"
                    "不得复用任何 R7 design namespace,下一轮必须 R7.1/R8"
                    " + 全新 seed space。",
    }
...
def r7_iteration_aborted() -> bool:
    if r7_aborted_marker_path().is_file():
        return True
...
def require_r7_iteration_active() -> None:
    """R7 各阶段入口共用守卫:aborted 后拒绝一切继续执行。"""
    if r7_iteration_aborted():
        raise RuntimeError(
            "R7 iteration 已 aborted;按 §16.2 永久结束,任何继续执行"
...
def qualification_r7_unlocked() -> bool:
    """完整解锁守卫(六要素;全部成立才允许派生 final namespace seed)。"""
    plan_path = qualification_r7_plan_path()
    digest_path = qualification_r7_digest_path()
    ...
    try:
        pack = load_selected_pack(qualification_r7_lock_dir())
    except RuntimeError:
        return False
...
def sealed_preflight_binds_plan(plan: dict[str, Any]) -> bool:
    ...
    return bool(att.get("plan_digest") == plan_digest_r7(plan))


def qualification_r7_unlocked_detail() -> dict[str, Any]:
    plan_path = qualification_r7_plan_path()
    digest_path = qualification_r7_digest_path()
    detail: dict[str, Any] = {
...
def _ledger_append(event: str, plan_digest: str, status: str,
    ...
    path = qualification_r7_exposure_ledger()
    path.parent.mkdir(parents=True, exist_ok=True)
    line = json.dumps({
        "event": event,
        "iteration": CURRICULUM261_ITERATION_ID_R7,
...
def write_qualification_r7_exposure(plan_digest: str,
    ...
    """
    path = qualification_r7_exposure_marker()
    path.parent.mkdir(parents=True, exist_ok=True)
    if status == "running":
        ...
        payload = {
            ...
            "written_utc": datetime.now(timezone.utc).isoformat(
                timespec="seconds"),
            "contract": "R7 final qualification 一旦开始执行即视为语料"
                        "暴露;无论结果如何(含 crash),同一 "
                        "qualification_r7 corpus 不得再次执行;marker "
                        "不可删除/覆盖/恢复为未暴露(ledger 兜底);下一"
                        "次必须 R7.1/R8 + 全新 seed space。",
        }
        ...
        try:
            ...
            raise RuntimeError(
                "qualification_r7 exposure marker 已存在(并发 final 或"
        ...
        try:
            ...
            os.close(fd)
        return
    if status not in R7_EXPOSURE_TERMINAL_STATUSES:
    ...
    if current.get("status") in R7_EXPOSURE_TERMINAL_STATUSES:
        raise RuntimeError(
...
def qualification_r7_exposed() -> bool:
    """本轮 iteration 是否已暴露(marker 存在 OR ledger 有任何事件)。"""
    if qualification_r7_exposure_marker().is_file():
        return True
    return any(e.get("iteration") == CURRICULUM261_ITERATION_ID_R7
               and "unparseable_line" not in e
               for e in ledger_entries())


class QualificationR7FileLock:
    ...
    def __enter__(self) -> "QualificationR7FileLock":
        import fcntl

        path = qualification_r7_lock_dir() / R7_FINAL_LOCK_NAME
        path.parent.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            self._fh = None
            raise RuntimeError(
                "另一个 R7 final qualification 进程持有锁(并发 final "
                "被拒绝)") from exc
        return self
...
# ------------------------------------------------- namespace 隔离验证
def verify_r7_namespace_isolation() -> dict[str, Any]:
    """§15 namespace-integrity:R7 namespace 与全部历史空间不相交。"""
    ...
    r7_set = set(CURRICULUM261_R7_NAMESPACES)
    historical = [
    ...
    for ns in list(CURRICULUM261_R7_NAMESPACES) + historical:
        vals = set()
    ...
    r7_keys = [k for k in keys if k in r7_set]
    r7_vs_hist_overlap = sorted(
        set().union(*[seen[k] for k in r7_keys]) & set().union(
            *[seen[k] for k in keys if k not in r7_set])
    ) if r7_keys and len(keys) > len(r7_keys) else []
    name_disjoint = bool(
        not (set(CURRICULUM261_R7_NAMESPACES)
             & (set(historical) | set(ns_262))))
    return {
        "format": "cur261-r7-seed-namespace-integrity-v1",
        "iteration": CURRICULUM261_ITERATION_ID_R7,
        "r7_namespaces": list(CURRICULUM261_R7_NAMESPACES),
        ...
        "r7_vs_historical_overlap": len(r7_vs_hist_overlap),
        "name_space_disjoint": name_disjoint,
        "matched_block_seed_key_checked": True,
        "qualification_r7_locked_before_use": bool(
            not qualification_r7_unlocked()),
        "qualification_r7_exposed": qualification_r7_exposed(),
        "iteration_aborted": r7_iteration_aborted(),
        "design_data_started": design_data_started(),
        "pass": bool(
            not collisions and not r7_vs_hist_overlap and name_disjoint),
    }
