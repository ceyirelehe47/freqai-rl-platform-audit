...
REHEARSAL_FIT_PAIRS_PER_RUNG_R10 = 2


def generate_fit_bank_r10(
...
def fit_preprocessor_v2_from_bank_r10(
        namespace: str, pack: dict[str, Any], *,
...
def run_calibration_corpus_c13_r10(
        preproc_v2: Any, pack: dict[str, Any], namespace: str, *,
        pairs_per_rung: int = CALIBRATION_PAIRS_PER_RUNG_R10,
) -> dict[str, Any]:
...
def run_c2_independent_corpus_r10(
        preproc_v2: Any, pack: dict[str, Any], namespace: str, *,
        pairs_per_rung: int = C2_INDEPENDENT_PAIRS_PER_RUNG_R10,
) -> dict[str, Any]:
...
def run_c2_semantic_corpus_r10(
        pack: dict[str, Any], namespace: str, *,
        n_blocks: int = SEMANTIC_BLOCKS_PER_CORPUS_R10,
        out_dir: Path | None = None,
        artifact_name: str | None = None) -> dict[str, Any]:
    """§18/§19 dedicated cue semantic corpus(R9 语义;R10 namespace)。
    ...
    ladder = r10_family_rung_params(FAMILY_C2, pack)
    ...
    result = {
        "format": "cur261-r10-semantic-corpus-v1",
        "namespace": namespace,
    ...
    if out_dir is not None:
        ...
        expected_name = semantic_artifact_filename_r10(namespace)
...
def c2_matched_conditions_r10(
    ...
    checks = {
        ...
        "cue_semantics_delegated_note": (
            ...
            "160-block semantic corpus(cue_semantic_*_r10 namespace)"),
    }
    return {
        "format": "cur261-r10-c2-matched-conditions-v1",
        "statistical": base,
...
def c2_independent_marginal_guard_r10(
    ...
    r10_semantics_ok = bool(
        ...
        and cue["pass"])
    guard = dict(base)
    guard["format"] = "cur261-r10-c2-marginal-guard-v1"
    guard["r10_semantics_rule"] = (
        "local cue ∧ context observability ∧ independent cue semantics"
        "(point recall ≥ 0.90 灾难护栏)")
    guard["r10_semantics_pass"] = r10_semantics_ok
    guard["semantics_pass"] = r10_semantics_ok
    guard["pass"] = bool(base["pass"] and r10_semantics_ok)
    return {
        "format": "cur261-r10-c2-independent-marginal-v1",
...
# ---------------------------------------------------- supervised(§8/§17)
def supervised_learnability_run_r10(
        preproc_v2: Any, pack: dict[str, Any], *,
        namespace: str,
        pairs_per_rung: int = CALIBRATION_PAIRS_PER_RUNG_R10,
        train_pair_limit: int = 6,
        model_seeds: tuple | list = R10_SUPERVISED_MODEL_SEEDS,
    ...
    for family in CURRICULUM261_FAMILIES:
        ...
        rung_params = r10_family_rung_params(family, pack)
        dataset = collect_policy_visible_dataset_r10(
        ...
        controls = ["U", "W", "B"]
        gated_controls = list(R10_SUPERVISED_GATE["gated_controls"])
        ...
        for seed in model_seeds:
            for control in controls:
                ...
                gated = bool(
                    ...
                    >= R10_SUPERVISED_GATE["behavior_gap_min"])
        ...
        family_pass = bool(
            n_passing >= R10_SUPERVISED_GATE["min_seeds_passing"])
        ...
        out.setdefault("dataset_identity", {})[family] = (
            supervised_dataset_identity_r10(dataset))
    out["alignment_all_ok"] = alignment_all
    out["pass"] = bool(overall and alignment_all)
    return out


def run_generator_stress_r10(
        pack: dict[str, Any], *,
        pairs_per_rung: int = 12,
        namespace: str = "stress_r10") -> dict[str, Any]:
    """generator stress(R6 冻结实现;namespace 显式 stress_r10)。

    R9 缺陷修复:R9 CLI 调用未传 namespace,R6 默认回落历史
    stress_r6 —— R10 wrapper 把 namespace 变为显式 keyword-only
    参数,默认 stress_r10。
    """
...
def run_c2_density_diagnostics_r10(
...
def run_c2_diagnostics_r10(records: list[Any]) -> dict[str, Any]:
...
__all__ = [
    "generate_fit_bank_r10", "fit_preprocessor_v2_from_bank_r10",
    "run_calibration_corpus_c13_r10", "run_c2_matched_corpus_r10",
    "run_c2_independent_corpus_r10", "run_c2_semantic_corpus_r10",
    "c2_matched_conditions_r10", "c2_independent_marginal_guard_r10",
    "supervised_learnability_run_r10", "run_generator_stress_r10",
    "run_c2_density_diagnostics_r10", "run_c2_diagnostics_r10",
    "load_selected_pack", "SEMANTIC_BLOCKS_PER_CORPUS_R10",
    "REHEARSAL_FIT_PAIRS_PER_RUNG_R10",
]
