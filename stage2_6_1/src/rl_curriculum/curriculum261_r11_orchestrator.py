# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R11:共享 calibration/final orchestration(§12)。
...
R11 修复(§12.4 共享 Orchestrator):
- 正式 calibrate(main/holdout)与 preplan rehearsal 调用同一
  orchestration 函数 orchestrate_calibration_bundle_r11;
- 唯一差异通过 R11ExecutionProfile 表达:namespace 集合与样本量
...
#: R11 supervised 正式 model seeds(§17:3 个全新 seeds;R11 与 R10
#: 的 20261001-3 不重合)。
R11_SUPERVISED_MODEL_SEEDS: tuple[int, ...] = (
    20261101, 20261102, 20261103)
R11_SUPERVISED_GATE: dict[str, Any] = {
    # repair R11(B2):min_seeds_passing 语义 = 每个 gated control
...
CALIBRATION_PAIRS_PER_RUNG_R11 = 10
C2_INDEPENDENT_PAIRS_PER_RUNG_R11 = 20
SEMANTIC_BLOCKS_PER_CORPUS_R11 = 160
EQUIVALENCE_PAIRS_PER_RUNG_R11 = 3


@dataclass(frozen=True)
class R11ExecutionProfile:
    ...
    artifact_suffix: str = ""
    #: repair R11(工作包 C):shadow 路由类(工程 rehearsal;非正式)
    shadow: bool = False
    #: repair R11(工作包 C):supervised 训练配置(仅允许缩短纯训练
...
def formal_main_profile_r11(n_blocks: int) -> R11ExecutionProfile:
    return R11ExecutionProfile(
        name="formal_main", preplan=False,
        c13_eval_namespace="calibration_r11",
        equivalence_namespace="calibration_r11",
        supervised_namespace="supervised_main_r11",
        semantic_namespace="cue_semantic_calibration_r11",
        c2_matched_namespace="calibration_r11",
        c2_independent_namespace="c2_independent_calibration_r11",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R11,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R11,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R11,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R11,
        supervised_train_pair_limit=6,
        supervised_model_seeds=R11_SUPERVISED_MODEL_SEEDS,
        equivalence_pairs_per_rung=EQUIVALENCE_PAIRS_PER_RUNG_R11,
    )


def formal_holdout_profile_r11(n_blocks: int) -> R11ExecutionProfile:
    return R11ExecutionProfile(
        name="formal_holdout", preplan=False,
        c13_eval_namespace="calibration_holdout_r11",
        equivalence_namespace="calibration_holdout_r11",
        supervised_namespace="supervised_holdout_r11",
        semantic_namespace="cue_semantic_holdout_r11",
        c2_matched_namespace="calibration_holdout_r11",
        c2_independent_namespace="c2_independent_holdout_r11",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R11,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R11,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R11,
...
def rehearsal_main_profile_r11() -> R11ExecutionProfile:
    return R11ExecutionProfile(
        name="rehearsal_main", preplan=True,
        c13_eval_namespace="preplan_calibration_main_r11",
        equivalence_namespace="preplan_calibration_main_r11",
        supervised_namespace="preplan_supervised_main_r11",
        semantic_namespace="preplan_semantic_main_r11",
        c2_matched_namespace="preplan_calibration_main_r11",
        c2_independent_namespace="preplan_calibration_main_r11",
...
def rehearsal_holdout_profile_r11() -> R11ExecutionProfile:
    return R11ExecutionProfile(
        name="rehearsal_holdout", preplan=True,
        c13_eval_namespace="preplan_calibration_holdout_r11",
        equivalence_namespace="preplan_calibration_holdout_r11",
        supervised_namespace="preplan_supervised_holdout_r11",
        semantic_namespace="preplan_semantic_validation_r11",
        c2_matched_namespace="preplan_calibration_holdout_r11",
        c2_independent_namespace="preplan_calibration_holdout_r11",
        c13_pairs_per_rung=1,
        c2_blocks=2,
        semantic_blocks=4,
        c2_independent_pairs_per_rung=2,
...
def shadow_main_profile_r11() -> R11ExecutionProfile:
    ...
    return R11ExecutionProfile(
        ...
        c13_eval_namespace="shadow_calibration_main_r11",
        equivalence_namespace="shadow_calibration_main_r11",
        supervised_namespace="shadow_supervised_main_r11",
        semantic_namespace="shadow_semantic_main_r11",
        c2_matched_namespace="shadow_calibration_main_r11",
        c2_independent_namespace="shadow_c2_independent_main_r11",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R11,
        c2_blocks=SHADOW_C2_BLOCKS_R11,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R11,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R11,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R11,
        supervised_train_pair_limit=6,
...
def shadow_holdout_profile_r11() -> R11ExecutionProfile:
    """full-scale shadow holdout(与 shadow_main 对称;独立 namespace)。"""
    return R11ExecutionProfile(
        name="shadow_holdout", preplan=False, shadow=True,
        c13_eval_namespace="shadow_calibration_holdout_r11",
        equivalence_namespace="shadow_calibration_holdout_r11",
        supervised_namespace="shadow_supervised_holdout_r11",
        semantic_namespace="shadow_semantic_validation_r11",
        c2_matched_namespace="shadow_calibration_holdout_r11",
        c2_independent_namespace="shadow_c2_independent_holdout_r11",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R11,
        c2_blocks=SHADOW_C2_BLOCKS_R11,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R11,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R11,
...
def preprocessing_robustness_checks_r11(
        routing_main: R11BundleRouting,
        routing_holdout: R11BundleRouting,
        ...
        pack: dict[str, Any], *,
        profile: R11ExecutionProfile,
        ledger: RoutingLedgerR11,
        profile_holdout: R11ExecutionProfile | None = None) -> dict[str, Any]:
    """§18 Preprocessing V2 重新资格审查(R11 版;结构承 R6 电池)。
    ...
    space_validation = validate_observation_space_v2(
        ...
        context="preprocessing_robustness_r11")
    ...
    for role, routing, equiv_records, prof in (
        ...
        report = reference_equivalence_run_r11(
    ...
    return {
        "format": "cur261-r11-preprocessing-robustness-v1",
        "iteration": "r11",
...
def orchestrate_calibration_stage_r11(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R11BundleRouting,
        routing_holdout: R11BundleRouting,
        records_main: list, records_holdout: list,
        profile_main: R11ExecutionProfile,
        profile_holdout: R11ExecutionProfile,
        override_fn: Any,
    ...
    repair R11(工作包 A):整个编排期间打开 generation invocation
    ...
    with sink:
        return _orchestrate_calibration_stage_inner_r11(
...
def _orchestrate_calibration_stage_inner_r11(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R11BundleRouting,
        routing_holdout: R11BundleRouting,
        ...
        design_digest: str | None = None,
        write_artifacts: bool = True) -> dict[str, Any]:
    """orchestrate_calibration_stage_r11 的执行核心(envelope sink 内)。"""
    from rl_curriculum.curriculum261_r11_calibration import (
        c2_independent_marginal_guard_r11,
        c2_matched_conditions_r11,
        run_c2_density_diagnostics_r11,
        run_c2_independent_corpus_r11,
        run_c2_matched_corpus_r11,
        run_c2_semantic_corpus_r11,
        run_calibration_corpus_c13_r11,
    ...
    ledger = RoutingLedgerR11()
    result: dict[str, Any] = {
        "format": "cur261-r11-calibration-orchestration-v1",
        "iteration": "r11",
    ...
    prep_rob = preprocessing_robustness_checks_r11(
    ...
    _write("bundle_routing_validation.json", {
        "contract": "cur261-r11-bundle-routing-contract-v1",
    ...
    for role, routing, profile in (
        ...
        v2_sup = require_eval_routing_r11(
            routing, profile.supervised_namespace,
            context=f"supervised_{role}", ledger=ledger)
        supervised[role] = supervised_learnability_run_r11(
            v2_sup, pack,
    ...
    for role, routing, profile in (
        ...
        c13 = run_calibration_corpus_c13_r11(
    ...
    # 密度诊断(跨双语料;诊断性质)
    density = run_c2_density_diagnostics_r11(
