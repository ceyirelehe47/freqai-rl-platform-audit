# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R11:Cue 检测语义合同 v2 与三路闭合合同审计。
...
1. mirror candidate 边界修正:真实 paired_noise() 只有
   ...
   实现在 curriculum261_r11_noise_replay.mirror_candidate_positions
...
2. 三路独立闭合(§12;R7 只有 analytic 自证 + 单一 bridge):
   ...
   C. Direct Generator Replay:两个全新 candidate-independent audit
      corpora(cue_contract_model_r11 / cue_contract_validation_r11,
...
dedicated semantic corpus(§14/§15,curriculum261_r11_cue_eval)。
"""
...
#: 其他语义指标业务阈值(与 R6/R7 相同;R11 用 cluster bounds)。
...
#: 合同审计配置(冻结;§12)。
AUDIT_MODEL_NAMESPACE = "cue_contract_model_r11"
AUDIT_VALIDATION_NAMESPACE = "cue_contract_validation_r11"
...
# ------------------------------------------------- 三路闭合 audit(§12)
#: §R11-10 cue audit plan(audit data 生成前锁定;不可修改)。
CUE_AUDIT_PLAN_FORMAT_R11 = "cur261-r11-cue-audit-plan-v1"
CUE_AUDIT_PLAN_FILENAME = "cue_audit_plan.json"
CUE_AUDIT_PLAN_DIGEST_FILENAME = "cue_audit_plan_digest.txt"
CUE_AUDIT_CODE_MODULES_R11 = (
...
def cue_audit_code_identity_r11() -> dict[str, str]:
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    out: dict[str, str] = {}
    for name in CUE_AUDIT_CODE_MODULES_R11:
        f = root / name
...
def cue_audit_plan_payload_r11(
        code_identity: dict[str, str] | None = None) -> dict[str, Any]:
    """§R11-10:audit data 生成前锁定的 plan payload(全部预注册)。"""
    return {
        "format": CUE_AUDIT_PLAN_FORMAT_R11,
        "audit_namespaces": {
...
def load_locked_cue_audit_plan_r11(out_dir: Path) -> dict[str, Any]:
    out_dir = Path(out_dir)
    path = out_dir / CUE_AUDIT_PLAN_FILENAME
    dpath = out_dir / CUE_AUDIT_PLAN_DIGEST_FILENAME
    if not path.is_file() or not dpath.is_file():
        raise RuntimeError(
            "cue audit plan 未锁定(§R11-10:正式 audit data 生成前"
            "必须先锁定)")
    ...
    current_ident = cue_audit_code_identity_r11()
    ...
    if drift:
        raise RuntimeError(
            f"cue audit plan code identity 与当前代码漂移(§R11-10;"
            f"audit plan 锁定后相关模块不得修改):{sorted(drift)}")
    return payload


def _once_vs_attempts_bitwise_check_r11(
        blocks: list[Any], ladder: dict[str, dict[str, Any]],
        namespace: str, max_blocks: int = 50) -> dict[str, Any]:
    """§R11-11:attempts-mode 选中 attempt==0 的 block 与 once-mode 同
...
def run_cue_contract_audit(
    ...
    """执行三路闭合合同审计(在任何 R11 design/semantic data 之前)。
    ...
    if require_locked_plan:
        ...
        audit_plan = load_locked_cue_audit_plan_r11(out_dir)
    ...
    for corpus_name, ns in (("model", model_ns),
        ...
        if mode == "once":
            ...
            # §R11-10/§R11-11:validation 语料用正式 attempts-mode
            ...
            once_bitwise = _once_vs_attempts_bitwise_check_r11(
                blocks_v, ladder, ns)
    ...
    # ---- §R11-11 once-mode vs attempts-mode 系统偏差对照 ----
    ...
    report: dict[str, Any] = {
        "format": "cur261-r11-cue-contract-audit-v1",
        ...
        "mirror_bound_v2": {
            ...
            "authority": "curriculum261_r11_noise_replay."
    ...
    if out_dir is not None:
        ...
        (out_dir / "once_vs_attempts_audit.json").write_text(
            json.dumps({
                "format": "cur261-r11-once-vs-attempts-audit-v1",
                "model_mode": "once",
                "validation_mode": "attempts",
        ...
        (out_dir / "cue_k_distribution.json").write_text(json.dumps({
            "format": "cur261-r11-cue-k-distribution-v1",
        ...
        (out_dir / "tail_mirror_validation.json").write_text(json.dumps({
            "format": "cur261-r11-tail-mirror-validation-v1",
        ...
        (out_dir / "noise_replay_validation.json").write_text(json.dumps({
            "format": "cur261-r11-noise-replay-validation-v1",
            "tolerance": 1e-12,
            "model": {
...
def cue_contract_audit_digest(report: dict[str, Any]) -> str:
    ...
    blob = json.dumps(core, sort_keys=True, ensure_ascii=False,
                      default=float)
    return "r11ca-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
...
def cue_semantic_contract_digest() -> str:
    blob = json.dumps(cue_semantic_contract_payload(), sort_keys=True,
                      ensure_ascii=False)
    return "r11cue-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
