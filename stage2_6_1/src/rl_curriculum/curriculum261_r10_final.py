# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R10:一次性 final qualification(§27-§29)。
...
9. marker -> completed;任何异常 -> marker -> crashed + crash evidence
   落盘 + re-raise(保留 marker,禁止修复后复用 qualification_r10)。
...
from rl_curriculum.curriculum261_r10_calibration import (
    C2_INDEPENDENT_PAIRS_PER_RUNG_R10,
    SEMANTIC_BLOCKS_PER_CORPUS_R10,
...
FINAL_FORMAT_R10 = "cur261-r10-final-qualification-v1"
...
from rl_curriculum.curriculum261_r10_orchestrator import (
    CALIBRATION_PAIRS_PER_RUNG_R10,
    R10_SUPERVISED_MODEL_SEEDS,
)


def qualification_r10_lock_dir_default() -> Path:
    from rl_curriculum.curriculum261_r10_namespaces import (
...
def _fresh_seed_validity_r10(n_checks: int = 10) -> dict[str, Any]:
    """fresh_holdout_r10 与 qualification_r10 的 seed 不相交验证。"""
    from rl_curriculum.curriculum261_api import derive261_seed

    mismatches = []
    for i in range(n_checks):
        for family in CURRICULUM261_FAMILIES:
            q = derive261_seed("qualification_r10", family, "D2", i, 0)
            for rung in CURRICULUM261_RUNGS:
                f = derive261_seed("fresh_holdout_r10", family, rung, i, 0)
                if q == f:
...
def run_final_qualification_r10(out_dir: Path,
                               vendor_dir: Path | None = None,
                               ) -> dict[str, Any]:
    """执行一次性 R10 final qualification(§28)。"""
    require_r10_iteration_active()
    if qualification_r10_exposed():
        raise RuntimeError(
            "R10 final qualification 已执行过(exposure marker/ledger "
            "存在)——同一 qualification_r10 corpus 不得再次执行;继续必须"
            "使用新迭代身份(R10)与全新 seed space")
    plan, digest = load_locked_plan_r10()
    if (plan.get("robustness_gate") or {}).get("pass") is not True:
        raise RuntimeError(
            "R10 plan 的 robustness_gate.pass != true——final "
            "qualification fail closed")
    pack = load_selected_pack(qualification_r10_lock_dir_default())
    ...
    code_ok = (plan["code_identity"] == _code_identity_r10())
    ...
    if not static_ok:
        raise RuntimeError(
            ...
            "之前 fail closed——本轮 qualification_r10 未消耗")
    ...
    if not freeze["pass"]:
        raise RuntimeError(
            f"R10 code freeze 校验失败(正式数据开始后源码漂移;"
            f"§6/§21 永久结束):{freeze}")
    ...
    try:
        with QualificationR10FileLock(blocking=False):
            write_qualification_r10_exposure(digest, status="running")
            result = execute_final_core_r10(
                out_dir, plan, pack, digest=digest, started=started,
                profile_name="formal_final",
                final_namespace="qualification_r10",
                fit_namespace="preprocess_fit_qualification_r10",
        ...
        try:
            (out_dir / "qualification_crash_traceback.log").write_text(
                exc, encoding="utf-8")
            write_qualification_r10_exposure(digest, status="crashed")
        except Exception:  # noqa: BLE001 - 崩溃路径不得掩盖原始异常
            pass
        raise


def execute_final_core_r10(
        out_dir: Path, plan: dict[str, Any], pack: dict[str, Any], *,
        digest: str = "", started: str | None = None,
        profile_name: str = "formal_final",
        final_namespace: str = "qualification_r10",
        fit_namespace: str = "preprocess_fit_qualification_r10",
    ...
    """final 主流程(正式与 rehearsal 共享同一执行核心;§12.4)。

    正式:run_final_qualification_r10(exposure/plan/freeze 治理外壳)
    以 qualification_r10 全量参数调用;
    rehearsal:preplan 以 preplan_final_r10 + 微样本参数 + 临时
    ...
    - reference equivalence 走 canonical 合同(reference_equivalence_
      run_r10:canonical vs scaled 逐位 + legacy 差异 + 0 unexplained),
    ...
    final_records = generate_fit_bank_r10(fit_namespace, pack)
    final_v2, final_manifest = fit_preprocessor_v2_from_bank_r10(
        fit_namespace, pack, records=final_records,
        parameter_pack_identity=pack["digest"])
    routing_final = build_routing_r10(
        "final", final_v2, preplan=rehearsal)
    ...
    space_validation = validate_observation_space_v2(
        ...
        [int(r.episodes[s].spec.seed) for r in contain_records
         for s in ("A", "B")], context="final_fit_space_v2_r10")
    ...
    for family in CURRICULUM261_FAMILIES:
        ...
        family_reports[family] = rung_report_r4(
            records, family, r10_family_rung_params(family, pack),
    ...
    c2_indep = run_c2_independent_corpus_r10(
        final_v2, pack,
        "c2_independent_qualification_r10" if not rehearsal
        else final_namespace,
        pairs_per_rung=int(independent_pairs_per_rung))
    c2_conditions = c2_matched_conditions_r10(c2_matched, pack)
    c2_marginal = c2_independent_marginal_guard_r10(
        c2_indep, pack, recall_floor_value)
    semantic_ns = ("cue_semantic_qualification_r10" if not rehearsal
                   else "preplan_semantic_main_r10")
    ...
    semantic = run_c2_semantic_corpus_r10(
        pack, semantic_ns,
        n_blocks=int(semantic_block_count), out_dir=out_dir,
        artifact_name=semantic_artifact_filename_r10(semantic_ns))
    ...
    reference_report = reference_equivalence_run_r10(
    ...
    if not rehearsal:
        causality = {
            ...
            "reference_causality": [
                check_reference_causality(
                    f, r10_family_rung_params(f, pack)["D2"],
                    thresholds[f])
                for f in CURRICULUM261_FAMILIES],
        }
    repro = []
    if not rehearsal:
        ...
        repro_pack_override = []
        for family in CURRICULUM261_FAMILIES:
            ov = r10_override_for(family, pack) or {}
        ...
        c2_ladder = r10_family_rung_params(FAMILY_C2, pack)
    ...
    fresh = _fresh_seed_validity_r10(10) if not rehearsal else {
        "pass": True, "rehearsal": True}
    ...
    cond_records = generate_fit_bank_r10(
        "preprocess_fit_calibration_r10", pack) if not rehearsal \
        else final_records
    cond_eval_records = [
        rec for recs in all_records.values() for rec in recs[:8]]
    conditioning = _cond(final_v2.inner, cond_records, cond_eval_records)
    supervised_ns = final_namespace if not rehearsal \
        else "preplan_supervised_main_r10"
    supervised = supervised_learnability_run_r10(
        final_v2, pack, namespace=supervised_ns,
        pairs_per_rung=(CALIBRATION_PAIRS_PER_RUNG_R10
                        if not rehearsal else 2),
        train_pair_limit=(6 if not rehearsal else 1),
        model_seeds=(R10_SUPERVISED_MODEL_SEEDS
                     if not rehearsal else (20261111,)))

    # 6) 密度 + 语义诊断 + scrambled
    density = run_c2_density_diagnostics_r10(
        c2_matched, c2_matched, pack)
    semantics = run_c2_diagnostics_r10(
        [rec for blk in c2_matched["blocks"]
