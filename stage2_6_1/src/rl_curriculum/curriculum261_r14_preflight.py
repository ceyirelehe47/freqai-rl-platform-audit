# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R14:final preflight(§25)。

A. pre-lock static:imports/vendor/Route C identity/code identity/
   V2 serialize+outer env+PPO smoke(ppo_smoke_r14)/matched generator
...
def _imports_ok_r14() -> dict[str, Any]:
    modules = [
...
def _dependency_resolution_ok_r14() -> dict[str, Any]:
    ...
    return resolve_dependency_identity_r14()


def _code_identity_digest_r14() -> dict[str, Any]:
    from rl_curriculum.curriculum261_r14_plan import _code_identity_r14

    ident = _code_identity_r14()
    missing = [k for k, v in ident.items() if v == "MISSING"]
    return {
        "identity": ident,
        "missing": missing,
        "digest": "r14ci-" + hashlib.sha256(
            _canonical(ident).encode("utf-8")).hexdigest(),
        "pass": not missing,
    }


def _matched_generator_probe_r14() -> dict[str, Any]:
    """matched generator + unique-event evaluator + cluster bootstrap
    小规模验证(ppo_smoke_r14 下 2 个 block;不触碰 design/cal/final)。"""
    try:
        ...
        for idx in range(2):
            blocks.append(generate_matched_block_with_attempts(
                dict(C2_RUNG_PARAMS), namespace="ppo_smoke_r14",
                block_index=idx))
        ...
        # R14 §7:真实(非 monkeypatch)candidate evaluator 集成探针
        ...
        eval_result = _evaluate_candidate_matched_r14(
            "preflight_sentinel", dict(C2_RUNG_PARAMS),
            "ppo_smoke_r14", thresholds, blocks=blocks, n_blocks=2)
        evaluator_ok = bool(
...
def run_prelock_static_preflight_r14(out_dir: Path,
                                    vendor_pin: str) -> dict[str, Any]:
    """§25.1:plan lock 前静态 preflight(只用非 final namespace)。"""
    out_dir = Path(out_dir)
    checks: dict[str, Any] = {}
    checks["imports"] = _imports_ok_r14()
    checks["dependency_resolution"] = _dependency_resolution_ok_r14()
    ...
    checks["code_identity"] = _code_identity_digest_r14()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        old = os.environ.get("CURRICULUM261_R14_LOCK_DIR")
        os.environ["CURRICULUM261_R14_LOCK_DIR"] = td
        try:
            ...
            write_qualification_r14_exposure("r14dp-preflight-probe",
                                            "running")
            try:
                write_qualification_r14_exposure("r14dp-preflight-probe",
                                                "running")
            ...
            if old is None:
                ...
                os.environ["CURRICULUM261_R14_LOCK_DIR"] = old
    checks["marker_atomic_exclusive"] = marker_ok
    checks["concurrent_final_lock_rejected"] = lock_ok

    result = {
        "format": "cur261-r14-prelock-static-preflight-v1",
        "iteration": "r14",
        "executed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespaces_touched": ["ppo_smoke_r14"],
...
def sealed_preflight_digest(att: dict[str, Any]) -> str:
    ...
    return "r14fa-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def run_postlock_sealed_preflight_r14(out_dir: Path,
                                     vendor_pin: str,
                                     ) -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    plan, plan_digest = load_locked_plan_r14()
    checks["plan_digest_recompute"] = True
    checks["plan_iteration_r14"] = bool(plan.get("iteration") == "r14")
    ...
    checks["r4_inheritance_verified"] = verify_r4_inheritance_r14(pack)
    ...
    code_digest = "r14ci-" + hashlib.sha256(
        _canonical(ident).encode("utf-8")).hexdigest()
    ...
    checks["final_runner_dependencies"] = _imports_ok_r14()["pass"]
    checks["plan_path_resolves"] = bool(
        qualification_r14_plan_path().is_file()
        and qualification_r14_digest_path().is_file()
        and r14_parameter_pack_path().is_file())
    ...
    attestation = {
        "format": "cur261-r14-sealed-final-preflight-v1",
        "iteration": "r14",
        "pass": att_pass,
...
def write_rehearsal_sealed_preflight_r14(
        out_dir: Path, *, plan_digest: str) -> dict[str, Any]:
    """§12 rehearsal 的 sealed preflight(临时目录;零正式 seed)。

    与正式 run_postlock_sealed_preflight_r14 同构的零 final-seed
    attestation(rehearsal 标志 + preplan namespace 声明)。
    """
    from rl_curriculum.curriculum261_r14_namespaces import (
        R14_SEALED_PREFLIGHT_DIGEST_FILENAME,
        R14_SEALED_PREFLIGHT_FILENAME,
    )
    ...
    (out_dir / R14_SEALED_PREFLIGHT_FILENAME).write_text(
        json.dumps(att, indent=1, ensure_ascii=False), encoding="utf-8")
    (out_dir / R14_SEALED_PREFLIGHT_DIGEST_FILENAME).write_text(
