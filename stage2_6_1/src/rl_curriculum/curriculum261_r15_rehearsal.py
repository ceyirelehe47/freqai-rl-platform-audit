# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R15:preplan full pipeline shadow rehearsal(§12)。

与正式执行共享同一 orchestration:
- calibration 阶段:orchestrate_calibration_stage_r15(rehearsal_*
  profiles;仅样本量与 namespace 与正式不同);
- final 阶段:r15_final.execute_final_core_r15(rehearsal profile;
  临时 exposure 目录);
...
from rl_curriculum.curriculum261_r15_orchestrator import (
    ...
    rehearsal_holdout_profile_r15,
    rehearsal_main_profile_r15,
)
from rl_curriculum.curriculum261_r15_routing import (
    R15_PREPLAN_ROLE_FIT_NAMESPACE,
    build_routing_r15,
    bundle_routing_contract_payload,
)

REHEARSAL_NAMESPACE_TAG = "r15_preplan_full_pipeline_rehearsal_v1"
...
def _rehearsal_pack() -> dict[str, Any]:
    ...
    pack = ladder_pack_payload_r15(
        ...
        design_plan_digest="r15dp-rehearsal-sentinel",
...
def run_preplan_full_pipeline_rehearsal_r15(
    ...
    from rl_curriculum.curriculum261_r15_param_pack import r15_override_for
    ...
    profile_main = rehearsal_main_profile_r15()
    profile_holdout = rehearsal_holdout_profile_r15()
    struct: dict[str, Any] = {
    ...
    for role, fit_ns, pairs in (
            ("main", R15_PREPLAN_ROLE_FIT_NAMESPACE["main"], 2),
            ("holdout", R15_PREPLAN_ROLE_FIT_NAMESPACE["holdout"], 2)):
        records = generate_fit_bank_r15(fit_ns, pack, pairs_per_rung=pairs)
        v2, manifest = fit_preprocessor_v2_from_bank_r15(
            fit_ns, pack, records=records,
            parameter_pack_identity=pack["digest"])
        routing = build_routing_r15(role, v2, preplan=True)
    ...
    stage = orchestrate_calibration_stage_r15(
        ...
        design_digest="r15dp-rehearsal-sentinel",
    ...
    try:
        from rl_curriculum.curriculum261_r15_plan import (
            build_rehearsal_qualification_plan_r15,
            lock_qualification_plan_r15,
            load_locked_qualification_plan_r15,
        )

        tmp_plan_dir = out_dir / "preplan_plan_temp"
        tmp_plan_dir.mkdir(parents=True, exist_ok=True)
        plan, plan_digest = build_rehearsal_qualification_plan_r15(
            pack=pack, stage_summary=stage,
            final_namespace="preplan_final_r15",
            fit_namespace="preplan_fit_main_r15")
        lock_qualification_plan_r15(tmp_plan_dir, plan)
        loaded, digest2 = load_locked_qualification_plan_r15(tmp_plan_dir)
        plan_info = {
            "locked": True,
            "digest": plan_digest,
            ...
            == json.dumps({**plan, "plan_digest": plan_digest},
                          sort_keys=True),
            "namespace": "preplan_final_r15(临时目录;零正式 ns)",
        }
    ...
    try:
        from rl_curriculum.curriculum261_r15_preflight import (
            write_rehearsal_sealed_preflight_r15,
        )

        att = write_rehearsal_sealed_preflight_r15(
    ...
    try:
        from rl_curriculum.curriculum261_r15_final import (
            execute_final_core_r15,
        )

        final_dir = out_dir / "preplan_final_temp"
        final_dir.mkdir(parents=True, exist_ok=True)
        final_result = execute_final_core_r15(
            final_dir, plan, pack,
        ...
        final_info = {
            ...
            "exposure_terminal_status": final_result.get(
                "exposure_status"),
            "namespaces": ["preplan_final_r15"],
        }
        struct["namespaces_touched"].append("preplan_final_r15")
    except Exception as exc:  # noqa: BLE001
