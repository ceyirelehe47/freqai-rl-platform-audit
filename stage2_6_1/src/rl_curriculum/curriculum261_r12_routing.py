# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R12:main/holdout/final bundle routing 合同(§9)。
...
R12 修复(§9):
- 显式 R12BundleRouting 数据类:role / fit namespace / 三层 bundle hash,
...
- RoutingLedgerR12 落盘完整路由矩阵(§9.4):corpus → expected fit
...
R12_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "preprocess_fit_calibration_r12",
    "holdout": "preprocess_fit_holdout_r12",
    "final": "preprocess_fit_qualification_r12",
}

#: §9.1 preplan/rehearsal 路由(execution profile 只换 namespace)。
R12_PREPLAN_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "preplan_fit_main_r12",
    "holdout": "preplan_fit_holdout_r12",
    "final": "preplan_fit_main_r12",  # rehearsal final-like 复用 main fit
    "diagnostic": "preplan_fit_main_r12",
}

#: repair R12(工作包 C):full-scale shadow rehearsal 路由(工程、
#: 非正式;两次独立 cold run;不产生任何参数选择数据)。
R12_SHADOW_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "shadow_fit_main_r12",
    "holdout": "shadow_fit_holdout_r12",
    "final": "shadow_fit_main_r12",  # final-like shadow 复用 main fit
}

#: 评估 namespace → 期望 role(§9.1;正式 + preplan + shadow)。
R12_EVAL_NAMESPACE_ROLE: dict[str, str] = {
    # ---- 正式 ----
    "calibration_r12": "main",
    "calibration_holdout_r12": "holdout",
    "qualification_r12": "final",
    "c2_independent_calibration_r12": "main",
    "c2_independent_holdout_r12": "holdout",
    "c2_independent_qualification_r12": "final",
    "supervised_main_r12": "main",
    "supervised_holdout_r12": "holdout",
    "cue_semantic_calibration_r12": "main",
    "cue_semantic_holdout_r12": "holdout",
    "cue_semantic_qualification_r12": "final",
    # ---- preplan / rehearsal / diagnostic(非正式)----
    "reference_diagnostic_main_r12": "diagnostic",
    "reference_diagnostic_holdout_r12": "diagnostic",
    "preplan_smoke_r12": "diagnostic",
    "preplan_candidate_eval_r12": "diagnostic",
    "preplan_supervised_main_r12": "main",
    "preplan_supervised_holdout_r12": "holdout",
    "preplan_calibration_main_r12": "main",
    "preplan_calibration_holdout_r12": "holdout",
    "preplan_final_r12": "final",
    # ---- full-scale shadow(工程;R12 工作包 C)----
    "shadow_supervised_main_r12": "main",
    "shadow_supervised_holdout_r12": "holdout",
    "shadow_calibration_main_r12": "main",
    "shadow_calibration_holdout_r12": "holdout",
    "shadow_semantic_main_r12": "main",
    "shadow_semantic_validation_r12": "holdout",
    "shadow_c2_independent_main_r12": "main",
    "shadow_c2_independent_holdout_r12": "holdout",
    "shadow_semantic_final_r12": "final",
}
...
class R12BundleRouting:
    ...
    _v2: Any
    preplan: bool = False
    #: repair R12(工作包 C):shadow 路由类(非正式;工程 rehearsal)
    shadow: bool = False
    ...
    def bundle(self, *, expected_role: str | None = None,
               ...
               context: str = "",
               ledger: "RoutingLedgerR12 | None" = None) -> Any:
...
def build_routing_r12(
        role: str, v2: Any, *,
        preplan: bool = False,
        shadow: bool = False,
        expected_bundle_hash: str | None = None) -> R12BundleRouting:
    ...
    repair R12:shadow=True 走 full-scale shadow 路由表(工程)。
    """
    ...
    table = (R12_SHADOW_ROLE_FIT_NAMESPACE if shadow
             else R12_PREPLAN_ROLE_FIT_NAMESPACE if preplan
             else R12_ROLE_FIT_NAMESPACE)
    ...
    if v2.namespace not in (list(R12_ROLE_FIT_NAMESPACE.values())
                            + list(
                                R12_PREPLAN_ROLE_FIT_NAMESPACE.values())
                            + list(
                                R12_SHADOW_ROLE_FIT_NAMESPACE.values())):
        raise RoutingContractError(
            f"bundle fit namespace {v2.namespace} 不属于任何 R12 路由表"
            f"(fail closed)")
    if expected_bundle_hash is not None \
...
def require_eval_routing_r12(
        routing: R12BundleRouting, eval_namespace: str, *,
        context: str = "",
        ledger: "RoutingLedgerR12 | None" = None) -> Any:
    ...
    拒绝(§9.3)。repair R12:非正式路由类(preplan/shadow)不得服务
    正式评估 namespace;正式路由不得服务任何非正式 namespace。
    """
    _NONFORMAL_PREFIXES = ("preplan_", "reference_diagnostic", "shadow_")
    expected_role = R12_EVAL_NAMESPACE_ROLE.get(eval_namespace)
    if expected_role is None:
        raise RoutingContractError(
            f"评估 namespace {eval_namespace} 不在 R12 权威路由映射中"
            f"(fail closed;context={context})")
    table = (R12_SHADOW_ROLE_FIT_NAMESPACE if routing.shadow
             else R12_PREPLAN_ROLE_FIT_NAMESPACE if routing.preplan
             else R12_ROLE_FIT_NAMESPACE)
...
def bundle_routing_contract_digest() -> str:
    ...
    return "r12rt-" + hashlib.sha256(json.dumps(
...
def bundle_routing_contract_payload() -> dict[str, Any]:
    ...
    return {
        "format": "cur261-r12-bundle-routing-contract-v1",
        "iteration": "r12",
        "formal_role_fit_namespace": dict(R12_ROLE_FIT_NAMESPACE),
        "preplan_role_fit_namespace": dict(R12_PREPLAN_ROLE_FIT_NAMESPACE),
        "eval_namespace_role": dict(R12_EVAL_NAMESPACE_ROLE),
        "fail_closed_rules": [
