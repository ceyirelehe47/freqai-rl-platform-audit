# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R7:256-step PPO plumbing smoke(§30)。

只验证:V2 fit state 加载(ppo_smoke_r7 非 final namespace fit)、
...
§30:model manifest 绑定 V2 bundle hash + R7 parameter pack digest +
...
def run_ppo_smoke_r7(
    ...
    if envelope_path is not None and Path(envelope_path).is_file():
        ...
        preproc, _ = fit_preprocessor_v2_from_bank_r7(
            "ppo_smoke_r7", pack or {"digest": "ppo-smoke-no-pack"})
    ...
    if pack:
        ...
        override = r7_override_for(FAMILY_C2, pack)
    rec = generate_pair(
        FAMILY_C2, "D1", 0, namespace="ppo_smoke_r7",
    ...
    features = select_features_strict(
        scaled_ep.df, schema, context="ppo_smoke_r7")
    ...
    checks = {
        ...
        "r7_parameter_pack_digest_bound": (
    ...
    return {
        "format": "cur261-r7-ppo-256step-smoke-v1",
        "iteration": "r7",
        "namespace": "ppo_smoke_r7",
        "n_steps": 256,
        "model_manifest": {
            "preprocessor_bundle_hash": preproc.bundle_hash,
            "r7_parameter_pack_digest": checks[
                "r7_parameter_pack_digest_bound"],
            ...
            "note": "§30:manifest 绑定 V2 bundle hash + R7 pack digest + "
