# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R16:共享 calibration/final orchestration(§12)。
...
R16 修复(§12.4 共享 Orchestrator):
- 正式 calibrate(main/holdout)与 preplan rehearsal 调用同一
  orchestration 函数 orchestrate_calibration_bundle_r16;
- 唯一差异通过 R16ExecutionProfile 表达:namespace 集合与样本量
...
#: R16 supervised 正式 model seeds(§17:3 个全新 seeds;R16 与 R10
#: 的 20261001-3 不重合)。
R16_SUPERVISED_MODEL_SEEDS: tuple[int, ...] = (
    20270111, 20270112, 20270113)
R16_SUPERVISED_GATE: dict[str, Any] = {
    # repair R16(B2):min_seeds_passing 语义 = 每个 gated control
...
CALIBRATION_PAIRS_PER_RUNG_R16 = 10
C2_INDEPENDENT_PAIRS_PER_RUNG_R16 = 20
SEMANTIC_BLOCKS_PER_CORPUS_R16 = 160
EQUIVALENCE_PAIRS_PER_RUNG_R16 = 3


@dataclass(frozen=True)
class R16ExecutionProfile:
    ...
    artifact_suffix: str = ""
    #: repair R16(工作包 C):shadow 路由类(工程 rehearsal;非正式)
    shadow: bool = False
    #: repair R16(工作包 C):supervised 训练配置(仅允许缩短纯训练
...
def formal_main_profile_r16(n_blocks: int) -> R16ExecutionProfile:
    return R16ExecutionProfile(
        name="formal_main", preplan=False,
        c13_eval_namespace="calibration_r16",
        equivalence_namespace="calibration_r16",
        supervised_namespace="supervised_main_r16",
        semantic_namespace="cue_semantic_calibration_r16",
        c2_matched_namespace="calibration_r16",
        c2_independent_namespace="c2_independent_calibration_r16",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R16,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R16,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R16,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R16,
        supervised_train_pair_limit=6,
        supervised_model_seeds=R16_SUPERVISED_MODEL_SEEDS,
        equivalence_pairs_per_rung=EQUIVALENCE_PAIRS_PER_RUNG_R16,
    )


def formal_holdout_profile_r16(n_blocks: int) -> R16ExecutionProfile:
    return R16ExecutionProfile(
        name="formal_holdout", preplan=False,
        c13_eval_namespace="calibration_holdout_r16",
        equivalence_namespace="calibration_holdout_r16",
        supervised_namespace="supervised_holdout_r16",
        semantic_namespace="cue_semantic_holdout_r16",
        c2_matched_namespace="calibration_holdout_r16",
        c2_independent_namespace="c2_independent_holdout_r16",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R16,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R16,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R16,
...
def rehearsal_main_profile_r16() -> R16ExecutionProfile:
    return R16ExecutionProfile(
        name="rehearsal_main", preplan=True,
        c13_eval_namespace="preplan_calibration_main_r16",
        equivalence_namespace="preplan_calibration_main_r16",
        supervised_namespace="preplan_supervised_main_r16",
        semantic_namespace="preplan_semantic_main_r16",
        c2_matched_namespace="preplan_calibration_main_r16",
        c2_independent_namespace="preplan_calibration_main_r16",
...
def rehearsal_holdout_profile_r16() -> R16ExecutionProfile:
    return R16ExecutionProfile(
        name="rehearsal_holdout", preplan=True,
        c13_eval_namespace="preplan_calibration_holdout_r16",
        equivalence_namespace="preplan_calibration_holdout_r16",
        supervised_namespace="preplan_supervised_holdout_r16",
        semantic_namespace="preplan_semantic_validation_r16",
        c2_matched_namespace="preplan_calibration_holdout_r16",
        c2_independent_namespace="preplan_calibration_holdout_r16",
        c13_pairs_per_rung=1,
        c2_blocks=2,
        semantic_blocks=4,
        c2_independent_pairs_per_rung=2,
...
#: 最大值 —— 无论 R16 机械选出哪个 n,shadow 的生成坐标都是其超集
#: (block index 0..19 覆盖 0..14 与 0..9)。
SHADOW_C2_BLOCKS_R16 = 20
...
def shadow_main_profile_r16() -> R16ExecutionProfile:
    ...
    return R16ExecutionProfile(
        ...
        c13_eval_namespace="shadow_calibration_main_r16",
        equivalence_namespace="shadow_calibration_main_r16",
        supervised_namespace="shadow_supervised_main_r16",
        semantic_namespace="shadow_semantic_main_r16",
        c2_matched_namespace="shadow_calibration_main_r16",
        c2_independent_namespace="shadow_c2_independent_main_r16",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R16,
        c2_blocks=SHADOW_C2_BLOCKS_R16,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R16,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R16,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R16,
        supervised_train_pair_limit=6,
...
def shadow_holdout_profile_r16() -> R16ExecutionProfile:
    """full-scale shadow holdout(与 shadow_main 对称;独立 namespace)。"""
    return R16ExecutionProfile(
        name="shadow_holdout", preplan=False, shadow=True,
        c13_eval_namespace="shadow_calibration_holdout_r16",
        equivalence_namespace="shadow_calibration_holdout_r16",
        supervised_namespace="shadow_supervised_holdout_r16",
        semantic_namespace="shadow_semantic_validation_r16",
        c2_matched_namespace="shadow_calibration_holdout_r16",
        c2_independent_namespace="shadow_c2_independent_holdout_r16",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R16,
        c2_blocks=SHADOW_C2_BLOCKS_R16,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R16,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R16,
...
# --------------------------------- R16RealArtifactCliRoundTrip-v1 profiles
#: rt(round-trip rehearsal)profiles:正式生成基数 + rt3_*_r16
...
#: 为任何机械选择结果的超集;与 SHADOW_C2_BLOCKS_R16 同理)。
RT_C2_BLOCKS_R16 = 20
...
#: CALIBRATION_PAIRS_PER_RUNG_R16=10 冻结不变。
RT_C13_PAIRS_PER_RUNG = 60


def rt_main_profile_r16() -> R16ExecutionProfile:
    """round-trip rehearsal main:正式生成规模 + rt namespace + 落盘。"""
    return R16ExecutionProfile(
        name="rt_main", preplan=False, shadow=False,
        c13_eval_namespace="rt3_calibration_main_r16",
        equivalence_namespace="rt3_calibration_main_r16",
        supervised_namespace="rt3_supervised_main_r16",
        semantic_namespace="rt3_semantic_main_r16",
        c2_matched_namespace="rt3_calibration_main_r16",
        c2_independent_namespace="rt3_c2_independent_main_r16",
        c13_pairs_per_rung=RT_C13_PAIRS_PER_RUNG,
        c2_blocks=RT_C2_BLOCKS_R16,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R16,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R16,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R16,
        supervised_train_pair_limit=6,
...
def rt_holdout_profile_r16() -> R16ExecutionProfile:
    """round-trip rehearsal holdout(与 rt_main 对称;独立 namespace)。"""
    return R16ExecutionProfile(
        name="rt_holdout", preplan=False, shadow=False,
        c13_eval_namespace="rt3_calibration_holdout_r16",
        equivalence_namespace="rt3_calibration_holdout_r16",
        supervised_namespace="rt3_supervised_holdout_r16",
        semantic_namespace="rt3_semantic_validation_r16",
        c2_matched_namespace="rt3_calibration_holdout_r16",
        c2_independent_namespace="rt3_c2_independent_holdout_r16",
        c13_pairs_per_rung=RT_C13_PAIRS_PER_RUNG,
        c2_blocks=RT_C2_BLOCKS_R16,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R16,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R16,
...
def preprocessing_robustness_checks_r16(
        routing_main: R16BundleRouting,
        routing_holdout: R16BundleRouting,
        ...
        pack: dict[str, Any], *,
        profile: R16ExecutionProfile,
        ledger: RoutingLedgerR16,
        profile_holdout: R16ExecutionProfile | None = None) -> dict[str, Any]:
    """§18 Preprocessing V2 重新资格审查(R16 版;结构承 R6 电池)。
    ...
    space_validation = validate_observation_space_v2(
        ...
        context="preprocessing_robustness_r16")
    ...
    return {
        "format": "cur261-r16-preprocessing-robustness-v1",
        "iteration": "r16",
...
def run_full_supervised_release_rehearsal(out_dir: Path) -> dict[str, Any]:
    ...
    训练面:
    - 正式 3 个 model seeds(R16_SUPERVISED_MODEL_SEEDS);
    ...
    参数。使用 preplan namespace(preplan_fit_main_r16 /
    preplan_supervised_main_r16 等)。
    """
    ...
    from rl_curriculum.curriculum261_r16_shadow import _shadow_pack

    out_dir = Path(out_dir)
    pack = _shadow_pack()
    override_fn = lambda family, p: r16_override_for(family, p)  # noqa: E731
    roles: dict[str, Any] = {}
    any_nan_inf = False
    alignment_failures: list[str] = []
    for role, fit_ns, sup_ns in (
            ("main", "preplan_fit_main_r16", "preplan_supervised_main_r16"),
            ("holdout", "preplan_fit_holdout_r16",
             "preplan_supervised_holdout_r16")):
        records = generate_fit_bank_r16(fit_ns, pack, pairs_per_rung=2)
        bundle, manifest = fit_preprocessor_v2_from_bank_r16(
            fit_ns, pack, records=records, pairs_per_rung=2,
            parameter_pack_identity=str(pack.get("digest", "")))
        sup = supervised_learnability_run_r16(
            bundle, pack, namespace=sup_ns,
            pairs_per_rung=10, train_pair_limit=6,
            model_seeds=R16_SUPERVISED_MODEL_SEEDS)
        ...
        roles[role] = {
            ...
            "model_seeds": list(R16_SUPERVISED_MODEL_SEEDS),
        ...
        (out_dir / f"full_supervised_rehearsal_{role}.json").write_text(
            ...
            encoding="utf-8")
    result = {
        "format": "cur261-r16-full-supervised-release-rehearsal-v1",
        "development_evidence_only": True,
        "not_used_for_parameter_selection": True,
        "formal_model_seeds": list(R16_SUPERVISED_MODEL_SEEDS),
        "formal_gate_constants": R16_SUPERVISED_GATE,
        "roles": roles,
...
def orchestrate_calibration_stage_r16(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R16BundleRouting,
        routing_holdout: R16BundleRouting,
        records_main: list, records_holdout: list,
        profile_main: R16ExecutionProfile,
        profile_holdout: R16ExecutionProfile,
        override_fn: Any,
    ...
    repair R16(工作包 A):整个编排期间打开 generation invocation
    ...
    with sink:
        return _orchestrate_calibration_stage_inner_r16(
...
def _orchestrate_calibration_stage_inner_r16(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R16BundleRouting,
        routing_holdout: R16BundleRouting,
        ...
        design_digest: str | None = None,
        write_artifacts: bool = True) -> dict[str, Any]:
    """orchestrate_calibration_stage_r16 的执行核心(envelope sink 内)。"""
    from rl_curriculum.curriculum261_r16_calibration import (
        ...
        run_calibration_corpus_c13_r16,
        supervised_learnability_run_r16,
    )
    ...
    ledger = RoutingLedgerR16()
    result: dict[str, Any] = {
        "format": "cur261-r16-calibration-orchestration-v1",
        "iteration": "r16",
    ...
    prep_rob = preprocessing_robustness_checks_r16(
    ...
    _write("bundle_routing_validation.json", {
        "contract": "cur261-r16-bundle-routing-contract-v1",
    ...
    for role, routing, profile in (
        ...
        v2_sup = require_eval_routing_r16(
            routing, profile.supervised_namespace,
            context=f"supervised_{role}", ledger=ledger)
        supervised[role] = supervised_learnability_run_r16(
            v2_sup, pack,
    ...
    for role, routing, profile in (
        ...
        c13 = run_calibration_corpus_c13_r16(
        ...
        semantic = run_c2_semantic_corpus_r16(
            ...
            artifact_name=semantic_artifact_filename_r16(
                profile.semantic_namespace))
        v2_i = require_eval_routing_r16(
        ...
        marginal = c2_independent_marginal_guard_r16(
            c2_indep, pack, recall_floor_value)
        matched_conditions = c2_matched_conditions_r16(c2_matched, pack)
    ...
    from rl_curriculum.curriculum261_r16_generation_evidence import (
        ExpectedCall,
