# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R14:一次性 final qualification(§27-§29)。
...
9. marker -> completed;任何异常 -> marker -> crashed + crash evidence
   落盘 + re-raise(保留 marker,禁止修复后复用 qualification_r14)。
...
from rl_curriculum.curriculum261_r14_calibration import (
    C2_INDEPENDENT_PAIRS_PER_RUNG_R14,
    SEMANTIC_BLOCKS_PER_CORPUS_R14,
...
FINAL_FORMAT_R14 = "cur261-r14-final-qualification-v1"
...
from rl_curriculum.curriculum261_r14_reference import (
    ...
    write_reference_equivalence_artifacts_r14,
)
from rl_curriculum.curriculum261_r14_orchestrator import (
    CALIBRATION_PAIRS_PER_RUNG_R14,
    R14_SUPERVISED_MODEL_SEEDS,
)


def qualification_r14_lock_dir_default() -> Path:
    from rl_curriculum.curriculum261_r14_namespaces import (
...
def _fresh_seed_validity_r14(n_checks: int = 10) -> dict[str, Any]:
    """fresh_holdout_r14 与 qualification_r14 的 seed 不相交验证。"""
    from rl_curriculum.curriculum261_api import derive261_seed

    mismatches = []
    for i in range(n_checks):
        for family in CURRICULUM261_FAMILIES:
            q = derive261_seed("qualification_r14", family, "D2", i, 0)
            for rung in CURRICULUM261_RUNGS:
                f = derive261_seed("fresh_holdout_r14", family, rung, i, 0)
                if q == f:
...
def run_final_qualification_r14(out_dir: Path,
                               vendor_dir: Path | None = None,
                               *,
                               rehearsal_profile: dict[str, Any] | None
                               = None,
                               ) -> dict[str, Any]:
    """执行一次性 R14 final qualification(§28)。

    rehearsal_profile(R14RealArtifactCliRoundTrip-v1;§四-4):非 None
    ...
    与样本量来自 rehearsal_profile(rt_*_r14 rehearsal-only namespace
    + 缩小规模;verdict 在缩小规模下不作资格判定,artifact 写盘供
    PPO smoke / 下游 reader 真实读取)。exposure marker 写入
    rehearsal 隔离目录(CURRICULUM261_R14_LOCK_DIR 指向 rehearsal
    目录;正式 namespace 不消耗)。
    """
    rt = rehearsal_profile is not None
    require_r14_iteration_active()
    if qualification_r14_exposed():
        raise RuntimeError(
            "R14 final qualification 已执行过(exposure marker/ledger "
            "存在)——同一 qualification_r14 corpus 不得再次执行;继续必须"
            "使用新迭代身份(R14)与全新 seed space")
    plan, digest = load_locked_plan_r14()
    if (plan.get("robustness_gate") or {}).get("pass") is not True:
        raise RuntimeError(
            "R14 plan 的 robustness_gate.pass != true——final "
            "qualification fail closed")
    pack = load_selected_pack(qualification_r14_lock_dir_default())
    ...
    code_ok = (plan["code_identity"] == _code_identity_r14())
    ...
    if not static_ok:
        raise RuntimeError(
            ...
            "之前 fail closed——本轮 qualification_r14 未消耗")
    ...
    if not freeze["pass"]:
        raise RuntimeError(
            f"R14 code freeze 校验失败(正式数据开始后源码漂移;"
            f"§6/§21 永久结束):{freeze}")
    ...
    if rt:
        ...
        core_kwargs = dict(
            ...
            final_namespace="qualification_r14",
            fit_namespace="preprocess_fit_qualification_r14",
    ...
    try:
        with QualificationR14FileLock(blocking=False):
            write_qualification_r14_exposure(digest, status="running")
            result = execute_final_core_r14(
        ...
        try:
            (out_dir / "qualification_crash_traceback.log").write_text(
                exc, encoding="utf-8")
            write_qualification_r14_exposure(digest, status="crashed")
        except Exception:  # noqa: BLE001 - 崩溃路径不得掩盖原始异常
            pass
        raise


def execute_final_core_r14(
        out_dir: Path, plan: dict[str, Any], pack: dict[str, Any], *,
        digest: str = "", started: str | None = None,
        profile_name: str = "formal_final",
        final_namespace: str = "qualification_r14",
        fit_namespace: str = "preprocess_fit_qualification_r14",
    ...
    正式:run_final_qualification_r14(exposure/plan/freeze 治理外壳)
    以 qualification_r14 全量参数调用;
    rehearsal:preplan 以 preplan_final_r14 + 微样本参数 + 临时
    exposure 目录调用(§12.2 final-like runner);
    shadow:full-scale shadow(工作包 C)以 shadow_fit_main_r14 +
    ...
    *_override 参数仅供 shadow(工程覆盖;正式路径不传,保持
    ...
    - reference equivalence 走 canonical 合同(reference_equivalence_
      run_r14:canonical vs scaled 逐位 + legacy 差异 + 0 unexplained),
    ...
    with sink:
        return _execute_final_core_inner_r14(
            out_dir, plan, pack, digest=digest, started=started,
...
def _build_gate_evidence_r14(
    ...
    gates["c2_matched_cue_point_diagnostics"] = {
        ...
        "note": "诊断失败不得改变 R14 verdict(R14 §四-3)",
    }
    failed_binding = sorted(
        k for k, v in checks.items()
        if isinstance(v, bool) and not v
        and r14_binding_status(k)["binding"])
    return {
        ...
        "post_exposure_policy": (
            ...
            "qualification_r14 语料(api 守卫 fail closed)"),
    }


def _execute_final_core_inner_r14(
        out_dir: Path, plan: dict[str, Any], pack: dict[str, Any], *,
        digest: str = "", started: str | None = None,
        profile_name: str = "formal_final",
        final_namespace: str = "qualification_r14",
        ...
        conditioning_fit_namespace: str | None = None,
        semantic_out_dir: Path | None = None) -> dict[str, Any]:
    """execute_final_core_r14 的执行核心(envelope sink 内)。"""
    kappa = float(plan["statistics_rule"]["kappa"]) \
        if "statistics_rule" in plan else 1.5
    if c2_blocks is None:
        c2_blocks = int(plan["final_sample_counts"]["c2_matched_blocks"])
    ...
    final_records = generate_fit_bank_r14(fit_namespace, pack)
    final_v2, final_manifest = fit_preprocessor_v2_from_bank_r14(
        fit_namespace, pack, records=final_records,
        parameter_pack_identity=pack["digest"])
    routing_final = build_routing_r14(
    ...
    space_validation = validate_observation_space_v2(
        ...
        [int(r.episodes[s].spec.seed) for r in contain_records
         for s in ("A", "B")], context="final_fit_space_v2_r14")
    ...
    for family in CURRICULUM261_FAMILIES:
        if family == FAMILY_C2:
            continue
        override = r14_override_for(family, pack)
        records = []
        for rung in CURRICULUM261_RUNGS:
        ...
        family_reports[family] = rung_report_r4(
            records, family, r14_family_rung_params(family, pack),
    ...
    c2_indep = run_c2_independent_corpus_r14(
        ...
        or ("c2_independent_qualification_r14" if not rehearsal
            else final_namespace),
        pairs_per_rung=int(independent_pairs_per_rung))
    c2_conditions = c2_matched_conditions_r14(c2_matched, pack)
    c2_marginal = c2_independent_marginal_guard_r14(
        c2_indep, pack, recall_floor_value)
    semantic_ns = semantic_namespace_override or (
        "cue_semantic_qualification_r14" if not rehearsal
        else "preplan_semantic_main_r14")
    ...
    semantic = run_c2_semantic_corpus_r14(
        ...
        artifact_name=semantic_artifact_filename_r14(semantic_ns))
    ...
    reference_report = reference_equivalence_run_r14(
    ...
    if not rehearsal or shadow or rt:
        causality = {
            ...
            "reference_causality": [
                check_reference_causality(
                    f, r14_family_rung_params(f, pack)["D2"],
                    thresholds[f])
                for f in CURRICULUM261_FAMILIES],
        }
    repro = []
    if not rehearsal or shadow or rt:
        ...
        repro_pack_override = []
        for family in CURRICULUM261_FAMILIES:
            ov = r14_override_for(family, pack) or {}
        ...
        c2_ladder = r14_family_rung_params(FAMILY_C2, pack)
    ...
    fresh = (_fresh_seed_validity_r14(10)
    ...
    cond_records = generate_fit_bank_r14(
        conditioning_fit_namespace or "preprocess_fit_calibration_r14",
    ...
    supervised_ns = supervised_namespace_override or (
        ...
        else "preplan_supervised_main_r14")
    supervised = supervised_learnability_run_r14(
        final_v2, pack, namespace=supervised_ns,
        pairs_per_rung=(supervised_pairs_per_rung_override
                        or (CALIBRATION_PAIRS_PER_RUNG_R14
    ...
    semantics = run_c2_diagnostics_r14(
        [rec for blk in c2_matched["blocks"]
         for rec in blk.pair_records.values()])
    scrambled = scrambled_gap_control(c2_matched["block_table"])

    # R14 gate topology(GateTopologyReconciliation-v1):matched corpus
    ...
    matched_point_diag = {
        ...
        "note": ("R6 点估计分离检查继续执行并报告;诊断失败不得改变 "
                 "R14 verdict(R14 §四-3;R13 曾把该项绑定为 "
    ...
    topology_digest_ok = bool(
        plan_topology_digest == r14_gate_topology_digest())
    ...
    checks: dict[str, Any] = {
        ...
        # R14 拆分:R13 的 c2_semantics_pass(三诊断 AND,含点估计
    ...
    result = {
        ...
        # R14 gate topology 声明(单一权威注册表来源)
        "gate_topology": {
            "version": R14_GATE_TOPOLOGY_VERSION,
            "digest": r14_gate_topology_digest(),
            ...
            "binding_checks": sorted(
                ...
                and r14_binding_status(k)["binding"]),
            "diagnostic_only_checks": ["c2_matched_cue_point_diagnostics"],
            "note": ("binding status 唯一来源 = "
                     "curriculum261_r14_gate_topology.R14_GATE_REGISTRY;"
                     "本 artifact 的每个 check 的 binding 状态见 "
                     "gate_evidence"),
        },
        # R14 §八:matched corpus 点估计诊断(diagnostic_only;
        # 诊断失败不改变 verdict——R13 败于此处)
        "c2_matched_cue_point_diagnostics": matched_point_diag,
        # R14 §八:每个 gate 的详细证据(一次性保存;final FAIL 后
        # 诊断只读本 artifact 与 qualification_raw.json,不得重生成)
        "gate_evidence": _build_gate_evidence_r14(
    ...
    # R14 §三精神:单一 canonical 键 preprocessor_bundle_hash(R13 的
