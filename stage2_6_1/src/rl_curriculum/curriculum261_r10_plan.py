# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R10:final qualification plan(§26)。

双 strict gate(calibration + holdout 各自独立 PASS;无 pooled 救援)
前置强制;plan 绑定 R10 pack/cue semantic 合同/p_contract/recall
...
from rl_curriculum.curriculum261_r10_namespaces import (
    CURRICULUM261_ITERATION_ID_R10,
    qualification_r10_digest_path,
    qualification_r10_plan_path,
)

PLAN_FORMAT_R10 = "cur261-r10-qualification-plan-v1"

#: R10 代码身份(逐模块内容哈希;进入 plan;final 复算比对)。
PLAN_CODE_MODULES_R10 = (
...
def _code_identity_r10() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    for name in PLAN_CODE_MODULES_R10:
        f = root / name
...
def build_plan_r10(
    ...
    plan: dict[str, Any] = {
        ...
        "code_freeze": {
            ...
            "contract": ("正式 R10 数据开始前全部源码+测试已提交;"
                         "正式数据开始后任何源码变化 -> R10 永久结束"
                         "(§6/§21)"),
        },
        ...
        "preprocessing_v2": {
            ...
            "implementation": "pinned vendor IFreqaiModel."
                              "define_data_pipeline(直接复用;R10 全新"
                              "语料重新资格验证)",
        ...
        "fit_bank_schedule": {
            "calibration": {
                "namespace": "preprocess_fit_calibration_r10",
                "pairs_per_rung": 4, "sides": ["A", "B"],
                "role": "main fit corpus(preprocessor only)"},
            "holdout": {
                "namespace": "preprocess_fit_holdout_r10",
                "pairs_per_rung": 4, "sides": ["A", "B"],
                "role": "independent fit holdout(preprocessor only)"},
            "final": {
                "namespace": "preprocess_fit_qualification_r10",
                "pairs_per_rung": 4, "sides": ["A", "B"],
        ...
        "semantic_corpus_schedule": {
            "design": {
                "namespaces": ["cue_semantic_design_main_r10",
                               "cue_semantic_design_validation_r10"],
                "blocks_per_corpus": 160,
                "ladder": "sentinel(candidate-independent)"},
            "calibration": {
                "namespace": "cue_semantic_calibration_r10",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "holdout": {
                "namespace": "cue_semantic_holdout_r10",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "final": {
                "namespace": "cue_semantic_qualification_r10",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
        },
        "final_sample_counts": {
        ...
        "qualification_bank_schedule": {
            "namespace": "qualification_r10",
            "c2_independent_marginal_namespace":
                "c2_independent_qualification_r10",
            "cue_semantic_namespace": "cue_semantic_qualification_r10",
            "fresh_seed_namespace": "fresh_holdout_r10",
        },
        ...
        "code_identity": _code_identity_r10(),
    }
    return plan


def plan_digest_r10(plan: dict[str, Any]) -> str:
...
def lock_plan_r10(plan: dict[str, Any]) -> tuple[Path, str]:
    ...
    path = qualification_r10_plan_path()
    if path.is_file():
        raise RuntimeError(
            "R10 qualification plan 已存在;plan lock 后禁止重写(§16.2/"
            "§26:任何修改须新 iteration R10)")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(plan, indent=2, ensure_ascii=False,
                               default=str), encoding="utf-8")
    digest = plan_digest_r10(plan)
    dpath = qualification_r10_digest_path()
    dpath.write_text(digest, encoding="utf-8")
    return path, digest


def load_locked_plan_r10() -> tuple[dict[str, Any], str]:
    path = qualification_r10_plan_path()
    dpath = qualification_r10_digest_path()
    if not path.is_file() or not dpath.is_file():
        raise RuntimeError("R10 qualification plan 未锁定")
    plan = json.loads(path.read_text(encoding="utf-8"))
    digest = plan_digest_r10(plan)
    if dpath.read_text(encoding="utf-8").strip() != digest:
        raise RuntimeError("R10 plan digest 复算不一致(fail closed)")
    return plan, digest


# ---------------------------------------------------- rehearsal plan(§12)
def build_rehearsal_qualification_plan_r10(
    ...
    绕过正式 build_plan_r10 的双 gate 断言(rehearsal 的小样本 gate
    结果不作资格判定),绑定 rehearsal digest 与共享 orchestrator
    身份;正式 plan 仍走 build_plan_r10 + lock_plan_r10。
    """
    plan = {
        "format": "cur261-r10-rehearsal-qualification-plan-v1",
        "iteration": "r10",
        "rehearsal": True,
    ...
    return plan, plan_digest_r10(plan)


def lock_qualification_plan_r10(
    ...
    path = lock_dir / "qualification_plan_r10.json"
    digest = plan_digest_r10(plan)
    plan = dict(plan)
    ...
    try:
        ...
        os.close(fd)
    (lock_dir / "qualification_plan_digest_r10.txt").write_text(
        digest, encoding="utf-8")
    return path, digest


def load_locked_qualification_plan_r10(
