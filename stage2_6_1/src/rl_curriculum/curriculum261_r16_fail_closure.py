# -*- coding: utf-8 -*-
"""R16 阶段精确 failure closure(§8.4 + §5.4)。
...
- qualification 三细分 phase 从权威 journal
  (curriculum261_r16_execgov.exposure_state)机械判定,marker 只是
...
#: fail closure 证据清单(R16 文件名布局)。
R16_EVIDENCE_KEYS: tuple[tuple[str, str], ...] = (
    ("design_plan_state", "r16_design_plan.json"),
    ("design_data_state", "r16_parameter_pack.json"),
    ("calibration_state", "calibration_evidence.json"),
    ("qualification_plan", "qualification_plan_r16.json"),
    ("sealed_preflight", "sealed_final_preflight_r16.json"),
...
def qualification_failure_subphase_r16(
...
def build_phase_accurate_fail_closure_r16(
    ...
    if failed_step == "bootstrap":
        ...
        step = R16_WORKFLOW_STEPS[idx]
        base_phase = step["failure_phase"]
    if base_phase == "qualification":
        phase, journal_ok = qualification_failure_subphase_r16(
        ...
        journal_ok = True
        phase = base_phase
    producer = r16_producer_of_artifact()
    names = list(r16_workflow_step_names())
    ...
    def evidence_state(artifact: str) -> str:
        ...
        if prod is None:
            return NOT_EXPECTED
        if rehearsal and artifact == "r16_report_values.json":
            return NOT_EXPECTED
    ...
    evidence: dict[str, str] = {
        key: evidence_state(art) for key, art in R16_EVIDENCE_KEYS}
    ...
    if failed_step == "bootstrap" or idx < names.index("qualify"):
        ...
        sub, _ = qualification_failure_subphase_r16(
            out_dir, rehearsal=rehearsal)
        exposure_state_val = {
    ...
    return {
        "format": "cur261-r16-fail-closure-v1",
        "iteration": "r16",
...
#: 机械转换后的 r16_cli 使用的名称别名(同一实现)。
build_phase_accurate_fail_closure = build_phase_accurate_fail_closure_r16
qualification_failure_subphase = qualification_failure_subphase_r16
