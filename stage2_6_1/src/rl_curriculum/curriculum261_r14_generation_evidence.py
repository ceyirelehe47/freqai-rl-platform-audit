# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R14:GenerationEvidenceCompleteness-v1(§15)。
...
治理层(R14 新增,orchestrator 重新计算,不信 recorder 自报):
...
COMPLETENESS_FORMAT = "cur261-r14-generation-evidence-completeness-v1"
R14_ITERATION = "r14"
...
def verify_generation_evidence_completeness(
    ...
    for r in rows:
        ...
        if env.get("iteration") != R14_ITERATION:
    ...
    if iteration_mismatch:
        problems.append(
            f"{iteration_mismatch} 行 iteration != r14(混入历史迭代)")
    # ---- 逐 call 完整性 + 坐标计数 ----
...
def generation_evidence_completeness_digest(result: dict[str, Any]) -> str:
    ...
    return "r14gec-" + hashlib.sha256(json.dumps(
