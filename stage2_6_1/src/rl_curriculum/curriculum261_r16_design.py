# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R16:clean matched-ladder design(dedicated semantic
corpus 先行 + corrected plan digest roundtrip + §21/§22 流程 + §18
一次性硬规则)。

R16 相对 R8 的工程修复(规范 §5/§6/§8;统计合同零修改):
...
- R7 design plan 的 digest 自引用缺陷:lock 把 design_plan_digest
  ...
  evidence)。R16 修复(§8):digest 复算排除 (locked_utc,
  design_plan_digest) 双字段;payload 自带 digest 同时可经正式
  load_locked_design_plan_r16 复算——正式 lock/load/digest recompute/
...
- R7 shared cue gate 直接用 40-block candidate design corpus,SE
  ...
  floor 0.930)。R16(§14/§15)把 candidate-independent cue 语义拆到
  预注册 160-block dedicated semantic corpus(每 corpus 独立 gate,
  FAIL 即 R16 design FAIL,不进入 candidate 设计);
- R7 的 R6 marginal-guard semantics 键错配(cue_semantics vs R6 冻结
  实现要求的 cue_payoff_separation;R7 calibration 未执行故未暴露)
  ——R16 显式组装:R6 统计条件 + 本模块语义条件(local cue ∧
  context ∧ v2 independent cue semantics)。

§8.4 一次性硬规则:正式 R16 design plan 锁定后发生任何 loader/代码
缺陷 => write_r16_iteration_aborted + R16 永久结束(保留 plan、不重锁、
不复用 namespace、下一轮 R16);run_design_stage_r16 内部任何
...
§7 要求:R16 在 plan lock 前验证这些模块与 R7 baseline 的 SHA256
...
from rl_curriculum.curriculum261_r16_param_pack import (
    ...
    R16_PACK_VERSION,
    ladder_pack_payload_r16,
    ladder_distance_from_historical_r16,
    load_selected_pack,
    pack_digest_r16,
    r16_candidate_grid,
    validate_r16_grid_semantics,
    write_selected_pack_r16,
)

DESIGN_FORMAT_R16 = "cur261-r16-design-plan-v1"
DESIGN_BLOCKS_PER_CORPUS_R16 = 40
DESIGN_NAMESPACES_R16 = ("design_r16_matched_main",
                        "design_r16_matched_validation")
DESIGN_INDEPENDENT_NAMESPACE_R16 = "design_r16_independent_marginal"
DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R16 = 20

#: §14/§15 dedicated semantic corpus(candidate-independent;在任何
#: R16 design data 生成前锁定;数据后禁止 160→240 扩样/第三 corpus/
#: 合并/删除)。
SEMANTIC_BLOCKS_PER_CORPUS_R16 = 160
SEMANTIC_NAMESPACES_R16 = ("cue_semantic_design_main_r16",
                          "cue_semantic_design_validation_r16")

#: §R16-8 semantic artifact 显式穷尽映射(禁止 endswith/后缀启发式;
#: R8 的 endswith("main") 使 main artifact 被 validation 覆盖)。
SEMANTIC_ARTIFACT_MAP_R16 = {
    "cue_semantic_design_main_r16": "semantic_design_main.json",
    "cue_semantic_design_validation_r16":
        "semantic_design_validation.json",
}
#: calibration/holdout/final 三阶段的 semantic artifact 显式映射。
SEMANTIC_STAGE_ARTIFACT_MAP_R16 = {
    "cue_semantic_calibration_r16": "cue_semantic_calibration.json",
    "cue_semantic_holdout_r16": "cue_semantic_holdout.json",
    "cue_semantic_qualification_r16":
        "qualification_cue_semantics.json",
    # preplan full pipeline rehearsal(§12;非正式;同一 writer)
    "preplan_semantic_main_r16": "preplan_semantic_main.json",
    "preplan_semantic_validation_r16":
        "preplan_semantic_validation.json",
    # full-scale shadow(工程;R16 工作包 C)
    "shadow_semantic_main_r16": "shadow_semantic_main.json",
    "shadow_semantic_validation_r16": "shadow_semantic_validation.json",
    "shadow_semantic_final_r16": "shadow_semantic_final.json",
    # R16RealArtifactCliRoundTrip-v1(rehearsal-only;§四-4)
    "rt_semantic_design_main_r16": "rt_semantic_design_main.json",
    "rt_semantic_design_validation_r16":
        "rt_semantic_design_validation.json",
    "rt3_semantic_main_r16": "rt_semantic_main.json",
    "rt3_semantic_validation_r16": "rt_semantic_validation.json",
    "rt3_semantic_final_r16": "rt_semantic_final.json",
}
SEMANTIC_CORPUS_ROLE_R16 = {
    "cue_semantic_design_main_r16": "main",
    "cue_semantic_design_validation_r16": "validation",
    "cue_semantic_calibration_r16": "calibration",
    "cue_semantic_holdout_r16": "holdout",
    "cue_semantic_qualification_r16": "qualification",
    "preplan_semantic_main_r16": "preplan_main",
    "preplan_semantic_validation_r16": "preplan_validation",
    "shadow_semantic_main_r16": "shadow_main",
    "shadow_semantic_validation_r16": "shadow_validation",
    "shadow_semantic_final_r16": "shadow_final",
    "rt_semantic_design_main_r16": "rt_design_main",
    "rt_semantic_design_validation_r16": "rt_design_validation",
    "rt3_semantic_main_r16": "rt_main",
    "rt3_semantic_validation_r16": "rt_validation",
    "rt3_semantic_final_r16": "rt_final",
}


def semantic_artifact_filename_r16(namespace: str) -> str:
    """§R16-8:namespace → 文件名的唯一合法路径(穷尽映射)。"""
    for table in (SEMANTIC_ARTIFACT_MAP_R16,
                  SEMANTIC_STAGE_ARTIFACT_MAP_R16):
        if namespace in table:
            return table[namespace]
    known = sorted(set(SEMANTIC_ARTIFACT_MAP_R16)
                   | set(SEMANTIC_STAGE_ARTIFACT_MAP_R16))
    raise RuntimeError(
        f"semantic namespace {namespace} 不在显式映射表(§R16-8:禁止 "
        f"endswith/后缀启发式;已知 namespace: {known})")


def semantic_artifact_identity_r16() -> dict[str, Any]:
    """§R16-8 artifact writer 身份(进入 design plan code identity)。"""
    return {
        "design_mapping": dict(SEMANTIC_ARTIFACT_MAP_R16),
        "stage_mapping": dict(SEMANTIC_STAGE_ARTIFACT_MAP_R16),
        "roles": dict(SEMANTIC_CORPUS_ROLE_R16),
...
def write_semantic_artifact_r16(out_dir: Path, namespace: str,
                               payload: dict[str, Any],
                               plan_digest: str,
                               event_rows: list[dict[str, Any]] | None
                               = None,
                               ) -> Path:
    """§R16-8:exclusive-create 写 semantic artifact(json + jsonl),
    ...
    fname = semantic_artifact_filename_r16(namespace)
    path = out_dir / fname
    body = dict(payload)
    body["namespace"] = namespace
    body["corpus_role"] = SEMANTIC_CORPUS_ROLE_R16[namespace]
    ...
    if (back.get("namespace") != namespace
            ...
            != SEMANTIC_CORPUS_ROLE_R16[namespace]):
        raise RuntimeError(
...
#: §16.1/§21 code identity 覆盖面(R16 全部实现 + 冻结复用模块 +
#: 合同依赖;design data 生成开始后任何漂移 => iteration aborted)。
DESIGN_CODE_MODULES_R16 = (
...
def _code_identity_design() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    for name in DESIGN_CODE_MODULES_R16:
        f = root / name
...
def verify_matched_core_identity_r10() -> dict[str, Any]:
    ...
    R10 的 design plan 在 lock 时记录了全部依赖模块的 sha256;R16
    不修改 c2 generator / matched tape / matched pairs 三个模块,
    当前哈希必须与 R10 记录值逐位一致(R16 对 api.py 的改动仅限
    R16 namespace 白名单、解锁守卫与被动 recorder 钩子的追加,
...
# ------------------------------------------------------------- plan
def design_plan_payload_r16(*, baseline_commit: str, vendor_pin: str,
    ...
    """构建并返回 R16 design plan payload(锁定后不得修改任何字段)。
    ...
    namespace/scale 覆盖参数默认为正式值;R16RealArtifactCliRoundTrip
    ...
    """
    grid = r16_candidate_grid()
    problems = validate_r16_grid_semantics()
    if problems:
        raise RuntimeError(f"R16 candidate grid 语义非法: {problems}")
    ...
    return {
        "format": DESIGN_FORMAT_R16,
        "iteration": "r16",
        "rehearsal": bool(rehearsal),
        "rehearsal_contract": (
            "R16RealArtifactCliRoundTrip-v1(rehearsal-only namespace;"
        ...
        "plan_digest_contract": {
            ...
            "roundtrip": "build -> lock(O_CREAT 语义:已存在即拒) -> "
                         "load_locked_design_plan_r16(独立进程) -> "
            ...
            "one_shot_rule": "§8.4:正式 plan 锁定后任何 loader/代码"
                             "缺陷 => R16 永久结束(aborted;不删不重锁;"
                             "不复用 namespace;下一轮 R16)",
        },
        "matched_ladder": {
        ...
        "semantic_corpora": {
            ...
            "ladder": "冻结 sentinel ladder(cur261-c2-v9 默认 D0-D3;"
                      "candidate-independent;与任何 R16 candidate 数值"
                      "无依赖)",
            ...
            "gate": "每 corpus 独立:unique-event coverage ≥3600 / "
                    ...
                    "replay 完整性;任一 FAIL => R16 design FAIL(§15)",
        ...
        "independent_marginal_guard": {
            ...
            "conditions": "marginal ordering / D3 positive / 基线 margins"
                          ...
                          ";matched PASS 不可覆盖 FAIL;失败 → R16 = FAIL",
        ...
        "fail_path": {
            ...
            "on_semantic_gate_fail": "任一 semantic corpus gate FAIL → "
                                     "R16 design FAIL(不生成 candidate "
                                     "blocks;§15/§25)",
            "on_no_qualified_combination": "R16 = FAIL;自动 power/semantic "
...
def design_plan_digest_r16(plan: dict[str, Any]) -> str:
    ...
    return "r16dp-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def lock_design_plan_r16(out_dir: Path, plan: dict[str, Any],
                        ) -> tuple[Path, str]:
    ...
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "r16_design_plan.json"
    if path.exists():
        raise RuntimeError(
            f"R16 design plan 已存在: {path}(§8.4 禁止删除/覆盖/重锁;"
            "重锁必须换新 iteration R16)")
    digest = design_plan_digest_r16(plan)
    plan = dict(plan)
    ...
    (out_dir / "r16_design_plan_digest.txt").write_text(
        digest, encoding="utf-8")
    return path, digest


def load_locked_design_plan_r16(out_dir: Path,
    ...
    path = out_dir / "r16_design_plan.json"
    if not path.is_file():
        raise RuntimeError(f"R16 design plan 未锁定: {path}")
    plan = json.loads(path.read_text(encoding="utf-8"))
    digest = design_plan_digest_r16(plan)
    if plan.get("design_plan_digest") != digest:
        raise RuntimeError("R16 design plan digest 复算不一致(fail closed)")
    digest_path = out_dir / "r16_design_plan_digest.txt"
    if not digest_path.is_file() or \
            digest_path.read_text(encoding="utf-8").strip() != digest:
        raise RuntimeError("R16 design plan digest 文件与 payload 不一致")
    return plan, digest
...
def _evaluate_candidate_matched_r16(
    ...
    # 密度(R6 冻结 gate)+ R16 candidate-specific cue 语义 + 独立性/
    ...
    semantics = {
        ...
        "context_observability": check_c2_context_observability(
            records),
        "candidate_cue_semantics_r16_cluster_aware": cue_sem,
        "cue_payoff_confusion": {
            k: v for k, v in cue_sem["per_rung"]["D0"]["sides"]["A"][
                "confusion"].items()},
        # R6 点阈值分离检查仅作诊断对照(绝不进入 R16 资格判定)
...
def _gap_series(block_table: dict[str, Any], hi: str, lo: str):
    # R16:block_gap_series 模块级解析(§6.2)。
...
def _maximin_score_r16(corpus_results: list[dict[str, Any]],
                      n: int) -> float:
    ...
    for res in corpus_results:
        ...
        for per_rung in res["semantics"][
                "candidate_cue_semantics_r16_cluster_aware"][
                "per_rung"].values():
...
# ------------------------------------------------- 主流程
def run_design_stage_r16(out_dir: Path, plan: dict[str, Any],
                        design_digest: str,
                        baseline_commit: str = "") -> dict[str, Any]:
    """R16 design 主流程:治理检查 → semantic corpora(160×2)+ gate
    (FAIL 即短路)→ 3 candidate × 2 corpus blocks → candidate 评估 →
    机械选择 → marginal guard → pack。

    §8.4:plan 锁定后本函数内部任何异常都转 write_r16_iteration_aborted
    + re-raise(R16 永久结束;不删 plan、不重锁、不复用 namespace)。
    """
    require_r16_iteration_active()
    out_dir = Path(out_dir)
    try:
        return _run_design_stage_inner_r16(
        ...
        write_r16_iteration_aborted(
            ...
            f"{str(exc)[:300]});按 §8.4 R16 永久结束")
        raise


def _run_design_stage_inner_r16(out_dir: Path, plan: dict[str, Any],
                               design_digest: str,
                               baseline_commit: str,
                               ) -> dict[str, Any]:
    ...
    if not identity["pass"]:
        raise RuntimeError(
            f"R16 design plan code identity 与当前代码不一致:"
    ...
    # 第一条 R16 design/semantic episode 生成前记录 started(§8.4)
    mark_design_data_started()
    ...
    if len(set(semantic_hashes)) != len(semantic_hashes):
        raise RuntimeError(
            ...
            "§R16-8 禁止生成 candidate blocks)")
    if not semantic_pass:
        summary = {
            "format": "cur261-r16-design-stage-v1",
            "iteration": "r16",
            ...
            "pass": False,
            "verdict": "R16 design FAIL:dedicated semantic corpus gate "
        ...
        (out_dir / "r16_sample_size_selection.json").write_text(
    ...
    for cand_id, ladder in grid.items():
        corpora = [
            _evaluate_candidate_matched_r16(
        ...
        for n in FORMAL_BLOCK_OPTIONS:
            ...
            if qualified_ns[str(n)]:
                scores[str(n)] = _maximin_score_r16(corpora, n)
        candidate_results[cand_id] = {
            ...
            "param_distance_from_historical": (
                ladder_distance_from_historical_r16(ladder)),
        }
    (out_dir / "r16_candidate_results.json").write_text(json.dumps(
    ...
    power = _build_power_summary_r16(
        candidate_results, semantic_summaries, selected_id, selected_n,
        design_digest)
    (out_dir / "r16_power_analysis.json").write_text(json.dumps(
    ...
    if selected_id is None:
        summary = {
            ...
            "pass": False,
            "verdict": "R16 FAIL:candidate × block count 无合格组合"
    ...
    if not marginal_artifact["guard"]["pass"]:
        summary = {
            ...
            "pass": False,
            "verdict": "R16 FAIL:选定 ladder 的独立-rung marginal guard "
    ...
    pack = ladder_pack_payload_r16(
        ...
        recall_floor_value=floor,
        noninferiority_delta=NONINFERIORITY_DELTA,
        semantic_blocks_per_corpus=SEMANTIC_BLOCKS_PER_CORPUS_R16,
    ...
    write_selected_pack_r16(out_dir, pack)
    ...
    selection = {
        "format": "cur261-r16-sample-size-selection-v1",
        "iteration": "r16",
        "design_plan_digest": design_digest,
        "selected_candidate": selected_id,
        "selected_block_count": selected_n,
    ...
    (out_dir / "r16_sample_size_selection.json").write_text(json.dumps(
...
def _run_independent_marginal_guard(
    ...
    """§26:design_r16_independent_marginal 语料的 marginal guard。

    R16 语义组装修正:R6 冻结的 c2_marginal_guard_conditions 检查的
    semantics 键为 cue_payoff_separation(R7 误传 cue_semantics——
    R7 calibration 未执行故未暴露);R16 以 semantics=None 调用 R6 统计
    ...
    for rung in CURRICULUM261_RUNGS:
        for idx in range(DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R16):
            indep_records.append(generate_pair(
                FAMILY_C2, rung, idx,
                namespace=DESIGN_INDEPENDENT_NAMESPACE_R16,
                rung_params_override={rung: dict(ladder[rung])}))
    indep_report = rung_report_r4(
        indep_records, FAMILY_C2, ladder, thresholds,
        preproc=None, corpus=DESIGN_INDEPENDENT_NAMESPACE_R16)
    ...
    base = c2_marginal_guard_conditions(
        ...
        semantics=None)
    r16_semantics_ok = bool(
        indep_semantics["local_cue_independence"]["pass"]
        and indep_semantics["context_observability"]["pass"]
        and indep_cue["pass"])
    marginal = dict(base)
    marginal["format"] = "cur261-r16-c2-marginal-guard-v1"
    marginal["r16_semantics_rule"] = (
        "local cue ∧ context observability ∧ independent cue semantics"
        "(point recall ≥ 0.90 灾难护栏 + canonical + non-cue FP;§26)")
    marginal["r16_semantics_pass"] = r16_semantics_ok
    marginal["semantics_pass"] = r16_semantics_ok
    marginal["pass"] = bool(base["pass"] and r16_semantics_ok)
    artifact = {
        "format": "cur261-r16-independent-marginal-design-v1",
        "namespace": DESIGN_INDEPENDENT_NAMESPACE_R16,
        "pairs_per_rung": DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R16,
...
def _build_power_summary_r16(candidate_results: dict[str, Any],
                            semantic_summaries: dict[str, Any],
    ...
    return {
        "format": "cur261-r16-power-analysis-v1",
        "design_plan_digest": design_digest,
        "selected_candidate": selected_id,
        "selected_block_count": selected_n,
