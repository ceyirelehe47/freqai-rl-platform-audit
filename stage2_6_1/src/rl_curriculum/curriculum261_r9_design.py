# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R9:clean matched-ladder design(dedicated semantic
corpus 先行 + corrected plan digest roundtrip + §21/§22 流程 + §18
一次性硬规则)。

R9 相对 R8 的工程修复(规范 §5/§6/§8;统计合同零修改):
...
- R7 design plan 的 digest 自引用缺陷:lock 把 design_plan_digest
  ...
  evidence)。R9 修复(§8):digest 复算排除 (locked_utc,
  design_plan_digest) 双字段;payload 自带 digest 同时可经正式
  load_locked_design_plan_r9 复算——正式 lock/load/digest recompute/
...
- R7 shared cue gate 直接用 40-block candidate design corpus,SE
  ...
  floor 0.930)。R9(§14/§15)把 candidate-independent cue 语义拆到
  预注册 160-block dedicated semantic corpus(每 corpus 独立 gate,
  FAIL 即 R9 design FAIL,不进入 candidate 设计);
- R7 的 R6 marginal-guard semantics 键错配(cue_semantics vs R6 冻结
  实现要求的 cue_payoff_separation;R7 calibration 未执行故未暴露)
  ——R9 显式组装:R6 统计条件 + 本模块语义条件(local cue ∧
  context ∧ v2 independent cue semantics)。

§8.4 一次性硬规则:正式 R9 design plan 锁定后发生任何 loader/代码
缺陷 => write_r9_iteration_aborted + R9 永久结束(保留 plan、不重锁、
不复用 namespace、下一轮 R10);run_design_stage_r9 内部任何
...
§7 要求:R9 在 plan lock 前验证这些模块与 R7 baseline 的 SHA256
...
DESIGN_FORMAT_R9 = "cur261-r9-design-plan-v1"
DESIGN_BLOCKS_PER_CORPUS_R9 = 40
DESIGN_NAMESPACES_R9 = ("design_r9_matched_main",
                        "design_r9_matched_validation")
DESIGN_INDEPENDENT_NAMESPACE_R9 = "design_r9_independent_marginal"
DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R9 = 20

#: §14/§15 dedicated semantic corpus(candidate-independent;在任何
#: R9 design data 生成前锁定;数据后禁止 160→240 扩样/第三 corpus/
#: 合并/删除)。
SEMANTIC_BLOCKS_PER_CORPUS_R9 = 160
SEMANTIC_NAMESPACES_R9 = ("cue_semantic_design_main_r9",
                          "cue_semantic_design_validation_r9")

#: §R9-8 semantic artifact 显式穷尽映射(禁止 endswith/后缀启发式;
#: R8 的 endswith("main") 使 main artifact 被 validation 覆盖)。
SEMANTIC_ARTIFACT_MAP_R9 = {
    "cue_semantic_design_main_r9": "semantic_design_main.json",
    "cue_semantic_design_validation_r9":
        "semantic_design_validation.json",
}
#: calibration/holdout/final 三阶段的 semantic artifact 显式映射。
SEMANTIC_STAGE_ARTIFACT_MAP_R9 = {
    "cue_semantic_calibration_r9": "cue_semantic_calibration.json",
    "cue_semantic_holdout_r9": "cue_semantic_holdout.json",
    "cue_semantic_qualification_r9":
        "qualification_cue_semantics.json",
}
SEMANTIC_CORPUS_ROLE_R9 = {
    "cue_semantic_design_main_r9": "main",
    "cue_semantic_design_validation_r9": "validation",
    "cue_semantic_calibration_r9": "calibration",
    "cue_semantic_holdout_r9": "holdout",
    "cue_semantic_qualification_r9": "qualification",
}


def semantic_artifact_filename_r9(namespace: str) -> str:
    """§R9-8:namespace → 文件名的唯一合法路径(穷尽映射)。"""
    for table in (SEMANTIC_ARTIFACT_MAP_R9,
                  SEMANTIC_STAGE_ARTIFACT_MAP_R9):
        if namespace in table:
            return table[namespace]
    known = sorted(set(SEMANTIC_ARTIFACT_MAP_R9)
                   | set(SEMANTIC_STAGE_ARTIFACT_MAP_R9))
    raise RuntimeError(
        f"semantic namespace {namespace} 不在显式映射表(§R9-8:禁止 "
        f"endswith/后缀启发式;已知 namespace: {known})")


def semantic_artifact_identity_r9() -> dict[str, Any]:
    """§R9-8 artifact writer 身份(进入 design plan code identity)。"""
    return {
        "design_mapping": dict(SEMANTIC_ARTIFACT_MAP_R9),
        "stage_mapping": dict(SEMANTIC_STAGE_ARTIFACT_MAP_R9),
        "roles": dict(SEMANTIC_CORPUS_ROLE_R9),
...
def write_semantic_artifact_r9(out_dir: Path, namespace: str,
                               payload: dict[str, Any],
                               plan_digest: str,
                               event_rows: list[dict[str, Any]] | None
                               = None,
                               ) -> Path:
    """§R9-8:exclusive-create 写 semantic artifact(json + jsonl),
    ...
    fname = semantic_artifact_filename_r9(namespace)
    path = out_dir / fname
    body = dict(payload)
    body["namespace"] = namespace
    body["corpus_role"] = SEMANTIC_CORPUS_ROLE_R9[namespace]
    ...
    if (back.get("namespace") != namespace
            ...
            != SEMANTIC_CORPUS_ROLE_R9[namespace]):
        raise RuntimeError(
...
#: §16.1/§21 code identity 覆盖面(R9 全部实现 + 冻结复用模块 +
#: 合同依赖;design data 生成开始后任何漂移 => iteration aborted)。
DESIGN_CODE_MODULES_R9 = (
    ...
    "curriculum261_r9_noise_replay.py",
    "curriculum261_r9_cue_contract.py",
    "curriculum261_r9_cue_eval.py",
    "curriculum261_r9_dependencies.py",
    "curriculum261_r9_namespaces.py",
    "curriculum261_r9_param_pack.py",
    "curriculum261_r9_preplan.py",
    "curriculum261_r9_design.py",
    "curriculum261_r9_calibration.py",
    "curriculum261_r9_plan.py",
    "curriculum261_r9_preflight.py",
    "curriculum261_r9_final.py",
    "curriculum261_r9_smoke.py",
...
def _code_identity_design() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    for name in DESIGN_CODE_MODULES_R9:
        f = root / name
...
def _r8_design_plan_path() -> Path:
    return (Path(__file__).resolve().parents[2] / "artifacts"
            / "route_c_stage2_6_1_repair8" / "r8_design_plan.json")


def verify_matched_core_identity_r8() -> dict[str, Any]:
    """§7:matched-ladder 核心模块与 R8 baseline 的 SHA256 一致。

    R8 的 design plan(R8 修复的 lock/load roundtrip 合同)在 lock 时
    记录了全部依赖模块的 sha256;R9 不修改 c2 generator / matched
    tape / matched pairs 三个模块,当前哈希必须与 R8 记录值逐位一致
    (R9 对 api.py 的改动仅限 R9 namespace 白名单与守卫的追加,
    paired_noise 行为由 exact replay 测试逐位锁定)。对拍源从 R7
    plan 升级为 R8 plan(R8 为最新冻结声明)。
    """
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    path = _r8_design_plan_path()
    if not path.is_file():
        return {
            "pass": False,
            "error": f"R8 design plan 不存在: {path}(无法对拍 "
                     "matched-ladder 模块身份)",
            "modules": {},
        }
    r8_plan = json.loads(path.read_text(encoding="utf-8"))
    r8_identity = r8_plan.get("code_identity", {})
    rows: dict[str, Any] = {}
    ok = True
    for name in MATCHED_CORE_MODULES_R9:
        current = (hashlib.sha256((root / name).read_bytes()).hexdigest()
                   if (root / name).is_file() else "MISSING")
        locked = r8_identity.get(name)
        match = bool(current == locked and current != "MISSING")
        ok = ok and match
        rows[name] = {"r8_locked": locked, "r9_current": current,
                      "match": match}
    return {"r8_design_plan_digest": r8_plan.get("design_plan_digest"),
            "modules": rows, "pass": bool(ok)}


# ------------------------------------------------------------- plan
def design_plan_payload_r9(*, baseline_commit: str, vendor_pin: str,
    ...
    """构建并返回 R9 design plan payload(锁定后不得修改任何字段)。
    ...
    if problems:
        raise RuntimeError(f"R9 candidate grid 语义非法: {problems}")
    matched_core = verify_matched_core_identity_r8()
    ...
    return {
        ...
        "plan_digest_contract": {
            ...
            "roundtrip": "build -> lock(O_CREAT 语义:已存在即拒) -> "
                         "load_locked_design_plan_r9(独立进程) -> "
        ...
        "matched_ladder": {
            ...
            "implementation": "R6 冻结实现零修改复用(import,不复制);"
                              ...
                              "值对拍(verify_matched_core_identity_r8)",
        ...
        "semantic_corpora": {
            "blocks_per_corpus": SEMANTIC_BLOCKS_PER_CORPUS_R9,
            "namespaces": list(SEMANTIC_NAMESPACES_R9),
            "ladder": "冻结 sentinel ladder(cur261-c2-v9 默认 D0-D3;"
                      "candidate-independent;与任何 R9 candidate 数值"
                      "无依赖)",
            ...
            "gate": "每 corpus 独立:unique-event coverage ≥3600 / "
                    ...
                    "replay 完整性;任一 FAIL => R9 design FAIL(§15)",
        ...
        "design_data": {
            "blocks_per_candidate_per_corpus": DESIGN_BLOCKS_PER_CORPUS_R9,
            "corpora": list(DESIGN_NAMESPACES_R9),
        ...
        "independent_marginal_guard": {
            "namespace": DESIGN_INDEPENDENT_NAMESPACE_R9,
            "pairs_per_rung": DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R9,
            "conditions": "marginal ordering / D3 positive / 基线 margins"
                          ...
                          ";matched PASS 不可覆盖 FAIL;失败 → R9 = FAIL",
        ...
        "fail_path": {
            ...
            "on_semantic_gate_fail": "任一 semantic corpus gate FAIL → "
                                     "R9 design FAIL(不生成 candidate "
                                     "blocks;§15/§25)",
            "on_no_qualified_combination": "R9 = FAIL;自动 power/semantic "
...
def design_plan_digest_r9(plan: dict[str, Any]) -> str:
    ...
    return "r9dp-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def lock_design_plan_r9(out_dir: Path, plan: dict[str, Any],
                        ) -> tuple[Path, str]:
    ...
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "r9_design_plan.json"
    if path.exists():
        raise RuntimeError(
            f"R9 design plan 已存在: {path}(§8.4 禁止删除/覆盖/重锁;"
            "重锁必须换新 iteration R10)")
    digest = design_plan_digest_r9(plan)
    plan = dict(plan)
    ...
    (out_dir / "r9_design_plan_digest.txt").write_text(
        digest, encoding="utf-8")
    return path, digest


def load_locked_design_plan_r9(out_dir: Path,
    ...
    path = out_dir / "r9_design_plan.json"
    if not path.is_file():
        raise RuntimeError(f"R9 design plan 未锁定: {path}")
    plan = json.loads(path.read_text(encoding="utf-8"))
    digest = design_plan_digest_r9(plan)
    if plan.get("design_plan_digest") != digest:
        raise RuntimeError("R9 design plan digest 复算不一致(fail closed)")
    digest_path = out_dir / "r9_design_plan_digest.txt"
    if not digest_path.is_file() or \
            digest_path.read_text(encoding="utf-8").strip() != digest:
        raise RuntimeError("R9 design plan digest 文件与 payload 不一致")
    return plan, digest
...
def _evaluate_candidate_matched_r9(
    ...
    # 密度(R6 冻结 gate)+ R9 candidate-specific cue 语义 + 独立性/
    ...
    semantics = {
        ...
        "context_observability": check_c2_context_observability(
            records),
        "candidate_cue_semantics_r9_cluster_aware": cue_sem,
        "cue_payoff_confusion": {
            k: v for k, v in cue_sem["per_rung"]["D0"]["sides"]["A"][
                "confusion"].items()},
        # R6 点阈值分离检查仅作诊断对照(绝不进入 R9 资格判定)
...
def _maximin_score_r9(corpus_results: list[dict[str, Any]],
                      n: int) -> float:
...
# ------------------------------------------------- 主流程
def run_design_stage_r9(out_dir: Path, plan: dict[str, Any],
                        design_digest: str,
                        baseline_commit: str = "") -> dict[str, Any]:
    """R9 design 主流程:治理检查 → semantic corpora(160×2)+ gate
    (FAIL 即短路)→ 3 candidate × 2 corpus blocks → candidate 评估 →
    机械选择 → marginal guard → pack。

    §8.4:plan 锁定后本函数内部任何异常都转 write_r9_iteration_aborted
    + re-raise(R9 永久结束;不删 plan、不重锁、不复用 namespace)。
    """
    require_r9_iteration_active()
    out_dir = Path(out_dir)
    try:
        return _run_design_stage_inner_r9(
        ...
        write_r9_iteration_aborted(
            ...
            f"{str(exc)[:300]});按 §8.4 R9 永久结束")
        raise


def _run_design_stage_inner_r9(out_dir: Path, plan: dict[str, Any],
                               design_digest: str,
                               baseline_commit: str,
                               ) -> dict[str, Any]:
    ...
    if not identity["pass"]:
        raise RuntimeError(
            f"R9 design plan code identity 与当前代码不一致:"
    ...
    # 第一条 R9 design/semantic episode 生成前记录 started(§8.4)
    mark_design_data_started()
    ...
    for corpus_ns in semantic_cfg["namespaces"]:
        ...
        # R9:显式穷尽映射 + exclusive create + reload 校验
        # (R8 的 endswith("main") 使 main artifact 被 validation 覆盖)
    ...
    # §R9-8:main/validation 两个 semantic artifact 必须同时存在且
    # sha256 不同(任一不满足 => 工程缺陷 => §18 一次性硬规则,
    # 禁止生成 candidate blocks)。
    semantic_paths = [out_dir / semantic_artifact_filename_r9(ns)
                      for ns in semantic_cfg["namespaces"]]
    if not all(p.is_file() for p in semantic_paths):
        raise RuntimeError(
            "semantic main/validation artifact 未同时落盘(§R9-8;"
            "禁止生成 candidate blocks)")
    ...
    if len(set(semantic_hashes)) != len(semantic_hashes):
        raise RuntimeError(
            ...
            "§R9-8 禁止生成 candidate blocks)")
    if not semantic_pass:
        summary = {
            "format": "cur261-r9-design-stage-v1",
            "iteration": "r9",
            ...
            "pass": False,
            "verdict": "R9 design FAIL:dedicated semantic corpus gate "
        ...
        (out_dir / "r9_sample_size_selection.json").write_text(
    ...
    for cand_id, ladder in grid.items():
        ...
        for n in FORMAL_BLOCK_OPTIONS:
            ...
            if qualified_ns[str(n)]:
                scores[str(n)] = _maximin_score_r9(corpora, n)
        candidate_results[cand_id] = {
            ...
            "param_distance_from_historical": (
                ladder_distance_from_historical_r9(ladder)),
        }
    (out_dir / "r9_candidate_results.json").write_text(json.dumps(
    ...
    power = _build_power_summary_r9(
        candidate_results, semantic_summaries, selected_id, selected_n,
        design_digest)
    (out_dir / "r9_power_analysis.json").write_text(json.dumps(
    ...
    if selected_id is None:
        summary = {
            ...
            "pass": False,
            "verdict": "R9 FAIL:candidate × block count 无合格组合"
    ...
    if not marginal_artifact["guard"]["pass"]:
        summary = {
            ...
            "pass": False,
            "verdict": "R9 FAIL:选定 ladder 的独立-rung marginal guard "
    ...
    pack = ladder_pack_payload_r9(
        ...
        recall_floor_value=floor,
        noninferiority_delta=NONINFERIORITY_DELTA,
        semantic_blocks_per_corpus=SEMANTIC_BLOCKS_PER_CORPUS_R9,
        candidate_evidence={
    ...
    write_selected_pack_r9(out_dir, pack)
    ...
    selection = {
        "format": "cur261-r9-sample-size-selection-v1",
        "iteration": "r9",
        "design_plan_digest": design_digest,
        "selected_candidate": selected_id,
        "selected_block_count": selected_n,
    ...
    (out_dir / "r9_sample_size_selection.json").write_text(json.dumps(
...
def _run_independent_marginal_guard(
    ...
    """§26:design_r9_independent_marginal 语料的 marginal guard。

    R9 语义组装修正:R6 冻结的 c2_marginal_guard_conditions 检查的
    semantics 键为 cue_payoff_separation(R7 误传 cue_semantics——
    R7 calibration 未执行故未暴露);R9 以 semantics=None 调用 R6 统计
    ...
    for rung in CURRICULUM261_RUNGS:
        for idx in range(DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R9):
    ...
    indep_report = rung_report_r4(
        ...
        preproc=None, corpus=DESIGN_INDEPENDENT_NAMESPACE_R9)
    ...
    r9_semantics_ok = bool(
        ...
        and indep_cue["pass"])
    marginal = dict(base)
    marginal["format"] = "cur261-r9-c2-marginal-guard-v1"
    marginal["r9_semantics_rule"] = (
        "local cue ∧ context observability ∧ independent cue semantics"
        "(point recall ≥ 0.90 灾难护栏 + canonical + non-cue FP;§26)")
    marginal["r9_semantics_pass"] = r9_semantics_ok
    marginal["semantics_pass"] = r9_semantics_ok
    marginal["pass"] = bool(base["pass"] and r9_semantics_ok)
    artifact = {
        "format": "cur261-r9-independent-marginal-design-v1",
        "namespace": DESIGN_INDEPENDENT_NAMESPACE_R9,
        "pairs_per_rung": DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R9,
...
def _build_power_summary_r9(candidate_results: dict[str, Any],
                            semantic_summaries: dict[str, Any],
    ...
    return {
        "format": "cur261-r9-power-analysis-v1",
        "design_plan_digest": design_digest,
        "selected_candidate": selected_id,
        "selected_block_count": selected_n,
