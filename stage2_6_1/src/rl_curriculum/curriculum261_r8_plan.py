# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R8:final qualification plan(§26)。

双 strict gate(calibration + holdout 各自独立 PASS;无 pooled 救援)
前置强制;plan 绑定 R8 pack/cue semantic 合同/p_contract/recall
...
from rl_curriculum.curriculum261_r8_namespaces import (
    CURRICULUM261_ITERATION_ID_R8,
    qualification_r8_digest_path,
    qualification_r8_plan_path,
)

PLAN_FORMAT_R8 = "cur261-r8-qualification-plan-v1"

#: R8 代码身份(逐模块内容哈希;进入 plan;final 复算比对)。
PLAN_CODE_MODULES_R8 = (
    ...
    "curriculum261_r8_calibration.py",
    "curriculum261_r8_preflight.py",
    "curriculum261_r8_plan.py",
    "curriculum261_r8_final.py",
    "curriculum261_r8_smoke.py",
    "curriculum261_r8_cli.py",
)


def _code_identity_r8() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    for name in PLAN_CODE_MODULES_R8:
        f = root / name
    ...
    plan: dict[str, Any] = {
        ...
        "preprocessing_v2": {
            ...
            "implementation": "pinned vendor IFreqaiModel."
                              "define_data_pipeline(直接复用;R8 全新"
                              "语料重新资格验证)",
        ...
        "fit_bank_schedule": {
            "calibration": {
                "namespace": "preprocess_fit_calibration_r8",
                "pairs_per_rung": 4, "sides": ["A", "B"],
                "role": "main fit corpus(preprocessor only)"},
            "holdout": {
                "namespace": "preprocess_fit_holdout_r8",
                "pairs_per_rung": 4, "sides": ["A", "B"],
        ...
        "semantic_corpus_schedule": {
            "design": {
                "namespaces": ["cue_semantic_design_main_r8",
                               "cue_semantic_design_validation_r8"],
                "blocks_per_corpus": 160,
                "ladder": "sentinel(candidate-independent)"},
            "calibration": {
                "namespace": "cue_semantic_calibration_r8",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "holdout": {
                "namespace": "cue_semantic_holdout_r8",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "final": {
                "namespace": "cue_semantic_qualification_r8",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
        },
        "final_sample_counts": {
        ...
        "qualification_bank_schedule": {
            ...
            "fresh_seed_namespace": "fresh_holdout_r8",
        },
        ...
        "code_identity": _code_identity_r8(),
    }
    return plan


def plan_digest_r8(plan: dict[str, Any]) -> str:
...
def lock_plan_r8(plan: dict[str, Any]) -> tuple[Path, str]:
    ...
    path = qualification_r8_plan_path()
    if path.is_file():
        raise RuntimeError(
            "R8 qualification plan 已存在;plan lock 后禁止重写(§16.2/"
            "§26:任何修改须新 iteration R8.1/R8)")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(plan, indent=2, ensure_ascii=False,
                               default=str), encoding="utf-8")
    digest = plan_digest_r8(plan)
    dpath = qualification_r8_digest_path()
    dpath.write_text(digest, encoding="utf-8")
    return path, digest


def load_locked_plan_r8() -> tuple[dict[str, Any], str]:
    path = qualification_r8_plan_path()
    dpath = qualification_r8_digest_path()
    if not path.is_file() or not dpath.is_file():
        raise RuntimeError("R8 qualification plan 未锁定")
    plan = json.loads(path.read_text(encoding="utf-8"))
    digest = plan_digest_r8(plan)
    if dpath.read_text(encoding="utf-8").strip() != digest:
        raise RuntimeError("R8 plan digest 复算不一致(fail closed)")
    return plan, digest
