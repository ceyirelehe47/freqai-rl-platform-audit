# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R17:依赖身份闭环与 Code Freeze 合同(§6/§7.3/§21)。
...
2. R17 Code Freeze(§6/§21):正式数据开始前记录全部 R17 源码模块的
   sha256 清单(R17_CODE_FREEZE_SHA + source tree digest);每个正式
   阶段开始前复算,任何漂移 fail closed(永久结束 R17)。
"""
...
DEPENDENCY_TABLE_R17: tuple[tuple[str, str], ...] = (
    # ---- 密度 / pair 评估(R5/R6 冻结)----
    ...
    # ---- R6 runner(R17 wrapper 的委托目标;签名核对对象)----
    ...
    # ---- cue 语义(R17 namespace)----
    ("candidate_cue_semantics",
     "rl_curriculum.curriculum261_r17_cue_eval"),
    ("semantic_cue_gate", "rl_curriculum.curriculum261_r17_cue_eval"),
    ("independent_cue_semantics",
     "rl_curriculum.curriculum261_r17_cue_eval"),
    # ---- R17 新合同(核心修复面)----
    ("canonical_episode", "rl_curriculum.curriculum261_r17_reference"),
    ("reference_equivalence_run_r17",
     "rl_curriculum.curriculum261_r17_reference"),
    ("collect_policy_visible_dataset_r17",
     "rl_curriculum.curriculum261_r17_labels"),
    ("build_routing_r17", "rl_curriculum.curriculum261_r17_routing"),
    ("require_eval_routing_r17",
     "rl_curriculum.curriculum261_r17_routing"),
    ...
    ("run_preplan_full_pipeline_rehearsal_r17",
     "rl_curriculum.curriculum261_r17_rehearsal"),
    ("live_signature_audit_r17",
     "rl_curriculum.curriculum261_r17_delegation"),
    ...
    ("generation_determinism_gate",
     "rl_curriculum.curriculum261_r17_determinism"),
    # ---- R17 full-scale shadow(工作包 C)----
    ("run_full_scale_shadow_r17", "rl_curriculum.curriculum261_r17_shadow"),
...
#: R17 code freeze 覆盖的源码模块(sha256 清单;§6/§21)。
R17_CODE_MODULES: tuple[str, ...] = (
...
def resolve_dependency_identity_r17() -> dict[str, Any]:
    """解析全部声明符号(fail closed:missing/错模块立即报错)。"""
    rows: dict[str, Any] = {}
    ok = True
    for symbol, defining_module in DEPENDENCY_TABLE_R17:
        try:
    ...
    sig = inspect.signature(supervised_learnability_run_r17)
    ...
    result = {
        "format": "cur261-r17-dependency-resolution-v1",
        "iteration": "r17", "symbols": rows,
        "dependencies": rows,
        "n_symbols": len(DEPENDENCY_TABLE_R17),
        "n_declared": len(DEPENDENCY_TABLE_R17),
    ...
    result["digest"] = dependency_identity_digest_r17(result)
    return result


def dependency_identity_digest_r17(resolution: dict[str, Any]) -> str:
...
def write_dependency_resolution_r17(out_dir: Path) -> dict[str, Any]:
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    resolution = resolve_dependency_identity_r17()
    resolution["digest"] = dependency_identity_digest_r17(resolution)
...
R17_CODE_MODULES: tuple[str, ...] = (
    ...
    "curriculum261_r17_shadow.py",
    "curriculum261_r17_smoke.py",
    # R17 新增(§四/§六/§七):权威 workflow/阶段精确 fail closure/
    # full-cold reader(R14 清单曾遗漏 full_cold——R17 补齐)
    "curriculum261_r17_full_cold.py",
    "curriculum261_r17_workflow.py",
    "curriculum261_r17_fail_closure.py",
)

#: R17 Implementation Freeze 的完整覆盖面(§六:不得只哈希若干 src
#: 模块——必须覆盖所有可能影响正式执行/诊断/封存/报告的实现文件)。
#: dev 树(WSL 执行面)递归目录:
R17_FREEZE_DEV_DIRS: tuple[str, ...] = (
    "src/rl_curriculum",
    "tests/route_c_stage2_6_1",
)
#: dev 树单文件(RouteC strategy / 依赖锁 / 环境配置 / CLI 启动脚本):
R17_FREEZE_DEV_FILES: tuple[str, ...] = (
...
R17_FREEZE_REPO_PATHS: tuple[str, ...] = (
    "stage2_6_1/src",
...
R17_FREEZE_EXCLUDE_DIRS: tuple[str, ...] = (
...
def _scan_freeze_dir(root: Path) -> dict[str, dict[str, Any]]:
    ...
    for path in sorted(root.rglob("*")):
        if any(part in R17_FREEZE_EXCLUDE_DIRS
...
def freeze_surface_manifest_r17() -> dict[str, Any]:
    """R17 全 freeze surface manifest(dev 递归 + repo Git identity)。
    ...
    ls_files = _git(repo, "ls-files", "-s", "--", *R17_FREEZE_REPO_PATHS)
...
def source_tree_digest_r17() -> dict[str, Any]:
    """全部 R17 源码模块的 sha256 清单与聚合 digest(模块级子集)。"""
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    modules: dict[str, str] = {}
    for name in R17_CODE_MODULES:
        path = root / name
    ...
    return {"format": "cur261-r17-source-tree-v1",
            "modules": modules, "n_modules": len(modules),
            "source_tree_digest": aggregate,
            "all_present": all(v != "MISSING" for v in modules.values())}


def write_r17_code_freeze(out_dir: Path, *, code_freeze_sha: str) -> dict:
    """记录 Implementation Freeze(§六全 surface;一次且仅一次;禁 A′)。

    R17 只有 Commit A(冻结)与 Commit B(结果)两个提交阶段:
    freeze 文件已存在 ⇒ 重复写入被拒绝(不存在 replacement freeze /
    hotfix-after-freeze 恢复路径;任何冻结后实现面变化 = R17 永久
    FAIL,下一轮必须 R17)。code_freeze_sha 必须等于 repo HEAD
    (即 Commit A 的 git commit sha)——冻结的就是这个提交。
    """
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "r17_code_freeze.json"
    if path.is_file():
        raise RuntimeError(
            "r17_code_freeze.json 已存在:Implementation Freeze 一次且"
            "仅一次(§六;禁止 A′/replacement freeze/hotfix-after-freeze;"
            "任何冻结后实现面变化 => R17 永久 FAIL,下一轮必须 R17)")
    manifest = freeze_surface_manifest_r17()
    ...
    payload = {
        "format": "cur261-r17-code-freeze-v1",
        "iteration": "r17",
        ...
        "contract": ("Commit A(Implementation Freeze):全部实现面"
                     ...
                     "已提交且工作树 clean;正式 R17 任何数据开始后"
                     "实现面变化(modified/added/removed/renamed/"
                     "untracked executable/symlink drift) -> R17 永久"
                     "结束,下一轮必须 R17(§六;不存在 A′/Commit A2/"
                     "hotfix after freeze)"),
    }
    (out_dir / "r17_code_freeze.json").write_text(
        json.dumps(payload, indent=1, ensure_ascii=False), encoding="utf-8")
    return payload


def verify_r17_code_freeze(out_dir: Path) -> dict[str, Any]:
    """复算全 freeze surface 与冻结 manifest 比对(漂移即 fail)。"""
    out_dir = Path(out_dir)
    path = out_dir / "r17_code_freeze.json"
    if not path.is_file():
        return {"pass": False,
                "error": "r17_code_freeze.json 不存在(正式数据开始前"
                         "必须先冻结;§六)"}
    frozen = json.loads(path.read_text(encoding="utf-8"))
    frozen_surface = frozen.get("freeze_surface") or {}
    frozen_files = frozen_surface.get("dev_files") or {}
    try:
        current = freeze_surface_manifest_r17()
    except Exception as exc:  # noqa: BLE001 —— fail closed
        return {"pass": False,
                "error": f"freeze_surface_manifest_r17 异常(fail "
