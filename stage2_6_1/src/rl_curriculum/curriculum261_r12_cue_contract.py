# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R12:Cue 检测语义合同 v2 审计 + 全局 K 分布门禁。
...
2. 新增 C2MirrorCountGlobalAudit-v1(curriculum261_r12_global_k):
...
三路闭合(承接 R11):
    ...
    C. Direct Generator Replay:两个全新 candidate-independent audit
       corpora(cue_contract_model_r12 / cue_contract_validation_r12,
...
dedicated semantic corpus(§14/§15,curriculum261_r12_cue_eval)。
"""
...
#: 其他语义指标业务阈值(与 R6/R7 相同;R12 用 cluster bounds)。
...
#: 合同审计配置(冻结;§12)。
AUDIT_MODEL_NAMESPACE = "cue_contract_model_r12"
AUDIT_VALIDATION_NAMESPACE = "cue_contract_validation_r12"
...
#: legacy 逐位置 4σ 诊断参数(R12 起仅诊断,不进 PASS/FAIL;§7/§11)。
AUDIT_K_MIN_EVENTS_PER_POSITION = 30
AUDIT_K_Z_THRESHOLD = 4.0
#: audit bootstrap(与 r12_cue_eval 同规格;本模块自含双侧 CI 实现)。
...
#: 与 R12 candidate 网格无依赖——alpha_bps/wick_kappa 不进入 cue 表/
...
def _tail_k_position_checks(events: list[dict[str, Any]], n: int,
    ...
    委托 curriculum261_r12_global_k.legacy_positionwise_z_diagnostic;
    输出携带 binding_gate=false / legacy_diagnostic_only=true,
    不再进入 R12 audit PASS 判据。
    """
...
def _tail_mirror_bound_integrity(
    ...
    return {
        "format": "cur261-r12-tail-mirror-bound-integrity-v1",
...
# ------------------------------------------------- 三路闭合 audit(§12)
#: §R12-10 cue audit plan(audit data 生成前锁定;不可修改)。
CUE_AUDIT_PLAN_FORMAT_R12 = "cur261-r12-cue-audit-plan-v1"
CUE_AUDIT_PLAN_FILENAME = "cue_audit_plan.json"
CUE_AUDIT_PLAN_DIGEST_FILENAME = "cue_audit_plan_digest.txt"
CUE_AUDIT_CODE_MODULES_R12 = (
    ...
    "curriculum261_r12_global_k.py",
)


def cue_audit_code_identity_r12() -> dict[str, str]:
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    out: dict[str, str] = {}
    for name in CUE_AUDIT_CODE_MODULES_R12:
        f = root / name
...
def cue_audit_plan_payload_r12(
        code_identity: dict[str, str] | None = None) -> dict[str, Any]:
    """§R12-10:audit data 生成前锁定的 plan payload(全部预注册)。"""
    return {
        "format": CUE_AUDIT_PLAN_FORMAT_R12,
        "audit_namespaces": {
        ...
        "code_identity": (code_identity
                          ...
                          else cue_audit_code_identity_r12()),
    }
...
def load_locked_cue_audit_plan_r12(out_dir: Path) -> dict[str, Any]:
    out_dir = Path(out_dir)
    path = out_dir / CUE_AUDIT_PLAN_FILENAME
    dpath = out_dir / CUE_AUDIT_PLAN_DIGEST_FILENAME
    if not path.is_file() or not dpath.is_file():
        raise RuntimeError(
            "cue audit plan 未锁定(§R12-10:正式 audit data 生成前"
            "必须先锁定)")
    ...
    current_ident = cue_audit_code_identity_r12()
    ...
    if drift:
        raise RuntimeError(
            f"cue audit plan code identity 与当前代码漂移(§R12-10;"
            f"audit plan 锁定后相关模块不得修改):{sorted(drift)}")
    return payload


def _once_vs_attempts_bitwise_check_r12(
        blocks: list[Any], ladder: dict[str, dict[str, Any]],
        namespace: str, max_blocks: int = 50) -> dict[str, Any]:
    """§R12-11:attempts-mode 选中 attempt==0 的 block 与 once-mode 同
...
def run_cue_contract_audit(
    ...
    """执行三路闭合合同审计(在任何 R12 design/semantic data 之前)。
    ...
    if require_locked_plan:
        ...
        audit_plan = load_locked_cue_audit_plan_r12(out_dir)
    ...
    for corpus_name, ns in (("model", model_ns),
        ...
        if mode == "once":
            ...
            # §R12-10/§R12-11:validation 语料用正式 attempts-mode
            ...
            once_bitwise = _once_vs_attempts_bitwise_check_r12(
                blocks_v, ladder, ns)
    ...
    # ---- §R12-11 once-mode vs attempts-mode 系统偏差对照 ----
    ...
    report: dict[str, Any] = {
        "format": "cur261-r12-cue-contract-audit-v1",
        ...
        "mirror_bound_v2": {
            ...
            "authority": "curriculum261_r12_noise_replay."
                         "mirror_candidate_positions",
        },
    ...
    if out_dir is not None:
        ...
        (out_dir / "once_vs_attempts_audit.json").write_text(
            json.dumps({
                "format": "cur261-r12-once-vs-attempts-audit-v1",
                "model_mode": "once",
                "validation_mode": "attempts",
        ...
        (out_dir / "cue_k_distribution.json").write_text(json.dumps({
            "format": "cur261-r12-cue-k-distribution-v1",
        ...
        (out_dir / "cue_global_k_cell_diagnostics.json").write_text(
            json.dumps({
                "format": "cur261-r12-global-k-cell-diagnostics-v1",
        ...
        (out_dir / "tail_mirror_validation.json").write_text(json.dumps({
            "format": "cur261-r12-tail-mirror-validation-v1",
        ...
        (out_dir / "noise_replay_validation.json").write_text(json.dumps({
            "format": "cur261-r12-noise-replay-validation-v1",
            "tolerance": 1e-12,
            "model": {
...
def cue_contract_audit_digest(report: dict[str, Any]) -> str:
    ...
    blob = json.dumps(core, sort_keys=True, ensure_ascii=False,
                      default=float)
    return "r12ca-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
...
def cue_semantic_contract_digest() -> str:
    blob = json.dumps(cue_semantic_contract_payload(), sort_keys=True,
                      ensure_ascii=False)
    return "r12cue-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
