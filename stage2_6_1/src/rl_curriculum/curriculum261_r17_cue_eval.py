# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R17:cluster-aware cue 语义评估器(unique event)。
...
- §14/§15 dedicated semantic corpus(160 matched blocks/corpus,
  ...
  R17 design FAIL,不进入 candidate 设计;
...
R17_CUE_BOOTSTRAP_RESAMPLES = 20000
R17_CUE_BOOTSTRAP_SEED = 20261203
...
def cluster_bootstrap_rate(
        ...
        n_boot: int = R17_CUE_BOOTSTRAP_RESAMPLES,
        seed: int = R17_CUE_BOOTSTRAP_SEED,
) -> dict[str, Any]:
...
def semantic_cue_gate(
    ...
    return {
        "format": "cur261-r17-semantic-cue-gate-v1",
        "label": label,
...
def candidate_cue_semantics(
    ...
    return {
        "format": "cur261-r17-candidate-cue-semantics-v1",
...
def cue_semantic_rule_identity() -> str:
    """R17 cue 语义 gate 规则身份(冻结;plan/pack 绑定)。"""
    payload = {
        ...
        "bootstrap_resamples": R17_CUE_BOOTSTRAP_RESAMPLES,
        "bootstrap_seed": R17_CUE_BOOTSTRAP_SEED,
...
def independent_cue_semantics(
    ...
    R17 变更(GateTopologyReconciliation-v2;修复 R14 隐藏双绑定):
    ...
    # ---- R17 §五:structural(binding)与 cue point diagnostics 分离 ----
    ...
    # source。R17 权威语义:independent marginal 的 binding 职责仅
    ...
    return {
        "format": "cur261-r17-independent-cue-semantics-v2",
        "candidate": candidate_id,
        ...
        "structural": {
            ...
            "note": (
                "R17 §五:independent marginal 的 binding 职责仅结构"
        ...
        "cue_point_diagnostics": {
            ...
            "note": (
                ...
                "pass——R17 永久移除);诊断 FAIL 不得改变 independent "
                "marginal binding result 与 final verdict"),
        },
        # 顶层 pass = structural(binding 语义)。消费方
        # (c2_independent_marginal_guard_r17)不得读取任何 cue
        # 点指标进入 guard.pass。
        "pass": structural_pass,
    }
