...
from rl_curriculum.curriculum261_r13_labels import (
    SUPERVISED_LABEL_CONTRACT,
    collect_policy_visible_dataset_r13,
    supervised_dataset_identity_r13,
)
...
REHEARSAL_FIT_PAIRS_PER_RUNG_R13 = 2


def generate_fit_bank_r13(
...
def fit_preprocessor_v2_from_bank_r13(
        namespace: str, pack: dict[str, Any], *,
...
def run_calibration_corpus_c13_r13(
        preproc_v2: Any, pack: dict[str, Any], namespace: str, *,
        pairs_per_rung: int = CALIBRATION_PAIRS_PER_RUNG_R13,
) -> dict[str, Any]:
...
def run_c2_independent_corpus_r13(
        preproc_v2: Any, pack: dict[str, Any], namespace: str, *,
        pairs_per_rung: int = C2_INDEPENDENT_PAIRS_PER_RUNG_R13,
) -> dict[str, Any]:
...
def run_c2_semantic_corpus_r13(
        pack: dict[str, Any], namespace: str, *,
        n_blocks: int = SEMANTIC_BLOCKS_PER_CORPUS_R13,
        out_dir: Path | None = None,
        artifact_name: str | None = None) -> dict[str, Any]:
    """§18/§19 dedicated cue semantic corpus(R9 语义;R13 namespace)。
    ...
    ladder = r13_family_rung_params(FAMILY_C2, pack)
    ...
    result = {
        "format": "cur261-r13-semantic-corpus-v1",
        "namespace": namespace,
    ...
    if out_dir is not None:
        ...
        expected_name = semantic_artifact_filename_r13(namespace)
...
def c2_matched_conditions_r13(
    ...
    checks = {
        ...
        "cue_semantics_delegated_note": (
            ...
            "160-block semantic corpus(cue_semantic_*_r13 namespace)"),
    }
    return {
        "format": "cur261-r13-c2-matched-conditions-v1",
        "statistical": base,
...
def c2_independent_marginal_guard_r13(
    ...
    r13_semantics_ok = bool(
        ...
        and cue["pass"])
    guard = dict(base)
    guard["format"] = "cur261-r13-c2-marginal-guard-v1"
    guard["r13_semantics_rule"] = (
        "local cue ∧ context observability ∧ independent cue semantics"
        "(point recall ≥ 0.90 灾难护栏)")
    guard["r13_semantics_pass"] = r13_semantics_ok
    guard["semantics_pass"] = r13_semantics_ok
    guard["pass"] = bool(base["pass"] and r13_semantics_ok)
    return {
        "format": "cur261-r13-c2-independent-marginal-v1",
...
# ---------------------------------------------------- supervised(§8/§17)
def supervised_learnability_run_r13(
        preproc_v2: Any, pack: dict[str, Any], *,
        namespace: str,
        pairs_per_rung: int = CALIBRATION_PAIRS_PER_RUNG_R13,
        train_pair_limit: int = 6,
        model_seeds: tuple | list = R13_SUPERVISED_MODEL_SEEDS,
    ...
    for family in CURRICULUM261_FAMILIES:
        ...
        rung_params = r13_family_rung_params(family, pack)
        dataset = collect_policy_visible_dataset_r13(
        ...
        controls = ["U", "W", "B"]
        gated_controls = list(R13_SUPERVISED_GATE["gated_controls"])
        seed_reports: list[dict[str, Any]] = []
        # ---- repair R13(B2):distinct model-seed 计数 ----
        # R10 缺陷:n_passing 把 W/B 每个通过的 run 各计 1 —— 同一
        # model seed 的 W+B 双通过被误算成"两个 seeds 通过"
        # (control-run count 冒充 seed count)。R13 合同:对每个
        ...
        for seed in model_seeds:
            for control in controls:
                ...
                gated = bool(
                    ...
                    >= R13_SUPERVISED_GATE["behavior_gap_min"])
        ...
        control_pass = {
            ...
            >= R13_SUPERVISED_GATE["min_seeds_passing"]
        ...
        out["families"][family] = {
            ...
            "distinct_seed_gate": {
                "rule": (
                    ...
                    f"{R13_SUPERVISED_GATE['min_seeds_passing']}/"
                    f"{R13_SUPERVISED_GATE['n_model_seeds']} distinct "
        ...
        out.setdefault("dataset_identity", {})[family] = (
            supervised_dataset_identity_r13(dataset))
    out["alignment_all_ok"] = alignment_all
    out["pass"] = bool(overall and alignment_all)
    return out


def run_generator_stress_r13(
        pack: dict[str, Any], *,
        pairs_per_rung: int = 12,
        namespace: str = "stress_r13") -> dict[str, Any]:
    """generator stress(R6 冻结实现;namespace 显式 stress_r13)。

    R9 缺陷修复:R9 CLI 调用未传 namespace,R6 默认回落历史
    stress_r6 —— R13 wrapper 把 namespace 变为显式 keyword-only
    参数,默认 stress_r13。
    """
...
def run_c2_density_diagnostics_r13(
...
def run_c2_diagnostics_r13(records: list[Any]) -> dict[str, Any]:
...
__all__ = [
    "generate_fit_bank_r13", "fit_preprocessor_v2_from_bank_r13",
    "run_calibration_corpus_c13_r13", "run_c2_matched_corpus_r13",
    "run_c2_independent_corpus_r13", "run_c2_semantic_corpus_r13",
    "c2_matched_conditions_r13", "c2_independent_marginal_guard_r13",
    "supervised_learnability_run_r13", "run_generator_stress_r13",
    "run_c2_density_diagnostics_r13", "run_c2_diagnostics_r13",
    "load_selected_pack", "SEMANTIC_BLOCKS_PER_CORPUS_R13",
    "REHEARSAL_FIT_PAIRS_PER_RUNG_R13",
]
