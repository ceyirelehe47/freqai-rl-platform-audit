# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R15:cluster-aware cue 语义评估器(unique event)。
...
- §14/§15 dedicated semantic corpus(160 matched blocks/corpus,
  ...
  R15 design FAIL,不进入 candidate 设计;
...
R15_CUE_BOOTSTRAP_RESAMPLES = 20000
R15_CUE_BOOTSTRAP_SEED = 20261203
...
def cluster_bootstrap_rate(
        ...
        n_boot: int = R15_CUE_BOOTSTRAP_RESAMPLES,
        seed: int = R15_CUE_BOOTSTRAP_SEED,
) -> dict[str, Any]:
...
def semantic_cue_gate(
    ...
    - tail-position 诊断(报告,不作独立 gate——audit 阶段已专项
      闭合)。
    任一 FAIL => R15 design FAIL(不进入 candidate 设计)。
    """
    ...
    return {
        "format": "cur261-r15-semantic-cue-gate-v1",
        "label": label,
...
def candidate_cue_semantics(
    ...
    return {
        "format": "cur261-r15-candidate-cue-semantics-v1",
...
def cue_semantic_rule_identity() -> str:
    """R15 cue 语义 gate 规则身份(冻结;plan/pack 绑定)。"""
    payload = {
        ...
        "bootstrap_resamples": R15_CUE_BOOTSTRAP_RESAMPLES,
        "bootstrap_seed": R15_CUE_BOOTSTRAP_SEED,
...
def independent_cue_semantics(
    ...
    R15 变更(GateTopologyReconciliation-v2;修复 R14 隐藏双绑定):
    ...
    # ---- R15 §五:structural(binding)与 cue point diagnostics 分离 ----
    ...
    # source。R15 权威语义:independent marginal 的 binding 职责仅
    ...
    return {
        ...
        "structural": {
            ...
            "note": (
                "R15 §五:independent marginal 的 binding 职责仅结构"
        ...
        "cue_point_diagnostics": {
            ...
            "note": (
                ...
                "pass——R15 永久移除);诊断 FAIL 不得改变 independent "
                "marginal binding result 与 final verdict"),
        },
        # 顶层 pass = structural(binding 语义)。消费方
        # (c2_independent_marginal_guard_r15)不得读取任何 cue
        # 点指标进入 guard.pass。
        "pass": structural_pass,
    }
