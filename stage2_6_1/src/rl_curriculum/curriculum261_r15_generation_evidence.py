# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R15:GenerationEvidenceCompleteness-v1(§15)。
...
治理层(R15 新增,orchestrator 重新计算,不信 recorder 自报):
...
COMPLETENESS_FORMAT = "cur261-r15-generation-evidence-completeness-v1"
R15_ITERATION = "r15"
...
def verify_generation_evidence_completeness(
    ...
    for r in rows:
        ...
        if env.get("iteration") != R15_ITERATION:
    ...
    if iteration_mismatch:
        problems.append(
            f"{iteration_mismatch} 行 iteration != r15(混入历史迭代)")
    # ---- 逐 call 完整性 + 坐标计数 ----
...
def generation_evidence_completeness_digest(result: dict[str, Any]) -> str:
    ...
    return "r15gec-" + hashlib.sha256(json.dumps(
