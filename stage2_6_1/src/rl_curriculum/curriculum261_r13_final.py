# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R13:一次性 final qualification(§27-§29)。
...
9. marker -> completed;任何异常 -> marker -> crashed + crash evidence
   落盘 + re-raise(保留 marker,禁止修复后复用 qualification_r13)。
...
from rl_curriculum.curriculum261_r13_calibration import (
    C2_INDEPENDENT_PAIRS_PER_RUNG_R13,
    SEMANTIC_BLOCKS_PER_CORPUS_R13,
...
FINAL_FORMAT_R13 = "cur261-r13-final-qualification-v1"
...
from rl_curriculum.curriculum261_r13_orchestrator import (
    CALIBRATION_PAIRS_PER_RUNG_R13,
    R13_SUPERVISED_MODEL_SEEDS,
)


def qualification_r13_lock_dir_default() -> Path:
    from rl_curriculum.curriculum261_r13_namespaces import (
...
def _fresh_seed_validity_r13(n_checks: int = 10) -> dict[str, Any]:
    """fresh_holdout_r13 与 qualification_r13 的 seed 不相交验证。"""
    from rl_curriculum.curriculum261_api import derive261_seed

    mismatches = []
    for i in range(n_checks):
        for family in CURRICULUM261_FAMILIES:
            q = derive261_seed("qualification_r13", family, "D2", i, 0)
            for rung in CURRICULUM261_RUNGS:
                f = derive261_seed("fresh_holdout_r13", family, rung, i, 0)
                if q == f:
...
def run_final_qualification_r13(out_dir: Path,
                               vendor_dir: Path | None = None,
                               *,
                               rehearsal_profile: dict[str, Any] | None
                               = None,
                               ) -> dict[str, Any]:
    """执行一次性 R13 final qualification(§28)。

    rehearsal_profile(R13RealArtifactCliRoundTrip-v1;§四-4):非 None
    ...
    与样本量来自 rehearsal_profile(rt_*_r13 rehearsal-only namespace
    + 缩小规模;verdict 在缩小规模下不作资格判定,artifact 写盘供
    PPO smoke / 下游 reader 真实读取)。exposure marker 写入
    rehearsal 隔离目录(CURRICULUM261_R13_LOCK_DIR 指向 rehearsal
    目录;正式 namespace 不消耗)。
    """
    rt = rehearsal_profile is not None
    require_r13_iteration_active()
    if qualification_r13_exposed():
        raise RuntimeError(
            "R13 final qualification 已执行过(exposure marker/ledger "
            "存在)——同一 qualification_r13 corpus 不得再次执行;继续必须"
            "使用新迭代身份(R13)与全新 seed space")
    plan, digest = load_locked_plan_r13()
    if (plan.get("robustness_gate") or {}).get("pass") is not True:
        raise RuntimeError(
            "R13 plan 的 robustness_gate.pass != true——final "
            "qualification fail closed")
    pack = load_selected_pack(qualification_r13_lock_dir_default())
    ...
    code_ok = (plan["code_identity"] == _code_identity_r13())
    ...
    if not static_ok:
        raise RuntimeError(
            ...
            "之前 fail closed——本轮 qualification_r13 未消耗")
    ...
    if not freeze["pass"]:
        raise RuntimeError(
            f"R13 code freeze 校验失败(正式数据开始后源码漂移;"
            f"§6/§21 永久结束):{freeze}")
    ...
    if rt:
        ...
        core_kwargs = dict(
            ...
            final_namespace="qualification_r13",
            fit_namespace="preprocess_fit_qualification_r13",
    ...
    try:
        with QualificationR13FileLock(blocking=False):
            write_qualification_r13_exposure(digest, status="running")
            result = execute_final_core_r13(
        ...
        try:
            (out_dir / "qualification_crash_traceback.log").write_text(
                exc, encoding="utf-8")
            write_qualification_r13_exposure(digest, status="crashed")
        except Exception:  # noqa: BLE001 - 崩溃路径不得掩盖原始异常
            pass
        raise


def execute_final_core_r13(
        out_dir: Path, plan: dict[str, Any], pack: dict[str, Any], *,
        digest: str = "", started: str | None = None,
        profile_name: str = "formal_final",
        final_namespace: str = "qualification_r13",
        fit_namespace: str = "preprocess_fit_qualification_r13",
    ...
    正式:run_final_qualification_r13(exposure/plan/freeze 治理外壳)
    以 qualification_r13 全量参数调用;
    rehearsal:preplan 以 preplan_final_r13 + 微样本参数 + 临时
    exposure 目录调用(§12.2 final-like runner);
    shadow:full-scale shadow(工作包 C)以 shadow_fit_main_r13 +
    ...
    *_override 参数仅供 shadow(工程覆盖;正式路径不传,保持
    ...
    - reference equivalence 走 canonical 合同(reference_equivalence_
      run_r13:canonical vs scaled 逐位 + legacy 差异 + 0 unexplained),
    ...
    with sink:
        return _execute_final_core_inner_r13(
            out_dir, plan, pack, digest=digest, started=started,
...
def _execute_final_core_inner_r13(
        out_dir: Path, plan: dict[str, Any], pack: dict[str, Any], *,
        digest: str = "", started: str | None = None,
        profile_name: str = "formal_final",
        final_namespace: str = "qualification_r13",
        ...
        conditioning_fit_namespace: str | None = None,
        semantic_out_dir: Path | None = None) -> dict[str, Any]:
    """execute_final_core_r13 的执行核心(envelope sink 内)。"""
    kappa = float(plan["statistics_rule"]["kappa"]) \
        if "statistics_rule" in plan else 1.5
    if c2_blocks is None:
        c2_blocks = int(plan["final_sample_counts"]["c2_matched_blocks"])
    ...
    final_records = generate_fit_bank_r13(fit_namespace, pack)
    final_v2, final_manifest = fit_preprocessor_v2_from_bank_r13(
        fit_namespace, pack, records=final_records,
        parameter_pack_identity=pack["digest"])
    routing_final = build_routing_r13(
    ...
    space_validation = validate_observation_space_v2(
        ...
        [int(r.episodes[s].spec.seed) for r in contain_records
         for s in ("A", "B")], context="final_fit_space_v2_r13")
    ...
    for family in CURRICULUM261_FAMILIES:
        ...
        family_reports[family] = rung_report_r4(
            records, family, r13_family_rung_params(family, pack),
    ...
    c2_indep = run_c2_independent_corpus_r13(
        ...
        or ("c2_independent_qualification_r13" if not rehearsal
            else final_namespace),
        pairs_per_rung=int(independent_pairs_per_rung))
    c2_conditions = c2_matched_conditions_r13(c2_matched, pack)
    c2_marginal = c2_independent_marginal_guard_r13(
        c2_indep, pack, recall_floor_value)
    semantic_ns = semantic_namespace_override or (
        "cue_semantic_qualification_r13" if not rehearsal
        else "preplan_semantic_main_r13")
    ...
    semantic = run_c2_semantic_corpus_r13(
        ...
        artifact_name=semantic_artifact_filename_r13(semantic_ns))
    ...
    reference_report = reference_equivalence_run_r13(
    ...
    if not rehearsal or shadow or rt:
        causality = {
            ...
            "reference_causality": [
                check_reference_causality(
                    f, r13_family_rung_params(f, pack)["D2"],
                    thresholds[f])
                for f in CURRICULUM261_FAMILIES],
        }
    repro = []
    if not rehearsal or shadow or rt:
        ...
        repro_pack_override = []
        for family in CURRICULUM261_FAMILIES:
            ov = r13_override_for(family, pack) or {}
        ...
        c2_ladder = r13_family_rung_params(FAMILY_C2, pack)
    ...
    fresh = (_fresh_seed_validity_r13(10)
    ...
    cond_records = generate_fit_bank_r13(
        conditioning_fit_namespace or "preprocess_fit_calibration_r13",
    ...
    supervised_ns = supervised_namespace_override or (
        ...
        else "preplan_supervised_main_r13")
    supervised = supervised_learnability_run_r13(
        final_v2, pack, namespace=supervised_ns,
        pairs_per_rung=(supervised_pairs_per_rung_override
                        or (CALIBRATION_PAIRS_PER_RUNG_R13
