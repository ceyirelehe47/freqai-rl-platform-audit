# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R14:live signature audit 与委托合同(§7)。
...
- AST 检查(§7.4):R14 源码中 supervised delegate 无第三位置参数、
...
DELEGATION_TABLE_R14: tuple[tuple[str, str, str | None,
                                  tuple[str, ...], tuple[str, ...],
                                  tuple[str, ...]], ...] = (
    ("supervised runner",
    ...
    ("supervised mlp trainer",
     ...
     ("net",)),
    ("fit-bank builder",
     "rl_curriculum.curriculum261_r14_calibration:generate_fit_bank_r14",
    ...
    ("robustness builder",
     ...
     "preprocessing_robustness_checks_r14",
     None,
     ("profile", "ledger"),
     ("profile", "ledger"),
     ("checks", "pass", "routing_matrix")),
    ("final runner",
     "rl_curriculum.curriculum261_r14_final:run_final_qualification_r14",
     None,
     ("vendor_dir",),
     (),
     ("verdict",)),
    ("full-scale shadow runner",
     "rl_curriculum.curriculum261_r14_shadow:run_full_scale_shadow_r14",
     None,
     ("run_tag",),
     ("run_tag",),
     ("orchestration_stage_completed", "final_like_executed")),
    ("shadow comparison runner",
     "rl_curriculum.curriculum261_r14_shadow:compare_full_scale_shadow_runs",
     None,
     (),
     (),
...
#: §7.4 AST 检查的 R14 模块(源码级委托合同)。
AST_MODULES_R14: tuple[str, ...] = (
...
def live_signature_audit_r14() -> dict[str, Any]:
    ...
    digest_payload = json.dumps(entries, sort_keys=True,
                                ensure_ascii=False)
    return {
        "format": "cur261-r14-delegation-signature-audit-v1",
        "iteration": "r14",
        "entries": entries,
        "all_pass": bool(all_ok),
        "digest": "r14sig-" + hashlib.sha256(
...
def _ast_call_violations(tree: ast.AST, src_name: str) -> list[str]:
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(fn, ast.Name):
            ...
            name = fn.attr
        if name == "supervised_learnability_run_r14":
            # §7.4:CLI/编排必须显式传 namespace=
            kw_names = [k.arg for k in node.keywords]
            if "namespace" not in kw_names:
                violations.append(
                    f"{src_name}:supervised_learnability_run_r14 调用"
...
def delegation_ast_checks_r14() -> dict[str, Any]:
    ...
    # R6 旧错误形态的负向断言:R14 模块不得复现
    # _supervised_run(preproc, pack, namespace) 位置第三参(来自 R9 的
    # wrapper 源码模式);R14 的 supervised 实现不经位置参数委托。
    calib = (pkg_dir / "curriculum261_r14_calibration.py")
    if calib.is_file():
    ...
    return {
        "format": "cur261-r14-delegation-ast-checks-v1",
        "iteration": "r14",
...
def calibration_call_contract_payload() -> dict[str, Any]:
    ...
    return {
        "format": "cur261-r14-calibration-call-contract-v1",
        "iteration": "r14",
        "r9_defects_closed": [
        ...
        "supervised_entrypoint": {
            "symbol": ("rl_curriculum.curriculum261_r14_calibration"
                       ".supervised_learnability_run_r14"),
