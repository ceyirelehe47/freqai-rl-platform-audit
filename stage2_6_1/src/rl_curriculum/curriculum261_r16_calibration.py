...
from rl_curriculum.curriculum261_r16_labels import (
    SUPERVISED_LABEL_CONTRACT,
    collect_policy_visible_dataset_r16,
    supervised_dataset_identity_r16,
)
...
REHEARSAL_FIT_PAIRS_PER_RUNG_R16 = 2


def generate_fit_bank_r16(
...
def fit_preprocessor_v2_from_bank_r16(
        namespace: str, pack: dict[str, Any], *,
...
def run_calibration_corpus_c13_r16(
        preproc_v2: Any, pack: dict[str, Any], namespace: str, *,
        pairs_per_rung: int = CALIBRATION_PAIRS_PER_RUNG_R16,
) -> dict[str, Any]:
...
def run_c2_independent_corpus_r16(
        preproc_v2: Any, pack: dict[str, Any], namespace: str, *,
        pairs_per_rung: int = C2_INDEPENDENT_PAIRS_PER_RUNG_R16,
) -> dict[str, Any]:
...
def run_c2_semantic_corpus_r16(
        pack: dict[str, Any], namespace: str, *,
        n_blocks: int = SEMANTIC_BLOCKS_PER_CORPUS_R16,
        out_dir: Path | None = None,
        artifact_name: str | None = None) -> dict[str, Any]:
    """§18/§19 dedicated cue semantic corpus(R9 语义;R16 namespace)。
    ...
    ladder = r16_family_rung_params(FAMILY_C2, pack)
    ...
    # R16 §六 binding lineage 自报:四类 cue rate metric 的唯一
    # binding source;leaf 清单与注册表
    # R16_DEDICATED_SEMANTIC_LEAVES 声明逐字一致
    # (r16_binding_lineage 的 actual 来源;pass 由下方
    ...
    result = {
        "format": "cur261-r16-semantic-corpus-v1",
        "namespace": namespace,
    ...
    if out_dir is not None:
        ...
        expected_name = semantic_artifact_filename_r16(namespace)
...
def c2_matched_conditions_r16(
    ...
    checks = {
        ...
        "cue_semantics_delegated_note": (
            ...
            "160-block semantic corpus(cue_semantic_*_r16 namespace)"),
    }
    # R16 §六 binding lineage 自报:进入本 pass 的全部原子 bool 叶子
    # (与注册表 R16_MATCHED_STRICT_LEAVES 声明逐字一致;
    # pass 从 checks 的 bool 子集派生——同源于本清单)。
    binding_leaf_checks = sorted(
        k for k, v in checks.items() if isinstance(v, bool))
    return {
        "format": "cur261-r16-c2-matched-conditions-v1",
        "statistical": base,
...
def c2_independent_marginal_guard_r16(
        indep: dict[str, Any], pack: dict[str, Any],
        recall_floor_value: float,
) -> dict[str, Any]:
    """C2 independent marginal guard v2(R16 §五)。

    R16 权威语义(GateTopologyReconciliation-v2;修复 R14 隐藏双绑定):
    ...
    guard = dict(base)
    guard["format"] = "cur261-r16-c2-marginal-guard-v2"
    guard["r16_semantics_rule"] = (
        ...
        "独立语料上仅 diagnostic——不进入本 pass(R16 §五;"
    ...
    guard["semantics_pass"] = guard["r16_semantics_pass"]
    guard["pass"] = bool(all(leaf_values.values()))
    return {
        "format": "cur261-r16-c2-independent-marginal-v2",
...
# ---------------------------------------------------- supervised(§8/§17)
def supervised_learnability_run_r16(
        preproc_v2: Any, pack: dict[str, Any], *,
        namespace: str,
        pairs_per_rung: int = CALIBRATION_PAIRS_PER_RUNG_R16,
        train_pair_limit: int = 6,
        model_seeds: tuple | list = R16_SUPERVISED_MODEL_SEEDS,
    ...
    for family in CURRICULUM261_FAMILIES:
        ...
        rung_params = r16_family_rung_params(family, pack)
        dataset = collect_policy_visible_dataset_r16(
        ...
        controls = ["U", "W", "B"]
        gated_controls = list(R16_SUPERVISED_GATE["gated_controls"])
        seed_reports: list[dict[str, Any]] = []
        # ---- repair R16(B2):distinct model-seed 计数 ----
        # R10 缺陷:n_passing 把 W/B 每个通过的 run 各计 1 —— 同一
        # model seed 的 W+B 双通过被误算成"两个 seeds 通过"
        # (control-run count 冒充 seed count)。R16 合同:对每个
        ...
        for seed in model_seeds:
            for control in controls:
                ...
                gated = bool(
                    ...
                    >= R16_SUPERVISED_GATE["behavior_gap_min"])
        ...
        control_pass = {
            ...
            >= R16_SUPERVISED_GATE["min_seeds_passing"]
        ...
        out["families"][family] = {
            ...
            "distinct_seed_gate": {
                "rule": (
                    ...
                    f"{R16_SUPERVISED_GATE['min_seeds_passing']}/"
                    f"{R16_SUPERVISED_GATE['n_model_seeds']} distinct "
        ...
        out.setdefault("dataset_identity", {})[family] = (
            supervised_dataset_identity_r16(dataset))
    out["alignment_all_ok"] = alignment_all
    out["pass"] = bool(overall and alignment_all)
    return out


def run_generator_stress_r16(
        pack: dict[str, Any], *,
        pairs_per_rung: int = 12,
        namespace: str = "stress_r16") -> dict[str, Any]:
    """generator stress(R6 冻结实现;namespace 显式 stress_r16)。

    R9 缺陷修复:R9 CLI 调用未传 namespace,R6 默认回落历史
    stress_r6 —— R16 wrapper 把 namespace 变为显式 keyword-only
    参数,默认 stress_r16。
    """
...
def run_c2_density_diagnostics_r16(
...
def run_c2_diagnostics_r16(records: list[Any]) -> dict[str, Any]:
...
__all__ = [
    "generate_fit_bank_r16", "fit_preprocessor_v2_from_bank_r16",
    "run_calibration_corpus_c13_r16", "run_c2_matched_corpus_r16",
    "run_c2_independent_corpus_r16", "run_c2_semantic_corpus_r16",
    "c2_matched_conditions_r16", "c2_independent_marginal_guard_r16",
    "supervised_learnability_run_r16", "run_generator_stress_r16",
    "run_c2_density_diagnostics_r16", "run_c2_diagnostics_r16",
    "load_selected_pack", "SEMANTIC_BLOCKS_PER_CORPUS_R16",
    "REHEARSAL_FIT_PAIRS_PER_RUNG_R16",
]
