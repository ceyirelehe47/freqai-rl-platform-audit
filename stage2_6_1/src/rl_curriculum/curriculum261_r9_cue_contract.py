# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R9:Cue 检测语义合同 v2 与三路闭合合同审计。
...
1. mirror candidate 边界修正:真实 paired_noise() 只有
   ...
   实现在 curriculum261_r9_noise_replay.mirror_candidate_positions
...
2. 三路独立闭合(§12;R7 只有 analytic 自证 + 单一 bridge):
   ...
   C. Direct Generator Replay:两个全新 candidate-independent audit
      corpora(cue_contract_model_r9 / cue_contract_validation_r9,
...
dedicated semantic corpus(§14/§15,curriculum261_r9_cue_eval)。
"""
...
#: 其他语义指标业务阈值(与 R6/R7 相同;R9 用 cluster bounds)。
...
#: 合同审计配置(冻结;§12)。
AUDIT_MODEL_NAMESPACE = "cue_contract_model_r9"
AUDIT_VALIDATION_NAMESPACE = "cue_contract_validation_r9"
AUDIT_BLOCKS_PER_CORPUS = 500
AUDIT_ATTEMPT = 0
AUDIT_RNG_SEED = 20261101
...
#: audit bootstrap(与 r9_cue_eval 同规格;本模块自含双侧 CI 实现)。
AUDIT_BOOTSTRAP_RESAMPLES = 20000
AUDIT_BOOTSTRAP_SEED = 20261102


#: audit 使用的 sentinel ladder(= 冻结 cur261-c2-v9 默认 D0-D3;
#: 与 R9 candidate 网格无依赖——alpha_bps/wick_kappa 不进入 cue 表/
...
# ------------------------------------------------- 三路闭合 audit(§12)
#: §R9-10 cue audit plan(audit data 生成前锁定;不可修改)。
CUE_AUDIT_PLAN_FORMAT_R9 = "cur261-r9-cue-audit-plan-v1"
CUE_AUDIT_PLAN_FILENAME = "cue_audit_plan.json"
CUE_AUDIT_PLAN_DIGEST_FILENAME = "cue_audit_plan_digest.txt"
CUE_AUDIT_CODE_MODULES_R9 = (
...
def cue_audit_code_identity_r9() -> dict[str, str]:
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    out: dict[str, str] = {}
    for name in CUE_AUDIT_CODE_MODULES_R9:
        f = root / name
...
def cue_audit_plan_payload_r9(
        code_identity: dict[str, str] | None = None) -> dict[str, Any]:
    """§R9-10:audit data 生成前锁定的 plan payload(全部预注册)。"""
    return {
        "format": CUE_AUDIT_PLAN_FORMAT_R9,
        "audit_namespaces": {
...
def load_locked_cue_audit_plan_r9(out_dir: Path) -> dict[str, Any]:
    out_dir = Path(out_dir)
    path = out_dir / CUE_AUDIT_PLAN_FILENAME
    dpath = out_dir / CUE_AUDIT_PLAN_DIGEST_FILENAME
    if not path.is_file() or not dpath.is_file():
        raise RuntimeError(
            "cue audit plan 未锁定(§R9-10:正式 audit data 生成前"
            "必须先锁定)")
    ...
    current_ident = cue_audit_code_identity_r9()
    ...
    if drift:
        raise RuntimeError(
            f"cue audit plan code identity 与当前代码漂移(§R9-10;"
            f"audit plan 锁定后相关模块不得修改):{sorted(drift)}")
    return payload


def _once_vs_attempts_bitwise_check_r9(
        blocks: list[Any], ladder: dict[str, dict[str, Any]],
        namespace: str, max_blocks: int = 50) -> dict[str, Any]:
    """§R9-11:attempts-mode 选中 attempt==0 的 block 与 once-mode 同
...
def run_cue_contract_audit(
    ...
    """执行三路闭合合同审计(在任何 R9 design/semantic data 之前)。
    ...
    if require_locked_plan:
        ...
        audit_plan = load_locked_cue_audit_plan_r9(out_dir)
    ...
    for corpus_name, ns in (("model", model_ns),
        ...
        if mode == "once":
            ...
            # §R9-10/§R9-11:validation 语料用正式 attempts-mode
            ...
            once_bitwise = _once_vs_attempts_bitwise_check_r9(
                blocks_v, ladder, ns)
    ...
    # ---- §R9-11 once-mode vs attempts-mode 系统偏差对照 ----
    ...
    report: dict[str, Any] = {
        "format": "cur261-r9-cue-contract-audit-v1",
        ...
        "mirror_bound_v2": {
            ...
            "authority": "curriculum261_r9_noise_replay."
    ...
    if out_dir is not None:
        ...
        (out_dir / "once_vs_attempts_audit.json").write_text(
            json.dumps({
                "format": "cur261-r9-once-vs-attempts-audit-v1",
                "model_mode": "once",
                "validation_mode": "attempts",
        ...
        (out_dir / "cue_k_distribution.json").write_text(json.dumps({
            "format": "cur261-r9-cue-k-distribution-v1",
        ...
        (out_dir / "tail_mirror_validation.json").write_text(json.dumps({
            "format": "cur261-r9-tail-mirror-validation-v1",
        ...
        (out_dir / "noise_replay_validation.json").write_text(json.dumps({
            "format": "cur261-r9-noise-replay-validation-v1",
            "tolerance": 1e-12,
            "model": {
...
def cue_contract_audit_digest(report: dict[str, Any]) -> str:
    ...
    blob = json.dumps(core, sort_keys=True, ensure_ascii=False,
                      default=float)
    return "r9ca-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
...
def cue_semantic_contract_digest() -> str:
    blob = json.dumps(cue_semantic_contract_payload(), sort_keys=True,
                      ensure_ascii=False)
    return "r9cue-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
