# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R8:final preflight(§25)。

A. pre-lock static:imports/vendor/Route C identity/code identity/
   V2 serialize+outer env+PPO smoke(ppo_smoke_r8)/matched generator
...
def _imports_ok_r8() -> dict[str, Any]:
    modules = [
        "rl_curriculum.curriculum261_r8_namespaces",
        "rl_curriculum.curriculum261_r8_noise_replay",
        "rl_curriculum.curriculum261_r8_cue_contract",
        "rl_curriculum.curriculum261_r8_cue_eval",
        "rl_curriculum.curriculum261_r8_param_pack",
        "rl_curriculum.curriculum261_r8_design",
        "rl_curriculum.curriculum261_r8_calibration",
        "rl_curriculum.curriculum261_r8_plan",
        "rl_curriculum.curriculum261_r8_final",
        "rl_curriculum.curriculum261_r8_smoke",
...
def _code_identity_digest_r8() -> dict[str, Any]:
    from rl_curriculum.curriculum261_r8_plan import _code_identity_r8

    ident = _code_identity_r8()
    missing = [k for k, v in ident.items() if v == "MISSING"]
    return {
        "identity": ident,
        "missing": missing,
        "digest": "r8ci-" + hashlib.sha256(
            _canonical(ident).encode("utf-8")).hexdigest(),
        "pass": not missing,
    }


def _matched_generator_probe_r8() -> dict[str, Any]:
    """matched generator + unique-event evaluator + cluster bootstrap
    小规模验证(ppo_smoke_r8 下 2 个 block;不触碰 design/cal/final)。"""
    try:
        ...
        for idx in range(2):
            blocks.append(generate_matched_block_with_attempts(
                dict(C2_RUNG_PARAMS), namespace="ppo_smoke_r8",
                block_index=idx))
...
def run_prelock_static_preflight_r8(out_dir: Path,
    ...
    checks["code_identity"] = _code_identity_digest_r8()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        old = os.environ.get("CURRICULUM261_R8_LOCK_DIR")
        os.environ["CURRICULUM261_R8_LOCK_DIR"] = td
        try:
            ...
            write_qualification_r8_exposure("r8dp-preflight-probe",
                                            "running")
            try:
                write_qualification_r8_exposure("r8dp-preflight-probe",
                                                "running")
            ...
            if old is None:
                ...
                os.environ["CURRICULUM261_R8_LOCK_DIR"] = old
    checks["marker_atomic_exclusive"] = marker_ok
    checks["concurrent_final_lock_rejected"] = lock_ok

    result = {
        "format": "cur261-r8-prelock-static-preflight-v1",
        "iteration": "r8",
        "executed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespaces_touched": ["ppo_smoke_r8"],
...
def sealed_preflight_digest(att: dict[str, Any]) -> str:
    ...
    return "r8fa-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def run_postlock_sealed_preflight_r8(out_dir: Path,
                                     vendor_pin: str,
                                     ) -> dict[str, Any]:
    """§25.2:plan lock 后 sealed preflight(零 final seed / 零 marker)。"""
    from rl_curriculum.curriculum261_r8_namespaces import (
        qualification_r8_plan_path,
        qualification_r8_digest_path,
        r8_parameter_pack_path,
    )
    ...
    checks: dict[str, Any] = {}
    plan, plan_digest = load_locked_plan_r8()
    checks["plan_digest_recompute"] = True
    checks["plan_iteration_r8"] = bool(plan.get("iteration") == "r8")
    ...
    checks["r4_inheritance_verified"] = verify_r4_inheritance_r8(pack)
    ...
    code_digest = "r8ci-" + hashlib.sha256(
        _canonical(ident).encode("utf-8")).hexdigest()
    ...
    checks["final_runner_dependencies"] = _imports_ok_r8()["pass"]
    checks["plan_path_resolves"] = bool(
        qualification_r8_plan_path().is_file()
        and qualification_r8_digest_path().is_file()
        and r8_parameter_pack_path().is_file())
    ...
    attestation = {
        "format": "cur261-r8-sealed-final-preflight-v1",
        "iteration": "r8",
        "pass": att_pass,
