# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R9 CLI(audit → cue-audit → preplan-smoke →
...
digest 自引用缺陷修复的正式验收);design plan 在第一条 R9 design/
...
BASELINE_COMMIT_R9 = "dfe646bbdd53d6053d483343c62088f45e92fb34"
...
def _default_art() -> Path:
    from rl_curriculum.curriculum261_r9_namespaces import (
        qualification_r9_lock_dir,
    )

    return qualification_r9_lock_dir()
...
def _pack(out_dir: Path) -> dict:
    from rl_curriculum.curriculum261_r9_param_pack import load_selected_pack
...
def _r8_abort_binding(out_dir: Path) -> dict:
    ...
    if marker is None or not marker.is_file():
        raise RuntimeError(
            ...
            "禁止在 marker 缺失时锁 R9 design plan)")
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    _write_json(out, "baseline_integrity.json", {
        "expected_baseline": BASELINE_COMMIT_R9,
        "release_repo_head": head,
        "baseline_matches": bool(head == BASELINE_COMMIT_R9),
    ...
    _dump_txt(out, "preprocessing_v2_contract_digest.txt",
              preprocessing_v2_contract_digest())
    records = generate_fit_bank("preplan_smoke_r9", args.fit_pairs)
    ...
    from rl_curriculum.curriculum261_r9_dependencies import (
        write_dependency_resolution_r9,
    )

    dep = write_dependency_resolution_r9(out)
...
def _official_entrypoint_validation() -> dict:
    ...
    mods = [
        "rl_curriculum.curriculum261_r9_namespaces",
        "rl_curriculum.curriculum261_r9_dependencies",
        "rl_curriculum.curriculum261_r9_noise_replay",
        "rl_curriculum.curriculum261_r9_cue_contract",
        "rl_curriculum.curriculum261_r9_cue_eval",
        "rl_curriculum.curriculum261_r9_param_pack",
        "rl_curriculum.curriculum261_r9_preplan",
        "rl_curriculum.curriculum261_r9_design",
        "rl_curriculum.curriculum261_r9_calibration",
        "rl_curriculum.curriculum261_r9_plan",
        "rl_curriculum.curriculum261_r9_preflight",
        "rl_curriculum.curriculum261_r9_final",
        "rl_curriculum.curriculum261_r9_smoke",
        "rl_curriculum.curriculum261_r9_cli",
    ]
    failed: dict[str, str] = {}
    for m in mods:
        try:
    ...
    from rl_curriculum.curriculum261_r9_cli import main as cli_main
    ...
    return {
        "format": "cur261-r9-official-entrypoint-v1",
        "entrypoint": "rl_curriculum.curriculum261_r9_cli.main",
...
def cmd_cue_audit(args: argparse.Namespace) -> int:
    ...
    out = Path(args.out_dir)
    # §R9-10:任何正式 R9 cue-audit data 生成前先锁定 audit plan
    ...
    if not (out / "cue_audit_plan.json").is_file():
        _, plan_digest = lock_cue_audit_plan_r9(out)
        print(f"[cue-audit] audit plan locked digest={plan_digest}")
    load_locked_cue_audit_plan_r9(out)
...
def cmd_preplan_smoke(args: argparse.Namespace) -> int:
    ...
    不参与参数选择;只用 preplan_smoke_r9,不用 design/calibration/
    ...
    blocks = [generate_matched_block_with_attempts(
        sentinel, namespace="preplan_smoke_r9", block_index=i)
        for i in range(3)]
    ...
    mini_gate = semantic_cue_gate(
        ...
        min_unique_positive_cues=1, label="preplan_smoke_r9")
    identity = {
        "sentinel": {r: {k: sentinel[r][k]
                         for k in ("alpha_bps", "wick_kappa")}
                     for r in sentinel},
        "sentinel_digest": "r9smoke-" + _hashlib.sha256(json.dumps(
            {r: sorted(sentinel[r].items()) for r in sorted(sentinel)},
            default=str).encode("utf-8")).hexdigest(),
        "n_blocks": len(blocks),
        "namespace": "preplan_smoke_r9",
    ...
    _write_json(out, "preplan_engineering_smoke.json", {
        "format": "cur261-r9-preplan-smoke-v1",
        "identity": identity,
    ...
    rehearsal = run_preplan_rehearsal_r9(out / "preplan")
    ...
    print("[preplan-smoke] pass(no crash;sentinel ladder only;"
          "replay+semantic-gate plumbing OK)")
    return 0
...
_SUBPROC_LOADER_DESIGN = (
    ...
    "(design_plan_digest_r9, load_locked_design_plan_r9);"
    "import json,sys;"
    "plan,digest=load_locked_design_plan_r9(sys.argv[1]);"
    "recomputed=design_plan_digest_r9(plan);"
    "print(json.dumps({"
...
_SUBPROC_LOADER_QUAL = (
    "import os,sys;"
    "os.environ['CURRICULUM261_R9_LOCK_DIR']=sys.argv[2];"
    "from rl_curriculum.curriculum261_r9_plan import "
    "(load_locked_plan_r9, plan_digest_r9);"
    "import json;"
    "plan,digest=load_locked_plan_r9();"
    "print(json.dumps({"
    "'loaded_digest':digest,"
    "'recomputed':plan_digest_r9(plan),"
    "'iteration':plan['iteration'],"
...
def _no_alternate_loader_check() -> dict:
    ...
    权威模块(r9_design 的 lock/load;r9_namespaces 的路径常量与本
    扫描自身);其余模块不得直接读 plan JSON。"""
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    allowed_design = {"curriculum261_r9_design.py",
                      "curriculum261_r9_namespaces.py",
                      "curriculum261_r9_cli.py"}
    allowed_qual = {"curriculum261_r9_namespaces.py",
                    "curriculum261_r9_plan.py",
                    "curriculum261_r9_cli.py"}
    design_hits: list[str] = []
    qual_hits: list[str] = []
    for f in sorted(root.glob("curriculum261_r9_*.py")):
        text = f.read_text(encoding="utf-8")
        if "r9_design_plan.json" in text and f.name not in allowed_design:
            design_hits.append(f.name)
        if ("qualification_plan_r9.json" in text
                and f.name not in allowed_qual):
            qual_hits.append(f.name)
    return {
        "design_plan_readers": ["curriculum261_r9_design.py"],
        "qualification_plan_readers": ["curriculum261_r9_namespaces.py"
                                       " (path fn) + "
                                       "curriculum261_r9_plan.py (loader)"],
...
def cmd_plan_roundtrip(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td)
        plan = design_plan_payload_r9(
            baseline_commit=BASELINE_COMMIT_R9,
            ...
            preplan_rehearsal_digest="r9pr-" + "0" * 64,
            r8_abort_evidence={"roundtrip_synth": True},
            cue_audit_plan_digest="r9ap-" + "0" * 64,
        )
        # build payload 的 canonical 快照(逐位一致验证基准)
        pre_digest = design_plan_digest_r9(plan)
        path, digest = lock_design_plan_r9(tmp, plan)
        ...
        checks["payload_bit_identical"] = bool(
            design_plan_digest_r9(reloaded) == pre_digest)
        # 新进程 load(真实生产路径)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        synth_pack = {
            "digest": "r9pk-" + "0" * 64,
            "pack_version": "CurriculumR9MatchedLadderPack-v1",
            "selected_c2_candidate": "c2l_midpoint",
            "selected_block_count": 15,
            "c2_ladder": {r: dict(v) for r, v in
                          C2_LADDER_CANDIDATES_R9["c2l_midpoint"].items()},
            ...
            "recall_floor": 0.93,
            "p_contract": 0.95,
            "cue_contract_audit_digest": "r9ca-" + "0" * 64,
        ...
        if qual_plan is not None:
            ...
            old_dir = os.environ.get("CURRICULUM261_R9_LOCK_DIR")
            os.environ["CURRICULUM261_R9_LOCK_DIR"] = str(tmp)
            try:
                qpath, qdigest = lock_plan_r9(qual_plan)
                checks["qualification_lock_ok"] = bool(
                    qpath.is_file()
                    and qdigest == plan_digest_r9(qual_plan))
                ...
                if old_dir is None:
                    os.environ.pop("CURRICULUM261_R9_LOCK_DIR", None)
                else:
                    os.environ["CURRICULUM261_R9_LOCK_DIR"] = old_dir
    ...
    from rl_curriculum.curriculum261_r9_design import (
        load_locked_design_plan_r9 as _unused_loader,  # noqa: F401
    )
    ...
    result = {
        "format": "cur261-r9-plan-roundtrip-validation-v1",
        "iteration": "r9",
        "checks": checks,
...
def cmd_design_plan_lock(args: argparse.Namespace) -> int:
    ...
    if not dep["pass"]:
        print(f"[design-plan-lock] 依赖解析未通过:{dep['problems'][:5]}"
              "——拒绝锁 plan(§6.3)")
        return 1
    audit_plan = load_locked_cue_audit_plan_r9(out)
    ...
    plan = design_plan_payload_r9(
        ...
        artifact_writer_identity=semantic_artifact_identity_r9(),
    ...
    path, digest = lock_design_plan_r9(out, plan)
...
def cmd_design(args: argparse.Namespace) -> int:
    ...
    out = Path(args.out_dir)
    plan, digest = load_locked_design_plan_r9(out)
    selection = run_design_stage_r9(out, plan, digest,
                                    baseline_commit=BASELINE_COMMIT_R9)
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r9_calibration import (
        c2_independent_marginal_guard_r9,
        c2_matched_conditions_r9,
        fit_preprocessor_v2_from_bank_r9,
        generate_fit_bank_r9,
        run_c2_density_diagnostics_r9,
        run_c2_diagnostics_r9,
        run_calibration_corpus_c13_r9,
        run_c2_independent_corpus_r9,
        run_c2_matched_corpus_r9,
        run_c2_semantic_corpus_r9,
        run_generator_stress_r9,
        supervised_learnability_run_r9,
    )
    ...
    from rl_curriculum.curriculum261_r9_param_pack import (
        frozen_parameter_identity_r9,
        r9_family_rung_params,
        r9_override_for,
        verify_r4_inheritance_r9,
    )

    require_r9_iteration_active()
    out = Path(args.out_dir)
    pack = _pack(out)
    inheritance = verify_r4_inheritance_r9(pack)
    if not inheritance:
        print("[calibrate] R4 inheritance 验证失败;fail closed")
        return 1
    n_blocks = int(pack["selected_block_count"])
    design_digest = (out / "r9_design_plan_digest.txt").read_text(
        encoding="utf-8").strip()
    recall_floor_value = float(pack["recall_floor"])

    print("[calibrate] fitting main preprocessor "
          "(preprocess_fit_calibration_r9)...")
    records_main = generate_fit_bank_r9(
        "preprocess_fit_calibration_r9", pack)
    v2_main, manifest_main = fit_preprocessor_v2_from_bank_r9(
        "preprocess_fit_calibration_r9", pack, records=records_main,
        parameter_pack_identity=pack["digest"])
    print("[calibrate] fitting holdout preprocessor "
          "(preprocess_fit_holdout_r9)...")
    records_hold = generate_fit_bank_r9(
        "preprocess_fit_holdout_r9", pack)
    v2_hold, manifest_hold = fit_preprocessor_v2_from_bank_r9(
        "preprocess_fit_holdout_r9", pack, records=records_hold,
        parameter_pack_identity=pack["digest"])
    ...
    _write_json(out, "fit_eval_isolation.json", {
        ...
        "holdout": {k: manifest_hold[k] for k in (
            "namespace", "pairs_per_rung", "n_pairs", "n_episodes",
            "n_rows", "integrity_all_ok", "multiset_hash")},
        "eval_namespaces": ["calibration_r9", "calibration_holdout_r9"],
        "c2_matched_namespaces": ["calibration_r9",
                                  "calibration_holdout_r9"],
        "c2_semantic_namespaces": ["cue_semantic_calibration_r9",
    ...
    eval_records = [
        generate_pair(f, r, 0, namespace="calibration_r9",
                      rung_params_override=r9_override_for(f, pack))
        for f in CURRICULUM261_FAMILIES for r in CURRICULUM261_RUNGS]
    equiv_records = [
        generate_pair(f, r, i, namespace="calibration_r9",
                      rung_params_override=r9_override_for(f, pack))
        for f in CURRICULUM261_FAMILIES for r in CURRICULUM261_RUNGS
        for i in range(3)]
    ...
    supervised = supervised_learnability_run_r9(v2_main, pack)
    _write_json(out, "supervised_learnability.json", supervised)

    print("[calibrate] C1/C3 calibration_r9 + calibration_holdout_r9...")
    calib_main_c13 = run_calibration_corpus_c13_r9(
        v2_main, pack, "calibration_r9")
    calib_hold_c13 = run_calibration_corpus_c13_r9(
        v2_main, pack, "calibration_holdout_r9")
    ...
    print("[calibrate] dedicated cue semantic corpora x2 (160 blocks)...")
    semantic_main = run_c2_semantic_corpus_r9(
        pack, "cue_semantic_calibration_r9",
        out_dir=out, artifact_name="cue_semantic_calibration.json")
    semantic_hold = run_c2_semantic_corpus_r9(
        pack, "cue_semantic_holdout_r9",
        out_dir=out, artifact_name="cue_semantic_holdout.json")
    print(f"[calibrate] semantic main pass={semantic_main['pass']} "
          f"(recall LCB="
          f"{semantic_main['shared']['recall']['bound']:.6f} "
          f"floor={recall_floor_value:.6f}); "
          f"holdout pass={semantic_hold['pass']} "
          f"(recall LCB="
          f"{semantic_hold['shared']['recall']['bound']:.6f})")

    print(f"[calibrate] C2 matched blocks x2 corpora (n={n_blocks})...")
    c2_matched_main = run_c2_matched_corpus_r9(
        v2_main, pack, "calibration_r9", n_blocks)
    c2_matched_hold = run_c2_matched_corpus_r9(
        v2_main, pack, "calibration_holdout_r9", n_blocks)
    ...
    c2_conditions_main = c2_matched_conditions_r9(c2_matched_main, pack)
    c2_conditions_hold = c2_matched_conditions_r9(
        c2_matched_hold, pack)
    ...
    c2_indep_main = run_c2_independent_corpus_r9(
        v2_main, pack, "c2_independent_calibration_r9")
    c2_indep_hold = run_c2_independent_corpus_r9(
        v2_main, pack, "c2_independent_holdout_r9")
    marginal_main = c2_independent_marginal_guard_r9(
        c2_indep_main, pack, recall_floor_value)
    marginal_hold = c2_independent_marginal_guard_r9(
    ...
    c2_diag = run_c2_diagnostics_r9(c2_sem_records)
    ...
    preprocessing_gate = {
        "format": "cur261-r9-preprocessing-robustness-gate-v1",
    ...
    # R9 curriculum gate(strict per-corpus AND + dedicated semantic
    # corpus ×2 + C2 matched(R9 条件)+ independent marginal guard)
    fam_conditions = {}
    for family in ("c1_opportunity", "c3_cost"):
        fam_conditions[family] = {
            "calibration_r9_conditions_strict": corpus_conditions_r6_pair(
                calib_main_c13["families"][family],
                ROBUSTNESS_KAPPA_R6),
            "calibration_holdout_r9_conditions_strict":
                corpus_conditions_r6_pair(
                    calib_hold_c13["families"][family],
                    ROBUSTNESS_KAPPA_R6),
        }
    curriculum_gate = {
        "format": "cur261-r9-curriculum-robustness-gate-v1",
        "iteration": "r9",
        "kappa": ROBUSTNESS_KAPPA_R6,
        "rule": "strict per-corpus AND + dedicated semantic corpus ×2 "
                "(160 blocks,selected ladder)+ C2 matched block + "
                "independent marginal guard;唯一口径;无 pooled 救援"
                "(§28/§29)",
        "families": fam_conditions,
        "c2_dedicated_semantic": {
            "calibration_r9": {
                "namespace": semantic_main["namespace"],
                "recall_lcb": semantic_main["shared"]["recall"]["bound"],
                "recall_floor": recall_floor_value,
                "noncue_fp_ucb": semantic_main["shared"][
                    "noncue_false_positive"]["bound"],
                "n_unique_positive_cues": semantic_main["shared"][
                    "n_unique_positive_cues"],
                "candidate_pass": semantic_main["candidate"]["pass"],
                "shared_pass": semantic_main["shared"]["pass"],
                "pass": semantic_main["pass"],
            },
            "calibration_holdout_r9": {
                "namespace": semantic_hold["namespace"],
                "recall_lcb": semantic_hold["shared"]["recall"]["bound"],
                "recall_floor": recall_floor_value,
                "noncue_fp_ucb": semantic_hold["shared"][
                    "noncue_false_positive"]["bound"],
                "n_unique_positive_cues": semantic_hold["shared"][
                    "n_unique_positive_cues"],
                "candidate_pass": semantic_hold["candidate"]["pass"],
                "shared_pass": semantic_hold["shared"]["pass"],
                "pass": semantic_hold["pass"],
            },
        },
        "c2_matched": {
            "calibration_r9_conditions": c2_conditions_main,
            "calibration_holdout_r9_conditions": c2_conditions_hold,
        },
        "c2_marginal": {
    ...
    curriculum_gate["pass"] = bool(
        all(cond_["calibration_r9_conditions_strict"]["pass"]
            and cond_["calibration_holdout_r9_conditions_strict"]["pass"]
            for cond_ in fam_conditions.values())
        and semantic_main["pass"] and semantic_hold["pass"]
    ...
    overall = {
        "format": "cur261-r9-robustness-gate-v1",
        "iteration": "r9",
        "preprocessing_gate": {
        ...
        "curriculum_gate": {
            ...
            "rule": "strict per-corpus AND + dedicated semantic corpus ×2"
                    " + C2 matched block + independent marginal guard"
                    "(唯一口径;无 pooled 救援)"},
        "supervised_gate": {
            "pass": supervised["pass"],
            "note": "representation gate(§29 PASS 条件)"},
        "c2_density_gate": {"pass": c2_density["pass"]},
        "c2_marginal_guard": {
            "main": marginal_main["guard"]["pass"],
            "holdout": marginal_hold["guard"]["pass"]},
        "c2_dedicated_semantic": {
            "main": semantic_main["pass"],
            "holdout": semantic_hold["pass"]},
    ...
    _write_json(out, "calibration_evidence.json", {
        ...
        "rung_params_by_family": {
            f: r9_family_rung_params(f, pack)
            for f in ("c1_opportunity", "c2_context", "c3_cost")},
        "frozen_parameter_identity": frozen_parameter_identity_r9(),
        "reference_thresholds_by_family": {
            f: dict(family_specs()[f].reference_defaults)
    ...
    _write_json(out, "calibration_summary.json", {
        "format": "cur261-r9-calibration-summary-v1",
        "c1_c3_main": {f: {
            "difficulty_ladder":
                calib_main_c13["families"][f]["difficulty_ladder"],
            "conditions": fam_conditions[f][
                "calibration_r9_conditions_strict"]}
            for f in ("c1_opportunity", "c3_cost")},
    ...
    _write_json(out, "calibration_holdout_summary.json", {
        "format": "cur261-r9-calibration-holdout-summary-v1",
        "c1_c3_holdout": {f: {
            "difficulty_ladder":
    ...
    print(f"[calibrate] preprocessing gate = "
          ...
          f"{curriculum_gate['pass']}; semantic corpora = "
          f"{semantic_main['pass']}/{semantic_hold['pass']}; "
          f"supervised = {supervised['pass']}; "
          f"density = {c2_density['pass']}")
    if not overall["pass"]:
        print("[calibrate] robustness gate FAIL——禁止 lock plan(§29);"
              "R9 = FAIL,修复须 R10 + 全新 namespaces")
        return 1
    return 0
...
def cmd_preflight_static(args: argparse.Namespace) -> int:
    ...
    result = run_prelock_static_preflight_r9(Path(args.out_dir),
                                             VENDOR_PIN)
...
def cmd_preflight_sealed(args: argparse.Namespace) -> int:
    ...
    att = run_postlock_sealed_preflight_r9(Path(args.out_dir),
                                           VENDOR_PIN)
...
def cmd_qualify(args: argparse.Namespace) -> int:
    ...
    result = run_final_qualification_r9(Path(args.out_dir))
...
def cmd_smoke(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r9_smoke import run_ppo_smoke_r9
    ...
    try:
        ...
        pass
    smoke = run_ppo_smoke_r9(pack=pack)
...
def cmd_namespace_integrity(args: argparse.Namespace) -> int:
    ...
    print(f"[namespace-integrity] pass={ns.get('pass')} "
          f"namespaces={len(ns.get('r9_namespaces', []))}")
    return 0 if ns.get("pass") else 1


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="r9-cli")
