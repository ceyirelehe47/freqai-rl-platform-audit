# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R6:256-step PPO plumbing smoke(§34)。

只验证:V2 fit state 加载(ppo_smoke_r6 非 final namespace fit)、
...
§34:model manifest 必须绑定 preprocessor bundle hash + R6 parameter
pack digest + observation identity(不得只绑定旧 r3pre-* state hash)。
"""
...
def run_ppo_smoke_r6(
    ...
    if envelope_path is not None and Path(envelope_path).is_file():
        ...
        preproc, _ = fit_preprocessor_v2_from_bank_r6(
            "ppo_smoke_r6", pack or {"digest": "ppo-smoke-no-pack"})
    ...
    if pack:
        ...
        override = r6_override_for(FAMILY_C2, pack)
    rec = generate_pair(
        FAMILY_C2, "D1", 0, namespace="ppo_smoke_r6",
    ...
    features = select_features_strict(
        scaled_ep.df, schema, context="ppo_smoke_r6")
    ...
    checks = {
        ...
        "r6_parameter_pack_digest_bound": (
    ...
    return {
        "format": "cur261-r6-ppo-256step-smoke-v1",
        "iteration": "r6",
        "namespace": "ppo_smoke_r6",
        "n_steps": 256,
        "model_manifest": {
            "preprocessor_bundle_hash": preproc.bundle_hash,
            "r6_parameter_pack_digest": checks[
                "r6_parameter_pack_digest_bound"],
            ...
            "note": "§34:manifest 绑定 bundle hash + R6 pack digest + "
                    "observation identity(不止 r3pre-* state hash)",
        },
        "checks": checks,
