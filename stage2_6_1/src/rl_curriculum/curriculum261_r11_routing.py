# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R11:main/holdout/final bundle routing 合同(§9)。
...
R11 修复(§9):
- 显式 R11BundleRouting 数据类:role / fit namespace / 三层 bundle hash,
...
- RoutingLedgerR11 落盘完整路由矩阵(§9.4):corpus → expected fit
...
R11_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "preprocess_fit_calibration_r11",
    "holdout": "preprocess_fit_holdout_r11",
    "final": "preprocess_fit_qualification_r11",
}

#: §9.1 preplan/rehearsal 路由(execution profile 只换 namespace)。
R11_PREPLAN_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "preplan_fit_main_r11",
    "holdout": "preplan_fit_holdout_r11",
    "final": "preplan_fit_main_r11",  # rehearsal final-like 复用 main fit
    "diagnostic": "preplan_fit_main_r11",
}

#: repair R11(工作包 C):full-scale shadow rehearsal 路由(工程、
#: 非正式;两次独立 cold run;不产生任何参数选择数据)。
R11_SHADOW_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "shadow_fit_main_r11",
    "holdout": "shadow_fit_holdout_r11",
    "final": "shadow_fit_main_r11",  # final-like shadow 复用 main fit
}

#: 评估 namespace → 期望 role(§9.1;正式 + preplan + shadow)。
R11_EVAL_NAMESPACE_ROLE: dict[str, str] = {
    # ---- 正式 ----
    "calibration_r11": "main",
    "calibration_holdout_r11": "holdout",
    "qualification_r11": "final",
    "c2_independent_calibration_r11": "main",
    "c2_independent_holdout_r11": "holdout",
    "c2_independent_qualification_r11": "final",
    "supervised_main_r11": "main",
    "supervised_holdout_r11": "holdout",
    "cue_semantic_calibration_r11": "main",
    "cue_semantic_holdout_r11": "holdout",
    "cue_semantic_qualification_r11": "final",
    # ---- preplan / rehearsal / diagnostic(非正式)----
    "reference_diagnostic_main_r11": "diagnostic",
    "reference_diagnostic_holdout_r11": "diagnostic",
    "preplan_smoke_r11": "diagnostic",
    "preplan_candidate_eval_r11": "diagnostic",
    "preplan_supervised_main_r11": "main",
    "preplan_supervised_holdout_r11": "holdout",
    "preplan_calibration_main_r11": "main",
    "preplan_calibration_holdout_r11": "holdout",
    "preplan_final_r11": "final",
    # ---- full-scale shadow(工程;R11 工作包 C)----
    "shadow_supervised_main_r11": "main",
    "shadow_supervised_holdout_r11": "holdout",
    "shadow_calibration_main_r11": "main",
    "shadow_calibration_holdout_r11": "holdout",
    "shadow_semantic_main_r11": "main",
    "shadow_semantic_validation_r11": "holdout",
    "shadow_c2_independent_main_r11": "main",
    "shadow_c2_independent_holdout_r11": "holdout",
    "shadow_semantic_final_r11": "final",
}
...
class R11BundleRouting:
    ...
    _v2: Any
    preplan: bool = False
    #: repair R11(工作包 C):shadow 路由类(非正式;工程 rehearsal)
    shadow: bool = False

    @property
    def nonformal(self) -> bool:
        """非正式路由(preplan rehearsal 或 full-scale shadow)。"""
        return self.preplan or self.shadow

    def bundle(self, *, expected_role: str | None = None,
               expected_fit_namespace: str | None = None,
               expected_bundle_hash: str | None = None,
               context: str = "",
               ledger: "RoutingLedgerR11 | None" = None) -> Any:
...
def build_routing_r11(
        role: str, v2: Any, *,
        preplan: bool = False,
        shadow: bool = False,
        expected_bundle_hash: str | None = None) -> R11BundleRouting:
    ...
    repair R11:shadow=True 走 full-scale shadow 路由表(工程)。
    """
    if role not in _ALLOWED_ROLES:
        raise RoutingContractError(f"未知 routing role: {role}")
    if preplan and shadow:
        raise RoutingContractError(
            "preplan 与 shadow 路由类互斥(不得同时为真)")
    table = (R11_SHADOW_ROLE_FIT_NAMESPACE if shadow
             else R11_PREPLAN_ROLE_FIT_NAMESPACE if preplan
             else R11_ROLE_FIT_NAMESPACE)
    ...
    if v2.namespace not in (list(R11_ROLE_FIT_NAMESPACE.values())
                            + list(
                                R11_PREPLAN_ROLE_FIT_NAMESPACE.values())
                            + list(
                                R11_SHADOW_ROLE_FIT_NAMESPACE.values())):
        raise RoutingContractError(
            f"bundle fit namespace {v2.namespace} 不属于任何 R11 路由表"
            f"(fail closed)")
    if expected_bundle_hash is not None \
...
def require_eval_routing_r11(
        routing: R11BundleRouting, eval_namespace: str, *,
        context: str = "",
        ledger: "RoutingLedgerR11 | None" = None) -> Any:
    ...
    拒绝(§9.3)。repair R11:非正式路由类(preplan/shadow)不得服务
    正式评估 namespace;正式路由不得服务任何非正式 namespace。
    """
    _NONFORMAL_PREFIXES = ("preplan_", "reference_diagnostic", "shadow_")
    expected_role = R11_EVAL_NAMESPACE_ROLE.get(eval_namespace)
    if expected_role is None:
        raise RoutingContractError(
            f"评估 namespace {eval_namespace} 不在 R11 权威路由映射中"
            f"(fail closed;context={context})")
    table = (R11_SHADOW_ROLE_FIT_NAMESPACE if routing.shadow
             else R11_PREPLAN_ROLE_FIT_NAMESPACE if routing.preplan
             else R11_ROLE_FIT_NAMESPACE)
...
def bundle_routing_contract_digest() -> str:
    ...
    return "r11rt-" + hashlib.sha256(json.dumps(
...
def bundle_routing_contract_payload() -> dict[str, Any]:
    ...
    return {
        "format": "cur261-r11-bundle-routing-contract-v1",
        "iteration": "r11",
        "formal_role_fit_namespace": dict(R11_ROLE_FIT_NAMESPACE),
        "preplan_role_fit_namespace": dict(R11_PREPLAN_ROLE_FIT_NAMESPACE),
        "eval_namespace_role": dict(R11_EVAL_NAMESPACE_ROLE),
        "fail_closed_rules": [
