# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R9:一次性 final qualification(§27-§29)。
...
9. marker -> completed;任何异常 -> marker -> crashed + crash evidence
   落盘 + re-raise(保留 marker,禁止修复后复用 qualification_r9)。
...
from rl_curriculum.curriculum261_r9_calibration import (
    C2_INDEPENDENT_PAIRS_PER_RUNG_R9,
...
FINAL_FORMAT_R9 = "cur261-r9-final-qualification-v1"


def qualification_r9_lock_dir_default() -> Path:
    from rl_curriculum.curriculum261_r9_namespaces import (
...
def _fresh_seed_validity_r9(n_checks: int = 10) -> dict[str, Any]:
    """fresh_holdout_r9 与 qualification_r9 的 seed 不相交验证。"""
    from rl_curriculum.curriculum261_api import derive261_seed

    mismatches = []
    for i in range(n_checks):
        for family in CURRICULUM261_FAMILIES:
            q = derive261_seed("qualification_r9", family, "D2", i, 0)
            for rung in CURRICULUM261_RUNGS:
                f = derive261_seed("fresh_holdout_r9", family, rung, i, 0)
                if q == f:
...
def run_final_qualification_r9(out_dir: Path,
                               vendor_dir: Path | None = None,
                               ) -> dict[str, Any]:
    """执行一次性 R9 final qualification(§28)。"""
    require_r9_iteration_active()
    if qualification_r9_exposed():
        raise RuntimeError(
            "R9 final qualification 已执行过(exposure marker/ledger "
            "存在)——同一 qualification_r9 corpus 不得再次执行;继续必须"
            "使用新迭代身份(R10)与全新 seed space")
    plan, digest = load_locked_plan_r9()
    if (plan.get("robustness_gate") or {}).get("pass") is not True:
        raise RuntimeError(
            "R9 plan 的 robustness_gate.pass != true——final "
            "qualification fail closed")
    pack = load_selected_pack(qualification_r9_lock_dir_default())
    ...
    code_ok = (plan["code_identity"] == _code_identity_r9())
    ...
    if not static_ok:
        raise RuntimeError(
            ...
            "之前 fail closed——本轮 qualification_r9 未消耗")
    ...
    try:
        with QualificationR9FileLock(blocking=False):
            write_qualification_r9_exposure(digest, status="running")
            result = _execute_final_r9(
        ...
        try:
            (out_dir / "qualification_crash_traceback.log").write_text(
                exc, encoding="utf-8")
            write_qualification_r9_exposure(digest, status="crashed")
        except Exception:  # noqa: BLE001 - 崩溃路径不得掩盖原始异常
            pass
        raise


def _execute_final_r9(out_dir: Path, plan: dict[str, Any], digest: str,
    ...
    final_records = generate_fit_bank_r9(
        "preprocess_fit_qualification_r9", pack)
    final_v2, final_manifest = fit_preprocessor_v2_from_bank_r9(
        "preprocess_fit_qualification_r9", pack,
    ...
    space_validation = validate_observation_space_v2(
        ...
        [int(r.episodes[s].spec.seed) for r in contain_records
         for s in ("A", "B")], context="final_fit_space_v2_r9")
    adversarial = adversarial_out_of_range_probe(final_v2, EVAL_CFG)

    # 2) qualification_r9 C1/C3 40 pairs(pack override 生效)
    ...
    for family in CURRICULUM261_FAMILIES:
        if family == FAMILY_C2:
            continue
        override = r9_override_for(family, pack)
        records = []
        for rung in CURRICULUM261_RUNGS:
            for idx in range(10):
                records.append(generate_pair(
                    family, rung, idx, namespace="qualification_r9",
                    rung_params_override=override))
        all_records[family] = records
        family_reports[family] = rung_report_r4(
            records, family, r9_family_rung_params(family, pack),
            thresholds[family], final_v2, corpus="qualification_r9")

    # 3) C2 matched blocks(selected n)+ independent marginal 语料
    c2_matched = run_c2_matched_corpus_r9(
        final_v2, pack, "qualification_r9", n_blocks)
    c2_indep = run_c2_independent_corpus_r9(
        final_v2, pack, "c2_independent_qualification_r9",
        pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R9)
    c2_conditions = c2_matched_conditions_r9(c2_matched, pack)
    c2_marginal = c2_independent_marginal_guard_r9(
    ...
    semantic = run_c2_semantic_corpus_r9(
        pack, "cue_semantic_qualification_r9",
        n_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R9, out_dir=out_dir,
        artifact_name="qualification_cue_semantics.json")

    # 4) reference 等价:全部 core episodes(禁止抽样)
    c2_ladder = r9_family_rung_params(FAMILY_C2, pack)
    reference_eq = []
    for family in CURRICULUM261_FAMILIES:
        if family == FAMILY_C2:
            ...
            for rec in all_records[family]:
                rung_params = r9_family_rung_params(family, pack)[
                    rec.rung]
    ...
    causality = {
        "observation_causality": [
            check_observation_causality(f, "D2", 0,
                                        namespace="qualification_r9")
            for f in CURRICULUM261_FAMILIES],
        "production_feature_equivalence": [
            check_production_feature_equivalence(
                f, "D2", 0, namespace="qualification_r9")
            for f in CURRICULUM261_FAMILIES],
        "reference_causality": [
            check_reference_causality(
                f, r9_family_rung_params(f, pack)["D2"], thresholds[f])
            for f in CURRICULUM261_FAMILIES],
    }
    repro = [check_reproducibility(f, r, 0, "qualification_r9")
             for f in CURRICULUM261_FAMILIES for r in ("D1", "D2")]
    repro_pack_override = []
    for family in CURRICULUM261_FAMILIES:
        ov = r9_override_for(family, pack) or {}
    ...
    repro_blk1 = generate_matched_block_with_attempts(
        c2_ladder, namespace="qualification_r9", block_index=0)
    ...
    fresh = _fresh_seed_validity_r9(10)
    ...
    cond_records = generate_fit_bank_r9(
    ...
    supervised = supervised_learnability_run_r9(
        final_v2, pack, namespace="qualification_r9")

    # 6) 密度 + matched 语义诊断 + scrambled
    density = run_c2_density_diagnostics_r9(
        c2_matched, c2_matched, pack)
    semantics = run_c2_diagnostics_r9(
        [rec for blk in c2_matched["blocks"]
    ...
    result = {
        ...
        "completed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespace": "qualification_r9",
        "c2_independent_namespace": "c2_independent_qualification_r9",
        "selected_block_count": n_blocks,
        "cue_semantic_namespace": "cue_semantic_qualification_r9",
    ...
    _dump("qualification_preprocessor_bundle.json", {
        ...
        "namespace": "preprocess_fit_qualification_r9",
    })
