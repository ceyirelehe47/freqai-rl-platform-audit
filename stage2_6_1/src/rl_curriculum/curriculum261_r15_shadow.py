# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R15:full-scale shadow rehearsal(工作包 C)。
...
R15 修复(§C):implementation freeze 之前,以专门的 shadow
namespace 执行接近正式规模的全链 rehearsal:
- 与正式流程相同的 orchestrator(orchestrate_calibration_stage_
  r15)与核心(execute_final_core_r15);禁止 monkeypatch 正式函数;
...
SHADOW_FORMAT = "cur261-r15-full-scale-shadow-rehearsal-v1"
...
def _shadow_pack() -> dict[str, Any]:
    ...
    pack = ladder_pack_payload_r15(
        ...
        design_plan_digest="r15dp-shadow-engineering",
...
def _shadow_override_fn(family: str, pack: dict[str, Any]):
    """与正式 r15_override_for 同构的覆盖函数(shadow 工程 pack)。"""
...
def run_full_scale_shadow_r15(out_dir: Path, *, run_tag: str) -> dict:
    ...
    from rl_curriculum.curriculum261_r15_orchestrator import (
        shadow_holdout_profile_r15,
        shadow_main_profile_r15,
    )
    ...
    summary: dict[str, Any] = {
        ...
        "cardinality": {
            ...
            "formal_order_held": (
                ...
                "supervised(orchestrate_calibration_stage_r15 正式顺序)"),
            "training_reduction": (
    ...
    final_result = execute_final_core_r15(
        ...
        final_namespace="shadow_calibration_main_r15",
        fit_namespace="shadow_fit_main_r15",
        c13_pairs_per_rung=10,
        ...
        independent_namespace="shadow_c2_independent_main_r15",
        semantic_namespace_override="shadow_semantic_final_r15",
        semantic_out_dir=out_dir,
        supervised_namespace_override="shadow_supervised_main_r15",
        supervised_model_seeds_override=SHADOW_MODEL_SEEDS,
        supervised_training_config=dict(SHADOW_TRAINING_CONFIG),
        conditioning_fit_namespace="shadow_fit_main_r15")
...
def orchestrate_stage(out_dir: Path, pack: dict[str, Any],
    ...
    return orchestrate_stage_inner(
        out_dir, pack, routings, records,
        shadow_main_profile_r15(), shadow_holdout_profile_r15())
...
def orchestrate_stage_inner(out_dir: Path, pack: dict[str, Any],
    ...
    return orchestrate_calibration_stage_r15(
        out_dir, pack,
...
def compare_full_scale_shadow_runs(dir_a: Path,
    ...
    return {
        "format": "cur261-r15-full-scale-shadow-comparison-v1",
        "iteration": "r15",
...
__all__ = [
    "run_full_scale_shadow_r15", "compare_full_scale_shadow_runs",
