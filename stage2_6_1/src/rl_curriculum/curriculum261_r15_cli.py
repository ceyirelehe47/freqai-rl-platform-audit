# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R15 CLI(audit → cue-audit → preplan-smoke →
...
为冻结前工程命令,在任何正式 R15 namespace 访问之前执行)。
...
digest 自引用缺陷修复的正式验收);design plan 在第一条 R15 design/
...
#: R15 启动基线 = R13 Commit B(诚实 FAIL 结果提交);§一 ancestry 语义。
BASELINE_COMMIT_R15 = "14a889c2854571e3ab5245ef51da7c858c83f59b"
#: R14 提交链锚点(R15 baseline = R14 Commit B;§二)。
...
#: producer/consumer 落盘接口 —— R15RealArtifactCliRoundTrip-v1 的
#: 直接动因。
...
#: (curriculum261_r15_historical),不在源码手工转录文件级长 digest。
...
#: TypeError 后按 §18 硬规则诚实 FAIL;design 全部结果仅作 R15
...
#: R15 继承)。R10 design/pack 全部
#: 结果仅作 R15 development evidence。
...
def _r12_abort_binding(out_dir: Path) -> dict:
    ...
    binding = {
        ...
        "final_verdict": "FAIL(永久;统计链 cue/global K/design/"
                         ...
                         "R15 输入;R15 必须全新 namespace 重新执行)",
        "fail_reasons": [
            ...
            "永久 FAIL,下一轮 R15)",
            "治理缺口(非硬失败,但为 R15 直接动因):Commit A 前的 "
        ...
        "latent_defect_2_discovered_by_r15": (
            "R15 全仓库接口审计另发现 sealed preflight 证据文件名错配"
            ...
            "R12 未活到该步故未触发;R15 已在 Commit A 前修复"),
        "statistical_chain_status_at_failure": (
            ...
            "holdout 独立 PASS —— 全部仅作历史记录,不进入 R15"),
        "abort_marker_exists": True,
        "abort_marker_blob_matches_baseline": _blob_matches(
...
def _r13_failure_binding(out_dir: Path) -> dict:
    ...
    binding = {
        ...
        "failure_classification": (
            ...
            "c2_semantics_pass;R13 不因 R15 修订被追认/撤销/改写"),
...
def _r8_abort_binding(out_dir: Path) -> dict:
    ...
    if marker is None or not marker.is_file():
        raise RuntimeError(
            ...
            "禁止在 marker 缺失时锁 R15 design plan)")
    data = json.loads(marker.read_text(encoding="utf-8"))
...
def _code_freeze_sha(out_dir: Path) -> str:
    ...
    if not freeze["pass"]:
        raise RuntimeError(
            f"R15 code freeze 校验失败:{freeze}(design plan 必须绑定"
...
def _r14_failure_binding(out_dir: Path, heb: dict) -> dict:
    ...
    提取;R14 永久 FAIL 不因 R15 修复被追认。
    """
    checks = heb.get("checks", {})
    r14_keys = sorted(k for k in checks if k.startswith("r14_"))
    binding = {
        "format": "cur261-r15-r14-iteration-failure-binding-v1",
        "iteration": "r14",
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    if freeze_sha:
        ...
        freeze_doc = write_r15_code_freeze(
        ...
        existing = verify_r15_code_freeze(Path(args.out_dir))
    ...
    _write_json(out, "baseline_ancestry.json", {
        "format": "cur261-r15-baseline-ancestry-v1",
        "expected_baseline": BASELINE_COMMIT_R15,
    ...
    _dump_txt(out, "preprocessing_v2_contract_digest.txt",
              preprocessing_v2_contract_digest())
    records = generate_fit_bank("preplan_smoke_r15", args.fit_pairs)
    ...
    from rl_curriculum.curriculum261_r15_dependencies import (
        write_dependency_resolution_r15,
    )

    dep = write_dependency_resolution_r15(out)
    from rl_curriculum.curriculum261_r15_delegation import (
    ...
    sig_audit = live_signature_audit_r15()
    ast_checks = delegation_ast_checks_r15()
    ...
    freeze_check = verify_r15_code_freeze(out)
    ...
    _write_json(out, "r11_cue_failure_binding.json", {
        "format": "cur261-r15-r11-cue-failure-binding-v1",
        ...
        "classification": "position-wise mirror-count distribution "
                          "failure(t=226 < 264,非 tail;R15 禁止继续"
                          "称为 tail-position failure)",
        "binding_gate_in_r15": False,
    ...
    _write_json(out, "r12_iteration_failure_binding.json", {
        "format": "cur261-r15-r12-iteration-failure-binding-v1",
        "iteration": "r12",
        ...
        "consumer": "cmd_lock_plan → build_plan_r15("
        ...
        "governance_gap": "R12 pre-freeze release rehearsal 使用 "
                          ...
                          "artifact 读取(§三-2;R15 "
                          "R15RealArtifactCliRoundTrip-v1 修复)",
        "binding_gate_in_r15": True,
    ...
    workflow_validation = validate_r15_workflow()
    _write_json(out, "r15_workflow_validation.json", workflow_validation)
...
def _official_entrypoint_validation() -> dict:
    ...
    mods = [
        ...
        "rl_curriculum.curriculum261_r15_noise_replay",
        "rl_curriculum.curriculum261_r15_cue_contract",
        "rl_curriculum.curriculum261_r15_cue_eval",
        "rl_curriculum.curriculum261_r15_global_k",
        "rl_curriculum.curriculum261_r15_historical",
        "rl_curriculum.curriculum261_r15_generation_evidence",
        "rl_curriculum.curriculum261_r15_param_pack",
        "rl_curriculum.curriculum261_r15_preplan",
        "rl_curriculum.curriculum261_r15_design",
        "rl_curriculum.curriculum261_r15_calibration",
        "rl_curriculum.curriculum261_r15_plan",
        "rl_curriculum.curriculum261_r15_preflight",
        "rl_curriculum.curriculum261_r15_final",
        "rl_curriculum.curriculum261_r15_smoke",
        ...
        "rl_curriculum.curriculum261_r15_gate_topology",
        "rl_curriculum.curriculum261_r15_provenance",
        "rl_curriculum.curriculum261_r15_full_cold",
        "rl_curriculum.curriculum261_r15_workflow",
        "rl_curriculum.curriculum261_r15_fail_closure",
        "rl_curriculum.curriculum261_r15_cli",
    ]
    failed: dict[str, str] = {}
    for m in mods:
        try:
    ...
    from rl_curriculum.curriculum261_r15_cli import main as cli_main
    ...
    return {
        "format": "cur261-r15-official-entrypoint-v1",
        "entrypoint": "rl_curriculum.curriculum261_r15_cli.main",
...
def cmd_cue_audit(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "rehearsal", False):
        report = run_cue_contract_audit(
            out,
            model_namespace="rt_cue_model_r15",
            validation_namespace="rt_cue_validation_r15",
        ...
        print(f"[cue-audit][rehearsal] pass={report['pass']} "
              ...
              f"namespaces=rt_cue_model_r15/rt_cue_validation_r15")
        if not report["pass"]:
            print("[cue-audit][rehearsal] rehearsal 审计 FAIL——"
                  "R15RealArtifactCliRoundTrip-v1 不得通过")
            return 1
        return 0
    # §R15-10:任何正式 R15 cue-audit data 生成前先锁定 audit plan
    ...
    if not (out / "cue_audit_plan.json").is_file():
        _, plan_digest = lock_cue_audit_plan_r15(out)
        print(f"[cue-audit] audit plan locked digest={plan_digest}")
    load_locked_cue_audit_plan_r15(out)
...
def cmd_preplan_smoke(args: argparse.Namespace) -> int:
    ...
    不参与参数选择;只用 preplan_smoke_r15,不用 design/calibration/
    ...
    blocks = [generate_matched_block_with_attempts(
        sentinel, namespace="preplan_smoke_r15", block_index=i)
        for i in range(3)]
    ...
    mini_gate = semantic_cue_gate(
        ...
        min_unique_positive_cues=1, label="preplan_smoke_r15")
    identity = {
    ...
    _write_json(out, "preplan_engineering_smoke.json", {
        "format": "cur261-r15-preplan-smoke-v1",
        "identity": identity,
    ...
    from rl_curriculum.curriculum261_r15_preplan import (
        run_preplan_rehearsal_r15,
    )

    rehearsal = run_preplan_rehearsal_r15(out / "preplan")
    ...
    from rl_curriculum.curriculum261_r15_preplan import (
        run_reference_root_cause_diagnosis_r15,
    )

    root_cause = run_reference_root_cause_diagnosis_r15(out / "preplan")
    ...
    from rl_curriculum.curriculum261_r15_rehearsal import (
        run_preplan_full_pipeline_rehearsal_r15,
    )

    full = run_preplan_full_pipeline_rehearsal_r15(out)
    if not full["pass"]:
...
_SUBPROC_LOADER_DESIGN = (
    ...
    "(design_plan_digest_r15, load_locked_design_plan_r15);"
    "import json,sys;"
    "plan,digest=load_locked_design_plan_r15(sys.argv[1]);"
    "recomputed=design_plan_digest_r15(plan);"
    "print(json.dumps({"
...
_SUBPROC_LOADER_QUAL = (
    "import os,sys;"
    "os.environ['CURRICULUM261_R15_LOCK_DIR']=sys.argv[2];"
    "from rl_curriculum.curriculum261_r15_plan import "
    "(load_locked_plan_r15, plan_digest_r15);"
    "import json;"
    "plan,digest=load_locked_plan_r15();"
    "print(json.dumps({"
    "'loaded_digest':digest,"
    "'recomputed':plan_digest_r15(plan),"
    "'iteration':plan['iteration'],"
...
def _no_alternate_loader_check() -> dict:
    ...
    权威模块(r15_design 的 lock/load;r15_namespaces 的路径常量与本
    扫描自身);其余模块不得直接读 plan JSON。"""
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    allowed_design = {"curriculum261_r15_design.py",
    ...
    allowed_qual = {"curriculum261_r15_namespaces.py",
    ...
    qual_hits: list[str] = []
    for f in sorted(root.glob("curriculum261_r15_*.py")):
        text = f.read_text(encoding="utf-8")
        if "r15_design_plan.json" in text and f.name not in allowed_design:
            design_hits.append(f.name)
        if ("qualification_plan_r15.json" in text
                and f.name not in allowed_qual):
            qual_hits.append(f.name)
    return {
        "design_plan_readers": ["curriculum261_r15_design.py"],
        "qualification_plan_readers": ["curriculum261_r15_namespaces.py"
                                       " (path fn) + "
                                       "curriculum261_r15_plan.py (loader)"],
...
def cmd_plan_roundtrip(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r15_design import (
        ...
        design_plan_payload_r15,
        lock_design_plan_r15,
    )
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td)
        plan = design_plan_payload_r15(
            baseline_commit=BASELINE_COMMIT_R15,
        ...
        pre_digest = design_plan_digest_r15(plan)
        path, digest = lock_design_plan_r15(tmp, plan)
        ...
        checks["payload_bit_identical"] = bool(
            design_plan_digest_r15(reloaded) == pre_digest)
        # 新进程 load(真实生产路径)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        synth_pack = {
            "digest": "r15pk-" + "0" * 64,
            "pack_version": "CurriculumR15MatchedLadderPack-v1",
            "selected_c2_candidate": "c2l_midpoint",
            "selected_block_count": 15,
            "c2_ladder": {r: dict(v) for r, v in
                          C2_LADDER_CANDIDATES_R15["c2l_midpoint"].items()},
        ...
        if qual_plan is not None:
            ...
            old_dir = os.environ.get("CURRICULUM261_R15_LOCK_DIR")
            os.environ["CURRICULUM261_R15_LOCK_DIR"] = str(tmp)
            try:
                qpath, qdigest = lock_plan_r15(qual_plan)
                checks["qualification_lock_ok"] = bool(
                    qpath.is_file()
                    and qdigest == plan_digest_r15(qual_plan))
                ...
                if old_dir is None:
                    os.environ.pop("CURRICULUM261_R15_LOCK_DIR", None)
                else:
                    os.environ["CURRICULUM261_R15_LOCK_DIR"] = old_dir
    ...
    from rl_curriculum.curriculum261_r15_design import (
        load_locked_design_plan_r15 as _unused_loader,  # noqa: F401
    )
    ...
    result = {
        "format": "cur261-r15-plan-roundtrip-validation-v1",
        "iteration": "r15",
        "checks": checks,
...
def cmd_design_plan_lock(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r15_dependencies import (
        resolve_dependency_identity_r15,
    )
    from rl_curriculum.curriculum261_r15_design import (
        semantic_artifact_identity_r15,
    )
    ...
    if not rehearsal.get("pass"):
        print("[design-plan-lock] preplan rehearsal 未通过——拒绝锁 "
              "plan(§9/§31)")
        return 1
    dep = resolve_dependency_identity_r15()
    if not dep["pass"]:
    ...
    if rt_mode:
        # R15RealArtifactCliRoundTrip-v1:rehearsal cue-audit 不锁定
        ...
        design_namespaces = ("rt_design_matched_main_r15",
                             "rt_design_matched_validation_r15")
        semantic_namespaces = ("rt_semantic_design_main_r15",
                               "rt_semantic_design_validation_r15")
        independent_namespace = "rt_design_independent_r15"
    else:
        audit_plan = load_locked_cue_audit_plan_r15(out)
    ...
    plan = design_plan_payload_r15(
        ...
        artifact_writer_identity=semantic_artifact_identity_r15(),
    ...
    path, digest = lock_design_plan_r15(out, plan)
...
def cmd_design(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r15_design import (
        load_locked_design_plan_r15,
        run_design_stage_r15,
    )

    out = Path(args.out_dir)
    plan, digest = load_locked_design_plan_r15(out)
    selection = run_design_stage_r15(out, plan, digest,
                                    baseline_commit=BASELINE_COMMIT_R15)
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    R15 变革(相对 R9 的手写长流程):
    - 全部评估走共享 orchestrator orchestrate_calibration_stage_r15
    ...
    from rl_curriculum.curriculum261_r15_orchestrator import (
        formal_holdout_profile_r15,
        formal_main_profile_r15,
        orchestrate_calibration_stage_r15,
    )
    from rl_curriculum.curriculum261_r15_param_pack import (
        frozen_parameter_identity_r15,
        r15_override_for,
        verify_r4_inheritance_r15,
    )
    from rl_curriculum.curriculum261_r15_routing import build_routing_r15
    from rl_curriculum.curriculum261_r3_calibration import (
    ...
    try:
        ...
        write_r15_iteration_aborted(
            ...
            f"{str(exc)[:400]});§11 硬规则:R15 永久结束,下一轮必须 "
            "R15 + 全新 namespace")
        raise
...
def _cmd_calibrate_inner(args: argparse.Namespace,
    ...
    require_r15_iteration_active()
    freeze = verify_r15_code_freeze(out)
    if not freeze["pass"]:
        print(f"[calibrate] R15 code freeze 校验失败(fail closed):"
              f"{freeze}")
        return 1
    pack = _pack(out)
    inheritance = verify_r4_inheritance_r15(pack)
    if not inheritance:
        print("[calibrate] R4 inheritance 验证失败;fail closed")
        return 1
    n_blocks = int(pack["selected_block_count"])
    design_digest = (out / "r15_design_plan_digest.txt").read_text(
        encoding="utf-8").strip()
    recall_floor_value = float(pack["recall_floor"])
    rt_mode = bool(getattr(args, "rehearsal", False))
    if rt_mode:
        # R15RealArtifactCliRoundTrip-v1:rt_* rehearsal-only namespace;
        ...
        fit_ns_main = "rt3_fit_main_r15"
        fit_ns_hold = "rt3_fit_holdout_r15"
        profile_main_obj = rt_main_profile_r15()
        profile_holdout_obj = rt_holdout_profile_r15()
        conditioning_eval_ns = "rt3_calibration_main_r15"
        stress_ns = "rt3_stress_r15"
        print("[calibrate][rehearsal] rt_* namespace;正式生成规模;"
              "write_artifacts=True(R15RealArtifactCliRoundTrip-v1)")
    else:
        fit_ns_main = "preprocess_fit_calibration_r15"
        fit_ns_hold = "preprocess_fit_holdout_r15"
        profile_main_obj = formal_main_profile_r15(n_blocks)
        profile_holdout_obj = formal_holdout_profile_r15(n_blocks)
        conditioning_eval_ns = "calibration_r15"
        stress_ns = "stress_r15"

    print(f"[calibrate] fitting main preprocessor ({fit_ns_main})...")
    records_main = generate_fit_bank_r15(fit_ns_main, pack)
    v2_main, manifest_main = fit_preprocessor_v2_from_bank_r15(
        fit_ns_main, pack, records=records_main,
        parameter_pack_identity=pack["digest"])
    routing_main = build_routing_r15("main", v2_main, rt=rt_mode)
    print(f"[calibrate] fitting holdout preprocessor ({fit_ns_hold})...")
    records_hold = generate_fit_bank_r15(fit_ns_hold, pack)
    v2_hold, manifest_hold = fit_preprocessor_v2_from_bank_r15(
        fit_ns_hold, pack, records=records_hold,
        parameter_pack_identity=pack["digest"])
    routing_holdout = build_routing_r15("holdout", v2_hold, rt=rt_mode)
    ...
    stage = orchestrate_calibration_stage_r15(
        out, pack,
        n_blocks=n_blocks,
    ...
    eval_records = [
        generate_pair(f, r, 0, namespace=conditioning_eval_ns,
                      rung_params_override=r15_override_for(f, pack))
    ...
    stress = run_generator_stress_r15(pack, namespace=stress_ns)
    _write_json(out, "generator_stress.json", stress)
    _write_json(out, "frozen_parameter_identity.json",
                frozen_parameter_identity_r15())

    gate = {
        "format": "cur261-r15-robustness-gate-v1",
        "iteration": "r15",
        "rehearsal": rt_mode,
    ...
    _write_json(out, "calibration_evidence.json", {
        ...
        "orchestrator": "orchestrate_calibration_stage_r15",
...
def cmd_preflight_static(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r15_preflight import (
        run_prelock_static_preflight_r15,
    )

    result = run_prelock_static_preflight_r15(Path(args.out_dir),
                                             VENDOR_PIN)
...
def cmd_lock_plan(args: argparse.Namespace) -> int:
    ...
    plan = build_plan_r15(
        ...
        frozen_parameter_identity=frozen_parameter_identity_r15(),
    ...
    path, digest = lock_plan_r15(plan)
...
def cmd_preflight_sealed(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r15_preflight import (
        run_postlock_sealed_preflight_r15,
    )

    att = run_postlock_sealed_preflight_r15(Path(args.out_dir),
                                           VENDOR_PIN)
...
#: R15RealArtifactCliRoundTrip-v1 的 final 阶段 rehearsal profile
#: (§四-4):rt_*_r15 rehearsal-only namespace + 缩小样本量;治理外壳
...
R15_RT_FINAL_PROFILE: dict = {
    "final_namespace": "rt3_qualification_r15",
    "fit_namespace": "rt3_fit_qualification_r15",
    "c13_pairs_per_rung": 2,
    "c2_blocks": 4,
    "semantic_block_count": 8,
    "independent_pairs_per_rung": 2,
    "independent_namespace": "rt3_c2_independent_main_r15",
    "semantic_namespace": "rt3_semantic_final_r15",
    "supervised_namespace": "rt3_supervised_main_r15",
    "supervised_model_seeds": (20270135,),
    "supervised_training_config": {"epochs": 2},
    "conditioning_fit_namespace": "rt3_fit_main_r15",
}
...
def cmd_qualify(args: argparse.Namespace) -> int:
    ...
    repair R15(§11/工作包 A2):final 的任何异常在处置前落盘全部
    ...
    --rehearsal(R15RealArtifactCliRoundTrip-v1):以 R15_RT_FINAL_
    ...
    rt_mode = bool(getattr(args, "rehearsal", False))
    try:
        result = run_final_qualification_r15(
            out, rehearsal_profile=R15_RT_FINAL_PROFILE if rt_mode
            else None)
    except Exception as exc:  # noqa: BLE001 —— §11 正式异常处置
        from rl_curriculum.curriculum261_generation_envelope import (
            dump_failure_evidence,
...
def cmd_smoke(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r15_smoke import run_ppo_smoke_r15
    ...
    try:
        ...
        pass
    smoke = run_ppo_smoke_r15(pack=pack)
...
def cmd_namespace_integrity(args: argparse.Namespace) -> int:
    ...
    print(f"[namespace-integrity] pass={ns.get('pass')} "
          f"namespaces={len(ns.get('r15_namespaces', []))}")
...
def cmd_determinism_matrix(args: argparse.Namespace) -> int:
    ...
    工程阶段命令(在任何正式 R15 namespace 访问之前执行);产物写
    ...
    from rl_curriculum.curriculum261_r15_determinism import (
        audit_generator_mutable_state,
        generation_determinism_gate,
        run_cross_process_determinism_matrix,
    )

    out = qualification_r15_lock_dir() / "determinism"
...
def cmd_shadow_run(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r15_shadow import (
        run_full_scale_shadow_r15,
    )

    base = qualification_r15_lock_dir() / "shadow"
    ...
    summary = run_full_scale_shadow_r15(run_dir, run_tag=str(args.run_tag))
...
def cmd_global_k_reanalysis(args: argparse.Namespace) -> int:
    ...
    - 明确声明:重分析只用于 method development 与回归,不改变 R11
      的 FAIL 结论;禁止依据该结果调整 R15 的 alpha/eligibility/
      randomization count/statistic/tail 定义。
    """
    import math as _math

    from rl_curriculum.curriculum261_r15_global_k import (
    ...
    _write_json(out, "global_k_historical_reanalysis.json", {
        "format": "cur261-r15-global-k-historical-reanalysis-v1",
...
def cmd_release_rehearsal(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r15_cue_contract import (
        cue_audit_plan_digest_r15,
        cue_audit_plan_payload_r15,
        load_locked_cue_audit_plan_r15,
        lock_cue_audit_plan_r15,
    )
    from rl_curriculum.curriculum261_r15_namespaces import (
    ...
    with tempfile.TemporaryDirectory(prefix="r15_rehearsal_") as td:
        tdp = Path(td)
        ...
        sections["audit_run1"] = {
            ...
            "freeze_writer_written": (tdp / "r15_code_freeze.json").is_file(),
        }
        # ---- cue-audit plan build/lock/load/verify 路径 ----
        plan_dir = tdp / "cue_plan"
        _, digest1 = lock_cue_audit_plan_r15(plan_dir)
        loaded = load_locked_cue_audit_plan_r15(plan_dir)
        plan_payload = cue_audit_plan_payload_r15()
        expected_digest = cue_audit_plan_digest_r15(plan_payload)
        ...
        # ---- plan 重复锁定必须被拒绝 ----
        try:
            lock_cue_audit_plan_r15(plan_dir)
        ...
        try:
            from rl_curriculum.curriculum261_r15_namespaces import (
                require_r15_iteration_active,
                r15_iteration_aborted as _aborted,
            )
            ...
            abort_written_and_enforced = (
                _aborted()
                and _verify_raises(require_r15_iteration_active))
        finally:
            if old_lock is None:
        ...
        mini_audit = run_cue_contract_audit(
            ...
            model_namespace="preplan_engineering_smoke_r15",
            validation_namespace="preplan_engineering_smoke_r15",
            require_locked_plan=False)
        rplan = design_plan_payload_r15(
            baseline_commit=BASELINE_COMMIT_R15,
            vendor_pin=VENDOR_PIN,
            v2_contract_digest=preprocessing_v2_contract_digest(),
            prior_r2_plan_digest=PRIOR_R2_PLAN_DIGEST,
            ...
            cue_audit_plan_digest="r15ap-" + "0" * 64)
        pre_d = design_plan_digest_r15(rplan)
        plan_dir = tdp / "design_plan"
        _, locked_d = lock_design_plan_r15(plan_dir, rplan)
        reloaded_plan, reloaded_d = load_locked_design_plan_r15(
            plan_dir)
        ...
        for _rk in ("design_plan_digest", "locked_utc"):
            ...
            _orig.pop(_rk, None)
        try:
            lock_design_plan_r15(plan_dir, rplan)
        ...
        from rl_curriculum.curriculum261_r15_dependencies import (
            write_r15_code_freeze as _wcf,
        )

        _wcf(cl_dir, code_freeze_sha=head)
        cl_path = write_path_cleanliness_r15(cl_dir, verdict="FAIL")
        cl_doc = json.loads(cl_path.read_text(encoding="utf-8"))
        try:
            write_path_cleanliness_r15(cl_dir, verdict="FAIL")
    ...
    with tempfile.TemporaryDirectory(prefix="r15_rehearsal2_") as td:
        tdp = Path(td)
        audit_args = argparse.Namespace(
            out_dir=str(tdp), fit_pairs=2, code_freeze_sha=head)
    ...
    result = {
        "format": "cur261-r15-pre-freeze-release-rehearsal-v1",
        "head_at_rehearsal": head,
        "monkeypatch_used": False,
        "temp_dirs_only": True,
        "formal_r15_namespace_accessed": False,
        "sections": sections,
        "pass": ok,
    }
...
def write_path_cleanliness_r15(out_dir: Path, *, verdict: str) -> Path:
    ...
    from rl_curriculum.curriculum261_r15_namespaces import (
        ...
        r15_iteration_aborted,
    )

    freeze_check = verify_r15_code_freeze(out_dir)
    result = _load(out_dir / "qualification_result.json")
    abort = _load(out_dir / "r15_iteration_aborted.json")
    design_plan_digest_txt = (out_dir / "r15_design_plan_digest.txt")
    # §四-2 修复:R12 版读取无 producer 的 calibration_report_{main,
    # holdout}.json 导致 calibration_state 恒为 absent;R15 读取
    ...
    for doc in (result, abort):
        if isinstance(doc, dict):
            ...
            if failing_gate:
                break
    payload: dict = {
        "format": "cur261-r15-path-cleanliness-v1",
        "verdict": verdict,
        ...
        "abort_artifact_sha256": _sha(
            out_dir / "r15_iteration_aborted.json"),
        ...
        "exposure_state": ("exposed" if qualification_r15_exposed()
                           else "not_exposed"),
        "final_namespace_state": (
            "executed" if isinstance(result, dict) else
            "aborted" if r15_iteration_aborted() else "untouched"),
        "pooled_rescue_used": False,
...
#: 工程 namespace preplan_*/shadow_*/stress_r15/training_r15/
#: fresh_holdout_r15/ppo_smoke_r15/reference_diagnostic_* 不在此列)。
R15_FORMAL_CRITICAL_NAMESPACES: tuple[str, ...] = (
...
def artifact_interface_audit() -> dict[str, Any]:
    ...
    """
    return {
        "format": "cur261-r15-artifact-interface-audit-v1",
        "iteration": "r15",
        "canonical_identity_fields": {
            "preprocessor_bundle_{calibration,holdout}.json":
                ...
                "'bundle_hash' 禁止冒充)",
            "r15_parameter_pack.json": "digest(r15pk-;pack_digest_r15"
                                       " 复算)",
            "r15_design_plan.json": "design_plan_digest(r15dp-)",
            "qualification_plan_r15.json": "plan_digest(r15qp-)",
            "sealed_final_preflight.json": "digest(r15fa-)",
        ...
        "edges": [
            {
                ...
                "r15_closure":
                    ...
                    "accessor(curriculum261_r15_plan.py;§四-1)",
            ...
            },
            {
                ...
                "r12_defect":
                    ...
                    "名称)——潜伏缺陷,R15 修复(§四-2)",
                "r15_closure":
                    "run_postlock_sealed_preflight_r15 证据文件清单"
                    "与真实 producer 对齐",
            ...
            },
            {
                ...
                "r12_defect":
                    ...
                    "supervised_learnability.json——潜伏缺陷,R15 修复",
                "rehearsal_step": "preflight-sealed",
            },
            {
            ...
            },
            {
                ...
                "artifacts": ["qualification_plan_r15.json",
            ...
            },
            {
                ...
                "artifacts": ["r15_parameter_pack.json",
            ...
            },
            {
                ...
                "(curriculum261_r15_full_cold.read_full_cold_evidence;"
                ...
                "r13_defect": "R13 rehearsal 将本边标记为 smoke "
                              ...
                              "reader 从未实际执行(§九-5)",
                "r15_closure": "R15 rehearsal 第 14 步真实执行 "
            ...
            },
            {
                ...
                "artifacts": ["r15_design_plan.json",
...
def cmd_real_artifact_rehearsal(args: argparse.Namespace) -> int:
    """§十一:R15RealArtifactCliRoundTrip-v1 —— Commit A 前必须 PASS。

    R15 关键变更(工作包 A):rehearsal 与 formal 走**同一权威
    ...
    rh = qualification_r15_lock_dir() / "real_artifact_rehearsal"
    ...
    validation = validate_r15_workflow()
    if not validation["pass"]:
        print(f"[real-artifact-rehearsal] workflow validation FAIL: "
              f"{validation['problems']}")
        _write_json(rh, "r15_workflow_validation.json", validation)
        return 1

    # 链子进程环境:lock dir 重定向到 rehearsal 隔离目录
    os.environ["CURRICULUM261_R15_LOCK_DIR"] = str(rh)
    ...
    pl = subprocess.run(
        [sys.executable, "-m",
         "rl_curriculum.curriculum261_r15_cli", "provenance-lock",
         "--out-dir", str(rh)],
    ...
    if pl.returncode != 0:
        ...
        _write_json(rh, "real_artifact_cli_roundtrip.json", {
            "format": "R15RealArtifactCliRoundTrip-v1",
            "pass": False,
    ...
    plan = build_workflow_plan(
        ...
        manifest_path=str(rh / "r15_formal_log_manifest.jsonl"))
    _write_json(rh, "r15_workflow_plan_rehearsal.json", plan)
    # 与 formal 完全相同的执行路径:subprocess 调 r15_run_step.py
    # chain(其内部调用权威 execute_workflow_chain)
    runner_dir = (release_repo / "stage2_6_1" / "runner"
                  if release_repo else None)
    chain_result_path = rh / "r15_chain_result.json"
    if runner_dir is not None and (
            runner_dir / "r15_run_step.py").is_file():
        cr = subprocess.run(
            [sys.executable, str(runner_dir / "r15_run_step.py"),
             "chain", str(rh / "r15_workflow_plan_rehearsal.json"),
    ...
    tail_rc = 0
    tail_rec: dict[str, Any] = {"step": R15_REHEARSAL_ONLY_TAIL[0]}
    if chain_rcs_ok:
        fc = subprocess.run(
            [sys.executable, "-m",
             "rl_curriculum.curriculum261_r15_cli", "fail-closure",
    ...
    probe_env["CURRICULUM261_R15_LOCK_DIR"] = str(rh)
    boundary: dict[str, Any] = {}
    boundary["plan_loader"] = _rt_probe(probe_env, (
        "import json,sys;"
        "sys.path.insert(0, sys.argv[2]);"
        "from rl_curriculum.curriculum261_r15_plan import "
        "(load_locked_plan_r15, read_preprocessor_bundle_hash);"
        "from pathlib import Path;"
        "plan,digest=load_locked_plan_r15();"
        "rh=Path(sys.argv[1]);"
    ...
    boundary["exposure_one_shot"] = _rt_probe(probe_env, (
        ...
        "from rl_curriculum.curriculum261_r15_namespaces import "
        "qualification_r15_exposed;"
        "print(json.dumps({'exposed':qualification_r15_exposed()}))"),
        str(rh), str(proj / "src"))
    ...
    for ledger_path in sorted(rh.rglob(
        ...
        for line in ledger_path.read_text(encoding="utf-8").splitlines():
            ...
            ns = str(row.get("call", {}).get("namespace",
                     row.get("namespace", "")))
            if ns in R15_FORMAL_CRITICAL_NAMESPACES:
    ...
    result = {
        ...
        "monkeypatch_used": False,
        "cli_entrypoint":
            "subprocess python -m rl_curriculum.curriculum261_r15_cli"
            "(经权威 execute_workflow_chain)",
        "workflow_executor":
            "curriculum261_r15_workflow.execute_workflow_chain"
...
# R15 §四:正式链步骤顺序的唯一来源是
# curriculum261_r15_workflow.R15_WORKFLOW_STEPS(单一权威定义;
# rehearsal/formal runner/verifier/顺序测试全部从它派生)。
# 本模块不得再维护第二份步骤列表(R14 缺陷:两份列表不一致导致
# formal 链缺 preplan-smoke 步)。
R15_FORMAL_LOG_MANIFEST_NAME = "r15_formal_log_manifest.jsonl"
...
def cmd_verify_formal_logs(args: argparse.Namespace) -> int:
    ...
    for r in records:
        # R15 chain 记录器字段(execute_workflow_chain):env_identity
    ...
    result = {"format": "cur261-r15-formal-log-verify-v2",
    ...
    _write_json(out, "r15_formal_log_verification.json", result)
...
def cmd_workflow_plan(args: argparse.Namespace) -> int:
    ...
    out = (Path(args.out_file) if getattr(args, "out_file", None)
           else Path(args.out_dir) / f"r15_workflow_plan_"
                                     f"{args.profile}.json")
...
def cmd_full_cold_reader_check(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r15_full_cold import (
...
def cmd_full_cold(args: argparse.Namespace) -> int:
    ...
    if regression["executed"]:
        evidence["pass"] = bool(evidence.get("pass")
                                and regression["rc"] == 0)
    evidence["format"] = "cur261-r15-full-cold-step-v1"
    _write_json(out, "full_cold_reader_check.json", evidence)
    reader_ok = bool(all(evidence["reader_checks"].values()))
...
def cmd_fail_closure(args: argparse.Namespace) -> int:
    ...
    if not args.rehearsal:
        write_r15_iteration_aborted(args.reason)
    # R15 §七:按实际停止阶段机械组装(不再使用固定成功阶段模板)
    ...
    cleanliness = write_path_cleanliness_r15(out, verdict=args.verdict)
    closure["cleanliness_path"] = str(cleanliness)
    closure["post_exposure_policy"] = (
        "只读原始 artifacts;不得重生成 qualification_r15 语料;"
        "不得创建新代码;下一轮必须 R16 + 全新 namespace")
    _write_json(out, "r15_fail_closure_summary.json", closure)
...
def cmd_report_read(args: argparse.Namespace) -> int:
    ...
    for name, keys in (
        ...
        ("r15_sample_size_selection.json",
...
def commit_b_allowlist_violations(paths: list[str]) -> list[str]:
    ...
    允许:stage2_6_1/artifacts/repair15/**、R15 指定报告 md、
    ...
    def _allowed(path: str) -> bool:
        ...
        if path.startswith("stage2_6_1/report/"):
            return "repair15" in path and path.endswith(".md")
        return path.startswith("stage2_6_1/artifacts/repair15/")
...
def cmd_commit_b_allowlist(args: argparse.Namespace) -> int:
    ...
    result = {
        "format": "cur261-r15-commit-b-allowlist-v1",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="r15-cli")
    ...
    for name in ("cue-audit", "design-plan-lock", "calibrate",
        ...
        p.add_argument("--rehearsal", action="store_true",
                       help="R15RealArtifactCliRoundTrip-v1 rehearsal-"
