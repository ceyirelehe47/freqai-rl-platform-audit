# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R9:final preflight(§25)。

A. pre-lock static:imports/vendor/Route C identity/code identity/
   V2 serialize+outer env+PPO smoke(ppo_smoke_r9)/matched generator
...
def _imports_ok_r9() -> dict[str, Any]:
    modules = [
...
def _dependency_resolution_ok_r9() -> dict[str, Any]:
    ...
    return resolve_dependency_identity_r9()


def _code_identity_digest_r9() -> dict[str, Any]:
    from rl_curriculum.curriculum261_r9_plan import _code_identity_r9

    ident = _code_identity_r9()
    missing = [k for k, v in ident.items() if v == "MISSING"]
    return {
        "identity": ident,
        "missing": missing,
        "digest": "r9ci-" + hashlib.sha256(
            _canonical(ident).encode("utf-8")).hexdigest(),
        "pass": not missing,
    }


def _matched_generator_probe_r9() -> dict[str, Any]:
    """matched generator + unique-event evaluator + cluster bootstrap
    小规模验证(ppo_smoke_r9 下 2 个 block;不触碰 design/cal/final)。"""
    try:
        ...
        for idx in range(2):
            blocks.append(generate_matched_block_with_attempts(
                dict(C2_RUNG_PARAMS), namespace="ppo_smoke_r9",
                block_index=idx))
        ...
        # R9 §7:真实(非 monkeypatch)candidate evaluator 集成探针
        ...
        eval_result = _evaluate_candidate_matched_r9(
            "preflight_sentinel", dict(C2_RUNG_PARAMS),
            "ppo_smoke_r9", thresholds, blocks=blocks, n_blocks=2)
        evaluator_ok = bool(
...
def run_prelock_static_preflight_r9(out_dir: Path,
                                    vendor_pin: str) -> dict[str, Any]:
    """§25.1:plan lock 前静态 preflight(只用非 final namespace)。"""
    out_dir = Path(out_dir)
    checks: dict[str, Any] = {}
    checks["imports"] = _imports_ok_r9()
    checks["dependency_resolution"] = _dependency_resolution_ok_r9()
    ...
    checks["code_identity"] = _code_identity_digest_r9()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        old = os.environ.get("CURRICULUM261_R9_LOCK_DIR")
        os.environ["CURRICULUM261_R9_LOCK_DIR"] = td
        try:
            ...
            write_qualification_r9_exposure("r9dp-preflight-probe",
                                            "running")
            try:
                write_qualification_r9_exposure("r9dp-preflight-probe",
                                                "running")
            ...
            if old is None:
                ...
                os.environ["CURRICULUM261_R9_LOCK_DIR"] = old
    checks["marker_atomic_exclusive"] = marker_ok
    checks["concurrent_final_lock_rejected"] = lock_ok

    result = {
        "format": "cur261-r9-prelock-static-preflight-v1",
        "iteration": "r9",
        "executed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespaces_touched": ["ppo_smoke_r9"],
...
def sealed_preflight_digest(att: dict[str, Any]) -> str:
    ...
    return "r9fa-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def run_postlock_sealed_preflight_r9(out_dir: Path,
                                     vendor_pin: str,
                                     ) -> dict[str, Any]:
    """§25.2:plan lock 后 sealed preflight(零 final seed / 零 marker)。"""
    from rl_curriculum.curriculum261_r9_namespaces import (
        qualification_r9_plan_path,
        qualification_r9_digest_path,
        r9_parameter_pack_path,
    )
    ...
    checks: dict[str, Any] = {}
    plan, plan_digest = load_locked_plan_r9()
    checks["plan_digest_recompute"] = True
    checks["plan_iteration_r9"] = bool(plan.get("iteration") == "r9")
    ...
    checks["r4_inheritance_verified"] = verify_r4_inheritance_r9(pack)
    ...
    code_digest = "r9ci-" + hashlib.sha256(
        _canonical(ident).encode("utf-8")).hexdigest()
    ...
    checks["final_runner_dependencies"] = _imports_ok_r9()["pass"]
    checks["plan_path_resolves"] = bool(
        qualification_r9_plan_path().is_file()
        and qualification_r9_digest_path().is_file()
        and r9_parameter_pack_path().is_file())
