# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R10 CLI(audit → cue-audit → preplan-smoke →
...
digest 自引用缺陷修复的正式验收);design plan 在第一条 R10 design/
...
BASELINE_COMMIT_R10 = "ab260684df340f89443ce9827f8f733e3ede4320"
...
#: TypeError 后按 §18 硬规则诚实 FAIL;design 全部结果仅作 R10
...
def _default_art() -> Path:
    from rl_curriculum.curriculum261_r10_namespaces import (
        qualification_r10_lock_dir,
    )

    return qualification_r10_lock_dir()
...
def _pack(out_dir: Path) -> dict:
    from rl_curriculum.curriculum261_r10_param_pack import load_selected_pack
...
def _r8_abort_binding(out_dir: Path) -> dict:
    ...
    if marker is None or not marker.is_file():
        raise RuntimeError(
            ...
            "禁止在 marker 缺失时锁 R10 design plan)")
...
def _code_freeze_sha(out_dir: Path) -> str:
    ...
    freeze = verify_r10_code_freeze(Path(out_dir))
    if not freeze["pass"]:
        raise RuntimeError(
            f"R10 code freeze 校验失败:{freeze}(design plan 必须绑定"
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    _write_json(out, "baseline_integrity.json", {
        "expected_baseline": BASELINE_COMMIT_R10,
        "release_repo_head": head,
        "baseline_matches": bool(head == BASELINE_COMMIT_R10),
    ...
    _dump_txt(out, "preprocessing_v2_contract_digest.txt",
              preprocessing_v2_contract_digest())
    records = generate_fit_bank("preplan_smoke_r10", args.fit_pairs)
    ...
    from rl_curriculum.curriculum261_r10_dependencies import (
        write_dependency_resolution_r10,
    )

    dep = write_dependency_resolution_r10(out)
    ...
    sig_audit = live_signature_audit_r10()
    ast_checks = delegation_ast_checks_r10()
...
def _official_entrypoint_validation() -> dict:
    ...
    mods = [
        "rl_curriculum.curriculum261_r10_namespaces",
        "rl_curriculum.curriculum261_r10_dependencies",
        "rl_curriculum.curriculum261_r10_noise_replay",
        "rl_curriculum.curriculum261_r10_cue_contract",
        "rl_curriculum.curriculum261_r10_cue_eval",
        "rl_curriculum.curriculum261_r10_param_pack",
        "rl_curriculum.curriculum261_r10_preplan",
        "rl_curriculum.curriculum261_r10_design",
        "rl_curriculum.curriculum261_r10_calibration",
        "rl_curriculum.curriculum261_r10_plan",
        "rl_curriculum.curriculum261_r10_preflight",
        "rl_curriculum.curriculum261_r10_final",
        "rl_curriculum.curriculum261_r10_smoke",
        "rl_curriculum.curriculum261_r10_cli",
    ]
    failed: dict[str, str] = {}
    for m in mods:
        try:
    ...
    from rl_curriculum.curriculum261_r10_cli import main as cli_main
    ...
    return {
        "format": "cur261-r10-official-entrypoint-v1",
        "entrypoint": "rl_curriculum.curriculum261_r10_cli.main",
...
def cmd_cue_audit(args: argparse.Namespace) -> int:
    ...
    if not (out / "cue_audit_plan.json").is_file():
        _, plan_digest = lock_cue_audit_plan_r10(out)
        print(f"[cue-audit] audit plan locked digest={plan_digest}")
    load_locked_cue_audit_plan_r10(out)
...
def cmd_preplan_smoke(args: argparse.Namespace) -> int:
    ...
    不参与参数选择;只用 preplan_smoke_r10,不用 design/calibration/
    ...
    blocks = [generate_matched_block_with_attempts(
        sentinel, namespace="preplan_smoke_r10", block_index=i)
        for i in range(3)]
    ...
    mini_gate = semantic_cue_gate(
        ...
        min_unique_positive_cues=1, label="preplan_smoke_r10")
    identity = {
        "sentinel": {r: {k: sentinel[r][k]
                         for k in ("alpha_bps", "wick_kappa")}
                     for r in sentinel},
        "sentinel_digest": "r10smoke-" + _hashlib.sha256(json.dumps(
            {r: sorted(sentinel[r].items()) for r in sorted(sentinel)},
            default=str).encode("utf-8")).hexdigest(),
        "n_blocks": len(blocks),
        "namespace": "preplan_smoke_r10",
    ...
    _write_json(out, "preplan_engineering_smoke.json", {
        "format": "cur261-r10-preplan-smoke-v1",
        "identity": identity,
    ...
    from rl_curriculum.curriculum261_r10_preplan import (
        run_preplan_rehearsal_r10,
    )

    rehearsal = run_preplan_rehearsal_r10(out / "preplan")
    ...
    root_cause = run_reference_root_cause_diagnosis_r10(out / "preplan")
    ...
    full = run_preplan_full_pipeline_rehearsal_r10(out)
    if not full["pass"]:
...
_SUBPROC_LOADER_DESIGN = (
    ...
    "(design_plan_digest_r10, load_locked_design_plan_r10);"
    "import json,sys;"
    "plan,digest=load_locked_design_plan_r10(sys.argv[1]);"
    "recomputed=design_plan_digest_r10(plan);"
    "print(json.dumps({"
...
_SUBPROC_LOADER_QUAL = (
    "import os,sys;"
    "os.environ['CURRICULUM261_R10_LOCK_DIR']=sys.argv[2];"
    "from rl_curriculum.curriculum261_r10_plan import "
    "(load_locked_plan_r10, plan_digest_r10);"
    "import json;"
    "plan,digest=load_locked_plan_r10();"
    "print(json.dumps({"
    "'loaded_digest':digest,"
    "'recomputed':plan_digest_r10(plan),"
    "'iteration':plan['iteration'],"
...
def _no_alternate_loader_check() -> dict:
    ...
    权威模块(r10_design 的 lock/load;r10_namespaces 的路径常量与本
    扫描自身);其余模块不得直接读 plan JSON。"""
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    allowed_design = {"curriculum261_r10_design.py",
                      "curriculum261_r10_namespaces.py",
                      "curriculum261_r10_cli.py"}
    allowed_qual = {"curriculum261_r10_namespaces.py",
                    "curriculum261_r10_plan.py",
                    "curriculum261_r10_cli.py"}
    design_hits: list[str] = []
    qual_hits: list[str] = []
    for f in sorted(root.glob("curriculum261_r10_*.py")):
        text = f.read_text(encoding="utf-8")
        if "r10_design_plan.json" in text and f.name not in allowed_design:
            design_hits.append(f.name)
        if ("qualification_plan_r10.json" in text
                and f.name not in allowed_qual):
            qual_hits.append(f.name)
    return {
        "design_plan_readers": ["curriculum261_r10_design.py"],
        "qualification_plan_readers": ["curriculum261_r10_namespaces.py"
                                       " (path fn) + "
                                       "curriculum261_r10_plan.py (loader)"],
...
def cmd_plan_roundtrip(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r10_design import (
        design_plan_digest_r10,
        design_plan_payload_r10,
        lock_design_plan_r10,
    )
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td)
        plan = design_plan_payload_r10(
            baseline_commit=BASELINE_COMMIT_R10,
            ...
            preplan_rehearsal_digest="r10pr-" + "0" * 64,
            r8_abort_evidence={"roundtrip_synth": True},
            cue_audit_plan_digest="r10ap-" + "0" * 64,
        )
        # build payload 的 canonical 快照(逐位一致验证基准)
        pre_digest = design_plan_digest_r10(plan)
        path, digest = lock_design_plan_r10(tmp, plan)
        ...
        checks["payload_bit_identical"] = bool(
            design_plan_digest_r10(reloaded) == pre_digest)
        # 新进程 load(真实生产路径)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        synth_pack = {
            "digest": "r10pk-" + "0" * 64,
            "pack_version": "CurriculumR10MatchedLadderPack-v1",
            "selected_c2_candidate": "c2l_midpoint",
            "selected_block_count": 15,
            "c2_ladder": {r: dict(v) for r, v in
                          C2_LADDER_CANDIDATES_R10["c2l_midpoint"].items()},
            ...
            "recall_floor": 0.93,
            "p_contract": 0.95,
            "cue_contract_audit_digest": "r10ca-" + "0" * 64,
        ...
        if qual_plan is not None:
            ...
            old_dir = os.environ.get("CURRICULUM261_R10_LOCK_DIR")
            os.environ["CURRICULUM261_R10_LOCK_DIR"] = str(tmp)
            try:
                qpath, qdigest = lock_plan_r10(qual_plan)
                checks["qualification_lock_ok"] = bool(
                    qpath.is_file()
                    and qdigest == plan_digest_r10(qual_plan))
                ...
                if old_dir is None:
                    os.environ.pop("CURRICULUM261_R10_LOCK_DIR", None)
                else:
                    os.environ["CURRICULUM261_R10_LOCK_DIR"] = old_dir
    ...
    from rl_curriculum.curriculum261_r10_design import (
        load_locked_design_plan_r10 as _unused_loader,  # noqa: F401
    )
    ...
    result = {
        "format": "cur261-r10-plan-roundtrip-validation-v1",
        "iteration": "r10",
        "checks": checks,
...
def cmd_design_plan_lock(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r10_dependencies import (
        resolve_dependency_identity_r10,
    )
    from rl_curriculum.curriculum261_r10_design import (
        semantic_artifact_identity_r10,
    )
    ...
    if not rehearsal.get("pass"):
        print("[design-plan-lock] preplan rehearsal 未通过——拒绝锁 "
              "plan(§9/§31)")
        return 1
    dep = resolve_dependency_identity_r10()
    if not dep["pass"]:
        print(f"[design-plan-lock] 依赖解析未通过:{dep['problems'][:5]}"
              "——拒绝锁 plan(§6.3)")
        return 1
    audit_plan = load_locked_cue_audit_plan_r10(out)
    ...
    plan = design_plan_payload_r10(
        ...
        artifact_writer_identity=semantic_artifact_identity_r10(),
    ...
    path, digest = lock_design_plan_r10(out, plan)
...
def cmd_design(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r10_design import (
        load_locked_design_plan_r10,
        run_design_stage_r10,
    )

    out = Path(args.out_dir)
    plan, digest = load_locked_design_plan_r10(out)
    selection = run_design_stage_r10(out, plan, digest,
                                    baseline_commit=BASELINE_COMMIT_R10)
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    R10 变革(相对 R9 的手写长流程):
    - 全部评估走共享 orchestrator orchestrate_calibration_stage_r10
    ...
    from rl_curriculum.curriculum261_r10_calibration import (
        fit_preprocessor_v2_from_bank_r10,
        generate_fit_bank_r10,
    ...
    from rl_curriculum.curriculum261_r10_param_pack import (
        frozen_parameter_identity_r10,
        r10_override_for,
        verify_r4_inheritance_r10,
    )
    from rl_curriculum.curriculum261_r10_routing import build_routing_r10
    from rl_curriculum.curriculum261_r3_calibration import (
    ...
    require_r10_iteration_active()
    out = Path(args.out_dir)
    freeze = verify_r10_code_freeze(out)
    if not freeze["pass"]:
        print(f"[calibrate] R10 code freeze 校验失败(fail closed):"
              f"{freeze}")
        return 1
    pack = _pack(out)
    inheritance = verify_r4_inheritance_r10(pack)
    if not inheritance:
        print("[calibrate] R4 inheritance 验证失败;fail closed")
        return 1
    n_blocks = int(pack["selected_block_count"])
    design_digest = (out / "r10_design_plan_digest.txt").read_text(
        encoding="utf-8").strip()
    recall_floor_value = float(pack["recall_floor"])

    print("[calibrate] fitting main preprocessor "
          "(preprocess_fit_calibration_r10)...")
    records_main = generate_fit_bank_r10(
        "preprocess_fit_calibration_r10", pack)
    v2_main, manifest_main = fit_preprocessor_v2_from_bank_r10(
        "preprocess_fit_calibration_r10", pack, records=records_main,
        parameter_pack_identity=pack["digest"])
    routing_main = build_routing_r10("main", v2_main)
    print("[calibrate] fitting holdout preprocessor "
          "(preprocess_fit_holdout_r10)...")
    records_hold = generate_fit_bank_r10(
        "preprocess_fit_holdout_r10", pack)
    v2_hold, manifest_hold = fit_preprocessor_v2_from_bank_r10(
        "preprocess_fit_holdout_r10", pack, records=records_hold,
        parameter_pack_identity=pack["digest"])
    routing_holdout = build_routing_r10("holdout", v2_hold)
    ...
    _write_json(out, "fit_eval_isolation.json", {
        ...
        "holdout": {k: manifest_hold[k] for k in (
            "namespace", "pairs_per_rung", "n_pairs", "n_episodes",
            "n_rows", "integrity_all_ok", "multiset_hash")},
        "eval_namespaces": ["calibration_r10", "calibration_holdout_r10"],
        "routing_contract": (
            "main 评估(calibration_r10/c2_independent_calibration_r10/"
            "supervised_main_r10)→ v2_main;holdout 评估"
            "(calibration_holdout_r10/c2_independent_holdout_r10/"
            "supervised_holdout_r10)→ v2_hold;fail closed(§9)"),
    ...
    stage = orchestrate_calibration_stage_r10(
        out, pack,
        n_blocks=n_blocks,
        ...
        profile_main=formal_main_profile_r10(n_blocks),
        profile_holdout=formal_holdout_profile_r10(n_blocks),
        override_fn=r10_override_for,
    ...
    eval_records = [
        generate_pair(f, r, 0, namespace="calibration_r10",
                      rung_params_override=r10_override_for(f, pack))
    ...
    _write_json(out, "calibration_evidence.json", {
        ...
        "orchestrator": "orchestrate_calibration_stage_r10",
...
def cmd_preflight_static(args: argparse.Namespace) -> int:
    ...
    result = run_prelock_static_preflight_r10(Path(args.out_dir),
                                             VENDOR_PIN)
...
def cmd_lock_plan(args: argparse.Namespace) -> int:
    ...
    plan = build_plan_r10(
        ...
        frozen_parameter_identity=frozen_parameter_identity_r10(),
    ...
    path, digest = lock_plan_r10(plan)
...
def cmd_preflight_sealed(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r10_preflight import (
        run_postlock_sealed_preflight_r10,
    )

    att = run_postlock_sealed_preflight_r10(Path(args.out_dir),
                                           VENDOR_PIN)
...
def cmd_qualify(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r10_final import (
        run_final_qualification_r10,
    )

    result = run_final_qualification_r10(Path(args.out_dir))
...
def cmd_smoke(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r10_smoke import run_ppo_smoke_r10
    ...
    try:
        ...
        pass
    smoke = run_ppo_smoke_r10(pack=pack)
...
def cmd_namespace_integrity(args: argparse.Namespace) -> int:
    ...
    print(f"[namespace-integrity] pass={ns.get('pass')} "
          f"namespaces={len(ns.get('r10_namespaces', []))}")
    return 0 if ns.get("pass") else 1


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="r10-cli")
