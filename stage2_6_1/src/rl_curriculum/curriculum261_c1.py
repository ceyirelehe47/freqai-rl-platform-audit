...
- 分段 regime 链:{opp: 每 bar 漂移 +a, neut: 0, neg: 每 bar -b};
  ...
  净漂移可搭,只输摩擦;
- 机会的可识别性来自 regime 的持续性:动量/趋势特征(ma_dev_24、
  htf_4h_mom)在 opp 段内持续为正,预测"漂移将继续";
...
#: neg 漂移按段总量精确平衡:draw 后 neg_drift_eff = opp_total/neg_bars,
#: 每 episode 净漂移恰为 0 -> Always Long 恒等于 -摩擦(构造级保证)。
C1_RUNG_PARAMS: dict[str, dict[str, Any]] = {
    "D0": {"opp_drift_bps": 55.0, "neg_drift_bps": 45.0, "vol_bps": 26.0,
           "seg_len_range": [28, 48], "state_weights": [0.32, 0.30, 0.38],
           "distractor_rate": 0.000},
    "D1": {"opp_drift_bps": 38.0, "neg_drift_bps": 32.0, "vol_bps": 28.0,
           "seg_len_range": [26, 44], "state_weights": [0.36, 0.30, 0.34],
           "distractor_rate": 0.000},
    "D2": {"opp_drift_bps": 26.0, "neg_drift_bps": 22.0, "vol_bps": 30.0,
           "seg_len_range": [24, 40], "state_weights": [0.33, 0.30, 0.37],
           "distractor_rate": 0.000},
    "D3": {"opp_drift_bps": 21.0, "neg_drift_bps": 18.0, "vol_bps": 32.0,
           "seg_len_range": [22, 36], "state_weights": [0.34, 0.30, 0.36],
           "distractor_rate": 0.000},
}

#: C1 参考策略阈值:ma_dev_24 > k x sigma(ma_dev_24)(sigma 按 rung 的
#: vol_bps/sqrt(24) 解析闭式给出;k 在 plan 中冻结)
C1_REFERENCE_DEFAULTS = {"ma_sigma_mult": 3.0}
...
class C1OpportunityGenerator(Curriculum261Base):
    ...
    def _generate(self, params, seed, rng):
        ...
        n_opp = int(np.sum(states == 2))
        n_neg = int(np.sum(states == 0))
        opp_drift = opp
...
class C1ReferencePolicy(ObservableBaselinePolicy):
    ...
    只读当前 observation 的 ma_dev_24 / ret_4 槽位;阈值 =
    ...
    def act(self, observation: np.ndarray) -> int:
        return int(
            self.read(observation, "ma_dev_24") > self.ma_dev_thr
            and self.read(observation, "ret_4") > 0.0
        )


def c1_reference_threshold(rung_params: dict[str, Any],
                           ma_sigma_mult: float) -> float:
    """3-sigma 门限闭式解析:ma_dev 噪声 sigma ~ vol/sqrt(24)。"""
    vol = float(rung_params["vol_bps"]) * 1e-4
    return float(ma_sigma_mult * vol / np.sqrt(24.0))
...
class C1ShortcutPolicy(ObservableBaselinePolicy):
    ...
    def act(self, observation: np.ndarray) -> int:
        return int(self.read(observation, "ret_4") > 0.0)
...
def c1_pair_integrity_metrics(episode) -> dict[str, Any]:
    ...
    ma_dev = episode.df["ma_dev_24"].to_numpy(dtype=np.float64)
    in_opp = states == 2
