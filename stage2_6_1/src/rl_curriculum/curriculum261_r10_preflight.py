# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R10:final preflight(§25)。

A. pre-lock static:imports/vendor/Route C identity/code identity/
   V2 serialize+outer env+PPO smoke(ppo_smoke_r10)/matched generator
...
def _imports_ok_r10() -> dict[str, Any]:
    modules = [
...
def _dependency_resolution_ok_r10() -> dict[str, Any]:
    ...
    return resolve_dependency_identity_r10()


def _code_identity_digest_r10() -> dict[str, Any]:
    from rl_curriculum.curriculum261_r10_plan import _code_identity_r10

    ident = _code_identity_r10()
    missing = [k for k, v in ident.items() if v == "MISSING"]
    return {
        "identity": ident,
        "missing": missing,
        "digest": "r10ci-" + hashlib.sha256(
            _canonical(ident).encode("utf-8")).hexdigest(),
        "pass": not missing,
    }


def _matched_generator_probe_r10() -> dict[str, Any]:
    """matched generator + unique-event evaluator + cluster bootstrap
    小规模验证(ppo_smoke_r10 下 2 个 block;不触碰 design/cal/final)。"""
    try:
        ...
        for idx in range(2):
            blocks.append(generate_matched_block_with_attempts(
                dict(C2_RUNG_PARAMS), namespace="ppo_smoke_r10",
                block_index=idx))
        ...
        # R10 §7:真实(非 monkeypatch)candidate evaluator 集成探针
        ...
        eval_result = _evaluate_candidate_matched_r10(
            "preflight_sentinel", dict(C2_RUNG_PARAMS),
            "ppo_smoke_r10", thresholds, blocks=blocks, n_blocks=2)
        evaluator_ok = bool(
...
def run_prelock_static_preflight_r10(out_dir: Path,
                                    vendor_pin: str) -> dict[str, Any]:
    """§25.1:plan lock 前静态 preflight(只用非 final namespace)。"""
    out_dir = Path(out_dir)
    checks: dict[str, Any] = {}
    checks["imports"] = _imports_ok_r10()
    checks["dependency_resolution"] = _dependency_resolution_ok_r10()
    ...
    checks["code_identity"] = _code_identity_digest_r10()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        old = os.environ.get("CURRICULUM261_R10_LOCK_DIR")
        os.environ["CURRICULUM261_R10_LOCK_DIR"] = td
        try:
            ...
            write_qualification_r10_exposure("r10dp-preflight-probe",
                                            "running")
            try:
                write_qualification_r10_exposure("r10dp-preflight-probe",
                                                "running")
            ...
            if old is None:
                ...
                os.environ["CURRICULUM261_R10_LOCK_DIR"] = old
    checks["marker_atomic_exclusive"] = marker_ok
    checks["concurrent_final_lock_rejected"] = lock_ok

    result = {
        "format": "cur261-r10-prelock-static-preflight-v1",
        "iteration": "r10",
        "executed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespaces_touched": ["ppo_smoke_r10"],
...
def sealed_preflight_digest(att: dict[str, Any]) -> str:
    ...
    return "r10fa-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def run_postlock_sealed_preflight_r10(out_dir: Path,
                                     vendor_pin: str,
                                     ) -> dict[str, Any]:
    """§25.2:plan lock 后 sealed preflight(零 final seed / 零 marker)。"""
    from rl_curriculum.curriculum261_r10_namespaces import (
        qualification_r10_plan_path,
        qualification_r10_digest_path,
        r10_parameter_pack_path,
    )
    ...
    checks: dict[str, Any] = {}
    plan, plan_digest = load_locked_plan_r10()
    checks["plan_digest_recompute"] = True
    checks["plan_iteration_r10"] = bool(plan.get("iteration") == "r10")
    ...
    checks["r4_inheritance_verified"] = verify_r4_inheritance_r10(pack)
    ...
    code_digest = "r10ci-" + hashlib.sha256(
        _canonical(ident).encode("utf-8")).hexdigest()
    ...
    checks["final_runner_dependencies"] = _imports_ok_r10()["pass"]
    checks["plan_path_resolves"] = bool(
        qualification_r10_plan_path().is_file()
        and qualification_r10_digest_path().is_file()
        and r10_parameter_pack_path().is_file())
...
def write_rehearsal_sealed_preflight_r10(
        out_dir: Path, *, plan_digest: str) -> dict[str, Any]:
    """§12 rehearsal 的 sealed preflight(临时目录;零正式 seed)。

    与正式 run_postlock_sealed_preflight_r10 同构的零 final-seed
    attestation(rehearsal 标志 + preplan namespace 声明)。
    """
    from rl_curriculum.curriculum261_r10_namespaces import (
        R10_SEALED_PREFLIGHT_DIGEST_FILENAME,
        R10_SEALED_PREFLIGHT_FILENAME,
    )
    ...
    (out_dir / R10_SEALED_PREFLIGHT_FILENAME).write_text(
        json.dumps(att, indent=1, ensure_ascii=False), encoding="utf-8")
    (out_dir / R10_SEALED_PREFLIGHT_DIGEST_FILENAME).write_text(
