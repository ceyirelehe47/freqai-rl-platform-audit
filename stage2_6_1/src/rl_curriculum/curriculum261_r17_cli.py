# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R17 CLI(audit → cue-audit → preplan-smoke →
...
为冻结前工程命令,在任何正式 R17 namespace 访问之前执行)。
...
digest 自引用缺陷修复的正式验收);design plan 在第一条 R17 design/
...
BASELINE_COMMIT_R17 = "14a889c2854571e3ab5245ef51da7c858c83f59b"
#: R14 提交链锚点(R17 baseline = R14 Commit B;§二)。
...
#: producer/consumer 落盘接口 —— R17RealArtifactCliRoundTrip-v1 的
#: 直接动因。
...
#: (curriculum261_r17_historical),不在源码手工转录文件级长 digest。
...
#: TypeError 后按 §18 硬规则诚实 FAIL;design 全部结果仅作 R17
...
#: R17 继承)。R10 design/pack 全部
#: 结果仅作 R17 development evidence。
...
def _default_art() -> Path:
    from rl_curriculum.curriculum261_r17_registry import (
        qualification_r17_lock_dir,
    )

    return qualification_r17_lock_dir()
...
def _r12_abort_binding(out_dir: Path) -> dict:
    ...
    binding = {
        ...
        "final_verdict": "FAIL(永久;统计链 cue/global K/design/"
                         ...
                         "R17 输入;R17 必须全新 namespace 重新执行)",
        "fail_reasons": [
            ...
            "永久 FAIL,下一轮 R17)",
            "治理缺口(非硬失败,但为 R17 直接动因):Commit A 前的 "
        ...
        "latent_defect_2_discovered_by_r17": (
            "R17 全仓库接口审计另发现 sealed preflight 证据文件名错配"
            ...
            "R12 未活到该步故未触发;R17 已在 Commit A 前修复"),
        "statistical_chain_status_at_failure": (
            ...
            "holdout 独立 PASS —— 全部仅作历史记录,不进入 R17"),
        "abort_marker_exists": True,
        "abort_marker_blob_matches_baseline": _blob_matches(
...
def _r13_failure_binding(out_dir: Path) -> dict:
    ...
    binding = {
        ...
        "failure_classification": (
            ...
            "c2_semantics_pass;R13 不因 R17 修订被追认/撤销/改写"),
...
def _r8_abort_binding(out_dir: Path) -> dict:
    ...
    if marker is None or not marker.is_file():
        raise RuntimeError(
            ...
            "禁止在 marker 缺失时锁 R17 design plan)")
    data = json.loads(marker.read_text(encoding="utf-8"))
...
def _code_freeze_sha(out_dir: Path) -> str:
    ...
    if not freeze["pass"]:
        raise RuntimeError(
            f"R17 code freeze 校验失败:{freeze}(design plan 必须绑定"
...
def _r14_failure_binding(out_dir: Path, heb: dict) -> dict:
    ...
    提取;R14 永久 FAIL 不因 R17 修复被追认。
    """
    checks = heb.get("checks", {})
    r14_keys = sorted(k for k in checks if k.startswith("r14_"))
    binding = {
        "format": "cur261-r17-r14-iteration-failure-binding-v1",
        "iteration": "r14",
...
def _r15_failure_binding(out_dir: Path, heb: dict) -> dict:
    ...
    提取;R15 永久 FAIL 不因 R17 修复被追认。
    """
    gov = heb.get("r15_governance_binding") or {}
    binding = {
        "format": "cur261-r17-r15-iteration-failure-binding-v1",
        "iteration": "r15",
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    if freeze_sha:
        from rl_curriculum.curriculum261_r17_dependencies import (
            write_r17_code_freeze,
        )

        freeze_doc = write_r17_code_freeze(
        ...
        existing = verify_r17_code_freeze(Path(args.out_dir))
    ...
    _write_json(out, "baseline_ancestry.json", {
        "format": "cur261-r17-baseline-ancestry-v1",
        "expected_baseline": BASELINE_COMMIT_R17,
    ...
    from rl_curriculum.curriculum261_r17_dependencies import (
        write_dependency_resolution_r17,
    )

    dep = write_dependency_resolution_r17(out)
    from rl_curriculum.curriculum261_r17_delegation import (
        calibration_call_contract_payload,
        delegation_ast_checks_r17,
        live_signature_audit_r17,
    )
    from rl_curriculum.curriculum261_r17_routing import (
        bundle_routing_contract_payload,
    )

    sig_audit = live_signature_audit_r17()
    ast_checks = delegation_ast_checks_r17()
    ...
    freeze_check = verify_r17_code_freeze(out)
    ...
    _write_json(out, "r11_cue_failure_binding.json", {
        "format": "cur261-r17-r11-cue-failure-binding-v1",
        ...
        "classification": "position-wise mirror-count distribution "
                          "failure(t=226 < 264,非 tail;R17 禁止继续"
                          "称为 tail-position failure)",
        "binding_gate_in_r17": False,
    ...
    _write_json(out, "r12_iteration_failure_binding.json", {
        "format": "cur261-r17-r12-iteration-failure-binding-v1",
        "iteration": "r12",
        ...
        "consumer": "cmd_lock_plan → build_plan_r17("
        ...
        "governance_gap": "R12 pre-freeze release rehearsal 使用 "
                          ...
                          "artifact 读取(§三-2;R17 "
                          "R17RealArtifactCliRoundTrip-v1 修复)",
        "binding_gate_in_r17": True,
    ...
    workflow_validation = validate_r17_workflow()
    _write_json(out, "r17_workflow_validation.json", workflow_validation)
    from rl_curriculum.curriculum261_r17_provenance import (
...
def _official_entrypoint_validation() -> dict:
    ...
    mods = [
        "rl_curriculum.curriculum261_r17_registry",
        "rl_curriculum.curriculum261_r17_dependencies",
        "rl_curriculum.curriculum261_r17_noise_replay",
        "rl_curriculum.curriculum261_r17_cue_contract",
        "rl_curriculum.curriculum261_r17_cue_eval",
        "rl_curriculum.curriculum261_r17_global_k",
        "rl_curriculum.curriculum261_r17_historical",
        "rl_curriculum.curriculum261_r17_generation_evidence",
        "rl_curriculum.curriculum261_r17_param_pack",
        "rl_curriculum.curriculum261_r17_preplan",
        "rl_curriculum.curriculum261_r17_design",
        "rl_curriculum.curriculum261_r17_calibration",
        "rl_curriculum.curriculum261_r17_plan",
        "rl_curriculum.curriculum261_r17_preflight",
        "rl_curriculum.curriculum261_r17_final",
        "rl_curriculum.curriculum261_r17_smoke",
        "rl_curriculum.curriculum261_generation_envelope",
        "rl_curriculum.curriculum261_r17_determinism",
        "rl_curriculum.curriculum261_r17_shadow",
        "rl_curriculum.curriculum261_r17_orchestrator",
        "rl_curriculum.curriculum261_r17_gate_topology",
        "rl_curriculum.curriculum261_r17_provenance",
        "rl_curriculum.curriculum261_r17_full_cold",
        "rl_curriculum.curriculum261_r17_workflow",
        "rl_curriculum.curriculum261_r17_fail_closure",
    ...
    from rl_curriculum.curriculum261_r17_cli import main as cli_main
    ...
    return {
        "format": "cur261-r17-official-entrypoint-v1",
        "entrypoint": "rl_curriculum.curriculum261_r17_cli.main",
...
def cmd_preplan_smoke(args: argparse.Namespace) -> int:
    ...
    _write_json(out, "preplan_engineering_smoke.json", {
        "format": "cur261-r17-preplan-smoke-v1",
        "identity": identity,
    ...
    from rl_curriculum.curriculum261_r17_preplan import (
        run_preplan_rehearsal_r17,
    )

    rehearsal = run_preplan_rehearsal_r17(out / "preplan")
    ...
    from rl_curriculum.curriculum261_r17_preplan import (
        run_reference_root_cause_diagnosis_r17,
    )

    root_cause = run_reference_root_cause_diagnosis_r17(out / "preplan")
    ...
    from rl_curriculum.curriculum261_r17_rehearsal import (
        run_preplan_full_pipeline_rehearsal_r17,
    )

    full = run_preplan_full_pipeline_rehearsal_r17(out)
    if not full["pass"]:
...
_SUBPROC_LOADER_DESIGN = (
    ...
    "(design_plan_digest_r17, load_locked_design_plan_r17);"
    "import json,sys;"
    "plan,digest=load_locked_design_plan_r17(sys.argv[1]);"
    "recomputed=design_plan_digest_r17(plan);"
    "print(json.dumps({"
...
_SUBPROC_LOADER_QUAL = (
    "import os,sys;"
    "os.environ['CURRICULUM261_R17_STATE_ROOT']=sys.argv[2];"
    "from rl_curriculum.curriculum261_r17_plan import "
    "(load_locked_plan_r17, plan_digest_r17);"
    "import json;"
    "plan,digest=load_locked_plan_r17();"
    "print(json.dumps({"
    "'loaded_digest':digest,"
    "'recomputed':plan_digest_r17(plan),"
    "'iteration':plan['iteration'],"
...
def _no_alternate_loader_check() -> dict:
    ...
    权威模块(r17_design 的 lock/load;r17_registry 的路径常量与本
    扫描自身);其余模块不得直接读 plan JSON。"""
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    allowed_design = {"curriculum261_r17_design.py",
    ...
    allowed_qual = {"curriculum261_r17_registry.py",
    ...
    qual_hits: list[str] = []
    for f in sorted(root.glob("curriculum261_r17_*.py")):
        text = f.read_text(encoding="utf-8")
        if "r17_design_plan.json" in text and f.name not in allowed_design:
            design_hits.append(f.name)
        if ("qualification_plan_r17.json" in text
                and f.name not in allowed_qual):
            qual_hits.append(f.name)
    return {
        "design_plan_readers": ["curriculum261_r17_design.py"],
        "qualification_plan_readers": ["curriculum261_r17_registry.py"
                                       " (path fn) + "
                                       "curriculum261_r17_plan.py (loader)"],
...
def cmd_plan_roundtrip(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r17_design import (
        design_plan_digest_r17,
        design_plan_payload_r17,
        lock_design_plan_r17,
    )
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        pre_digest = design_plan_digest_r17(plan)
        path, digest = lock_design_plan_r17(tmp, plan)
        ...
        checks["payload_bit_identical"] = bool(
            design_plan_digest_r17(reloaded) == pre_digest)
        # 新进程 load(真实生产路径)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        synth_pack = {
            "digest": "r15pk-" + "0" * 64,
            "pack_version": "CurriculumR17MatchedLadderPack-v1",
            "selected_c2_candidate": "c2l_midpoint",
            "selected_block_count": 15,
            "c2_ladder": {r: dict(v) for r, v in
                          C2_LADDER_CANDIDATES_R17["c2l_midpoint"].items()},
        ...
        if qual_plan is not None:
            ...
            old_dir = os.environ.get("CURRICULUM261_R17_STATE_ROOT")
            os.environ["CURRICULUM261_R17_STATE_ROOT"] = str(tmp)
            try:
                qpath, qdigest = lock_plan_r17(qual_plan)
                checks["qualification_lock_ok"] = bool(
                    qpath.is_file()
                    and qdigest == plan_digest_r17(qual_plan))
                ...
                if old_dir is None:
                    ...
                    os.environ["CURRICULUM261_R17_STATE_ROOT"] = old_dir
    ...
    from rl_curriculum.curriculum261_r17_design import (
        load_locked_design_plan_r17 as _unused_loader,  # noqa: F401
    )
    ...
    result = {
        "format": "cur261-r17-plan-roundtrip-validation-v1",
        "iteration": "r17",
        "checks": checks,
...
def cmd_design_plan_lock(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r17_dependencies import (
        resolve_dependency_identity_r17,
    )
    from rl_curriculum.curriculum261_r17_design import (
        semantic_artifact_identity_r17,
    )
    ...
    if not rehearsal.get("pass"):
        print("[design-plan-lock] preplan rehearsal 未通过——拒绝锁 "
              "plan(§9/§31)")
        return 1
    dep = resolve_dependency_identity_r17()
    if not dep["pass"]:
    ...
    if rt_mode:
        # R17RealArtifactCliRoundTrip-v1:rehearsal cue-audit 不锁定
    ...
    plan = design_plan_payload_r17(
        ...
        artifact_writer_identity=semantic_artifact_identity_r17(),
    ...
    path, digest = lock_design_plan_r17(out, plan)
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    R17 变革(相对 R9 的手写长流程):
    - 全部评估走共享 orchestrator orchestrate_calibration_stage_r17
    ...
    from rl_curriculum.curriculum261_r17_param_pack import (
        frozen_parameter_identity_r17,
        r17_override_for,
        verify_r4_inheritance_r17,
    )
    ...
    try:
        ...
        write_r17_iteration_aborted(
            ...
            f"{str(exc)[:400]});§11 硬规则:R17 永久结束,下一轮必须 "
            "R17 + 全新 namespace")
        raise
...
def _cmd_calibrate_inner(args: argparse.Namespace,
    ...
    require_r17_iteration_active()
    freeze = verify_r17_code_freeze(out)
    if not freeze["pass"]:
        print(f"[calibrate] R17 code freeze 校验失败(fail closed):"
              f"{freeze}")
        return 1
    pack = _pack(out)
    inheritance = verify_r4_inheritance_r17(pack)
    if not inheritance:
        print("[calibrate] R4 inheritance 验证失败;fail closed")
        return 1
    n_blocks = int(pack["selected_block_count"])
    design_digest = (out / "r17_design_plan_digest.txt").read_text(
        encoding="utf-8").strip()
    recall_floor_value = float(pack["recall_floor"])
    rt_mode = bool(getattr(args, "rehearsal", False))
    if rt_mode:
        # R17RealArtifactCliRoundTrip-v1:rt_* rehearsal-only namespace;
    ...
    v2_main, manifest_main = fit_preprocessor_v2_from_bank_r17(
        fit_ns_main, pack, records=records_main,
        parameter_pack_identity=pack["digest"])
    routing_main = build_routing_r17("main", v2_main, rt=rt_mode)
    print(f"[calibrate] fitting holdout preprocessor ({fit_ns_hold})...")
    records_hold = generate_fit_bank_r17(fit_ns_hold, pack)
    v2_hold, manifest_hold = fit_preprocessor_v2_from_bank_r17(
        fit_ns_hold, pack, records=records_hold,
        parameter_pack_identity=pack["digest"])
    routing_holdout = build_routing_r17("holdout", v2_hold, rt=rt_mode)
    ...
    stage = orchestrate_calibration_stage_r17(
        out, pack,
        n_blocks=n_blocks,
    ...
    eval_records = [
        generate_pair(f, r, 0, namespace=conditioning_eval_ns,
                      rung_params_override=r17_override_for(f, pack))
    ...
    stress = run_generator_stress_r17(pack, namespace=stress_ns)
    _write_json(out, "generator_stress.json", stress)
    _write_json(out, "frozen_parameter_identity.json",
                frozen_parameter_identity_r17())

    gate = {
        "format": "cur261-r17-robustness-gate-v1",
        "iteration": "r17",
        "rehearsal": rt_mode,
...
def cmd_lock_plan(args: argparse.Namespace) -> int:
    ...
    plan = build_plan_r17(
        ...
        frozen_parameter_identity=frozen_parameter_identity_r17(),
...
def cmd_preflight_sealed(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r17_preflight import (
        run_postlock_sealed_preflight_r17,
    )

    att = run_postlock_sealed_preflight_r17(Path(args.out_dir),
                                           VENDOR_PIN)
...
#: R17RealArtifactCliRoundTrip-v1 的 final 阶段 rehearsal profile
#: (§四-4):rt_*_r17 rehearsal-only namespace + 缩小样本量;治理外壳
...
def cmd_qualify(args: argparse.Namespace) -> int:
    """final qualification worker(被协调者委派;一次性)。

    R17 生命周期:本命令是**被委派的资格执行者**——链会话由
    外层协调者持有;--await-delegation <write_fd> <read_fd> 提供
    受控 pipe(注册本进程实例身份 → 接收绑定该身份的 token)。
    缺失该参数的正式调用被拒绝(worker 无法自行获得授权;直接
    调用不再是可用正式入口;§WP1/§WP2)。

    worker 不写权威 journal;异常处置 = dump failure evidence +
    abort 请求文件(协调者代写 chain_iteration_aborted)。

    --rehearsal:以 R17_RT_FINAL_PROFILE 在 rt3_*_r17 namespace +
    ...
    if await_delegation:
        w_fd, r_fd = (int(x) for x in await_delegation[:2])
    elif not rt_mode:
        raise SystemExit(
            "正式 qualify 必须经协调者受控委派启动(--await-"
            "delegation pipe 缺失;直接调用 worker 入口无法获得"
            "绑定本进程实例的生成授权;R17 单写者/委派合同)")
    else:
        w_fd = r_fd = None
    try:
        ...
        if not rt_mode:
            write_r17_iteration_aborted(
                f"final qualification worker 执行异常"
                f"({type(exc).__name__}:"
                f"{str(exc)[:400]});abort 请求交协调者代写")
        raise
...
def cmd_namespace_integrity(args: argparse.Namespace) -> int:
    ...
    print(f"[namespace-integrity] pass={ns.get('pass')} "
          f"namespaces={len(ns.get('r17_registry', []))}")
...
def cmd_determinism_matrix(args: argparse.Namespace) -> int:
    ...
    工程阶段命令(在任何正式 R17 namespace 访问之前执行);产物写
    <artifacts>/determinism/。generation_determinism_contract.json
    是 audit 阶段的硬前置(A6)。
    """
    from rl_curriculum.curriculum261_r17_determinism import (
...
def cmd_shadow_run(args: argparse.Namespace) -> int:
    ...
    """
    from rl_curriculum.curriculum261_r17_shadow import (
        run_full_scale_shadow_r17,
    )

    base = Path(args.out_dir) / "shadow"
    ...
    summary = run_full_scale_shadow_r17(run_dir, run_tag=str(args.run_tag))
...
def cmd_global_k_reanalysis(args: argparse.Namespace) -> int:
    ...
    - 明确声明:重分析只用于 method development 与回归,不改变 R11
      的 FAIL 结论;禁止依据该结果调整 R17 的 alpha/eligibility/
    ...
    _write_json(out, "global_k_historical_reanalysis.json", {
        "format": "cur261-r17-global-k-historical-reanalysis-v1",
...
def cmd_release_rehearsal(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="r17_rehearsal_") as td:
        tdp = Path(td)
        ...
        sections["audit_run1"] = {
            ...
            "freeze_writer_written": (tdp / "r17_code_freeze.json").is_file(),
        }
        # ---- cue-audit plan build/lock/load/verify 路径 ----
        plan_dir = tdp / "cue_plan"
        _, digest1 = lock_cue_audit_plan_r17(plan_dir)
        loaded = load_locked_cue_audit_plan_r17(plan_dir)
        plan_payload = cue_audit_plan_payload_r17()
        expected_digest = cue_audit_plan_digest_r17(plan_payload)
        ...
        # ---- plan 重复锁定必须被拒绝 ----
        try:
            lock_cue_audit_plan_r17(plan_dir)
        ...
        try:
            from rl_curriculum.curriculum261_r17_registry import (
                require_r17_iteration_active,
                r17_iteration_aborted as _aborted,
            )
            ...
            abort_written_and_enforced = (
                _aborted()
                and _verify_raises(require_r17_iteration_active))
        finally:
            if old_lock is None:
        ...
        mini_audit = run_cue_contract_audit(
            ...
            model_namespace="preplan_engineering_smoke_r17",
            validation_namespace="preplan_engineering_smoke_r17",
            require_locked_plan=False)
        rplan = design_plan_payload_r17(
            baseline_commit=BASELINE_COMMIT_R17,
            vendor_pin=VENDOR_PIN,
            v2_contract_digest=preprocessing_v2_contract_digest(),
            prior_r2_plan_digest=PRIOR_R2_PLAN_DIGEST,
            ...
            cue_audit_plan_digest="r15ap-" + "0" * 64)
        pre_d = design_plan_digest_r17(rplan)
        plan_dir = tdp / "design_plan"
        _, locked_d = lock_design_plan_r17(plan_dir, rplan)
        reloaded_plan, reloaded_d = load_locked_design_plan_r17(
            plan_dir)
        ...
        for _rk in ("design_plan_digest", "locked_utc"):
            ...
            _orig.pop(_rk, None)
        try:
            lock_design_plan_r17(plan_dir, rplan)
        ...
        from rl_curriculum.curriculum261_r17_dependencies import (
            write_r17_code_freeze as _wcf,
        )

        _wcf(cl_dir, code_freeze_sha=head)
        cl_path = write_path_cleanliness_r17(cl_dir, verdict="FAIL")
        cl_doc = json.loads(cl_path.read_text(encoding="utf-8"))
        try:
            write_path_cleanliness_r17(cl_dir, verdict="FAIL")
    ...
    with tempfile.TemporaryDirectory(prefix="r17_rehearsal2_") as td:
        tdp = Path(td)
        audit_args = argparse.Namespace(
            out_dir=str(tdp), fit_pairs=2, code_freeze_sha=head)
    ...
    result = {
        "format": "cur261-r17-pre-freeze-release-rehearsal-v1",
        "head_at_rehearsal": head,
        "monkeypatch_used": False,
        "temp_dirs_only": True,
        "formal_r17_namespace_accessed": False,
        "sections": sections,
        "pass": ok,
    }
...
def write_path_cleanliness_r17(out_dir: Path, *, verdict: str) -> Path:
    ...
    from rl_curriculum.curriculum261_r17_registry import (
        qualification_r17_exposed,
        r17_iteration_aborted,
    )

    freeze_check = verify_r17_code_freeze(out_dir)
    result = _load(out_dir / "qualification_result.json")
    abort = _load(out_dir / "r17_iteration_aborted.json")
    design_plan_digest_txt = (out_dir / "r17_design_plan_digest.txt")
    # §四-2 修复:R12 版读取无 producer 的 calibration_report_{main,
    # holdout}.json 导致 calibration_state 恒为 absent;R17 读取
    ...
    for doc in (result, abort):
        if isinstance(doc, dict):
            ...
            if failing_gate:
                break
    payload: dict = {
        "format": "cur261-r17-path-cleanliness-v1",
        "verdict": verdict,
        ...
        "abort_artifact_sha256": _sha(
            out_dir / "r17_iteration_aborted.json"),
        ...
        "exposure_state": ("exposed" if qualification_r17_exposed()
                           else "not_exposed"),
        "final_namespace_state": (
            "executed" if isinstance(result, dict) else
            "aborted" if r17_iteration_aborted() else "untouched"),
        "pooled_rescue_used": False,
...
#: 工程 namespace preplan_*/shadow_*/stress_r17/training_r17/
#: fresh_holdout_r17/ppo_smoke_r17/reference_diagnostic_* 不在此列)。
R17_FORMAL_CRITICAL_NAMESPACES: tuple[str, ...] = (
    "cue_contract_model_r17", "cue_contract_validation_r17",
    "cue_k_global_null_r17",
    "cue_semantic_design_main_r17", "cue_semantic_design_validation_r17",
    "design_r17_matched_main", "design_r17_matched_validation",
    "design_r17_independent_marginal",
    "preprocess_fit_calibration_r17", "preprocess_fit_holdout_r17",
    "preprocess_fit_qualification_r17",
    "supervised_main_r17", "supervised_holdout_r17",
    "cue_semantic_calibration_r17", "cue_semantic_holdout_r17",
    "cue_semantic_qualification_r17",
    "calibration_r17", "calibration_holdout_r17", "qualification_r17",
    "c2_independent_calibration_r17", "c2_independent_holdout_r17",
...
def artifact_interface_audit() -> dict[str, Any]:
    ...
    """
    return {
        "format": "cur261-r17-artifact-interface-audit-v1",
        "iteration": "r17",
        "canonical_identity_fields": {
            "preprocessor_bundle_{calibration,holdout}.json":
                ...
                "'bundle_hash' 禁止冒充)",
            "r17_parameter_pack.json": "digest(r15pk-;pack_digest_r17"
                                       " 复算)",
            "r17_design_plan.json": "design_plan_digest(r17dp-)",
            "qualification_plan_r17.json": "plan_digest(r15qp-)",
            "sealed_final_preflight_r17.json": "digest(r15fa-)",
        ...
        "edges": [
            {
                ...
                "r17_closure":
                    ...
                    "accessor(curriculum261_r17_plan.py;§四-1)",
            ...
            },
            {
                ...
                "r12_defect":
                    ...
                    "名称)——潜伏缺陷,R17 修复(§四-2)",
                "r17_closure":
                    "run_postlock_sealed_preflight_r17 证据文件清单"
                    "与真实 producer 对齐",
            ...
            },
            {
                ...
                "r12_defect":
                    ...
                    "supervised_learnability.json——潜伏缺陷,R17 修复",
                "rehearsal_step": "preflight-sealed",
            },
            {
                "edge": "static-preflight→sealed-preflight",
                "producer_command": "preflight-static",
                "artifacts": ["prelock_static_preflight_r17.json"],
                "consumer_command": "preflight-sealed",
                "consumer_fields": ["pass"],
                "r12_defect": "无",
                "rehearsal_step": "preflight-sealed",
            },
            {
                ...
                "artifacts": ["qualification_plan_r17.json",
            ...
            },
            {
                ...
                "artifacts": ["sealed_final_preflight_r17.json",
            ...
            },
            {
                ...
                "artifacts": ["r17_parameter_pack.json",
            ...
            },
            {
                ...
                "(curriculum261_r17_full_cold.read_full_cold_evidence;"
                ...
                "r13_defect": "R13 rehearsal 将本边标记为 smoke "
                              ...
                              "reader 从未实际执行(§九-5)",
                "r17_closure": "R17 rehearsal 第 14 步真实执行 "
            ...
            },
            {
                ...
                "artifacts": ["r17_design_plan.json",
...
def cmd_real_artifact_rehearsal(args: argparse.Namespace) -> int:
    """§十一:R17RealArtifactCliRoundTrip-v1 —— Commit A 前必须 PASS。

    R17 关键变更(工作包 A):rehearsal 与 formal 走**同一权威
    ...
    rh = Path(args.out_dir) / "real_artifact_rehearsal"
    ...
    validation = validate_r17_workflow()
    if not validation["pass"]:
        print(f"[real-artifact-rehearsal] workflow validation FAIL: "
              f"{validation['problems']}")
        _write_json(rh, "r17_workflow_validation.json", validation)
        return 1

    # 链子进程环境:lock dir 重定向到 rehearsal 隔离目录
    os.environ["CURRICULUM261_R17_STATE_ROOT"] = str(rh)
    ...
    pl = subprocess.run(
        [sys.executable, "-m",
         "rl_curriculum.curriculum261_r17_cli", "provenance-lock",
         "--out-dir", str(rh)],
    ...
    if pl.returncode != 0:
        ...
        _write_json(rh, "real_artifact_cli_roundtrip.json", {
            "format": "R17RealArtifactCliRoundTrip-v1",
            "pass": False,
    ...
    plan = build_workflow_plan(
        ...
        manifest_path=str(rh / "r17_formal_log_manifest.jsonl"))
    _write_json(rh, "r17_workflow_plan_rehearsal.json", plan)
    # 与 formal 完全相同的执行路径:subprocess 调 r17_run_step.py
    # chain(其内部调用权威 execute_workflow_chain)
    runner_dir = (release_repo / "stage2_6_1" / "runner"
                  if release_repo else None)
    chain_result_path = rh / "r17_chain_result.json"
    if runner_dir is not None and (
            runner_dir / "r17_run_step.py").is_file():
        cr = subprocess.run(
            [sys.executable, str(runner_dir / "r17_run_step.py"),
             "chain", str(rh / "r17_workflow_plan_rehearsal.json"),
    ...
    tail_rc = 0
    tail_rec: dict[str, Any] = {"step": R17_REHEARSAL_ONLY_TAIL[0]}
    if chain_rcs_ok:
        fc = subprocess.run(
            [sys.executable, "-m",
             "rl_curriculum.curriculum261_r17_cli", "fail-closure",
    ...
    probe_env["CURRICULUM261_R17_STATE_ROOT"] = str(rh)
    boundary: dict[str, Any] = {}
    boundary["plan_loader"] = _rt_probe(probe_env, (
        "import json,sys;"
        "sys.path.insert(0, sys.argv[2]);"
        "from rl_curriculum.curriculum261_r17_plan import "
        "(load_locked_plan_r17, read_preprocessor_bundle_hash);"
        "from pathlib import Path;"
        "plan,digest=load_locked_plan_r17();"
        "rh=Path(sys.argv[1]);"
    ...
    boundary["exposure_one_shot"] = _rt_probe(probe_env, (
        ...
        "from rl_curriculum.curriculum261_r17_registry import "
        "qualification_r17_exposed;"
        "print(json.dumps({'exposed':qualification_r17_exposed()}))"),
        str(rh), str(proj / "src"))
    ...
    for ledger_path in sorted(rh.rglob(
        ...
        for line in ledger_path.read_text(encoding="utf-8").splitlines():
            ...
            ns = str(row.get("call", {}).get("namespace",
                     row.get("namespace", "")))
            if ns in R17_FORMAL_CRITICAL_NAMESPACES:
    ...
    result = {
        ...
        "monkeypatch_used": False,
        "cli_entrypoint":
            "subprocess python -m rl_curriculum.curriculum261_r17_cli"
            "(经权威 execute_workflow_chain)",
        "workflow_executor":
            "curriculum261_r17_workflow.execute_workflow_chain"
...
# R17 §四:正式链步骤顺序的唯一来源是
# curriculum261_r17_workflow.R17_WORKFLOW_STEPS(单一权威定义;
# rehearsal/formal runner/verifier/顺序测试全部从它派生)。
# 本模块不得再维护第二份步骤列表(R14 缺陷:两份列表不一致导致
# formal 链缺 preplan-smoke 步)。
R17_FORMAL_LOG_MANIFEST_NAME = "r17_formal_log_manifest.jsonl"
...
def cmd_verify_formal_logs(args: argparse.Namespace) -> int:
    ...
    for r in records:
        # R17 chain 记录器字段(execute_workflow_chain):env_identity
    ...
    _write_json(out, "r17_formal_log_verification.json", result)
...
def cmd_workflow_plan(args: argparse.Namespace) -> int:
    ...
    out = (Path(args.out_file) if getattr(args, "out_file", None)
           else Path(args.out_dir) / f"r17_workflow_plan_"
                                     f"{args.profile}.json")
...
def cmd_full_cold_reader_check(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r17_full_cold import (
...
def cmd_full_cold(args: argparse.Namespace) -> int:
    ...
    if regression["executed"]:
        evidence["pass"] = bool(evidence.get("pass")
                                and regression["rc"] == 0)
    evidence["format"] = "cur261-r17-full-cold-step-v1"
    _write_json(out, "full_cold_reader_check.json", evidence)
    reader_ok = bool(all(evidence["reader_checks"].values()))
...
def cmd_fail_closure(args: argparse.Namespace) -> int:
    ...
    if not args.rehearsal:
        write_r17_iteration_aborted(args.reason)
    # R17 §七:按实际停止阶段机械组装(不再使用固定成功阶段模板)
    from rl_curriculum.curriculum261_r17_fail_closure import (
    ...
    cleanliness = write_path_cleanliness_r17(out, verdict=args.verdict)
    closure["cleanliness_path"] = str(cleanliness)
    closure["post_exposure_policy"] = (
        "只读原始 artifacts;不得重生成 qualification_r17 语料;"
        "不得创建新代码;下一轮必须 R17 + 全新 namespace")
    _write_json(out, "r17_fail_closure_summary.json", closure)
...
def cmd_report_read(args: argparse.Namespace) -> int:
    ...
    for name, keys in (
        ...
        ("r17_sample_size_selection.json",
...
def commit_b_allowlist_violations(paths: list[str]) -> list[str]:
    ...
    允许:stage2_6_1/artifacts/repair17/**、R17 指定报告 md、
    ...
    def _allowed(path: str) -> bool:
        ...
        if path.startswith("stage2_6_1/report/"):
            return "repair17" in path and path.endswith(".md")
        return path.startswith("stage2_6_1/artifacts/repair17/")
...
def cmd_commit_b_allowlist(args: argparse.Namespace) -> int:
    ...
    result = {
        "format": "cur261-r17-commit-b-allowlist-v1",
...
#: WSL 根文件系统存储天花板(用户设定;超过拒绝启动,fail closed)。
R17_STORAGE_CEILING_GB = 300


def _storage_used_gb() -> int:
    """df / 的已用 GB(稀疏感知;与 launcher 同口径;-1=无法测量)。"""
    try:
        out = subprocess.run(["df", "-BK", "--output=used", "/"],
                             capture_output=True, text=True,
                             timeout=30).stdout
        used_kb = int(out.strip().splitlines()[-1]
                      .strip().rstrip("K"))
        return used_kb // (1024 * 1024)
    except Exception:  # noqa: BLE001 —— 无法测量 = 拒绝
        return -1


def cmd_chain_run(args: argparse.Namespace) -> int:
    """R17 链协调者:获取会话 → 权威 chain → 终态处置 → 释放。

    F1 闭合的核心入口:任何正式 workflow plan、bootstrap
    accepted、步骤 artifact、raw log 或 manifest 写入之前取得
    iteration 级唯一链会话;同一协调者保持控制直到收尾。

    --rehearsal:同一代码路径与所有权/委派/日志规则;state root
    与 out_dir 由调用方指向隔离工程目录(r17_rt_rehearsal.sh)。
    """
    import json as _json
    ...
    if used_gb < 0 or used_gb >= R17_STORAGE_CEILING_GB:
        print(f"[chain-run] FAIL: WSL 存储已用 {used_gb}GB 达到/"
              f"超过天花板 {R17_STORAGE_CEILING_GB}GB(用户设定;"
              "fail closed;先清理 ~/.cache/rl_builder_bundles)")
        return 96
    ...
    plan_path = out_dir / ("r17_workflow_plan_rehearsal.json"
                           if args.rehearsal
                           else "r17_workflow_plan_formal.json")
    binding = {
        "mode": profile,
    ...
    try:
        ...
        if chain_result["ok"]:
            session.release(summary=(
                "R17 chain completed all steps;delivery verification "
                "follows as read-only closing duty(chain 外只读职责,"
                "不再新增 journal 事件)"))
            return 0
        # 失败:fail-closure 已由 chain 执行器调用(关闭职责);
        # 此处由 owner 写 iteration 终止并释放(活跃 owner 路径)。
        session.record_iteration_aborted(
            chain_result["failure_reason"][:2000])
        session.release(summary=(
            f"R17 chain failed at {chain_result['failed_step']}"))
        return 1
    except BaseException:
        # 协调者崩溃:保持会话现场(不截断、不倒回);已 exposure
        # 则机会已消耗,由受限事后封口处置,不在此伪装终态。
        raise


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="r17-cli")
