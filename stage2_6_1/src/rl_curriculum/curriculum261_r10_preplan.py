# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R10:preplan 工程闭环(§7/§8/§9)。

三个组件(全部真实执行,禁止 monkeypatch;只用非正式 namespace):

1. §7 真实 candidate evaluator 集成 smoke(preplan_candidate_eval_r10):
   ...
   evaluator,使其从未在测试中真实执行;R10 将该路径变成 plan lock 的
   硬前置项。
...
SMOKE_NAMESPACE_R10 = "preplan_candidate_eval_r10"
SEMANTIC_MAIN_NAMESPACE_R10 = "preplan_semantic_main_r10"
SEMANTIC_VALIDATION_NAMESPACE_R10 = "preplan_semantic_validation_r10"
AUDIT_MODEL_NAMESPACE_R10 = "preplan_smoke_r10"
AUDIT_VALIDATION_NAMESPACE_R10 = "preplan_candidate_eval_r10"
...
def run_candidate_evaluator_integration_smoke_r10(
        out_dir: Path | None = None,
        n_blocks: int = 4) -> dict[str, Any]:
    """§7:真实(非 monkeypatch)candidate evaluator 集成 smoke。

    1 candidate(sentinel ladder)× 1 corpus(preplan_candidate_eval_r10)
    ...
    blocks = [generate_matched_block_with_attempts(
        ladder, namespace=SMOKE_NAMESPACE_R10, block_index=i)
        for i in range(n_blocks)]
    result = _evaluate_candidate_matched_r10(
        "preplan_sentinel", ladder, SMOKE_NAMESPACE_R10, _thresholds(),
    ...
    reload_ok = bool(
        ...
        == {"10", "15", "20"})
    report = {
        "format": "cur261-r10-candidate-evaluator-smoke-v1",
        "namespace": SMOKE_NAMESPACE_R10,
...
def run_semantic_writer_validation_r10(out_dir: Path | None = None,
                                     ) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as td:
        tdp = Path(td)
        main_ns = "cue_semantic_design_main_r10"
        validation_ns = "cue_semantic_design_validation_r10"
        p_main = write_semantic_artifact_r10(
            tdp, main_ns, {"n_blocks": 2, "marker": "main"},
            "r10dp-writer-validation", event_rows=[{"cue_bar": 5}])
        p_valid = write_semantic_artifact_r10(
        ...
        checks["filename_mapping_exact"] = bool(
            ...
            and semantic_artifact_filename_r10(main_ns)
        ...
        try:
            semantic_artifact_filename_r10("cue_semantic_design_main")
        except RuntimeError:
            unknown_ns_rejected = True
        checks["unknown_namespace_rejected"] = unknown_ns_rejected
        suffix_heuristic_absent = not any(
            "endswith" in str(v) for v in SEMANTIC_ARTIFACT_MAP_R10)
        checks["no_suffix_heuristic_in_mapping"] = suffix_heuristic_absent
    report = {
        "format": "cur261-r10-semantic-writer-validation-v1",
        "mapping": {k: v for k, v in SEMANTIC_ARTIFACT_MAP_R10.items()},
        "checks": checks,
...
def _exposure_marker_rehearsal() -> dict[str, Any]:
    ...
    env_key = "CURRICULUM261_R10_LOCK_DIR"
    old = os.environ.get(env_key)
    try:
        with tempfile.TemporaryDirectory() as td:
            os.environ[env_key] = td
            write_qualification_r10_exposure(
                "r10dp-rehearsal-exposure", "running")
            running_exposed = qualification_r10_exposed()
            ...
            write_qualification_r10_exposure(
                "r10dp-rehearsal-exposure", "completed")
            terminal_single_shot = False
            try:
                write_qualification_r10_exposure(
                    "r10dp-rehearsal-exposure", "failed")
            except Exception:  # noqa: BLE001
...
def _static_builder_rehearsal() -> dict[str, Any]:
    ...
    pack = ladder_pack_payload_r10(
        ...
        design_plan_digest="r10dp-rehearsal",
        matched_contract_identity="rehearsal",
        block_integrity_identity="rehearsal",
        cue_semantic_contract_digest="r10cue-rehearsal",
        cue_semantic_rule_identity="r10csg-rehearsal",
        cue_audit_digest="r10ca-rehearsal",
    ...
    pack_digest = pack_digest_r10(pack)
    pack_ok = bool(pack["semantic_blocks_per_corpus"] == 160
                   and pack_digest.startswith("r10pk-"))
    # final plan builder dry construction:qp10- 前缀 + PRIOR 绑定存在
    probe_digest = plan_digest_r10({"rehearsal": True})
    qp9_ok = bool(probe_digest.startswith("qp10-"))
    sig_final = str(inspect.signature(run_final_qualification_r10))
    ...
    return {
        ...
        "calibration_runner_importable": bool(
            callable(run_c2_semantic_corpus_r10)),
        "sealed_preflight_builder_importable": bool(
            callable(run_postlock_sealed_preflight_r10)),
        "final_runner_static_import_ok": final_static_ok,
        "cue_audit_plan_lock_functions_importable": bool(
            callable(lock_cue_audit_plan_r10)
            and callable(load_locked_cue_audit_plan_r10)
            and callable(run_cue_contract_audit)),
        "design_plan_lock_functions_importable": bool(
            callable(design_plan_payload_r10)
            and callable(lock_design_plan_r10)
            and callable(load_locked_design_plan_r10)),
...
def _semantic_mini_corpora(n_blocks: int = 4) -> dict[str, Any]:
    ...
    from rl_curriculum.curriculum261_r10_cue_eval import semantic_cue_gate

    ladder = {rung: dict(params) for rung, params in C2_RUNG_PARAMS.items()}
    out: dict[str, Any] = {}
    for label, ns in (("main", SEMANTIC_MAIN_NAMESPACE_R10),
                      ("validation", SEMANTIC_VALIDATION_NAMESPACE_R10)):
        blocks = [generate_matched_block_with_attempts(
...
def run_preplan_rehearsal_r10(out_dir: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        mini_audit = run_cue_contract_audit(
            ...
            model_namespace=AUDIT_MODEL_NAMESPACE_R10,
            validation_namespace=AUDIT_VALIDATION_NAMESPACE_R10,
    ...
    writer = run_semantic_writer_validation_r10(out_dir)
    evaluator = run_candidate_evaluator_integration_smoke_r10(out_dir)
    ...
    report = {
        "format": "cur261-r10-preplan-rehearsal-v1",
        "formal_namespaces_used": False,
        "monkeypatch_used": False,
        "namespaces": {
            "audit_model": AUDIT_MODEL_NAMESPACE_R10,
            "audit_validation": AUDIT_VALIDATION_NAMESPACE_R10,
            "evaluator_smoke": SMOKE_NAMESPACE_R10,
            "semantic_main": SEMANTIC_MAIN_NAMESPACE_R10,
            "semantic_validation": SEMANTIC_VALIDATION_NAMESPACE_R10},
        "sections": sections,
        "detail": {
            "cue_audit_mini": {
    ...
    report["rehearsal_digest"] = "r10pr-" + hashlib.sha256(json.dumps(
...
# --------------------------------------------- §10 reference 根因诊断
def run_reference_root_cause_diagnosis_r10(out_dir: Path) -> dict[str, Any]:
    """§10:在 reference_diagnostic_main_r10 namespace 重现并定位
    ...
    from rl_curriculum.curriculum261_r10_reference import (
        ...
        write_reference_equivalence_artifacts_r10,
    )
    from rl_curriculum.curriculum261_r10_rehearsal import _rehearsal_pack
    from rl_curriculum.curriculum261_r10_routing import build_routing_r10

    out_dir = Path(out_dir)
    pack = _rehearsal_pack()
    records = generate_fit_bank_r10(
        "preplan_fit_main_r10", pack, pairs_per_rung=2)
    v2, _manifest = fit_preprocessor_v2_from_bank_r10(
        "preplan_fit_main_r10", pack, records=records,
        parameter_pack_identity=pack["digest"])
    routing = build_routing_r10(
        "diagnostic", v2, preplan=True)
    equiv_records = []
    for family in CURRICULUM261_FAMILIES:
        override = r10_override_for(family, pack)
        for rung in CURRICULUM261_RUNGS:
            equiv_records.append(generate_pair(
                family, rung, 0,
                namespace="reference_diagnostic_main_r10",
                rung_params_override=override))
    report = reference_equivalence_run_r10(
        equiv_records, routing.bundle(), pack,
        eval_namespace="reference_diagnostic_main_r10",
    ...
    root_cause = {
        "format": "cur261-r10-reference-root-cause-v1",
        "iteration": "r10",
        "target": "R9 preprocessing_v2_requalification:"
                  "reference_equivalence_all=false(仅布尔值落盘)",
        "diagnostic_namespace": "reference_diagnostic_main_r10",
