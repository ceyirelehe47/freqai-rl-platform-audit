# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R14:HistoricalEvidenceBinding-v1(git ancestry 语义)。

R12/R13/R14 治理语义(R14 相对 R13 的新增检查):
...
3. R13 final qualification 已 exposure 一次且 terminal=failed;
   ...
   ——gate topology 冲突的机械证据;R13 永久 FAIL 不因 R14 修订
   而追认/撤销/改写);
...
#: R14 启动基线(唯一允许的锚点常量;git 对象可机器验证)。
R14_EXPECTED_BASELINE = "b8e1de05cc3040ddc81634eb36d735a9fe3483da"
#: R13 提交链锚点(治理证据;§二)。
...
#: gate_topology_reconciliation.json 为 R14 新增,在 HEAD 才存在,
...
def historical_evidence_binding(repo: Path) -> dict[str, Any]:
    """构建并验证 HistoricalEvidenceBinding-v1(R14 版;全部机器生成)。
    ...
    checks["baseline_commit_exists"] = _git_ok(
        repo, "cat-file", "-e", f"{R14_EXPECTED_BASELINE}^{{commit}}")
    if not checks["baseline_commit_exists"]:
        return {"format": "cur261-r14-historical-evidence-binding-v1",
                "iteration": R14_ITERATION,
                "expected_baseline": R14_EXPECTED_BASELINE,
                "ok": False,
    ...
    checks["baseline_is_ancestor_of_head"] = _git_ok(
        repo, "merge-base", "--is-ancestor", R14_EXPECTED_BASELINE, "HEAD")
    # ---- 3. merge-base == expected baseline ----
    mb = _git(repo, "merge-base", R14_EXPECTED_BASELINE, "HEAD")
    checks["merge_base_equals_baseline"] = (mb == R14_EXPECTED_BASELINE)
    checks["merge_base"] = mb
    # ---- 4. 分支名 ----
    current = _git(repo, "branch", "--show-current")
    checks["current_branch"] = current
    checks["r14_branch_name_ok"] = current == "route-c-stage2-6-1-repair14"
    # ---- R11 提交链关系(持续保留)----
    ...
    checks["r12_commit_b_is_r13_chain_grandparent"] = (
        R12_COMMIT_B != R14_EXPECTED_BASELINE)
    ...
    for rel in R12_PRESERVED_KEY_FILES:
        base_blob = _blob_sha(repo, R14_EXPECTED_BASELINE, rel)
    ...
    abort_blob = _blob_sha(repo, R14_EXPECTED_BASELINE, abort_rel)
    ho = subprocess.run(
    ...
    checks["r12_qualification_exposure_absent"] = bool(
        _blob_sha(repo, R14_EXPECTED_BASELINE, exposure_rel) is None
    ...
    checks["r12_qualification_plan_never_locked"] = bool(
        _blob_sha(repo, R14_EXPECTED_BASELINE, plan_rel) is None
    ...
    try:
        r13_exp = json.loads(_git(
            repo, "show", f"{R14_EXPECTED_BASELINE}:{r13_exp_rel}"))
    ...
    try:
        r13_res = json.loads(_git(
            repo, "show", f"{R14_EXPECTED_BASELINE}:{r13_res_rel}"))
    except (RuntimeError, json.JSONDecodeError):
        r13_res = {}
    ...
    return {
        "format": "cur261-r14-historical-evidence-binding-v1",
        "iteration": R14_ITERATION,
        "expected_baseline": R14_EXPECTED_BASELINE,
        "head": head,
        ...
        "r13_governance_binding": {
            ...
            "clean_two_commit_chain": "A(47d3f22 实现冻结)→ B(b8e1de0 "
                                      "诚实 FAIL 结果);无 A′/A2/hotfix",
            "r13_final_verdict": "FAIL(永久;不因 R14 gate topology 修订"
                                 "而追认/撤销/改写)",
...
def historical_evidence_binding_digest(binding: dict[str, Any]) -> str:
    core = {k: binding.get(k) for k in (
        ...
        "failed_checks", "ok")}
    return "r14heb-" + hashlib.sha256(json.dumps(
