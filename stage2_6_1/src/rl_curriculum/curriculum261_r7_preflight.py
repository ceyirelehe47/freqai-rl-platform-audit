# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R7:final preflight(§25)。

A. pre-lock static:imports/vendor/Route C identity/code identity/
   V2 serialize+outer env+PPO smoke(ppo_smoke_r7)/matched generator
...
def _imports_ok_r7() -> dict[str, Any]:
    modules = [
        "rl_curriculum.curriculum261_r7_namespaces",
        "rl_curriculum.curriculum261_r7_cue_contract",
        "rl_curriculum.curriculum261_r7_cue_eval",
        "rl_curriculum.curriculum261_r7_param_pack",
        "rl_curriculum.curriculum261_r7_design",
        "rl_curriculum.curriculum261_r7_calibration",
        "rl_curriculum.curriculum261_r7_plan",
        "rl_curriculum.curriculum261_r7_final",
        "rl_curriculum.curriculum261_r7_smoke",
...
def _code_identity_digest_r7() -> dict[str, Any]:
    from rl_curriculum.curriculum261_r7_plan import _code_identity_r7

    ident = _code_identity_r7()
    missing = [k for k, v in ident.items() if v == "MISSING"]
    return {
        "identity": ident,
        "missing": missing,
        "digest": "r7ci-" + hashlib.sha256(
            _canonical(ident).encode("utf-8")).hexdigest(),
        "pass": not missing,
    }


def _matched_generator_probe_r7() -> dict[str, Any]:
    """matched generator + unique-event evaluator + cluster bootstrap
    小规模验证(ppo_smoke_r7 下 2 个 block;不触碰 design/cal/final)。"""
    try:
        ...
        for idx in range(2):
            blocks.append(generate_matched_block_with_attempts(
                dict(C2_RUNG_PARAMS), namespace="ppo_smoke_r7",
                block_index=idx))
...
def run_prelock_static_preflight_r7(out_dir: Path,
    ...
    checks["code_identity"] = _code_identity_digest_r7()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        old = os.environ.get("CURRICULUM261_R7_LOCK_DIR")
        os.environ["CURRICULUM261_R7_LOCK_DIR"] = td
        try:
            ...
            write_qualification_r7_exposure("r7dp-preflight-probe",
                                            "running")
            try:
                write_qualification_r7_exposure("r7dp-preflight-probe",
                                                "running")
            ...
            if old is None:
                os.environ.pop("CURRICULUM261_R7_LOCK_DIR", None)
            else:
                os.environ["CURRICULUM261_R7_LOCK_DIR"] = old
    checks["marker_atomic_exclusive"] = marker_ok
    checks["concurrent_final_lock_rejected"] = lock_ok

    result = {
        "format": "cur261-r7-prelock-static-preflight-v1",
        "iteration": "r7",
        "executed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespaces_touched": ["ppo_smoke_r7"],
...
def sealed_preflight_digest(att: dict[str, Any]) -> str:
    ...
    return "r7fa-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def run_postlock_sealed_preflight_r7(out_dir: Path,
                                     vendor_pin: str,
                                     ) -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    plan, plan_digest = load_locked_plan_r7()
    checks["plan_digest_recompute"] = True
    checks["plan_iteration_r7"] = bool(plan.get("iteration") == "r7")
    ...
    checks["r4_inheritance_verified"] = verify_r4_inheritance_r7(pack)
    ...
    code_digest = "r7ci-" + hashlib.sha256(
        _canonical(ident).encode("utf-8")).hexdigest()
    ...
    checks["final_runner_dependencies"] = _imports_ok_r7()["pass"]
    checks["plan_path_resolves"] = bool(
        qualification_r7_plan_path().is_file()
        and qualification_r7_digest_path().is_file()
        and r7_parameter_pack_path().is_file())
    ...
    attestation = {
        "format": "cur261-r7-sealed-final-preflight-v1",
        "iteration": "r7",
        "pass": att_pass,
