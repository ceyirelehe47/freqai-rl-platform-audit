# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R17:共享 calibration/final orchestration(§12)。
...
R17 修复(§12.4 共享 Orchestrator):
- 正式 calibrate(main/holdout)与 preplan rehearsal 调用同一
  orchestration 函数 orchestrate_calibration_bundle_r17;
- 唯一差异通过 R17ExecutionProfile 表达:namespace 集合与样本量
...
#: R17 supervised 正式 model seeds(§17:3 个全新 seeds;R17 与 R10
#: 的 20261001-3 不重合)。
R17_SUPERVISED_MODEL_SEEDS: tuple[int, ...] = (
    20270111, 20270112, 20270113)
R17_SUPERVISED_GATE: dict[str, Any] = {
    # repair R17(B2):min_seeds_passing 语义 = 每个 gated control
...
CALIBRATION_PAIRS_PER_RUNG_R17 = 10
C2_INDEPENDENT_PAIRS_PER_RUNG_R17 = 20
SEMANTIC_BLOCKS_PER_CORPUS_R17 = 160
EQUIVALENCE_PAIRS_PER_RUNG_R17 = 3


@dataclass(frozen=True)
class R17ExecutionProfile:
    ...
    artifact_suffix: str = ""
    #: repair R17(工作包 C):shadow 路由类(工程 rehearsal;非正式)
    shadow: bool = False
    #: repair R17(工作包 C):supervised 训练配置(仅允许缩短纯训练
...
def formal_main_profile_r17(n_blocks: int) -> R17ExecutionProfile:
    return R17ExecutionProfile(
        ...
        c13_eval_namespace="calibration_r17",
        equivalence_namespace="calibration_r17",
        supervised_namespace="supervised_main_r17",
        semantic_namespace="cue_semantic_calibration_r17",
        c2_matched_namespace="calibration_r17",
        c2_independent_namespace="c2_independent_calibration_r17",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R17,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R17,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R17,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R17,
        supervised_train_pair_limit=6,
        supervised_model_seeds=R17_SUPERVISED_MODEL_SEEDS,
        equivalence_pairs_per_rung=EQUIVALENCE_PAIRS_PER_RUNG_R17,
    )
...
def formal_holdout_profile_r17(n_blocks: int) -> R17ExecutionProfile:
    return R17ExecutionProfile(
        ...
        c13_eval_namespace="calibration_holdout_r17",
        equivalence_namespace="calibration_holdout_r17",
        supervised_namespace="supervised_holdout_r17",
        semantic_namespace="cue_semantic_holdout_r17",
        c2_matched_namespace="calibration_holdout_r17",
        c2_independent_namespace="c2_independent_holdout_r17",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R17,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R17,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R17,
...
def rehearsal_main_profile_r17() -> R17ExecutionProfile:
    return R17ExecutionProfile(
        name="rehearsal_main", preplan=True,
        c13_eval_namespace="preplan_calibration_main_r17",
        equivalence_namespace="preplan_calibration_main_r17",
        supervised_namespace="preplan_supervised_main_r17",
        semantic_namespace="preplan_semantic_main_r17",
        c2_matched_namespace="preplan_calibration_main_r17",
        c2_independent_namespace="preplan_calibration_main_r17",
...
def rehearsal_holdout_profile_r17() -> R17ExecutionProfile:
    return R17ExecutionProfile(
        name="rehearsal_holdout", preplan=True,
        c13_eval_namespace="preplan_calibration_holdout_r17",
        equivalence_namespace="preplan_calibration_holdout_r17",
        supervised_namespace="preplan_supervised_holdout_r17",
        semantic_namespace="preplan_semantic_validation_r17",
        c2_matched_namespace="preplan_calibration_holdout_r17",
        c2_independent_namespace="preplan_calibration_holdout_r17",
        c13_pairs_per_rung=1,
        c2_blocks=2,
        semantic_blocks=4,
        c2_independent_pairs_per_rung=2,
...
#: 最大值 —— 无论 R17 机械选出哪个 n,shadow 的生成坐标都是其超集
#: (block index 0..19 覆盖 0..14 与 0..9)。
SHADOW_C2_BLOCKS_R17 = 20
...
def shadow_main_profile_r17() -> R17ExecutionProfile:
    ...
    return R17ExecutionProfile(
        ...
        c13_eval_namespace="shadow_calibration_main_r17",
        equivalence_namespace="shadow_calibration_main_r17",
        supervised_namespace="shadow_supervised_main_r17",
        semantic_namespace="shadow_semantic_main_r17",
        c2_matched_namespace="shadow_calibration_main_r17",
        c2_independent_namespace="shadow_c2_independent_main_r17",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R17,
        c2_blocks=SHADOW_C2_BLOCKS_R17,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R17,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R17,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R17,
        supervised_train_pair_limit=6,
...
def shadow_holdout_profile_r17() -> R17ExecutionProfile:
    """full-scale shadow holdout(与 shadow_main 对称;独立 namespace)。"""
    return R17ExecutionProfile(
        name="shadow_holdout", preplan=False, shadow=True,
        c13_eval_namespace="shadow_calibration_holdout_r17",
        equivalence_namespace="shadow_calibration_holdout_r17",
        supervised_namespace="shadow_supervised_holdout_r17",
        semantic_namespace="shadow_semantic_validation_r17",
        c2_matched_namespace="shadow_calibration_holdout_r17",
        c2_independent_namespace="shadow_c2_independent_holdout_r17",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R17,
        c2_blocks=SHADOW_C2_BLOCKS_R17,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R17,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R17,
...
# --------------------------------- R17RealArtifactCliRoundTrip-v1 profiles
#: rt(round-trip rehearsal)profiles:正式生成基数 + rt3_*_r17
...
#: 为任何机械选择结果的超集;与 SHADOW_C2_BLOCKS_R17 同理)。
RT_C2_BLOCKS_R17 = 20
...
#: CALIBRATION_PAIRS_PER_RUNG_R17=10 冻结不变。
RT_C13_PAIRS_PER_RUNG = 60


def rt_main_profile_r17(small: bool = False) -> R17ExecutionProfile:
    """round-trip rehearsal main:rt3 namespace + 落盘。

    small=False:正式生成规模(R16 real-artifact 大模式;保留供
    显式大规模工程验证)。
    small=True(R17 shell 入口 r17_rt_rehearsal.sh 链路验证的
    默认):预登记工程缩小规模——rehearsal 目的是验证工程链
    (所有权/委派/raw logs/交付),不是统计;c13 2/rung、
    matched 4、semantic 8、independent 2/rung、supervised
    4/rung、equiv 1/rung(接口覆盖不变:同文件种类、同调用
    路径;§8.1 允许的 rehearsal 样本规模差异)。
    """
    return R17ExecutionProfile(
        name="rt_main", preplan=False, shadow=False,
        c13_eval_namespace="rt3_calibration_main_r17",
        equivalence_namespace="rt3_calibration_main_r17",
        supervised_namespace="rt3_supervised_main_r17",
        semantic_namespace="rt3_semantic_main_r17",
        c2_matched_namespace="rt3_calibration_main_r17",
        c2_independent_namespace="rt3_c2_independent_main_r17",
        ...
        c2_independent_pairs_per_rung=2 if small else
        ...
        equivalence_pairs_per_rung=1 if small else
        EQUIVALENCE_PAIRS_PER_RUNG_R17,
...
def rt_holdout_profile_r17(small: bool = False) -> R17ExecutionProfile:
    ...
    return R17ExecutionProfile(
        ...
        c13_eval_namespace="rt3_calibration_holdout_r17",
        equivalence_namespace="rt3_calibration_holdout_r17",
        supervised_namespace="rt3_supervised_holdout_r17",
        semantic_namespace="rt3_semantic_validation_r17",
        c2_matched_namespace="rt3_calibration_holdout_r17",
        c2_independent_namespace="rt3_c2_independent_holdout_r17",
        c13_pairs_per_rung=2 if small else RT_C13_PAIRS_PER_RUNG,
        c2_blocks=4 if small else RT_C2_BLOCKS_R17,
        semantic_blocks=8 if small else
        SEMANTIC_BLOCKS_PER_CORPUS_R17,
...
def preprocessing_robustness_checks_r17(
        routing_main: R17BundleRouting,
        routing_holdout: R17BundleRouting,
        ...
        pack: dict[str, Any], *,
        profile: R17ExecutionProfile,
        ledger: RoutingLedgerR17,
        profile_holdout: R17ExecutionProfile | None = None) -> dict[str, Any]:
    """§18 Preprocessing V2 重新资格审查(R17 版;结构承 R6 电池)。
    ...
    space_validation = validate_observation_space_v2(
        ...
        context="preprocessing_robustness_r17")
    ...
    return {
        "format": "cur261-r17-preprocessing-robustness-v1",
        "iteration": "r17",
...
def run_full_supervised_release_rehearsal(out_dir: Path) -> dict[str, Any]:
    ...
    训练面:
    - 正式 3 个 model seeds(R17_SUPERVISED_MODEL_SEEDS);
    ...
    参数。使用 preplan namespace(preplan_fit_main_r17 /
    preplan_supervised_main_r17 等)。
    """
    from rl_curriculum.curriculum261_r17_calibration import (
        fit_preprocessor_v2_from_bank_r17,
        generate_fit_bank_r17,
        supervised_learnability_run_r17,
    )
    from rl_curriculum.curriculum261_r17_param_pack import (
        r17_override_for,
    )
    from rl_curriculum.curriculum261_r17_shadow import _shadow_pack

    out_dir = Path(out_dir)
    pack = _shadow_pack()
    override_fn = lambda family, p: r17_override_for(family, p)  # noqa: E731
    roles: dict[str, Any] = {}
    any_nan_inf = False
    alignment_failures: list[str] = []
    for role, fit_ns, sup_ns in (
            ("main", "preplan_fit_main_r17", "preplan_supervised_main_r17"),
            ("holdout", "preplan_fit_holdout_r17",
             "preplan_supervised_holdout_r17")):
        records = generate_fit_bank_r17(fit_ns, pack, pairs_per_rung=2)
        bundle, manifest = fit_preprocessor_v2_from_bank_r17(
            fit_ns, pack, records=records, pairs_per_rung=2,
            parameter_pack_identity=str(pack.get("digest", "")))
        sup = supervised_learnability_run_r17(
            bundle, pack, namespace=sup_ns,
            pairs_per_rung=10, train_pair_limit=6,
            model_seeds=R17_SUPERVISED_MODEL_SEEDS)
        ...
        roles[role] = {
            ...
            "model_seeds": list(R17_SUPERVISED_MODEL_SEEDS),
        ...
        (out_dir / f"full_supervised_rehearsal_{role}.json").write_text(
            ...
            encoding="utf-8")
    result = {
        "format": "cur261-r17-full-supervised-release-rehearsal-v1",
        "development_evidence_only": True,
        "not_used_for_parameter_selection": True,
        "formal_model_seeds": list(R17_SUPERVISED_MODEL_SEEDS),
        "formal_gate_constants": R17_SUPERVISED_GATE,
        "roles": roles,
...
def orchestrate_calibration_stage_r17(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R17BundleRouting,
        routing_holdout: R17BundleRouting,
        records_main: list, records_holdout: list,
        profile_main: R17ExecutionProfile,
        profile_holdout: R17ExecutionProfile,
        override_fn: Any,
    ...
    repair R17(工作包 A):整个编排期间打开 generation invocation
    ...
    with sink:
        return _orchestrate_calibration_stage_inner_r17(
...
def _orchestrate_calibration_stage_inner_r17(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R17BundleRouting,
        routing_holdout: R17BundleRouting,
        ...
        design_digest: str | None = None,
        write_artifacts: bool = True) -> dict[str, Any]:
    """orchestrate_calibration_stage_r17 的执行核心(envelope sink 内)。"""
    from rl_curriculum.curriculum261_r17_calibration import (
        c2_independent_marginal_guard_r17,
        c2_matched_conditions_r17,
        run_c2_density_diagnostics_r17,
        run_c2_independent_corpus_r17,
        run_c2_matched_corpus_r17,
        run_c2_semantic_corpus_r17,
        run_calibration_corpus_c13_r17,
        supervised_learnability_run_r17,
    )
    ...
    ledger = RoutingLedgerR17()
    result: dict[str, Any] = {
        "format": "cur261-r17-calibration-orchestration-v1",
        "iteration": "r17",
    ...
    _write("bundle_routing_validation.json", {
        "contract": "cur261-r17-bundle-routing-contract-v1",
    ...
    for role, routing, profile in (
        ...
        v2_sup = require_eval_routing_r17(
            routing, profile.supervised_namespace,
            context=f"supervised_{role}", ledger=ledger)
        supervised[role] = supervised_learnability_run_r17(
            v2_sup, pack,
    ...
    # 密度诊断(跨双语料;诊断性质)
    density = run_c2_density_diagnostics_r17(
