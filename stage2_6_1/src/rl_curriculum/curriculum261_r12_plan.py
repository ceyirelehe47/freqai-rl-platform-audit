# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R12:final qualification plan(§26)。

双 strict gate(calibration + holdout 各自独立 PASS;无 pooled 救援)
前置强制;plan 绑定 R12 pack/cue semantic 合同/p_contract/recall
...
from rl_curriculum.curriculum261_r12_namespaces import (
    CURRICULUM261_ITERATION_ID_R12,
    qualification_r12_digest_path,
    qualification_r12_plan_path,
)

PLAN_FORMAT_R12 = "cur261-r12-qualification-plan-v1"

#: R12 代码身份(逐模块内容哈希;进入 plan;final 复算比对)。
PLAN_CODE_MODULES_R12 = (
...
def _code_identity_r12() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    for name in PLAN_CODE_MODULES_R12:
        f = root / name
...
def build_plan_r12(
    ...
    plan: dict[str, Any] = {
        ...
        "code_freeze": {
            ...
            "contract": ("正式 R12 数据开始前全部源码+测试已提交;"
                         "正式数据开始后任何源码变化 -> R12 永久结束"
                         "(§6/§21)"),
        },
        ...
        "prior_r10": {
            ...
            "role": ("R10 design/pack 全部结果仅作 development "
                     "evidence;R12 在全新 namespace 重做机械 design"),
        },
        ...
        "preprocessing_v2": {
            ...
            "implementation": "pinned vendor IFreqaiModel."
                              "define_data_pipeline(直接复用;R12 全新"
                              "语料重新资格验证)",
        ...
        "fit_bank_schedule": {
            "calibration": {
                "namespace": "preprocess_fit_calibration_r12",
                "pairs_per_rung": 4, "sides": ["A", "B"],
                "role": "main fit corpus(preprocessor only)"},
            "holdout": {
                "namespace": "preprocess_fit_holdout_r12",
                "pairs_per_rung": 4, "sides": ["A", "B"],
                "role": "independent fit holdout(preprocessor only)"},
            "final": {
                "namespace": "preprocess_fit_qualification_r12",
                "pairs_per_rung": 4, "sides": ["A", "B"],
        ...
        "semantic_corpus_schedule": {
            "design": {
                "namespaces": ["cue_semantic_design_main_r12",
                               "cue_semantic_design_validation_r12"],
                "blocks_per_corpus": 160,
                "ladder": "sentinel(candidate-independent)"},
            "calibration": {
                "namespace": "cue_semantic_calibration_r12",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "holdout": {
                "namespace": "cue_semantic_holdout_r12",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "final": {
                "namespace": "cue_semantic_qualification_r12",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
        },
        "final_sample_counts": {
        ...
        "qualification_bank_schedule": {
            "namespace": "qualification_r12",
            "c2_independent_marginal_namespace":
                "c2_independent_qualification_r12",
            "cue_semantic_namespace": "cue_semantic_qualification_r12",
            "fresh_seed_namespace": "fresh_holdout_r12",
        },
        ...
        "code_identity": _code_identity_r12(),
    }
    return plan


def plan_digest_r12(plan: dict[str, Any]) -> str:
...
def lock_plan_r12(plan: dict[str, Any]) -> tuple[Path, str]:
    ...
    path = qualification_r12_plan_path()
    if path.is_file():
        raise RuntimeError(
            "R12 qualification plan 已存在;plan lock 后禁止重写(§16.2/"
            "§26:任何修改须新 iteration R12)")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(plan, indent=2, ensure_ascii=False,
                               default=str), encoding="utf-8")
    digest = plan_digest_r12(plan)
    dpath = qualification_r12_digest_path()
    dpath.write_text(digest, encoding="utf-8")
    return path, digest


def load_locked_plan_r12() -> tuple[dict[str, Any], str]:
    path = qualification_r12_plan_path()
    dpath = qualification_r12_digest_path()
    if not path.is_file() or not dpath.is_file():
        raise RuntimeError("R12 qualification plan 未锁定")
    plan = json.loads(path.read_text(encoding="utf-8"))
    digest = plan_digest_r12(plan)
    if dpath.read_text(encoding="utf-8").strip() != digest:
        raise RuntimeError("R12 plan digest 复算不一致(fail closed)")
    return plan, digest


# ---------------------------------------------------- rehearsal plan(§12)
def build_rehearsal_qualification_plan_r12(
    ...
    绕过正式 build_plan_r12 的双 gate 断言(rehearsal 的小样本 gate
    结果不作资格判定),绑定 rehearsal digest 与共享 orchestrator
    身份;正式 plan 仍走 build_plan_r12 + lock_plan_r12。
    """
    plan = {
        "format": "cur261-r12-rehearsal-qualification-plan-v1",
        "iteration": "r12",
        "rehearsal": True,
    ...
    return plan, plan_digest_r12(plan)


def lock_qualification_plan_r12(
    ...
    path = lock_dir / "qualification_plan_r12.json"
    digest = plan_digest_r12(plan)
    plan = dict(plan)
    ...
    try:
        ...
        os.close(fd)
    (lock_dir / "qualification_plan_digest_r12.txt").write_text(
        digest, encoding="utf-8")
    return path, digest


def load_locked_qualification_plan_r12(
