#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""R21 有效收集环境审计器(v6 受控 full 配置的运行期执行端)。
...
与受控收集 hook 实现的来源:
  ...
  第三层(注册操作边界守卫,v6,R24/d872f41 审查反例):成功
  通知覆盖不了 ``register()`` 抛异常的部分安装路径——实测锚定
  目标环境 pytest 9.1.1 / pluggy 1.6.0:``PluginManager.register``
  先写 ``_name2plugin`` 再按 ``dir()`` 序逐个 verify/安装
  hookimpl;某 hookimpl 校验抛 PluginValidationError 时,先前
  已装入的钩子与注册表项**不回滚**,而 pytest 的
  ``pytest_plugin_registered.call_historic`` 只在
  ``super().register()`` 正常返回后发出。守卫对**本次运行的实际
  管理器实例**包一层 register(实例属性,不打全局类补丁;进程内
  测试自建的独立 PytestPluginManager 不受影响),异常退出后按
  实际对象核查(get_hookcallers/is_registered 公开 API)并立即
  append-only 落盘;原异常原样上抛,不吞、不改写为成功:
    ...
    - 确无任何装入且未入册的前置失败(如重名 ValueError 在写
      注册表前抛出)⇒ classification=rejected 事件留痕,不伪造
      参与事实,不阻断;
    - 捕获方随后的 unregister/回滚/末尾干净不消除已落盘事实;
      监测自身缺陷(register_guard_internal_error、
      register_guard_not_established、记录写失败)一律 fail
      closed。
...
AUDIT_FORMAT = "cur261-r24-collection-audit-v3"
...
def _handle_registration(plugin, plugin_name: str, manager) -> None:
    ...
    if lifecycle["monitor"] is None:
        ...
        guard = _install_register_guard(manager)
        lifecycle["register_guard"] = guard
        if guard is not None:
            _append_lifecycle_lines(
                [{"kind": "register_guard", **guard}])
...
def _install_register_guard(manager) -> dict | None:
    """注册操作边界窄适配(v6,R24/d872f41 审查反例)。

    pluggy 1.6.0 ``register()`` 先写 ``_name2plugin`` 再按 ``dir()``
    序逐个 verify/安装 hookimpl;某个 hookimpl 校验抛
    PluginValidationError 时,先前已装入的钩子与注册表项不回滚,
    而成功通知只在 ``super().register()`` 正常返回后发出。这里对
    **本次运行的实际管理器实例**包一层 register(实例属性;不打
    全局类补丁,进程内测试自建的独立 PytestPluginManager 不受
    影响),异常退出后按实际对象核查并落盘;原异常原样上抛。
    """
    lifecycle = _STATE["lifecycle"]
    if lifecycle.get("register_guard") is not None:
        return lifecycle["register_guard"]
    if manager is None:
        return None
    try:
        original = manager.register
    except (AttributeError, TypeError) as exc:
        fallback = {"kind": "register_guard_not_installable",
                    "error": repr(exc)[:200]}
        lifecycle["violations"].append(fallback)
        _STATE["violations"].append(fallback)
        try:
    ...
    def guarded_register(plugin, name=None, _original=original):
        try:
            return _original(plugin, name)
        except BaseException as exc:  # noqa: BLE001 —— 核查后原样上抛
            try:
                _handle_registration_exception(plugin, name,
                                               manager, exc)
            except Exception as internal:  # noqa: BLE001 —— 守卫自身缺陷
                fallback = {
                    "kind": "register_guard_internal_error",
                    "error": repr(internal)[:300]}
                lifecycle["violations"].append(fallback)
                _STATE["violations"].append(fallback)
                try:
                    _append_lifecycle_lines(
    ...
    guarded_register.__name__ = "r21_guarded_register"
    guarded_register.__qualname__ = (
        "r21_collection_auditor.r21_guarded_register")
    try:
        manager.register = guarded_register
    except (AttributeError, TypeError) as exc:
        fallback = {"kind": "register_guard_not_installable",
                    "error": repr(exc)[:200]}
        lifecycle["violations"].append(fallback)
    ...
    return {
        ...
        "wrapped_hook": "register",
        "coverage": "exception-exit state recheck",
        "installed_utc": _utc(),
        "pid": os.getpid(),
    }


def _installed_impls_for(plugin, manager) -> list[dict] | None:
    """异常退出后按实际对象归属在场实现(公开 get_hookcallers;
    含 specname 别名与受控面之外的 hook)。不可读 ⇒ None。"""
    try:
        ...
        for caller in sorted(callers,
            ...
            for impl in caller.get_hookimpls():
                ...
                specname = (impl.opts.get("specname")
                            if isinstance(impl.opts, dict) else None) \
                    or getattr(impl, "specname", "") or ""
                rows.append({"hook": getattr(caller, "name", ""),
                             "specname": specname or "",
                             "qualname": qualname[:160],
                             "origin_file": origin,
                             "origin_sha256": sha})
        return rows
    except Exception:  # noqa: BLE001 —— 状态不可证 ⇒ 按不可证处理
        return None


def _handle_registration_exception(plugin, requested_name,
                                   manager, exc) -> None:
    """register() 异常退出后的不可撤回检查点(v6)。

    按实际管理器状态记录(注册表残留、已装入实现、对象来源分类),
    判定与 v5 成功路径同规则;捕获方随后的 unregister/回滚/末尾
    干净不消除本检查点已落盘的事实。确无任何装入且未入册的前置
    失败(如重名 ValueError 在写注册表前抛出)如实留痕
    (classification=rejected),不伪造参与事实、不阻断。
    """
    if _STATE["manifest"] is None:
        _STATE["manifest"] = _load_manifest()
    if _STATE["out_path"] is None:
    ...
    try:
        remains = bool(manager.is_registered(plugin))
    except Exception:  # noqa: BLE001
        state_unreadable = True
    installed = _installed_impls_for(plugin, manager)
    if installed is None:
        state_unreadable = True
        installed = []
    classification, rel = _classify_registered(
        plugin, requested_name, _STATE["manifest"])
    if not remains and not installed and not state_unreadable:
        # 从未进入管理器(写入注册表前抛出)⇒ 真实前置失败态。
        reported = "rejected"
    else:
    ...
    event = {
        ...
        "plugin_name": str(requested_name
                           or manager.get_name(plugin)
                           or "")[:200],
        "module": _module_name(plugin)[:200],
        "origin_file": _module_of(plugin)[:300],
        ...
        "remains_registered": remains,
        "state_unreadable": state_unreadable,
        "installed_hooks": sorted({row["hook"] for row in installed}),
        "installed_impls": installed,
    }
    lifecycle["events"].append(event)
    pending: list[dict] = [{"kind": "event", **event}]
    violation = None
    if classification in ("core", "auditor"):
        pass
    elif classification == "conftest":
        approved = _STATE["manifest"].get("approved_generate_tests", {})
        for row in installed:
            if row["hook"] in FILTERING_HOOKS:
                violation = {
                    "kind": ("lifecycle_registration_exception_"
                             "partial_install"),
                    "plugin_name": event["plugin_name"],
                    "hooks": [row["hook"]],
                    "specnames": [row["specname"]],
                    "origin": row["origin_file"][:300],
        ...
        if installed:
            violation = {
                "kind": ("lifecycle_registration_exception_"
                         "partial_install"),
                "plugin_name": event["plugin_name"],
                "hooks": sorted({row["hook"] for row in installed}),
                "specnames": [row["specname"] for row in installed],
                "origin": event["origin_file"],
                "error_type": event["error_type"]}
        elif remains or state_unreadable:
            violation = {
                "kind": "lifecycle_registration_exception_unclean",
                "plugin_name": event["plugin_name"],
                "origin": event["origin_file"],
                "remains_registered": remains,
                "state_unreadable": state_unreadable,
...
def _document() -> dict:
    ...
    return {
        ...
        "lifecycle": {
            ...
            "register_guard": lifecycle["register_guard"],
...
def pytest_configure(config):
    ...
    if _STATE["lifecycle"]["monitor"] is None:
        ...
        _STATE["violations"].append(violation)
    if _STATE["lifecycle"]["register_guard"] is None:
        # v6 防御:注册边界守卫未建立(监测未生效/包裹失败)⇒
        # 部分安装路径不可证 ⇒ 不通过(fail closed)。
        violation = {"kind": "register_guard_not_established"}
        _STATE["lifecycle"]["violations"].append(violation)
        _STATE["violations"].append(violation)
