...
读回路径(v3:身份绑定 + 回执写入隔离,纯标准库,不 import 生成/评估栈):
...
- 身份绑定(v3 新增,按 cur261_generation_envelope 原合同):
  ...
  generator_identity 对照、base_params 与 rung_params 同名键对照;
- 只读与写隔离(v3):读回前后对源目录做(集合,字节哈希,大小,mtime)
  快照,最终快照在读回操作与回执写入全部完成后拍摄;回执目标及临时件
  在**写入前**做路径准入——与源切片树/外部 p52 依据等只读输入重叠
  (按解析后真实路径与父子关系判定,含 .. / 符号链接 / 已有别名)的
  目标一律拒绝(零源写入,非零退出),不跟随预置临时件链接;
...
def _resolve_target_strict(p: Path) -> Path:
    """解析目标的真实路径:已存在部分走 realpath(解析全部符号链接),
    不存在部分拼在已解析祖先之后;目标本身是符号链接时报歧义拒绝。"""
    p = Path(os.path.abspath(p))
    if p.is_symlink():
        raise ReportTargetRejected(
            f"回执目标本身是符号链接(不接受写入别名): {p}")
    missing: list[str] = []
    cur = p
    while not cur.exists() and cur != cur.parent:
        missing.append(cur.name)
        cur = cur.parent
    base = Path(os.path.realpath(cur))  # cur 存在:解析全部链接
    for name in reversed(missing):
        base = base / name
    return base


def _input_file_identities(prot: Path) -> set:
    """保护根内全部真实文件的 (st_dev, st_ino) 集合(硬链接别名检测)。"""
    ids: set = set()
    try:
        if prot.is_symlink() or not prot.exists():
            return ids
        if prot.is_file():
            st = prot.stat()
...
def _assert_report_target_safe(report_path: Path,
                               protect_roots: list) -> None:
    """写前准入:回执目标(含 _atomic_write_receipt 将创建的任何临时件
    所在的目标目录)不得落在任何只读输入之内。

    判定基于解析后真实路径与实际父子关系(非字符串前缀);对已存在
    目标另做文件身份核对(st_dev/st_ino,防指向输入文件的硬链接别名)。
    ...
    # 已存在目标的文件身份:与任何输入文件同 inode(硬链接别名)即拒绝
    if target.exists() and not target.is_symlink():
        tst = target.stat()
        t_id = (tst.st_dev, tst.st_ino)
        for prot in protect_roots:
            if t_id in _input_file_identities(Path(prot)):
                raise ReportTargetRejected(
                    f"report 目标与只读输入内文件是同一 inode"
                    f"(硬链接别名): {target}")
...
def _atomic_write_receipt(path: Path, payload: dict) -> None:
    """回执专用安全写:仅在与临时件目录均已通过准入后调用。

    与生成路径的 _atomic_write 不同:不用固定 .tmp 名,用独占创建
    (open 'x')的唯一临时名,不跟随预置链接;失败抛 ReceiptWriteError,
    不回退写入任何其他位置。
    """
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
...
def readback(out_dir: Path, report_path: Path,
    ...
    # 写前准入:先于任何为回执创建目录/打开临时件/写入的操作
    _assert_report_target_safe(report_path, protect)
    ...
    report = {
        "format": "r17-c3-engineering-slice-readback-v3",
        "engineering_only": True,
