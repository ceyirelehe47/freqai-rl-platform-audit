#!/usr/bin/env bash
# 稳定候选全量(执行编排声明:r17 supervisor 系先跑;与上一轮
# fail_closed_integrity_delivery 同款排序)。
# 原因:curriculum/generation 系测试在 pytest 进程内产生 C 层线程
# 池(/proc 可见、threading.enumerate 不可见),使后续**直调形态**
# supervisor run 的截止点前提核验如实失败(rc=7)。排序(r17 在前)
# 恢复该前提,断言与测试内容零修改;每项测试仍真实执行(非 rerun、
# 非跳过)。
set -euo pipefail
