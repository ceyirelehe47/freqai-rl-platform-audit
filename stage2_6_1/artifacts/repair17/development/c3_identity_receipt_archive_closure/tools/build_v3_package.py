#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""R17 c3-identity-receipt-archive-closure:本轮新验证包构建(v3 包)。

用与上轮相同的旧业务字节(BASE commit git blob + 工作树双核验)加本轮
新 reader(v3)建独立验证包;不替换、不重签上轮 c3_readback_decision_
delivery 的旧包(manifest/anchor/reader 均保持历史)。

用法(WSL 或发布树):
  build_v3_package.py build   --repo <repo> --out <pkg_dir>
...
BASE_COMMIT = "759598800d1a1b22b282a6c916c88d914cb2d623"
RUN_ID = "c3-irac-package-v3"
...
SOURCES: tuple[tuple[str, str, str], ...] = (
    ...
    *[("slice_pair_D%d_p%d" % (di, pi),
    ...
    ("reader_v3", "stage2_6_1/runner/r17_c3_engineering_slice.py",
...
def cmd_build(repo: Path, out: Path) -> int:
    if out.exists() and any(out.iterdir()):
        ...
        return 2
    payload = out / "payload"
    receipts = out.parent / "receipts_v3"
    ...
    # I02 自洽错配夹具(独立 tmp;整包复制→改 D0_p0 三下游+重算行哈希
    # →manifest 行与 anchor 重锚:字节层自洽,reader 必须语义拒绝)
    fixture = out.parent / "tmp_fixture_i02"
    if fixture.exists():
    ...
    dpath.write_text(json.dumps(doc, ensure_ascii=False, indent=1),
                     encoding="utf-8")
    rows_path = (fixture / "payload" / "engineering_slice"
                 / "slice_results.jsonl")
    ...
    idx_path = fixture / "payload" / "engineering_slice" / \
        "slice_results.jsonl"
    for r in frows:
        if r["role"] == "slice_pair_D0_p0":
            r["sha256"] = sha256_file(dpath)
            r["bytes"] = dpath.stat().st_size
        elif r["role"] == "slice_index":
            r["sha256"] = sha256_file(idx_path)
            r["bytes"] = idx_path.stat().st_size
    ...
    fr = run_reader(reader, fixture / "payload" / "engineering_slice",
                    ...
                    "D0_p52.json",
                    receipts / "fixture_i02_reader.json")
    fdoc = {}
    try:
        fdoc = json.loads(receipts.joinpath(
            "fixture_i02_reader.json").read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        pass
    checks = {p.get("check") for p in fdoc.get("problems", [])}
    report["steps"].append({
        "step": "fixture_i02_self_consistent",
        "verifier_rc": fv.returncode, "reader_rc": fr.returncode,
        "expected": {"verifier_rc": 0, "reader_rc": 1},
        "reader_problem_checks": sorted(checks)})
    ok_fixture = (fv.returncode == 0 and fr.returncode == 1
                  and "selected_envelope_output_binding" in checks)

    ok = (vr.returncode == 0 and sem.returncode == 0 and ok_fixture)
    report["build_ok"] = bool(ok)
    (out.parent / "build_report_v3.json").write_text(
