#!/usr/bin/env bash
# 真正使用新根的隔离冷读(WP3;在一次性 user+mount namespace 内验证)。
#
# 隔离事实(进程级,不动真实目录):
#   unshare --user --map-root-user --mount 内以 tmpfs 覆盖 /mnt
#   → 原开发证据根(/mnt/f/...)对验证进程完全不可访问;
#   只保留:新副本根(家目录侧)、系统解释器与标准库。
# 只读性:验证前后对 payload 记录全树 (mtime,size,sha256),必须不变。
#
# 用法: bash cold_read_isolated.sh <copy_root> <receipt_dir>
# 返回: 0=隔离冷读通过;1=验证失败;2=环境/用法错误
set -uo pipefail
COPY_ROOT="$(readlink -f "${1:?用法: cold_read_isolated.sh <copy_root> <receipt_dir>}")"
...
# 隔离验证(namespace 内执行;verify 回执与隔离事实先落副本内——
# /mnt 在 namespace 中被遮蔽,外层回执目录不可达;结束后由外层
# 搬运到隔离外持久 receipts,副本保持纯净)
# shellcheck disable=SC2016
INNER='
set -u
# 原根不可访问(前置事实):tmpfs 覆盖 /mnt(原开发证据根在 /mnt/f)
mount -t tmpfs tmpfs /mnt || exit 9
original_root_visible="no"
if ls /mnt/f >/dev/null 2>&1; then original_root_visible="yes"; fi
...
rc=$?
printf "%s" "{\"original_root_visible\":\"$original_root_visible\",\"verify_rc\":$rc,\"cwd\":\"$PWD\",\"argv\":\"$argv_dump\"}" > inner_isolation.json
exit $rc
'
...
try:
    ...
    pass
rec = {
    "schema": "r17-cold-read-isolated-v1",
    "copy_root": copy_root,
    "verify_rc": int(vrc),
    "payload_readonly_unchanged": ro == "yes",
    "isolation": {
        "method": "unshare --user --map-root-user --mount + tmpfs over /mnt",
        "original_root_visible": inner_data.get(
            "original_root_visible"),
        "verifier_cwd": inner_data.get("cwd"),
        "verifier_argv_note": inner_data.get("argv"),
    },
    "rc": (0 if (int(vrc) == 0 and ro == "yes"
                 and inner_data.get("original_root_visible") == "no")
           else 1),
}
...
PYEOF
# 回执 rc:隔离成立(原根不可访问)+verify rc0+只读不变 才是 0
