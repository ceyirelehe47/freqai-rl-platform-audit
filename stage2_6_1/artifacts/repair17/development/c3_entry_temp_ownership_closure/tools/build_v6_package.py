#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""R17 c3-entry-temp-ownership-closure:交接包路线验证包构建(v6 包)。

用相同旧业务字节(BASE=24eaa81 git blob + 工作树双核验)加本轮新
reader(交接包 receipt_block 实现:原末端条目 lstat/严格解析错误
分类/临时件所有权)建独立验证包;不替换、不重签此前任何轮次的旧包
(v3/v4/v5 均保持历史)。

v6 夹具(同 v5 语义,验证交接包实现):
...
用法(WSL 或发布树):
  build_v6_package.py build --repo <repo> --out <pkg_dir>
...
SOURCES: tuple[tuple[str, str, str], ...] = (
    ...
    *[("slice_pair_D%d_p%d" % (di, pi),
    ...
    ("reader_v6", "stage2_6_1/runner/r17_c3_engineering_slice.py",
...
def fixture_to01(reader: Path) -> dict:
    ...
    work = reader.parent.parent / "artifacts" / "repair17" / \
        ...
        "verification_v6" / "tmp_fixture_to01"
    if work.exists():
        shutil.rmtree(work)
    work.mkdir(parents=True)
    out = work / "out"
    ...
    spec = importlib.util.spec_from_file_location("eto_reader_v6", reader)
...
def cmd_build(repo: Path, out: Path, authority_src: Path) -> int:
    if out.exists() and any(out.iterdir()):
        ...
        return 2
    payload = out / "payload"
    receipts = out.parent / "receipts_v6"
    ...
    (out.parent / "build_report_v6.json").write_text(
