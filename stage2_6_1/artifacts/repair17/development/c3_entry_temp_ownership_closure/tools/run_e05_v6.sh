...
AGG="$VER/aggregate"
RUN="$DEV/run_supervision/runs/c3eto_full_20260909_r3"
...
cat > "$AGG/config_pos.json" <<EOF
{
 ...
 "c3_semantic_receipt": "$VER/receipts_v6/semantic_health.json",
 "c3_cold_read_report": "$VER/cold_read_report_v6.json",
 "old_counterexamples": "$CE/old_reader_counterexamples_v6.json",
 "fixed_flip": "$CE/fixed_reader_flip_v6.json",
...
for name in pos neg1 neg2; do
  "$CONDA_PY" "$TOOLS/aggregate_v6.py" --config "$AGG/config_$name.json" \
