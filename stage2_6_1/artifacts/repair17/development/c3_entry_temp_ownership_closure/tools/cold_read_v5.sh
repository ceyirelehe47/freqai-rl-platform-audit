#!/usr/bin/env bash
# R17 c3-entry-temp-ownership-closure:本轮 v5 包隔离冷读(矩阵 E01)。
# 用法(WSL): cold_read_v5.sh <package_dir> <work_root> <out_report_json>
...
[ -d "$PKG/payload" ] || { echo "FATAL: 非 v5 包目录: $PKG" >&2; exit 2; }
...
doc = {"schema": "c3-eto-cold-read-v5",
