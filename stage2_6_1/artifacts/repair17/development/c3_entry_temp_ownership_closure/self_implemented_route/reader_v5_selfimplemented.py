...
- 回执不覆盖(v4 新增):本次调用开始前已存在的最终目标一律拒绝
  ...
  创建的对象;
- 末端条目判定与解析错误分类(v5 新增):原末端条目在正确的父目录
  解析上下文中不跟随末端链接检查(resolved_parent+basename 的
  lstat/lexists,不从解析后 target.parent 倒推),跨目录悬空链接
  即使目标缺失也写前拒绝;词法非末端组件(保留 ..)逐组件只读
  分类,非目录祖先(含 file/../x 折叠形态与经链接解析为非目录)、
  悬空祖先链接、缺失之后又 .. 、权限/循环等解析错误与"可确认的
  缺失"分开,一律写前准入拒绝,不折叠、不改写、不放行;
- 临时件创建所有权(v5 新增):独占创建(O_EXCL)成功才取得该临时
  件的清理所有权;创建失败(EEXIST 碰撞/权限等)本次未创建任何
  对象,不进入清理路径,临时名位置既有对象(任何形态)保持原样;
  创建成功后的写入/发布失败只清理本次确实创建的临时件;清理
  自身失败如实传播(附注不掩盖原始错误),不以完成 PASS 掩盖;
...
rc 语义:
  ...
  2   = 用法错误/输出目录非空/回执目标写前准入被拒(含本次调用前
        已存在的显式目标、跨目录末端链接、非目录祖先/权限/循环等
        解析错误分类;零源写入)
  4   = p52 负例意外接受或证据缺陷(事实先落盘)
  5   = 读回内部异常
  6   = 读回回执写失败(准入已过,落盘仍失败;含临时件清理失败)
"""
...
def _resolve_target_strict(p: Path) -> Path:
    ...
    - realpath(任意 strict 模式)都会把"普通文件后接 .."词法折叠掉、
      把悬空祖先链接当作缺失(3.11.16 实测),不足以作为准入依据;
      因此先用词法非末端组件(保留 ..,Path 不折叠 ..)逐组件只读
      分类:非目录祖先(含经链接解析为非目录)、悬空祖先链接、
      缺失组件之后又 .. 、权限/循环/其他解析错误一律拒绝 —— 与
      "已确认祖先下的纯缺失后缀"分开,不折叠、不改写、不放行;
    - 存活路径用 realpath(strict=ALLOW_MISSING)(3.11.13+ 加入,
      3.11.16 实测仅容忍缺失、其余错误仍抛出;不可用时退化为
      strict=True 的 fail-closed 拒绝,绝不静默回退容忍一切错误
      的 strict=False)给出跟随链接后的规范位置;
    - 本函数只读:不创建、不修改任何对象;末端名与末端条目存在性
      判定不在此处(由 _assert_report_target_safe 按回执目标语义
      检查 —— parent/protect 根的末端 .. 是合法目录形态)。
    """
        ...
        if comp == "..":
            if missing_seen:
                raise ReportTargetRejected(
                    f"路径含缺失祖先后又 ..(歧义形态,写前拒绝): {p}")
            if stack:
                stack.pop()
            continue
        if missing_seen:
            continue  # 纯缺失后缀内的组件:无需再探测
        prefix = "/" + "/".join([*stack, comp])
        try:
            st = os.stat(prefix)
        except FileNotFoundError:
            if os.path.lexists(prefix):
                raise ReportTargetRejected(
                    f"祖先是悬空链接(歧义形态,写前拒绝): {prefix}"
                ) from None
            missing_seen = True
            continue
        except NotADirectoryError as exc:
            raise ReportTargetRejected(
                f"祖先不是目录(非目录错误,写前拒绝): {prefix}") from exc
        except PermissionError as exc:
            raise ReportTargetRejected(
                f"祖先不可访问(权限错误,写前拒绝): {prefix}") from exc
        except OSError as exc:
            raise ReportTargetRejected(
                f"祖先解析失败({type(exc).__name__},写前拒绝): "
                f"{prefix}") from exc
        if not stat.S_ISDIR(st.st_mode):
            raise ReportTargetRejected(
                f"祖先不是目录(非目录错误,写前拒绝): {prefix}")
        stack.append(comp)
    try:
        return Path(os.path.realpath(
            p, strict=getattr(os.path, "ALLOW_MISSING", True)))
    except OSError as exc:
        raise ReportTargetRejected(
...
def _assert_report_target_safe(report_path: Path,
    ...
    """写前准入(v5):返回确认后的回执最终路径(本次调用的唯一写入目标)。

    后续创建目录/打开临时件/发布/存在性检查与回执输出全部消费该
    确认结果 —— 检查目标与实际使用目标是同一个对象。规则:

    1. 目标、目标父目录(临时件所在)与全部保护根按真实文件系统
       语义解析(见 _resolve_target_strict;非目录祖先/悬空祖先
       链接/缺失后又 ../权限/循环等解析错误在解析内写前拒绝);
       确认目标或其父目录落在任何保护根之内即拒绝(真实父子关系,
       非字符串前缀;同名前缀兄弟目录不受影响);
    2. 原末端条目:在正确的父目录解析上下文(realpath(parent)
       给出的真实位置)中不跟随末端链接检查 —— 本次调用开始前
       已存在(任何形态:有效 JSON/空/普通文件/目录/链接条目,
       含跨目录悬空链接)一律拒绝;不从"解析后 target.parent+原
       名字"倒推(跨目录链接下那是指错位置的对象);
    3. 原末端条目不存在 ⇒ 末端无可跟随对象,确认写入目标与原末端
       条目同一位置(汇合一致性防御,不静默采用另一位置);
    4. 固定名临时件链接指向只读输入时拒绝(fail closed;检查基于
       确认位置)。
    ...
    if report_path.name in ("", ".", ".."):
        raise ReportTargetRejected(
            f"回执目标末端不是普通文件名(拒绝而非改写): {report_path}")
    ...
    # 原末端条目:正确的父目录真实位置 + 原名字,不跟随末端链接
    original_entry = target_dir / name
    if os.path.lexists(original_entry):
        raise ReportTargetRejected(
            f"回执最终目标本次调用前已存在(已有显式目标不覆盖,"
            f"新结果请另存): {original_entry}")
    confirmed = original_entry
    if confirmed != target:
        raise ReportTargetRejected(
            f"确认目标与语义解析结果不一致(歧义拒绝): "
            f"{confirmed} vs {target}")
    # 预置的固定名临时件链接:安全写从不使用固定 .tmp 名,但仍明确
    # 拒绝"确认位置目录内已存在指向输入的固定名链接"(fail closed)。
    fixed_tmp = confirmed.with_name(confirmed.name + ".tmp")
...
def _atomic_write_receipt(path: Path, payload: dict,
    ...
    """回执专用安全写(v5):只创建不替换;仅更新本次独占的未完成候选。

    path 必须是 _assert_report_target_safe 返回的确认目标(所有
    mkdir/临时件/发布都作用在确认位置)。

    - owned_id=None(首次发布):在目标父目录以 O_CREAT|O_EXCL 独占
      创建唯一临时名(不跟随/不覆盖该位置的既有条目),以
      os.link(tmp, path) 原子创建最终目标 —— 目标已存在时
      FileExistsError,绝不替换既有文件(与 os.replace 的本质
      区别);发布后删除临时件,返回新目标的 (st_dev, st_ino)
      供本次调用后续更新做所有权核对。
    ...
    - 临时件所有权(v5):清理责任从独占创建成功才开始。创建失败
      (EEXIST 碰撞/权限等)本次未创建任何对象,不进入清理路径,
      临时名位置既有对象(普通文件/链接条目,任何形态)保持原样;
      创建成功后的写入/flush/fsync/发布失败只清理本次确实创建
      并拥有的临时件;清理自身失败如实传播 —— 原始异常在传播中
      则以附注(PEP 678)记录,不掩盖原始错误原因;无原始异常
      (发布已完成)则以清理失败为本次失败,非零,不打印完成
      PASS,未清理对象的位置记录在错误信息中。
    - 句柄责任:底层 os.open 成功后本次立即接管该描述符;fdopen
      包装失败时先关闭描述符再传播,不泄漏;写入异常由 with
      保证关闭。
    ...
    try:
        ...
        tmp_created = True  # 独占创建成功:本次拥有该临时件
        try:
            fh = os.fdopen(fd, "w", encoding="utf-8")
        except BaseException:
            os.close(fd)  # 包装失败:句柄责任仍在本次,先关闭再传播
            raise
        with fh:
        ...
        if tmp_created and tmp is not None:
            # 采样须在 unlink 之前:try 块异常传播中 sys.exc_info() 即
            # 原始异常;正常 return 时为 None(except 块内采到的只会是
            # unlink 自己的错误,不是原始错误)
            ...
            try:
                ...
                pass  # os.replace 已消耗该名,或并发下已消失
            except OSError as exc:
                note = (f"临时件清理失败(未清理对象保留: {tmp}): "
                        f"{type(exc).__name__}: {exc}")
                if active is not None:
                    active.add_note(note)  # 原始错误保持传播
                else:
                    raise ReceiptWriteError(note) from exc
...
def readback(out_dir: Path, report_path: Path,
    ...
    report = {
        "format": "r17-c3-engineering-slice-readback-v5",
        "engineering_only": True,
