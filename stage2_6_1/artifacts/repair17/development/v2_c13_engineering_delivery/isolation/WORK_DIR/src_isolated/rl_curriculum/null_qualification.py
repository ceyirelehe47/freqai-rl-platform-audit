...
精确计算 1-[(1-fee)/(1+fee)]*[(1-slip)/(1+slip)] = 0.002/1.001 =
0.001998001998...;阶段 2.6.0e 废除旧错误公式 1-(1-fee)^2*
(1-slippage)^2 = 0.001999);生成器参数通道
...
#: 已弃用格式(v1 文档声明式 / v2 布尔 + bar 级统计 / v3 旧错误摩擦
#: 公式 1-(1-fee)^2*(1-slippage)^2)不得被新执行器接受
...
def qualify_null_family(
    ...
    *,
    ...
    # 阶段 2.6.0f 工作包 C:episode_bars 从参数解析器取得(fail
    # closed;支持 episode_bars / duration_hours 双通道)——不再存在
    # 缺键静默回退 96 的默认值
    ...
    try:
        episode_bars = int(resolve_duration(
            dict(params), timeframe)["resolved_bars"])
    except ParamResolutionError as exc:
        raise NullQualificationError(
            f"family-level 资格审查的 Episode 时长无法解析: {exc}"
            f"(必须显式声明 episode_bars 或 duration_hours;"
            f"不得使用默认值)") from exc
...
def verify_null_qualification_bindings(
    ...
    *,
    ...
    for family in required_families:
        ...
        # ---- margin 只能来自规范(A4;阶段 2.6.0e:冻结账本精确公式)
        ...
        if margin_block.get("derivation", {}).get("formula") != \
            ...
            report_problems.append(
                "报告 margin 推导公式与冻结账本公式不一致(margin 只能"
                "来自 qualification spec;旧公式 1-(1-fee)^2*"
                "(1-slippage)^2 已被废除)")
        if margin_block.get("derivation", {}).get("units") != \
                "simple_return":
            report_problems.append(
                "报告 margin 单位必须为 simple_return(net_return 尺度)")
        # ---- 三态结论对账
