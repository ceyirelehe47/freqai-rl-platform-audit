# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R11:一次性 final qualification(§27-§29)。
...
9. marker -> completed;任何异常 -> marker -> crashed + crash evidence
   落盘 + re-raise(保留 marker,禁止修复后复用 qualification_r11)。
...
FINAL_FORMAT_R11 = "cur261-r11-final-qualification-v1"
...
def qualification_r11_lock_dir_default() -> Path:
    from rl_curriculum.curriculum261_r11_namespaces import (
...
def _fresh_seed_validity_r11(n_checks: int = 10) -> dict[str, Any]:
    """fresh_holdout_r11 与 qualification_r11 的 seed 不相交验证。"""
    from rl_curriculum.curriculum261_api import derive261_seed

    mismatches = []
    for i in range(n_checks):
        for family in CURRICULUM261_FAMILIES:
            q = derive261_seed("qualification_r11", family, "D2", i, 0)
            for rung in CURRICULUM261_RUNGS:
                f = derive261_seed("fresh_holdout_r11", family, rung, i, 0)
                if q == f:
...
def run_final_qualification_r11(out_dir: Path,
                               vendor_dir: Path | None = None,
                               ) -> dict[str, Any]:
    """执行一次性 R11 final qualification(§28)。"""
    require_r11_iteration_active()
    if qualification_r11_exposed():
        raise RuntimeError(
            "R11 final qualification 已执行过(exposure marker/ledger "
            "存在)——同一 qualification_r11 corpus 不得再次执行;继续必须"
            "使用新迭代身份(R11)与全新 seed space")
    plan, digest = load_locked_plan_r11()
    if (plan.get("robustness_gate") or {}).get("pass") is not True:
        raise RuntimeError(
            "R11 plan 的 robustness_gate.pass != true——final "
            "qualification fail closed")
    pack = load_selected_pack(qualification_r11_lock_dir_default())
    ...
    code_ok = (plan["code_identity"] == _code_identity_r11())
    ...
    if not static_ok:
        raise RuntimeError(
            ...
            "之前 fail closed——本轮 qualification_r11 未消耗")
    ...
    if not freeze["pass"]:
        raise RuntimeError(
            f"R11 code freeze 校验失败(正式数据开始后源码漂移;"
            f"§6/§21 永久结束):{freeze}")
    ...
    try:
        with QualificationR11FileLock(blocking=False):
            write_qualification_r11_exposure(digest, status="running")
            result = execute_final_core_r11(
                out_dir, plan, pack, digest=digest, started=started,
                profile_name="formal_final",
                final_namespace="qualification_r11",
                fit_namespace="preprocess_fit_qualification_r11",
        ...
        try:
            (out_dir / "qualification_crash_traceback.log").write_text(
                exc, encoding="utf-8")
            write_qualification_r11_exposure(digest, status="crashed")
        except Exception:  # noqa: BLE001 - 崩溃路径不得掩盖原始异常
            pass
        raise


def execute_final_core_r11(
        out_dir: Path, plan: dict[str, Any], pack: dict[str, Any], *,
        digest: str = "", started: str | None = None,
        profile_name: str = "formal_final",
        final_namespace: str = "qualification_r11",
        fit_namespace: str = "preprocess_fit_qualification_r11",
    ...
    正式:run_final_qualification_r11(exposure/plan/freeze 治理外壳)
    以 qualification_r11 全量参数调用;
    rehearsal:preplan 以 preplan_final_r11 + 微样本参数 + 临时
    exposure 目录调用(§12.2 final-like runner);
    shadow:full-scale shadow(工作包 C)以 shadow_fit_main_r11 +
    ...
    *_override 参数仅供 shadow(工程覆盖;正式路径不传,保持
    ...
    - reference equivalence 走 canonical 合同(reference_equivalence_
      run_r11:canonical vs scaled 逐位 + legacy 差异 + 0 unexplained),
    ...
    with sink:
        return _execute_final_core_inner_r11(
            out_dir, plan, pack, digest=digest, started=started,
...
def _execute_final_core_inner_r11(
        out_dir: Path, plan: dict[str, Any], pack: dict[str, Any], *,
        digest: str = "", started: str | None = None,
        profile_name: str = "formal_final",
        final_namespace: str = "qualification_r11",
        ...
        conditioning_fit_namespace: str | None = None,
        semantic_out_dir: Path | None = None) -> dict[str, Any]:
    """execute_final_core_r11 的执行核心(envelope sink 内)。"""
    kappa = float(plan["statistics_rule"]["kappa"]) \
        if "statistics_rule" in plan else 1.5
    if c2_blocks is None:
        c2_blocks = int(plan["final_sample_counts"]["c2_matched_blocks"])
    ...
    final_records = generate_fit_bank_r11(fit_namespace, pack)
    final_v2, final_manifest = fit_preprocessor_v2_from_bank_r11(
        fit_namespace, pack, records=final_records,
        parameter_pack_identity=pack["digest"])
    routing_final = build_routing_r11(
    ...
    space_validation = validate_observation_space_v2(
        ...
        [int(r.episodes[s].spec.seed) for r in contain_records
         for s in ("A", "B")], context="final_fit_space_v2_r11")
    ...
    for family in CURRICULUM261_FAMILIES:
        ...
        family_reports[family] = rung_report_r4(
            records, family, r11_family_rung_params(family, pack),
    ...
    c2_indep = run_c2_independent_corpus_r11(
        ...
        or ("c2_independent_qualification_r11" if not rehearsal
            else final_namespace),
        pairs_per_rung=int(independent_pairs_per_rung))
    c2_conditions = c2_matched_conditions_r11(c2_matched, pack)
    c2_marginal = c2_independent_marginal_guard_r11(
        c2_indep, pack, recall_floor_value)
    semantic_ns = semantic_namespace_override or (
        "cue_semantic_qualification_r11" if not rehearsal
        else "preplan_semantic_main_r11")
    ...
    semantic = run_c2_semantic_corpus_r11(
        ...
        artifact_name=semantic_artifact_filename_r11(semantic_ns))
    ...
    reference_report = reference_equivalence_run_r11(
    ...
    if not rehearsal or shadow:
        causality = {
            ...
            "reference_causality": [
                check_reference_causality(
                    f, r11_family_rung_params(f, pack)["D2"],
                    thresholds[f])
                for f in CURRICULUM261_FAMILIES],
        }
    repro = []
    if not rehearsal or shadow:
        ...
        repro_pack_override = []
        for family in CURRICULUM261_FAMILIES:
            ov = r11_override_for(family, pack) or {}
        ...
        c2_ladder = r11_family_rung_params(FAMILY_C2, pack)
    ...
    fresh = (_fresh_seed_validity_r11(10)
    ...
    cond_records = generate_fit_bank_r11(
        conditioning_fit_namespace or "preprocess_fit_calibration_r11",
    ...
    supervised_ns = supervised_namespace_override or (
        ...
        else "preplan_supervised_main_r11")
    supervised = supervised_learnability_run_r11(
        final_v2, pack, namespace=supervised_ns,
        pairs_per_rung=(supervised_pairs_per_rung_override
                        or (CALIBRATION_PAIRS_PER_RUNG_R11
    ...
    semantics = run_c2_diagnostics_r11(
        [rec for blk in c2_matched["blocks"]
