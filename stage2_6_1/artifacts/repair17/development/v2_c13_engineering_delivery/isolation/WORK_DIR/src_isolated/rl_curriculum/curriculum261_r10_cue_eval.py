# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R10:cluster-aware cue 语义评估器(unique event)。
...
- §14/§15 dedicated semantic corpus(160 matched blocks/corpus,
  ...
  R10 design FAIL,不进入 candidate 设计;
...
R10_CUE_BOOTSTRAP_RESAMPLES = 20000
R10_CUE_BOOTSTRAP_SEED = 20261203
...
def canonical_cue_observations(
    ...
    cue_table_digest = "r10ct-" + hashlib.sha256(
...
def cluster_bootstrap_rate(
        ...
        n_boot: int = R10_CUE_BOOTSTRAP_RESAMPLES,
        seed: int = R10_CUE_BOOTSTRAP_SEED,
) -> dict[str, Any]:
...
def semantic_cue_gate(
    ...
    - tail-position 诊断(报告,不作独立 gate——audit 阶段已专项
      闭合)。
    任一 FAIL => R10 design FAIL(不进入 candidate 设计)。
    """
    ...
    return {
        "format": "cur261-r10-semantic-cue-gate-v1",
        "label": label,
...
def candidate_cue_semantics(
    ...
    return {
        "format": "cur261-r10-candidate-cue-semantics-v1",
...
def cue_semantic_rule_identity() -> str:
    """R10 cue 语义 gate 规则身份(冻结;plan/pack 绑定)。"""
    payload = {
        ...
        "bootstrap_resamples": R10_CUE_BOOTSTRAP_RESAMPLES,
        "bootstrap_seed": R10_CUE_BOOTSTRAP_SEED,
    ...
    return "r10csg-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
...
def independent_cue_semantics(
    ...
    R10 变更:recall 正式 cluster gate 已由 160-block dedicated
    ...
    for rung in covered_rungs2:
        ...
        for side in ("A", "B"):
            ...
            prec = cluster_bootstrap_rate(
                ...
                side="lower")
            per_side[side] = {
                # R10 §26:precision/false-cue 降级为诊断(不进 gate)
                "payoff_false_cue": {
                    **fc, "max": C2_PAYOFF_BAR_FALSE_CUE_MAX,
                    "pass": bool(
                        fc["bound"] <= C2_PAYOFF_BAR_FALSE_CUE_MAX)},
    ...
    return {
        "format": "cur261-r10-independent-cue-semantics-v1",
        "candidate": candidate_id,
