# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R14:clean matched-ladder design(dedicated semantic
corpus 先行 + corrected plan digest roundtrip + §21/§22 流程 + §18
一次性硬规则)。

R14 相对 R8 的工程修复(规范 §5/§6/§8;统计合同零修改):
...
- R7 design plan 的 digest 自引用缺陷:lock 把 design_plan_digest
  ...
  evidence)。R14 修复(§8):digest 复算排除 (locked_utc,
  design_plan_digest) 双字段;payload 自带 digest 同时可经正式
  load_locked_design_plan_r14 复算——正式 lock/load/digest recompute/
...
- R7 shared cue gate 直接用 40-block candidate design corpus,SE
  ...
  floor 0.930)。R14(§14/§15)把 candidate-independent cue 语义拆到
  预注册 160-block dedicated semantic corpus(每 corpus 独立 gate,
  FAIL 即 R14 design FAIL,不进入 candidate 设计);
- R7 的 R6 marginal-guard semantics 键错配(cue_semantics vs R6 冻结
  实现要求的 cue_payoff_separation;R7 calibration 未执行故未暴露)
  ——R14 显式组装:R6 统计条件 + 本模块语义条件(local cue ∧
  context ∧ v2 independent cue semantics)。

§8.4 一次性硬规则:正式 R14 design plan 锁定后发生任何 loader/代码
缺陷 => write_r14_iteration_aborted + R14 永久结束(保留 plan、不重锁、
不复用 namespace、下一轮 R15);run_design_stage_r14 内部任何
...
§7 要求:R14 在 plan lock 前验证这些模块与 R7 baseline 的 SHA256
...
from rl_curriculum.curriculum261_r14_param_pack import (
    ...
    ladder_distance_from_historical_r14,
    load_selected_pack,
    pack_digest_r14,
    r14_candidate_grid,
    validate_r14_grid_semantics,
    write_selected_pack_r14,
)

DESIGN_FORMAT_R14 = "cur261-r14-design-plan-v1"
DESIGN_BLOCKS_PER_CORPUS_R14 = 40
DESIGN_NAMESPACES_R14 = ("design_r14_matched_main",
                        "design_r14_matched_validation")
DESIGN_INDEPENDENT_NAMESPACE_R14 = "design_r14_independent_marginal"
DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R14 = 20

#: §14/§15 dedicated semantic corpus(candidate-independent;在任何
#: R14 design data 生成前锁定;数据后禁止 160→240 扩样/第三 corpus/
#: 合并/删除)。
SEMANTIC_BLOCKS_PER_CORPUS_R14 = 160
SEMANTIC_NAMESPACES_R14 = ("cue_semantic_design_main_r14",
                          "cue_semantic_design_validation_r14")

#: §R14-8 semantic artifact 显式穷尽映射(禁止 endswith/后缀启发式;
#: R8 的 endswith("main") 使 main artifact 被 validation 覆盖)。
SEMANTIC_ARTIFACT_MAP_R14 = {
    "cue_semantic_design_main_r14": "semantic_design_main.json",
    "cue_semantic_design_validation_r14":
        "semantic_design_validation.json",
}
#: calibration/holdout/final 三阶段的 semantic artifact 显式映射。
SEMANTIC_STAGE_ARTIFACT_MAP_R14 = {
    "cue_semantic_calibration_r14": "cue_semantic_calibration.json",
    "cue_semantic_holdout_r14": "cue_semantic_holdout.json",
    "cue_semantic_qualification_r14":
        "qualification_cue_semantics.json",
    # preplan full pipeline rehearsal(§12;非正式;同一 writer)
    "preplan_semantic_main_r14": "preplan_semantic_main.json",
    "preplan_semantic_validation_r14":
        "preplan_semantic_validation.json",
    # full-scale shadow(工程;R14 工作包 C)
    "shadow_semantic_main_r14": "shadow_semantic_main.json",
    "shadow_semantic_validation_r14": "shadow_semantic_validation.json",
    "shadow_semantic_final_r14": "shadow_semantic_final.json",
    # R14RealArtifactCliRoundTrip-v1(rehearsal-only;§四-4)
    ...
    "rt3_semantic_validation_r14": "rt_semantic_validation.json",
    "rt3_semantic_final_r14": "rt_semantic_final.json",
}
SEMANTIC_CORPUS_ROLE_R14 = {
    "cue_semantic_design_main_r14": "main",
    "cue_semantic_design_validation_r14": "validation",
    "cue_semantic_calibration_r14": "calibration",
    "cue_semantic_holdout_r14": "holdout",
    "cue_semantic_qualification_r14": "qualification",
    "preplan_semantic_main_r14": "preplan_main",
    "preplan_semantic_validation_r14": "preplan_validation",
    "shadow_semantic_main_r14": "shadow_main",
    "shadow_semantic_validation_r14": "shadow_validation",
    "shadow_semantic_final_r14": "shadow_final",
    "rt_semantic_design_main_r14": "rt_design_main",
    "rt_semantic_design_validation_r14": "rt_design_validation",
    "rt3_semantic_main_r14": "rt_main",
    "rt3_semantic_validation_r14": "rt_validation",
    "rt3_semantic_final_r14": "rt_final",
}


def semantic_artifact_filename_r14(namespace: str) -> str:
    """§R14-8:namespace → 文件名的唯一合法路径(穷尽映射)。"""
    for table in (SEMANTIC_ARTIFACT_MAP_R14,
                  SEMANTIC_STAGE_ARTIFACT_MAP_R14):
        if namespace in table:
            return table[namespace]
    known = sorted(set(SEMANTIC_ARTIFACT_MAP_R14)
                   | set(SEMANTIC_STAGE_ARTIFACT_MAP_R14))
    raise RuntimeError(
        f"semantic namespace {namespace} 不在显式映射表(§R14-8:禁止 "
        f"endswith/后缀启发式;已知 namespace: {known})")


def semantic_artifact_identity_r14() -> dict[str, Any]:
    """§R14-8 artifact writer 身份(进入 design plan code identity)。"""
    return {
        "design_mapping": dict(SEMANTIC_ARTIFACT_MAP_R14),
        "stage_mapping": dict(SEMANTIC_STAGE_ARTIFACT_MAP_R14),
        "roles": dict(SEMANTIC_CORPUS_ROLE_R14),
...
def write_semantic_artifact_r14(out_dir: Path, namespace: str,
                               payload: dict[str, Any],
                               plan_digest: str,
                               event_rows: list[dict[str, Any]] | None
                               = None,
                               ) -> Path:
    """§R14-8:exclusive-create 写 semantic artifact(json + jsonl),
    ...
    fname = semantic_artifact_filename_r14(namespace)
    path = out_dir / fname
    body = dict(payload)
    body["namespace"] = namespace
    body["corpus_role"] = SEMANTIC_CORPUS_ROLE_R14[namespace]
    ...
    if (back.get("namespace") != namespace
            ...
            != SEMANTIC_CORPUS_ROLE_R14[namespace]):
        raise RuntimeError(
...
#: §16.1/§21 code identity 覆盖面(R14 全部实现 + 冻结复用模块 +
#: 合同依赖;design data 生成开始后任何漂移 => iteration aborted)。
DESIGN_CODE_MODULES_R14 = (
    ...
    "curriculum261_r14_noise_replay.py",
    "curriculum261_r14_cue_contract.py",
    "curriculum261_r14_cue_eval.py",
    "curriculum261_r14_dependencies.py",
    "curriculum261_r14_routing.py",
    "curriculum261_r14_reference.py",
    "curriculum261_r14_labels.py",
    "curriculum261_r14_orchestrator.py",
    "curriculum261_r14_delegation.py",
    "curriculum261_r14_rehearsal.py",
    "curriculum261_r14_namespaces.py",
    "curriculum261_r14_param_pack.py",
    "curriculum261_r14_preplan.py",
    "curriculum261_r14_design.py",
    "curriculum261_r14_calibration.py",
    "curriculum261_r14_plan.py",
    "curriculum261_r14_preflight.py",
    "curriculum261_r14_final.py",
    "curriculum261_r14_smoke.py",
...
def _code_identity_design() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    for name in DESIGN_CODE_MODULES_R14:
        f = root / name
...
def verify_matched_core_identity_r10() -> dict[str, Any]:
    ...
    R10 的 design plan 在 lock 时记录了全部依赖模块的 sha256;R14
    不修改 c2 generator / matched tape / matched pairs 三个模块,
    当前哈希必须与 R10 记录值逐位一致(R14 对 api.py 的改动仅限
    R14 namespace 白名单、解锁守卫与被动 recorder 钩子的追加,
...
# ------------------------------------------------------------- plan
def design_plan_payload_r14(*, baseline_commit: str, vendor_pin: str,
    ...
    """构建并返回 R14 design plan payload(锁定后不得修改任何字段)。
    ...
    namespace/scale 覆盖参数默认为正式值;R14RealArtifactCliRoundTrip
    ...
    """
    grid = r14_candidate_grid()
    problems = validate_r14_grid_semantics()
    if problems:
        raise RuntimeError(f"R14 candidate grid 语义非法: {problems}")
    ...
    return {
        "format": DESIGN_FORMAT_R14,
        "iteration": "r14",
        "rehearsal": bool(rehearsal),
        "rehearsal_contract": (
            "R14RealArtifactCliRoundTrip-v1(rehearsal-only namespace;"
        ...
        "plan_digest_contract": {
            ...
            "roundtrip": "build -> lock(O_CREAT 语义:已存在即拒) -> "
                         "load_locked_design_plan_r14(独立进程) -> "
        ...
        "semantic_corpora": {
            ...
            "ladder": "冻结 sentinel ladder(cur261-c2-v9 默认 D0-D3;"
                      "candidate-independent;与任何 R14 candidate 数值"
                      "无依赖)",
            ...
            "gate": "每 corpus 独立:unique-event coverage ≥3600 / "
                    ...
                    "replay 完整性;任一 FAIL => R14 design FAIL(§15)",
        ...
        "independent_marginal_guard": {
            ...
            "conditions": "marginal ordering / D3 positive / 基线 margins"
                          ...
                          ";matched PASS 不可覆盖 FAIL;失败 → R14 = FAIL",
        ...
        "fail_path": {
            ...
            "on_semantic_gate_fail": "任一 semantic corpus gate FAIL → "
                                     "R14 design FAIL(不生成 candidate "
                                     "blocks;§15/§25)",
            "on_no_qualified_combination": "R14 = FAIL;自动 power/semantic "
...
def design_plan_digest_r14(plan: dict[str, Any]) -> str:
    ...
    return "r14dp-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def lock_design_plan_r14(out_dir: Path, plan: dict[str, Any],
                        ) -> tuple[Path, str]:
    ...
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "r14_design_plan.json"
    if path.exists():
        raise RuntimeError(
            f"R14 design plan 已存在: {path}(§8.4 禁止删除/覆盖/重锁;"
            "重锁必须换新 iteration R14)")
    digest = design_plan_digest_r14(plan)
    plan = dict(plan)
    ...
    (out_dir / "r14_design_plan_digest.txt").write_text(
        digest, encoding="utf-8")
    return path, digest


def load_locked_design_plan_r14(out_dir: Path,
    ...
    path = out_dir / "r14_design_plan.json"
    if not path.is_file():
        raise RuntimeError(f"R14 design plan 未锁定: {path}")
    plan = json.loads(path.read_text(encoding="utf-8"))
    digest = design_plan_digest_r14(plan)
    if plan.get("design_plan_digest") != digest:
        raise RuntimeError("R14 design plan digest 复算不一致(fail closed)")
    digest_path = out_dir / "r14_design_plan_digest.txt"
    if not digest_path.is_file() or \
            digest_path.read_text(encoding="utf-8").strip() != digest:
        raise RuntimeError("R14 design plan digest 文件与 payload 不一致")
    return plan, digest
...
def _evaluate_candidate_matched_r14(
    ...
    # 密度(R6 冻结 gate)+ R14 candidate-specific cue 语义 + 独立性/
    ...
    semantics = {
        ...
        "context_observability": check_c2_context_observability(
            records),
        "candidate_cue_semantics_r14_cluster_aware": cue_sem,
        "cue_payoff_confusion": {
            k: v for k, v in cue_sem["per_rung"]["D0"]["sides"]["A"][
                "confusion"].items()},
        # R6 点阈值分离检查仅作诊断对照(绝不进入 R14 资格判定)
...
def _gap_series(block_table: dict[str, Any], hi: str, lo: str):
    # R14:block_gap_series 模块级解析(§6.2)。
...
def _maximin_score_r14(corpus_results: list[dict[str, Any]],
                      n: int) -> float:
    ...
    for res in corpus_results:
        ...
        for per_rung in res["semantics"][
                "candidate_cue_semantics_r14_cluster_aware"][
                "per_rung"].values():
...
# ------------------------------------------------- 主流程
def run_design_stage_r14(out_dir: Path, plan: dict[str, Any],
                        design_digest: str,
                        baseline_commit: str = "") -> dict[str, Any]:
    """R14 design 主流程:治理检查 → semantic corpora(160×2)+ gate
    (FAIL 即短路)→ 3 candidate × 2 corpus blocks → candidate 评估 →
    机械选择 → marginal guard → pack。

    §8.4:plan 锁定后本函数内部任何异常都转 write_r14_iteration_aborted
    + re-raise(R14 永久结束;不删 plan、不重锁、不复用 namespace)。
    """
    require_r14_iteration_active()
    out_dir = Path(out_dir)
    try:
        return _run_design_stage_inner_r14(
        ...
        write_r14_iteration_aborted(
            ...
            f"{str(exc)[:300]});按 §8.4 R14 永久结束")
        raise


def _run_design_stage_inner_r14(out_dir: Path, plan: dict[str, Any],
                               design_digest: str,
                               baseline_commit: str,
                               ) -> dict[str, Any]:
    ...
    if not identity["pass"]:
        raise RuntimeError(
            f"R14 design plan code identity 与当前代码不一致:"
    ...
    # 第一条 R14 design/semantic episode 生成前记录 started(§8.4)
    mark_design_data_started()
    ...
    if len(set(semantic_hashes)) != len(semantic_hashes):
        raise RuntimeError(
            ...
            "§R14-8 禁止生成 candidate blocks)")
    if not semantic_pass:
        summary = {
            "format": "cur261-r14-design-stage-v1",
            "iteration": "r14",
            ...
            "pass": False,
            "verdict": "R14 design FAIL:dedicated semantic corpus gate "
        ...
        (out_dir / "r14_sample_size_selection.json").write_text(
    ...
    for cand_id, ladder in grid.items():
        corpora = [
            _evaluate_candidate_matched_r14(
        ...
        for n in FORMAL_BLOCK_OPTIONS:
            ...
            if qualified_ns[str(n)]:
                scores[str(n)] = _maximin_score_r14(corpora, n)
        candidate_results[cand_id] = {
            ...
            "param_distance_from_historical": (
                ladder_distance_from_historical_r14(ladder)),
        }
    (out_dir / "r14_candidate_results.json").write_text(json.dumps(
    ...
    power = _build_power_summary_r14(
        candidate_results, semantic_summaries, selected_id, selected_n,
        design_digest)
    (out_dir / "r14_power_analysis.json").write_text(json.dumps(
    ...
    if selected_id is None:
        summary = {
            ...
            "pass": False,
            "verdict": "R14 FAIL:candidate × block count 无合格组合"
    ...
    if not marginal_artifact["guard"]["pass"]:
        summary = {
            ...
            "pass": False,
            "verdict": "R14 FAIL:选定 ladder 的独立-rung marginal guard "
    ...
    pack = ladder_pack_payload_r14(
        ...
        recall_floor_value=floor,
        noninferiority_delta=NONINFERIORITY_DELTA,
        semantic_blocks_per_corpus=SEMANTIC_BLOCKS_PER_CORPUS_R14,
    ...
    write_selected_pack_r14(out_dir, pack)
    ...
    selection = {
        "format": "cur261-r14-sample-size-selection-v1",
        "iteration": "r14",
        "design_plan_digest": design_digest,
        "selected_candidate": selected_id,
        "selected_block_count": selected_n,
    ...
    (out_dir / "r14_sample_size_selection.json").write_text(json.dumps(
...
def _run_independent_marginal_guard(
    ...
    """§26:design_r14_independent_marginal 语料的 marginal guard。

    R14 语义组装修正:R6 冻结的 c2_marginal_guard_conditions 检查的
    semantics 键为 cue_payoff_separation(R7 误传 cue_semantics——
    R7 calibration 未执行故未暴露);R14 以 semantics=None 调用 R6 统计
    ...
    for rung in CURRICULUM261_RUNGS:
        for idx in range(DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R14):
            indep_records.append(generate_pair(
                FAMILY_C2, rung, idx,
                namespace=DESIGN_INDEPENDENT_NAMESPACE_R14,
                rung_params_override={rung: dict(ladder[rung])}))
    indep_report = rung_report_r4(
        indep_records, FAMILY_C2, ladder, thresholds,
        preproc=None, corpus=DESIGN_INDEPENDENT_NAMESPACE_R14)
    ...
    base = c2_marginal_guard_conditions(
        ...
        semantics=None)
    r14_semantics_ok = bool(
        indep_semantics["local_cue_independence"]["pass"]
        and indep_semantics["context_observability"]["pass"]
        and indep_cue["pass"])
    marginal = dict(base)
    marginal["format"] = "cur261-r14-c2-marginal-guard-v1"
    marginal["r14_semantics_rule"] = (
        "local cue ∧ context observability ∧ independent cue semantics"
        "(point recall ≥ 0.90 灾难护栏 + canonical + non-cue FP;§26)")
    marginal["r14_semantics_pass"] = r14_semantics_ok
    marginal["semantics_pass"] = r14_semantics_ok
    marginal["pass"] = bool(base["pass"] and r14_semantics_ok)
    artifact = {
        "format": "cur261-r14-independent-marginal-design-v1",
        "namespace": DESIGN_INDEPENDENT_NAMESPACE_R14,
        "pairs_per_rung": DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R14,
...
def _build_power_summary_r14(candidate_results: dict[str, Any],
                            semantic_summaries: dict[str, Any],
    ...
    return {
        "format": "cur261-r14-power-analysis-v1",
        "design_plan_digest": design_digest,
        "selected_candidate": selected_id,
        "selected_block_count": selected_n,
