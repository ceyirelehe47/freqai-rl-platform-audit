# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R13:full-scale shadow rehearsal(工作包 C)。
...
R13 修复(§C):implementation freeze 之前,以专门的 shadow
namespace 执行接近正式规模的全链 rehearsal:
- 与正式流程相同的 orchestrator(orchestrate_calibration_stage_
  r13)与核心(execute_final_core_r13);禁止 monkeypatch 正式函数;
...
SHADOW_FORMAT = "cur261-r13-full-scale-shadow-rehearsal-v1"
...
def _shadow_pack() -> dict[str, Any]:
    ...
    pack = ladder_pack_payload_r13(
        ...
        design_plan_digest="r13dp-shadow-engineering",
        matched_contract_identity="shadow-engineering",
        block_integrity_identity="shadow-engineering",
        cue_semantic_contract_digest="r13cue-shadow-engineering",
        cue_semantic_rule_identity="shadow-engineering",
        cue_audit_digest="r13ca-shadow-engineering",
        p_contract=0.9504,
...
def _shadow_override_fn(family: str, pack: dict[str, Any]):
    """与正式 r13_override_for 同构的覆盖函数(shadow 工程 pack)。"""
...
def run_full_scale_shadow_r13(out_dir: Path, *, run_tag: str) -> dict:
    ...
    from rl_curriculum.curriculum261_r13_orchestrator import (
        shadow_holdout_profile_r13,
        shadow_main_profile_r13,
    )
    ...
    summary: dict[str, Any] = {
        ...
        "cardinality": {
            ...
            "formal_order_held": (
                ...
                "supervised(orchestrate_calibration_stage_r13 正式顺序)"),
            "training_reduction": (
    ...
    final_result = execute_final_core_r13(
        ...
        final_namespace="shadow_calibration_main_r13",
        fit_namespace="shadow_fit_main_r13",
        c13_pairs_per_rung=10,
        ...
        independent_namespace="shadow_c2_independent_main_r13",
        semantic_namespace_override="shadow_semantic_final_r13",
        semantic_out_dir=out_dir,
        supervised_namespace_override="shadow_supervised_main_r13",
        supervised_model_seeds_override=SHADOW_MODEL_SEEDS,
        supervised_training_config=dict(SHADOW_TRAINING_CONFIG),
        conditioning_fit_namespace="shadow_fit_main_r13")
...
def orchestrate_stage(out_dir: Path, pack: dict[str, Any],
    ...
    return orchestrate_stage_inner(
        out_dir, pack, routings, records,
        shadow_main_profile_r13(), shadow_holdout_profile_r13())
...
def orchestrate_stage_inner(out_dir: Path, pack: dict[str, Any],
    ...
    return orchestrate_calibration_stage_r13(
        out_dir, pack,
...
def _artifact_identity_digest(path: Path) -> str:
    ...
    return stable_digest(_strip_non_identity(payload), "r13art-")
...
def compare_full_scale_shadow_runs(dir_a: Path,
    ...
    return {
        "format": "cur261-r13-full-scale-shadow-comparison-v1",
        "iteration": "r13",
...
__all__ = [
    "run_full_scale_shadow_r13", "compare_full_scale_shadow_runs",
