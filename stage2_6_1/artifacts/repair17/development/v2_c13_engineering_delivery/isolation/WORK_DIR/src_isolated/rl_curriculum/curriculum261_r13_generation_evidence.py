# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R13:GenerationEvidenceCompleteness-v1(§15)。
...
治理层(R13 新增,orchestrator 重新计算,不信 recorder 自报):
...
COMPLETENESS_FORMAT = "cur261-r13-generation-evidence-completeness-v1"
R13_ITERATION = "r13"
...
def verify_generation_evidence_completeness(
    ...
    for r in rows:
        ...
        if env.get("iteration") != R13_ITERATION:
    ...
    if iteration_mismatch:
        problems.append(
            f"{iteration_mismatch} 行 iteration != r13(混入历史迭代)")
    # ---- 逐 call 完整性 + 坐标计数 ----
...
def generation_evidence_completeness_digest(result: dict[str, Any]) -> str:
    ...
    return "r13gec-" + hashlib.sha256(json.dumps(
