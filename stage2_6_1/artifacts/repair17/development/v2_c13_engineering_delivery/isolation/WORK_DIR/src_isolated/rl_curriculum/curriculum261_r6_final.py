# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R6:一次性 final qualification(§31-§33)。
...
4. 完成全部不需要 final seed 的静态身份检查(冻结合同/vendor/V2
   digest/code identity/matched 合同身份);
...
8. 执行 final:C1/C3 各 40 pairs + C2 matched blocks(selected n,
   总 80 + 4n pairs)+ C2 独立 marginal 语料 + 全套验证 + strict
   final conditions;
9. marker -> completed;任何异常 -> marker -> crashed + crash evidence
   落盘 + re-raise(保留 marker,禁止修复后复用 qualification_r6)。

§33 final 必须验证:preprocessing(production equivalence/unbounded/no
clip/provenance/fit-eval isolation/survival/position/serialization/
...
separation/bootstrap/positive-gap rate)、C2 independent guard、
contract(历史 hashes/pack identity/block 合同/selected n/plan/code/
vendor/runtime/exposure identity)。
"""
...
from rl_curriculum.curriculum261_r6_calibration import (
    C2_INDEPENDENT_PAIRS_PER_RUNG_R6,
...
FINAL_FORMAT_R6 = "cur261-r6-final-qualification-v1"


def qualification_r6_lock_dir_default() -> Path:
    from rl_curriculum.curriculum261_r6_namespaces import (
...
def _fresh_seed_validity_r6(n_checks: int = 10) -> dict[str, Any]:
    """fresh_holdout_r6 与 qualification_r6 的 seed 不相交验证。"""
    from rl_curriculum.curriculum261_api import derive261_seed

    mismatches = []
    for i in range(n_checks):
        for family in CURRICULUM261_FAMILIES:
            q = derive261_seed("qualification_r6", family, "D2", i, 0)
            for rung in CURRICULUM261_RUNGS:
                f = derive261_seed("fresh_holdout_r6", family, rung, i, 0)
                if q == f:
...
def run_final_qualification_r6(out_dir: Path,
                               vendor_dir: Path | None = None,
                               ) -> dict[str, Any]:
    """执行一次性 R6 final qualification(§32)。"""
    if qualification_r6_exposed():
        raise RuntimeError(
            "R6 final qualification 已执行过(exposure marker/ledger "
            "存在)——同一 qualification_r6 corpus 不得再次执行;继续必须"
            "使用新迭代身份(R6.1/R7)与全新 seed space")
    plan, digest = load_locked_plan_r6()
    if (plan.get("robustness_gate") or {}).get("pass") is not True:
        raise RuntimeError(
            "R6 plan 的 robustness_gate.pass != true——final "
            "qualification fail closed")
    pack = load_selected_pack(qualification_r6_lock_dir_default())
    ...
    code_ok = (plan["code_identity"] == _code_identity_r6())
    matched_ok = (
        pack["matched_ladder_contract_identity"]
        == matched_ladder_contract_identity()
        == plan["matched_ladder"]["contract_identity"]
        and int(pack["selected_block_count"])
        == int(plan["final_sample_counts"]["c2_matched_blocks"]))
    static_ok = bool(frozen["pass"] and upstream_ok and contract_ok
                     and code_ok and matched_ok)
    if not static_ok:
        raise RuntimeError(
            "final 静态身份检查失败(frozen contracts/vendor/V2 digest/"
            "code identity/matched 合同与 selected n);在 exposure "
            "marker 之前 fail closed——本轮 qualification_r6 未消耗")

    started = datetime.now(timezone.utc).isoformat(timespec="seconds")
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    try:
        with QualificationR6FileLock(blocking=False):
            # ---- 原子 exposure marker(ledger 先行)----
            write_qualification_r6_exposure(digest, status="running")
            result = _execute_final_r6(
        ...
        try:
            (out_dir / "qualification_crash_traceback.log").write_text(
                exc, encoding="utf-8")
            write_qualification_r6_exposure(digest, status="crashed")
        except Exception:  # noqa: BLE001 - 崩溃路径不得掩盖原始异常
            pass
        raise


def _execute_final_r6(out_dir: Path, plan: dict[str, Any], digest: str,
    ...
    final_records = generate_fit_bank_r6(
        "preprocess_fit_qualification_r6", pack)
    final_v2, final_manifest = fit_preprocessor_v2_from_bank_r6(
        "preprocess_fit_qualification_r6", pack,
    ...
    envelope_path = out_dir / "qualification_preprocessor_bundle.json"
    ...
    space_validation = validate_observation_space_v2(
        ...
        [int(r.episodes[s].spec.seed) for r in contain_records
         for s in ("A", "B")], context="final_fit_space_v2_r6")
    adversarial = adversarial_out_of_range_probe(final_v2, EVAL_CFG)

    # 2) C1/C3 qualification pairs(各 4 rungs x 10)
        ...
        for rung in CURRICULUM261_RUNGS:
            for idx in range(10):
                records.append(generate_pair(
                    family, rung, idx, namespace="qualification_r6",
                    rung_params_override=override))
        all_records[family] = records
        family_reports[family] = rung_report_r4(
            records, family, r6_family_rung_params(family, pack),
            thresholds[family], final_v2, corpus="qualification_r6")

    # 3) C2 matched blocks(qualification_r6,selected n)
    c2_matched = run_c2_matched_corpus_r6(
        final_v2, pack, "qualification_r6", n_blocks)
    c2_matched_conditions = c2_matched["matched_conditions"]

    # 4) C2 独立 marginal 语料(c2_independent_qualification_r6)
    c2_indep = run_c2_independent_corpus_r6(
        final_v2, pack, "c2_independent_qualification_r6",
        pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R6)
    c2_marginal = c2_independent_marginal_guard_r6(c2_indep, pack)

    # 5) reference 等价:全部 final episodes(禁止抽样)
    reference_eq = []
    for family in ("c1_opportunity", "c3_cost"):
        for rec in all_records[family]:
            rung_params = r6_family_rung_params(family, pack)[rec.rung]
            ...
            for side in ("A", "B"):
                reference_eq.append(reference_equivalence_check(
                    ...
                    RAW_SCHEMA))
    c2_ladder = r6_family_rung_params(FAMILY_C2, pack)
    for blk in c2_matched["blocks"]:
        for rung in CURRICULUM261_RUNGS:
            rec = blk.pair_records[rung]
            for side in ("A", "B"):
                reference_eq.append(reference_equivalence_check(
                    rec.episodes[side], FAMILY_C2,
                    dict(c2_ladder[rung]), thresholds[FAMILY_C2],
                    final_v2.inner, EVAL_CFG, RAW_SCHEMA))
    for rec in c2_indep["records"]:
        for side in ("A", "B"):
            reference_eq.append(reference_equivalence_check(
                rec.episodes[side], FAMILY_C2,
                dict(c2_ladder[rec.rung]), thresholds[FAMILY_C2],
                final_v2.inner, EVAL_CFG, RAW_SCHEMA))
    ...
    causality = {
        ...
        "production_feature_equivalence": [
            check_production_feature_equivalence(
                f, "D2", 0, namespace="qualification_r6")
            for f in CURRICULUM261_FAMILIES],
        "reference_causality": [
            check_reference_causality(
                f, r6_family_rung_params(f, pack)["D2"], thresholds[f])
            for f in CURRICULUM261_FAMILIES],
    }
    repro = [check_reproducibility(f, r, 0, "qualification_r6")
             for f in CURRICULUM261_FAMILIES for r in ("D1", "D2")]
    # pack override 复现(独立-rung 路径)
    repro_pack_override = []
    for family in CURRICULUM261_FAMILIES:
        ov = r6_override_for(family, pack) or {}
    ...
    # matched block 复现(同 block seed 重生成逐位一致)
    repro_blk1 = generate_matched_block_with_attempts(
        c2_ladder, namespace="qualification_r6", block_index=0)
    ...
    latent_records = [r for recs in all_records.values() for r in recs] \
    ...
    fresh = _fresh_seed_validity_r6(10)

    # 6) representation + supervised(final fit state)
    from rl_curriculum.curriculum261_r3_calibration import (
        conditioning_profile as _cond,
    )

    cond_records = generate_fit_bank_r6(
    ...
    supervised = supervised_learnability_run_r6(
        final_v2, pack, namespace="qualification_r6")

    # 7) 密度 + 三语义(matched 语料)
    density = run_c2_density_diagnostics_r6(
        c2_matched, c2_matched, pack)
    semantics = run_c2_diagnostics_r6(
        [rec for blk in c2_matched["blocks"]
         for rec in blk.pair_records.values()])
    scrambled = scrambled_gap_control(c2_matched["block_table"])

    # 8) strict final conditions
    conditions: dict[str, Any] = {}
    for family in ("c1_opportunity", "c3_cost"):
        conditions[family] = corpus_conditions_r6_pair(
            family_reports[family], kappa)
    conditions[FAMILY_C2] = c2_matched_conditions
    ...
    checks: dict[str, Any] = {
        ...
        "c2_matched_strict_pass": bool(c2_matched_conditions["pass"]),
    ...
    result = {
        ...
        "completed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespace": "qualification_r6",
        "c2_independent_namespace": "c2_independent_qualification_r6",
    ...
    (out_dir / "qualification_raw.json").write_text(json.dumps({
        ...
        "c2_matched": {
            ...
            "matched_conditions": c2_matched_conditions,
        },
