...
from rl_curriculum.curriculum261_r12_labels import (
    SUPERVISED_LABEL_CONTRACT,
    collect_policy_visible_dataset_r12,
    supervised_dataset_identity_r12,
)
...
REHEARSAL_FIT_PAIRS_PER_RUNG_R12 = 2


def generate_fit_bank_r12(
...
def fit_preprocessor_v2_from_bank_r12(
        namespace: str, pack: dict[str, Any], *,
...
def run_calibration_corpus_c13_r12(
        preproc_v2: Any, pack: dict[str, Any], namespace: str, *,
        pairs_per_rung: int = CALIBRATION_PAIRS_PER_RUNG_R12,
) -> dict[str, Any]:
...
def run_c2_independent_corpus_r12(
        preproc_v2: Any, pack: dict[str, Any], namespace: str, *,
        pairs_per_rung: int = C2_INDEPENDENT_PAIRS_PER_RUNG_R12,
) -> dict[str, Any]:
...
def run_c2_semantic_corpus_r12(
        pack: dict[str, Any], namespace: str, *,
        n_blocks: int = SEMANTIC_BLOCKS_PER_CORPUS_R12,
        out_dir: Path | None = None,
        artifact_name: str | None = None) -> dict[str, Any]:
    """§18/§19 dedicated cue semantic corpus(R9 语义;R12 namespace)。
    ...
    ladder = r12_family_rung_params(FAMILY_C2, pack)
    ...
    result = {
        "format": "cur261-r12-semantic-corpus-v1",
        "namespace": namespace,
    ...
    if out_dir is not None:
        ...
        expected_name = semantic_artifact_filename_r12(namespace)
...
def c2_matched_conditions_r12(
    ...
    checks = {
        ...
        "cue_semantics_delegated_note": (
            ...
            "160-block semantic corpus(cue_semantic_*_r12 namespace)"),
    }
    return {
        "format": "cur261-r12-c2-matched-conditions-v1",
        "statistical": base,
...
def c2_independent_marginal_guard_r12(
    ...
    r12_semantics_ok = bool(
        ...
        and cue["pass"])
    guard = dict(base)
    guard["format"] = "cur261-r12-c2-marginal-guard-v1"
    guard["r12_semantics_rule"] = (
        "local cue ∧ context observability ∧ independent cue semantics"
        "(point recall ≥ 0.90 灾难护栏)")
    guard["r12_semantics_pass"] = r12_semantics_ok
    guard["semantics_pass"] = r12_semantics_ok
    guard["pass"] = bool(base["pass"] and r12_semantics_ok)
    return {
        "format": "cur261-r12-c2-independent-marginal-v1",
...
# ---------------------------------------------------- supervised(§8/§17)
def supervised_learnability_run_r12(
        preproc_v2: Any, pack: dict[str, Any], *,
        namespace: str,
        pairs_per_rung: int = CALIBRATION_PAIRS_PER_RUNG_R12,
        train_pair_limit: int = 6,
        model_seeds: tuple | list = R12_SUPERVISED_MODEL_SEEDS,
    ...
    for family in CURRICULUM261_FAMILIES:
        ...
        rung_params = r12_family_rung_params(family, pack)
        dataset = collect_policy_visible_dataset_r12(
        ...
        controls = ["U", "W", "B"]
        gated_controls = list(R12_SUPERVISED_GATE["gated_controls"])
        seed_reports: list[dict[str, Any]] = []
        # ---- repair R12(B2):distinct model-seed 计数 ----
        # R10 缺陷:n_passing 把 W/B 每个通过的 run 各计 1 —— 同一
        # model seed 的 W+B 双通过被误算成"两个 seeds 通过"
        # (control-run count 冒充 seed count)。R12 合同:对每个
        ...
        for seed in model_seeds:
            for control in controls:
                ...
                gated = bool(
                    ...
                    >= R12_SUPERVISED_GATE["behavior_gap_min"])
        ...
        control_pass = {
            ...
            >= R12_SUPERVISED_GATE["min_seeds_passing"]
        ...
        out["families"][family] = {
            ...
            "distinct_seed_gate": {
                "rule": (
                    ...
                    f"{R12_SUPERVISED_GATE['min_seeds_passing']}/"
                    f"{R12_SUPERVISED_GATE['n_model_seeds']} distinct "
        ...
        out.setdefault("dataset_identity", {})[family] = (
            supervised_dataset_identity_r12(dataset))
    out["alignment_all_ok"] = alignment_all
    out["pass"] = bool(overall and alignment_all)
    return out


def run_generator_stress_r12(
        pack: dict[str, Any], *,
        pairs_per_rung: int = 12,
        namespace: str = "stress_r12") -> dict[str, Any]:
    """generator stress(R6 冻结实现;namespace 显式 stress_r12)。

    R9 缺陷修复:R9 CLI 调用未传 namespace,R6 默认回落历史
    stress_r6 —— R12 wrapper 把 namespace 变为显式 keyword-only
    参数,默认 stress_r12。
    """
...
def run_c2_density_diagnostics_r12(
...
def run_c2_diagnostics_r12(records: list[Any]) -> dict[str, Any]:
...
__all__ = [
    "generate_fit_bank_r12", "fit_preprocessor_v2_from_bank_r12",
    "run_calibration_corpus_c13_r12", "run_c2_matched_corpus_r12",
    "run_c2_independent_corpus_r12", "run_c2_semantic_corpus_r12",
    "c2_matched_conditions_r12", "c2_independent_marginal_guard_r12",
    "supervised_learnability_run_r12", "run_generator_stress_r12",
    "run_c2_density_diagnostics_r12", "run_c2_diagnostics_r12",
    "load_selected_pack", "SEMANTIC_BLOCKS_PER_CORPUS_R12",
    "REHEARSAL_FIT_PAIRS_PER_RUNG_R12",
]
