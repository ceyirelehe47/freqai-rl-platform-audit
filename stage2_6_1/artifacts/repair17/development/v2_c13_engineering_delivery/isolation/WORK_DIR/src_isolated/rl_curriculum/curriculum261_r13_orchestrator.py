# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R13:共享 calibration/final orchestration(§12)。
...
R13 修复(§12.4 共享 Orchestrator):
- 正式 calibrate(main/holdout)与 preplan rehearsal 调用同一
  orchestration 函数 orchestrate_calibration_bundle_r13;
- 唯一差异通过 R13ExecutionProfile 表达:namespace 集合与样本量
...
#: R13 supervised 正式 model seeds(§17:3 个全新 seeds;R13 与 R10
#: 的 20261001-3 不重合)。
R13_SUPERVISED_MODEL_SEEDS: tuple[int, ...] = (
    20270111, 20270112, 20270113)
R13_SUPERVISED_GATE: dict[str, Any] = {
    # repair R13(B2):min_seeds_passing 语义 = 每个 gated control
...
CALIBRATION_PAIRS_PER_RUNG_R13 = 10
C2_INDEPENDENT_PAIRS_PER_RUNG_R13 = 20
SEMANTIC_BLOCKS_PER_CORPUS_R13 = 160
EQUIVALENCE_PAIRS_PER_RUNG_R13 = 3


@dataclass(frozen=True)
class R13ExecutionProfile:
    ...
    artifact_suffix: str = ""
    #: repair R13(工作包 C):shadow 路由类(工程 rehearsal;非正式)
    shadow: bool = False
    #: repair R13(工作包 C):supervised 训练配置(仅允许缩短纯训练
...
def formal_main_profile_r13(n_blocks: int) -> R13ExecutionProfile:
    return R13ExecutionProfile(
        name="formal_main", preplan=False,
        c13_eval_namespace="calibration_r13",
        equivalence_namespace="calibration_r13",
        supervised_namespace="supervised_main_r13",
        semantic_namespace="cue_semantic_calibration_r13",
        c2_matched_namespace="calibration_r13",
        c2_independent_namespace="c2_independent_calibration_r13",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R13,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R13,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R13,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R13,
        supervised_train_pair_limit=6,
        supervised_model_seeds=R13_SUPERVISED_MODEL_SEEDS,
        equivalence_pairs_per_rung=EQUIVALENCE_PAIRS_PER_RUNG_R13,
    )


def formal_holdout_profile_r13(n_blocks: int) -> R13ExecutionProfile:
    return R13ExecutionProfile(
        name="formal_holdout", preplan=False,
        c13_eval_namespace="calibration_holdout_r13",
        equivalence_namespace="calibration_holdout_r13",
        supervised_namespace="supervised_holdout_r13",
        semantic_namespace="cue_semantic_holdout_r13",
        c2_matched_namespace="calibration_holdout_r13",
        c2_independent_namespace="c2_independent_holdout_r13",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R13,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R13,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R13,
...
def rehearsal_main_profile_r13() -> R13ExecutionProfile:
    return R13ExecutionProfile(
        name="rehearsal_main", preplan=True,
        c13_eval_namespace="preplan_calibration_main_r13",
        equivalence_namespace="preplan_calibration_main_r13",
        supervised_namespace="preplan_supervised_main_r13",
        semantic_namespace="preplan_semantic_main_r13",
        c2_matched_namespace="preplan_calibration_main_r13",
        c2_independent_namespace="preplan_calibration_main_r13",
...
def rehearsal_holdout_profile_r13() -> R13ExecutionProfile:
    return R13ExecutionProfile(
        name="rehearsal_holdout", preplan=True,
        c13_eval_namespace="preplan_calibration_holdout_r13",
        equivalence_namespace="preplan_calibration_holdout_r13",
        supervised_namespace="preplan_supervised_holdout_r13",
        semantic_namespace="preplan_semantic_validation_r13",
        c2_matched_namespace="preplan_calibration_holdout_r13",
        c2_independent_namespace="preplan_calibration_holdout_r13",
        c13_pairs_per_rung=1,
        c2_blocks=2,
        semantic_blocks=4,
        c2_independent_pairs_per_rung=2,
...
#: 最大值 —— 无论 R13 机械选出哪个 n,shadow 的生成坐标都是其超集
#: (block index 0..19 覆盖 0..14 与 0..9)。
SHADOW_C2_BLOCKS_R13 = 20
...
def shadow_main_profile_r13() -> R13ExecutionProfile:
    ...
    return R13ExecutionProfile(
        ...
        c13_eval_namespace="shadow_calibration_main_r13",
        equivalence_namespace="shadow_calibration_main_r13",
        supervised_namespace="shadow_supervised_main_r13",
        semantic_namespace="shadow_semantic_main_r13",
        c2_matched_namespace="shadow_calibration_main_r13",
        c2_independent_namespace="shadow_c2_independent_main_r13",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R13,
        c2_blocks=SHADOW_C2_BLOCKS_R13,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R13,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R13,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R13,
        supervised_train_pair_limit=6,
...
def shadow_holdout_profile_r13() -> R13ExecutionProfile:
    """full-scale shadow holdout(与 shadow_main 对称;独立 namespace)。"""
    return R13ExecutionProfile(
        name="shadow_holdout", preplan=False, shadow=True,
        c13_eval_namespace="shadow_calibration_holdout_r13",
        equivalence_namespace="shadow_calibration_holdout_r13",
        supervised_namespace="shadow_supervised_holdout_r13",
        semantic_namespace="shadow_semantic_validation_r13",
        c2_matched_namespace="shadow_calibration_holdout_r13",
        c2_independent_namespace="shadow_c2_independent_holdout_r13",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R13,
        c2_blocks=SHADOW_C2_BLOCKS_R13,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R13,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R13,
...
# --------------------------------- R13RealArtifactCliRoundTrip-v1 profiles
#: rt(round-trip rehearsal)profiles:正式生成基数 + rt2_*_r13
...
#: 为任何机械选择结果的超集;与 SHADOW_C2_BLOCKS_R13 同理)。
RT_C2_BLOCKS_R13 = 20
...
#: CALIBRATION_PAIRS_PER_RUNG_R13=10 冻结不变。
RT_C13_PAIRS_PER_RUNG = 60


def rt_main_profile_r13() -> R13ExecutionProfile:
    """round-trip rehearsal main:正式生成规模 + rt namespace + 落盘。"""
    return R13ExecutionProfile(
        name="rt_main", preplan=False, shadow=False,
        c13_eval_namespace="rt2_calibration_main_r13",
        equivalence_namespace="rt2_calibration_main_r13",
        supervised_namespace="rt2_supervised_main_r13",
        semantic_namespace="rt2_semantic_main_r13",
        c2_matched_namespace="rt2_calibration_main_r13",
        c2_independent_namespace="rt2_c2_independent_main_r13",
        c13_pairs_per_rung=RT_C13_PAIRS_PER_RUNG,
        c2_blocks=RT_C2_BLOCKS_R13,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R13,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R13,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R13,
        supervised_train_pair_limit=6,
...
def rt_holdout_profile_r13() -> R13ExecutionProfile:
    """round-trip rehearsal holdout(与 rt_main 对称;独立 namespace)。"""
    return R13ExecutionProfile(
        name="rt_holdout", preplan=False, shadow=False,
        c13_eval_namespace="rt2_calibration_holdout_r13",
        equivalence_namespace="rt2_calibration_holdout_r13",
        supervised_namespace="rt2_supervised_holdout_r13",
        semantic_namespace="rt2_semantic_validation_r13",
        c2_matched_namespace="rt2_calibration_holdout_r13",
        c2_independent_namespace="rt2_c2_independent_holdout_r13",
        c13_pairs_per_rung=RT_C13_PAIRS_PER_RUNG,
        c2_blocks=RT_C2_BLOCKS_R13,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R13,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R13,
...
def preprocessing_robustness_checks_r13(
        routing_main: R13BundleRouting,
        routing_holdout: R13BundleRouting,
        ...
        pack: dict[str, Any], *,
        profile: R13ExecutionProfile,
        ledger: RoutingLedgerR13,
        profile_holdout: R13ExecutionProfile | None = None) -> dict[str, Any]:
    """§18 Preprocessing V2 重新资格审查(R13 版;结构承 R6 电池)。
    ...
    space_validation = validate_observation_space_v2(
        ...
        context="preprocessing_robustness_r13")
    ...
    return {
        "format": "cur261-r13-preprocessing-robustness-v1",
        "iteration": "r13",
...
def run_full_supervised_release_rehearsal(out_dir: Path) -> dict[str, Any]:
    ...
    训练面:
    - 正式 3 个 model seeds(R13_SUPERVISED_MODEL_SEEDS);
    ...
    参数。使用 preplan namespace(preplan_fit_main_r13 /
    preplan_supervised_main_r13 等)。
    """
    from rl_curriculum.curriculum261_r13_calibration import (
        fit_preprocessor_v2_from_bank_r13,
        generate_fit_bank_r13,
        supervised_learnability_run_r13,
    )
    from rl_curriculum.curriculum261_r13_param_pack import (
        r13_override_for,
    )
    from rl_curriculum.curriculum261_r13_shadow import _shadow_pack

    out_dir = Path(out_dir)
    pack = _shadow_pack()
    override_fn = lambda family, p: r13_override_for(family, p)  # noqa: E731
    roles: dict[str, Any] = {}
    any_nan_inf = False
    alignment_failures: list[str] = []
    for role, fit_ns, sup_ns in (
            ("main", "preplan_fit_main_r13", "preplan_supervised_main_r13"),
            ("holdout", "preplan_fit_holdout_r13",
             "preplan_supervised_holdout_r13")):
        records = generate_fit_bank_r13(fit_ns, pack, pairs_per_rung=2)
        bundle, manifest = fit_preprocessor_v2_from_bank_r13(
            fit_ns, pack, records=records, pairs_per_rung=2,
            parameter_pack_identity=str(pack.get("digest", "")))
        sup = supervised_learnability_run_r13(
            bundle, pack, namespace=sup_ns,
            pairs_per_rung=10, train_pair_limit=6,
            model_seeds=R13_SUPERVISED_MODEL_SEEDS)
        ...
        roles[role] = {
            ...
            "model_seeds": list(R13_SUPERVISED_MODEL_SEEDS),
        ...
        (out_dir / f"full_supervised_rehearsal_{role}.json").write_text(
            ...
            encoding="utf-8")
    result = {
        "format": "cur261-r13-full-supervised-release-rehearsal-v1",
        "development_evidence_only": True,
        "not_used_for_parameter_selection": True,
        "formal_model_seeds": list(R13_SUPERVISED_MODEL_SEEDS),
        "formal_gate_constants": R13_SUPERVISED_GATE,
        "roles": roles,
...
def orchestrate_calibration_stage_r13(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R13BundleRouting,
        routing_holdout: R13BundleRouting,
        records_main: list, records_holdout: list,
        profile_main: R13ExecutionProfile,
        profile_holdout: R13ExecutionProfile,
        override_fn: Any,
    ...
    repair R13(工作包 A):整个编排期间打开 generation invocation
    ...
    with sink:
        return _orchestrate_calibration_stage_inner_r13(
...
def _orchestrate_calibration_stage_inner_r13(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R13BundleRouting,
        routing_holdout: R13BundleRouting,
        ...
        design_digest: str | None = None,
        write_artifacts: bool = True) -> dict[str, Any]:
    """orchestrate_calibration_stage_r13 的执行核心(envelope sink 内)。"""
    from rl_curriculum.curriculum261_r13_calibration import (
        c2_independent_marginal_guard_r13,
        c2_matched_conditions_r13,
        run_c2_density_diagnostics_r13,
        run_c2_independent_corpus_r13,
        run_c2_matched_corpus_r13,
        run_c2_semantic_corpus_r13,
        run_calibration_corpus_c13_r13,
        supervised_learnability_run_r13,
    )
    ...
    ledger = RoutingLedgerR13()
    result: dict[str, Any] = {
        "format": "cur261-r13-calibration-orchestration-v1",
        "iteration": "r13",
    ...
    prep_rob = preprocessing_robustness_checks_r13(
    ...
    _write("bundle_routing_validation.json", {
        "contract": "cur261-r13-bundle-routing-contract-v1",
    ...
    for role, routing, profile in (
        ...
        v2_sup = require_eval_routing_r13(
            routing, profile.supervised_namespace,
            context=f"supervised_{role}", ledger=ledger)
        supervised[role] = supervised_learnability_run_r13(
            v2_sup, pack,
    ...
    for role, routing, profile in (
        ...
        c13 = run_calibration_corpus_c13_r13(
    ...
    # 密度诊断(跨双语料;诊断性质)
    density = run_c2_density_diagnostics_r13(
