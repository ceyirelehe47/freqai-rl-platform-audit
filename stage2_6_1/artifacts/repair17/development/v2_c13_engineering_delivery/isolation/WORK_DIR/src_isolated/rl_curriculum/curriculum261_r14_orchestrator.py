# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R14:共享 calibration/final orchestration(§12)。
...
R14 修复(§12.4 共享 Orchestrator):
- 正式 calibrate(main/holdout)与 preplan rehearsal 调用同一
  orchestration 函数 orchestrate_calibration_bundle_r14;
- 唯一差异通过 R14ExecutionProfile 表达:namespace 集合与样本量
...
from rl_curriculum.curriculum261_r14_reference import (
    reference_equivalence_run_r14,
)
from rl_curriculum.curriculum261_r14_routing import (
    R14BundleRouting,
    RoutingLedgerR14,
    require_eval_routing_r14,
)
...
#: R14 supervised 正式 model seeds(§17:3 个全新 seeds;R14 与 R10
#: 的 20261001-3 不重合)。
R14_SUPERVISED_MODEL_SEEDS: tuple[int, ...] = (
    20270111, 20270112, 20270113)
R14_SUPERVISED_GATE: dict[str, Any] = {
    # repair R14(B2):min_seeds_passing 语义 = 每个 gated control
...
CALIBRATION_PAIRS_PER_RUNG_R14 = 10
C2_INDEPENDENT_PAIRS_PER_RUNG_R14 = 20
SEMANTIC_BLOCKS_PER_CORPUS_R14 = 160
EQUIVALENCE_PAIRS_PER_RUNG_R14 = 3


@dataclass(frozen=True)
class R14ExecutionProfile:
    ...
    artifact_suffix: str = ""
    #: repair R14(工作包 C):shadow 路由类(工程 rehearsal;非正式)
    shadow: bool = False
    #: repair R14(工作包 C):supervised 训练配置(仅允许缩短纯训练
...
def formal_main_profile_r14(n_blocks: int) -> R14ExecutionProfile:
    return R14ExecutionProfile(
        name="formal_main", preplan=False,
        c13_eval_namespace="calibration_r14",
        equivalence_namespace="calibration_r14",
        supervised_namespace="supervised_main_r14",
        semantic_namespace="cue_semantic_calibration_r14",
        c2_matched_namespace="calibration_r14",
        c2_independent_namespace="c2_independent_calibration_r14",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R14,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R14,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R14,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R14,
        supervised_train_pair_limit=6,
        supervised_model_seeds=R14_SUPERVISED_MODEL_SEEDS,
        equivalence_pairs_per_rung=EQUIVALENCE_PAIRS_PER_RUNG_R14,
    )


def formal_holdout_profile_r14(n_blocks: int) -> R14ExecutionProfile:
    return R14ExecutionProfile(
        name="formal_holdout", preplan=False,
        c13_eval_namespace="calibration_holdout_r14",
        equivalence_namespace="calibration_holdout_r14",
        supervised_namespace="supervised_holdout_r14",
        semantic_namespace="cue_semantic_holdout_r14",
        c2_matched_namespace="calibration_holdout_r14",
        c2_independent_namespace="c2_independent_holdout_r14",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R14,
        c2_blocks=int(n_blocks),
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R14,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R14,
...
def rehearsal_main_profile_r14() -> R14ExecutionProfile:
    return R14ExecutionProfile(
        name="rehearsal_main", preplan=True,
        c13_eval_namespace="preplan_calibration_main_r14",
        equivalence_namespace="preplan_calibration_main_r14",
        supervised_namespace="preplan_supervised_main_r14",
        semantic_namespace="preplan_semantic_main_r14",
        c2_matched_namespace="preplan_calibration_main_r14",
        c2_independent_namespace="preplan_calibration_main_r14",
...
def rehearsal_holdout_profile_r14() -> R14ExecutionProfile:
    return R14ExecutionProfile(
        name="rehearsal_holdout", preplan=True,
        c13_eval_namespace="preplan_calibration_holdout_r14",
        equivalence_namespace="preplan_calibration_holdout_r14",
        supervised_namespace="preplan_supervised_holdout_r14",
        semantic_namespace="preplan_semantic_validation_r14",
        c2_matched_namespace="preplan_calibration_holdout_r14",
        c2_independent_namespace="preplan_calibration_holdout_r14",
        c13_pairs_per_rung=1,
        c2_blocks=2,
        semantic_blocks=4,
        c2_independent_pairs_per_rung=2,
...
#: 最大值 —— 无论 R14 机械选出哪个 n,shadow 的生成坐标都是其超集
#: (block index 0..19 覆盖 0..14 与 0..9)。
SHADOW_C2_BLOCKS_R14 = 20
...
def shadow_main_profile_r14() -> R14ExecutionProfile:
    ...
    return R14ExecutionProfile(
        ...
        c13_eval_namespace="shadow_calibration_main_r14",
        equivalence_namespace="shadow_calibration_main_r14",
        supervised_namespace="shadow_supervised_main_r14",
        semantic_namespace="shadow_semantic_main_r14",
        c2_matched_namespace="shadow_calibration_main_r14",
        c2_independent_namespace="shadow_c2_independent_main_r14",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R14,
        c2_blocks=SHADOW_C2_BLOCKS_R14,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R14,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R14,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R14,
        supervised_train_pair_limit=6,
...
def shadow_holdout_profile_r14() -> R14ExecutionProfile:
    """full-scale shadow holdout(与 shadow_main 对称;独立 namespace)。"""
    return R14ExecutionProfile(
        name="shadow_holdout", preplan=False, shadow=True,
        c13_eval_namespace="shadow_calibration_holdout_r14",
        equivalence_namespace="shadow_calibration_holdout_r14",
        supervised_namespace="shadow_supervised_holdout_r14",
        semantic_namespace="shadow_semantic_validation_r14",
        c2_matched_namespace="shadow_calibration_holdout_r14",
        c2_independent_namespace="shadow_c2_independent_holdout_r14",
        c13_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R14,
        c2_blocks=SHADOW_C2_BLOCKS_R14,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R14,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R14,
...
# --------------------------------- R14RealArtifactCliRoundTrip-v1 profiles
#: rt(round-trip rehearsal)profiles:正式生成基数 + rt3_*_r14
...
#: 为任何机械选择结果的超集;与 SHADOW_C2_BLOCKS_R14 同理)。
RT_C2_BLOCKS_R14 = 20
...
#: CALIBRATION_PAIRS_PER_RUNG_R14=10 冻结不变。
RT_C13_PAIRS_PER_RUNG = 60


def rt_main_profile_r14() -> R14ExecutionProfile:
    """round-trip rehearsal main:正式生成规模 + rt namespace + 落盘。"""
    return R14ExecutionProfile(
        name="rt_main", preplan=False, shadow=False,
        c13_eval_namespace="rt3_calibration_main_r14",
        equivalence_namespace="rt3_calibration_main_r14",
        supervised_namespace="rt3_supervised_main_r14",
        semantic_namespace="rt3_semantic_main_r14",
        c2_matched_namespace="rt3_calibration_main_r14",
        c2_independent_namespace="rt3_c2_independent_main_r14",
        c13_pairs_per_rung=RT_C13_PAIRS_PER_RUNG,
        c2_blocks=RT_C2_BLOCKS_R14,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R14,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R14,
        supervised_pairs_per_rung=CALIBRATION_PAIRS_PER_RUNG_R14,
        supervised_train_pair_limit=6,
...
def rt_holdout_profile_r14() -> R14ExecutionProfile:
    """round-trip rehearsal holdout(与 rt_main 对称;独立 namespace)。"""
    return R14ExecutionProfile(
        name="rt_holdout", preplan=False, shadow=False,
        c13_eval_namespace="rt3_calibration_holdout_r14",
        equivalence_namespace="rt3_calibration_holdout_r14",
        supervised_namespace="rt3_supervised_holdout_r14",
        semantic_namespace="rt3_semantic_validation_r14",
        c2_matched_namespace="rt3_calibration_holdout_r14",
        c2_independent_namespace="rt3_c2_independent_holdout_r14",
        c13_pairs_per_rung=RT_C13_PAIRS_PER_RUNG,
        c2_blocks=RT_C2_BLOCKS_R14,
        semantic_blocks=SEMANTIC_BLOCKS_PER_CORPUS_R14,
        c2_independent_pairs_per_rung=C2_INDEPENDENT_PAIRS_PER_RUNG_R14,
...
def preprocessing_robustness_checks_r14(
        routing_main: R14BundleRouting,
        routing_holdout: R14BundleRouting,
        ...
        pack: dict[str, Any], *,
        profile: R14ExecutionProfile,
        ledger: RoutingLedgerR14,
        profile_holdout: R14ExecutionProfile | None = None) -> dict[str, Any]:
    """§18 Preprocessing V2 重新资格审查(R14 版;结构承 R6 电池)。
    ...
    space_validation = validate_observation_space_v2(
        ...
        context="preprocessing_robustness_r14")
    ...
    return {
        "format": "cur261-r14-preprocessing-robustness-v1",
        "iteration": "r14",
...
def run_full_supervised_release_rehearsal(out_dir: Path) -> dict[str, Any]:
    ...
    训练面:
    - 正式 3 个 model seeds(R14_SUPERVISED_MODEL_SEEDS);
    ...
    参数。使用 preplan namespace(preplan_fit_main_r14 /
    preplan_supervised_main_r14 等)。
    """
    from rl_curriculum.curriculum261_r14_calibration import (
        fit_preprocessor_v2_from_bank_r14,
        generate_fit_bank_r14,
        supervised_learnability_run_r14,
    )
    from rl_curriculum.curriculum261_r14_param_pack import (
        r14_override_for,
    )
    from rl_curriculum.curriculum261_r14_shadow import _shadow_pack

    out_dir = Path(out_dir)
    pack = _shadow_pack()
    override_fn = lambda family, p: r14_override_for(family, p)  # noqa: E731
    roles: dict[str, Any] = {}
    any_nan_inf = False
    alignment_failures: list[str] = []
    for role, fit_ns, sup_ns in (
            ("main", "preplan_fit_main_r14", "preplan_supervised_main_r14"),
            ("holdout", "preplan_fit_holdout_r14",
             "preplan_supervised_holdout_r14")):
        records = generate_fit_bank_r14(fit_ns, pack, pairs_per_rung=2)
        bundle, manifest = fit_preprocessor_v2_from_bank_r14(
            fit_ns, pack, records=records, pairs_per_rung=2,
            parameter_pack_identity=str(pack.get("digest", "")))
        sup = supervised_learnability_run_r14(
            bundle, pack, namespace=sup_ns,
            pairs_per_rung=10, train_pair_limit=6,
            model_seeds=R14_SUPERVISED_MODEL_SEEDS)
        ...
        roles[role] = {
            ...
            "model_seeds": list(R14_SUPERVISED_MODEL_SEEDS),
        ...
        (out_dir / f"full_supervised_rehearsal_{role}.json").write_text(
            ...
            encoding="utf-8")
    result = {
        "format": "cur261-r14-full-supervised-release-rehearsal-v1",
        "development_evidence_only": True,
        "not_used_for_parameter_selection": True,
        "formal_model_seeds": list(R14_SUPERVISED_MODEL_SEEDS),
        "formal_gate_constants": R14_SUPERVISED_GATE,
        "roles": roles,
...
def orchestrate_calibration_stage_r14(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R14BundleRouting,
        routing_holdout: R14BundleRouting,
        records_main: list, records_holdout: list,
        profile_main: R14ExecutionProfile,
        profile_holdout: R14ExecutionProfile,
        override_fn: Any,
    ...
    repair R14(工作包 A):整个编排期间打开 generation invocation
    ...
    with sink:
        return _orchestrate_calibration_stage_inner_r14(
...
def _orchestrate_calibration_stage_inner_r14(
        out_dir: Path, pack: dict[str, Any], *,
        n_blocks: int, recall_floor_value: float,
        routing_main: R14BundleRouting,
        routing_holdout: R14BundleRouting,
        ...
        design_digest: str | None = None,
        write_artifacts: bool = True) -> dict[str, Any]:
    """orchestrate_calibration_stage_r14 的执行核心(envelope sink 内)。"""
    from rl_curriculum.curriculum261_r14_calibration import (
        c2_independent_marginal_guard_r14,
        c2_matched_conditions_r14,
        run_c2_density_diagnostics_r14,
        run_c2_independent_corpus_r14,
        run_c2_matched_corpus_r14,
        run_c2_semantic_corpus_r14,
        run_calibration_corpus_c13_r14,
        supervised_learnability_run_r14,
    )
    ...
    ledger = RoutingLedgerR14()
    result: dict[str, Any] = {
        "format": "cur261-r14-calibration-orchestration-v1",
        "iteration": "r14",
    ...
    prep_rob = preprocessing_robustness_checks_r14(
    ...
    _write("bundle_routing_validation.json", {
        "contract": "cur261-r14-bundle-routing-contract-v1",
    ...
    for role, routing, profile in (
        ...
        v2_sup = require_eval_routing_r14(
            routing, profile.supervised_namespace,
            context=f"supervised_{role}", ledger=ledger)
        supervised[role] = supervised_learnability_run_r14(
            v2_sup, pack,
    ...
    for role, routing, profile in (
        ...
        c13 = run_calibration_corpus_c13_r14(
    ...
    # 密度诊断(跨双语料;诊断性质)
    density = run_c2_density_diagnostics_r14(
