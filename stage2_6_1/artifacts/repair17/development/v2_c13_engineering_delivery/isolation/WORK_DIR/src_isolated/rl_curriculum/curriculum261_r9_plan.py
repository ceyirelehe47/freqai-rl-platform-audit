# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R9:final qualification plan(§26)。

双 strict gate(calibration + holdout 各自独立 PASS;无 pooled 救援)
前置强制;plan 绑定 R9 pack/cue semantic 合同/p_contract/recall
...
from rl_curriculum.curriculum261_r9_namespaces import (
    CURRICULUM261_ITERATION_ID_R9,
    qualification_r9_digest_path,
    qualification_r9_plan_path,
)

PLAN_FORMAT_R9 = "cur261-r9-qualification-plan-v1"

#: R9 代码身份(逐模块内容哈希;进入 plan;final 复算比对)。
PLAN_CODE_MODULES_R9 = (
...
def _code_identity_r9() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    for name in PLAN_CODE_MODULES_R9:
        f = root / name
    ...
    plan: dict[str, Any] = {
        ...
        "preprocessing_v2": {
            ...
            "implementation": "pinned vendor IFreqaiModel."
                              "define_data_pipeline(直接复用;R9 全新"
                              "语料重新资格验证)",
        ...
        "fit_bank_schedule": {
            "calibration": {
                "namespace": "preprocess_fit_calibration_r9",
                "pairs_per_rung": 4, "sides": ["A", "B"],
                "role": "main fit corpus(preprocessor only)"},
            "holdout": {
                "namespace": "preprocess_fit_holdout_r9",
                "pairs_per_rung": 4, "sides": ["A", "B"],
        ...
        "semantic_corpus_schedule": {
            "design": {
                "namespaces": ["cue_semantic_design_main_r9",
                               "cue_semantic_design_validation_r9"],
                "blocks_per_corpus": 160,
                "ladder": "sentinel(candidate-independent)"},
            "calibration": {
                "namespace": "cue_semantic_calibration_r9",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "holdout": {
                "namespace": "cue_semantic_holdout_r9",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "final": {
                "namespace": "cue_semantic_qualification_r9",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
        },
        "final_sample_counts": {
        ...
        "qualification_bank_schedule": {
            ...
            "fresh_seed_namespace": "fresh_holdout_r9",
        },
        ...
        "code_identity": _code_identity_r9(),
    }
    return plan


def plan_digest_r9(plan: dict[str, Any]) -> str:
    payload = {k: v for k, v in plan.items() if k != "created_utc"}
    return "qp9-" + hashlib.sha256(json.dumps(
        payload, sort_keys=True, separators=(",", ":"),
        ensure_ascii=False, default=str).encode("utf-8")).hexdigest()


def lock_plan_r9(plan: dict[str, Any]) -> tuple[Path, str]:
    ...
    path = qualification_r9_plan_path()
    if path.is_file():
        raise RuntimeError(
            "R9 qualification plan 已存在;plan lock 后禁止重写(§16.2/"
            "§26:任何修改须新 iteration R10)")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(plan, indent=2, ensure_ascii=False,
                               default=str), encoding="utf-8")
    digest = plan_digest_r9(plan)
    dpath = qualification_r9_digest_path()
    dpath.write_text(digest, encoding="utf-8")
    return path, digest


def load_locked_plan_r9() -> tuple[dict[str, Any], str]:
    path = qualification_r9_plan_path()
    dpath = qualification_r9_digest_path()
    if not path.is_file() or not dpath.is_file():
        raise RuntimeError("R9 qualification plan 未锁定")
    plan = json.loads(path.read_text(encoding="utf-8"))
    digest = plan_digest_r9(plan)
    if dpath.read_text(encoding="utf-8").strip() != digest:
        raise RuntimeError("R9 plan digest 复算不一致(fail closed)")
    return plan, digest
