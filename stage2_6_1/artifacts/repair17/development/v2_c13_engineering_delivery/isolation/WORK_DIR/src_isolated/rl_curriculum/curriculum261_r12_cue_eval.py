# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R12:cluster-aware cue 语义评估器(unique event)。
...
- §14/§15 dedicated semantic corpus(160 matched blocks/corpus,
  ...
  R12 design FAIL,不进入 candidate 设计;
...
R12_CUE_BOOTSTRAP_RESAMPLES = 20000
R12_CUE_BOOTSTRAP_SEED = 20261203
...
def canonical_cue_observations(
    ...
    cue_table_digest = "r12ct-" + hashlib.sha256(
...
def cluster_bootstrap_rate(
        ...
        n_boot: int = R12_CUE_BOOTSTRAP_RESAMPLES,
        seed: int = R12_CUE_BOOTSTRAP_SEED,
) -> dict[str, Any]:
...
def semantic_cue_gate(
    ...
    - tail-position 诊断(报告,不作独立 gate——audit 阶段已专项
      闭合)。
    任一 FAIL => R12 design FAIL(不进入 candidate 设计)。
    """
    ...
    return {
        "format": "cur261-r12-semantic-cue-gate-v1",
        "label": label,
...
def candidate_cue_semantics(
    ...
    return {
        "format": "cur261-r12-candidate-cue-semantics-v1",
...
def cue_semantic_rule_identity() -> str:
    """R12 cue 语义 gate 规则身份(冻结;plan/pack 绑定)。"""
    payload = {
        ...
        "bootstrap_resamples": R12_CUE_BOOTSTRAP_RESAMPLES,
        "bootstrap_seed": R12_CUE_BOOTSTRAP_SEED,
    ...
    return "r12csg-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
...
def independent_cue_semantics(
    ...
    R12 变更:recall 正式 cluster gate 已由 160-block dedicated
    ...
    for rung in covered_rungs2:
        ...
        for side in ("A", "B"):
            ...
            prec = cluster_bootstrap_rate(
                ...
                side="lower")
            per_side[side] = {
                # R12 §26:precision/false-cue 降级为诊断(不进 gate)
                "payoff_false_cue": {
                    **fc, "max": C2_PAYOFF_BAR_FALSE_CUE_MAX,
                    "pass": bool(
                        fc["bound"] <= C2_PAYOFF_BAR_FALSE_CUE_MAX)},
    ...
    return {
        "format": "cur261-r12-independent-cue-semantics-v1",
        "candidate": candidate_id,
