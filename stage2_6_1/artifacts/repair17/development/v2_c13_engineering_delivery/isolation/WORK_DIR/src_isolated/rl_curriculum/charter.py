...
REQUIRED_CHARTER_FIELDS: tuple[str, ...] = (
    ...
    "timeframe",                 # 显式 timeframe(不得依赖默认 15m)
    "observation_schema",        # 课程级 observation schema(可哈希)
    "signal_groups",             # 预注册信号组(消融考试)
    "nuisance_features",         # 预注册 nuisance 槽位声明
    "formal_null_families",      # 正式多类 Null 家族
)
