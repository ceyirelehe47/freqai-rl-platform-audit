# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R6:final 前两级 preflight(§29)。

A. Pre-Lock Static Preflight(plan lock 前执行;不触碰任何 final data):
   - 全部 R6 模块 import / final runner 依赖;
   - vendor 路径解析 + vendor SHA/clean;
   - Route C identity / R6 code identity(无 MISSING);
   - 输出目录写权限;
   - V2 serialize/reload + outer env + SB3 check_env + PPO smoke
     (非 final namespace:ppo_smoke_r6);
   - matched block generator + block attempt 语义(poo_smoke_r6 下
     小规模 block 生成验证;不触碰 design/calibration/final);
...
B. Post-Lock Sealed Preflight(plan lock 后、final 前执行):
   - plan digest 重算、code identity、parameter pack、robustness
     evidence、vendor、输出权限、final runner 依赖、selected block
     count 绑定、matched contract 绑定;
   - **不派生任何 preprocess_fit_qualification_r6 / qualification_r6
...
def _imports_ok() -> dict[str, Any]:
    ...
    modules = [
        "rl_curriculum.curriculum261_r6_namespaces",
        ...
        "rl_curriculum.curriculum261_r6_plan",
        "rl_curriculum.curriculum261_r6_final",
        "rl_curriculum.curriculum261_r6_smoke",
...
def _code_identity_digest() -> dict[str, Any]:
    from rl_curriculum.curriculum261_r6_plan import _code_identity_r6

    ident = _code_identity_r6()
    missing = [k for k, v in ident.items() if v == "MISSING"]
    return {
        "identity": ident,
        "missing": missing,
        "digest": "r6ci-" + hashlib.sha256(
            _canonical(ident).encode("utf-8")).hexdigest(),
        "pass": not missing,
    }


def _matched_generator_probe() -> dict[str, Any]:
    """matched block generator + block attempt 语义小规模验证
    (ppo_smoke_r6 下 2 个 block;不触碰 design/calibration/final)。"""
    try:
        ...
        for idx in range(2):
            blocks.append(generate_matched_block_with_attempts(
                dict(C2_RUNG_PARAMS), namespace="ppo_smoke_r6",
                block_index=idx))
...
def run_prelock_static_preflight(out_dir: Path,
    ...
    """§29A:plan lock 前静态 preflight(只用非 final namespace)。"""
    ...
    # V2 + outer env + check_env + PPO smoke(ppo_smoke_r6,非 final)
    try:
        from rl_curriculum.curriculum261_r6_smoke import run_ppo_smoke_r6

        smoke = run_ppo_smoke_r6()
    ...
    # matched block generator + block attempt 语义(ppo_smoke_r6)
    checks["matched_block_generator"] = _matched_generator_probe()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        old = os.environ.get("CURRICULUM261_R6_LOCK_DIR")
        os.environ["CURRICULUM261_R6_LOCK_DIR"] = td
        try:
            ...
            write_qualification_r6_exposure("r6dp-preflight-probe",
                                            "running")
            try:
                write_qualification_r6_exposure("r6dp-preflight-probe",
                                                "running")
            ...
            if old is None:
                os.environ.pop("CURRICULUM261_R6_LOCK_DIR", None)
            else:
                os.environ["CURRICULUM261_R6_LOCK_DIR"] = old
    checks["marker_atomic_exclusive"] = marker_ok
    checks["concurrent_final_lock_rejected"] = lock_ok

    result = {
        "format": "cur261-r6-prelock-static-preflight-v1",
        "iteration": "r6",
        "executed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespaces_touched": ["ppo_smoke_r6"],
        ...
        "pass": bool(
            ...
            and checks["matched_block_generator"]["pass"]
...
def run_postlock_sealed_preflight(out_dir: Path,
    ...
    """§29B:plan lock 后 sealed preflight。
    ...
    checks: dict[str, Any] = {}
    plan, plan_digest = load_locked_plan_r6()
    checks["plan_digest_recompute"] = True
    checks["plan_iteration_r6"] = bool(plan.get("iteration") == "r6")
    ...
    code_digest = "r6ci-" + hashlib.sha256(
        _canonical(ident).encode("utf-8")).hexdigest()
    ...
    checks["plan_path_resolves"] = bool(
        qualification_r6_plan_path().is_file()
        and qualification_r6_digest_path().is_file()
        and r6_parameter_pack_path().is_file())
    ...
    attestation = {
        "format": "cur261-r6-sealed-final-preflight-v1",
        "iteration": "r6",
        "pass": att_pass,
