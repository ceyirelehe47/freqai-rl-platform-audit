# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R11:版本化 clean matched-ladder 参数包
(CurriculumR11MatchedLadderPack-v1)。

R7 诚实 FAIL(shared cue gate main LCB 0.912 < floor 0.930;plan
loader digest 自引用缺陷;mirror 边界错误)。R11 在全新 seed 空间
重做 clean design(§1/§2):
...
复用 curriculum261_r6_param_pack(R11 不复制不修改,实现零语义漂移
由构造保证;R6 模块 sha256 进入 R11 code identity,并与 R7 baseline
的记录值对拍)。
"""
...
R11_PACK_VERSION = "CurriculumR11MatchedLadderPack-v1"
...
#: 作为 R11 development evidence,R11 必须在新 namespace 重做)。
...
#: evidence。全部结果仅作 R11 development evidence;R11 必须在全新
...
#: §17 预注册 R11 candidate 网格(恰好 3 个;生成任何 R11 design
#: episode 前锁定进 r11_design_plan)。
...
C2_LADDER_CANDIDATES_R11: dict[str, dict[str, dict[str, Any]]] = {
...
def r11_candidate_grid() -> dict[str, Any]:
    """预注册 R11 ladder 网格深拷贝(design plan 锁定用)。"""
    import copy

    return copy.deepcopy(C2_LADDER_CANDIDATES_R11)


def validate_r11_grid_semantics() -> list[str]:
    """§17 网格级校验(数量恰好 3;D3 alpha 边界;单调;键白名单)。"""
    issues: list[str] = []
    ids = list(C2_LADDER_CANDIDATES_R11)
    if len(ids) != 3:
        issues.append(f"candidate 数量必须恰好 3(§17 禁止第四个),"
                      f"收到 {len(ids)}")
    for cid, ladder in C2_LADDER_CANDIDATES_R11.items():
...
#: R11 允许覆盖的 family 白名单(与 R6/R7 一致)。
R11_OVERRIDE_FAMILIES = ("c1_opportunity", "c2_context", "c3_cost")
...
def ladder_pack_payload_r11(
    ...
    """构造 R11 clean matched-ladder pack payload(§27)。
    ...
    - R11 新增绑定(§16):code freeze SHA、policy-visible reference
    ...
    return {
        "format": "cur261-r11-clean-matched-ladder-pack-v1",
        "pack_version": R11_PACK_VERSION,
        "iteration": "r11",
        ...
        "generation_determinism_contract_identity": (
            "cur261-r11-generation-invocation-envelope-v1(Generation"
        ...
        "override_scope": {
            "families": list(R11_OVERRIDE_FAMILIES),
            "c2_rungs": ["D0", "D1", "D2", "D3"],
            "rules": "C1/C3 D3 = R4 选定候选(逐位继承);C2 四档全量"
                     "经 R11 design 选出(只动 alpha_bps/wick_kappa);"
...
def pack_digest_r11(pack: dict[str, Any]) -> str:
    """pack digest(canonical JSON;排除运行时间与 digest 自身字段)。"""
    payload = dict(pack)
    payload.pop("created_utc", None)
    payload.pop("digest", None)
    return "r11pk-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def write_selected_pack_r11(out_dir: Path, pack: dict[str, Any]) -> Path:
    from datetime import datetime, timezone

    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    pack = dict(pack)
    pack["digest"] = pack_digest_r11(pack)
    pack.setdefault(
        "created_utc",
        datetime.now(timezone.utc).isoformat(timespec="seconds"))
    path = out_dir / "r11_parameter_pack.json"
    path.write_text(json.dumps(pack, indent=2, ensure_ascii=False,
                               default=str), encoding="utf-8")
    (out_dir / "r11_parameter_pack_digest.txt").write_text(
...
def load_selected_pack(out_dir: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise RuntimeError(
            f"R11 parameter pack 不存在: {path}(design + marginal guard "
            "通过并锁定后才允许 calibration/final)")
    pack = json.loads(path.read_text(encoding="utf-8"))
    stored = pack.get("digest")
    if not stored or pack_digest_r11(pack) != stored:
        raise RuntimeError("R11 parameter pack digest 复算不一致(fail closed)")
    digest_path = out_dir / "r11_parameter_pack_digest.txt"
    if digest_path.is_file() and \
            digest_path.read_text(encoding="utf-8").strip() != stored:
        raise RuntimeError("R11 parameter pack digest 文件与 payload 不一致")
    return pack


def r11_override_for(
        family: str,
        pack: dict[str, Any]) -> dict[str, dict[str, Any]] | None:
    return _override_for(family, pack)


def apply_r11_override(
...
def r11_family_rung_params(
        family: str, pack: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    return _family_rung_params(family, pack)


def frozen_parameter_identity_r11() -> dict[str, Any]:
    """R11 冻结参数面身份(C1/C3 D0-D2 + C2 结构键;final 复算比对)。"""
    from rl_curriculum.curriculum261_r6_param_pack import (
        frozen_parameter_identity_r6,
    )

    out = frozen_parameter_identity_r6()
    out["r11_note"] = "R11 不修改任何冻结参数面(与 R6/R7 逐位一致)"
    return out


def verify_r4_inheritance_r11(pack: dict[str, Any]) -> dict[str, Any]:
...
def ladder_distance_from_historical_r11(
        ladder: dict[str, dict[str, Any]]) -> float:
    return ladder_distance_from_historical(ladder)


__all__ = [
    "R11_PACK_VERSION", "R4_PARAMETER_PACK_DIGEST",
    "R5_DESIGN_PLAN_DIGEST", "R6_DESIGN_PLAN_DIGEST",
    "R7_DESIGN_PLAN_DIGEST",
    "C2_LADDER_CANDIDATES_R11", "r11_candidate_grid",
    "validate_r11_grid_semantics", "R11_OVERRIDE_FAMILIES",
    "ladder_pack_payload_r11", "pack_digest_r11", "write_selected_pack_r11",
    "load_selected_pack", "r11_override_for", "apply_r11_override",
    "r11_family_rung_params", "frozen_parameter_identity_r11",
    "verify_r4_inheritance_r11", "ladder_distance_from_historical_r11",
