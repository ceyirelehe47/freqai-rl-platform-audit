# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R8:clean matched-ladder design(dedicated semantic
corpus 先行 + corrected plan digest roundtrip + §21/§22 流程 + §8.4
一次性硬规则)。
...
- R7 design plan 的 digest 自引用缺陷:lock 把 design_plan_digest
  ...
  evidence)。R8 修复(§8):digest 复算排除 (locked_utc,
  design_plan_digest) 双字段;payload 自带 digest 同时可经正式
  load_locked_design_plan_r8 复算——正式 lock/load/digest recompute/
...
- R7 shared cue gate 直接用 40-block candidate design corpus,SE
  ...
  floor 0.930)。R8(§14/§15)把 candidate-independent cue 语义拆到
  预注册 160-block dedicated semantic corpus(每 corpus 独立 gate,
  FAIL 即 R8 design FAIL,不进入 candidate 设计);
- R7 的 R6 marginal-guard semantics 键错配(cue_semantics vs R6 冻结
  实现要求的 cue_payoff_separation;R7 calibration 未执行故未暴露)
  ——R8 显式组装:R6 统计条件 + 本模块语义条件(local cue ∧
  context ∧ v2 independent cue semantics)。

§8.4 一次性硬规则:正式 R8 design plan 锁定后发生任何 loader/代码
缺陷 => write_r8_iteration_aborted + R8 永久结束(保留 plan、不重锁、
不复用 namespace、下一轮 R8.1/R9);run_design_stage_r8 内部任何
...
§7 要求:R8 在 plan lock 前验证这些模块与 R7 baseline 的 SHA256
...
from rl_curriculum.curriculum261_r8_param_pack import (
    ...
    R8_PACK_VERSION,
    ladder_pack_payload_r8,
    ladder_distance_from_historical_r8,
    pack_digest_r8,
    r8_candidate_grid,
    validate_r8_grid_semantics,
    write_selected_pack_r8,
)

DESIGN_FORMAT_R8 = "cur261-r8-design-plan-v1"
DESIGN_BLOCKS_PER_CORPUS_R8 = 40
DESIGN_NAMESPACES_R8 = ("design_r8_matched_main",
                        "design_r8_matched_validation")
DESIGN_INDEPENDENT_NAMESPACE_R8 = "design_r8_independent_marginal"
DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R8 = 20

#: §14/§15 dedicated semantic corpus(candidate-independent;在任何
#: R8 design data 生成前锁定;数据后禁止 160→240 扩样/第三 corpus/
#: 合并/删除)。
SEMANTIC_BLOCKS_PER_CORPUS_R8 = 160
SEMANTIC_NAMESPACES_R8 = ("cue_semantic_design_main_r8",
                          "cue_semantic_design_validation_r8")
...
#: §16.1/§21 code identity 覆盖面(R8 全部实现 + 冻结复用模块 +
#: 合同依赖;design data 生成开始后任何漂移 => iteration aborted)。
DESIGN_CODE_MODULES_R8 = (
    ...
    "curriculum261_r8_noise_replay.py",
    "curriculum261_r8_cue_contract.py",
    "curriculum261_r8_cue_eval.py",
    "curriculum261_r8_namespaces.py",
    "curriculum261_r8_param_pack.py",
    "curriculum261_r8_design.py",
)

#: §7 matched-ladder 核心 + C2 generator 模块(必须与 R7 baseline 的
#: SHA256 一致;R7 design plan 的 code_identity 记录值对拍)。
MATCHED_CORE_MODULES_R8 = (
    "curriculum261_c2.py",
    "curriculum261_r6_tape.py",
    "curriculum261_r6_pairs.py",
)
...
def _code_identity_design() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    for name in DESIGN_CODE_MODULES_R8:
        f = root / name
...
def _r7_design_plan_path() -> Path:
    return (Path(__file__).resolve().parents[2] / "artifacts"
            / "route_c_stage2_6_1_repair7" / "r7_design_plan.json")


def verify_matched_core_identity_r7() -> dict[str, Any]:
    """§7:matched-ladder 核心模块与 R7 baseline 的 SHA256 一致。

    R7 的 design plan 在 lock 时记录了全部依赖模块的 sha256;R8 不
    修改 c2 generator / matched tape / matched pairs 三个模块,当前
    哈希必须与 R7 记录值逐位一致(R8 对 api.py 的改动仅限 R8
    namespace 白名单与守卫的追加,paired_noise 行为由 exact replay
    测试逐位锁定)。
    """
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    path = _r7_design_plan_path()
    if not path.is_file():
        return {
            "pass": False,
            "error": f"R7 design plan 不存在: {path}(无法对拍 "
                     "matched-ladder 模块身份)",
            "modules": {},
        }
    r7_plan = json.loads(path.read_text(encoding="utf-8"))
    r7_identity = r7_plan.get("code_identity", {})
    rows: dict[str, Any] = {}
    ok = True
    for name in MATCHED_CORE_MODULES_R8:
        current = (hashlib.sha256((root / name).read_bytes()).hexdigest()
                   if (root / name).is_file() else "MISSING")
        locked = r7_identity.get(name)
        match = bool(current == locked and current != "MISSING")
        ok = ok and match
        rows[name] = {"r7_locked": locked, "r8_current": current,
                      "match": match}
    return {"r7_design_plan_digest": r7_plan.get("design_plan_digest"),
            "modules": rows, "pass": bool(ok)}


# ------------------------------------------------------------- plan
def design_plan_payload_r8(*, baseline_commit: str, vendor_pin: str,
    ...
    """构建并返回 R8 design plan payload(锁定后不得修改任何字段)。
    ...
    grid = r8_candidate_grid()
    problems = validate_r8_grid_semantics()
    if problems:
        raise RuntimeError(f"R8 candidate grid 语义非法: {problems}")
    matched_core = verify_matched_core_identity_r7()
    ...
    return {
        ...
        "plan_digest_contract": {
            ...
            "roundtrip": "build -> lock(O_CREAT 语义:已存在即拒) -> "
                         "load_locked_design_plan_r8(独立进程) -> "
            ...
            "one_shot_rule": "§8.4:正式 plan 锁定后任何 loader/代码"
                             "缺陷 => R8 永久结束(aborted;不删不重锁;"
                             "不复用 namespace;下一轮 R8.1/R9)",
        },
        "matched_ladder": {
            ...
            "implementation": "R6 冻结实现零修改复用(import,不复制);"
                              ...
                              "进入 code_identity 且与 R7 baseline 记录"
                              "值对拍(verify_matched_core_identity_r7)",
            "matched_core_identity_r7": matched_core,
            "shared_components": [
        ...
        "semantic_corpora": {
            "blocks_per_corpus": SEMANTIC_BLOCKS_PER_CORPUS_R8,
            "namespaces": list(SEMANTIC_NAMESPACES_R8),
            "ladder": "冻结 sentinel ladder(cur261-c2-v9 默认 D0-D3;"
                      "candidate-independent;与任何 R8 candidate 数值"
                      "无依赖)",
            ...
            "gate": "每 corpus 独立:unique-event coverage ≥3600 / "
                    ...
                    "replay 完整性;任一 FAIL => R8 design FAIL(§15)",
        ...
        "design_data": {
            "blocks_per_candidate_per_corpus": DESIGN_BLOCKS_PER_CORPUS_R8,
            "corpora": list(DESIGN_NAMESPACES_R8),
        ...
        "independent_marginal_guard": {
            "namespace": DESIGN_INDEPENDENT_NAMESPACE_R8,
            "pairs_per_rung": DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R8,
            "conditions": "marginal ordering / D3 positive / 基线 margins"
                          ...
                          ";matched PASS 不可覆盖 FAIL;失败 → R8 = FAIL",
        ...
        "fail_path": {
            ...
            "on_semantic_gate_fail": "任一 semantic corpus gate FAIL → "
                                     "R8 design FAIL(不生成 candidate "
                                     "blocks;§15/§25)",
            "on_no_qualified_combination": "R8 = FAIL;自动 power/semantic "
...
def design_plan_digest_r8(plan: dict[str, Any]) -> str:
    ...
    return "r8dp-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def lock_design_plan_r8(out_dir: Path, plan: dict[str, Any],
                        ) -> tuple[Path, str]:
    ...
    path = out_dir / "r8_design_plan.json"
    if path.exists():
        raise RuntimeError(
            f"R8 design plan 已存在: {path}(§8.4 禁止删除/覆盖/重锁;"
            "重锁必须换新 iteration R8.1/R9)")
    digest = design_plan_digest_r8(plan)
    plan = dict(plan)
    ...
    (out_dir / "r8_design_plan_digest.txt").write_text(
        digest, encoding="utf-8")
    return path, digest


def load_locked_design_plan_r8(out_dir: Path,
    ...
    path = out_dir / "r8_design_plan.json"
    if not path.is_file():
        raise RuntimeError(f"R8 design plan 未锁定: {path}")
    plan = json.loads(path.read_text(encoding="utf-8"))
    digest = design_plan_digest_r8(plan)
    if plan.get("design_plan_digest") != digest:
        raise RuntimeError("R8 design plan digest 复算不一致(fail closed)")
    digest_path = out_dir / "r8_design_plan_digest.txt"
    if not digest_path.is_file() or \
            digest_path.read_text(encoding="utf-8").strip() != digest:
        raise RuntimeError("R8 design plan digest 文件与 payload 不一致")
    return plan, digest
...
def _evaluate_candidate_matched_r8(
    ...
    # 密度(R6 冻结 gate)+ R8 candidate-specific cue 语义 + 独立性/
    # 可观察性(冻结公共实现)
    ...
    semantics = {
        ...
        "context_observability": check_c2_context_observability(
            records),
        "candidate_cue_semantics_r8_cluster_aware": cue_sem,
        "cue_payoff_confusion": {
            k: v for k, v in cue_sem["per_rung"]["D0"]["sides"]["A"][
                "confusion"].items()},
        # R6 点阈值分离检查仅作诊断对照(绝不进入 R8 资格判定)
...
def _maximin_score_r8(corpus_results: list[dict[str, Any]],
                      n: int) -> float:
    ...
    for res in corpus_results:
        ...
        for per_rung in res["semantics"][
                "candidate_cue_semantics_r8_cluster_aware"][
                "per_rung"].values():
...
# ------------------------------------------------- 主流程
def run_design_stage_r8(out_dir: Path, plan: dict[str, Any],
                        design_digest: str,
                        baseline_commit: str = "") -> dict[str, Any]:
    """R8 design 主流程:治理检查 → semantic corpora(160×2)+ gate
    (FAIL 即短路)→ 3 candidate × 2 corpus blocks → candidate 评估 →
    机械选择 → marginal guard → pack。

    §8.4:plan 锁定后本函数内部任何异常都转 write_r8_iteration_aborted
    + re-raise(R8 永久结束;不删 plan、不重锁、不复用 namespace)。
    """
    require_r8_iteration_active()
    out_dir = Path(out_dir)
    try:
        return _run_design_stage_inner_r8(
        ...
        write_r8_iteration_aborted(
            ...
            f"{str(exc)[:300]});按 §8.4 R8 永久结束")
        raise


def _run_design_stage_inner_r8(out_dir: Path, plan: dict[str, Any],
                               design_digest: str,
                               baseline_commit: str,
                               ) -> dict[str, Any]:
    ...
    if not identity["pass"]:
        raise RuntimeError(
            f"R8 design plan code identity 与当前代码不一致:"
    ...
    # 第一条 R8 design/semantic episode 生成前记录 started(§8.4)
    mark_design_data_started()
    ...
    for corpus_ns in semantic_cfg["namespaces"]:
        ...
        fname = ("semantic_design_main.json"
                 if corpus_ns.endswith("main") else
                 "semantic_design_validation.json")
        # event trace 单独落盘(§37 semantic_design_*.jsonl)
        trace_rows = gate.pop("event_trace", [])
        gate_out = dict(gate)
        gate_out["event_count"] = len(trace_rows)
        (out_dir / fname).write_text(json.dumps(
            gate_out, indent=2, ensure_ascii=False, default=float),
            encoding="utf-8")
        with open(out_dir / (fname.replace(".json", ".jsonl")), "w",
    ...
    if not semantic_pass:
        summary = {
            "format": "cur261-r8-design-stage-v1",
            "iteration": "r8",
            ...
            "pass": False,
            "verdict": "R8 design FAIL:dedicated semantic corpus gate "
        ...
        (out_dir / "r8_sample_size_selection.json").write_text(
    ...
    for cand_id, ladder in grid.items():
        corpora = [
            _evaluate_candidate_matched_r8(
        ...
        for n in FORMAL_BLOCK_OPTIONS:
            ...
            if qualified_ns[str(n)]:
                scores[str(n)] = _maximin_score_r8(corpora, n)
        candidate_results[cand_id] = {
            ...
            "param_distance_from_historical": (
                ladder_distance_from_historical_r8(ladder)),
        }
    (out_dir / "r8_candidate_results.json").write_text(json.dumps(
    ...
    power = _build_power_summary_r8(
        candidate_results, semantic_summaries, selected_id, selected_n,
        design_digest)
    (out_dir / "r8_power_analysis.json").write_text(json.dumps(
    ...
    if selected_id is None:
        summary = {
            ...
            "pass": False,
            "verdict": "R8 FAIL:candidate × block count 无合格组合"
    ...
    if not marginal_artifact["guard"]["pass"]:
        summary = {
            ...
            "pass": False,
            "verdict": "R8 FAIL:选定 ladder 的独立-rung marginal guard "
    ...
    pack = ladder_pack_payload_r8(
        ...
        recall_floor_value=floor,
        noninferiority_delta=NONINFERIORITY_DELTA,
        semantic_blocks_per_corpus=SEMANTIC_BLOCKS_PER_CORPUS_R8,
        candidate_evidence={
    ...
    write_selected_pack_r8(out_dir, pack)
    ...
    selection = {
        "format": "cur261-r8-sample-size-selection-v1",
        "iteration": "r8",
        "design_plan_digest": design_digest,
        "selected_candidate": selected_id,
        "selected_block_count": selected_n,
    ...
    (out_dir / "r8_sample_size_selection.json").write_text(json.dumps(
...
def _run_independent_marginal_guard(
    ...
    """§26:design_r8_independent_marginal 语料的 marginal guard。

    R8 语义组装修正:R6 冻结的 c2_marginal_guard_conditions 检查的
    semantics 键为 cue_payoff_separation(R7 误传 cue_semantics——
    R7 calibration 未执行故未暴露);R8 以 semantics=None 调用 R6 统计
    ...
    for rung in CURRICULUM261_RUNGS:
        for idx in range(DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R8):
    ...
    indep_report = rung_report_r4(
        ...
        preproc=None, corpus=DESIGN_INDEPENDENT_NAMESPACE_R8)
    ...
    base = c2_marginal_guard_conditions(
        ...
        semantics=None)
    r8_semantics_ok = bool(
        indep_semantics["local_cue_independence"]["pass"]
        and indep_semantics["context_observability"]["pass"]
        and indep_cue["pass"])
    marginal = dict(base)
    marginal["format"] = "cur261-r8-c2-marginal-guard-v1"
    marginal["r8_semantics_rule"] = (
        "local cue ∧ context observability ∧ independent cue semantics"
        "(point recall ≥ 0.90 灾难护栏 + canonical + non-cue FP;§26)")
    marginal["r8_semantics_pass"] = r8_semantics_ok
    marginal["semantics_pass"] = r8_semantics_ok
    marginal["pass"] = bool(base["pass"] and r8_semantics_ok)
    artifact = {
        "format": "cur261-r8-independent-marginal-design-v1",
        "namespace": DESIGN_INDEPENDENT_NAMESPACE_R8,
        "pairs_per_rung": DESIGN_INDEPENDENT_PAIRS_PER_RUNG_R8,
...
def _build_power_summary_r8(candidate_results: dict[str, Any],
                            semantic_summaries: dict[str, Any],
    ...
    return {
        "format": "cur261-r8-power-analysis-v1",
        "design_plan_digest": design_digest,
        "selected_candidate": selected_id,
        "selected_block_count": selected_n,
