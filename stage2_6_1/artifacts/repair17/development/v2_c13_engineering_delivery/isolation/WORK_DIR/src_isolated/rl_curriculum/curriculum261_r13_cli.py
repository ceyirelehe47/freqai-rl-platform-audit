# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R13 CLI(audit → cue-audit → preplan-smoke →
...
为冻结前工程命令,在任何正式 R13 namespace 访问之前执行)。
...
digest 自引用缺陷修复的正式验收);design plan 在第一条 R13 design/
...
#: R13 启动基线 = R12 Commit B(诚实 FAIL 结果提交);§一 ancestry 语义。
BASELINE_COMMIT_R13 = "960dbe19701901f9262614aadf8b7f97742fab4d"
...
#: producer/consumer 落盘接口 —— R13RealArtifactCliRoundTrip-v1 的
#: 直接动因。
...
#: (curriculum261_r13_historical),不在源码手工转录文件级长 digest。
...
#: TypeError 后按 §18 硬规则诚实 FAIL;design 全部结果仅作 R13
...
#: R13 继承)。R10 design/pack 全部
#: 结果仅作 R13 development evidence。
...
def _default_art() -> Path:
    ...
    return qualification_r13_lock_dir()
    ...
    if release_repo:
        ...
        for key, rel in r11_files.items():
            ...
            base_blob = subprocess.run(
                ["git", "rev-parse", f"{BASELINE_COMMIT_R13}:{rel}"],
...
def _r12_abort_binding(out_dir: Path) -> dict:
    ...
    binding = {
        ...
        "final_verdict": "FAIL(永久;统计链 cue/global K/design/"
                         ...
                         "R13 输入;R13 必须全新 namespace 重新执行)",
        "fail_reasons": [
            ...
            "永久 FAIL,下一轮 R13)",
            "治理缺口(非硬失败,但为 R13 直接动因):Commit A 前的 "
        ...
        "latent_defect_2_discovered_by_r13": (
            "R13 全仓库接口审计另发现 sealed preflight 证据文件名错配"
            ...
            "R12 未活到该步故未触发;R13 已在 Commit A 前修复"),
        "statistical_chain_status_at_failure": (
            ...
            "holdout 独立 PASS —— 全部仅作历史记录,不进入 R13"),
        "abort_marker_exists": True,
        "abort_marker_blob_matches_baseline": _blob_matches(
...
def _r8_abort_binding(out_dir: Path) -> dict:
    ...
    if marker is None or not marker.is_file():
        raise RuntimeError(
            ...
            "禁止在 marker 缺失时锁 R13 design plan)")
    data = json.loads(marker.read_text(encoding="utf-8"))
...
def _code_freeze_sha(out_dir: Path) -> str:
    ...
    freeze = verify_r13_code_freeze(Path(out_dir))
    if not freeze["pass"]:
        raise RuntimeError(
            f"R13 code freeze 校验失败:{freeze}(design plan 必须绑定"
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    if freeze_sha:
        ...
        freeze_doc = write_r13_code_freeze(
        ...
        existing = verify_r13_code_freeze(Path(args.out_dir))
    ...
    _write_json(out, "baseline_ancestry.json", {
        "format": "cur261-r13-baseline-ancestry-v1",
        "expected_baseline": BASELINE_COMMIT_R13,
    ...
    _dump_txt(out, "preprocessing_v2_contract_digest.txt",
              preprocessing_v2_contract_digest())
    records = generate_fit_bank("preplan_smoke_r13", args.fit_pairs)
    ...
    from rl_curriculum.curriculum261_r13_dependencies import (
        write_dependency_resolution_r13,
    )

    dep = write_dependency_resolution_r13(out)
    from rl_curriculum.curriculum261_r13_delegation import (
        calibration_call_contract_payload,
        delegation_ast_checks_r13,
        live_signature_audit_r13,
    )
    from rl_curriculum.curriculum261_r13_routing import (
        bundle_routing_contract_payload,
    )

    sig_audit = live_signature_audit_r13()
    ast_checks = delegation_ast_checks_r13()
    ...
    freeze_check = verify_r13_code_freeze(out)
    ...
    _write_json(out, "r11_cue_failure_binding.json", {
        "format": "cur261-r13-r11-cue-failure-binding-v1",
        ...
        "classification": "position-wise mirror-count distribution "
                          "failure(t=226 < 264,非 tail;R13 禁止继续"
                          "称为 tail-position failure)",
        "binding_gate_in_r13": False,
    ...
    _write_json(out, "r12_iteration_failure_binding.json", {
        "format": "cur261-r13-r12-iteration-failure-binding-v1",
        "iteration": "r12",
        ...
        "consumer": "cmd_lock_plan → build_plan_r13("
        ...
        "governance_gap": "R12 pre-freeze release rehearsal 使用 "
                          ...
                          "artifact 读取(§三-2;R13 "
                          "R13RealArtifactCliRoundTrip-v1 修复)",
        "binding_gate_in_r13": True,
...
def _official_entrypoint_validation() -> dict:
    ...
    mods = [
        "rl_curriculum.curriculum261_r13_namespaces",
        "rl_curriculum.curriculum261_r13_dependencies",
        "rl_curriculum.curriculum261_r13_noise_replay",
        "rl_curriculum.curriculum261_r13_cue_contract",
        "rl_curriculum.curriculum261_r13_cue_eval",
        "rl_curriculum.curriculum261_r13_global_k",
        "rl_curriculum.curriculum261_r13_historical",
        "rl_curriculum.curriculum261_r13_generation_evidence",
        "rl_curriculum.curriculum261_r13_param_pack",
        "rl_curriculum.curriculum261_r13_preplan",
        "rl_curriculum.curriculum261_r13_design",
        "rl_curriculum.curriculum261_r13_calibration",
        "rl_curriculum.curriculum261_r13_plan",
        "rl_curriculum.curriculum261_r13_preflight",
        "rl_curriculum.curriculum261_r13_final",
        "rl_curriculum.curriculum261_r13_smoke",
        "rl_curriculum.curriculum261_generation_envelope",
        "rl_curriculum.curriculum261_r13_determinism",
        "rl_curriculum.curriculum261_r13_shadow",
        "rl_curriculum.curriculum261_r13_orchestrator",
        "rl_curriculum.curriculum261_r13_cli",
    ]
    failed: dict[str, str] = {}
    for m in mods:
        try:
    ...
    from rl_curriculum.curriculum261_r13_cli import main as cli_main
    ...
    return {
        "format": "cur261-r13-official-entrypoint-v1",
        "entrypoint": "rl_curriculum.curriculum261_r13_cli.main",
...
def _verify_namespace_safe() -> dict:
    try:
        from rl_curriculum.curriculum261_r13_namespaces import (
            verify_r13_namespace_isolation,
        )

        return verify_r13_namespace_isolation()
    except Exception as exc:  # noqa: BLE001
        return {"pass": False, "error": str(exc)[:300]}


def cmd_cue_audit(args: argparse.Namespace) -> int:
    """§12 三路闭合合同审计(在任何 R13 design/semantic data 之前)。

    --rehearsal(R13RealArtifactCliRoundTrip-v1):在 rt_cue_*_r13
    rehearsal-only namespace 上执行正式规模(500+500)审计,不锁定
    audit plan(非正式参数与锁定 plan 合同互斥;正式运行必须无该
    标志),不访问任何 R13 正式 cue/design namespace。
    """
    ...
    if getattr(args, "rehearsal", False):
        report = run_cue_contract_audit(
            out,
            model_namespace="rt_cue_model_r13",
            validation_namespace="rt_cue_validation_r13",
        ...
        print(f"[cue-audit][rehearsal] pass={report['pass']} "
              ...
              f"namespaces=rt_cue_model_r13/rt_cue_validation_r13")
        if not report["pass"]:
            print("[cue-audit][rehearsal] rehearsal 审计 FAIL——"
                  "R13RealArtifactCliRoundTrip-v1 不得通过")
            return 1
        return 0
    # §R13-10:任何正式 R13 cue-audit data 生成前先锁定 audit plan
    ...
    if not (out / "cue_audit_plan.json").is_file():
        _, plan_digest = lock_cue_audit_plan_r13(out)
        print(f"[cue-audit] audit plan locked digest={plan_digest}")
    load_locked_cue_audit_plan_r13(out)
...
def cmd_preplan_smoke(args: argparse.Namespace) -> int:
    ...
    不参与参数选择;只用 preplan_smoke_r13,不用 design/calibration/
    ...
    from rl_curriculum.curriculum261_r13_noise_replay import (
    ...
    blocks = [generate_matched_block_with_attempts(
        sentinel, namespace="preplan_smoke_r13", block_index=i)
        for i in range(3)]
    ...
    mini_gate = semantic_cue_gate(
        ...
        min_unique_positive_cues=1, label="preplan_smoke_r13")
    identity = {
        "sentinel": {r: {k: sentinel[r][k]
                         for k in ("alpha_bps", "wick_kappa")}
                     for r in sentinel},
        "sentinel_digest": "r13smoke-" + _hashlib.sha256(json.dumps(
            {r: sorted(sentinel[r].items()) for r in sorted(sentinel)},
            default=str).encode("utf-8")).hexdigest(),
        "n_blocks": len(blocks),
        "namespace": "preplan_smoke_r13",
    ...
    _write_json(out, "preplan_engineering_smoke.json", {
        "format": "cur261-r13-preplan-smoke-v1",
        "identity": identity,
    ...
    from rl_curriculum.curriculum261_r13_preplan import (
        run_preplan_rehearsal_r13,
    )

    rehearsal = run_preplan_rehearsal_r13(out / "preplan")
    ...
    from rl_curriculum.curriculum261_r13_preplan import (
        run_reference_root_cause_diagnosis_r13,
    )

    root_cause = run_reference_root_cause_diagnosis_r13(out / "preplan")
    ...
    from rl_curriculum.curriculum261_r13_rehearsal import (
        run_preplan_full_pipeline_rehearsal_r13,
    )

    full = run_preplan_full_pipeline_rehearsal_r13(out)
    if not full["pass"]:
...
_SUBPROC_LOADER_DESIGN = (
    ...
    "(design_plan_digest_r13, load_locked_design_plan_r13);"
    "import json,sys;"
    "plan,digest=load_locked_design_plan_r13(sys.argv[1]);"
    "recomputed=design_plan_digest_r13(plan);"
    "print(json.dumps({"
...
_SUBPROC_LOADER_QUAL = (
    "import os,sys;"
    "os.environ['CURRICULUM261_R13_LOCK_DIR']=sys.argv[2];"
    "from rl_curriculum.curriculum261_r13_plan import "
    "(load_locked_plan_r13, plan_digest_r13);"
    "import json;"
    "plan,digest=load_locked_plan_r13();"
    "print(json.dumps({"
    "'loaded_digest':digest,"
    "'recomputed':plan_digest_r13(plan),"
    "'iteration':plan['iteration'],"
...
def _no_alternate_loader_check() -> dict:
    ...
    权威模块(r13_design 的 lock/load;r13_namespaces 的路径常量与本
    扫描自身);其余模块不得直接读 plan JSON。"""
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    allowed_design = {"curriculum261_r13_design.py",
                      "curriculum261_r13_namespaces.py",
                      "curriculum261_r13_cli.py"}
    allowed_qual = {"curriculum261_r13_namespaces.py",
                    "curriculum261_r13_plan.py",
                    "curriculum261_r13_cli.py"}
    design_hits: list[str] = []
    qual_hits: list[str] = []
    for f in sorted(root.glob("curriculum261_r13_*.py")):
        text = f.read_text(encoding="utf-8")
        if "r13_design_plan.json" in text and f.name not in allowed_design:
            design_hits.append(f.name)
        if ("qualification_plan_r13.json" in text
                and f.name not in allowed_qual):
            qual_hits.append(f.name)
    return {
        "design_plan_readers": ["curriculum261_r13_design.py"],
        "qualification_plan_readers": ["curriculum261_r13_namespaces.py"
                                       " (path fn) + "
                                       "curriculum261_r13_plan.py (loader)"],
...
def cmd_plan_roundtrip(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td)
        plan = design_plan_payload_r13(
            baseline_commit=BASELINE_COMMIT_R13,
            ...
            preplan_rehearsal_digest="r13pr-" + "0" * 64,
            r8_abort_evidence={"roundtrip_synth": True},
            cue_audit_plan_digest="r13ap-" + "0" * 64,
        )
        # build payload 的 canonical 快照(逐位一致验证基准)
        pre_digest = design_plan_digest_r13(plan)
        path, digest = lock_design_plan_r13(tmp, plan)
        ...
        checks["payload_bit_identical"] = bool(
            design_plan_digest_r13(reloaded) == pre_digest)
        # 新进程 load(真实生产路径)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        synth_pack = {
            "digest": "r13pk-" + "0" * 64,
            "pack_version": "CurriculumR13MatchedLadderPack-v1",
            "selected_c2_candidate": "c2l_midpoint",
            "selected_block_count": 15,
            "c2_ladder": {r: dict(v) for r, v in
                          C2_LADDER_CANDIDATES_R13["c2l_midpoint"].items()},
            ...
            "recall_floor": 0.93,
            "p_contract": 0.95,
            "cue_contract_audit_digest": "r13ca-" + "0" * 64,
        ...
        if qual_plan is not None:
            ...
            old_dir = os.environ.get("CURRICULUM261_R13_LOCK_DIR")
            os.environ["CURRICULUM261_R13_LOCK_DIR"] = str(tmp)
            try:
                qpath, qdigest = lock_plan_r13(qual_plan)
                checks["qualification_lock_ok"] = bool(
                    qpath.is_file()
                    and qdigest == plan_digest_r13(qual_plan))
                ...
                if old_dir is None:
                    os.environ.pop("CURRICULUM261_R13_LOCK_DIR", None)
                else:
                    os.environ["CURRICULUM261_R13_LOCK_DIR"] = old_dir
    ...
    from rl_curriculum.curriculum261_r13_design import (
        load_locked_design_plan_r13 as _unused_loader,  # noqa: F401
    )
    ...
    result = {
        "format": "cur261-r13-plan-roundtrip-validation-v1",
        "iteration": "r13",
        "checks": checks,
...
def cmd_design_plan_lock(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r13_design import (
        semantic_artifact_identity_r13,
    )
    ...
    if rt_mode:
        # R13RealArtifactCliRoundTrip-v1:rehearsal cue-audit 不锁定
        # audit plan(非正式参数),plan 内以显式标记代替 digest;正式
        # 路径必须携带真实锁定 digest(下方 else 分支)。
        audit_plan_digest_value = "r13ap-rt-rehearsal-no-locked-plan"
        design_namespaces = ("rt_design_matched_main_r13",
                             "rt_design_matched_validation_r13")
        semantic_namespaces = ("rt_semantic_design_main_r13",
                               "rt_semantic_design_validation_r13")
        independent_namespace = "rt_design_independent_r13"
    else:
        audit_plan = load_locked_cue_audit_plan_r13(out)
    ...
    plan = design_plan_payload_r13(
        ...
        artifact_writer_identity=semantic_artifact_identity_r13(),
    ...
    path, digest = lock_design_plan_r13(out, plan)
...
def cmd_design(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r13_design import (
        ...
        run_design_stage_r13,
    )

    out = Path(args.out_dir)
    plan, digest = load_locked_design_plan_r13(out)
    selection = run_design_stage_r13(out, plan, digest,
                                    baseline_commit=BASELINE_COMMIT_R13)
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    R13 变革(相对 R9 的手写长流程):
    - 全部评估走共享 orchestrator orchestrate_calibration_stage_r13
    ...
    from rl_curriculum.curriculum261_r13_calibration import (
        ...
        run_generator_stress_r13,
    )
    from rl_curriculum.curriculum261_r13_dependencies import (
        verify_r13_code_freeze,
    )
    from rl_curriculum.curriculum261_r13_namespaces import (
        require_r13_iteration_active,
        write_r13_iteration_aborted,
    )
    from rl_curriculum.curriculum261_r13_orchestrator import (
        formal_holdout_profile_r13,
        formal_main_profile_r13,
        orchestrate_calibration_stage_r13,
    )
    from rl_curriculum.curriculum261_r13_param_pack import (
        frozen_parameter_identity_r13,
        r13_override_for,
        verify_r4_inheritance_r13,
    )
    ...
    try:
        ...
        write_r13_iteration_aborted(
            ...
            f"{str(exc)[:400]});§11 硬规则:R13 永久结束,下一轮必须 "
            "R13 + 全新 namespace")
        raise
...
def _cmd_calibrate_inner(args: argparse.Namespace,
    ...
    require_r13_iteration_active()
    freeze = verify_r13_code_freeze(out)
    if not freeze["pass"]:
        print(f"[calibrate] R13 code freeze 校验失败(fail closed):"
              f"{freeze}")
        return 1
    pack = _pack(out)
    inheritance = verify_r4_inheritance_r13(pack)
    if not inheritance:
        print("[calibrate] R4 inheritance 验证失败;fail closed")
        return 1
    n_blocks = int(pack["selected_block_count"])
    design_digest = (out / "r13_design_plan_digest.txt").read_text(
        encoding="utf-8").strip()
    recall_floor_value = float(pack["recall_floor"])
    rt_mode = bool(getattr(args, "rehearsal", False))
    if rt_mode:
        # R13RealArtifactCliRoundTrip-v1:rt_* rehearsal-only namespace;
        ...
        fit_ns_main = "rt2_fit_main_r13"
        fit_ns_hold = "rt2_fit_holdout_r13"
        profile_main_obj = rt_main_profile_r13()
        profile_holdout_obj = rt_holdout_profile_r13()
        conditioning_eval_ns = "rt2_calibration_main_r13"
        stress_ns = "rt2_stress_r13"
        print("[calibrate][rehearsal] rt_* namespace;正式生成规模;"
              "write_artifacts=True(R13RealArtifactCliRoundTrip-v1)")
    else:
        fit_ns_main = "preprocess_fit_calibration_r13"
        fit_ns_hold = "preprocess_fit_holdout_r13"
        profile_main_obj = formal_main_profile_r13(n_blocks)
        profile_holdout_obj = formal_holdout_profile_r13(n_blocks)
        conditioning_eval_ns = "calibration_r13"
        stress_ns = "stress_r13"

    print(f"[calibrate] fitting main preprocessor ({fit_ns_main})...")
    records_main = generate_fit_bank_r13(fit_ns_main, pack)
    v2_main, manifest_main = fit_preprocessor_v2_from_bank_r13(
        fit_ns_main, pack, records=records_main,
        parameter_pack_identity=pack["digest"])
    routing_main = build_routing_r13("main", v2_main, rt=rt_mode)
    print(f"[calibrate] fitting holdout preprocessor ({fit_ns_hold})...")
    records_hold = generate_fit_bank_r13(fit_ns_hold, pack)
    v2_hold, manifest_hold = fit_preprocessor_v2_from_bank_r13(
        fit_ns_hold, pack, records=records_hold,
        parameter_pack_identity=pack["digest"])
    routing_holdout = build_routing_r13("holdout", v2_hold, rt=rt_mode)
    ...
    stage = orchestrate_calibration_stage_r13(
        out, pack,
        n_blocks=n_blocks,
    ...
    eval_records = [
        generate_pair(f, r, 0, namespace=conditioning_eval_ns,
                      rung_params_override=r13_override_for(f, pack))
    ...
    stress = run_generator_stress_r13(pack, namespace=stress_ns)
    _write_json(out, "generator_stress.json", stress)
    _write_json(out, "frozen_parameter_identity.json",
                frozen_parameter_identity_r13())

    gate = {
        "format": "cur261-r13-robustness-gate-v1",
        "iteration": "r13",
        "rehearsal": rt_mode,
...
def cmd_preflight_static(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r13_preflight import (
        run_prelock_static_preflight_r13,
    )

    result = run_prelock_static_preflight_r13(Path(args.out_dir),
                                             VENDOR_PIN)
...
def cmd_lock_plan(args: argparse.Namespace) -> int:
    ...
    calibration_bundle_hash = read_preprocessor_bundle_hash(
        ...
        consumer_command="r13-cli lock-plan")
    ...
    plan = build_plan_r13(
        ...
        frozen_parameter_identity=frozen_parameter_identity_r13(),
    ...
    path, digest = lock_plan_r13(plan)
...
def cmd_preflight_sealed(args: argparse.Namespace) -> int:
    ...
    att = run_postlock_sealed_preflight_r13(Path(args.out_dir),
                                           VENDOR_PIN)
...
#: R13RealArtifactCliRoundTrip-v1 的 final 阶段 rehearsal profile
#: (§四-4):rt_*_r13 rehearsal-only namespace + 缩小样本量;治理外壳
...
R13_RT_FINAL_PROFILE: dict = {
    "final_namespace": "rt2_qualification_r13",
    "fit_namespace": "rt2_fit_qualification_r13",
    "c13_pairs_per_rung": 2,
    "c2_blocks": 4,
    "semantic_block_count": 8,
    "independent_pairs_per_rung": 2,
    "independent_namespace": "rt2_c2_independent_main_r13",
    "semantic_namespace": "rt2_semantic_final_r13",
    "supervised_namespace": "rt2_supervised_main_r13",
    "supervised_model_seeds": (20270135,),
    "supervised_training_config": {"epochs": 2},
    "conditioning_fit_namespace": "rt2_fit_main_r13",
}
...
def cmd_qualify(args: argparse.Namespace) -> int:
    ...
    repair R13(§11/工作包 A2):final 的任何异常在处置前落盘全部
    逐 attempt invocation envelopes,随后由 run_final_qualification_
    r13 的既有 crash 处理写 exposure=crashed;此处补写 iteration
    aborted marker(R10 缺口:final 阶段异常同样触发 §11 硬规则)。

    --rehearsal(R13RealArtifactCliRoundTrip-v1):以 R13_RT_FINAL_
    ...
    rt_mode = bool(getattr(args, "rehearsal", False))
    try:
        result = run_final_qualification_r13(
            out, rehearsal_profile=R13_RT_FINAL_PROFILE if rt_mode
            else None)
    except Exception as exc:  # noqa: BLE001 —— §11 正式异常处置
        from rl_curriculum.curriculum261_generation_envelope import (
            dump_failure_evidence,
...
def cmd_smoke(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r13_smoke import run_ppo_smoke_r13
    ...
    try:
        ...
        pass
    smoke = run_ppo_smoke_r13(pack=pack)
...
def cmd_namespace_integrity(args: argparse.Namespace) -> int:
    ...
    print(f"[namespace-integrity] pass={ns.get('pass')} "
          f"namespaces={len(ns.get('r13_namespaces', []))}")
...
def cmd_determinism_matrix(args: argparse.Namespace) -> int:
    ...
    工程阶段命令(在任何正式 R13 namespace 访问之前执行);产物写
    ...
    from rl_curriculum.curriculum261_r13_determinism import (
        audit_generator_mutable_state,
        generation_determinism_gate,
        run_cross_process_determinism_matrix,
    )

    out = qualification_r13_lock_dir() / "determinism"
...
def cmd_shadow_run(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r13_shadow import (
        run_full_scale_shadow_r13,
    )

    base = qualification_r13_lock_dir() / "shadow"
    ...
    summary = run_full_scale_shadow_r13(run_dir, run_tag=str(args.run_tag))
...
def cmd_global_k_reanalysis(args: argparse.Namespace) -> int:
    ...
    - 明确声明:重分析只用于 method development 与回归,不改变 R11
      的 FAIL 结论;禁止依据该结果调整 R13 的 alpha/eligibility/
      randomization count/statistic/tail 定义。
    """
    import math as _math

    from rl_curriculum.curriculum261_r13_global_k import (
    ...
    _write_json(out, "global_k_historical_reanalysis.json", {
        "format": "cur261-r13-global-k-historical-reanalysis-v1",
...
def cmd_release_rehearsal(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r13_cue_contract import (
        cue_audit_plan_digest_r13,
        cue_audit_plan_payload_r13,
        load_locked_cue_audit_plan_r13,
        lock_cue_audit_plan_r13,
    )
    from rl_curriculum.curriculum261_r13_namespaces import (
    ...
    with tempfile.TemporaryDirectory(prefix="r13_rehearsal_") as td:
        tdp = Path(td)
        ...
        sections["audit_run1"] = {
            ...
            "freeze_writer_written": (tdp / "r13_code_freeze.json").is_file(),
        }
        # ---- cue-audit plan build/lock/load/verify 路径 ----
        plan_dir = tdp / "cue_plan"
        _, digest1 = lock_cue_audit_plan_r13(plan_dir)
        loaded = load_locked_cue_audit_plan_r13(plan_dir)
        plan_payload = cue_audit_plan_payload_r13()
        expected_digest = cue_audit_plan_digest_r13(plan_payload)
        ...
        # ---- plan 重复锁定必须被拒绝 ----
        try:
            lock_cue_audit_plan_r13(plan_dir)
        ...
        try:
            from rl_curriculum.curriculum261_r13_namespaces import (
                require_r13_iteration_active,
                r13_iteration_aborted as _aborted,
            )
            ...
            abort_written_and_enforced = (
                _aborted()
                and _verify_raises(require_r13_iteration_active))
        finally:
            if old_lock is None:
        ...
        mini_audit = run_cue_contract_audit(
            ...
            model_namespace="preplan_engineering_smoke_r13",
            validation_namespace="preplan_engineering_smoke_r13",
            require_locked_plan=False)
        rplan = design_plan_payload_r13(
            baseline_commit=BASELINE_COMMIT_R13,
            vendor_pin=VENDOR_PIN,
            v2_contract_digest=preprocessing_v2_contract_digest(),
            prior_r2_plan_digest=PRIOR_R2_PLAN_DIGEST,
            ...
            preplan_smoke_identity={
                "sentinel_digest": "r13smoke-" + "0" * 64,
                "cue_contract_digest": "r13cue-" + "0" * 64,
            ...
            cue_audit_plan_digest="r13ap-" + "0" * 64)
        pre_d = design_plan_digest_r13(rplan)
        plan_dir = tdp / "design_plan"
        _, locked_d = lock_design_plan_r13(plan_dir, rplan)
        reloaded_plan, reloaded_d = load_locked_design_plan_r13(
            plan_dir)
        ...
        for _rk in ("design_plan_digest", "locked_utc"):
            ...
            _orig.pop(_rk, None)
        try:
            lock_design_plan_r13(plan_dir, rplan)
        ...
        from rl_curriculum.curriculum261_r13_dependencies import (
            write_r13_code_freeze as _wcf,
        )

        _wcf(cl_dir, code_freeze_sha=head)
        cl_path = write_path_cleanliness_r13(cl_dir, verdict="FAIL")
        cl_doc = json.loads(cl_path.read_text(encoding="utf-8"))
        try:
            write_path_cleanliness_r13(cl_dir, verdict="FAIL")
    ...
    with tempfile.TemporaryDirectory(prefix="r13_rehearsal2_") as td:
        tdp = Path(td)
        audit_args = argparse.Namespace(
            out_dir=str(tdp), fit_pairs=2, code_freeze_sha=head)
    ...
    result = {
        "format": "cur261-r13-pre-freeze-release-rehearsal-v1",
        "head_at_rehearsal": head,
        "monkeypatch_used": False,
        "temp_dirs_only": True,
        "formal_r13_namespace_accessed": False,
        "sections": sections,
        "pass": ok,
    }
...
def write_path_cleanliness_r13(out_dir: Path, *, verdict: str) -> Path:
    ...
    from rl_curriculum.curriculum261_r13_namespaces import (
        ...
        r13_iteration_aborted,
    )

    freeze_check = verify_r13_code_freeze(out_dir)
    result = _load(out_dir / "qualification_result.json")
    abort = _load(out_dir / "r13_iteration_aborted.json")
    design_plan_digest_txt = (out_dir / "r13_design_plan_digest.txt")
    # §四-2 修复:R12 版读取无 producer 的 calibration_report_{main,
    # holdout}.json 导致 calibration_state 恒为 absent;R13 读取
    ...
    for doc in (result, abort):
        if isinstance(doc, dict):
            ...
            if failing_gate:
                break
    payload: dict = {
        "format": "cur261-r13-path-cleanliness-v1",
        "verdict": verdict,
        ...
        "abort_artifact_sha256": _sha(
            out_dir / "r13_iteration_aborted.json"),
        ...
        "exposure_state": ("exposed" if qualification_r13_exposed()
                           else "not_exposed"),
        "final_namespace_state": (
            "executed" if isinstance(result, dict) else
            "aborted" if r13_iteration_aborted() else "untouched"),
        "pooled_rescue_used": False,
...
#: 工程 namespace preplan_*/shadow_*/stress_r13/training_r13/
#: fresh_holdout_r13/ppo_smoke_r13/reference_diagnostic_* 不在此列)。
R13_FORMAL_CRITICAL_NAMESPACES: tuple[str, ...] = (
...
def artifact_interface_audit() -> dict[str, Any]:
    ...
    """
    return {
        "format": "cur261-r13-artifact-interface-audit-v1",
        "iteration": "r13",
        "canonical_identity_fields": {
            "preprocessor_bundle_{calibration,holdout}.json":
                ...
                "'bundle_hash' 禁止冒充)",
            "r13_parameter_pack.json": "digest(r13pk-;pack_digest_r13"
                                       " 复算)",
            "r13_design_plan.json": "design_plan_digest(r13dp-)",
            "qualification_plan_r13.json": "plan_digest(r13qp-)",
            "sealed_final_preflight.json": "digest(r13fa-)",
        ...
        "edges": [
            {
                ...
                "r13_closure":
                    ...
                    "accessor(curriculum261_r13_plan.py;§四-1)",
            ...
            },
            {
                ...
                "r12_defect":
                    ...
                    "名称)——潜伏缺陷,R13 修复(§四-2)",
                "r13_closure":
                    "run_postlock_sealed_preflight_r13 证据文件清单"
                    "与真实 producer 对齐",
            ...
            },
            {
                ...
                "r12_defect":
                    ...
                    "supervised_learnability.json——潜伏缺陷,R13 修复",
                "rehearsal_step": "preflight-sealed",
            },
            {
            ...
            },
            {
                ...
                "artifacts": ["qualification_plan_r13.json",
            ...
            },
            {
                ...
                "artifacts": ["r13_parameter_pack.json",
                              "r13_parameter_pack_digest.txt"],
                "consumer_command":
            ...
            },
            {
                ...
                "consumer_command": "下游 reader / full-cold 测试",
                "consumer_fields": [
                    "pass",
            ...
            },
            {
                ...
                "artifacts": ["r13_design_plan.json",
                              "r13_design_plan_digest.txt"],
...
def cmd_real_artifact_rehearsal(args: argparse.Namespace) -> int:
    ...
    rh = qualification_r13_lock_dir() / "real_artifact_rehearsal"
    ...
    def _run_step(name: str, cli_args: list[str], *,
        ...
        env["PYTHONPATH"] = str(proj / "src") + os.pathsep + env.get(
            "PYTHONPATH", "")
        env["CURRICULUM261_R13_LOCK_DIR"] = str(rh)
        argv = [sys.executable, "-m",
                "rl_curriculum.curriculum261_r13_cli", *cli_args]
        rec: dict[str, Any] = {
            "step": name,
            "argv": argv,
    ...
    _run_step(
        ...
        outputs=("r13_code_freeze.json", "baseline_ancestry.json",
    ...
    _run_step("design-plan-lock", ["design-plan-lock", "--out-dir",
                                   str(rh), "--rehearsal"],
              outputs=("r13_design_plan.json",
                       "r13_design_plan_digest.txt"))
    _run_step("design", ["design", "--out-dir", str(rh)],
              outputs=("r13_parameter_pack.json",
                       "r13_parameter_pack_digest.txt"))
    ...
    _run_step("lock-plan", ["lock-plan", "--out-dir", str(rh)],
              inputs=("preprocessor_bundle_calibration.json",
                      ...
                      "robustness_gate.json",
                      "calibration_evidence.json"),
              outputs=("qualification_plan_r13.json",
                       "qualification_plan_digest_r13.txt"))
    _run_step("preflight-sealed", ["preflight-sealed", "--out-dir",
                                   str(rh)],
              inputs=("qualification_plan_r13.json",
                      "r13_parameter_pack.json"),
    ...
    _run_step("smoke", ["smoke", "--out-dir", str(rh)],
              inputs=("r13_parameter_pack.json",
                      "qualification_result.json"),
    ...
    probe_env["CURRICULUM261_R13_LOCK_DIR"] = str(rh)
    boundary: dict[str, Any] = {}
    boundary["plan_loader"] = _rt_probe(probe_env, (
        "import json,sys;"
        "sys.path.insert(0, sys.argv[2]);"
        "from rl_curriculum.curriculum261_r13_plan import "
        "(load_locked_plan_r13, read_preprocessor_bundle_hash);"
        "from pathlib import Path;"
        "plan,digest=load_locked_plan_r13();"
        "rh=Path(sys.argv[1]);"
    ...
    boundary["exposure_one_shot"] = _rt_probe(probe_env, (
        ...
        "from rl_curriculum.curriculum261_r13_namespaces import "
        "qualification_r13_exposed;"
        "print(json.dumps({'exposed':qualification_r13_exposed()}))"),
        str(rh), str(proj / "src"))
    ...
    for ledger_path in sorted(rh.rglob(
        ...
        for line in ledger_path.read_text(encoding="utf-8").splitlines():
            ...
            ns = str(row.get("call", {}).get("namespace",
                     row.get("namespace", "")))
            if ns in R13_FORMAL_CRITICAL_NAMESPACES:
    ...
    result = {
        "format": "R13RealArtifactCliRoundTrip-v1",
        "iteration": "r13",
        "head_at_rehearsal": head,
        "rehearsal_dir": str(rh),
        "monkeypatch_used": False,
        "cli_entrypoint":
            "subprocess python -m rl_curriculum.curriculum261_r13_cli",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="r13-cli")
    ...
    for name in ("cue-audit", "design-plan-lock", "calibrate",
        ...
        p.add_argument("--rehearsal", action="store_true",
                       help="R13RealArtifactCliRoundTrip-v1 rehearsal-"
