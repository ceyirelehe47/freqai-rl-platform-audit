# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R16:final preflight(§25)。

A. pre-lock static:imports/vendor/Route C identity/code identity/
   V2 serialize+outer env+PPO smoke(ppo_smoke_r16)/matched generator
...
def _imports_ok_r16() -> dict[str, Any]:
    modules = [
...
def _dependency_resolution_ok_r16() -> dict[str, Any]:
    ...
    return resolve_dependency_identity_r16()


def _code_identity_digest_r16() -> dict[str, Any]:
    from rl_curriculum.curriculum261_r16_plan import _code_identity_r16

    ident = _code_identity_r16()
    missing = [k for k, v in ident.items() if v == "MISSING"]
    return {
        "identity": ident,
        "missing": missing,
        "digest": "r15ci-" + hashlib.sha256(
            _canonical(ident).encode("utf-8")).hexdigest(),
        "pass": not missing,
    }


def _matched_generator_probe_r16() -> dict[str, Any]:
    """matched generator + unique-event evaluator + cluster bootstrap
    小规模验证(ppo_smoke_r16 下 2 个 block;不触碰 design/cal/final)。"""
    try:
        ...
        for idx in range(2):
            blocks.append(generate_matched_block_with_attempts(
                dict(C2_RUNG_PARAMS), namespace="ppo_smoke_r16",
                block_index=idx))
        ...
        # R16 §7:真实(非 monkeypatch)candidate evaluator 集成探针
        ...
        eval_result = _evaluate_candidate_matched_r16(
            "preflight_sentinel", dict(C2_RUNG_PARAMS),
            "ppo_smoke_r16", thresholds, blocks=blocks, n_blocks=2)
        evaluator_ok = bool(
...
def run_prelock_static_preflight_r16(out_dir: Path,
                                    vendor_pin: str) -> dict[str, Any]:
    """§25.1:plan lock 前静态 preflight(只用非 final namespace)。"""
    out_dir = Path(out_dir)
    checks: dict[str, Any] = {}
    checks["imports"] = _imports_ok_r16()
    checks["dependency_resolution"] = _dependency_resolution_ok_r16()
    ...
    checks["code_identity"] = _code_identity_digest_r16()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        old = os.environ.get("CURRICULUM261_R16_STATE_ROOT")
        os.environ["CURRICULUM261_R16_STATE_ROOT"] = td
        try:
            # R16:exposure 状态机探针走执行治理内核(权威 journal;
            ...
            try:
                probe.record_exposure_started(
                    "r16dp-preflight-probe")
                try:
                    probe.record_exposure_started(
                        "r16dp-preflight-probe-2")
                    double_create_rejected = False
                except RuntimeError:
                    double_create_rejected = True
                marker_ok = bool(
                    double_create_rejected
                    and _probe_exposure_state()["exposed"])
                try:
                    R16FormalSession.acquire(
                        binding={"probe": "preflight-lock-probe"})
            ...
            if old is None:
                ...
                os.environ["CURRICULUM261_R16_STATE_ROOT"] = old
    checks["marker_atomic_exclusive"] = marker_ok
    checks["concurrent_final_lock_rejected"] = lock_ok

    result = {
        "format": "cur261-r16-prelock-static-preflight-v1",
        "iteration": "r16",
        "executed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespaces_touched": ["ppo_smoke_r16"],
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "prelock_static_preflight_r16.json").write_text(
...
def sealed_preflight_digest(att: dict[str, Any]) -> str:
    ...
    return "r15fa-" + hashlib.sha256(
        _canonical(payload).encode("utf-8")).hexdigest()


def run_postlock_sealed_preflight_r16(out_dir: Path,
                                     vendor_pin: str,
                                     ) -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    plan, plan_digest = load_locked_plan_r16()
    checks["plan_digest_recompute"] = True
    checks["plan_iteration_r16"] = bool(plan.get("iteration") == "r16")
    ...
    checks["r4_inheritance_verified"] = verify_r4_inheritance_r16(pack)
    ...
    out = Path(out_dir)
    # §四-2 修复:R12 版检查 R3/R4 时代的文件名(preprocessing_
    # robustness_gate.json / curriculum_robustness_gate.json /
    # supervised_learnability.json),而 R12/R16 calibrate 实际写
    # preprocessing_v2_requalification.json / robustness_gate.json /
    # supervised_learnability_{main,holdout}.json —— 修完 bundle_hash
    # 后的第二个必崩点;R16 与真实 producer 文件名对齐。
    for name in ("preprocessing_v2_requalification.json",
                 ...
                 "prelock_static_preflight_r16.json"):
        path = out / name
        if path.is_file():
    ...
    code_digest = "r15ci-" + hashlib.sha256(
        _canonical(ident).encode("utf-8")).hexdigest()
    ...
    checks["final_runner_dependencies"] = _imports_ok_r16()["pass"]
    checks["plan_path_resolves"] = bool(
        qualification_r16_plan_path().is_file()
        and qualification_r16_digest_path().is_file()
        and r16_parameter_pack_path().is_file())
    ...
    attestation = {
        "format": "cur261-r16-sealed-final-preflight-v1",
        "iteration": "r16",
        "pass": att_pass,
    ...
    (out / "sealed_final_preflight_r16.json").write_text(
        json.dumps(attestation, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8")
    (out / "sealed_final_preflight_r16_digest.txt").write_text(
        attestation["digest"], encoding="utf-8")
    return attestation


def write_rehearsal_sealed_preflight_r16(
        out_dir: Path, *, plan_digest: str) -> dict[str, Any]:
    """§12 rehearsal 的 sealed preflight(临时目录;零正式 seed)。

    与正式 run_postlock_sealed_preflight_r16 同构的零 final-seed
    attestation(rehearsal 标志 + preplan namespace 声明)。
    """
    from rl_curriculum.curriculum261_r16_namespaces import (
        R16_SEALED_PREFLIGHT_DIGEST_FILENAME,
        R16_SEALED_PREFLIGHT_FILENAME,
    )
    ...
    (out_dir / R16_SEALED_PREFLIGHT_FILENAME).write_text(
        json.dumps(att, indent=1, ensure_ascii=False), encoding="utf-8")
    (out_dir / R16_SEALED_PREFLIGHT_DIGEST_FILENAME).write_text(
...
def verify_sealed_attestation(out_dir: Path) -> dict[str, Any]:
    out_dir = Path(out_dir)
    path = out_dir / "sealed_final_preflight_r16.json"
    if not path.is_file():
    ...
    try:
        ...
        digest_ok = False
    digest_file = out_dir / "sealed_final_preflight_r16_digest.txt"
