# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R8:Cue 检测语义合同 v2 与三路闭合合同审计。
...
1. mirror candidate 边界修正:真实 paired_noise() 只有
   ...
   实现在 curriculum261_r8_noise_replay.mirror_candidate_positions
...
2. 三路独立闭合(§12;R7 只有 analytic 自证 + 单一 bridge):
   ...
   C. Direct Generator Replay:两个全新 candidate-independent audit
      corpora(cue_contract_model_r8 / cue_contract_validation_r8,
...
dedicated semantic corpus(§14/§15,curriculum261_r8_cue_eval)。
"""
...
#: 其他语义指标业务阈值(与 R6/R7 相同;R8 用 cluster bounds)。
...
#: 合同审计配置(冻结;§12)。
AUDIT_MODEL_NAMESPACE = "cue_contract_model_r8"
AUDIT_VALIDATION_NAMESPACE = "cue_contract_validation_r8"
AUDIT_BLOCKS_PER_CORPUS = 500
AUDIT_ATTEMPT = 0
AUDIT_RNG_SEED = 20261008
...
#: audit bootstrap(与 r8_cue_eval 同规格;本模块自含双侧 CI 实现)。
AUDIT_BOOTSTRAP_RESAMPLES = 20000
AUDIT_BOOTSTRAP_SEED = 20261009


#: audit 使用的 sentinel ladder(= 冻结 cur261-c2-v9 默认 D0-D3;
#: 与 R8 candidate 网格无依赖——alpha_bps/wick_kappa 不进入 cue 表/
...
def run_cue_contract_audit(out_dir: Path | None = None,
    ...
    """执行三路闭合合同审计(在任何 R8 design/semantic data 之前)。
    ...
    for corpus_name, ns in (("model", AUDIT_MODEL_NAMESPACE),
                            ("validation", AUDIT_VALIDATION_NAMESPACE)):
        items: list[tuple[int, int, dict[str, Any]]] = []
        cue_table_consistent = True
        for block_index in range(AUDIT_BLOCKS_PER_CORPUS):
            block_seed = derive261_block_seed(ns, block_index, AUDIT_ATTEMPT)
            episodes = generate_matched_block_once(ladder, block_seed, ns)
            ref_cue = episodes["D0"]["A"].hidden["cue_dir"].to_numpy()
    ...
    report: dict[str, Any] = {
        "format": "cur261-r8-cue-contract-audit-v1",
        ...
        "audit_namespaces": {
            ...
            "validation": AUDIT_VALIDATION_NAMESPACE},
        "audit_blocks_per_corpus": AUDIT_BLOCKS_PER_CORPUS,
        ...
        "mirror_bound_v2": {
            ...
            "authority": "curriculum261_r8_noise_replay."
    ...
    if out_dir is not None:
        ...
        (out_dir / "cue_k_distribution.json").write_text(json.dumps({
            "format": "cur261-r8-cue-k-distribution-v1",
        ...
        (out_dir / "tail_mirror_validation.json").write_text(json.dumps({
            "format": "cur261-r8-tail-mirror-validation-v1",
        ...
        (out_dir / "noise_replay_validation.json").write_text(json.dumps({
            "format": "cur261-r8-noise-replay-validation-v1",
            "tolerance": 1e-12,
            "model": {
...
def cue_contract_audit_digest(report: dict[str, Any]) -> str:
    ...
    blob = json.dumps(core, sort_keys=True, ensure_ascii=False,
                      default=float)
    return "r8ca-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
...
def cue_semantic_contract_digest() -> str:
    blob = json.dumps(cue_semantic_contract_payload(), sort_keys=True,
                      ensure_ascii=False)
    return "r8cue-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
