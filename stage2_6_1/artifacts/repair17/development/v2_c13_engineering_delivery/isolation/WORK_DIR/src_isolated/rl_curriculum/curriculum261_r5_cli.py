# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R5 CLI(WP 串联)。

子命令(按执行顺序):
- audit:             启动审计(baseline/historical digests/vendor/
                     Route C integrity + production preprocessing 审计);
- design-plan-lock:  §13 design plan 构建与锁定(生成任何 design data 前);
- design:            §10/§14/§15/§16/§17 两级 candidate design + 选择
                     + parameter pack 锁定;
- calibrate:         §22/§23/§24 strict 双 corpus calibration(一次性);
- preflight-static:  §26A pre-lock 静态 preflight;
- lock-plan:         §28 plan 构建与锁定(双 strict gate 前置);
- preflight-sealed:  §26B post-lock sealed preflight + attestation;
- qualify:           §29 一次性 final qualification(120 pairs);
- smoke:             256-step PPO plumbing smoke(V2 outer adapter);
- namespace-integrity:§12 R5 namespace 隔离验证。

CLI output 目录与 lock-marker 目录统一(默认均为
artifacts/route_c_stage2_6_1_repair5 = CURRICULUM261_R5_LOCK_DIR)。
"""
...
#: 本轮 baseline(Stage 2.6.1 Repair R4 诚实 FAIL checkpoint)。
BASELINE_COMMIT_R5 = "95bb927f3ba46fa18b98602ea05c37ed67df198b"
VENDOR_PIN = "52bc96f4480b1a0da6a9b455bd00b17fbb6786a5"
PRIOR_R4_BASELINE_COMMIT = "d105405e5ddd989d6faf0601e912907746ad8980"
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    # 1) baseline integrity(发布仓库 HEAD = R4 FAIL checkpoint)
    pub_repo = Path(__file__).resolve().parents[2] / ".." / ".."
    release_repo = None
    ...
    r4_pack_file = (release_repo
                    / "stage2_6_1/artifacts/repair4/r4_parameter_pack.json"
    ...
    historical_binding = {
        ...
        "r4_parameter_pack_digest_expected": PRIOR_R4_PARAMETER_PACK_DIGEST,
        "r4_parent_of_baseline": PRIOR_R4_BASELINE_COMMIT,
    }
    ...
    _write_json(out, "baseline_integrity.json", {
        "expected_baseline": BASELINE_COMMIT_R5,
        "release_repo_head": head,
        "baseline_matches": bool(head == BASELINE_COMMIT_R5),
        "note": "发布仓库 HEAD 应等于 R4 诚实 FAIL checkpoint"
                "(或其明确后继;组装 R5 产物前)",
    })
    ...
    # production preprocessing 审计 + 数值等价(fit pairs 来自非 final
    ...
    _dump_txt(out, "preprocessing_v2_contract_digest.txt",
              preprocessing_v2_contract_digest())
    records = generate_fit_bank("ppo_smoke_r5", args.fit_pairs)
...
def cmd_design_plan_lock(args: argparse.Namespace) -> int:
    ...
    plan = design_plan_payload(
        baseline_commit=BASELINE_COMMIT_R5,
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r5_calibration import (
        ...
        preprocessing_robustness_checks_r5,
        run_c2_density_diagnostics_r5,
        run_c2_diagnostics_r5,
        run_calibration_corpus_r5,
        run_generator_stress_r5,
    ...
    print("[calibrate] fitting main preprocessor "
          "(preprocess_fit_calibration_r5)...")
    records_main = generate_fit_bank_r5(
        "preprocess_fit_calibration_r5", pack)
    v2_main, manifest_main = fit_preprocessor_v2_from_bank_r5(
        "preprocess_fit_calibration_r5", pack, records=records_main,
        parameter_pack_identity=pack["digest"])
    print("[calibrate] fitting holdout preprocessor "
          "(preprocess_fit_holdout_r5)...")
    records_hold = generate_fit_bank_r5(
        "preprocess_fit_holdout_r5", pack)
    v2_hold, manifest_hold = fit_preprocessor_v2_from_bank_r5(
        "preprocess_fit_holdout_r5", pack, records=records_hold,
        parameter_pack_identity=pack["digest"])
    ...
    _write_json(out, "fit_eval_isolation.json", {
        ...
        "holdout": {k: manifest_hold[k] for k in (
            "namespace", "pairs_per_rung", "n_pairs", "n_episodes",
            "n_rows", "integrity_all_ok", "multiset_hash")},
        "eval_namespaces": ["calibration_r5", "calibration_holdout_r5"],
    ...
    eval_records = [
        generate_pair(f, r, 0, namespace="calibration_r5",
                      rung_params_override=r5_override_for(f, pack))
        for f in CURRICULUM261_FAMILIES for r in CURRICULUM261_RUNGS]
    equiv_records = [
        generate_pair(f, r, i, namespace="calibration_r5",
                      rung_params_override=r5_override_for(f, pack))
        for f in CURRICULUM261_FAMILIES for r in CURRICULUM261_RUNGS
        for i in range(3)]
    prep_rob = preprocessing_robustness_checks_r5(
    ...
    supervised = supervised_learnability_run_r5(v2_main, pack)
    _write_json(out, "supervised_learnability.json", supervised)

    print("[calibrate] curriculum calibration_r5 (strict)...")
    calib_main = run_calibration_corpus_r5(
        v2_main, pack, "calibration_r5", out_dir=out,
        prefix="calibration")
    print("[calibrate] curriculum calibration_holdout_r5 (strict)...")
    calib_hold = run_calibration_corpus_r5(
        v2_main, pack, "calibration_holdout_r5", out_dir=out,
        prefix="calibration_holdout")

    print("[calibrate] C2 diagnostics + density (R5 语料)...")
    c2_diag = run_c2_diagnostics_r5(pack)
    ...
    c2_density = run_c2_density_diagnostics_r5(pack)
    _write_json(out, "c2_density_diagnostics.json", c2_density)
    print("[calibrate] generator stress (stress_r5)...")
    stress = run_generator_stress_r5(pack)
    _write_json(out, "generator_stress_summary.json", stress)

    preprocessing_gate = {
        "format": "cur261-r5-preprocessing-robustness-gate-v1",
    ...
    curriculum_gate = curriculum_robustness_gate_r5(
        calib_main, calib_hold, kappa=ROBUSTNESS_KAPPA_R5,
    ...
    overall = {
        "format": "cur261-r5-robustness-gate-v1",
        "iteration": "r5",
        "preprocessing_gate": {
            "pass": preprocessing_gate["pass"]},
        "curriculum_gate": {
            "pass": curriculum_gate["pass"],
            "rule": "strict per-corpus AND(唯一口径;无 pooled 救援)"},
        "supervised_gate": {
    ...
    _write_json(out, "calibration_evidence.json", {
        ...
        "design_plan_digest": (out / "r5_design_plan_digest.txt"
                               ).read_text(encoding="utf-8").strip(),
        "rung_params_by_family": {
            f: r5_family_rung_params(f, pack)
            for f in ("c1_opportunity", "c2_context", "c3_cost")},
        "frozen_parameter_identity": frozen_parameter_identity_r5(
            pack["tier"]),
    ...
    if not overall["pass"]:
        print("[calibrate] robustness gate FAIL——禁止 lock plan(§25);"
              "R5 = FAIL,修复须 R5.1/R6 + 全新 namespaces")
        return 1
    return 0
...
def _verify_namespace_safe() -> dict:
    try:
        return verify_r5_namespace_isolation()
...
def cmd_lock_plan(args: argparse.Namespace) -> int:
    ...
    if not gate.get("pass"):
        print("[lock-plan] robustness gate 非 PASS,拒绝 lock(§25/§28)")
        return 1
    ...
    if static.get("pass") is not True:
        print("[lock-plan] pre-lock static preflight 未 PASS(§26A 前置)"
              "——拒绝 lock")
        return 1
    plan = build_plan_r5(
        baseline_commit=BASELINE_COMMIT_R5,
        ...
        prior_r2_plan_digest=PRIOR_R2_PLAN_DIGEST,
        prior_diag262r2_plan_digest=PRIOR_DIAG262R2_PLAN_DIGEST,
        prior_r4_baseline_commit=PRIOR_R4_BASELINE_COMMIT,
...
def cmd_qualify(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r5_final import (
        run_final_qualification_r5,
    )

    result = run_final_qualification_r5(Path(args.out_dir))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="curriculum261-r5",
        description="Stage 2.6.1 Repair R5:C2 ladder 功效修复与 "
                    "Clean One-Shot Qualification")
