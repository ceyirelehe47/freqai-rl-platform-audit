# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R15:final qualification plan(§26)。

双 strict gate(calibration + holdout 各自独立 PASS;无 pooled 救援)
前置强制;plan 绑定 R15 pack/cue semantic 合同/p_contract/recall
...
from rl_curriculum.curriculum261_r15_gate_topology import (
    ...
    r15_gate_topology_payload,
    r15_overridden_strict_gate_rule_text,
)
...
PLAN_FORMAT_R15 = "cur261-r15-qualification-plan-v1"
...
#: cmd_lock_plan → build_plan_r15(calibration_bundle_hash /
#: holdout_bundle_hash)。R12 的永久 FAIL 根因即 consumer 读取了
#: 'bundle_hash' 而非本字段(§二);R15 禁止同时宽松接受两种键名。
...
#: R15 代码身份(逐模块内容哈希;进入 plan;final 复算比对)。
...
def _code_identity_r15() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    for name in PLAN_CODE_MODULES_R15:
        f = root / name
...
def build_plan_r15(
    ...
    plan: dict[str, Any] = {
        ...
        "code_freeze": {
            ...
            "contract": ("正式 R15 数据开始前全部源码+测试已提交;"
                         "正式数据开始后任何源码变化 -> R15 永久结束"
                         "(§6/§21)"),
        },
        ...
        "prior_r10": {
            ...
            "role": ("R10 design/pack 全部结果仅作 development "
                     "evidence;R15 在全新 namespace 重做机械 design"),
        },
        "prior_r12": {
            ...
            "role": ("R12 统计链全部 PASS 仅作历史记录,不进入 R15;"
                     "R15 在全新 namespace 重做全链,且以 "
                     "R15RealArtifactCliRoundTrip-v1 在 Commit A 前覆盖"
                     "真实落盘 artifact 读写边界"),
        },
        ...
        "preprocessing_v2": {
            ...
            "implementation": "pinned vendor IFreqaiModel."
                              "define_data_pipeline(直接复用;R15 全新"
                              "语料重新资格验证)",
        ...
        "fit_bank_schedule": {
            "calibration": {
                "namespace": "preprocess_fit_calibration_r15",
                "pairs_per_rung": 4, "sides": ["A", "B"],
                "role": "main fit corpus(preprocessor only)"},
            "holdout": {
                "namespace": "preprocess_fit_holdout_r15",
                "pairs_per_rung": 4, "sides": ["A", "B"],
                "role": "independent fit holdout(preprocessor only)"},
            "final": {
                "namespace": "preprocess_fit_qualification_r15",
                "pairs_per_rung": 4, "sides": ["A", "B"],
        ...
        "semantic_corpus_schedule": {
            "design": {
                "namespaces": ["cue_semantic_design_main_r15",
                               "cue_semantic_design_validation_r15"],
                "blocks_per_corpus": 160,
                "ladder": "sentinel(candidate-independent)"},
            "calibration": {
                "namespace": "cue_semantic_calibration_r15",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "holdout": {
                "namespace": "cue_semantic_holdout_r15",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
            "final": {
                "namespace": "cue_semantic_qualification_r15",
                "blocks_per_corpus": 160,
                "ladder": "selected(pack)"},
        },
        "final_sample_counts": {
        ...
        "qualification_bank_schedule": {
            "namespace": "qualification_r15",
            "c2_independent_marginal_namespace":
                "c2_independent_qualification_r15",
            "cue_semantic_namespace": "cue_semantic_qualification_r15",
            "fresh_seed_namespace": "fresh_holdout_r15",
        },
        ...
        "code_identity": _code_identity_r15(),
        # R15 §四-5:gate topology 权威注册表绑定(顶层;final 的
...
def plan_digest_r15(plan: dict[str, Any]) -> str:
    payload = {k: v for k, v in plan.items() if k != "created_utc"}
    return "r15qp-" + hashlib.sha256(json.dumps(
        payload, sort_keys=True, separators=(",", ":"),
        ensure_ascii=False, default=str).encode("utf-8")).hexdigest()


def lock_plan_r15(plan: dict[str, Any]) -> tuple[Path, str]:
    ...
    path = qualification_r15_plan_path()
    if path.is_file():
        raise RuntimeError(
            "R15 qualification plan 已存在;plan lock 后禁止重写(§16.2/"
            "§26:任何修改须新 iteration R15)")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(plan, indent=2, ensure_ascii=False,
                               default=str), encoding="utf-8")
    digest = plan_digest_r15(plan)
    dpath = qualification_r15_digest_path()
    dpath.write_text(digest, encoding="utf-8")
    return path, digest


def load_locked_plan_r15() -> tuple[dict[str, Any], str]:
    path = qualification_r15_plan_path()
    dpath = qualification_r15_digest_path()
    if not path.is_file() or not dpath.is_file():
        raise RuntimeError("R15 qualification plan 未锁定")
    plan = json.loads(path.read_text(encoding="utf-8"))
    digest = plan_digest_r15(plan)
    if dpath.read_text(encoding="utf-8").strip() != digest:
        raise RuntimeError("R15 plan digest 复算不一致(fail closed)")
    return plan, digest


# ---------------------------------------------------- rehearsal plan(§12)
def build_rehearsal_qualification_plan_r15(
    ...
    绕过正式 build_plan_r15 的双 gate 断言(rehearsal 的小样本 gate
    结果不作资格判定),绑定 rehearsal digest 与共享 orchestrator
    身份;正式 plan 仍走 build_plan_r15 + lock_plan_r15。
    """
    plan = {
        "format": "cur261-r15-rehearsal-qualification-plan-v1",
        "iteration": "r15",
        "rehearsal": True,
...
def lock_qualification_plan_r15(
    ...
    path = lock_dir / "qualification_plan_r15.json"
    digest = plan_digest_r15(plan)
    plan = dict(plan)
    ...
    try:
        ...
        os.close(fd)
    (lock_dir / "qualification_plan_digest_r15.txt").write_text(
        digest, encoding="utf-8")
    return path, digest


def load_locked_qualification_plan_r15(
