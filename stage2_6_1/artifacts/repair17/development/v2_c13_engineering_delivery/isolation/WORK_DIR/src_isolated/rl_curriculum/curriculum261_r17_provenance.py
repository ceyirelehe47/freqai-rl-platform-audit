"""GateTopologyReconciliation-v2:R17 gate 拓扑修订的历史来源证明。

在任何 R17 design/calibration/final 数据生成前创建并锁定(§五/§六)。
机器绑定 R13/R14 冲突的全部两侧证据,证明 R17 的拓扑修订是修复
...
结论(写入 payload,机器可核):
- R13/R14 均仍永久 FAIL(不因 R17 修订被追认/撤销/改写);
- R17 修复的是各自 Commit A 中 exposure 前已存在的合同矛盾与
  冻结面内的编排缺陷;
- 不以 R13/R14 observed recall 数值作为规则选择依据;
- R17 不修改 dedicated semantic thresholds 与 160-block 规模。
"""
...
R17_PROVENANCE_FORMAT = "GateTopologyReconciliation-v2"
R17_PROVENANCE_ARTIFACT_NAME = "gate_topology_reconciliation.json"
...
def build_gate_topology_reconciliation() -> dict[str, Any]:
    ...
    r13_binding = {
        ...
        "note": ("R13 不因 R17 修订 gate topology 而被追认、撤销或"
                 "改写为 PASS"),
    }
    ...
    r14_binding = {
        ...
        "remains_permanent_fail": True,
        "note": ("R14 不因 R17 修复隐藏双绑定/编排缺陷而被追认;"
    ...
    conclusions = {
        ...
        "r17_fixes_pre_exposure_contract_contradiction": True,
        "r17_fixes_r14_hidden_dual_binding": True,
        "r17_fixes_r14_uniqueness_fail_open": True,
        "r17_fixes_r14_orchestration_single_source": True,
        ...
        "dedicated_thresholds_note": (
            ...
            "p_contract - 0.02))原样沿用;R17 不改阈值/不降样本量/"
            "不改 160-block 规模"),
    }
    uniqueness = r17_cue_semantic_binding_uniqueness()

    payload = {
        "format": R17_PROVENANCE_FORMAT,
        "iteration": "r17",
        "r17_gate_topology_version": R17_GATE_TOPOLOGY_VERSION,
        "r17_gate_topology_digest": r17_gate_topology_digest(),
    ...
    digest = "r17gtrec-" + hashlib.sha256(json.dumps(
...
def write_gate_topology_reconciliation(out_dir: Path) -> dict[str, Any]:
    ...
    path = out_dir / R17_PROVENANCE_ARTIFACT_NAME
    if path.is_file():
        raise RuntimeError(
            f"{R17_PROVENANCE_ARTIFACT_NAME} 已存在:provenance 证据"
            "一次且仅一次锁定(§五;不得重写/替换)")
