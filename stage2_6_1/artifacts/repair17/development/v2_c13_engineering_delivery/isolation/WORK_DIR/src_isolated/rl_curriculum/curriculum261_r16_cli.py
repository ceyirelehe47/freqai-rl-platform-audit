# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R16 CLI(audit → cue-audit → preplan-smoke →
...
为冻结前工程命令,在任何正式 R16 namespace 访问之前执行)。
...
digest 自引用缺陷修复的正式验收);design plan 在第一条 R16 design/
...
#: R16 启动基线 = R13 Commit B(诚实 FAIL 结果提交);§一 ancestry 语义。
BASELINE_COMMIT_R16 = "14a889c2854571e3ab5245ef51da7c858c83f59b"
#: R14 提交链锚点(R16 baseline = R14 Commit B;§二)。
...
#: producer/consumer 落盘接口 —— R16RealArtifactCliRoundTrip-v1 的
#: 直接动因。
...
#: (curriculum261_r16_historical),不在源码手工转录文件级长 digest。
...
#: TypeError 后按 §18 硬规则诚实 FAIL;design 全部结果仅作 R16
...
#: R16 继承)。R10 design/pack 全部
#: 结果仅作 R16 development evidence。
    ...
    if release_repo:
        ...
        for key, rel in r11_files.items():
            ...
            base_blob = subprocess.run(
                ["git", "rev-parse", f"{R13_COMMIT_B}:{rel}"],
...
def _r12_abort_binding(out_dir: Path) -> dict:
    ...
    binding = {
        ...
        "final_verdict": "FAIL(永久;统计链 cue/global K/design/"
                         ...
                         "R16 输入;R16 必须全新 namespace 重新执行)",
        "fail_reasons": [
            ...
            "永久 FAIL,下一轮 R16)",
            "治理缺口(非硬失败,但为 R16 直接动因):Commit A 前的 "
        ...
        "latent_defect_2_discovered_by_r16": (
            "R16 全仓库接口审计另发现 sealed preflight 证据文件名错配"
            ...
            "R12 未活到该步故未触发;R16 已在 Commit A 前修复"),
        "statistical_chain_status_at_failure": (
            ...
            "holdout 独立 PASS —— 全部仅作历史记录,不进入 R16"),
        "abort_marker_exists": True,
        "abort_marker_blob_matches_baseline": _blob_matches(
...
def _r13_failure_binding(out_dir: Path) -> dict:
    ...
    binding = {
        ...
        "failure_classification": (
            ...
            "c2_semantics_pass;R13 不因 R16 修订被追认/撤销/改写"),
...
def _r8_abort_binding(out_dir: Path) -> dict:
    ...
    if marker is None or not marker.is_file():
        raise RuntimeError(
            ...
            "禁止在 marker 缺失时锁 R16 design plan)")
    data = json.loads(marker.read_text(encoding="utf-8"))
...
def _code_freeze_sha(out_dir: Path) -> str:
    ...
    if not freeze["pass"]:
        raise RuntimeError(
            f"R16 code freeze 校验失败:{freeze}(design plan 必须绑定"
...
def _r14_failure_binding(out_dir: Path, heb: dict) -> dict:
    """§二:R14 历史 binding(编排缺陷/隐藏双绑定/永久 FAIL)。

    从 historical evidence binding(已含 r14_* 检查与治理块)机械
    提取;R14 永久 FAIL 不因 R16 修复被追认。
    """
    checks = heb.get("checks", {})
    r14_keys = sorted(k for k in checks if k.startswith("r14_"))
    binding = {
        "format": "cur261-r16-r14-iteration-failure-binding-v1",
        "iteration": "r14",
        "commit_a": R14_COMMIT_A,
        "commit_b": R14_COMMIT_B,
        "r14_checks": {k: checks.get(k) for k in r14_keys},
        "governance_binding": heb.get("r14_governance_binding"),
        "r14_remains_permanent_fail": True,
        "pass": all(bool(checks.get(k)) for k in r14_keys),
        "binding_gate_in_r16": True,
    }
    _write_json(out_dir, "r14_failure_binding.json", binding)
    return binding


def _r15_failure_binding(out_dir: Path, heb: dict) -> dict:
    """§二:R15 历史 binding(CRLF 启动失败/永久 FAIL)。

    从 historical evidence binding 的 r15_governance_binding 机械
    提取;R15 永久 FAIL 不因 R16 修复被追认。
    """
    gov = heb.get("r15_governance_binding") or {}
    binding = {
        "format": "cur261-r16-r15-iteration-failure-binding-v1",
        "iteration": "r15",
        "commit_a": R15_COMMIT_A,
        "commit_b": R15_COMMIT_B,
        "r15_governance_binding": gov,
        "r15_remains_permanent_fail": True,
        "pass": bool(gov),
        "binding_gate_in_r16": True,
    }
    _write_json(out_dir, "r15_failure_binding.json", binding)
    return binding
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    if freeze_sha:
        from rl_curriculum.curriculum261_r16_dependencies import (
            write_r16_code_freeze,
        )

        freeze_doc = write_r16_code_freeze(
        ...
        existing = verify_r16_code_freeze(Path(args.out_dir))
    ...
    _write_json(out, "baseline_ancestry.json", {
        "format": "cur261-r16-baseline-ancestry-v1",
        "expected_baseline": BASELINE_COMMIT_R16,
    ...
    _dump_txt(out, "preprocessing_v2_contract_digest.txt",
              preprocessing_v2_contract_digest())
    records = generate_fit_bank("preplan_smoke_r16", args.fit_pairs)
    ...
    from rl_curriculum.curriculum261_r16_dependencies import (
        write_dependency_resolution_r16,
    )

    dep = write_dependency_resolution_r16(out)
    from rl_curriculum.curriculum261_r16_delegation import (
        calibration_call_contract_payload,
        delegation_ast_checks_r16,
        live_signature_audit_r16,
    )
    from rl_curriculum.curriculum261_r16_routing import (
        bundle_routing_contract_payload,
    )

    sig_audit = live_signature_audit_r16()
    ast_checks = delegation_ast_checks_r16()
    ...
    freeze_check = verify_r16_code_freeze(out)
    ...
    _write_json(out, "r11_cue_failure_binding.json", {
        "format": "cur261-r16-r11-cue-failure-binding-v1",
        ...
        "classification": "position-wise mirror-count distribution "
                          "failure(t=226 < 264,非 tail;R16 禁止继续"
                          "称为 tail-position failure)",
        "binding_gate_in_r16": False,
    ...
    _write_json(out, "r12_iteration_failure_binding.json", {
        "format": "cur261-r16-r12-iteration-failure-binding-v1",
        "iteration": "r12",
        ...
        "consumer": "cmd_lock_plan → build_plan_r16("
        ...
        "governance_gap": "R12 pre-freeze release rehearsal 使用 "
                          ...
                          "artifact 读取(§三-2;R16 "
                          "R16RealArtifactCliRoundTrip-v1 修复)",
        "binding_gate_in_r16": True,
    ...
    r14_binding = _r14_failure_binding(out, heb)
    r15_binding = _r15_failure_binding(out, heb)
    from rl_curriculum.curriculum261_r16_workflow import (
        validate_r16_workflow,
    )

    workflow_validation = validate_r16_workflow()
    _write_json(out, "r16_workflow_validation.json", workflow_validation)
    from rl_curriculum.curriculum261_r16_provenance import (
    ...
    print(f"[audit] equivalence pass={eq['pass']} "
          ...
          f"r14_binding pass={r14_binding['pass']} "
          f"workflow pass={workflow_validation['pass']} "
    ...
    ok = bool(eq["pass"] and ns.get("pass")
              ...
              and r14_binding["pass"]
              and workflow_validation["pass"]
...
def _official_entrypoint_validation() -> dict:
    ...
    mods = [
        ...
        "rl_curriculum.curriculum261_r16_dependencies",
        "rl_curriculum.curriculum261_r16_noise_replay",
        "rl_curriculum.curriculum261_r16_cue_contract",
        "rl_curriculum.curriculum261_r16_cue_eval",
        "rl_curriculum.curriculum261_r16_global_k",
        "rl_curriculum.curriculum261_r16_historical",
        "rl_curriculum.curriculum261_r16_generation_evidence",
        "rl_curriculum.curriculum261_r16_param_pack",
        "rl_curriculum.curriculum261_r16_preplan",
        "rl_curriculum.curriculum261_r16_design",
        "rl_curriculum.curriculum261_r16_calibration",
        "rl_curriculum.curriculum261_r16_plan",
        "rl_curriculum.curriculum261_r16_preflight",
        "rl_curriculum.curriculum261_r16_final",
        "rl_curriculum.curriculum261_r16_smoke",
        "rl_curriculum.curriculum261_generation_envelope",
        "rl_curriculum.curriculum261_r16_determinism",
        "rl_curriculum.curriculum261_r16_shadow",
        "rl_curriculum.curriculum261_r16_orchestrator",
        "rl_curriculum.curriculum261_r16_gate_topology",
        "rl_curriculum.curriculum261_r16_provenance",
        "rl_curriculum.curriculum261_r16_full_cold",
        "rl_curriculum.curriculum261_r16_workflow",
        "rl_curriculum.curriculum261_r16_fail_closure",
        "rl_curriculum.curriculum261_r16_cli",
    ]
    failed: dict[str, str] = {}
    for m in mods:
        try:
    ...
    from rl_curriculum.curriculum261_r16_cli import main as cli_main
    ...
    subcommands = [
        ...
        "provenance-verify", "workflow-plan", "full-cold",
        "full-cold-reader-check", "fail-closure", "report-read",
        "verify-formal-logs", "commit-b-allowlist",
        "formal-log-record"]
    nal = _no_alternate_loader_check()
    return {
        "format": "cur261-r16-official-entrypoint-v1",
        "entrypoint": "rl_curriculum.curriculum261_r16_cli.main",
...
def _verify_namespace_safe() -> dict:
    try:
        from rl_curriculum.curriculum261_r16_namespaces import (
            verify_r16_namespace_isolation,
        )

        return verify_r16_namespace_isolation()
    except Exception as exc:  # noqa: BLE001
        return {"pass": False, "error": str(exc)[:300]}


def cmd_cue_audit(args: argparse.Namespace) -> int:
    """§12 三路闭合合同审计(在任何 R16 design/semantic data 之前)。

    --rehearsal(R16RealArtifactCliRoundTrip-v1):在 rt_cue_*_r16
    rehearsal-only namespace 上执行正式规模(500+500)审计,不锁定
    audit plan(非正式参数与锁定 plan 合同互斥;正式运行必须无该
    标志),不访问任何 R16 正式 cue/design namespace。
    """
    from rl_curriculum.curriculum261_r16_cue_contract import (
        cue_semantic_contract_digest,
        cue_semantic_contract_payload,
        load_locked_cue_audit_plan_r16,
        lock_cue_audit_plan_r16,
    ...
    if getattr(args, "rehearsal", False):
        report = run_cue_contract_audit(
            out,
            model_namespace="rt_cue_model_r16",
            validation_namespace="rt_cue_validation_r16",
        ...
        print(f"[cue-audit][rehearsal] pass={report['pass']} "
              ...
              f"namespaces=rt_cue_model_r16/rt_cue_validation_r16")
        if not report["pass"]:
            print("[cue-audit][rehearsal] rehearsal 审计 FAIL——"
                  "R16RealArtifactCliRoundTrip-v1 不得通过")
            return 1
        return 0
    # §R16-10:任何正式 R16 cue-audit data 生成前先锁定 audit plan
    ...
    if not (out / "cue_audit_plan.json").is_file():
        _, plan_digest = lock_cue_audit_plan_r16(out)
        print(f"[cue-audit] audit plan locked digest={plan_digest}")
    load_locked_cue_audit_plan_r16(out)
...
def cmd_preplan_smoke(args: argparse.Namespace) -> int:
    ...
    不参与参数选择;只用 preplan_smoke_r16,不用 design/calibration/
    holdout/final namespace)。"""
    from rl_curriculum.curriculum261_c2 import C2_RUNG_PARAMS
    from rl_curriculum.curriculum261_r16_cue_eval import (
        canonical_cue_observations,
        cluster_bootstrap_rate,
        semantic_cue_gate,
    )
    from rl_curriculum.curriculum261_r16_noise_replay import (
    ...
    blocks = [generate_matched_block_with_attempts(
        sentinel, namespace="preplan_smoke_r16", block_index=i)
        for i in range(3)]
    ...
    mini_gate = semantic_cue_gate(
        ...
        min_unique_positive_cues=1, label="preplan_smoke_r16")
    identity = {
        "sentinel": {r: {k: sentinel[r][k]
                         for k in ("alpha_bps", "wick_kappa")}
                     for r in sentinel},
        "sentinel_digest": "r15smoke-" + _hashlib.sha256(json.dumps(
            {r: sorted(sentinel[r].items()) for r in sorted(sentinel)},
            default=str).encode("utf-8")).hexdigest(),
        "n_blocks": len(blocks),
        "namespace": "preplan_smoke_r16",
    ...
    _write_json(out, "preplan_engineering_smoke.json", {
        "format": "cur261-r16-preplan-smoke-v1",
        "identity": identity,
    ...
    from rl_curriculum.curriculum261_r16_preplan import (
        run_preplan_rehearsal_r16,
    )

    rehearsal = run_preplan_rehearsal_r16(out / "preplan")
    ...
    from rl_curriculum.curriculum261_r16_preplan import (
        run_reference_root_cause_diagnosis_r16,
    )

    root_cause = run_reference_root_cause_diagnosis_r16(out / "preplan")
    ...
    from rl_curriculum.curriculum261_r16_rehearsal import (
        run_preplan_full_pipeline_rehearsal_r16,
    )

    full = run_preplan_full_pipeline_rehearsal_r16(out)
    if not full["pass"]:
...
_SUBPROC_LOADER_DESIGN = (
    ...
    "(design_plan_digest_r16, load_locked_design_plan_r16);"
    "import json,sys;"
    "plan,digest=load_locked_design_plan_r16(sys.argv[1]);"
    "recomputed=design_plan_digest_r16(plan);"
    "print(json.dumps({"
...
_SUBPROC_LOADER_QUAL = (
    "import os,sys;"
    "os.environ['CURRICULUM261_R16_STATE_ROOT']=sys.argv[2];"
    "from rl_curriculum.curriculum261_r16_plan import "
    "(load_locked_plan_r16, plan_digest_r16);"
    "import json;"
    "plan,digest=load_locked_plan_r16();"
    "print(json.dumps({"
    "'loaded_digest':digest,"
    "'recomputed':plan_digest_r16(plan),"
    "'iteration':plan['iteration'],"
...
def _no_alternate_loader_check() -> dict:
    ...
    权威模块(r16_design 的 lock/load;r16_namespaces 的路径常量与本
    扫描自身);其余模块不得直接读 plan JSON。"""
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    allowed_design = {"curriculum261_r16_design.py",
                      "curriculum261_r16_namespaces.py",
                      "curriculum261_r16_cli.py",
                      # workflow/fail_closure 仅**声明**文件名
                      # (requires/output/证据清单),不读取 plan JSON
                      "curriculum261_r16_workflow.py",
                      "curriculum261_r16_fail_closure.py"}
    allowed_qual = {"curriculum261_r16_namespaces.py",
    ...
    qual_hits: list[str] = []
    for f in sorted(root.glob("curriculum261_r16_*.py")):
        text = f.read_text(encoding="utf-8")
        if "r16_design_plan.json" in text and f.name not in allowed_design:
            design_hits.append(f.name)
        if ("qualification_plan_r16.json" in text
                and f.name not in allowed_qual):
            qual_hits.append(f.name)
    return {
        "design_plan_readers": ["curriculum261_r16_design.py"],
        "qualification_plan_readers": ["curriculum261_r16_namespaces.py"
                                       " (path fn) + "
                                       "curriculum261_r16_plan.py (loader)"],
...
def cmd_plan_roundtrip(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r16_design import (
        design_plan_digest_r16,
        design_plan_payload_r16,
        lock_design_plan_r16,
    )
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td)
        plan = design_plan_payload_r16(
            baseline_commit=BASELINE_COMMIT_R16,
            ...
            preplan_rehearsal_digest="r15pr-" + "0" * 64,
            r8_abort_evidence={"roundtrip_synth": True},
            cue_audit_plan_digest="r15ap-" + "0" * 64,
        )
        # build payload 的 canonical 快照(逐位一致验证基准)
        pre_digest = design_plan_digest_r16(plan)
        path, digest = lock_design_plan_r16(tmp, plan)
        ...
        checks["payload_bit_identical"] = bool(
            design_plan_digest_r16(reloaded) == pre_digest)
        # 新进程 load(真实生产路径)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        synth_pack = {
            "digest": "r15pk-" + "0" * 64,
            "pack_version": "CurriculumR16MatchedLadderPack-v1",
            "selected_c2_candidate": "c2l_midpoint",
            "selected_block_count": 15,
            "c2_ladder": {r: dict(v) for r, v in
                          C2_LADDER_CANDIDATES_R16["c2l_midpoint"].items()},
            ...
            "recall_floor": 0.93,
            "p_contract": 0.95,
            "cue_contract_audit_digest": "r15ca-" + "0" * 64,
        ...
        if qual_plan is not None:
            ...
            old_dir = os.environ.get("CURRICULUM261_R16_STATE_ROOT")
            os.environ["CURRICULUM261_R16_STATE_ROOT"] = str(tmp)
            try:
                qpath, qdigest = lock_plan_r16(qual_plan)
                checks["qualification_lock_ok"] = bool(
                    qpath.is_file()
                    and qdigest == plan_digest_r16(qual_plan))
                ...
                if old_dir is None:
                    os.environ.pop("CURRICULUM261_R16_STATE_ROOT", None)
                else:
                    os.environ["CURRICULUM261_R16_STATE_ROOT"] = old_dir
    ...
    from rl_curriculum.curriculum261_r16_design import (
        load_locked_design_plan_r16 as _unused_loader,  # noqa: F401
    )
    ...
    result = {
        "format": "cur261-r16-plan-roundtrip-validation-v1",
        "iteration": "r16",
        "checks": checks,
...
def cmd_design_plan_lock(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r16_dependencies import (
        resolve_dependency_identity_r16,
    )
    from rl_curriculum.curriculum261_r16_design import (
        semantic_artifact_identity_r16,
    )
    ...
    if not rehearsal.get("pass"):
        print("[design-plan-lock] preplan rehearsal 未通过——拒绝锁 "
              "plan(§9/§31)")
        return 1
    dep = resolve_dependency_identity_r16()
    if not dep["pass"]:
    ...
    if rt_mode:
        # R16RealArtifactCliRoundTrip-v1:rehearsal cue-audit 不锁定
        ...
        design_namespaces = ("rt_design_matched_main_r16",
                             "rt_design_matched_validation_r16")
        semantic_namespaces = ("rt_semantic_design_main_r16",
                               "rt_semantic_design_validation_r16")
        independent_namespace = "rt_design_independent_r16"
    else:
        audit_plan = load_locked_cue_audit_plan_r16(out)
    ...
    plan = design_plan_payload_r16(
        ...
        artifact_writer_identity=semantic_artifact_identity_r16(),
    ...
    path, digest = lock_design_plan_r16(out, plan)
...
def cmd_design(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r16_design import (
        load_locked_design_plan_r16,
        run_design_stage_r16,
    )

    out = Path(args.out_dir)
    plan, digest = load_locked_design_plan_r16(out)
    selection = run_design_stage_r16(out, plan, digest,
                                    baseline_commit=BASELINE_COMMIT_R16)
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    R16 变革(相对 R9 的手写长流程):
    - 全部评估走共享 orchestrator orchestrate_calibration_stage_r16
    ...
    from rl_curriculum.curriculum261_r16_orchestrator import (
        formal_holdout_profile_r16,
        formal_main_profile_r16,
        orchestrate_calibration_stage_r16,
    )
    from rl_curriculum.curriculum261_r16_param_pack import (
        frozen_parameter_identity_r16,
        r16_override_for,
        verify_r4_inheritance_r16,
    )
    from rl_curriculum.curriculum261_r16_routing import build_routing_r16
    from rl_curriculum.curriculum261_r3_calibration import (
    ...
    try:
        ...
        write_r16_iteration_aborted(
            ...
            f"{str(exc)[:400]});§11 硬规则:R16 永久结束,下一轮必须 "
            "R16 + 全新 namespace")
        raise
...
def _cmd_calibrate_inner(args: argparse.Namespace,
    ...
    require_r16_iteration_active()
    freeze = verify_r16_code_freeze(out)
    if not freeze["pass"]:
        print(f"[calibrate] R16 code freeze 校验失败(fail closed):"
              f"{freeze}")
        return 1
    pack = _pack(out)
    inheritance = verify_r4_inheritance_r16(pack)
    if not inheritance:
        print("[calibrate] R4 inheritance 验证失败;fail closed")
        return 1
    n_blocks = int(pack["selected_block_count"])
    design_digest = (out / "r16_design_plan_digest.txt").read_text(
        encoding="utf-8").strip()
    recall_floor_value = float(pack["recall_floor"])
    rt_mode = bool(getattr(args, "rehearsal", False))
    if rt_mode:
        # R16RealArtifactCliRoundTrip-v1:rt_* rehearsal-only namespace;
        # 正式生成规模;profile 落盘全部 gate artifacts(供 lock-plan /
        # preflight-sealed 真实读取;§四-4)。
        from rl_curriculum.curriculum261_r16_orchestrator import (
            rt_holdout_profile_r16,
            rt_main_profile_r16,
        )

        fit_ns_main = "rt3_fit_main_r16"
        fit_ns_hold = "rt3_fit_holdout_r16"
        profile_main_obj = rt_main_profile_r16()
        profile_holdout_obj = rt_holdout_profile_r16()
        conditioning_eval_ns = "rt3_calibration_main_r16"
        stress_ns = "rt3_stress_r16"
        print("[calibrate][rehearsal] rt_* namespace;正式生成规模;"
              "write_artifacts=True(R16RealArtifactCliRoundTrip-v1)")
    else:
        fit_ns_main = "preprocess_fit_calibration_r16"
        fit_ns_hold = "preprocess_fit_holdout_r16"
        profile_main_obj = formal_main_profile_r16(n_blocks)
        profile_holdout_obj = formal_holdout_profile_r16(n_blocks)
        conditioning_eval_ns = "calibration_r16"
        stress_ns = "stress_r16"

    print(f"[calibrate] fitting main preprocessor ({fit_ns_main})...")
    records_main = generate_fit_bank_r16(fit_ns_main, pack)
    v2_main, manifest_main = fit_preprocessor_v2_from_bank_r16(
        fit_ns_main, pack, records=records_main,
        parameter_pack_identity=pack["digest"])
    routing_main = build_routing_r16("main", v2_main, rt=rt_mode)
    print(f"[calibrate] fitting holdout preprocessor ({fit_ns_hold})...")
    records_hold = generate_fit_bank_r16(fit_ns_hold, pack)
    v2_hold, manifest_hold = fit_preprocessor_v2_from_bank_r16(
        fit_ns_hold, pack, records=records_hold,
        parameter_pack_identity=pack["digest"])
    routing_holdout = build_routing_r16("holdout", v2_hold, rt=rt_mode)
    ...
    stage = orchestrate_calibration_stage_r16(
        out, pack,
        n_blocks=n_blocks,
    ...
    eval_records = [
        generate_pair(f, r, 0, namespace=conditioning_eval_ns,
                      rung_params_override=r16_override_for(f, pack))
    ...
    stress = run_generator_stress_r16(pack, namespace=stress_ns)
    _write_json(out, "generator_stress.json", stress)
    _write_json(out, "frozen_parameter_identity.json",
                frozen_parameter_identity_r16())

    gate = {
        "format": "cur261-r16-robustness-gate-v1",
        "iteration": "r16",
        "rehearsal": rt_mode,
...
def cmd_preflight_static(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r16_preflight import (
        run_prelock_static_preflight_r16,
    )

    result = run_prelock_static_preflight_r16(Path(args.out_dir),
                                             VENDOR_PIN)
...
def cmd_lock_plan(args: argparse.Namespace) -> int:
    ...
    calibration_bundle_hash = read_preprocessor_bundle_hash(
        ...
        consumer_command="r15-cli lock-plan")
    ...
    plan = build_plan_r16(
        ...
        frozen_parameter_identity=frozen_parameter_identity_r16(),
    ...
    path, digest = lock_plan_r16(plan)
...
def cmd_preflight_sealed(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r16_preflight import (
        run_postlock_sealed_preflight_r16,
    )

    att = run_postlock_sealed_preflight_r16(Path(args.out_dir),
                                           VENDOR_PIN)
...
#: R16RealArtifactCliRoundTrip-v1 的 final 阶段 rehearsal profile
#: (§四-4):rt_*_r16 rehearsal-only namespace + 缩小样本量;治理外壳
...
def cmd_qualify(args: argparse.Namespace) -> int:
    """final qualification(一次性;R16 所有权生命周期)。

    repair R16(§6.2):竞争请求(未取得正式会话所有权的第二个
    请求)被 R16OwnershipError 拒绝时,本命令只退出——不 dump
    failure evidence、不写 iteration aborted、不触碰任何正式
    marker/manifest(R15 的无差别 except 把抢锁失败也写成
    qualification crashed 的跨进程污染路径在此关闭)。

    其他异常(final 内部已按 §6.4 封口 crashed 后 re-raise,或
    前置检查失败)在处置前落盘全部逐 attempt invocation
    envelopes,随后写 iteration aborted(§11 硬规则)。

    --rehearsal:以 R16_RT_FINAL_PROFILE 在 rt3_*_r16 namespace +
    缩小规模执行同代码路径;rc 反映执行与 artifact 完整性。
    """
    ...
    rt_mode = bool(getattr(args, "rehearsal", False))
    try:
        result = run_final_qualification_r16(
            out, rehearsal_profile=R16_RT_FINAL_PROFILE if rt_mode
            else None)
    except R16OwnershipError:
        # §6.2 竞争/状态不允许:零正式副作用,直接退出。
        raise
    except Exception as exc:  # noqa: BLE001 —— §11 正式异常处置
        from rl_curriculum.curriculum261_generation_envelope import (
            dump_failure_evidence,
...
def cmd_smoke(args: argparse.Namespace) -> int:
    from rl_curriculum.curriculum261_r16_smoke import run_ppo_smoke_r16
    ...
    try:
        ...
        pass
    smoke = run_ppo_smoke_r16(pack=pack)
...
def cmd_namespace_integrity(args: argparse.Namespace) -> int:
    ...
    print(f"[namespace-integrity] pass={ns.get('pass')} "
          f"namespaces={len(ns.get('r16_namespaces', []))}")
...
def cmd_determinism_matrix(args: argparse.Namespace) -> int:
    ...
    工程阶段命令(在任何正式 R16 namespace 访问之前执行);产物写
    ...
    from rl_curriculum.curriculum261_r16_determinism import (
        audit_generator_mutable_state,
        generation_determinism_gate,
        run_cross_process_determinism_matrix,
    )

    out = qualification_r16_lock_dir() / "determinism"
...
def cmd_shadow_run(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r16_shadow import (
        run_full_scale_shadow_r16,
    )

    base = qualification_r16_lock_dir() / "shadow"
    ...
    summary = run_full_scale_shadow_r16(run_dir, run_tag=str(args.run_tag))
...
def cmd_global_k_reanalysis(args: argparse.Namespace) -> int:
    ...
    - 明确声明:重分析只用于 method development 与回归,不改变 R11
      的 FAIL 结论;禁止依据该结果调整 R16 的 alpha/eligibility/
      randomization count/statistic/tail 定义。
    """
    import math as _math

    from rl_curriculum.curriculum261_r16_global_k import (
    ...
    _write_json(out, "global_k_historical_reanalysis.json", {
        "format": "cur261-r16-global-k-historical-reanalysis-v1",
...
def cmd_release_rehearsal(args: argparse.Namespace) -> int:
    ...
    from rl_curriculum.curriculum261_r16_cue_contract import (
        cue_audit_plan_digest_r16,
        cue_audit_plan_payload_r16,
        load_locked_cue_audit_plan_r16,
        lock_cue_audit_plan_r16,
    )
    from rl_curriculum.curriculum261_r16_namespaces import (
    ...
    with tempfile.TemporaryDirectory(prefix="r16_rehearsal_") as td:
        tdp = Path(td)
        ...
        sections["audit_run1"] = {
            ...
            "freeze_writer_written": (tdp / "r16_code_freeze.json").is_file(),
        }
        # ---- cue-audit plan build/lock/load/verify 路径 ----
        plan_dir = tdp / "cue_plan"
        _, digest1 = lock_cue_audit_plan_r16(plan_dir)
        loaded = load_locked_cue_audit_plan_r16(plan_dir)
        plan_payload = cue_audit_plan_payload_r16()
        expected_digest = cue_audit_plan_digest_r16(plan_payload)
        ...
        # ---- plan 重复锁定必须被拒绝 ----
        try:
            lock_cue_audit_plan_r16(plan_dir)
        ...
        try:
            from rl_curriculum.curriculum261_r16_namespaces import (
                require_r16_iteration_active,
                r16_iteration_aborted as _aborted,
            )
            ...
            abort_written_and_enforced = (
                _aborted()
                and _verify_raises(require_r16_iteration_active))
        finally:
            if old_lock is None:
        ...
        mini_audit = run_cue_contract_audit(
            ...
            model_namespace="preplan_engineering_smoke_r16",
            validation_namespace="preplan_engineering_smoke_r16",
            require_locked_plan=False)
        rplan = design_plan_payload_r16(
            baseline_commit=BASELINE_COMMIT_R16,
            vendor_pin=VENDOR_PIN,
            v2_contract_digest=preprocessing_v2_contract_digest(),
            prior_r2_plan_digest=PRIOR_R2_PLAN_DIGEST,
            ...
            preplan_smoke_identity={
                "sentinel_digest": "r15smoke-" + "0" * 64,
                "cue_contract_digest": "r15cue-" + "0" * 64,
            ...
            cue_audit_plan_digest="r15ap-" + "0" * 64)
        pre_d = design_plan_digest_r16(rplan)
        plan_dir = tdp / "design_plan"
        _, locked_d = lock_design_plan_r16(plan_dir, rplan)
        reloaded_plan, reloaded_d = load_locked_design_plan_r16(
            plan_dir)
        ...
        for _rk in ("design_plan_digest", "locked_utc"):
            ...
            _orig.pop(_rk, None)
        try:
            lock_design_plan_r16(plan_dir, rplan)
        ...
        from rl_curriculum.curriculum261_r16_dependencies import (
            write_r16_code_freeze as _wcf,
        )

        _wcf(cl_dir, code_freeze_sha=head)
        cl_path = write_path_cleanliness_r16(cl_dir, verdict="FAIL")
        cl_doc = json.loads(cl_path.read_text(encoding="utf-8"))
        try:
            write_path_cleanliness_r16(cl_dir, verdict="FAIL")
    ...
    with tempfile.TemporaryDirectory(prefix="r16_rehearsal2_") as td:
        tdp = Path(td)
        audit_args = argparse.Namespace(
            out_dir=str(tdp), fit_pairs=2, code_freeze_sha=head)
    ...
    result = {
        "format": "cur261-r16-pre-freeze-release-rehearsal-v1",
        "head_at_rehearsal": head,
        "monkeypatch_used": False,
        "temp_dirs_only": True,
        "formal_r16_namespace_accessed": False,
        "sections": sections,
        "pass": ok,
    }
...
def write_path_cleanliness_r16(out_dir: Path, *, verdict: str) -> Path:
    ...
    freeze_check = verify_r16_code_freeze(out_dir)
    result = _load(out_dir / "qualification_result.json")
    abort = _load(out_dir / "r16_iteration_aborted.json")
    design_plan_digest_txt = (out_dir / "r16_design_plan_digest.txt")
    # §四-2 修复:R12 版读取无 producer 的 calibration_report_{main,
    # holdout}.json 导致 calibration_state 恒为 absent;R16 读取
    ...
    for doc in (result, abort):
        if isinstance(doc, dict):
            ...
            if failing_gate:
                break
    payload: dict = {
        "format": "cur261-r16-path-cleanliness-v1",
        "verdict": verdict,
        ...
        "abort_artifact_sha256": _sha(
            out_dir / "r16_iteration_aborted.json"),
        ...
        "exposure_state": ("exposed" if qualification_r16_exposed()
                           else "not_exposed"),
        "final_namespace_state": (
            "executed" if isinstance(result, dict) else
            "aborted" if r16_iteration_aborted() else "untouched"),
        "pooled_rescue_used": False,
...
#: 工程 namespace preplan_*/shadow_*/stress_r16/training_r16/
#: fresh_holdout_r16/ppo_smoke_r16/reference_diagnostic_* 不在此列)。
R16_FORMAL_CRITICAL_NAMESPACES: tuple[str, ...] = (
...
def artifact_interface_audit() -> dict[str, Any]:
    ...
    """
    return {
        "format": "cur261-r16-artifact-interface-audit-v1",
        "iteration": "r16",
        "canonical_identity_fields": {
            "preprocessor_bundle_{calibration,holdout}.json":
                ...
                "'bundle_hash' 禁止冒充)",
            "r16_parameter_pack.json": "digest(r15pk-;pack_digest_r16"
                                       " 复算)",
            "r16_design_plan.json": "design_plan_digest(r16dp-)",
            "qualification_plan_r16.json": "plan_digest(r15qp-)",
            "sealed_final_preflight_r16.json": "digest(r15fa-)",
        ...
        "edges": [
            {
                ...
                "r16_closure":
                    ...
                    "accessor(curriculum261_r16_plan.py;§四-1)",
            ...
            },
            {
                ...
                "r12_defect":
                    ...
                    "名称)——潜伏缺陷,R16 修复(§四-2)",
                "r16_closure":
                    "run_postlock_sealed_preflight_r16 证据文件清单"
                    "与真实 producer 对齐",
            ...
            },
            {
                ...
                "r12_defect":
                    ...
                    "supervised_learnability.json——潜伏缺陷,R16 修复",
                "rehearsal_step": "preflight-sealed",
            },
            {
                "edge": "static-preflight→sealed-preflight",
                "producer_command": "preflight-static",
                "artifacts": ["prelock_static_preflight_r16.json"],
                "consumer_command": "preflight-sealed",
                "consumer_fields": ["pass"],
                "r12_defect": "无",
                "rehearsal_step": "preflight-sealed",
            },
            {
                ...
                "artifacts": ["qualification_plan_r16.json",
            ...
            },
            {
                ...
                "artifacts": ["sealed_final_preflight_r16.json",
            ...
            },
            {
                ...
                "artifacts": ["r16_parameter_pack.json",
            ...
            },
            {
                ...
                "(curriculum261_r16_full_cold.read_full_cold_evidence;"
                ...
                "r13_defect": "R13 rehearsal 将本边标记为 smoke "
                              ...
                              "reader 从未实际执行(§九-5)",
                "r16_closure": "R16 rehearsal 第 14 步真实执行 "
            ...
            },
            {
                ...
                "artifacts": ["r16_design_plan.json",
...
def cmd_real_artifact_rehearsal(args: argparse.Namespace) -> int:
    """§十一:R16RealArtifactCliRoundTrip-v1 —— Commit A 前必须 PASS。

    R16 关键变更(工作包 A):rehearsal 与 formal 走**同一权威
    workflow executor**(execute_workflow_chain);步骤 name/order
    完全一致,差异仅 profile(namespace/规模/--rehearsal 旗标/
    --skip-regression/输出文件名)。R14 的缺陷——rehearsal 链有
    preplan-smoke 而 formal runner 没有(两份独立硬编码列表)——
    在结构上不再可能。

    链外动作(§四):pre-freeze provenance-lock(一次且仅一次)+
    尾部 fail-closure-rehearsal 边界探针(declared
    rehearsal_only_tail)。
    """
    import os
    ...
    rh = qualification_r16_lock_dir() / "real_artifact_rehearsal"
    ...
    # workflow 结构校验(Commit A 前必须 PASS;§四)
    validation = validate_r16_workflow()
    if not validation["pass"]:
        print(f"[real-artifact-rehearsal] workflow validation FAIL: "
              f"{validation['problems']}")
        _write_json(rh, "r16_workflow_validation.json", validation)
        return 1

    # 链子进程环境:lock dir 重定向到 rehearsal 隔离目录
    os.environ["CURRICULUM261_R16_STATE_ROOT"] = str(rh)
    chain_env = dict(os.environ)
    chain_env["PYTHONPATH"] = str(proj / "src") + os.pathsep + \
        chain_env.get("PYTHONPATH", "")

    # ---- 链外动作 1:pre-freeze provenance-lock(一次且仅一次)----
    pre_steps: list[dict[str, Any]] = []
    pl = subprocess.run(
        [sys.executable, "-m",
         "rl_curriculum.curriculum261_r16_cli", "provenance-lock",
         "--out-dir", str(rh)],
        cwd=str(proj), env=chain_env, capture_output=True, text=True)
    pre_steps.append({"step": "provenance-lock(pre-freeze,链外)",
                      "returncode": pl.returncode,
                      "stdout_tail": pl.stdout[-400:],
                      "stderr_tail": pl.stderr[-300:]})
    if pl.returncode != 0:
        print(f"[real-artifact-rehearsal] pre-freeze provenance-lock "
              f"rc={pl.returncode};停止")
        _write_json(rh, "real_artifact_cli_roundtrip.json", {
            "format": "R16RealArtifactCliRoundTrip-v1",
            "pass": False,
            "failure": "pre-freeze provenance-lock 失败",
            "pre_steps": pre_steps})
        return 1

    # ---- 权威 chain 执行(与 formal 同一 execute_workflow_chain)----
    plan = build_workflow_plan(
        "rehearsal", out_dir=str(rh), freeze_sha=head,
        manifest_path=str(rh / "r16_formal_log_manifest.jsonl"))
    _write_json(rh, "r16_workflow_plan_rehearsal.json", plan)
    # 与 formal 完全相同的执行路径:subprocess 调 r16_run_step.py
    # chain(其内部调用权威 execute_workflow_chain)
    runner_dir = (release_repo / "stage2_6_1" / "runner"
                  if release_repo else None)
    chain_result_path = rh / "r16_chain_result.json"
    if runner_dir is not None and (
            runner_dir / "r16_run_step.py").is_file():
        cr = subprocess.run(
            [sys.executable, str(runner_dir / "r16_run_step.py"),
             "chain", str(rh / "r16_workflow_plan_rehearsal.json"),
             "--result-out", str(chain_result_path)],
            cwd=str(proj), env=chain_env, capture_output=True, text=True)
        chain = (json.loads(chain_result_path.read_text(
            encoding="utf-8")) if chain_result_path.is_file()
            else {"ok": False, "failed_step": "chain-launcher",
                  "failure_reason": cr.stderr[-500:], "records": []})
        chain_rcs_ok = bool(chain["ok"] and cr.returncode == 0)
    else:
        chain = execute_workflow_chain(
            plan, log_dir=rh / "chain_logs", project_dir=proj)
        chain_rcs_ok = bool(chain["ok"])

    # ---- 链外动作 2:rehearsal 专属尾步(失败路径 writer 边界)----
    tail_rc = 0
    tail_rec: dict[str, Any] = {"step": R16_REHEARSAL_ONLY_TAIL[0]}
    if chain_rcs_ok:
        fc = subprocess.run(
            [sys.executable, "-m",
             "rl_curriculum.curriculum261_r16_cli", "fail-closure",
             "--out-dir", str(rh), "--failed-step", "report-read",
             "--reason",
             "rt-rehearsal-failure-path-boundary-probe"
             "(rehearsal-only;验证 §十四 阶段精确失败路径 writer "
             "真实落盘)",
             "--verdict", "FAIL", "--rehearsal"],
            cwd=str(proj), env=chain_env, capture_output=True, text=True)
        tail_rc = fc.returncode
        tail_rec.update({"returncode": fc.returncode,
                         "stdout_tail": fc.stdout[-400:]})
    else:
        tail_rec.update({"skipped": True,
                         "reason": f"chain 失败于 "
                                   f"{chain['failed_step']}"})

    chain_rcs_ok = chain_rcs_ok and tail_rc == 0

    # ---- 边界字段级探针(独立进程;正式 loader 函数;§十一)----
    probe_env = dict(os.environ)
    probe_env["PYTHONPATH"] = str(proj / "src")
    probe_env["CURRICULUM261_R16_STATE_ROOT"] = str(rh)
    boundary: dict[str, Any] = {}
    boundary["plan_loader"] = _rt_probe(probe_env, (
        "import json,sys;"
        "sys.path.insert(0, sys.argv[2]);"
        "from rl_curriculum.curriculum261_r16_plan import "
        "(load_locked_plan_r16, read_preprocessor_bundle_hash);"
        "from pathlib import Path;"
        "plan,digest=load_locked_plan_r16();"
        "rh=Path(sys.argv[1]);"
    ...
    boundary["qualification_output_reader"] = _rt_probe(probe_env, (
        ...
        "'binding_lineage_pass':r.get('binding_lineage',{}).get("
        "'pass'),"
    ...
    boundary["exposure_one_shot"] = _rt_probe(probe_env, (
        ...
        "from rl_curriculum.curriculum261_r16_namespaces import "
        "qualification_r16_exposed;"
        "print(json.dumps({'exposed':qualification_r16_exposed()}))"),
        str(rh), str(proj / "src"))
    ...
    for ledger_path in sorted(rh.rglob(
        ...
        for line in ledger_path.read_text(encoding="utf-8").splitlines():
            ...
            ns = str(row.get("call", {}).get("namespace",
                     row.get("namespace", "")))
            if ns in R16_FORMAL_CRITICAL_NAMESPACES:
    ...
    # ---- producer/consumer 覆盖(从权威 workflow plan 机械推导)----
    records_by_step = {r["step"]: r for r in chain["records"]}
    ...
    for step in plan["steps"]:
        ...
        outs_ok = all(v is not None for v in
                      (rec.get("output_artifacts") or {}).values())
        covered[step["name"]] = {
            "requires": list(step["requires_artifacts"]),
            "outputs": list(step["output_artifacts"]),
            "step_rc_ok": step_ok,
            "inputs_sha_present": ins_ok,
            "outputs_sha_present": outs_ok,
            "covered": bool(step_ok and ins_ok and outs_ok),
        }
    audit_coverage_ok = bool(covered) and all(
        v["covered"] for v in covered.values())
    ...
    result = {
        ...
        "monkeypatch_used": False,
        "cli_entrypoint":
            "subprocess python -m rl_curriculum.curriculum261_r16_cli"
            "(经权威 execute_workflow_chain)",
        "workflow_executor":
            "curriculum261_r16_workflow.execute_workflow_chain"
            "(rehearsal 与 formal 同一执行器;§十一)",
        "workflow_graph_digest": plan["workflow_graph_digest"],
        "workflow_validation": validation,
        "pre_freeze_steps": pre_steps,
        "rehearsal_only_tail_step": tail_rec,
        "chain_failed_step": chain["failed_step"],
        ...
        "generation_scale": (
            ...
            "independent 20/rung);削减仅 profile 允许项:"
            "--rehearsal 旗标(rt namespace)、audit --fit-pairs 2、"
            "supervised 纯训练(1 model seed + epochs=2)、"
            "full-cold --skip-regression(不实际跑回归套件)、"
            "qualify 阶段缩小样本量(c13 2/rung、matched 4、"
            "semantic 8、independent 2/rung;verdict 不作资格判定)"),
        "boundary_checks": boundary,
        "boundary_checks_pass": boundary_ok,
        "namespace_purity": namespace_purity,
        "workflow_coverage": covered,
        "workflow_coverage_pass": audit_coverage_ok,
    ...
    print(f"[real-artifact-rehearsal] pass={ok} "
          f"chain={chain_rcs_ok} boundary={boundary_ok} "
...
# R16 §四:正式链步骤顺序的唯一来源是
# curriculum261_r16_workflow.R16_WORKFLOW_STEPS(单一权威定义;
# rehearsal/formal runner/verifier/顺序测试全部从它派生)。
# 本模块不得再维护第二份步骤列表(R14 缺陷:两份列表不一致导致
# formal 链缺 preplan-smoke 步)。
R16_FORMAL_LOG_MANIFEST_NAME = "r16_formal_log_manifest.jsonl"
...
def cmd_verify_formal_logs(args: argparse.Namespace) -> int:
    """§十-2:expected 序列由权威 workflow 定义机械派生并比对。"""
    from rl_curriculum.curriculum261_r16_workflow import (
    ...
    try:
        expected = expected_formal_log_prefix(stopped)
    except ValueError as exc:
        print(f"[verify-formal-logs] {exc}")
        return 1
    manifest = Path(args.manifest)
    ...
    for r in records:
        # R16 chain 记录器字段(execute_workflow_chain):env_identity
        # (R14 cmd_formal_log_record 的 env 兼容)+ workflow digest
        for key in ("argv", "cwd", "env_identity", "start_utc",
                    "end_utc", "rc", "stdout_sha256", "stderr_sha256",
                    "workflow_graph_digest"):
            if key not in r and not (key == "env_identity"
                                     and "env" in r):
                problems.append(f"步骤 {r.get('step')} 缺字段 {key}")
        if int(r.get("rc", 1)) != 0:
            problems.append(f"步骤 {r.get('step')} rc!=0")
    result = {"format": "cur261-r16-formal-log-verify-v2",
    ...
    _write_json(out, "r16_formal_log_verification.json", result)
...
def cmd_workflow_plan(args: argparse.Namespace) -> int:
    """§四:按 profile 展开权威 workflow 计划(rehearsal/formal 共用
    同一 execute_workflow_chain 执行器;shell runner 零流程知识)。"""
    from rl_curriculum.curriculum261_r16_workflow import (
        build_workflow_plan,
        validate_r16_workflow,
    )
    ...
    if not v["pass"]:
        print(f"[workflow-plan] validation FAIL: {v['problems']}")
        return 1
    freeze_sha = str(getattr(args, "freeze_sha", "") or "")
    plan = build_workflow_plan(
        args.profile, out_dir=args.out_dir, freeze_sha=freeze_sha)
    out = (Path(args.out_file) if getattr(args, "out_file", None)
           else Path(args.out_dir) / f"r16_workflow_plan_"
                                     f"{args.profile}.json")
    _write_json(out.parent, out.name, plan)
    print(f"[workflow-plan] profile={plan['profile']} "
          f"steps={len(plan['steps'])} "
          f"digest={plan['workflow_graph_digest'][:18]}... -> {out}")
    return 0


def cmd_provenance_verify(args: argparse.Namespace) -> int:
    """§四:Commit A 后 formal 恒执行的 provenance verify(不重新 lock,
    不做'已存在则静默跳过';manifest 记录 verify)。"""
    from rl_curriculum.curriculum261_r16_provenance import (
        verify_gate_topology_reconciliation,
    )
...
def cmd_full_cold(args: argparse.Namespace) -> int:
    """§十三-15:full-cold = reader check + full-cold 回归套件。

    权威 workflow 的 full-cold 步入口(rehearsal --skip-regression
    是 profile 允许的唯一削减;formal 必跑回归套件)。
    """
    from rl_curriculum.curriculum261_r16_full_cold import (
        read_full_cold_evidence,
    )
    ...
    evidence = read_full_cold_evidence(Path(args.artifacts_dir))
    out = Path(args.out_dir) if args.out_dir else Path(args.artifacts_dir)
    regression: dict[str, Any] = {"executed": False, "skipped": True,
                                  "skip_reason": "--skip-regression"
                                  "(rehearsal profile)"}
    if not args.skip_regression:
        proj = Path(__file__).resolve().parents[2]
        runner = proj / "stage2_6_1" / "runner" / "regression_runner.py"
        res = subprocess.run(
            [sys.executable, str(runner), "full-cold"],
            cwd=str(proj), capture_output=True, text=True)
        regression = {"executed": True, "rc": res.returncode,
                      "stdout_tail": res.stdout[-1500:],
                      "stderr_tail": res.stderr[-600:]}
        print(f"[full-cold] regression rc={res.returncode}")
    evidence["regression_fullcold"] = regression
    if regression["executed"]:
        evidence["pass"] = bool(evidence.get("pass")
                                and regression["rc"] == 0)
    evidence["format"] = "cur261-r16-full-cold-step-v1"
    _write_json(out, "full_cold_reader_check.json", evidence)
    reader_ok = bool(all(evidence["reader_checks"].values()))
    print(f"[full-cold] reader_integrity={reader_ok} "
          f"verdict={evidence.get('verdict')} "
          f"regression={regression.get('rc', 'skipped')} "
          f"overall={evidence.get('pass')}")
    if args.skip_regression:
        # rehearsal(--skip-regression):rt verdict 不作资格判定
        # (R14 既定语义;接口覆盖验证由 reader 完整性承载)
        return 0 if reader_ok else 1
...
def cmd_fail_closure(args: argparse.Namespace) -> int:
    ...
    if not args.rehearsal:
        write_r16_iteration_aborted(args.reason)
    # R16 §七:按实际停止阶段机械组装(不再使用固定成功阶段模板)
    from rl_curriculum.curriculum261_r16_fail_closure import (
        build_phase_accurate_fail_closure,
    )

    closure = build_phase_accurate_fail_closure(
        out, failed_step=args.failed_step, reason=args.reason,
        verdict=args.verdict, rehearsal=bool(args.rehearsal))
    cleanliness = write_path_cleanliness_r16(out, verdict=args.verdict)
    closure["cleanliness_path"] = str(cleanliness)
    closure["post_exposure_policy"] = (
        "只读原始 artifacts;不得重生成 qualification_r16 语料;"
        "不得创建新代码;下一轮必须 R16 + 全新 namespace")
    _write_json(out, "r16_fail_closure_summary.json", closure)
    print(f"[fail-closure] rehearsal={bool(args.rehearsal)} "
          f"verdict={args.verdict} failed_step={args.failed_step} "
          f"phase={closure['failure_phase']} "
          f"exposure={closure['exposure_state']}")
    return 0
...
def cmd_report_read(args: argparse.Namespace) -> int:
    ...
    for name, keys in (
        ...
        ("r16_sample_size_selection.json",
...
def commit_b_allowlist_violations(paths: list[str]) -> list[str]:
    ...
    允许:stage2_6_1/artifacts/repair16/**、R16 指定报告 md、
    ...
    def _allowed(path: str) -> bool:
        ...
        if path.startswith("stage2_6_1/report/"):
            return "repair16" in path and path.endswith(".md")
        return path.startswith("stage2_6_1/artifacts/repair16/")
...
def cmd_commit_b_allowlist(args: argparse.Namespace) -> int:
    ...
    result = {
        "format": "cur261-r16-commit-b-allowlist-v1",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="r16-cli")
    ...
    for name in ("cue-audit", "design-plan-lock", "calibrate",
        ...
        p.add_argument("--rehearsal", action="store_true",
                       help="R16RealArtifactCliRoundTrip-v1 rehearsal-"
    ...
    _with_out(sub.add_parser("provenance-verify"))
    wp = sub.add_parser("workflow-plan")
    wp.add_argument("--profile", required=True,
                    choices=("formal", "rehearsal"))
    wp.add_argument("--out-dir", default=None)
    wp.add_argument("--freeze-sha", default="")
    wp.add_argument("--out-file", default=None)
    fc15 = sub.add_parser("full-cold")
    fc15.add_argument("--artifacts-dir", required=True)
    fc15.add_argument("--out-dir", default=None)
    fc15.add_argument("--skip-regression", action="store_true")
    fc15.add_argument("--expect-verdict", default=None)
    ...
    fc.add_argument("--failed-step", required=True,
                    help="失败步骤名(权威 workflow 步骤集内 或 "
                         "'bootstrap'=workflow 之前启动失败;"
                         "phase/证据状态由其机械派生)")
    ...
    handlers = {
        ...
        "provenance-verify": cmd_provenance_verify,
        "workflow-plan": cmd_workflow_plan,
        "full-cold": cmd_full_cold,
