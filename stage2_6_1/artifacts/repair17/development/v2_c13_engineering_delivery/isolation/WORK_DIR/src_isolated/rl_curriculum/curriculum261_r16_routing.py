# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R16:main/holdout/final bundle routing 合同(§9)。
...
R16 修复(§9):
- 显式 R16BundleRouting 数据类:role / fit namespace / 三层 bundle hash,
...
- RoutingLedgerR16 落盘完整路由矩阵(§9.4):corpus → expected fit
...
R16_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "preprocess_fit_calibration_r16",
    "holdout": "preprocess_fit_holdout_r16",
    "final": "preprocess_fit_qualification_r16",
}

#: §9.1 preplan/rehearsal 路由(execution profile 只换 namespace)。
R16_PREPLAN_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "preplan_fit_main_r16",
    "holdout": "preplan_fit_holdout_r16",
    "final": "preplan_fit_main_r16",  # rehearsal final-like 复用 main fit
    "diagnostic": "preplan_fit_main_r16",
}

#: repair R16(工作包 C):full-scale shadow rehearsal 路由(工程、
#: 非正式;两次独立 cold run;不产生任何参数选择数据)。
R16_SHADOW_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "shadow_fit_main_r16",
    "holdout": "shadow_fit_holdout_r16",
    "final": "shadow_fit_main_r16",  # final-like shadow 复用 main fit
}

#: R16RealArtifactCliRoundTrip-v1(§四-4):rt rehearsal 路由(工程、
#: 非正式;rt_*_r16 rehearsal-only namespace;与 shadow 同构但独立
#: 路由类——rt 的 final 用独立 fit namespace,不复用 main fit)。
R16_RT_ROLE_FIT_NAMESPACE: dict[str, str] = {
    "main": "rt3_fit_main_r16",
    "holdout": "rt3_fit_holdout_r16",
    "final": "rt3_fit_qualification_r16",
}

#: 评估 namespace → 期望 role(§9.1;正式 + preplan + shadow)。
R16_EVAL_NAMESPACE_ROLE: dict[str, str] = {
    # ---- 正式 ----
    "calibration_r16": "main",
    "calibration_holdout_r16": "holdout",
    "qualification_r16": "final",
    "c2_independent_calibration_r16": "main",
    "c2_independent_holdout_r16": "holdout",
    "c2_independent_qualification_r16": "final",
    "supervised_main_r16": "main",
    "supervised_holdout_r16": "holdout",
    "cue_semantic_calibration_r16": "main",
    "cue_semantic_holdout_r16": "holdout",
    "cue_semantic_qualification_r16": "final",
    # ---- preplan / rehearsal / diagnostic(非正式)----
    "reference_diagnostic_main_r16": "diagnostic",
    "reference_diagnostic_holdout_r16": "diagnostic",
    "preplan_smoke_r16": "diagnostic",
    "preplan_candidate_eval_r16": "diagnostic",
    "preplan_supervised_main_r16": "main",
    "preplan_supervised_holdout_r16": "holdout",
    "preplan_calibration_main_r16": "main",
    "preplan_calibration_holdout_r16": "holdout",
    "preplan_final_r16": "final",
    # ---- full-scale shadow(工程;R16 工作包 C)----
    "shadow_supervised_main_r16": "main",
    "shadow_supervised_holdout_r16": "holdout",
    "shadow_calibration_main_r16": "main",
    "shadow_calibration_holdout_r16": "holdout",
    "shadow_semantic_main_r16": "main",
    "shadow_semantic_validation_r16": "holdout",
    "shadow_c2_independent_main_r16": "main",
    "shadow_c2_independent_holdout_r16": "holdout",
    "shadow_semantic_final_r16": "final",
    # ---- R16RealArtifactCliRoundTrip-v1(工程;§四-4)----
    "rt3_calibration_main_r16": "main",
    "rt3_calibration_holdout_r16": "holdout",
    "rt3_supervised_main_r16": "main",
    "rt3_supervised_holdout_r16": "holdout",
    "rt3_semantic_main_r16": "main",
    "rt3_semantic_validation_r16": "holdout",
    "rt3_c2_independent_main_r16": "main",
    "rt3_c2_independent_holdout_r16": "holdout",
    "rt3_qualification_r16": "final",
    "rt3_semantic_final_r16": "final",
    "rt_semantic_design_main_r16": "diagnostic",
    "rt_semantic_design_validation_r16": "diagnostic",
}
...
class R16BundleRouting:
    ...
    _v2: Any
    preplan: bool = False
    #: repair R16(工作包 C):shadow 路由类(非正式;工程 rehearsal)
    shadow: bool = False
    #: R16RealArtifactCliRoundTrip-v1(§四-4):rt rehearsal 路由类
    rt: bool = False
    ...
    def bundle(self, *, expected_role: str | None = None,
               ...
               context: str = "",
               ledger: "RoutingLedgerR16 | None" = None) -> Any:
...
def build_routing_r16(
        role: str, v2: Any, *,
        preplan: bool = False,
        shadow: bool = False,
        rt: bool = False,
        expected_bundle_hash: str | None = None) -> R16BundleRouting:
    ...
    repair R16:shadow=True 走 full-scale shadow 路由表(工程)。
    """
    ...
    table = (R16_RT_ROLE_FIT_NAMESPACE if rt
             else R16_SHADOW_ROLE_FIT_NAMESPACE if shadow
             else R16_PREPLAN_ROLE_FIT_NAMESPACE if preplan
             else R16_ROLE_FIT_NAMESPACE)
    ...
    if v2.namespace not in (list(R16_ROLE_FIT_NAMESPACE.values())
                            + list(
                                R16_PREPLAN_ROLE_FIT_NAMESPACE.values())
                            + list(
                                R16_SHADOW_ROLE_FIT_NAMESPACE.values())
                            + list(
                                R16_RT_ROLE_FIT_NAMESPACE.values())):
        raise RoutingContractError(
            f"bundle fit namespace {v2.namespace} 不属于任何 R16 路由表"
            f"(fail closed)")
    if expected_bundle_hash is not None \
...
def require_eval_routing_r16(
        routing: R16BundleRouting, eval_namespace: str, *,
        context: str = "",
        ledger: "RoutingLedgerR16 | None" = None) -> Any:
    ...
    拒绝(§9.3)。repair R16:非正式路由类(preplan/shadow)不得服务
    正式评估 namespace;正式路由不得服务任何非正式 namespace。
    """
    _NONFORMAL_PREFIXES = ("preplan_", "reference_diagnostic",
                           "shadow_", "rt_", "rt3_")
    expected_role = R16_EVAL_NAMESPACE_ROLE.get(eval_namespace)
    if expected_role is None:
        raise RoutingContractError(
            f"评估 namespace {eval_namespace} 不在 R16 权威路由映射中"
            f"(fail closed;context={context})")
    table = (R16_RT_ROLE_FIT_NAMESPACE if routing.rt
             else R16_SHADOW_ROLE_FIT_NAMESPACE if routing.shadow
             else R16_PREPLAN_ROLE_FIT_NAMESPACE if routing.preplan
             else R16_ROLE_FIT_NAMESPACE)
...
def bundle_routing_contract_payload() -> dict[str, Any]:
    ...
    return {
        "format": "cur261-r16-bundle-routing-contract-v1",
        "iteration": "r16",
        "formal_role_fit_namespace": dict(R16_ROLE_FIT_NAMESPACE),
        "preplan_role_fit_namespace": dict(R16_PREPLAN_ROLE_FIT_NAMESPACE),
        "eval_namespace_role": dict(R16_EVAL_NAMESPACE_ROLE),
        "fail_closed_rules": [
