...
def nuisance_column_values(
    *, family: str, family_version: str, params: dict[str, Any],
    ...
    阶段 2.6.0e 工作包 D4:与 BaseMarketGenerator._attach_nuisance_slots
    一致,antithetic_flip 不进入 counter-hash 派生(同一 pair 的
    nuisance 槽位逐位一致)。
    """
    nuisance_params = {k: v for k, v in params.items()
                       if k != "antithetic_flip"}
    base = json.dumps(
        [family, family_version, nuisance_params, int(seed), "_nuisance_salt"],
