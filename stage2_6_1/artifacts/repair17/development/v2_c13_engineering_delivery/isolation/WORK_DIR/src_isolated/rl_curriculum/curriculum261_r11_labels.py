# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R11:PolicyVisibleSupervisedLabel-v1(§8)。
...
def _build_env_for(episode: Any, schema: Any) -> Any:
    ...
    features = select_features_strict(
        episode.df, schema, context="supervised_label_r11")
...
def collect_policy_visible_dataset_r11(
    ...
    for rec in records:
        ...
        for side in ("A", "B"):
            ...
            raw_x64 = ep.df[list(PRODUCTION_FEATURE_COLUMNS)].to_numpy(
                dtype=np.float64)
            # ---- repair R11(B1):逐 episode 对齐记录(全量聚合输入)----
    ...
    # ---- repair R11(B1):alignment 全量聚合(fail closed)----
    # R10 缺陷:返回的 alignment_ok 取自循环最后一个 episode 的
    # ep_alignment_ok —— 早期 episode 失败、末尾成功时总体仍可能为
    # true。R11 合同:所有 episode 均对齐 AND alignment_failures 为空
    ...
    return {
        "format": "cur261-r11-policy-visible-supervised-dataset-v1",
...
def supervised_dataset_identity_r11(
    ...
    return {
        "format": "cur261-r11-supervised-dataset-identity-v1",
