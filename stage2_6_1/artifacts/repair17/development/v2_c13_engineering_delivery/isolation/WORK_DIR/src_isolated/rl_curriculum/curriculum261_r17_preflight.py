# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R17:final preflight(§25)。

A. pre-lock static:imports/vendor/Route C identity/code identity/
   V2 serialize+outer env+PPO smoke(ppo_smoke_r17)/matched generator
...
def _imports_ok_r17() -> dict[str, Any]:
    modules = [
...
def _dependency_resolution_ok_r17() -> dict[str, Any]:
    ...
    return resolve_dependency_identity_r17()


def _code_identity_digest_r17() -> dict[str, Any]:
    from rl_curriculum.curriculum261_r17_plan import _code_identity_r17

    ident = _code_identity_r17()
...
def _matched_generator_probe_r17() -> dict[str, Any]:
    ...
    try:
        ...
        # R17 §7:真实(非 monkeypatch)candidate evaluator 集成探针
        ...
        eval_result = _evaluate_candidate_matched_r17(
            "preflight_sentinel", dict(C2_RUNG_PARAMS),
            "ppo_smoke_r17", thresholds, blocks=blocks, n_blocks=2)
        evaluator_ok = bool(
...
def run_prelock_static_preflight_r17(out_dir: Path,
    ...
    checks: dict[str, Any] = {}
    checks["imports"] = _imports_ok_r17()
    checks["dependency_resolution"] = _dependency_resolution_ok_r17()
    ...
    checks["code_identity"] = _code_identity_digest_r17()
        ...
        try:
            ...
            try:
                probe.open_qualification_window(
                    "r17dp-preflight-probe")
                try:
                    probe.open_qualification_window(
                        "r17dp-preflight-probe-2")
    ...
    result = {
        "format": "cur261-r17-prelock-static-preflight-v1",
        "iteration": "r17",
        "executed_utc": datetime.now(timezone.utc).isoformat(
            timespec="seconds"),
        "namespaces_touched": ["ppo_smoke_r17"],
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "prelock_static_preflight_r17.json").write_text(
...
def run_postlock_sealed_preflight_r17(out_dir: Path,
                                     vendor_pin: str,
                                     ) -> dict[str, Any]:
    ...
    checks: dict[str, Any] = {}
    plan, plan_digest = load_locked_plan_r17()
    checks["plan_digest_recompute"] = True
    checks["plan_iteration_r17"] = bool(plan.get("iteration") == "r17")
    ...
    checks["r4_inheritance_verified"] = verify_r4_inheritance_r17(pack)
    ...
    for name in ("preprocessing_v2_requalification.json",
                 ...
                 "prelock_static_preflight_r17.json"):
        path = out / name
        if path.is_file():
    ...
    checks["final_runner_dependencies"] = _imports_ok_r17()["pass"]
    checks["plan_path_resolves"] = bool(
        qualification_r17_plan_path().is_file()
        and qualification_r17_digest_path().is_file()
        and r17_parameter_pack_path().is_file())
    ...
    attestation = {
        "format": "cur261-r17-sealed-final-preflight-v1",
        "iteration": "r17",
        "pass": att_pass,
...
def write_rehearsal_sealed_preflight_r17(
    ...
    (out_dir / R17_SEALED_PREFLIGHT_FILENAME).write_text(
        json.dumps(att, indent=1, ensure_ascii=False), encoding="utf-8")
    (out_dir / R17_SEALED_PREFLIGHT_DIGEST_FILENAME).write_text(
...
def verify_sealed_attestation(out_dir: Path) -> dict[str, Any]:
    out_dir = Path(out_dir)
    path = out_dir / "sealed_final_preflight_r17.json"
    if not path.is_file():
    ...
    try:
        ...
        digest_ok = False
    digest_file = out_dir / "sealed_final_preflight_r17_digest.txt"
