# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R16:生成确定性与 mutable state 审计(工作包 A)。
...
#: 矩阵的 R16 目标调用(stress_r16 诊断 namespace;c3/D0 覆盖
#: R10 失败形态;pair 3 任意但固定)。
R16_MATRIX_TARGET = {
    "iteration": "r16",
    "namespace": "stress_r16",
    "family": "c3_cost",
    "rung": "D0",
    "pair_index": 3,
}

_PROBE_ARGV_MARKER = "--cur261-r16-determinism-probe"
...
def audit_generator_mutable_state(out_dir: Path) -> dict[str, Any]:
    ...
    target0 = run_target_call(dict(R16_MATRIX_TARGET))
    findings: list[str] = []
    ...
    def _check_phase(name: str) -> None:
        ...
        s1 = {f: stable_digest(
            ...
            .generator_state_digest(spec.generator), "g-")
            for f, spec in sorted(family_specs().items())}
        t1 = run_target_call(dict(R16_MATRIX_TARGET))
    ...
    result = {
        "format": "cur261-r16-generator-state-mutation-audit-v1",
        "iteration": "r16",
...
_PROBE_PRELUDES: dict[str, str] = {
    ...
    "c1_generation":
        ...
        "    generate_pair('c1_opportunity', r, 0, namespace='stress_r16')",
    "c2_generation":
        "from rl_curriculum.curriculum261_pairs import generate_pair\n"
        "for r in ('D0','D1','D2','D3'):\n"
        "    generate_pair('c2_context', r, 0, namespace='stress_r16')",
    ...
    "preprocessing_battery":
        ...
        "recs = [generate_pair(f, r, i, namespace='stress_r16')\n"
...
def _probe_main(argv: list[str]) -> int:
    ...
    ap.add_argument("--target", default="r16",
                    choices=("r16", "r10"))
...
def run_cross_process_determinism_matrix(out_dir: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        base_res = _run_probe(tmp, "baseline", target="r16")
        ...
        _add("10_main_holdout_bundle_first",
             _run_probe(tmp, "s10", target="r16",
                        prelude="main_holdout_bundle_flow"),
             base, "预先执行 main/holdout 双 bundle fit 流程")
        # 11. 同一进程重复两次
        s11 = _run_probe(tmp, "s11", target="r16", repeat_twice=True)
        scenarios.append({
        ...
        _add("12_two_cold_processes",
             _run_probe(tmp, "s12", target="r16"),
             base, "基线冷进程 vs 第二个独立冷进程")
        # 13. 不同线程环境变量
        _add("13_thread_env_variant",
             _run_probe(tmp, "s13", target="r16",
    ...
    result = {
        "format": "cur261-r16-cross-process-determinism-matrix-v1",
        "iteration": "r16",
        "r16_target": dict(R16_MATRIX_TARGET),
...
def generation_determinism_gate(out_dir: Path) -> dict[str, Any]:
    ...
    # envelope 完整性与可重放性:现场重放 stress_r16 目标,对每个
    # attempt envelope 做 digest/seed/roundtrip 检查 + 同进程重放一致
    family = R16_MATRIX_TARGET["family"]
    rp = _rung_params_for(family, R16_MATRIX_TARGET["rung"])
    rec = EnvelopeRecorder(
        iteration="r16", namespace=R16_MATRIX_TARGET["namespace"],
        family=family, rung=R16_MATRIX_TARGET["rung"],
        pair_index=R16_MATRIX_TARGET["pair_index"], rung_params=rp)
    error: str | None = None
    try:
        generate_pair_with_attempts(
            family_specs()[family].generator, rp,
            namespace=R16_MATRIX_TARGET["namespace"], family=family,
            rung=R16_MATRIX_TARGET["rung"],
            pair_index=R16_MATRIX_TARGET["pair_index"],
            structural_validator=pair_acceptance_contract(family),
            recorder=rec)
    ...
    result = {
        "format": "cur261-r16-generation-determinism-contract-v1",
        "iteration": "r16",
        "contract": (
