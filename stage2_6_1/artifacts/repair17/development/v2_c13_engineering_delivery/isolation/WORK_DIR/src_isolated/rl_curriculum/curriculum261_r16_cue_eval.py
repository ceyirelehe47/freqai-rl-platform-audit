# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R16:cluster-aware cue 语义评估器(unique event)。
...
- §14/§15 dedicated semantic corpus(160 matched blocks/corpus,
  ...
  R16 design FAIL,不进入 candidate 设计;
...
R16_CUE_BOOTSTRAP_RESAMPLES = 20000
R16_CUE_BOOTSTRAP_SEED = 20261203
...
def canonical_cue_observations(
    ...
    cue_table_digest = "r15ct-" + hashlib.sha256(
...
def cluster_bootstrap_rate(
        ...
        n_boot: int = R16_CUE_BOOTSTRAP_RESAMPLES,
        seed: int = R16_CUE_BOOTSTRAP_SEED,
) -> dict[str, Any]:
...
def semantic_cue_gate(
    ...
    - tail-position 诊断(报告,不作独立 gate——audit 阶段已专项
      闭合)。
    任一 FAIL => R16 design FAIL(不进入 candidate 设计)。
    """
    ...
    return {
        "format": "cur261-r16-semantic-cue-gate-v1",
        "label": label,
...
def candidate_cue_semantics(
    ...
    return {
        "format": "cur261-r16-candidate-cue-semantics-v1",
...
def cue_semantic_rule_identity() -> str:
    """R16 cue 语义 gate 规则身份(冻结;plan/pack 绑定)。"""
    payload = {
        ...
        "bootstrap_resamples": R16_CUE_BOOTSTRAP_RESAMPLES,
        "bootstrap_seed": R16_CUE_BOOTSTRAP_SEED,
    ...
    return "r15csg-" + hashlib.sha256(blob.encode("utf-8")).hexdigest()
...
def independent_cue_semantics(
    ...
    R16 变更(GateTopologyReconciliation-v2;修复 R14 隐藏双绑定):
    返回结构拆为 structural(binding:仅 canonical consistency)
    与 cue_point_diagnostics(diagnostic_only:point recall/
    noncue FP UCB/cue precision/payoff false-cue 四类点指标)。
    顶层 pass = structural pass——independent marginal guard 的
    binding 职责不含任何 cue 点指标;四类 cue rate metric 的
    唯一正式 binding source = dedicated 160-block semantic corpus。
    ...
    for rung in covered_rungs2:
        ...
        per_rung[rung] = {"sides": per_side, "diagnostic_only": True}
    # ---- R16 §五:structural(binding)与 cue point diagnostics 分离 ----
    # R14 缺陷(隐藏双绑定)修复:R14 把 point recall >= 0.90 与
    # noncue FP UCB <= 0.01 AND 进本函数 pass,该 pass 又被 AND 进
    # marginal guard.pass → c2_independent_marginal_pass → final
    # verdict——形成 dedicated 之外的第二个传递性 cue binding
    # source。R16 权威语义:independent marginal 的 binding 职责仅
    # 结构完整性(canonical consistency);四类 cue 点指标在独立语料
    # 上全部 diagnostic_only(verdict-neutral),正式 binding 唯一
    # source = dedicated 160-block semantic corpus。
    structural_checks = {
    ...
    return {
        "format": "cur261-r16-independent-cue-semantics-v2",
        "candidate": candidate_id,
        ...
        "structural": {
            ...
            "binding_leaf_checks": sorted(structural_checks),
            "pass": structural_pass,
            "note": (
                "R16 §五:independent marginal 的 binding 职责仅结构"
                "完整性;binding_leaf_checks 是进入 binding pass 的"
                "全部原子叶子(gate lineage audit 的自报来源)"),
        },
        "cue_point_diagnostics": {
            ...
            "diagnostic_leaf_checks": sorted(diagnostic_checks),
            "point_recall": float(recall["point"]),
            "absolute_minimum_recall": ABSOLUTE_MINIMUM_RECALL,
            "noncue_fp_ucb": float(noncue["bound"]),
            "noncue_fp_ucb_max": C2_NON_CUE_FALSE_POSITIVE_MAX,
            "per_rung": per_rung,
            "pass": bool(all(diagnostic_checks.values())),
            "note": (
                "point recall ≥ 0.90 与 noncue FP UCB ≤ 0.01 阈值"
                "保留为独立语料诊断对照(R14 曾将其 AND 进 binding "
                "pass——R16 永久移除);诊断 FAIL 不得改变 independent "
                "marginal binding result 与 final verdict"),
        },
        # 顶层 pass = structural(binding 语义)。消费方
        # (c2_independent_marginal_guard_r16)不得读取任何 cue
        # 点指标进入 guard.pass。
        "pass": structural_pass,
    }
