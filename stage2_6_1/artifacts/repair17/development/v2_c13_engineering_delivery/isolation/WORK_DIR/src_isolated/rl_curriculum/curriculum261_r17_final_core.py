"""R17 final 领域核心(机械转换自 r15_final 的 core 部分)。

治理外壳(run_final_qualification_r17:会话/exposure/授权/
终态)在 curriculum261_r17_final.py;本文件只含
execute_final_core_r17 与辅助函数,不得包含任何 exposure/
...
FINAL_FORMAT_R17 = "cur261-r15-final-qualification-v1"
...
from rl_curriculum.curriculum261_r17_orchestrator import (
    ...
    R17_SUPERVISED_MODEL_SEEDS,
)


def qualification_r17_lock_dir_default() -> Path:
    from rl_curriculum.curriculum261_r17_registry import (
...
def _fresh_seed_validity_r17(n_checks: int = 10, *,
                             final_namespace: str = "qualification_r17",
                             holdout_namespace: str
                             = "fresh_holdout_r17") -> dict[str, Any]:
    """fresh_holdout 与 final qualification 的 seed 不相交验证。

    R17(§3.1 最小适配):namespace 参数化以便 R17 final 以
    qualification_r17/fresh_holdout_r17 复用同一检查;默认值保持
    R17 行为不变。
    """
...
def execute_final_core_r17(
        ...
        fresh_seed_final_namespace: str = "qualification_r17",
        fresh_seed_holdout_namespace: str = "fresh_holdout_r17") -> dict[str, Any]:
    """final 主流程(正式与 rehearsal/shadow 共享同一执行核心;§12.4)。

    正式:run_final_qualification_r17(exposure/plan/freeze 治理外壳)
    以 qualification_r17 全量参数调用;
    rehearsal:preplan 以 preplan_final_r17 + 微样本参数 + 临时
    exposure 目录调用(§12.2 final-like runner);
    shadow:full-scale shadow(工作包 C)以 shadow_fit_main_r17 +
    ...
    *_override 参数仅供 shadow(工程覆盖;正式路径不传,保持
    ...
    - reference equivalence 走 canonical 合同(reference_equivalence_
      run_r17:canonical vs scaled 逐位 + legacy 差异 + 0 unexplained),
    ...
    with sink:
        return _execute_final_core_inner_r17(
...
def _build_gate_evidence_r17(
    ...
    (binding lineage,§六)。
    """
    from rl_curriculum.curriculum261_r17_gate_topology import (
        r17_binding_status,
    )
    detail_sources = _detail_sources_r17(
    ...
    for name, value in checks.items():
        if not isinstance(value, bool):
            continue
        status = r17_binding_status(name)
    ...
    gates["c2_matched_cue_point_diagnostics"] = {
        ...
        "note": "诊断失败不得改变 R17 verdict(R17 §四-3)",
    }
    failed_binding = sorted(
        k for k, v in checks.items()
        if isinstance(v, bool) and not v
        and r17_binding_status(k)["binding"])
    return {
        ...
        "post_exposure_policy": (
            ...
            "qualification_r17 语料(api 守卫 fail closed)"),
    }


def _execute_final_core_inner_r17(
        out_dir: Path, plan: dict[str, Any], pack: dict[str, Any], *,
        digest: str = "", started: str | None = None,
        profile_name: str = "formal_final",
        final_namespace: str = "qualification_r17",
        ...
        fresh_seed_final_namespace: str = "qualification_r17",
        fresh_seed_holdout_namespace: str = "fresh_holdout_r17") -> dict[str, Any]:
    """execute_final_core_r17 的执行核心(envelope sink 内)。"""
    kappa = float(plan["statistics_rule"]["kappa"]) \
        if "statistics_rule" in plan else 1.5
    if c2_blocks is None:
        c2_blocks = int(plan["final_sample_counts"]["c2_matched_blocks"])
    ...
    final_records = generate_fit_bank_r17(fit_namespace, pack)
    final_v2, final_manifest = fit_preprocessor_v2_from_bank_r17(
        fit_namespace, pack, records=final_records,
        parameter_pack_identity=pack["digest"])
    routing_final = build_routing_r17(
    ...
    space_validation = validate_observation_space_v2(
        ...
        [int(r.episodes[s].spec.seed) for r in contain_records
         for s in ("A", "B")], context="final_fit_space_v2_r17")
    ...
    c2_indep = run_c2_independent_corpus_r17(
        ...
        or ("c2_independent_qualification_r17" if not rehearsal
            else final_namespace),
        pairs_per_rung=int(independent_pairs_per_rung))
    c2_conditions = c2_matched_conditions_r17(c2_matched, pack)
    c2_marginal = c2_independent_marginal_guard_r17(
        c2_indep, pack, recall_floor_value)
    semantic_ns = semantic_namespace_override or (
        "cue_semantic_qualification_r17" if not rehearsal
        else "preplan_semantic_main_r17")
    ...
    semantic = run_c2_semantic_corpus_r17(
        ...
        artifact_name=semantic_artifact_filename_r17(semantic_ns))
    ...
    reference_report = reference_equivalence_run_r17(
    ...
    if not rehearsal or shadow or rt:
        causality = {
            ...
            "reference_causality": [
                check_reference_causality(
                    f, r17_family_rung_params(f, pack)["D2"],
                    thresholds[f])
                for f in CURRICULUM261_FAMILIES],
        }
    repro = []
    if not rehearsal or shadow or rt:
        ...
        repro_pack_override = []
        for family in CURRICULUM261_FAMILIES:
            ov = r17_override_for(family, pack) or {}
        ...
        c2_ladder = r17_family_rung_params(FAMILY_C2, pack)
    ...
    fresh = (_fresh_seed_validity_r17(
    ...
    cond_records = generate_fit_bank_r17(
        conditioning_fit_namespace or "preprocess_fit_calibration_r17",
    ...
    supervised_ns = supervised_namespace_override or (
        ...
        else "preplan_supervised_main_r17")
    supervised = supervised_learnability_run_r17(
        final_v2, pack, namespace=supervised_ns,
        pairs_per_rung=(supervised_pairs_per_rung_override
                        or (CALIBRATION_PAIRS_PER_RUNG_R17
    ...
    semantics = run_c2_diagnostics_r17(
        [rec for blk in c2_matched["blocks"]
         for rec in blk.pair_records.values()])
    scrambled = scrambled_gap_control(c2_matched["block_table"])

    # R17 gate topology(GateTopologyReconciliation-v1):matched corpus
    ...
    matched_point_diag = {
        ...
        "note": ("R6 点估计分离检查继续执行并报告;诊断失败不得改变 "
                 "R17 verdict(R17 §四-3;R13 曾把该项绑定为 "
                 "c2_semantics_pass 并 FAIL——拓扑冲突已由 "
                 "GateTopologyReconciliation-v1 机械证明)"),
    }
    from rl_curriculum.curriculum261_r17_gate_topology import (
        R17_GATE_TOPOLOGY_VERSION,
        r17_binding_lineage,
        r17_binding_status,
        r17_cue_semantic_binding_uniqueness,
        r17_gate_topology_digest,
    )
    plan_topology_digest = plan.get("gate_topology_digest") if isinstance(
        plan, dict) else None
    topology_digest_ok = bool(
        plan_topology_digest == r17_gate_topology_digest())
    ...
    # R17 §六:binding lineage audit(声明 leaf 与真实 aggregator
    # 自报一致;cue metric binding 强制自报;fail closed)。
    binding_lineage = r17_binding_lineage(_detail_sources_r17(
        conditions=conditions, semantic=semantic, semantics=semantics,
    ...
    checks: dict[str, Any] = {
        ...
        # R17 拆分:R13 的 c2_semantics_pass(三诊断 AND,含点估计
        ...
        "gate_topology_digest_consistent": topology_digest_ok,
        # R17 §六:binding lineage(传递闭包 leaf 声明与真实
    ...
    result = {
        ...
        # R17 §五:independent corpus cue 点指标诊断(diagnostic_only;
        # verdict_neutral——R14 隐藏双绑定修复的可审计面)
        "independent_cue_point_diagnostics": c2_marginal.get(
            "independent_cue_point_diagnostics"),
        # R17 gate topology 声明(单一权威注册表来源;v2)
        "gate_topology": {
            "version": R17_GATE_TOPOLOGY_VERSION,
            "digest": r17_gate_topology_digest(),
            ...
            "note": ("binding status 唯一来源 = "
                     "curriculum261_r17_gate_topology.R17_GATE_REGISTRY;"
                     "本 artifact 的每个 check 的 binding 状态见 "
                     "gate_evidence"),
        },
        # R17 §六:binding lineage audit 结果(逐 binding check 的
        # declared/actual leaf 比对)
        "binding_lineage": binding_lineage,
        # R17 §八:matched corpus 点估计诊断(diagnostic_only;
        # 诊断失败不改变 verdict——R13 败于此处)
        "c2_matched_cue_point_diagnostics": matched_point_diag,
        # R17 §八:每个 gate 的详细证据(一次性保存;final FAIL 后
        # 诊断只读本 artifact 与 qualification_raw.json,不得重生成)
        "gate_evidence": _build_gate_evidence_r17(
    ...
    # R17 §三精神:单一 canonical 键 preprocessor_bundle_hash(R13 的
