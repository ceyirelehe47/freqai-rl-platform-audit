# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R13:preplan full pipeline shadow rehearsal(§12)。

与正式执行共享同一 orchestration:
- calibration 阶段:orchestrate_calibration_stage_r13(rehearsal_*
  profiles;仅样本量与 namespace 与正式不同);
- final 阶段:r13_final.execute_final_core_r13(rehearsal profile;
  临时 exposure 目录);
...
from rl_curriculum.curriculum261_r13_orchestrator import (
    ...
    rehearsal_holdout_profile_r13,
    rehearsal_main_profile_r13,
)
from rl_curriculum.curriculum261_r13_routing import (
    R13_PREPLAN_ROLE_FIT_NAMESPACE,
    build_routing_r13,
    bundle_routing_contract_payload,
)

REHEARSAL_NAMESPACE_TAG = "r13_preplan_full_pipeline_rehearsal_v1"
...
def _rehearsal_pack() -> dict[str, Any]:
    ...
    pack = ladder_pack_payload_r13(
        ...
        design_plan_digest="r13dp-rehearsal-sentinel",
        matched_contract_identity="rehearsal-sentinel",
        block_integrity_identity="rehearsal-sentinel",
        cue_semantic_contract_digest="r13cue-rehearsal-sentinel",
        cue_semantic_rule_identity="rehearsal-sentinel",
        cue_audit_digest="r13ca-rehearsal-sentinel",
        p_contract=0.9504,
...
def run_preplan_full_pipeline_rehearsal_r13(
    ...
    from rl_curriculum.curriculum261_r13_param_pack import r13_override_for
    ...
    profile_main = rehearsal_main_profile_r13()
    profile_holdout = rehearsal_holdout_profile_r13()
    struct: dict[str, Any] = {
    ...
    for role, fit_ns, pairs in (
            ("main", R13_PREPLAN_ROLE_FIT_NAMESPACE["main"], 2),
            ("holdout", R13_PREPLAN_ROLE_FIT_NAMESPACE["holdout"], 2)):
        records = generate_fit_bank_r13(fit_ns, pack, pairs_per_rung=pairs)
        v2, manifest = fit_preprocessor_v2_from_bank_r13(
            fit_ns, pack, records=records,
            parameter_pack_identity=pack["digest"])
        routing = build_routing_r13(role, v2, preplan=True)
    ...
    stage = orchestrate_calibration_stage_r13(
        ...
        design_digest="r13dp-rehearsal-sentinel",
    ...
    try:
        from rl_curriculum.curriculum261_r13_plan import (
            build_rehearsal_qualification_plan_r13,
            lock_qualification_plan_r13,
            load_locked_qualification_plan_r13,
        )

        tmp_plan_dir = out_dir / "preplan_plan_temp"
        tmp_plan_dir.mkdir(parents=True, exist_ok=True)
        plan, plan_digest = build_rehearsal_qualification_plan_r13(
            pack=pack, stage_summary=stage,
            final_namespace="preplan_final_r13",
            fit_namespace="preplan_fit_main_r13")
        lock_qualification_plan_r13(tmp_plan_dir, plan)
        loaded, digest2 = load_locked_qualification_plan_r13(tmp_plan_dir)
        plan_info = {
            "locked": True,
            "digest": plan_digest,
            ...
            == json.dumps({**plan, "plan_digest": plan_digest},
                          sort_keys=True),
            "namespace": "preplan_final_r13(临时目录;零正式 ns)",
        }
    ...
    try:
        from rl_curriculum.curriculum261_r13_preflight import (
            write_rehearsal_sealed_preflight_r13,
        )

        att = write_rehearsal_sealed_preflight_r13(
    ...
    try:
        from rl_curriculum.curriculum261_r13_final import (
            execute_final_core_r13,
        )

        final_dir = out_dir / "preplan_final_temp"
        final_dir.mkdir(parents=True, exist_ok=True)
        final_result = execute_final_core_r13(
            final_dir, plan, pack,
        ...
        final_info = {
            ...
            "exposure_terminal_status": final_result.get(
                "exposure_status"),
            "namespaces": ["preplan_final_r13"],
        }
        struct["namespaces_touched"].append("preplan_final_r13")
    except Exception as exc:  # noqa: BLE001
    ...
    struct["rehearsal_digest"] = "r13rh-" + hashlib.sha256(
