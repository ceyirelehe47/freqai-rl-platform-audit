"""R17 权威 cue semantic gate topology 注册表 v2(单一权威来源)。
...
R17 权威语义(本模块是唯一来源;design plan / qualification plan /
...
2. matched corpus 上的 R6 点估计分离检查:diagnostic_only=True /
   binding_gate=False(R14 已降级,R17 继承)。
...
3. independent marginal corpus 上的 cue 点指标(point recall /
   ...
   failed_binding_checks(R17 §五;对 R14 隐藏绑定的修复)。
...
R17_GATE_TOPOLOGY_VERSION = "GateTopologyReconciliation-v2"

#: matched corpus 点估计诊断保留的 R6 冻结阈值(仅诊断对照)。
R17_MATCHED_POINT_DIAGNOSTIC_THRESHOLDS = {
    "cue_recall_min": 0.95,
...
#: 不含任何 cue 点指标)。
R17_INDEPENDENT_MARGINAL_RESPONSIBILITIES = (
    "marginal_ordering",
    "d3_positive",
...
#: c2_independent_marginal_guard_r17 的 guard["binding_leaf_checks"]
#: 自报逐字一致;v2 修复点:不含 point_recall/noncue UCB)。
R17_INDEPENDENT_MARGINAL_LEAVES: tuple[str, ...] = (
    "context_observability",
    "d3_mean_positive",
    "density_pass",
...
#: run_c2_semantic_corpus_r17 的 result["binding_leaf_checks"]
#: 自报逐字一致)。
R17_DEDICATED_SEMANTIC_LEAVES: tuple[str, ...] = (
...
#: c2_matched_conditions_r17 的 binding_leaf_checks 自报一致)。
R17_MATCHED_STRICT_LEAVES: tuple[str, ...] = (
...
def _binding(name: str, source: str, rule: str,
    ...
    if not entry["leaf_metrics"]:
        raise RuntimeError(
            ...
            "(R17 §六-3:缺 lineage 声明即 fail closed)")
    entry.update(extra)
    return entry
...
R17_GATE_REGISTRY: dict[str, dict[str, Any]] = {
    # --- 工程完整性(binding) ---
    ...
    "c2_matched_strict_pass": _binding(
        ...
        leaf_metrics=R17_MATCHED_STRICT_LEAVES,
        responsibilities=list(R17_MATCHED_CORPUS_RESPONSIBILITIES)),
    "c2_independent_marginal_pass": _binding(
        ...
        "不进入本 pass(R17 §五;R14 隐藏双绑定修复)",
        leaf_metrics=R17_INDEPENDENT_MARGINAL_LEAVES,
        metric_scope=(),
    ...
    "c2_dedicated_semantic_corpus_pass": _binding(
        ...
        leaf_metrics=R17_DEDICATED_SEMANTIC_LEAVES,
        metric_scope=R17_CUE_METRICS,
    ...
    "gate_topology_digest_consistent": _binding(
        "gate_topology_digest_consistent", "r17_gate_registry",
    ...
    "c2_matched_cue_point_diagnostics": _diagnostic(
        ...
        "改变 R17 verdict(R17 §四-3)",
        thresholds=dict(R17_MATCHED_POINT_DIAGNOSTIC_THRESHOLDS),
        historical_note=(
    ...
    "independent_cue_point_diagnostics": _diagnostic(
        ...
        "(R17 §五;R14 曾将其 AND 进 marginal guard.pass 构成"
        "隐藏双绑定——R17 永久移除)",
        metric_scope=R17_CUE_METRICS,
        thresholds={
...
def _validate_registry() -> None:
    ...
    for name, entry in R17_GATE_REGISTRY.items():
        if entry["binding"]:
            if "metric_scope" not in entry:
                raise RuntimeError(
                    f"binding 条目 '{name}' 缺 metric_scope 声明"
                    "(R17 fail-closed:R14 的 optional 缺省漏检"
                    "即源于此)")
...
def r17_gate_registry() -> dict[str, dict[str, Any]]:
    """注册表深拷贝(调用方不得原地修改权威状态)。"""
    return json.loads(json.dumps(R17_GATE_REGISTRY))


def r17_gate_topology_payload() -> dict[str, Any]:
    """进 plan/final/report 的拓扑声明 payload。"""
    registry = r17_gate_registry()
    ...
    return {
        ...
        "cue_semantic_binding_source": R17_CUE_SEMANTIC_BINDING_SOURCE,
        "cue_metrics": list(R17_CUE_METRICS),
        "matched_responsibilities": list(
            R17_MATCHED_CORPUS_RESPONSIBILITIES),
        "independent_marginal_responsibilities": list(
            R17_INDEPENDENT_MARGINAL_RESPONSIBILITIES),
        "independent_marginal_leaves": list(
            R17_INDEPENDENT_MARGINAL_LEAVES),
        "dedicated_semantic_leaves": list(
            R17_DEDICATED_SEMANTIC_LEAVES),
...
def r17_gate_topology_digest(payload: dict[str, Any] | None = None
                             ) -> str:
    """注册表内容的规范 digest(r17gt- 前缀)。"""
    body = payload if payload is not None else r17_gate_topology_payload()
    return "r17gt-" + hashlib.sha256(json.dumps(
        body, sort_keys=True, ensure_ascii=False,
    ).encode("utf-8")).hexdigest()


def r17_cue_semantic_binding_uniqueness() -> dict[str, Any]:
    ...
    unique_ok = (list(sources) == [R17_CUE_SEMANTIC_BINDING_SOURCE]
                 and not leaf_violations)
    return {
        "pass": unique_ok,
        "binding_sources_for_cue_semantics": sources,
        "expected_unique_source": R17_CUE_SEMANTIC_BINDING_SOURCE,
        "leaf_metric_violations": leaf_violations,
        "rule": (
            "binding 条目 metric_scope 传递闭包唯一 source + "
            "leaf_metrics 不含 cue metric 名(R17 §六-5)"),
    }


def r17_binding_status(check: str) -> dict[str, Any]:
    """查询单个 check 的权威 binding status(fail closed:未注册即错)。"""
    try:
        entry = R17_GATE_REGISTRY[check]
    except KeyError as exc:
        raise RuntimeError(
            f"check '{check}' 未在 R17 gate topology 注册表注册——"
            "所有 verdict 级 check 必须在单一权威来源登记 binding "
            "status(R17 §四-5)") from exc
    return {"binding": entry["binding"],
            "diagnostic_only": entry["diagnostic_only"],
            "authoritative_source": entry["authoritative_source"]}


def r17_binding_lineage(detail_sources: dict[str, Any] | None = None
                        ) -> dict[str, Any]:
    ...
    return {
        "format": "cur261-r17-binding-lineage-v2",
        "pass": not problems,
        "problems": problems,
        "entries": entries,
        "rule": (
            "每个 binding check 的 leaf 声明与真实 aggregator 自报"
            "一致;cue metric binding 强制自报(R17 §六)"),
    }


def r17_overridden_strict_gate_rule_text() -> dict[str, str]:
    """R6 STRICT_GATE_RULE_IDENTITY 的 R17 修正覆盖键。

    R6 字典的 c2_matched/c2_marginal_guard 条目携带旧拓扑文字。
    R17 plan 展开时用本函数的键覆盖之——不修改共享历史模块。
    """
    return {
        "c2_matched": (
        ...
        "c2_marginal_guard": (
            ...
            "FAIL;无 SE 要求(R17 GateTopologyReconciliation-v2)"),
        "cue_semantic_gate_topology": (
            ...
            "(R17 GateTopologyReconciliation-v2)"),
    }
