# -*- coding: utf-8 -*-
"""阶段 2.6.1 Repair R12:依赖身份闭环与 Code Freeze 合同(§6/§7.3/§21)。

两部分:
1. 依赖身份表(R9 §6 机制继承):plan lock 前对全部关键符号做模块级
   解析(live import),记录 resolved module / source sha256,
   生成 r12dep- digest —— blind-rename 继承的签名错误在此被显式核对;
2. R12 Code Freeze(§6/§21):正式数据开始前记录全部 R12 源码模块的
   sha256 清单(R12_CODE_FREEZE_SHA + source tree digest);每个正式
   阶段开始前复算,任何漂移 fail closed(永久结束 R12)。
"""
...
DEPENDENCY_TABLE_R12: tuple[tuple[str, str], ...] = (
    # ---- 密度 / pair 评估(R5/R6 冻结)----
    ...
    # ---- R6 runner(R12 wrapper 的委托目标;签名核对对象)----
    ...
    # ---- cue 语义(R12 namespace)----
    ("candidate_cue_semantics",
     "rl_curriculum.curriculum261_r12_cue_eval"),
    ("semantic_cue_gate", "rl_curriculum.curriculum261_r12_cue_eval"),
    ("independent_cue_semantics",
     "rl_curriculum.curriculum261_r12_cue_eval"),
    # ---- R12 新合同(核心修复面)----
    ("canonical_episode", "rl_curriculum.curriculum261_r12_reference"),
    ("reference_equivalence_run_r12",
     "rl_curriculum.curriculum261_r12_reference"),
    ("collect_policy_visible_dataset_r12",
     "rl_curriculum.curriculum261_r12_labels"),
    ("build_routing_r12", "rl_curriculum.curriculum261_r12_routing"),
    ("require_eval_routing_r12",
     "rl_curriculum.curriculum261_r12_routing"),
    ...
    ("run_preplan_full_pipeline_rehearsal_r12",
     "rl_curriculum.curriculum261_r12_rehearsal"),
    ("live_signature_audit_r12",
     "rl_curriculum.curriculum261_r12_delegation"),
    ...
    ("generation_determinism_gate",
     "rl_curriculum.curriculum261_r12_determinism"),
    # ---- R12 full-scale shadow(工作包 C)----
    ("run_full_scale_shadow_r12", "rl_curriculum.curriculum261_r12_shadow"),
...
#: R12 code freeze 覆盖的源码模块(sha256 清单;§6/§21)。
R12_CODE_MODULES: tuple[str, ...] = (
    ...
    "curriculum261_r12_determinism.py",
...
def resolve_dependency_identity_r12() -> dict[str, Any]:
    """解析全部声明符号(fail closed:missing/错模块立即报错)。"""
    rows: dict[str, Any] = {}
    ok = True
    for symbol, defining_module in DEPENDENCY_TABLE_R12:
        try:
    ...
    result = {
        "format": "cur261-r12-dependency-resolution-v1",
        "iteration": "r12", "symbols": rows,
        "dependencies": rows,
        "n_symbols": len(DEPENDENCY_TABLE_R12),
        "n_declared": len(DEPENDENCY_TABLE_R12),
    ...
    result["digest"] = dependency_identity_digest_r12(result)
    return result


def dependency_identity_digest_r12(resolution: dict[str, Any]) -> str:
    payload = json.dumps(resolution, sort_keys=True, ensure_ascii=False,
                         default=str)
    return "r12dep-" + hashlib.sha256(
        payload.encode("utf-8")).hexdigest()


def write_dependency_resolution_r12(out_dir: Path) -> dict[str, Any]:
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    resolution = resolve_dependency_identity_r12()
    resolution["digest"] = dependency_identity_digest_r12(resolution)
...
# ------------------------------------------------------- Code Freeze(§6)
def source_tree_digest_r12() -> dict[str, Any]:
    """全部 R12 源码模块的 sha256 清单与聚合 digest。"""
    import rl_curriculum

    root = Path(rl_curriculum.__file__).parent
    modules: dict[str, str] = {}
    for name in R12_CODE_MODULES:
        path = root / name
        modules[name] = (hashlib.sha256(path.read_bytes()).hexdigest()
                         if path.is_file() else "MISSING")
    aggregate = "r12src-" + hashlib.sha256(json.dumps(
        modules, sort_keys=True).encode("utf-8")).hexdigest()
    return {"format": "cur261-r12-source-tree-v1",
            "modules": modules, "n_modules": len(modules),
            "source_tree_digest": aggregate,
            "all_present": all(v != "MISSING" for v in modules.values())}


def write_r12_code_freeze(out_dir: Path, *, code_freeze_sha: str) -> dict:
    """记录 Implementation Freeze(§5/§6;一次且仅一次;禁 A′)。

    R12 只有 Commit A(冻结)与 Commit B(结果)两个提交阶段:
    freeze 文件已存在 ⇒ 重复写入被拒绝(不存在 replacement freeze /
    hotfix-after-freeze 恢复路径;任何冻结后源码变化 = R12 永久
    FAIL,下一轮必须 R13)。
    """
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "r12_code_freeze.json"
    if path.is_file():
        raise RuntimeError(
            "r12_code_freeze.json 已存在:Implementation Freeze 一次且"
            "仅一次(§5;禁止 A′/replacement freeze/hotfix-after-freeze;"
            "任何冻结后源码变化 => R12 永久 FAIL,下一轮必须 R13)")
    tree = source_tree_digest_r12()
    if not tree["all_present"]:
        raise RuntimeError("R12 源码树不完整(fail closed)")
    payload = {
        "format": "cur261-r12-code-freeze-v1",
        "iteration": "r12",
        ...
        "contract": ("Commit A(Implementation Freeze):全部源码+测试已"
                     "提交且工作树 clean;正式 R12 audit/cue-audit/"
                     "design/calibration/final 数据开始后源码变化 -> "
                     "R12 永久结束,下一轮必须 R13(§5/§6/§25;不存在"
                     "A′/Commit A2/hotfix after freeze)"),
    }
    (out_dir / "r12_code_freeze.json").write_text(
        json.dumps(payload, indent=1, ensure_ascii=False), encoding="utf-8")
    return payload


def verify_r12_code_freeze(out_dir: Path) -> dict[str, Any]:
    """复算源码树与冻结清单比对(每个正式阶段入口;漂移即 fail)。"""
    out_dir = Path(out_dir)
    path = out_dir / "r12_code_freeze.json"
    if not path.is_file():
        return {"pass": False,
                "error": "r12_code_freeze.json 不存在(正式数据开始前"
                         "必须先冻结;§6)"}
    frozen = json.loads(path.read_text(encoding="utf-8"))
    try:
        current = source_tree_digest_r12()
    except Exception as exc:  # noqa: BLE001 —— fail closed
        return {"pass": False,
                "error": f"source_tree_digest_r12 异常(fail closed):"
