#!/usr/bin/env bash
# R15 §十-1:全量测试证据(JUnit XML + 完整 stdout/stderr + 环境
...
EV="$HOME/projects/crypto_rl/r15_test_evidence"
LOG="$HOME/projects/crypto_rl/r15_test_evidence.log"
mkdir -p "$EV"
rm -f "$EV"/*.json "$EV"/*.xml "$EV"/*.txt 2>/dev/null || true

PYTEST_ARGS=(-q tests/route_c_stage2_6_1
             --junitxml="$EV/r15_full_tests.junit.xml")
...
counts["tail"] = tail
junit = ev / "r15_full_tests.junit.xml"
payload = {
    "format": "cur261-r15-test-evidence-v1",
    "pytest_command": "python -m pytest -q tests/route_c_stage2_6_1 "
                      "--junitxml=r15_full_tests.junit.xml",
    "environment": {
...
(ev / "r15_test_evidence_summary.json").write_text(
