#!/usr/bin/env bash
# R13:R13RealArtifactCliRoundTrip-v1 真实 CLI 全链 rehearsal(长任务)
...
LOG="$HOME/projects/crypto_rl/r13_rt_rehearsal.log"

if [ "${R13_RT_FRESH:-1}" = "1" ]; then
  rm -rf "$ART/real_artifact_rehearsal"
fi

{
  date -u +"start %Y-%m-%dT%H:%M:%SZ"
  python -m rl_curriculum.curriculum261_r13_cli real-artifact-rehearsal
  echo "rehearsal_rc=$?"
