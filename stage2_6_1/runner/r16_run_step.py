#!/usr/bin/env python3
"""R16 权威 chain 执行器薄壳(Commit A 冻结文件;§十三)。

用法:
    python r16_run_step.py chain <plan.json> [--result-out <path>]

- chain 模式:读 workflow-plan 生成的计划 JSON,调用
  curriculum261_r16_workflow.execute_workflow_chain_r16(与 rehearsal
  完全相同的权威执行器;每步独立 subprocess + manifest 记录 +
  prerequisite/postcondition 检查 + 失败自动 fail-closure);
- 本文件不定义任何步骤顺序/列表(单一权威来源);
- R16:bootstrap 接受证据由 chain 执行器写入
  (r16_bootstrap_accepted.json),使 workflow 之前的启动失败与
  workflow 内步骤失败可区分(§5.4)。
"""
...
def main(argv: list[str]) -> int:
    ...
    chain = execute_workflow_chain_r16(plan, log_dir=log_dir)
