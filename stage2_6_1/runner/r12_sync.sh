#!/usr/bin/env bash
# R12:发布仓库(E:\trading\freqai-rl-audit) -> WSL 开发树(~/projects/crypto_rl)
# 用法(Windows): wsl -d CryptoRL-Ubuntu-24.04 -- bash -c "tr -d '\r' < /mnt/e/trading/freqai-rl-audit/stage2_6_1/runner/r12_sync.sh | bash"
# 或(WSL内): tr -d '\r' < /mnt/e/trading/freqai-rl-audit/stage2_6_1/runner/r12_sync.sh | bash
set -euo pipefail
...
# 1) 源码(全部 stage2_6_1 源码,含 r12 新模块)
...
mkdir -p "$D/artifacts/route_c_stage2_6_1_repair12"
echo "r12_sync: done (src+tests -> $D)"
