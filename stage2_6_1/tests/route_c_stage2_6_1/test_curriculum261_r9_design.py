# -*- coding: utf-8 -*-
"""R9 §36 测试:design plan digest roundtrip 修复(§8)与 design 流程
...
from rl_curriculum.curriculum261_r9_design import (
    ...
    lock_design_plan_r9,
    run_design_stage_r9,
    verify_matched_core_identity_r8,
)
...
def _mini_plan() -> dict:
    ...
    grid = {
        ...
        for i in range(3)}
    return {
        "format": "cur261-r9-design-plan-v1",
        "iteration": "r9",
...
def test_digest_not_self_referential(tmp_path):
    ...
    plan = _mini_plan()
    pre = design_plan_digest_r9(plan)
    path, digest = lock_design_plan_r9(tmp_path, plan)
    assert digest == pre
    # 正式 loader 复算成功(R7 在此处永远失败)
    loaded, d2 = load_locked_design_plan_r9(tmp_path)
    assert d2 == digest
    assert loaded["design_plan_digest"] == digest
    # 篡改 payload 任意字段 → 复算 mismatch(fail closed)
    tampered = json.loads(
        (tmp_path / "r9_design_plan.json").read_text(encoding="utf-8"))
    tampered["iteration"] = "r10"
    (tmp_path / "r9_design_plan.json").write_text(
        json.dumps(tampered), encoding="utf-8")
    with pytest.raises(RuntimeError, match="digest 复算不一致"):
        load_locked_design_plan_r9(tmp_path)


def test_existing_plan_not_overwritable(tmp_path):
    plan = _mini_plan()
    lock_design_plan_r9(tmp_path, plan)
    with pytest.raises(RuntimeError, match="禁止删除/覆盖/重锁"):
        lock_design_plan_r9(tmp_path, plan)


def test_matched_core_identity_matches_r8_baseline():
    ident = verify_matched_core_identity_r8()
...
def test_semantic_gate_fail_short_circuits_design(tmp_path, monkeypatch):
    ...
    plan = _mini_plan()
    monkeypatch.setattr(r9design, "verify_design_code_identity",
                        lambda p: {"pass": True, "drift": {}})
    monkeypatch.setattr(r9design, "require_r9_iteration_active",
                        lambda: None)
    monkeypatch.setattr(r9design, "mark_design_data_started",
                        lambda: None)
    monkeypatch.setattr(r9design, "write_r9_iteration_aborted",
                        lambda reason: None)
    eval_calls: list[str] = []
    monkeypatch.setattr(
        r9design, "_evaluate_candidate_matched_r9",
        lambda *a, **k: eval_calls.append(a[0]) or {})
    monkeypatch.setattr(
        r9design, "generate_matched_block_with_attempts",
        lambda ladder, *, namespace, block_index: _fake_blocks(1)[0])
    # semantic gate 强制 FAIL(不可能 floor)
    monkeypatch.setattr(
        r9design, "semantic_cue_gate",
...
def test_design_stage_exception_writes_aborted(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="started ledger 异常"):
        run_design_stage_r9(tmp_path, plan, "r9dp-test")
