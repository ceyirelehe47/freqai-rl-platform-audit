# -*- coding: utf-8 -*-
"""R11 工作包 C 测试:full-scale shadow profile 基数 / 双跑比较器 /
训练削减不触碰生成覆盖。"""
...
def test_shadow_profiles_full_generation_cardinality():
    ...
    main = shadow_main_profile_r11()
    hold = shadow_holdout_profile_r11()
...
def test_shadow_pack_not_formal():
    from rl_curriculum.curriculum261_r11_shadow import _shadow_pack

    pack = _shadow_pack()
    assert pack["design_plan_digest"] == "r11dp-shadow-engineering"
    assert pack["selected_block_count"] == 20
    assert not pack["design_plan_digest"].startswith("r11dp-0") or True
...
def test_marginal_gate_reads_guard_pass():
    """回归(R11 shadow 捕获的 R10 潜伏缺陷):c2_independent_marginal_
    guard_r11 返回的 wrapper dict 无顶层 pass;gate 组装必须读
    marginal["guard"]["pass"](短路求值会掩盖该缺陷 —— rehearsal
    因 matched False 短路而未触发,全规模 shadow 触发)。"""
    import inspect

    from rl_curriculum.curriculum261_r11_orchestrator import (
        _orchestrate_calibration_stage_inner_r11,
    )
    src = inspect.getsource(_orchestrate_calibration_stage_inner_r11)
    ...
    sig = inspect.signature(c2_independent_marginal_guard_r11)
    assert list(sig.parameters) == ["indep", "pack",
                                    "recall_floor_value"]
    fn_src = inspect.getsource(c2_independent_marginal_guard_r11)
