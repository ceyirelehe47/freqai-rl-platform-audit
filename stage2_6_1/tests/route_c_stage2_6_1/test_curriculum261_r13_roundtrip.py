# -*- coding: utf-8 -*-
"""R13 测试:真实 artifact round-trip(§四-1/§四-3)。
...
的端到端链由 R13RealArtifactCliRoundTrip-v1(cmd_real_artifact_
...
def _tiny_real_v2():
    ...
    records = generate_fit_bank_r13("preplan_fit_main_r13", pack)
...
def real_identity(tmp_path_factory):
    ...
    out_dir = tmp_path_factory.mktemp("r13_bundle")
...
class TestReadPreprocessorBundleHash:
    ...
    def test_missing_artifact_fail_closed(self, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="缺失"):
            read_preprocessor_bundle_hash(
                missing, consumer_command="r13-cli lock-plan")
    ...
    def test_missing_canonical_field_fail_closed(self, real_identity,
        ...
        with pytest.raises(RuntimeError) as excinfo:
            read_preprocessor_bundle_hash(
                path, consumer_command="r13-cli lock-plan")
        msg = str(excinfo.value)
        assert "preprocessor_bundle_hash" in msg
        assert str(path) in msg
        assert "r13-cli lock-plan" in msg
...
class TestCliIndependentProcess:
    ...
    def test_audit_without_freeze_fails_closed(self, tmp_path):
        ...
        env = dict(os.environ)
        env["PYTHONPATH"] = str(proj / "src")
        env["CURRICULUM261_R13_LOCK_DIR"] = str(tmp_path)
        res = subprocess.run(
            [sys.executable, "-m",
             "rl_curriculum.curriculum261_r13_cli",
    ...
    def test_real_artifact_rehearsal_registered(self):
        ...
        res = subprocess.run(
            [sys.executable, "-m",
             "rl_curriculum.curriculum261_r13_cli", "--help"],
...
class TestDesignPayloadRehearsalOverrides:
    def _payload(self, **overrides):
        ...
        kwargs = dict(
            ...
            cue_audit={"p_contract": 0.95, "audit_digest": "r13ca-test",
                       "pass": True},
            preplan_smoke_identity={"sentinel_digest": "r13smoke-test"},
            dependency_identity={"test": True},
            artifact_writer_identity={"test": True},
            preplan_rehearsal_digest="r13pr-test",
            r8_abort_evidence={"test": True},
        )
        kwargs.update(overrides)
        return design_plan_payload_r13(**kwargs)
    ...
    def test_rehearsal_overrides_recorded(self):
        ...
        assert plan["semantic_corpora"]["namespaces"] == [
            "rt_semantic_design_main_r13",
            "rt_semantic_design_validation_r13"]
        assert plan["independent_marginal_guard"][
            "namespace"] == "rt_design_independent_r13"
    ...
    def test_rt_semantic_map_explicit(self):
        ...
        assert semantic_artifact_filename_r13(
        ...
        with pytest.raises(RuntimeError):
            semantic_artifact_filename_r13("unknown_semantic_ns_r13")
...
class TestExposureOneShot:
    def test_double_create_rejected(self, tmp_path, monkeypatch):
        ...
        write_qualification_r13_exposure("r13qp-test", "running")
...
class TestArtifactInterfaceAudit:
    def test_audit_table_structure(self):
        ...
        assert audit["format"] == "cur261-r13-artifact-interface-audit-v1"
        ...
        # R12 缺陷边必须声明 r12_defect 与 r13_closure
        ...
        assert bundle_edge["r13_closure"]
    ...
    def test_rehearsal_steps_declared(self):
        ...
        assert steps <= {"determinism-matrix", "audit", "cue-audit",
                         "design-plan-lock", "design", "calibrate",
                         "preflight-sealed", "lock-plan", "qualify",
                         "smoke"}
