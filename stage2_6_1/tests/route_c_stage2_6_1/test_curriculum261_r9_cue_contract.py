# -*- coding: utf-8 -*-
"""R9 §36 测试:Cue Semantic Contract v2(§9/§12/§13)。
...
def test_contract_identity_stable():
    ...
    assert d1.startswith("r9cue-") and len(d1) == 6 + 64
    import rl_curriculum.curriculum261_r9_cue_contract as mod
...
def test_audit_small_reproducible_and_writes_artifacts(
    ...
    # §R9-10:generation_mode 冻结(model=once / validation=attempts)
    ...
    assert (r1["direct_generator"]["validation"]["generation_mode"]
            == "attempts")
    # §R9-11:once vs attempts 对照 + attempt histogram
