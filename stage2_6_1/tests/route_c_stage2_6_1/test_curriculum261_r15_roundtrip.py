# -*- coding: utf-8 -*-
"""R15 测试:真实 artifact round-trip(§四-1/§四-3)。
...
的端到端链由 R15RealArtifactCliRoundTrip-v1(cmd_real_artifact_
...
def _tiny_real_v2():
    ...
    records = generate_fit_bank_r15("preplan_fit_main_r15", pack)
...
def real_identity(tmp_path_factory):
    ...
    out_dir = tmp_path_factory.mktemp("r15_bundle")
...
class TestCliIndependentProcess:
    ...
    def test_audit_without_freeze_fails_closed(self, tmp_path):
        ...
        env = dict(os.environ)
        env["PYTHONPATH"] = str(proj / "src")
        env["CURRICULUM261_R15_LOCK_DIR"] = str(tmp_path)
        res = subprocess.run(
            [sys.executable, "-m",
             "rl_curriculum.curriculum261_r15_cli",
    ...
    def test_real_artifact_rehearsal_registered(self):
        ...
        res = subprocess.run(
            [sys.executable, "-m",
             "rl_curriculum.curriculum261_r15_cli", "--help"],
...
class TestDesignPayloadRehearsalOverrides:
    def _payload(self, **overrides):
        ...
        kwargs = dict(
            ...
            preplan_rehearsal_digest="r15pr-test",
            r8_abort_evidence={"test": True},
        )
        kwargs.update(overrides)
        return design_plan_payload_r15(**kwargs)
    ...
    def test_rehearsal_overrides_recorded(self):
        ...
        assert plan["semantic_corpora"]["namespaces"] == [
            "rt_semantic_design_main_r15",
            "rt_semantic_design_validation_r15"]
        assert plan["independent_marginal_guard"][
            "namespace"] == "rt_design_independent_r15"
    ...
    def test_rt_semantic_map_explicit(self):
        ...
        assert semantic_artifact_filename_r15(
        ...
        with pytest.raises(RuntimeError):
            semantic_artifact_filename_r15("unknown_semantic_ns_r15")
...
class TestArtifactInterfaceAudit:
    def test_audit_table_structure(self):
        ...
        assert audit["format"] == "cur261-r15-artifact-interface-audit-v1"
        ...
        # R12 缺陷边必须声明 r12_defect 与 r15_closure
        ...
        assert bundle_edge["r15_closure"]
