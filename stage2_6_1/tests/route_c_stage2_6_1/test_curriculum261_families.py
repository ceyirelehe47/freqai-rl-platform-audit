...
def _mini_corpus(family, rungs=("D0", "D3"), n=2):
    return [generate_pair(family, r, i, namespace="calibration")
...
class TestC2:
    def test_local_cue_gating_structure_exists(self):
        rec = generate_pair("c2_context", "D2", 0, namespace="calibration")
        for side in ("A", "B"):
            h = rec.episodes[side].hidden
            cue = h["cue_dir"].to_numpy()
            gate = (h["gate_g1"].to_numpy() if side == "A"
                    else h["vol_state"].to_numpy())
            # 两种门控极性都与 cue 共存(对比性前提)
            assert ((cue != 0) & (gate > 0)).sum() >= 1
            assert ((cue != 0) & (gate < 0)).sum() >= 1

    def test_gate_binding_swaps_between_variants(self):
        rec = generate_pair("c2_context", "D1", 0, namespace="calibration")
        c = rec.integrity["construction"]
        assert c["A_gate_is_g1"] and c["B_gate_is_vol_regime"]
        assert c["A_payoff_bound_to_g1"] and c["B_payoff_bound_to_vol"]
        assert c["A_payoff_not_bound_to_vol"]
        assert c["B_payoff_not_bound_to_g1"]
...
class TestC3:
    ...
    def test_above_and_below_cost_signals_exist_on_a(self):
        rec = generate_pair("c3_cost", "D2", 0, namespace="calibration")
        h = rec.episodes["A"].hidden
    ...
    def test_b_variant_all_below_cost_and_decoupled(self):
        rec = generate_pair("c3_cost", "D3", 0, namespace="calibration")
        c = rec.integrity["construction"]
