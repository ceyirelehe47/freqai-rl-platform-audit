# -*- coding: utf-8 -*-
"""R12 工作包 A 测试:跨进程确定性与 mutable state(§12 类别
7/8/9/10/13/14)。

全部场景通过 curriculum261_r12_determinism 的 probe 入口在
...
DET_MOD = Path(
    __import__("rl_curriculum.curriculum261_r12_determinism",
               fromlist=["__file__"]).__file__).resolve()


def _probe(tmp_path, name, *, target="r12", prelude="",
           env_extra=None):
...
def _base(tmp_path):
    return _extract(_probe(tmp_path, "base", target="r12"))
...
def test_same_invocation_torch_import_first(tmp_path):
    base = _base(tmp_path)
    got = _extract(_probe(
        tmp_path, "torch", target="r12", prelude="import_torch_first"))
    assert got["attempt_digests"] == base["attempt_digests"]
...
for fam in ("c1_opportunity", "c2_context", "c3_cost"):
    generate_pair(fam, "D1", 0, namespace="stress_r12")
ladder = {r: dict(family_specs()["c2_context"].rung_params[r])
          for r in ("D0", "D1", "D2", "D3")}
generate_matched_block_with_attempts(
    ladder, namespace="stress_r12", block_index=0)
