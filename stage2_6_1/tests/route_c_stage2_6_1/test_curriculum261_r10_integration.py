# -*- coding: utf-8 -*-
"""R10 §31 集成测试:candidate evaluator 全链路闭环与 artifact 显式映射。
...
def test_dependency_resolution_table_full_pass():
    from rl_curriculum.curriculum261_r10_dependencies import (
        ...
        DEPENDENCY_TABLE_R10,
        resolve_dependency_identity_r10,
    )

    rep = resolve_dependency_identity_r10()
    assert rep["pass"], rep["problems"]
    assert rep["n_declared"] == len(DEPENDENCY_TABLE_R10)
    ...
    assert rep["digest"].startswith("r10dep-")
    # 每个声明的 symbol 在 import 期可从声明模块取到
    import importlib

    for symbol, module in DEPENDENCY_TABLE_R10:
        assert hasattr(importlib.import_module(module), symbol), symbol


def test_r10_design_has_no_function_body_import_of_density():
    """§6.2:r10_design 的 evaluator 路径无函数内延迟 import(模块级
    解析;R8 缺陷的结构性回归测试)。"""
    import inspect

    import rl_curriculum.curriculum261_r10_design as r10design

    src = inspect.getsource(r10design)
    assert ("from rl_curriculum.curriculum261_r6_pairs import "
            "c2_density_summary") not in src
    for fn_name in ("_evaluate_candidate_matched_r10",
                    "_run_independent_marginal_guard"):
        body = inspect.getsource(getattr(r10design, fn_name))
...
def test_real_candidate_evaluator_full_chain_no_monkeypatch(tmp_path):
    ...
    blocks = [generate_matched_block_with_attempts(
        ladder, namespace="preplan_candidate_eval_r10", block_index=i)
        for i in range(2)]
    result = _evaluate_candidate_matched_r10(
        "integration_sentinel", ladder, "preplan_candidate_eval_r10",
...
def test_preplan_evaluator_smoke_artifact(tmp_path):
    ...
    from rl_curriculum.curriculum261_r10_preplan import (
        run_candidate_evaluator_integration_smoke_r10,
    )

    report = run_candidate_evaluator_integration_smoke_r10(
        tmp_path, n_blocks=2)
    ...
    assert on_disk["namespace"] == "preplan_candidate_eval_r10"
...
def test_semantic_writer_explicit_mapping_no_suffix_heuristic(tmp_path):
    ...
    assert (semantic_artifact_filename_r10("cue_semantic_design_main_r10")
            == "semantic_design_main.json")
    assert (semantic_artifact_filename_r10(
        "cue_semantic_design_validation_r10")
        == "semantic_design_validation.json")
    # main/validation 永不写入同一个 path
    names = set(SEMANTIC_ARTIFACT_MAP_R10.values())
    assert len(names) == len(SEMANTIC_ARTIFACT_MAP_R10)
    # 三阶段映射同样穷尽且互不冲突
    all_names = names | set(SEMANTIC_STAGE_ARTIFACT_MAP_R10.values())
    assert len(all_names) == (len(SEMANTIC_ARTIFACT_MAP_R10)
                              + len(SEMANTIC_STAGE_ARTIFACT_MAP_R10))
    # 后缀启发式在 R10 的映射/writer 实现代码中不复存在(结构性回归
    # 测试;模块 docstring 保留对 R8 缺陷的历史说明属正常)
    import inspect

    from rl_curriculum.curriculum261_r10_design import (
        semantic_artifact_filename_r10 as _saf,
        write_semantic_artifact_r10 as _wsa,
    )
    ...
    p_main = write_semantic_artifact_r10(
        tmp_path, "cue_semantic_design_main_r10", {"marker": "m"},
        "r10dp-test", event_rows=[{"cue_bar": 1}])
    p_valid = write_semantic_artifact_r10(
        tmp_path, "cue_semantic_design_validation_r10",
        {"marker": "v"}, "r10dp-test", event_rows=[{"cue_bar": 1}])
    ...
    assert back["namespace"] == "cue_semantic_design_main_r10"
    assert back["corpus_role"] == "main"
    assert back["design_plan_digest"] == "r10dp-test"
    assert back["event_count"] == 1
    assert (tmp_path / "semantic_design_main.jsonl").is_file()
    # 覆盖尝试必须失败
    with pytest.raises((OSError, FileExistsError)):
        write_semantic_artifact_r10(
            tmp_path, "cue_semantic_design_main_r10", {"marker": "x"},
            "r10dp-test")


def test_semantic_writer_validation_artifact(tmp_path):
    from rl_curriculum.curriculum261_r10_preplan import (
        run_semantic_writer_validation_r10,
    )

    report = run_semantic_writer_validation_r10(tmp_path)
    assert report["pass"], report["checks"]
    on_disk = json.loads(
...
def test_cue_audit_plan_lock_roundtrip_and_no_relock(tmp_path):
    from rl_curriculum.curriculum261_r10_cue_contract import (
        cue_audit_plan_digest_r10,
        cue_audit_plan_payload_r10,
        load_locked_cue_audit_plan_r10,
        lock_cue_audit_plan_r10,
    )

    path, digest = lock_cue_audit_plan_r10(tmp_path)
    assert digest.startswith("r10ap-")
    assert path.is_file()
    payload = load_locked_cue_audit_plan_r10(tmp_path)
    ...
    # digest 不自引用
    assert cue_audit_plan_digest_r10(payload) == digest
    # 重锁拒绝
    with pytest.raises(RuntimeError, match="禁止修改/重锁"):
        lock_cue_audit_plan_r10(tmp_path)
    # 篡改 → 复算失败
    ...
    with pytest.raises(RuntimeError, match="复算不一致"):
        load_locked_cue_audit_plan_r10(tmp_path)
...
def test_cue_audit_requires_locked_plan_only_when_formal(tmp_path):
    ...
    with pytest.raises(RuntimeError,
        ...
        run_cue_contract_audit(
            ...
            model_namespace="preplan_smoke_r10",
            validation_namespace="preplan_candidate_eval_r10",
            require_locked_plan=True)


# ------------------------------------------------ §23 plan 前缀
def test_qualification_plan_digest_uses_qp10_prefix():
    from rl_curriculum.curriculum261_r10_plan import plan_digest_r10

    digest = plan_digest_r10({"probe": True})
    assert digest.startswith("qp10-") and len(digest) == 5 + 64
...
def test_next_round_wording_is_r10():
    """§23/§34:错误文案必须写"下一轮必须 R11",不得出现 R10.1。"""
    import inspect

    import rl_curriculum.curriculum261_r10_design as r10design
    import rl_curriculum.curriculum261_r10_namespaces as r10namespaces
    import rl_curriculum.curriculum261_r10_plan as r10plan

    for mod in (r10design, r10namespaces, r10plan):
        src = inspect.getsource(mod)
        assert "R10.1" not in src, mod.__name__
        assert "R10.1/R10" not in src, mod.__name__


def test_r8_namespaces_and_markers_not_referenced_by_r10_design_data():
    """§17:R10 全部 namespace 与 R8 零重合(字符串层与 seed 层)。"""
    ...
    assert not (r8_set & r10_set)
    assert len(r10_set) == len(CURRICULUM261_R10_NAMESPACES) == 38
    for required in ("cue_contract_model_r10", "cue_contract_validation_r10",
                     "preplan_smoke_r10", "preplan_candidate_eval_r10",
                     "preplan_semantic_main_r10",
                     ...
                     "cue_semantic_calibration_r10",
                     "cue_semantic_holdout_r10",
                     ...
                     "stress_r10", "fresh_holdout_r10", "training_r10",
                     "ppo_smoke_r10"):
        assert required in r10_set, required


def test_qualification_r10_seeds_locked_before_unlock(tmp_path,
                                                     monkeypatch):
    """§17/§24:qualification_r10 namespace 在六要素解锁前封闭。"""
