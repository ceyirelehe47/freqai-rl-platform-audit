# -*- coding: utf-8 -*-
"""R11 集成测试:tiny supervised 全链(R11 runner + 修复后的两个
gate 真实接线;§12 类别 26/28 的 R11 侧)。"""
...
def tiny_supervised():
    # torch 线程面隔离(v2 轮 S3 全量回归归因):supervised 训练链在
    # train_supervised_mlp 内 import torch,会在 pytest 进程留下未
    # 屏蔽 CUDA/jemalloc 线程,使字母序在后的直调形态 supervisor
    # 测试截止点前提核验 rc=7(与 conftest 的 BLAS/CUDA 缓解同族;
    # 生产的 supervised 链在独立进程执行)。tiny 全链在一次性子进程
    # 中真实执行,断言语义不变。
    import json
    import os
    import subprocess
    ...
    code = (
        ...
        '    fit_preprocessor_v2_from_bank_r11, generate_fit_bank_r11,\n'
        '    supervised_learnability_run_r11)\n'
        'from rl_curriculum.curriculum261_r11_shadow import _shadow_pack\n'
        'pack = _shadow_pack()\n'
        'bank = generate_fit_bank_r11("preplan_fit_main_r11", pack, '
        'pairs_per_rung=1)\n'
        'v2, _ = fit_preprocessor_v2_from_bank_r11(\n'
        '    "preplan_fit_main_r11", pack, records=bank, pairs_per_rung=1,'
        '\n    parameter_pack_identity=pack["digest"])\n'
        'result = supervised_learnability_run_r11(\n'
        '    v2, pack, namespace="preplan_supervised_main_r11",\n'
    ...
    for fam, block in result["families"].items():
        ...
        for control in ("W", "B"):
            ...
            assert set(gate["passing_seed_ids_by_control"][
                control]) == ids
            expect = len(ids) >= R11_SUPERVISED_GATE["min_seeds_passing"]
...
def test_recorder_sink_active_during_orchestration():
    ...
    src = inspect.getsource(orchestrate_calibration_stage_r11)
    ...
    src2 = inspect.getsource(execute_final_core_r11)
