# -*- coding: utf-8 -*-
"""R10 §36 测试:Cue Semantic Contract v2(§9/§12/§13)。
...
def test_contract_identity_stable():
    ...
    assert d1.startswith("r10cue-") and len(d1) == 7 + 64
    import rl_curriculum.curriculum261_r10_cue_contract as mod
...
def test_audit_small_reproducible_and_writes_artifacts(
    ...
    # §R10-10:generation_mode 冻结(model=once / validation=attempts)
    assert r1["generation_mode"] == {"model": "once",
                                     "validation": "attempts"}
    assert r1["direct_generator"]["model"]["generation_mode"] == "once"
    assert (r1["direct_generator"]["validation"]["generation_mode"]
            == "attempts")
    # §R10-11:once vs attempts 对照 + attempt histogram
    ova = r1["once_vs_attempts"]
    assert ova["model_mode"] == "once"
    assert ova["validation_mode"] == "attempts"
    assert ova["first_pass_bitwise_check"]["n_blocks_checked"] > 0
    assert ova["first_pass_bitwise_check"]["bitwise_ok"]
    assert "recall_modes_consistent" in ova
    assert "k_modes_consistent" in ova
    assert "attempt_histogram_validation" in ova
    assert "once_vs_attempts_consistent" in r1["checks"]
    assert (tmp_path / "once_vs_attempts_audit.json").is_file()
    # 落盘完整性(§11/§37)
    audit = json.loads(
