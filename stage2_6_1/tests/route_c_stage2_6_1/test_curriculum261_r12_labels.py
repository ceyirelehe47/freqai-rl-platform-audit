# -*- coding: utf-8 -*-
"""R12 工作包 B 测试:supervised gate 两个假 PASS 修复。
...
def _make_dataset(alignment_failures, episode_records, rows=None,
                  n_steps_total=None):
    """直接构造 collect_policy_visible_dataset_r12 的返回形态。"""
...
def _aggregate_via_module(dataset):
    ...
    from rl_curriculum.curriculum261_r12_labels import (
        collect_policy_visible_dataset_r12 as _orig,
    )
...
def _aggregation_from_real_function():
    ...
    src = inspect.getsource(collect_policy_visible_dataset_r12)
...
def tiny_c1_dataset():
    ...
    fit_records = [generate_pair(
        f, r, 0, namespace="preplan_fit_main_r12")
    ...
    return collect_policy_visible_dataset_r12(
        records, "c1_opportunity", rung_params, pre,
        eval_namespace="preplan_supervised_main_r12")
...
def _family_block(passing_seeds_by_control, gate=None):
    """构造 supervised_learnability_run_r12 的 family 输出形态并
    独立复算机械 gate(不信任自报 pass 字段)。"""
    from rl_curriculum.curriculum261_r12_orchestrator import (
        R12_SUPERVISED_GATE,
    )
    gate = gate or R12_SUPERVISED_GATE
...
def test_b2_single_seed_wb_double_pass_is_not_two_seeds():
    ...
    R10 缺陷:n_passing=2 >= 2 => 误 PASS。R12:distinct W=1,
    distinct B=1 => FAIL。
    """
...
def test_b2_gate_constants_semantics():
    ...
    assert R12_SUPERVISED_GATE["gated_controls"] == ["W", "B"]
    assert R12_SUPERVISED_GATE["n_model_seeds"] == 3
    assert R12_SUPERVISED_GATE["min_seeds_passing"] == 2
...
def test_b2_implementation_records_distinct_seed_evidence():
    ...
    src = inspect.getsource(supervised_learnability_run_r12)
