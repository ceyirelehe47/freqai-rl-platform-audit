# -*- coding: utf-8 -*-
"""R12 治理测试:code freeze / routing fail-closed / 正式与 rehearsal
...
def test_routing_rejects_role_namespace_cross():
    ...
    routing = R12BundleRouting(
        role="main", fit_namespace="preprocess_fit_calibration_r12",
    ...
    with pytest.raises(RoutingContractError):
        require_eval_routing_r12(
            routing, "supervised_holdout_r12", context="t")
    # 正式 routing 不得服务 preplan/shadow namespace
    with pytest.raises(RoutingContractError):
        require_eval_routing_r12(
            routing, "preplan_supervised_main_r12", context="t")
    with pytest.raises(RoutingContractError):
        require_eval_routing_r12(
            routing, "shadow_supervised_main_r12", context="t")
    # 未知 namespace
    with pytest.raises(RoutingContractError):
        require_eval_routing_r12(routing, "nope_ns", context="t")
...
def test_shadow_routing_cannot_serve_formal():
    ...
    shadow_routing = R12BundleRouting(
        role="main", fit_namespace="shadow_fit_main_r12",
        bundle_hash="b", parameter_state_hash="p",
        manifest_multiset_hash="m", _v2=None, shadow=True)
    with pytest.raises(RoutingContractError):
        require_eval_routing_r12(
            shadow_routing, "supervised_main_r12", context="t")
    # shadow routing 服务 shadow namespace 正常
    v2 = require_eval_routing_r12(
        shadow_routing, "shadow_supervised_main_r12", context="t")
    assert v2 is None  # _v2=None 占位


# ------------------------------------------------ 25: freeze 变化拒绝
def test_code_freeze_rejects_source_change(r12_lock_dir):
    ...
    write_r12_code_freeze(r12_lock_dir, code_freeze_sha="deadbeeftest")
    # 篡改冻结清单中一个模块 sha => 复验必须拒绝
    art = r12_lock_dir / "r12_code_freeze.json"
    ...
    result = verify_r12_code_freeze(r12_lock_dir)
    assert result["pass"] is False


def test_code_freeze_modules_cover_new_modules():
    from rl_curriculum.curriculum261_r12_dependencies import R12_CODE_MODULES

    mods = set(R12_CODE_MODULES)
...
def test_formal_and_rehearsal_share_orchestrator():
    ...
    from rl_curriculum.curriculum261_r12_cli import _cmd_calibrate_inner
    ...
    rehearsal_src = inspect.getsource(
        run_preplan_full_pipeline_rehearsal_r12)
    shadow_src = inspect.getsource(run_full_scale_shadow_r12)
    for src in (formal_src, rehearsal_src, shadow_src):
        assert "orchestrate_calibration_stage_r12" in src
    # final 核心共享
    from rl_curriculum.curriculum261_r12_final import (
        execute_final_core_r12,
    )
    final_src = inspect.getsource(execute_final_core_r12)
    assert "_execute_final_core_inner_r12" in final_src
...
def test_no_monkeypatch_in_formal_modules():
    ...
    for name in ("curriculum261_r12_cli.py",
...
def test_calibrate_wrapper_writes_aborted_and_dumps_evidence(
        tmp_path, r12_lock_dir, monkeypatch):
    ...
    def _boom(*a, **k):
        ...
        rec = EnvelopeRecorder(
            iteration="r12", namespace="stress_r12", family="c3_cost",
            rung="D0", pair_index=9, rung_params=rp)
        try:
            generate_pair_with_attempts(
                family_specs()["c3_cost"].generator, rp,
                namespace="stress_r12", family="c3_cost", rung="D0",
                pair_index=9,
    ...
    # abort marker 已写
    assert (r12_lock_dir / "r12_iteration_aborted.json").is_file()
    marker = json.loads(
        (r12_lock_dir / "r12_iteration_aborted.json").read_text(
            encoding="utf-8"))
    assert marker["iteration"] == "r12"
...
def test_generation_determinism_gate_binding_requires_pass(
    ...
    with pytest.raises(RuntimeError, match="determinism"):
        _generation_determinism_gate_binding(r12_lock_dir)
    det = r12_lock_dir / "determinism"
    ...
    binding = _generation_determinism_gate_binding(r12_lock_dir)
