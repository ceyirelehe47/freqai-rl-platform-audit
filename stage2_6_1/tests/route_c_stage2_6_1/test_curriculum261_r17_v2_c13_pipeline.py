...
def test_v04_registry_exact_increment():
    ...
    # 精确 157/12(v2c13 世代 97/4 + R18 增量 30/4 + R19 尝试增量
    # 30/4);非重言式:与显式基线集合 + v1 四名 + v2 四名 + R18 尝试名
    # + R19 尝试名的并集精确相等。
    ...
    assert len(ns) == 157 and len(formal) == 12
    ...
    # R18/R19 尝试正式四名已注册且被镜像双表一致收录。
    assert set(R18_FORMAL_FOUR) <= set(formal)
    assert set(R19_FORMAL_FOUR) <= set(formal)
    # v1 名保留且仍非正式。
...
def test_v0x_both_trees_import_same_contract():
    ...
    # dataclass + `from __future__ import annotations`:模块必须先进
    # sys.modules,否则 dataclasses 解析字符串注解时取不到模块字典。
