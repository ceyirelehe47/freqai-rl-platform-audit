# -*- coding: utf-8 -*-
"""R11 工作包 A 测试:Generation Invocation Envelope。
...
def _recorded(family="c3_cost", rung="D0", pair=3):
    ...
    rec = EnvelopeRecorder(
        iteration="r11", namespace=NS, family=family, rung=rung,
...
def test_generator_call_does_not_mutate_input_params(family):
    ...
    rec = EnvelopeRecorder(
        iteration="r11", namespace=NS, family=family, rung="D1",
...
def test_failure_evidence_saved_before_raise(tmp_path):
    ...
    rp["cue_rate"] = 0.05
    rec = EnvelopeRecorder(
        iteration="r11", namespace=NS, family="c3_cost", rung="D0",
...
def test_ledger_sink_records_all_attempts(tmp_path):
    ...
    assert d1.startswith("r11ledger-")
