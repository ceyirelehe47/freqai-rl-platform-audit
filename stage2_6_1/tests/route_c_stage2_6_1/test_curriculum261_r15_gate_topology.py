...
#: _execute_final_core_inner_r15 的 checks dict 一致;两处同步锁定)。
EXPECTED_FINAL_BOOL_CHECKS = {
    ...
    "binding_lineage_consistent",
}


class TestRegistryTopology:
    def test_version_and_unique_cue_binding_source(self):
        assert R15_GATE_TOPOLOGY_VERSION == "GateTopologyReconciliation-v2"
        uniq = r15_cue_semantic_binding_uniqueness()
    ...
    def test_point_diagnostics_entry_is_non_binding(self):
        entry = R15_GATE_REGISTRY["c2_matched_cue_point_diagnostics"]
    ...
    def test_thresholds_frozen(self):
        assert R15_MATCHED_POINT_DIAGNOSTIC_THRESHOLDS == {
            "cue_recall_min": 0.95,
            "cue_precision_min": 0.85,
            "non_cue_false_positive_max": 0.01,
            "payoff_bar_false_cue_max": 0.06,
    ...
    def test_registry_covers_all_final_bool_checks(self):
        ...
        for name in EXPECTED_FINAL_BOOL_CHECKS:
            assert name in R15_GATE_REGISTRY, (
                f"final check '{name}' 未在 gate registry 注册")
            assert r15_binding_status(name)["binding"] is True

    def test_binding_status_fail_closed_on_unknown(self):
        with pytest.raises(RuntimeError, match="未在 R15 gate topology"):
            r15_binding_status("nonexistent_check_name")

    def test_dedicated_semantic_is_only_cue_binding(self):
        entry = R15_GATE_REGISTRY["c2_dedicated_semantic_corpus_pass"]
        assert entry["binding"] is True
        assert entry["authoritative_source"] == (
            R15_CUE_SEMANTIC_BINDING_SOURCE)
        assert entry["semantic_blocks_per_corpus"] == 160

    def test_v2_independent_cue_point_diagnostics_entry(self):
        """§五:independent corpus cue 点指标诊断条目(非 binding)。"""
        entry = R15_GATE_REGISTRY["independent_cue_point_diagnostics"]
        assert entry["binding"] is False
        assert entry["diagnostic_only"] is True
        assert entry["binding_gate"] is False
        assert entry["metric_scope"] == (
            "cue_recall", "cue_precision", "non_cue_false_positive",
            "payoff_bar_false_cue")

    def test_v2_binding_entries_have_metric_scope_and_leaves(self):
        """§六 fail closed:binding 条目必须有 metric_scope 与
        leaf_metrics(构造期校验);independent marginal 的 leaves
        不含 cue 点指标。"""
        for name, entry in R15_GATE_REGISTRY.items():
            if entry["binding"]:
                assert "metric_scope" in entry, name
                assert entry["leaf_metrics"], name
        leaves = set(R15_GATE_REGISTRY[
            "c2_independent_marginal_pass"]["leaf_metrics"])
        assert "point_recall_ge_absolute_floor" not in leaves
        assert "noncue_fp_ucb_le_max" not in leaves
        assert "independent_cue_canonical_consistency" in leaves

    def test_v2_uniqueness_leaf_cross_check(self):
        """leaf 名携带 cue metric 规范名 ⇒ violation(fail closed)。"""
        from rl_curriculum.curriculum261_r15_gate_topology import (
            r15_cue_semantic_binding_uniqueness as _u,
        )
        u = _u()
        assert u["pass"] is True
        assert u["leaf_metric_violations"] == []

    def test_matched_and_marginal_responsibilities_declared(self):
        matched = R15_GATE_REGISTRY["c2_matched_strict_pass"]
        ...
        marginal = R15_GATE_REGISTRY["c2_independent_marginal_pass"]
...
def _make_gate_evidence(point_pass: bool,
    ...
    """最小化构造 _build_gate_evidence_r15 的输入并调用真实实现。"""
    ...
    evidence = _build_gate_evidence_r15(
        ...
        pack={"digest": "r15pk-test"},
        plan_digest="r15qp-test",
...
class TestVerdictTopologyRegression:
    ...
    def test_binding_status_consistent_across_plan_final_report(self):
        ...
        """
        payload = r15_gate_topology_payload()
        registry_binding = sorted(
            k for k, v in R15_GATE_REGISTRY.items() if v["binding"])
        registry_diagnostic = sorted(
            k for k, v in R15_GATE_REGISTRY.items() if not v["binding"])
...
class TestFinalSourceContract:
    ...
    def test_final_no_c2_semantics_pass_binding(self):
        text = (SRC / "curriculum261_r15_final.py").read_text(
            encoding="utf-8")
    ...
    def test_r13_final_binding_line_absent_in_r15(self):
        """R13 的三诊断 AND 绑定(c2_semantics_pass = all(semantics
        三项 pass),点估计 gate 随之 binding)在 R15 不得复现。"""
        text = (SRC / "curriculum261_r15_final.py").read_text(
            encoding="utf-8")
    ...
    def test_plan_overrides_r6_old_topology_text(self):
        text = (SRC / "curriculum261_r15_plan.py").read_text(
            encoding="utf-8")
        assert "r15_overridden_strict_gate_rule_text()" in text
        assert "gate_topology_digest" in text
        overrides = r15_overridden_strict_gate_rule_text()
    ...
    def test_digest_is_deterministic_and_prefixed(self):
        d1 = r15_gate_topology_digest()
        d2 = r15_gate_topology_digest(r15_gate_topology_payload())
        assert d1 == d2
        assert d1.startswith("r15gt-")


class TestR13CalibrationDelegationHonored:
    def test_calibration_delegation_note_exists(self):
        text = (SRC / "curriculum261_r15_calibration.py").read_text(
            encoding="utf-8")
        assert "cue_semantics_delegated_note" in text
        assert "run_c2_diagnostics_r15" in text
        # 定位文字保持诊断对照
        assert "诊断对照" in text

    def test_cue_eval_independent_point_gate_floor_is_0_90(self):
        """independent marginal 的点估计灾难护栏保持 0.90(非 0.95)。"""
        text = (SRC / "curriculum261_r15_cue_eval.py").read_text(
            encoding="utf-8")
