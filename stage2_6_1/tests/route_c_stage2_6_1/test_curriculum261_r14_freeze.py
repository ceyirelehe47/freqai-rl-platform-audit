# -*- coding: utf-8 -*-
"""R14 §六:全 freeze surface(递归 manifest + Git tree identity)测试。
...
def freeze_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "rl_curriculum.curriculum261_r14_dependencies._freeze_dev_root",
        lambda: dev)
    monkeypatch.setattr(
...
class TestFreezeSurfaceManifest:
    ...
    def test_write_freeze_rejects_dirty_repo(self, freeze_env):
        ...
        with pytest.raises(RuntimeError, match="未提交变化"):
            write_r14_code_freeze(out, code_freeze_sha=head)
...
class TestFreezeVerifyDriftDetection:
    ...
    def test_no_drift_passes(self, freeze_env):
        ...
        result = verify_r14_code_freeze(out)
