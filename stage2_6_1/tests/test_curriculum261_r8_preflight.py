# -*- coding: utf-8 -*-
"""R8 §36 测试:preflight(§30)与 plan roundtrip(§8.3)。
...
def test_prelock_static_preflight(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CURRICULUM261_R8_LOCK_DIR", str(tmp_path / "lock"))
    result = run_prelock_static_preflight_r8(
    ...
    assert result["namespaces_touched"] == ["ppo_smoke_r8"]
...
def _write_preplan_inputs(out: "pathlib.Path") -> None:
    ...
    (out / "preplan_engineering_smoke.json").write_text(json.dumps({
        "identity": {"sentinel_digest": "r8smoke-" + "0" * 64},
        "pass": True,
    }), encoding="utf-8")
...
def test_cli_plan_roundtrip_subcommand(tmp_path, monkeypatch):
    ...
    from rl_curriculum.curriculum261_r8_cli import main

    monkeypatch.setenv("CURRICULUM261_R8_LOCK_DIR",
                       str(tmp_path / "lock"))
    out = tmp_path / "art"
...
def test_design_plan_lock_requires_roundtrip(tmp_path, monkeypatch):
    ...
    assert rc == 1
    assert not (out / "r8_design_plan.json").exists()
