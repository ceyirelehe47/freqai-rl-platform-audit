# -*- coding: utf-8 -*-
"""R8 §36 测试:parameter pack(§17/§27)——恰好 3 候选、digest
...
def test_grid_exactly_three_candidates():
    assert len(C2_LADDER_CANDIDATES_R8) == 3
    assert validate_r8_grid_semantics() == []
    ids = set(C2_LADDER_CANDIDATES_R8)
    assert ids == {"c2l_historical_control", "c2l_conservative",
                   "c2l_midpoint"}
    # D3 alpha 覆盖 28/30/32
    d3 = {float(v["D3"]["alpha_bps"]) for v in
          C2_LADDER_CANDIDATES_R8.values()}
...
def test_grid_forbidden_fourth_candidate_rejected():
    import rl_curriculum.curriculum261_r8_param_pack as mod

    original = dict(mod.C2_LADDER_CANDIDATES_R8)
    try:
        mod.C2_LADDER_CANDIDATES_R8["c2l_extra"] = dict(
            original["c2l_midpoint"])
        problems = validate_r8_grid_semantics()
        assert any("恰好 3" in p for p in problems)
    finally:
        mod.C2_LADDER_CANDIDATES_R8.clear()
        mod.C2_LADDER_CANDIDATES_R8.update(original)


def test_pack_digest_roundtrip_and_write(tmp_path):
    ladder = r8_candidate_grid()["c2l_midpoint"]
    pack = ladder_pack_payload_r8(
        selected_c2_candidate="c2l_midpoint",
        c2_ladder=ladder,
        selected_block_count=15,
        design_plan_digest="r8dp-" + "a" * 64,
        matched_contract_identity="r6ml-x",
        block_integrity_identity="bt-x",
        cue_semantic_contract_digest="r8cue-x",
        cue_semantic_rule_identity="r8csg-x",
        cue_audit_digest="r8ca-x",
        p_contract=0.9509,
    ...
    assert pack["pack_version"] == R8_PACK_VERSION
    ...
    d1 = pack_digest_r8(pack)
    pack["digest"] = d1
    assert pack_digest_r8(pack) == d1  # digest 不自引用
    write_selected_pack_r8(tmp_path, pack)
    loaded = load_selected_pack(tmp_path)
    assert loaded["digest"] == d1
    assert loaded["selected_block_count"] == 15
    # 篡改 payload → 复算失败
    tampered = json.loads(
        (tmp_path / "r8_parameter_pack.json").read_text(encoding="utf-8"))
    tampered["selected_block_count"] = 20
    (tmp_path / "r8_parameter_pack.json").write_text(
...
def test_distance_from_historical_zero_for_control():
    hist = r8_candidate_grid()["c2l_historical_control"]
    assert ladder_distance_from_historical_r8(hist) == 0.0
    cons = r8_candidate_grid()["c2l_conservative"]
    assert ladder_distance_from_historical_r8(cons) > 0.0
