...
def build_replication_evidence(
    ...
    *,
    ...
    """工作包 E:从某作弊原因的实际多 Episode 反事实记录聚合证据。

    输入 records 为该原因下每 Episode(或多切割点)的 PairResult
    .to_record();base_net_by_episode 为 {episode_seed: 原始收益}。

    记录:实际测试 Episode 数/不同 seed 数/失败数/失败比例/动作分歧
    分布/配对收益差/优势崩溃 bootstrap/首个分歧位置聚合/是否达到
    预注册复制门槛。
    """
    ...
    for rec in records:
        ...
        if seed is None:
            continue
        tested_seeds.add(int(seed))
        if not rec.get("pass"):
            failing_seeds.add(int(seed))
        if rec.get("action_match_rate") is not None:
            action_divergences.append(1.0 - float(rec["action_match_rate"]))
        if rec.get("first_divergence_step") is not None:
            first_divs.append(int(rec["first_divergence_step"]))
        vn = rec.get("variant", {}).get("net_return")
        if isinstance(vn, (int, float)):
            variant_nets.append(float(vn))
            base = base_net_by_episode.get(int(seed))
            if base is not None:
                paired_diffs.append(float(vn) - float(base))
        for v in (rec.get("extra", {}).get("variant_net_returns") or []):
            if isinstance(v, (int, float)):
                variant_nets.append(float(v))
    n_episodes = len(tested_seeds)
    n_distinct = len(tested_seeds)
    n_failing = len(failing_seeds)
    # 优势崩溃:变体收益的 bootstrap 上界低于最低有效线(稳定崩溃)
    if variant_nets:
    ...
    paired_boot = (paired_bootstrap_ci(paired_diffs,
                   ...
                   {"n": 0, "stat": float("nan"), "ci_low": float("nan"),
                    "ci_high": float("nan")})
    return {
        "tested_episodes": n_episodes,
        "distinct_seeds": n_distinct,
        "failing_episodes": n_failing,
        "failure_ratio": (n_failing / n_episodes) if n_episodes else 0.0,
        "failing_seed_list": sorted(failing_seeds),
        "action_divergence_rates": action_divergences,
        "first_divergence_positions": sorted(set(first_divs)),
