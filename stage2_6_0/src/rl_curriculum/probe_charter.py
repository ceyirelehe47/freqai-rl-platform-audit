...
def audit_probe_charter() -> dict[str, Any]:
    ...
    return {
        ...
        "model_visible_information": [
            "ret_1", "ret_4", "ret_12", "vol_24", "ma_ratio", "当前目标仓位",
        ],
        ...
        "null_control_construction": (
            "probe_null_control:与探针A相同的 regime 参数采样后,对收益"
            "序列做确定性相位随机化重排(保留边际分布,切断可观察信息到"
            "未来收益的可预测关系);OHLCV 按同一规则重建"
        ),
        "oracle": (
            "OracleSegmentedDriftPolicy:读隐藏 regime_direction>0 做多"
            "(仅课程可解性上限,永远不得作为模型训练输入)"
        ),
        "observable_rule_baseline": (
            "RuleTrendPolicy:ma_ratio > 0.001 且 ret_4 > 0 做多"
            "(只用模型同样可见的信息;阈值在本章程冻结)"
        ),
        "trivial_baselines": [
        ...
        "anti_cheat_exams": [
            ...
            "irrelevant_feature_injection", "irrelevant_feature_shuffle",
            "signal_ablation", "trend_direction_mirror",
            "cost_monotonicity", "null_control",
        ],
        "behavior_metrics": [
            "扣费收益(中位数/均值/q10/最差)", "最大回撤", "换手率",
            "交易次数", "平均仓位", "平均持仓时长", "动作序列 SHA-256",
            "相对 Always Flat 差值", "相对规则基线差值", "相对 Oracle regret",
        ],
        "hard_fail_conditions": [
            "任一反作弊考试判定 SUSPECTED_CHEATING",
            "Null Control 出现稳定正超额收益或持续高换手",
            "真信号消融后优势不下降",
            "成本提高后净值系统性上升",
        ],
        ...
        "timeframe": "15m",
        "fee": 0.001,
        "curriculum_infra_version": "rl-curriculum-stage2_6_0-v1",
    }
