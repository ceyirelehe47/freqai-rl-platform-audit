...
from rl_platform.versions import spec_versions  # noqa: E402
...
def build_execution_contract_manifest(route_c: dict) -> dict:
    ...
    return {
        "spec_versions": spec_versions(),  # 阶段 2.6.0 工作包 0:冻结版本
...
def render_config(args) -> dict:
    ...
    # 阶段 2.6.0 工作包 0:冻结规范版本进入 config -> 进入 config_hash
    # -> 进入指纹;版本变化必然改变指纹与 identifier。
    rc.update(spec_versions())
...
def build_manifest(conf: dict, fingerprint: str, identifier: str,
    ...
    return {
        ...
        "spec_versions": spec_versions(),  # 阶段 2.6.0 工作包 0
...
def main() -> int:
    ...
    if ret == 0 and pred_dir.is_dir():
        ...
        # 阶段 2.6.0 工作包 0:freqtrade 成功(退出码 0)但 backtesting_predictions
        # 不存在 -> 缓存后处理失败:整轮实验 invalid、非零退出码、manifest 记录
        # 原始异常;不得写成 SKIPPED 后继续成功;模型目录保留。
        missing_error = FileNotFoundError(
            f"freqtrade 退出码 0 但预测目录不存在: {pred_dir}")
        manifest["cache_content_manifest"] = {
            "error": repr(missing_error),
            "invalid": True,
            "self_check": "INCONSISTENT",
            "missing_predictions_dir": str(pred_dir),
            "freqtrade_exit_code": ret,
            "quarantine": None,
        ...
        manifest_path.write_text(
            json.dumps(manifest, indent=2, ensure_ascii=False))
        print(
            f"[run_experiment] 缓存后处理失败(退出码 {CACHE_PIPELINE_EXIT_CODE})"
            f": {missing_error!r}",
            file=sys.stderr,
        )
        return CACHE_PIPELINE_EXIT_CODE
        ...
        manifest["cache_content_manifest"] = {
            "state": "SKIPPED",
            "reason": f"freqtrade 退出码 {ret}(非 0),无预测目录可校验",
        }
