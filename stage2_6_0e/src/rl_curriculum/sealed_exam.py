...
SEALED_EXAM_PROTOCOL = "sealed-exam-commitment-v5"
...
class SealedExamCommitment:
    ...
    def from_json(text: str) -> "SealedExamCommitment":
        ...
        if data.get("protocol_version") != SEALED_EXAM_PROTOCOL:
            if data.get("protocol_version") in _DEPRECATED_PROTOCOLS:
                raise SealedExamError(
                    ...
                    f"为只看点估计(阶段 2.6.0e)——不得被 v5 执行器自动"
        ...
        if not runtime_manifest or not runtime_hash:
            raise SealedExamError(
                "v5 承诺必须绑定候选运行时内容(candidate_runtime_"
        ...
        if not spec_hash.startswith("nqs-"):
            raise SealedExamError(
                "v5 承诺必须绑定 Null 资格规范哈希(null_qualification_"
        ...
        if (not str(power.get("report_hash") or "").startswith("npa-")
            ...
            raise SealedExamError(
                "v5 承诺必须绑定完整功效分析(null_power_analysis:"
        ...
        if (not str(pack_validity.get("report_hash") or "").startswith("npv-")
            ...
            raise SealedExamError(
                "v5 承诺必须绑定实际 Null pack 的 pack-level validity"
        ...
        if not str(data.get("pack_builder_code_hash") or "").startswith(
                "npb-"):
            raise SealedExamError(
                "v5 承诺必须绑定 pack 构建算法 manifest 哈希(pack_"
                "builder_code_hash, npb-):必须绑定真实 builder"
                "(assemble/seed 推导/pair 顺序/attempt 循环/validator),"
                "只哈希 validator 文件的旧绑定已废除(阶段 2.6.0e D6;"
                "构建规则必须在候选 checkpoint 出现前冻结)")
...
def verify_sealed_commitment(
    ...
    *,
    ...
    # 12b. 完整 power report 重跑验证(阶段 2.6.0e 工作包 C:public
    ...
    # 12c. pack 构建算法 manifest 与 pack-level validity 绑定(D6/B2/
    #      B4:npb- 必须绑定真实 builder——assemble/seed 推导/pair 顺序/
    #      attempt 循环/validator;执行器在候选评估前对物化 pack 现算
    #      pack validity 并与承诺 hash 对账;此处校验结构自洽)
    from rl_curriculum.null_pack_validation import (
        pack_builder_manifest_hash as _npb_manifest_hash,
    )

    check("pack_builder_code_hash",
          commitment.pack_builder_code_hash == _npb_manifest_hash(),
          f"pack 构建算法 manifest 哈希不匹配:承诺 "
          f"{commitment.pack_builder_code_hash} vs 当前 "
          f"{_npb_manifest_hash()}(只哈希 validator 的旧绑定已废除;"
          f"builder/seed 推导/pair 顺序/attempt policy 任一变化都必须"
          f"创建新承诺)")
