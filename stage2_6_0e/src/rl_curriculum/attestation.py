"""阶段 2.6.0b 工作包 G + 阶段 2.6.0c 工作包 A:可信训练来源证明
(training attestation)与 issuer 信任根自洽校验。
...
class TrustedIssuerConfig:
    ...
    @staticmethod
    def from_payload(payload: dict[str, Any]) -> "TrustedIssuerConfig":
        """从 canonical issuer payload 完整重建(阶段 2.6.0c 工作包 A)。

        唯一合法来源是 sealed commitment 内的 trusted_issuer 字段;
        重建前先做自洽校验(verify_issuer_payload_self_consistency),
        protocol 字段必须显式读取——不再静默回落默认值(旧重建路径
        丢弃 protocol 使 issuer 协议不可校验)。
        """
        ...
        return TrustedIssuerConfig(
            ...
            required_training_runner_hash=str(
                payload["required_training_runner_hash"]),
            allow_smoke=bool(payload["allow_smoke"]),
            protocol=str(payload["protocol"]),
        )


#: issuer canonical payload 的精确键集合(缺失/多余键都拒绝;A2)
ISSUER_PAYLOAD_KEYS: frozenset[str] = frozenset({
    "protocol", "issuer_id", "public_key_pem", "key_fingerprint",
    "required_training_runner_hash", "allow_smoke",
})
#: 受信 runner hash 的最小格式约束(非空、可打印、有最低熵长度;A2)
_RUNNER_HASH_MIN_LEN = 8


def verify_issuer_payload_self_consistency(payload: dict[str, Any]) -> None:
    """阶段 2.6.0c 工作包 A2:commitment 内 issuer payload 自洽校验。

    在 attestation 验证之前执行,任何问题抛 AttestationError
    (调用方映射 EXAM_INVALID):
    - 键集合精确等于 canonical 键集合(不允许缺失/未识别字段);
    - protocol 必须等于当前受支持 attestation 协议版本;
    - issuer_id 非空;
    - public_key_pem 可加载为 Ed25519 公钥;
    - 按 public_key_pem 重新计算的 fingerprint 等于 payload 记录;
    - required_training_runner_hash 非空且格式合法;
    - allow_smoke 必须是 bool(它是承诺冻结的一部分,不得缺省)。
    """
    if not isinstance(payload, dict):
        raise AttestationError("issuer payload 不是对象")
    keys = set(payload)
    if keys != set(ISSUER_PAYLOAD_KEYS):
        missing = sorted(ISSUER_PAYLOAD_KEYS - keys)
        extra = sorted(keys - ISSUER_PAYLOAD_KEYS)
        raise AttestationError(
            f"issuer payload 键集合与 canonical 集合不一致:"
            f"缺失 {missing},未识别 {extra}")
    if payload["protocol"] != ATTESTATION_PROTOCOL:
        raise AttestationError(
            f"issuer 协议 {payload['protocol']!r} != {ATTESTATION_PROTOCOL!r}"
            f"(承诺绑定的 attestation 协议版本不受支持)")
    issuer_id = payload["issuer_id"]
    if not isinstance(issuer_id, str) or not issuer_id.strip():
        raise AttestationError("issuer_id 为空或非字符串")
    pem = payload["public_key_pem"]
    if not isinstance(pem, str) or "BEGIN PUBLIC KEY" not in pem:
        raise AttestationError("public_key_pem 不是 PEM 公钥文本")
    try:
        pub = serialization_load_public(pem.encode("utf-8"))
    except Exception as exc:  # noqa: BLE001
        raise AttestationError(
            f"public_key_pem 无法加载为 Ed25519 公钥: {exc!r}") from exc
    del pub  # 只验证可加载性,不产生其他副作用
    recomputed = key_fingerprint(pem.encode("utf-8"))
    if recomputed != payload["key_fingerprint"]:
        raise AttestationError(
            f"issuer 公钥指纹不自洽:按 public_key_pem 重算 {recomputed},"
            f"承诺记录 {payload['key_fingerprint']!r}(公钥与指纹不一致)")
    runner_hash = payload["required_training_runner_hash"]
    if (not isinstance(runner_hash, str)
            or len(runner_hash.strip()) < _RUNNER_HASH_MIN_LEN
            or any(c.isspace() for c in runner_hash)):
        raise AttestationError(
            "required_training_runner_hash 为空/过短/含空白:"
            f"{runner_hash!r}(格式不合法)")
    if not isinstance(payload["allow_smoke"], bool):
        raise AttestationError(
            "allow_smoke 必须是布尔值(承诺冻结字段,不得缺省或字符串化)")
