...
def audit_probe_charter() -> dict[str, Any]:
    ...
    return {
        ...
        "null_control_construction": (
            "正式多类 Null(结论必须跨族一致):probe_null_sign 符号随机化"
            "(保留 |收益| 与波动聚集,切断方向);probe_null_block 分块重排"
            "(保留块内结构,切断跨块关系);probe_null_volstate 波动状态"
            "条件随机化(档内置换,保留精确边际与波动档位,切断跨槽方向)。"
            "probe_null_control 全排列保留为探针,不单独构成 Null 结论;"
            "Fourier 相位替身经验证保留线性自相关(趋势规则仍有优势),"
            "已否决"
        ),
        "oracle": (
        ...
        "curriculum_infra_version": "rl-curriculum-stage2_6_0a-v1",
    }
