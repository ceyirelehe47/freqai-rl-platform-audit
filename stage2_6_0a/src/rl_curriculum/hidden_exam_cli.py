"""阶段 2.6.0a 工作包 E/F/G/H:密封隐藏考试 CLI(v2)。
...
正式(密封)模式:
    python -m rl_curriculum.hidden_exam_cli \
        ...
        [--detailed detailed.json] [--no-subprocess]

- --sealed-manifest 必填:逐项验证预承诺(pack/charter/observation
  schema/spec versions/generator 代码哈希/evaluator/counterfactual/
  verdict/EvalConfig/checkpoint 要求);任一不匹配 -> EXAM_INVALID;
- 考试条件(fee/滑点/初始资金/窗口/确定性)由 sealed manifest 与
  考试上下文冻结;--fee/--slippage/--window-size/--initial-cash 在
  正式模式提供即拒绝(不存在"忽略哈希/强制继续"参数);
- checkpoint 必须 formal_eligible=true(v2 sidecar + 章程 + observation
  绑定);legacy/smoke checkpoint 拒绝;
- PASS/FAIL/SUSPECTED_CHEATING/EXAM_INVALID 由冻结判定器产生,
  不再是 overall median > 0;
- 默认输出最小化(minimal_hidden_output);--detailed 写出详细结果的
  同时退休该考试包,再次评估被拒;
...
CLI_VERSION = "hidden-exam-cli-v2"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(description="密封隐藏考试 CLI(v2)")
