"""工作包 C + 阶段 2.6.0a 工作包 E2/K:可插拔人工市场生成器协议。
...
def resolve_duration(params: dict[str, Any], timeframe: str) -> dict[str, Any]:
    """工作包 E2:Episode 真实时长解析(原始值与解析结果全部入哈希)。

    - params 含 duration_hours:按 rounding 规则(默认 ceil)解析为 bars;
    - params 含 episode_bars:bars 即原始值,反推真实时长;
    - 两者同给必须一致,否则 GeneratorError。
    """
    from rl_curriculum.timebase import bars_to_duration_hours, duration_to_bars
    ...
    if has_dur:
        ...
        if has_bars and int(params["episode_bars"]) != bars:
            raise GeneratorError(
                f"duration_hours={hours} 在 {timeframe} 下解析为 {bars} bars,"
                f"与显式 episode_bars={params['episode_bars']} 不一致")
        return {
            "timeframe": timeframe,
            ...
            "resolved_real_hours": bars_to_duration_hours(bars, timeframe),
        }
    bars = int(params["episode_bars"])
    return {
        "timeframe": timeframe,
        "source": "episode_bars_direct",
        "requested_real_hours": bars_to_duration_hours(bars, timeframe),
...
class BaseMarketGenerator(ABC):
    ...
    def generate(self, params: dict[str, Any], seed: int, split: str = "train",
        ...
        df = self._attach_nuisance_slots(df, params, seed)
...
def _verify_feature_causality(
    ...
    for cut in sorted({max(n // 2, 1), n}):
        ...
        rebuilt = generator._attach_nuisance_slots(  # noqa: SLF001
            rebuilt, episode.spec.params, episode.spec.seed)
