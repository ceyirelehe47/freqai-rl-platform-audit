"""工作包 F/N + 阶段 2.6.0a 工作包 A/D:确定性 Episode 评估器与统计纪律。
...
- Episode 生命周期:每 Episode reset_episode(derived_seed),derived_seed
  由 EpisodeSpec 规范化哈希派生(与输入顺序无关);
...
def run_observation_episode(
    ...
    policy.reset_episode(derive_episode_seed(episode.spec))
    actions: list[int] = []
    equities: list[float] = []
    ...
    while not done:
        ...
        action = int(policy.act(obs))  # 只有 observation
...
def run_test_probe_episode(
    ...
    env = _build_env(episode, cfg, schema)
    obs, _ = env.reset(seed=episode.spec.seed)
    probe.reset_episode(derive_episode_seed(episode.spec))
