...
## 6. 时序证据级别勘误(2026-09-25;RouteC_FullCollection_
ResearchDesign_NextGoal_v1 §4)

§2 条目"计划先于训练固定"的证据级别收窄:`plan mtime 08:38:13Z
< results 完成时间 08:42:31Z` **只支持"计划文件的记录时间早于
结果完成"**,不能单独证明计划先于**训练开始**——训练开始时刻
没有启动日志存档,4 分钟间隔内"先训练、后补写计划"在纯 mtime
层面未被排除。`lock_g5c_plan` 一次性写入与 `run_g5c` 启动时
load/digest 重验的**代码设计**是辅助证据,不能替代缺失的当时
启动记录;本条不重算任何旧表格、不重训、不改动 G5c 数值与
"运行命令/解释器/cwd/stdout/rc 无原始存档"的永久披露。不能用
今天的源码或文件时间倒填历史。A/C 轮(2026-09-25)起的新执行
从开始即保留执行原件,避免形成同类缺口。
