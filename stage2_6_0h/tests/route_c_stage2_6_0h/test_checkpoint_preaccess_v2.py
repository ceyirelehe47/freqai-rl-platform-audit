...
- os.stat/os.access/os.readlink 在 CPython 3.11 无审计事件:stat 级
  不可利用由 pivot_root namespace 不可命名保证(沙箱报告探针);
- Candidate 沙箱未启动:Builder 阶段出现的非 Builder-Runner 子进程
  Popen 记为违规;
- Runner 沙箱内的宿主路径探针全部 ENOENT(真实隔离 Runner)。
"""
...
def test_runner_sandbox_host_paths_unnameable(sealed_exam_env,
                                              private_builder_a,
                                              tmp_path):
    """G(方案 4):真实隔离 Runner 的沙箱报告探针证明宿主路径
    ...
    probes = sb["probes"]
    assert probes["host_etc_unnameable"]["result"] == "ENOENT"
    assert probes["host_sys_unnameable"]["result"] == "ENOENT"
    assert probes["host_home_unnameable"]["result"] == "ENOENT"
    assert probes["dev_shm_private"]["listing"] == []
