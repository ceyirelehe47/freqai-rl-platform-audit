"""工作包 F:evidence v2 核心哈希与对账。

- 新核心字段:effective_sandbox_hash / access_summary_hash /
  process_tree_policy / child_process_count / exec_count /
  runner_isolation;
- verify 重算全部核心哈希;detail.sandbox_report 重算 esb-;
  detail.access_summary 重算 acs-;
- 篡改 detail 的 sandbox/access 绕过 bre- 的攻击被拒绝;
- 隔离降级(seccomp 关)的 evidence 无法通过 verify。
"""
...
def test_core_fields_include_new_hashes():
    for field in ("effective_sandbox_hash", "access_summary_hash",
...
def test_mock_evidence_carries_new_fields(sealed_exam_env):
    ...
    assert ev["effective_sandbox_hash"] == "esb-public-assembly"
...
def test_private_evidence_requires_real_sandbox(sealed_exam_env):
    """私有通道 evidence 的 esb- 哨兵必须真实(不能是组装哨兵)。"""
    from rl_curriculum.builder_evidence import (
        MOCK_SANDBOX_HASH,
    ...
    run = {
        ...
        "access_summary": {},
        "effective_sandbox_hash": MOCK_SANDBOX_HASH,  # 哨兵冒充
    }
    with pytest.raises(BuilderProvenanceError, match="esb-"):
        build_builder_run_evidence(
            identity=env["provider"].builder_identity(),
...
def test_descendant_run_rejected_in_evidence_build(sealed_exam_env):
    ...
    run = {
        ...
        "access_summary": {},
        "effective_sandbox_hash": "esb-" + "9" * 64,
        "child_process_count": 1,
        "exec_count": 0,
        "process_tree_policy": PROCESS_TREE_SINGLE,
    }
    with pytest.raises(BuilderProvenanceError, match="esb-|后代|进程树"):
        build_builder_run_evidence(
            identity=env["provider"].builder_identity(),
            request=env["commitment"].builder_build_request,
            runs=[dict(run), dict(run)],
