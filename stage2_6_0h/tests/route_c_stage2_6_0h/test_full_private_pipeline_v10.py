"""阶段 2.6.0h:工作包 I——完整 Private 正式链路 v10。

- 测试私有 Builder(40 pair / 5 attempts,真实 attempt log first_pass);
- 两次 precommit(独立隔离 Runner:seccomp 进程树 + pivot_root +
  私有 /dev)确定性运行;
- Builder Run Evidence v2(esb-/acs-/进程树) -> commitment v9 ->
  正式考试第三次重放(六组一致性);
- duration/power/pack validity 全链对账;受信 training attestation;
- Candidate 沙箱;256-step PPO smoke 正常 FAIL;
- CLI v10 --builder-evidence;坏 evidence(tamper esb-)拒绝。
"""
...
def private_chain(tmp_path_factory, null_qual_chain, schema, cfg):
    ...
    双跑 evidence v2 + 私有 pack + commitment v9。"""
...
def test_evidence_v2_core_hashes_present(private_chain):
    ev = private_chain["evidence"]
    assert ev["format"] == "builder-run-evidence-v2"
...
def test_commitment_v9_binds_policy_and_evidence(private_chain):
    ...
    assert c.builder_run_evidence["effective_sandbox_hash"].startswith(
        "esb-")
...
def test_full_pipeline_exam_replay_and_normal_fail(
    ...
    """完整正式链路:CLI v10;第三次重放六组一致;PPO smoke FAIL。"""
    ...
    assert out["exam_cli_version"] == "hidden-exam-cli-v10"
    ...
    assert prov["replay_effective_sandbox_hash"] == \
        ch["evidence"]["effective_sandbox_hash"]
...
def test_tampered_evidence_rejected(private_chain, private_checkpoint,
                                    tmp_path):
    """evidence 的 esb- 被篡改(重签)后 formal 拒绝。"""
    ...
    evidence["detail"]["sandbox_report"]["seccomp_mode"] = 0
