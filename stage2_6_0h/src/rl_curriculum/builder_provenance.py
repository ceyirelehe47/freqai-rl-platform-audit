...
#: 运行时依赖锁格式(D1/D2/D3)
RUNTIME_LOCK_FORMAT = "builder-runtime-lock-v2"
#: 访问摘要格式(F)
...
def runtime_lock_hash(lock: dict[str, Any]) -> str:
    """运行时依赖锁 v2 的 canonical hash(nrl-;进入 evidence)。"""
...
def check_runtime_lock_against_static(
    ...
    *, require_single_process: bool = True,
    ...
    """实际运行时锁与静态闭包预检的对账(G3 + D1/D2)。

    v2 新增:
    - 进程树:require_single_process 时 process_tree_policy 必须是
      single_builder_process 且 child_process_count==exec_count==0;
    - distribution 内容:verify_content 时在主进程重算每个 distribution
      的实际内容摘要(dcd-)并与锁内记录对账——修改 package 文件但
      保持 RECORD 不变的篡改在此被发现(RECORD 声明哈希与实际文件
      哈希逐条重算);
    - native library:锁必须携带内容绑定条目(D3)。
    """
    if not isinstance(lock, dict) or lock.get(
            "format") != RUNTIME_LOCK_FORMAT:
        raise BuilderProvenanceError(
    ...
    if verify_content:
        ...
        for entry in lock.get("distributions") or []:
            ...
            if not dist_name or not expected_digest.startswith("dcd-"):
                raise BuilderProvenanceError(
                    f"运行时锁的 distribution {dist_name!r} 缺少实际"
                    f"内容摘要 dcd-(D2:只哈希 RECORD 不检查实际文件"
                    f"的 v1 锁不再被接受)")
            try:
                dist = md.distribution(dist_name)
            except Exception as exc:  # noqa: BLE001
                raise BuilderProvenanceError(
                    f"distribution {dist_name!r} 当前不可解析: "
                    f"{exc}") from exc
            from rl_builder_runtime.runner import (
                distribution_content_digest,
            )

            try:
                recomputed = distribution_content_digest(dist)
            except Exception as exc:  # noqa: BLE001
                raise BuilderProvenanceError(
                    f"distribution {dist_name!r} 实际内容验证失败"
                    f"(RECORD 与实文件不符;fail closed): {exc}") from exc
            if recomputed != expected_digest:
                raise BuilderProvenanceError(
                    f"distribution {dist_name!r} 实际内容摘要与锁记录"
                    f"不一致(重算 {recomputed} vs 锁 {expected_digest};"
                    f"package 文件被修改而 RECORD 未更新,或依赖环境"
                    f"漂移;旧 evidence 失效)")
...
def verify_builder_provenance(
    ...
    if builder_evidence is None:
        raise BuilderProvenanceError(
            "缺少 Builder Run Evidence(v9 承诺必须绑定 builder_run_"
    ...
    # F:重放的沙箱/访问/进程树身份与 evidence 对账
    if str(replay.get("effective_sandbox_hash") or "") != str(
            builder_evidence.get("effective_sandbox_hash") or ""):
        raise BuilderProvenanceError(
            "重放的 Effective Sandbox Report 哈希与 evidence 不一致"
            "(实际生效沙箱状态漂移;C2/F;EXAM_INVALID)")
    ...
    return {
        ...
        "replay_runtime_lock_hash": runtime_lock_hash(
            replay["runtime_lock"]),
        "replay_effective_sandbox_hash": str(
            replay.get("effective_sandbox_hash") or ""),
