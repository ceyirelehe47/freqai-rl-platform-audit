...
SEALED_EXAM_PROTOCOL = "sealed-exam-commitment-v9"
...
class SealedExamCommitment:
    ...
    def from_json(text: str) -> "SealedExamCommitment":
        ...
        if not str(run_evidence.get("effective_sandbox_hash") or ""):
            raise SealedExamError(
                "v9 承诺的 builder_run_evidence 必须绑定实际生效沙箱"
                "报告哈希(effective_sandbox_hash;esb- 或组装通道哨兵)"
                "——没有 effective sandbox report 的材料被拒绝(阶段"
                " 2.6.0h C2;EXAM_INVALID)")
...
def verify_sealed_commitment(
    ...
    *,
    ...
    ev_ok = (
        ...
        and bool(ev.get("effective_sandbox_hash"))
    ...
    check("builder_run_evidence_binding", ev_ok,
          ...
          f"nbr/pack 或 effective_sandbox/access_summary/process_tree"
