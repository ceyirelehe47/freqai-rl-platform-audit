"""Builder Run Evidence 与确定性证明(阶段 2.6.0h:v2)。

v2 语义(在 0g v1 之上):evidence 核心字段新增——
- effective_sandbox_hash(esb-,实际生效沙箱状态报告:C1/C2);
- access_summary_hash(acs-,访问摘要 v2:F);
- process_tree_policy / child_process_count / exec_count(D1);
- runner_isolation(隔离进程身份)。

公开组装通道(mock_payload_assembly)使用确定性哨兵
(esb-public-assembly / in_process_public_assembly);私有通道
必须携带真实 esb- 且 detail.sandbox_report 重算哈希一致。

一致性从"三组 hash"升级为六组:pack / attempt log / runtime lock /
effective sandbox / access summary + 进程树计数。precommit 双跑与
考试期第三次重放均按六组对账。
"""
...
BUILDER_RUN_EVIDENCE_FORMAT = "builder-run-evidence-v2"

#: 组装通道哨兵(公开组装无 Runner 沙箱;确定性字面量)
MOCK_SANDBOX_HASH = "esb-public-assembly"
...
EVIDENCE_CORE_FIELDS: tuple[str, ...] = (
    ...
    "deterministic",
    "effective_sandbox_hash", "access_summary_hash",
    "process_tree_policy", "child_process_count", "exec_count",
    "runner_isolation",
    "runs",
)
...
def _run_consistency(run: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "effective_sandbox_hash": str(
            run.get("effective_sandbox_hash") or ""),
...
def build_builder_run_evidence(
    *, identity: Any, request: dict[str, Any], runs: list[dict[str, Any]],
    ...
    """从两次 precommit run record 组装完整 Builder Run Evidence(v2)。"""
    if len(runs) != 2:
        raise BuilderProvenanceError(
    ...
    if mode == BUILDER_RUN_MODE_EXECUTION:
        sandbox_hash = str(run.get("effective_sandbox_hash") or "")
        if not sandbox_hash.startswith("esb-") or \
                sandbox_hash == MOCK_SANDBOX_HASH:
            raise BuilderProvenanceError(
                "私有通道 evidence 缺少真实 Effective Sandbox Report"
                " 哈希(esb-;组装哨兵冒充/缺失均 fail closed;C2)")
    ...
    core = {
        ...
        "deterministic": True,
        "effective_sandbox_hash": sandbox_hash,
    ...
    evidence["detail"] = {
        ...
        "sandbox_report": dict(run.get("effective_sandbox") or {}),
...
def verify_builder_run_evidence(
    ...
    if not summary:
        raise BuilderProvenanceError(
            "承诺缺少 builder_run_evidence 绑定(v9 承诺必须绑定 "
    ...
    if mode == BUILDER_RUN_MODE_EXECUTION:
        if not str(core.get("effective_sandbox_hash") or "").startswith(
                "esb-"):
            raise BuilderProvenanceError(
                "私有通道 evidence 缺少真实 Effective Sandbox Report"
                " 哈希(没有实际生效沙箱证明的材料被拒绝;C2;EXAM_INVALID)")
        ...
        if core.get("effective_sandbox_hash") != MOCK_SANDBOX_HASH:
            raise BuilderProvenanceError(
                "组装通道 evidence 的沙箱哨兵不符(EXAM_INVALID)")
    ...
    # summary/sandbox report 全部重算哈希对账(篡改 detail 绕过 bre-
    # 的攻击被拒绝)
    detail = evidence.get("detail") or {}
    if not isinstance(detail, dict):
        raise BuilderProvenanceError(
            "evidence 缺少 detail(完整运行证据:runtime_lock/attempt_"
            "log/access 摘要/sandbox report;不能只信任核心摘要)")
    try:
    ...
    if mode == BUILDER_RUN_MODE_EXECUTION:
        ...
        report = dict(detail.get("sandbox_report") or {})
        if not report:
            raise BuilderProvenanceError(
                "私有通道 evidence.detail 缺少 sandbox_report(没有"
                "实际生效沙箱报告的材料被拒绝;EXAM_INVALID)")
        try:
            detail_sandbox_hash = sandbox_report_hash(report)
        except Exception as exc:  # noqa: BLE001
            raise BuilderProvenanceError(
                f"evidence.detail.sandbox_report 无法规范化: "
                f"{type(exc).__name__}: {exc}") from exc
        if detail_sandbox_hash != str(
                core.get("effective_sandbox_hash") or ""):
            raise BuilderProvenanceError(
                "evidence.detail.sandbox_report 重算哈希与核心"
                " effective_sandbox_hash 不一致(detail 被改写;"
                "EXAM_INVALID)")
        from rl_curriculum.builder_runner import (
        ...
        try:
            ...
            raise BuilderProvenanceError(
                f"evidence 的 Effective Sandbox Report 未通过不变量"
                f"校验(隔离降级;C3;EXAM_INVALID): {exc}") from exc
...
def replay_builder_for_evidence(
    ...
    """E3:考试期第三次重放(全新 Runner/组装)并对账 evidence(v2)。

    六组一致性键(pack/attempt log/runtime lock/effective sandbox/
    access summary + 进程树计数)必须与 precommit 完全一致;runner
    code/sandbox profile 同步对账。
    """
    ...
    if view != expected:
        raise BuilderProvenanceError(
            ...
            f"种子/依赖漂移/沙箱漂移等不确定性;Builder 确定性或沙箱"
            f"状态不成立;EXAM_INVALID)")
...
def load_builder_run_evidence(path: Path | str) -> dict[str, Any]:
    ...
    if not isinstance(evidence, dict) or evidence.get(
        ...
        raise BuilderProvenanceError(
            f"Builder Run Evidence 必须是 {BUILDER_RUN_EVIDENCE_FORMAT!r}"
            f" dict(收到 {type(evidence).__name__};v1 evidence 已被"
            f" v2 执行器拒绝)")
