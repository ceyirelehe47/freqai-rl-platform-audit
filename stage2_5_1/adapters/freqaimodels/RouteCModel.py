"""阶段 2.5.1 路线 C:FreqAI 薄适配模型(阶段 2.5 基础上加固)。
...
class RouteCModel(BaseReinforcementLearningModel):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        # 唯一 PPO 参数来源:freqai.route_c.ppo(冲突即报错,工作包 A)
        ...
        # None 表示尚未初始化(live 下由 Trade 状态解析,回测下为 0)。
        # 仅用于 backtest 跨窗口延续;不得成为 live 实际仓位的唯一来源(工作包 G)。
        self._last_target_position: int | None = None
        # 每窗 PPO 预算记录(工作包 A;证据脚本从模型目录 json 与此列表双路读取)
        self.ppo_budget_records: list[dict[str, Any]] = []
        # live heartbeat trace(工作包 G;测试与证据脚本读取)
    ...
    def set_train_and_eval_environments(
        ...
        净值 log 奖励、终端强制清算、当根 high/low 价格限制(工作包 C)。"""
    ...
    def rl_model_predict(
        ...
        live:live_predict_frame 历史回填(隔离状态)与实时决策(Trade 真值)
        分离;每个 heartbeat 重新读取 Trade 表(工作包 G)。
        两分支都接收 dk.do_predict mask:无效行不更新目标状态(工作包 F)。
        """
        ...
        if self.live:
            ...
            actions, trace = live_predict_frame(
                ...
                pair=dk.pair,
                window_size=self.CONV_WIDTH,
                read_position_fn=get_initial_position_live,
                do_predict=dp,
