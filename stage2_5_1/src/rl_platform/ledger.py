"""Long/Flat 净值账本(阶段 2.5 路线 C 核心模块,阶段 2.5.1 工作包 C 加固)。
...
阶段 2.5.1 价格限制(工作包 C 第九节,镜像 Freqtrade 回测器规则):
- 买入成交价 = min(请求价, 执行 bar 的 high)
- 卖出成交价 = max(请求价, 执行 bar 的 low)
即 custom_entry_price / custom_exit_price 在回测器中被 clamp 的同一规则;
两侧使用同一最终成交价,窄 K 线下请求价超出当根 high/low 时不再分叉。

终端清算口径修正(阶段 2.5.1):
- 阶段 2.5 的 liquidate 按卖出滑点清算;
- Freqtrade 回测器对回测结束仍持仓的 trade 用最后一根 bar 的 open 平仓
  (handle_left_open),不走 custom price,不收滑点,只扣手续费;
- 为消除"训练有滑点、回测无滑点"的不一致(任务书三十三节 FAIL 条件),
  本阶段起 liquidate 与回测器对齐:按 raw_open 成交,无滑点,扣手续费。
"""
...
class LongFlatLedger:
    ...
    买入:全部可用现金按成交价(含滑点,并限制在当根 high 内)买入,
    手续费按名义金额计,约束 notional + fee = cash,
    即 qty = cash / (exec_price * (1 + fee))。
    卖出:全部 BTC 按成交价(含滑点,并限制在当根 low 内)卖出,
    所得 = notional * (1 - fee)。
    """
    ...
    def _buy_all(self, raw_open: float, high: float | None = None) -> TradeRecord:
    ...
    def _sell_all(
        self, raw_open: float, low: float | None = None, direction: str = "sell"
    ) -> TradeRecord:
        requested = raw_open * (1.0 - self.slippage_bps / 10000.0)
