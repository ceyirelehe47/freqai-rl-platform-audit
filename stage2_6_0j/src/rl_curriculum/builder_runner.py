"""隔离 Builder Runner 启动器(阶段 2.6.0j,评估主进程侧)。

主评估进程对私有 Builder 只允许(沿 0g/0h/0i):

- 读取并哈希 Builder 文件(builder package tree manifest;npb-);
- AST 静态检查(builder_identity 静态部分);
- 创建隔离 Runner(本模块);发送规范化 request;接收 v4 响应。

阶段 2.6.0j 沙箱层次(bootstrap 纯 syscall 布置,deny 降级即拒绝):
    ...
    fork launcher(userns[父写映射]/mountns/netns/utsns 固定 hostname)
      ...
      -> Worker Prepare:v2 filter + vDSO 冻结 stub + PR_SET_TSC +
         native 预加载 + 顶层纯度 AST + 导入闭包 v4(全部在 build 前)
      -> quiesce 握手:Supervisor 外部 /proc 实测(maps/task/status)
      -> Worker Seal(ACK 后不可逆):PR_SET_MDWE + fd 隔离(stdin 关
         闭、1/2->/dev/null、真实管道仅 RESULT_FD) + final compute
         filter(default deny + 参数过滤)
      -> Worker Compute:build_pack 纯计算 -> final(RESULT_FD 单向)
      -> Supervisor 二次外部实测(线程/进程/seccomp/映射)并校验
      -> 运行后 staging 全量复验(TOCTOU/E10)
...
class BuilderRunnerProfile:
    ...
    阶段 2.6.0j(v4):在 v3 之上绑定不可逆密封计算边界——native
    allowlist 预加载、模块顶层纯度 AST 验证、PR_SET_MDWE(W^X)、
    Worker fd 隔离(stdin 关闭、RESULT_FD 单向输出通道)、final
    compute filter(default deny + 最小 allowlist + 参数过滤)。
    install_seccomp=False 或 process_tree_policy=allow_descendants
    仅供攻击演示测试——evidence 校验要求密闭输入证明,此类 run 无法
    进入正式材料。dependency_profile=compat 允许 NumPy 等第三方
    native 依赖,但 evidence 层拒绝其形成可信材料(formal_eligible
    =false)。
    """
    ...
    #: A3:依赖面策略(formal=纯 Python+受审计 native allowlist;
    #: compat=放开第三方 native 但 formal_eligible=false)
    dependency_profile: str = DEP_PROFILE_FORMAL
    ...
    def __post_init__(self) -> None:
        ...
        if self.dependency_profile not in (DEP_PROFILE_FORMAL,
                                           DEP_PROFILE_COMPAT):
            raise BuilderRunnerError(
                f"未知依赖策略 {self.dependency_profile!r}")
    ...
    def canonical_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "sealed_compute": {
                ...
                "mdwe": "PR_SET_MDWE_REFUSE_EXEC_GAIN"
                        "(kernel>=6.14;else final-filter-no-exec)",
                "fd_isolation": {
                    "stdin": "closed-after-ack",
                    "stdout_stderr": "devnull",
                    "result_fd": RESULT_FD,
                    "result_channel": "runner-only-write-after-build",
                },
                "top_level_purity": "ast-allowlist+runtime-audit",
                "dependency_policy": self.dependency_profile,
                "compute_forbidden_events": [
                    "import", "open", "os.listdir", "os.scandir",
                    "os.system", "subprocess.Popen", "ctypes.dlopen",
                    "ctypes.dlsym", "os.fork", "os.exec", "os.posix_spawn",
                    "os.spawn", "exec", "compile", "builtins.input",
                ],
                "precommit_double_run": True,
                "third_replay_in_exam": True,
            },
            ...
            "deterministic_input_report": SEALED_COMPUTE_REPORT_FORMAT,
            "env_whitelist": [
                ...
                "RL_BUILDER_PROCESS_TREE", "RL_BUILDER_DEP_PROFILE",
...
# ------------------------------------------------------------ launcher
def _fixed_exec_env(process_tree_policy: str,
                    dependency_profile: str = DEP_PROFILE_FORMAL,
                    ) -> dict[str, str]:
    ...
    return {
        ...
        "RL_BUILDER_DEP_PROFILE": dependency_profile,
    }
...
def _quiesce_snapshot(worker_pid: int) -> dict:
    ...
    2.6.0j:新增 Seccomp filter 计数与 exec 映射计数(Compute 前后
    对比,证明 final filter 已叠加且 Compute 未新增可执行内存)。
    """
    snap: dict[str, Any] = {}
    status: dict[str, str] = {}
    ...
    snap["seccomp_filter_count"] = int(
        status.get("Seccomp_filters", "0") or 0)
    ...
    snap["exec_mapping_count"] = int(exec_mappings)
    return snap
...
def check_effective_deterministic_input_report(
    ...
    """D2/F1:sealed compute report v2 的不变量校验(降级即拒绝)。"""
    from rl_builder_runtime import PRIVATE_DEV_NODES
    from rl_builder_runtime.runner import (
        SECCOMP_PROCESS_POLICY,
        seccomp_filter_digest,
    ...
    if report.get("format") != SEALED_COMPUTE_REPORT_FORMAT:
        raise BuilderRunnerError(
            f"确定性输入报告 format 必须是 {SEALED_COMPUTE_REPORT_FORMAT!r}"
            f"(收到 {report.get('format')!r};2.6.0i 的 edic-v1 材料已"
            f"不足以表达 Seal 后计算边界)")
    ...
    if profile.process_tree_policy == SINGLE_PROCESS:
        ...
        # ---- 2.6.0j:Seal 后计算边界证明(sealed_compute 块) ----
        sc = report.get("sealed_compute") or {}
        if not sc:
            raise BuilderRunnerError(
                "sealed compute report 缺少 sealed_compute 块(拒绝)")
        if sc.get("phase_plan") != "prepare->seal->compute":
            raise BuilderRunnerError(
                f"阶段计划异常: {sc.get('phase_plan')!r}(拒绝)")
        fin = sc.get("final_seccomp") or {}
        if fin.get("policy") != FINAL_COMPUTE_POLICY:
            raise BuilderRunnerError(
                "final compute filter 策略载荷与期望不符(拒绝)")
        if fin.get("filter_digest") != final_filter_digest():
            raise BuilderRunnerError(
                "final compute filter 摘要与期望不符(拒绝)")
        if fin.get("default_action") != "EPERM":
            raise BuilderRunnerError(
                f"final compute filter default action 必须是 EPERM"
                f"(收到 {fin.get('default_action')!r};default allow 的"
                f"2.6.0i 语义已被拒绝)")
        purity = sc.get("top_level_purity") or {}
        if purity.get("all_ok") is not True or not purity.get("digest"):
            raise BuilderRunnerError(
                "builder 模块顶层纯度验证未通过或摘要缺失(拒绝)")
        dep = sc.get("dependency_policy") or {}
        if dep.get("format") != "builder-dependency-policy-v1":
            raise BuilderRunnerError("依赖策略载荷缺失(拒绝)")
        if profile.dependency_profile == DEP_PROFILE_FORMAL:
            if dep.get("profile") != "formal" \
                    or dep.get("formal_eligible") is not True:
                raise BuilderRunnerError(
                    "正式链依赖策略必须是 formal(拒绝)")
            if dep.get("third_party_native") != "rejected":
                raise BuilderRunnerError(
                    "正式链第三方 native 依赖必须被拒(拒绝)")
        sup = report.get("supervisor") or {}
        seal_after = sup.get("compute_after") or {}
        if profile.process_tree_policy == SINGLE_PROCESS:
            if seal_after.get("thread_count") != 1:
                tc = seal_after.get("thread_count")
                raise BuilderRunnerError(
                    f"Compute 后二次实测线程数 {tc}(必须为 1;拒绝)")
            if seal_after.get("child_process_count") != 0:
                raise BuilderRunnerError(
                    "Compute 后二次实测存在后代进程(拒绝)")
            if seal_after.get("seccomp_mode") != 2:
                sm = seal_after.get("seccomp_mode")
                raise BuilderRunnerError(
                    f"Compute 后 seccomp 模式 {sm}(必须为 2;拒绝)")
            if seal_after.get("seccomp_filter_count", 0) < 2:
                fc = seal_after.get("seccomp_filter_count")
                raise BuilderRunnerError(
                    f"Compute 后 seccomp filter 数量 {fc} < 2(final "
                    f"compute filter 未叠加;拒绝)")
            if seal_after.get("exec_mapping_growth") is not None \
                    and seal_after.get("exec_mapping_growth") != 0:
                g = seal_after.get("exec_mapping_growth")
                raise BuilderRunnerError(
                    f"Compute 后 exec 映射数增长 {g}"
                    f"({seal_after.get('exec_mapping_count')} vs 前;"
                    f"可执行内存新增;拒绝)")
            fd_iso = (sup.get("seal_state") or {}).get("fd_isolation") or {}
            if fd_iso.get("stdin") != "closed":
                raise BuilderRunnerError(
                    "Seal fd 隔离 stdin 状态未证明关闭(拒绝)")
            fin_installed = ((sup.get("seal_state") or {}).get(
                "final_filter") or {})
            if profile.process_tree_policy == SINGLE_PROCESS:
                if fin_installed.get("installed") is not True:
                    raise BuilderRunnerError(
                        "final compute filter 未被证明安装(拒绝)")
                if fin_installed.get("filter_hash") != final_filter_digest():
                    raise BuilderRunnerError(
                        "final compute filter 实际安装哈希与期望不符(拒绝)")
            if fd_iso.get("result_fd") != RESULT_FD:
                raise BuilderRunnerError(
                    f"RESULT_FD 必须是 {RESULT_FD}(拒绝)")
            mdwe = (sup.get("seal_state") or {}).get("mdwe") or {}
            if mdwe.get("enabled") is not True \
                    and not (mdwe.get("supported") is False):
                raise BuilderRunnerError(
                    "PR_SET_MDWE 未启用且无不支持证明(final filter 参数"
                    "过滤必须作为记录在案的同级后端;拒绝)")
...
def run_isolated_builder_run(
    ...
    """一次完整的密闭 Runner 构建(私有 Builder 专用;2.6.0j 语义)。

    序列:staging 复制与双对账 -> bundle 组装(链内复用)-> fork
    launcher 沙箱启动(挂载后全量复验在 Worker exec 前完成)-> 发送
    冻结请求 -> Worker Prepare(纯度/闭包/EDIC v2)-> quiesce 外部
    实测 -> ACK -> Worker Seal(MDWE/fd/final filter)-> Compute ->
    final(RESULT_FD)-> Supervisor 二次外部实测 -> ACK2 -> EDIC v2
    不变量校验 -> 锁 v4 组装(导入闭包 + native 闭包 + 线程静止 +
    bundle 绑定 + sealed_compute)-> 运行后 staging 全量复验 ->
    返回 run record。
    """
    import rl_builder_runtime
    from rl_builder_runtime import BUILDER_WORKER_PROTOCOL
    from rl_builder_runtime.sealed_compute import final_filter_digest
    from rl_curriculum.builder_provenance import (
    ...
    try:
        ...
        try:
            ...
            if first.get("phase") == "quiesce":
                ...
                if not line2.strip():
                    raise BuilderRunnerError(
                        "Compute 后无响应(Worker 死于 sealed compute 边界"
                        "或崩溃;fd1/2 已是 /dev/null,无诊断字节可读;"
                        "fail closed)")
                resp = json.loads(line2)
                # 2.6.0j:final 帧后、Worker 退出前的二次外部实测
                # (Compute 前后对比:线程/进程/seccomp 叠加/exec 映射)
                after = _quiesce_snapshot(int(state["worker_pid"]))
                after_snapshot = {
                    "thread_count": after["thread_count"],
                    "child_process_count": len(after["child_pids"]),
                    "seccomp_mode": after["seccomp_mode"],
                    "seccomp_filter_count": after["seccomp_filter_count"],
                    "no_new_privs": after["no_new_privs"],
                    "exec_mapping_count": after["exec_mapping_count"],
                    "exec_mapping_growth": after["exec_mapping_count"]
                    - quiesce["exec_mapping_count"],
                    "thread_growth": after["thread_count"]
                    - quiesce["thread_count"],
                }
                # 释放 Worker(其阻塞在 RESULT_ACK_FD read 上)
                try:
                    proc.stdin.write("ACK2\n")
                    proc.stdin.flush()
                except (BrokenPipeError, OSError):
                    pass  # Worker 已退出(异常路径);实测已完成
                state["after_snapshot"] = after_snapshot
            else:
                resp = first
        ...
        seal_state = dict(resp.get("seal_state") or {})
        if not isinstance(result, dict) or not lock_parts \
                or not edic:
            raise BuilderRunnerError(
                "Runner 响应缺 build_result/lock_parts/edic"
                "(已脱敏;fail closed)")
        if lock_parts.get("format") != RUNTIME_LOCK_FORMAT_V4:
            raise BuilderRunnerError(
                f"运行时锁必须是 {RUNTIME_LOCK_FORMAT_V4!r}(收到 "
                f"{lock_parts.get('format')!r};0i 的 v3 锁不再被接受)")
        ...
        edic["supervisor"] = {
            ...
            "compute_after": dict(state.get("after_snapshot") or {}),
        ...
        # 锁 v4 组装(worker 部分 + 外部实测)
        # 子进程计数取 max(外部实测后代, 审计尝试):短命子进程在 quiesce
        # 前退出时由审计计数兜底(沿 0h 语义;正式链两者都必须为 0)。
        # 2.6.0j:quiesce 行在 Compute 前发出,Compute 期审计计数由
        # final 帧的 compute_audit 补充(取两阶段最大值)
        compute_audit = dict(resp.get("compute_audit") or {})
        attempts = max(int(lock_parts.get("child_process_attempts")
                           or 0),
                       int(compute_audit.get("child_process_attempts")
                           or 0))
        exec_total = max(int(lock_parts.get("exec_attempts") or 0),
                         int(compute_audit.get("exec_attempts") or 0))
        child_count = max(len(quiesce["child_pids"]), attempts)
        lock = dict(lock_parts)
        lock["format"] = RUNTIME_LOCK_FORMAT_V4
        lock["child_process_count"] = int(child_count)
        lock["exec_count"] = exec_total
        ...
        lock["thread_state"] = {
            ...
            "task_comms": quiesce["task_comms"],
            "thread_count_after_compute": (
                edic["supervisor"]["compute_after"] or {}).get(
                    "thread_count"),
        }
        ...
        lock["sealed_compute"] = {
            "phase_plan": "prepare->seal->compute",
            "dependency_profile": lock_parts.get("dependency_profile"),
            "top_level_purity": (edic.get("sealed_compute") or {}).get(
                "top_level_purity"),
            "final_seccomp_filter_hash": (
                edic.get("sealed_compute") or {}).get(
                    "final_seccomp", {}).get("filter_digest"),
            "mdwe": seal_state.get("mdwe"),
            "fd_isolation": seal_state.get("fd_isolation"),
            "compute_after": edic["supervisor"]["compute_after"],
            "phase_violations": list(
                (access or {}).get("phase_violations") or []) + list(
                compute_audit.get("phase_violations") or []),
        }
        ...
        return {
            ...
            "final_seccomp_filter_hash": final_filter_digest(),
            ...
            "child_process_attempts": attempts,
