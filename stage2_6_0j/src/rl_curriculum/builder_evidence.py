"""Builder Run Evidence 与确定性证明(阶段 2.6.0j:v4)。

v4 语义(在 0i v3 之上):evidence 核心字段升级为不可逆密封计算——
- final_seccomp_filter_hash(scf-,default deny + 参数过滤的 final
  compute filter 摘要;Compute 阶段 syscall 边界);
- dependency_profile(formal/compat;compat 运行 formal_eligible=
  false,不得形成可信材料);
- top_level_purity/MDWE/fd 隔离/Compute 后二次实测进入 runtime
  lock v4(sealed_compute 块)与 sealed compute report v2;
- runtime_bundle_hash(rbm-,内容寻址运行时环境 manifest 摘要);
- deterministic_input_hash(edi-,sealed compute report v2:bundle/
  文件系统/proc/dev/clock/entropy/seccomp/线程静止/环境身份/探针
  /seal 边界,由 Worker 实测与 Supervisor 前后两次外部实测合并);
- 0i 的 v3 材料(final filter 缺失/default allow)被 v4 执行器拒绝。

公开组装通道(mock_payload_assembly)使用确定性哨兵
(edi-public-assembly / rbm-public-assembly);私有通道必须携带真实
edi-/rbm-/scf- 且 detail.deterministic_input_report 重算哈希一致。

一致性键从七组升级为:pack / attempt log / runtime lock /
deterministic input report / access summary + 进程树计数 + final
filter 哈希。precommit 双跑与考试期第三次重放按同一键组对账。
"""
...
EVIDENCE_CORE_FIELDS: tuple[str, ...] = (
    ...
    "final_seccomp_filter_hash", "dependency_profile",
    "access_summary_hash",
...
def builder_run_evidence_hash(evidence: dict[str, Any]) -> str:
    ...
    if not isinstance(evidence, dict) or evidence.get(
        ...
        raise BuilderProvenanceError(
            ...
            f"{(evidence or {}).get('format')!r};0h v2 与 0i v3 "
            f"evidence 已被 v4 执行器拒绝)")
    return "bre-" + _canonical_json_hash(
        builder_run_evidence_core(evidence))


#: 单次运行的一致性键(edi-/rbm-/scf- 进键;不含 run 序号)
...
def _run_consistency(run: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "final_seccomp_filter_hash": str(
            run.get("final_seccomp_filter_hash") or ""),
        "dependency_profile": str(run.get("dependency_profile") or ""),
    }
...
def build_builder_run_evidence(
    ...
    if mode == BUILDER_RUN_MODE_EXECUTION:
        ...
        fin_hash = str(run.get("final_seccomp_filter_hash") or "")
        if not fin_hash.startswith("scf-") \
                or fin_hash == MOCK_FINAL_FILTER_HASH:
            raise BuilderProvenanceError(
                "私有通道 evidence 缺少 final compute filter 哈希"
                "(scf-;default deny 边界未被绑定,2.6.0i 的 default "
                "allow 材料被拒绝)")
        if str(run.get("dependency_profile") or "") != "formal":
            raise BuilderProvenanceError(
                "私有通道 evidence 的依赖策略必须是 formal(兼容"
                "profile 的运行不得形成可信材料;A3)")
    ...
    core = {
        ...
        "final_seccomp_filter_hash": views[0]["final_seccomp_filter_hash"]
        if mode == BUILDER_RUN_MODE_EXECUTION else MOCK_FINAL_FILTER_HASH,
        "dependency_profile": views[0]["dependency_profile"]
        if mode == BUILDER_RUN_MODE_EXECUTION else "public-assembly",
...
def replay_builder_for_evidence(
    ...
    expected = {
        ...
        "final_seccomp_filter_hash": str(
            core.get("final_seccomp_filter_hash") or ""),
        "dependency_profile": str(core.get("dependency_profile") or ""),
    }
    if view != expected:
        raise BuilderProvenanceError(
