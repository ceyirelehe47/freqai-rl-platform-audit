"""工作包 C/E + D8:协议升级(null-qualification-v4 / sealed v5 /
CLI v6,阶段 2.6.0e 同步)与全链路绑定对账。

- 旧 v1/v2/v3 Null 报告与 v3/v4 承诺均不得被新执行器自动接受;
...
def test_v3_commitment_rejected_by_v4_executor(sealed_exam_env):
    ...
    v4_json = sealed_exam_env["commitment"].to_json().replace(
        "sealed-exam-commitment-v11", "sealed-exam-commitment-v4")
    ...
    v3_json = sealed_exam_env["commitment"].to_json().replace(
        "sealed-exam-commitment-v11", "sealed-exam-commitment-v3")
...
def test_commitment_binds_full_null_chain(sealed_exam_env):
    ...
    def pack_builder_code_hash():
        return mock_builder_identity().manifest_hash
    ...
    assert c.pack_builder_code_hash == (
        mock_builder_identity().manifest_hash)
    pv = c.pack_validity
...
def test_reports_carry_full_statistical_evidence(null_qual_chain):
    ...
    from rl_curriculum.null_friction import LEDGER_ROUND_TRIP_FORMULA

    chain = null_qual_chain
    for fam, rep in chain["reports"].items():
        assert rep["format"] == "null-qualification-v4"
        ...
        assert rep["margin"]["derivation"]["units"] == "simple_return"
...
def test_power_analysis_binding_and_targets(null_qual_chain):
    """A5(2.6.0e v2 语义):功效分析确定可复现;四块 required 场景
    全覆盖;cluster 阶梯选择记录;目标以 Wilson 保守界判定。"""
    from rl_curriculum.null_power_analysis import (
        run_power_analysis,
    ...
    assert t["confidence_method"].startswith("wilson")
    assert power["required_scenarios_complete"] is True
    assert power["skipped_required_scenarios"] == []
    assert power["format"] == "null-power-analysis-v2"
    # 阶梯选择:选定档必须同时满足功效硬目标与前缀资格经济充分性
    sel = power["cluster_selection"]
    assert sel["selected"] == MIN_QUALIFICATION_CLUSTERS
    by_n = {e["n"]: e for e in sel["ladder"]}
    assert by_n[sel["selected"]]["selectable"] is True
    assert by_n[sel["selected"]]["targets_met"] is True
    assert by_n[sel["selected"]][
        "qualification_economics_sufficient"] is True
    # 目标覆盖全部 family x required block(不只 Always Long)
    for key, block_detail in t["by_family_block"].items():
        assert block_detail["met"] is True, key
        assert {"zero_edge", "violation_plus_1x",
                "violation_plus_2x"} <= set(block_detail)
    # 确定性重跑 -> 同一报告 hash(执行器重验证的契约)
