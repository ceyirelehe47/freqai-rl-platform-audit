...
def run_attack(tmp_path, seed_pack_and_dc):
    ...
    top_imports="", ...) -> 成功返回 run record dict;失败返回
    ("异常类型名", "消息")。2.6.0j:top_imports 允许把 allowlist 模块
    (os/time/json 等)提升到模块顶层(Prepare 加载)——Compute 内
    import 违零 import 合同,依赖顶层形态的语义测试用它迁移。
    """
    ...
    def _run(body: str, *, external_dependencies=None, profile=None,
             ...
             bundle_pool=None):
        if top_imports:
            from tests.route_c_stage2_6_0j.conftest import (
                write_attack_builder_2j,
            )

            root = write_attack_builder_2j(
                tmp_path / label, body, max_attempts=max_attempts,
                top_imports=top_imports, label=label,
