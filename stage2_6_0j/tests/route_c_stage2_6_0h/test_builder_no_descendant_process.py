"""工作包 A:Builder 后代执行禁止(真实隔离 Runner 攻击;2.6.0i)。

- A1:fork/vfork/clone/clone3/execve/execveat/ptrace/process_vm_*/
  mount/umount2/unshare/setns 全拒;clone 不再放行 CLONE_THREAD
  (正式 Builder 禁止一切线程;clone_thread_denied=ERRNO1);
- A3 攻击:subprocess 启动子 Python import 未注册第三方包、/bin/sh、
  os.posix_spawn、fork+exec、ctypes.CDLL、外部可执行文件、线程创建
  ——全部在 pack 采信前失败(消息含相关 stage);
- PoC(修复前漏检证明):seccomp 关闭的演示 profile 下子进程 import
  不进入父 Runner 锁(v1 缺陷),输出三次一致;v3 语义下此类 run 的
  child_process_count>0/evidence 被拒。
"""
...
def _assert_stage_failure(result):
    """攻击必须失败,且失败消息携带 Runner stage(定位到具体阶段)。"""
    assert isinstance(result, Exception), "攻击必须被拒绝"
    assert "stage" in str(result), str(result)
...
def test_ctypes_cdll_unbound_rejected(sealed_exam_env, tmp_path):
    """ctypes.CDLL 加载 builder 自备 .so(未绑定位置)被锁拒绝。"""
    import shutil
...
def test_thread_creation_denied(sealed_exam_env, tmp_path):
    """A1(2.6.0i):clone 不再放行 CLONE_THREAD——线程创建也被拒绝
    (seccomp clone_action=EPERM;quiesce 线程静止恰 1 任务)。"""
    body = (
        "    import threading\n"
        "    box = []\n"
    ...
    root = write_attack_builder(tmp_path / "thread_denied", body,
                                label="thread-denied")
    provider = private_provider_from_root(root)
    result = _run(provider, sealed_exam_env, body, root=root)
    _assert_stage_failure(result)
...
def test_clone_thread_probe_denied_on_real_run(sealed_exam_env,
                                               private_builder_a):
    """真实成功 run 的 EDIC:clone(线程)探针 ERRNO1 + 线程静止。"""
    ...
    assert record["child_process_count"] == 0
    assert record["exec_count"] == 0
...
def test_poc_descendant_import_bypasses_v1_lock(sealed_exam_env,
    ...
    for _ in range(3):
        ...
        # 但进程树计数暴露了后代进程(v3 新语义:max(外部实测,审计))
        assert record["child_process_count"] >= 1
        assert record["thread_policy"] == "demo_allow_descendants"
        assert record["runtime_lock"]["process_tree_policy"] == \
            "allow_descendants"
...
def test_descendant_run_cannot_formal_evidence(sealed_exam_env, tmp_path,
                                               descendant_demo_profile):
    """v3 语义:child_process_count>0 的 run 无法通过 evidence 校验。"""
    body = (
        "    import subprocess, sys\n"
        "    subprocess.run(\n"
