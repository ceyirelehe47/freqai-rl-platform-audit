...
def test_runner_lock_records_closure_and_bundle(sealed_exam_env, tmp_path):
    """真实 Runner 锁(2.6.0j 升级):正式 profile 拒绝 Compute 内
    import numpy(A3);兼容 profile 顶层 import 于 Prepare 完成,
    import_closure 逐文件 64-hex 绑定;numpy distribution 有
    file+sha256;native 绑定齐全;bundle rbm-。"""
    ...
    req = attack_request(provider, sealed_exam_env["pack"],
                         _dc(sealed_exam_env))
    # A3:正式 profile 拒绝 Compute 内 NumPy import
    with pytest.raises(BuilderRunnerError):
        run_isolated_builder_run(
            provider.builder_identity(), req, builder_root=root)
    # 兼容 profile:顶层 import 形态(numpy 于 Prepare 完成并预热)
    import shutil as _shutil

    root2 = tmp_path / "np_top"
    root2.mkdir()
    src = (root / "builder_attack.py").read_text(encoding="utf-8")
    inject = ("import numpy\n"
              "_WARM = numpy.random.default_rng(7)\n")
    src2 = src.replace("def build_pack(request):",
                       inject + "def build_pack(request):", 1)
    # Compute 内 import 违零 import 合同(即便 compat):删除函数体内
    # 的 import 行,仅保留顶层注入(Prepare 完成)
    src2 = src2.replace("    import numpy\n", "")
    (root2 / "builder_attack.py").write_text(src2, encoding="utf-8")
    for aux in ("params.json", "provider_config.json"):
        _shutil.copy(root / aux, root2 / aux)
    provider2 = private_provider_from_root(root2)
    record = run_isolated_builder_run(
        provider2.builder_identity(), req, builder_root=root2,
