...
实际暴露(filled_amount)定义(依据固定 commit 52bc96f 源码):
    ...
    + 活动入场单累计成交 (open entry orders 的 safe_filled,扣 base 币手续费)
    - 活动退出单累计成交 (open exit orders 的 safe_filled)
...
class ExecutionSnapshot:
    ...
    def describe(self) -> str:
        lines = [
            ...
            f"活动入场单: {[f'{o.order_id} filled={o.filled}/{o.amount}' for o in self.open_entry_orders]}",
            f"活动退出单: {[f'{o.order_id} filled={o.filled}/{o.amount}' for o in self.open_exit_orders]}",
        ]
...
def resolve_execution_state(
    ...
    entry_open_filled = sum(o.filled for o in entry_orders)
    exit_open_filled = sum(o.filled for o in exit_orders)
