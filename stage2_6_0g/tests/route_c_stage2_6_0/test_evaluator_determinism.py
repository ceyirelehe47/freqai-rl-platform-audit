...
def test_repeat_evaluation_identical(gen_a, schema):
    eps = _eps(gen_a)
...
def test_summarize_stats_complete(gen_a, schema):
    eps = _eps(gen_a)
...
def test_reward_inconsistency_fails_closed(gen_a, cfg, schema):
    """reward 与最终净值不一致 -> EvaluationError(EXAM_INVALID)。"""
    ep = gen_a.generate(TRAIN_PARAMS, seed=171, timeframe="15m")
    ...
    # 一致性检查通过。
    result, _actions, _obs = run_policy_episode(
...
def test_nan_metrics_fail_closed(gen_a, schema):
    ...
    with pytest.raises(Exception):
        evaluate_policy(_NaNPolicy(ma_threshold=0.001), eps, CFG, schema)


def test_behavior_fingerprint_stable(gen_a, schema):
...
def test_eval_config_recorded(gen_a, schema):
    rep = evaluate_policy(
        RuleTrendPolicy(ma_threshold=0.001), _eps(gen_a), CFG, schema)
