"""阶段 2.6.0g 收尾:builder-build-request-v2 / result-v2 / attempt log
合同的精确字段校验(工作包 C/D)。
...
def _base_request(frozen_request):
    ...
    req["format"] = "builder-build-request-v2"
    req["runner_protocol"] = "builder-runner-protocol-v2"
    return req
...
def test_result_contract_fields_enforced():
    ...
    code = (
        ...
        "    r = {'format': 'builder-build-result-v2',\n"
...
def test_attempt_log_selected_must_be_accept():
    ...
    with pytest.raises(BuilderProvenanceError, match="selected_attempt"):
        check_attempt_log(log)
