...
阶段 2.6.0b 更新:严格三族改为 sign/volstate/stochvol;probe_null_block
降级为诊断族(is_null_family=False)不进入正式集合。"""
...
def test_three_formal_null_families_registered():
    ...
    # 阶段 2.6.0b:block shuffle 降级为诊断族,不在严格集合中
    block = DEFAULT_GENERATOR_REGISTRY["probe_null_block"]
    assert block.is_null_family is False
