"""阶段 2.6.0a 工作包 A + 阶段 2.6.0b 工作包 B:基线策略库 / Oracle / SB3 候选适配(obs-only)。
...
阶段 2.6.0b 工作包 B:
- 所有 reset_episode() 无参数(候选不接收任何 Episode 身份 token);
- RandomPolicy 的确定性由 episode_instance(episode_seed) 工厂承载:
  评估器每 Episode 构造独立实例,seed 只进入 ObservableBaselinePolicy
  通道,正式候选永不接收。
...
class RandomPolicy(ObservableBaselinePolicy):
    """随机基线:每 Episode 由评估器经 episode_instance(seed) 构造。

    确定性由评估器管理(derived_episode_seed(spec) 作为构造参数),
    不受上一 Episode 调用次数影响、不受 Episode 输入顺序影响、
    重复评估完全一致。seed 只存在于基线通道——正式 CandidatePolicy
    的 reset_episode() 无参数,永不接收 seed(工作包 B)。
    """
    ...
    def episode_instance(self, episode_seed: int) -> "RandomPolicy":
        return RandomPolicy(seed=int(episode_seed))
...
class SB3CheckpointPolicy(CandidatePolicy):
    ...
    工作包 B:reset_episode() 无参数(前馈 PPO 无跨 Episode 状态;
    RNN 候选在 reset 内显式清零,不接收任何 Episode 身份 token)。
    """
