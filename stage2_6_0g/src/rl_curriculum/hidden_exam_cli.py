...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.builder_provider == "mock":
        ...
        # 阶段 2.6.0g P5:CLI 与承诺创建端共用同一配置解析
        # (load_builder_provider_config / private_provider_from_config,
        # 单一字段清单——pair_count_per_family / max_attempts /
        # external_dependencies 等字段不再被 CLI 遗漏;未知字段与
        # 缺失必填字段 fail closed;entrypoint 真实存在性验证在
        # Provider 构造期执行)
        try:
            builder_provider = private_provider_from_config(
                args.builder_provider_root)
        except BuilderIdentityError as exc:
            print(
                f"[hidden_exam_cli] 私有 Provider 配置/入口无效: {exc}",
                file=sys.stderr,
            )
            return 2
    else:
