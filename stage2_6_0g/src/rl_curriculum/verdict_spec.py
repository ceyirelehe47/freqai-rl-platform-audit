"""阶段 2.6.0a 工作包 G + 阶段 2.6.0b 工作包 D/E/H/I + 阶段 2.6.0c
工作包 C:冻结课程判定器。

CourseVerdictSpec(course-verdict-spec-v3)新增(阶段 2.6.0c):
- seed_aggregation: 反作弊复制证据的 seed 内聚合规则(预注册名,
  当前 "per-seed-worst-variant-v1"):同一 seed 的多个
  cut/variant 先在 seed 内聚合(变体净收益取最坏值),bootstrap 的独立抽样单位是 seed——多 cut 不得
  当作多个独立 Episode/seed;聚合规则进入判定器哈希与密封承诺,
  不得考后挑选聚合方式(C3)。
...
VERDICT_SPEC_FORMAT = "course-verdict-spec-v3"
#: 当前支持的 seed 内聚合规则(build_replication_evidence 实现)
SUPPORTED_SEED_AGGREGATIONS: tuple[str, ...] = (
...
class CourseVerdictSpec:
    ...
    # ---- seed 内聚合规则(阶段 2.6.0c 工作包 C3:预注册,冻结进哈希)
    ...
    def __post_init__(self) -> None:
        if self.seed_aggregation not in SUPPORTED_SEED_AGGREGATIONS:
            raise VerdictSpecError(
                f"未注册的 seed 聚合规则 {self.seed_aggregation!r}:"
                f"只允许 {list(SUPPORTED_SEED_AGGREGATIONS)}"
                f"(聚合规则必须考试前冻结,不得考后挑拣)")
...
def probe_course_verdict_spec() -> CourseVerdictSpec:
    ...
    min_effective_net_return=0.02(2.6.0c 工作包 C 校准):作弊判定的
    "最低有效成绩"与"优势崩溃线"同一条线——空仓变体(净收益恰为 0)
    构成优势崩溃(bootstrap 上界 0 < 0.02),与 tests/route_c_stage2_6_0
    作弊场景阈值一致。
    """
    return CourseVerdictSpec(
        version="probe-course-verdict-mock-v3",
...
def verdict_spec_from_json(data: dict[str, Any]) -> CourseVerdictSpec:
    ...
    return CourseVerdictSpec(
        ...
        seed_aggregation=str(
            data.get("seed_aggregation", "per-seed-worst-variant-v1")),
