"""Long/Flat 净值账本(阶段 2.5 路线 C 核心模块)。
...
确定性滑点(基点表示):
- 买入成交价 = open[t+1] * (1 + slippage_bps / 10000)
- 卖出成交价 = open[t+1] * (1 - slippage_bps / 10000)
"""
...
class LongFlatLedger:
    ...
    买入:全部可用现金按成交价(含滑点)买入,手续费按名义金额计,
    约束 notional + fee = cash,即 qty = cash / (exec_price * (1 + fee))。
    卖出:全部 BTC 按成交价(含滑点)卖出,所得 = notional * (1 - fee)。
    """
    ...
    def _buy_all(self, raw_open: float) -> TradeRecord:
        exec_price = raw_open * (1.0 + self.slippage_bps / 10000.0)
    ...
    def apply_target(self, target: int, raw_open: float) -> TradeRecord:
        """在执行 bar 的 raw_open 上把仓位调整到目标(幂等:重复目标不交易)。"""
    ...
    def liquidate(self, raw_open: float) -> TradeRecord:
        """Episode 终端强制清算(在最终执行 bar 的 open 上卖出,含滑点与费用)。"""
        if self.btc > 0:
            return self._sell_all(raw_open, direction="liquidate")
        return TradeRecord(
            direction="hold", raw_open=raw_open, exec_price=raw_open, notional=0.0,
            fee_paid=0.0, slippage_cost=0.0, qty=0.0,
            cash_after=self.cash, btc_after=self.btc,
