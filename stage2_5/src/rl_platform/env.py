"""对齐的 Long/Flat Gymnasium 环境(阶段 2.5 路线 C 核心)。
...
bar N-1。terminated 时若仍持多头,在 open[N-1] 按卖出滑点+手续费强制清仓,
与 Freqtrade 回测器 handle_left_open 用最后一根 bar 的 open 强平同族。
...
class AlignedLongFlatEnv(gym.Env):
    ...
    参数:
        ...
        prices:   含 open/close 两列的 DataFrame(原始价格,未缩放);
        fee:      单边手续费率;
        slippage_bps: 确定性滑点(基点);
        initial_cash: 期初现金;
        reward_scale: 奖励固定缩放常数(写入配置;info 同时保留原始值);
        window_size:  观察特征窗口行数(含当前行);
    ...
    def __init__(
        ...
        self.ledger = LongFlatLedger(
            initial_cash=self.initial_cash, fee=self.fee, slippage_bps=self.slippage_bps
        )
