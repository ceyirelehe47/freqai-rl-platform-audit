...
ART = ROOT / "artifacts" / "route_c_stage2_6_0i"

#: 全量目录(逐目录独立 pytest;与历史脚本一致)
ALL_DIRS = [
...
#: 0i 硬链接别名 TOCTOU)。这些目录必须**独占**运行:其余目录的
#: bundle 组装/挂载视图复验会读取 env 文件,并发就地篡改会造成跨
#: 目录互扰(full-cold 首轮实测:0h 32 errors 由并发 0i 的 env 就地
#: 写引发)。运行顺序:普通目录并行完成后,互斥目录逐个独占执行。
...
def _run_all(dirs: list[str], mode: str, workers: int) -> dict:
    ...
    total = {k: sum(r[k] for r in results) for k in
             ("passed", "failed", "skipped", "xfailed", "error")}
    def _dir_ok(r):
        if r["dir"] == "tests/freqai_rl_platform_audit" \
                and r["exit_code"] == 5 and r["passed"] == 0 \
                and r["failed"] == 0 and r["error"] == 0:
            return True  # 该目录只含脚本,无 pytest 测试(exit 5=未收集)
...
def _select_affected() -> tuple[list[str], list[str], str]:
    ...
    if not changed:
        return (["tests/route_c_stage2_6_0i"],
                ["无变更:默认跑 2.6.0i 目录"], "affected")
    reasons: list[str] = []
    dirs: set[str] = set()
    escalate = False
    for path in changed:
...
def mode_quick(extra: str | None) -> int:
    targets = ["tests/route_c_stage2_6_0i"]
    if extra:
