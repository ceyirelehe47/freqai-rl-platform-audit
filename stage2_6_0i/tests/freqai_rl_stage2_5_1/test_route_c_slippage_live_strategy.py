...
def run_bt_round(fee: float, slippage_bps: float, narrow: bool = False) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # 阶段 2.5.2a:策略已是市场订单,校验要求 entry price_side = other
        conf["entry_pricing"]["price_side"] = "other"
...
def test_route_c_slippage_parity_end_to_end():
    ...
    # 阶段 2.5.2a 更新:非零滑点与窄 K 线轮基于 custom price 钩子
    # (依赖执行 K 线最终 high/low 的非因果合同,已被 market_open_causal
    # 市场订单合同取代)——保留零滑点轮,价格语义改为 open[t+1] 恒等;
    # 非零模拟滑点只属环境压力测试,不声称回测 parity。
    cases = [
