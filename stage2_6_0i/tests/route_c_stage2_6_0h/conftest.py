"""阶段 2.6.0h 测试夹具(2.6.0i 协议适配):Builder 后代执行、密闭
确定性输入证明与 Attempt 选择策略闭环。

- A1/A3:seccomp v2 进程树策略(子进程/exec/线程全禁)与真实攻击
  Builder(子 Python import 未注册第三方、/bin/sh、posix_spawn、
  fork+exec、ctypes.CDLL、外部可执行文件);
- B1/B3:内容寻址 bundle 根 fs(私有最小 /dev、确定性熵、宿主
  /dev/shm//etc//sys/临时目录不可利用);
- C1-C3:Effective Deterministic Input Report(edi-;实际生效状态
  + 探针 + bundle 绑定);
- D1-D4:运行时依赖锁 v3(进程树边界 + bundle manifest 内容绑定 +
  native 绑定);
- E2/E3:first_pass attempt 硬约束;
- F:evidence v3 新核心哈希(edi-/rbm-/acs-/进程树计数/isolation);
- G:checkpoint 前访问守卫 v2(事件扩展 + namespace 不可命名);
- sealed-exam-commitment-v10 / null-pack-builder-manifest-v5 /
  builder-run-evidence-v3 / hidden-exam-cli-v11 全链路。
"""
