...
def _attack_builder(import_lines, error_prefix):
    return (
        ...
        f"    return {{'format': 'builder-build-result-v3',\n"
        f"            'runner_protocol': 'builder-runner-protocol-v3',\n"
        f"            'status': 'failed', 'pack': None,\n"
        f"            'attempt_log': {{'format': 'builder-attempt-log-v2',\n"
...
def test_function_level_import_audited(tmp_path, sealed_exam_env,
    ...
    # 2.6.0i:A2 按实际文件路径归属并绑定 bundle manifest 字节
    assert numpy_entry["distribution"] == "numpy"
    assert numpy_entry["file"].startswith("/")
    assert len(numpy_entry["sha256"]) == 64
    int(numpy_entry["sha256"], 16)  # 64 位 hex
    ...
    lock = {
        ...
        "distributions": [
            {"module": "leftpad", "distribution": "leftpad",
             ...
             "file": "/lib/python3.11/site-packages/leftpad/__init__.py",
             "sha256": "f" * 64,
             "imported": ["leftpad"]},
        ],
        "import_closure": [{
            "module": "leftpad", "loader": "SourceFileLoader",
            "origin_kind": "file",
            "file": "/lib/python3.11/site-packages/leftpad/__init__.py",
            "sha256": "f" * 64,
            "owner": "distribution", "distribution": "leftpad",
        }],
        "native_libraries": [],
    }
