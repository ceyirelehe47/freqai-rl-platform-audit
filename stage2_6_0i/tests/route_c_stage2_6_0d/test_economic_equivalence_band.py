...
- margin = 冻结账本一次完整往返的真实摩擦(阶段 2.6.0e 修正:
  1-[(1-fee)/(1+fee)]*[(1-slip)/(1+slip)] = 0.002/1.001 =
  0.001998001998...,不写死常数;旧公式 1-(1-fee)^2*(1-slip)^2 =
  0.001999 已废除);
...
from rl_curriculum.null_friction import (
    ledger_round_trip_friction as _exact_friction,
)
...
def test_margin_is_exact_round_trip_friction(cfg):
    """A4 硬要求(2.6.0e 修正):margin 按冻结账本真实乘法成本精确
    计算(fee=0.001、slippage=0 时 = 0.002/1.001 =
    0.001998001998...,不是旧公式的 0.001999,更不是写死的 0.002)。"""
    margin = round_trip_friction(cfg)
    assert margin == _exact_friction(0.001, 0.0)
    assert abs(margin - 0.002 / 1.001) < 1e-15
    assert margin < 0.002  # 精确值严格小于名义 0.002
    # 旧错误公式必须被明确区分(不得回退)
    assert margin != 1 - (1 - 0.001) ** 2
    assert abs(margin - 0.001999) > 1e-9
...
def test_margin_below_both_review_counterexamples(cfg):
    """margin(0.001998002)远小于两个审查反例(+2.40% / +0.75%)与旧
