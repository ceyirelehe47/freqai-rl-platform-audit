...
def test_bool_only_binding_rejected(sealed_exam_env):
    ...
    assert any("bool-only" in p or "v3 结构" in p
...
def test_binding_with_pass_but_no_report_rejected(sealed_exam_env):
    ...
    report = verify_null_qualification_bindings(
        ...
        **_verify_kwargs(sealed_exam_env))
    assert not report["pass"]
    assert any("v3 结构" in p or "真实报告" in p
               for p in report["problems"])
...
def test_seed_count_insufficient_rejected(sealed_exam_env):
    """报告独立 cluster 数不足(阶段 2.6.0d 功效门槛 64)被拒。"""
    bindings = _ok_bindings(sealed_exam_env)
    payload = bindings["probe_null_sign"]["report_payload"]
    payload["seeds"] = [11, 22]
    ...
    report = verify_null_qualification_bindings(
        ...
        **_verify_kwargs(sealed_exam_env))
    assert not report["pass"]
    assert any("cluster 数不足" in p or "INSUFFICIENT" in p
               for p in report["problems"])
...
def test_false_required_check_rejected(sealed_exam_env):
    bindings = _ok_bindings(sealed_exam_env)
    payload = bindings["probe_null_sign"]["report_payload"]
    payload["checks"]["oracle_no_tradable_edge"] = False
    bindings["probe_null_sign"]["report_hash"] = \
        qualification_report_hash(payload)
    report = verify_null_qualification_bindings(
        bindings, required_families=list(FAMILIES),
...
def test_real_reports_carry_full_reconciliation_material(
        null_qual_reports):
    """真实生成的报告内嵌全部对账材料(每族;阶段 2.6.0d v3 三态)。"""
    from rl_curriculum.null_qualification import CLUSTER_AGGREGATION

    for fam, report in null_qual_reports.items():
        assert set(report) == set(NULL_REPORT_REQUIRED_KEYS)
        assert report["format"] == NULL_QUALIFICATION_FORMAT
        assert report["verdict"] == "QUALIFIED"
        assert report["pass"] is True
        assert report["n_clusters"] >= MIN_QUALIFICATION_CLUSTERS
        assert report["distinct_seeds"] == report["n_clusters"]
        assert report["cluster_aggregation"] == CLUSTER_AGGREGATION
        assert set(report["checks"]) == set(REQUIRED_NULL_CHECKS)
        # 阶段 2.6.0d:资格参数经 qualification spec(nqs-)绑定,
        # 报告携带 spec hash/统计协议/真实时长/精确 margin 推导
        assert report["qualification_spec_hash"].startswith("nqs-")
        assert report["statistical_protocol"]["confidence_level"] == 0.95
        assert report["statistical_protocol"]["bootstrap_seed"] == 20260826
        assert report["episode_duration_hours"] == 24.0
        from rl_curriculum.null_friction import LEDGER_ROUND_TRIP_FORMULA
        assert report["margin"]["derivation"]["formula"] == (
