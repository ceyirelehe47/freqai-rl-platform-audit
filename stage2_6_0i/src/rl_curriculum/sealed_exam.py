...
SEALED_EXAM_PROTOCOL = "sealed-exam-commitment-v10"
_DEPRECATED_PROTOCOLS = ("sealed-exam-commitment-v1",
                         ...
                         "sealed-exam-commitment-v8",
                         "sealed-exam-commitment-v9")
...
class SealedExamCommitment:
    ...
    #: builder attempt 选择策略(阶段 2.6.0h E1:first_pass 显式进入
    #: 承诺;与冻结构建请求/manifest v5 attempt_policy/evidence nap-
    #: 四方一致,不可在多个合格 pack 中后选)
    builder_attempt_policy: dict[str, Any] = field(default_factory=dict)
    #: 受信训练签发方(工作包 G6)
    ...
    def canonical_payload(self) -> dict[str, Any]:
        return {
            ...
            "builder_attempt_policy": self.builder_attempt_policy,
    ...
    def from_json(text: str) -> "SealedExamCommitment":
        ...
        if data.get("protocol_version") != SEALED_EXAM_PROTOCOL:
            if data.get("protocol_version") in _DEPRECATED_PROTOCOLS:
                raise SealedExamError(
                    ...
                    f"AST 静态闭包与配置自报(阶段 2.6.0g 收尾);v8 的"
                    f"Builder 仍可创建子进程且子进程 import 不进入父 "
                    f"Runner 的运行时依赖锁,宿主 /dev(/dev/shm)//etc//"
                    f"sys 整体可见可利用,运行时锁只哈希 distribution "
                    f"的 RECORD 文件而不验证实际安装文件内容,attempt "
                    f"log 无 first_pass 强约束(可跳过更早的合格 attempt"
                    f"选中更后者),evidence 只绑定期望 profile 而未绑定"
                    f"实际生效的 namespace/Landlock/seccomp/mount/"
                    f"rlimit 与 access summary/进程树计数,主进程 "
                    f"checkpoint 前访问守卫只覆盖 open 事件(阶段 "
                    f"2.6.0h)——不得被 v9 执行器自动接受,必须以 "
        ...
        if not runtime_manifest or not runtime_hash:
            raise SealedExamError(
                "v9 承诺必须绑定候选运行时内容(candidate_runtime_"
        ...
        if not duration_contract_hash.startswith("ndc-") or (
            ...
            raise SealedExamError(
                "v9 承诺必须绑定全局 strict Null duration contract"
        ...
        if not spec_hash.startswith("nqs-"):
            raise SealedExamError(
                "v9 承诺必须绑定 Null 资格规范哈希(null_qualification_"
        ...
        if (not str(power.get("report_hash") or "").startswith("npa-")
            ...
            raise SealedExamError(
                "v9 承诺必须绑定完整功效分析(null_power_analysis:"
        ...
        if (not str(pack_validity.get("report_hash") or "").startswith("npv-")
            ...
            raise SealedExamError(
                "v9 承诺必须绑定实际 Null pack 的 pack-level validity"
        ...
        if not str(data.get("pack_builder_code_hash") or "").startswith(
                "npb-"):
            raise SealedExamError(
                "v9 承诺必须绑定 pack 构建算法 manifest 哈希(pack_"
        ...
        if not build_request_hash.startswith("nbr-") or not build_request:
            raise SealedExamError(
                "v9 承诺必须绑定 builder 冻结构建请求(builder_build_"
        ...
        if not str(run_evidence.get("evidence_hash") or "").startswith(
                "bre-"):
            raise SealedExamError(
                "v9 承诺必须绑定 Builder Run Evidence 摘要(builder_run_"
        ...
        if not str(run_evidence.get("deterministic_input_hash") or ""):
            raise SealedExamError(
                "v10 承诺的 builder_run_evidence 必须绑定密闭确定性"
                "输入报告哈希(deterministic_input_hash;edi- 或组装"
                "通道哨兵)——没有确定性输入报告的材料被拒绝(阶段"
                " 2.6.0i D2;EXAM_INVALID)")
        if not str(run_evidence.get("runtime_bundle_hash") or ""):
            raise SealedExamError(
                "v10 承诺的 builder_run_evidence 必须绑定内容寻址 "
                "runtime bundle 哈希(runtime_bundle_hash;rbm- 或组装"
                "通道哨兵)——以活环境为输入的材料被拒绝(阶段"
                " 2.6.0i A1;EXAM_INVALID)")
        if not str(run_evidence.get("thread_policy") or ""):
            raise SealedExamError(
                "v10 承诺的 builder_run_evidence 必须绑定线程策略"
                "(threads_forbidden_clone_denied;2.6.0i C4;"
                "EXAM_INVALID)")
        if not str(run_evidence.get("process_tree_policy") or ""):
            raise SealedExamError(
                "v9 承诺的 builder_run_evidence 必须绑定进程树策略"
                "(process_tree_policy;single_builder_process;"
                "EXAM_INVALID)")
        if run_evidence.get("child_process_count") != 0 or \
                run_evidence.get("exec_count") != 0:
            raise SealedExamError(
                "v9 承诺绑定了存在后代进程/exec 的 evidence(允许"
                "子进程的材料被拒绝;阶段 2.6.0h D1;EXAM_INVALID)")
        if not str(run_evidence.get("runner_isolation") or ""):
            raise SealedExamError(
                "v9 承诺的 builder_run_evidence 必须绑定 runner "
                "isolation 身份(EXAM_INVALID)")
        c = SealedExamCommitment(
            ...
            builder_attempt_policy=dict(
                data.get("builder_attempt_policy") or {}),
...
def verify_sealed_commitment(
    ...
    *,
    ...
    ev_ok = (
        ...
        and bool(ev.get("deterministic_input_hash"))
        and bool(ev.get("runtime_bundle_hash"))
        and bool(ev.get("access_summary_hash"))
        and bool(ev.get("thread_policy"))
        and bool(ev.get("process_tree_policy"))
        and ev.get("child_process_count") == 0
        and ev.get("exec_count") == 0
        and bool(ev.get("runner_isolation")))
    check("builder_run_evidence_binding", ev_ok,
          ...
          f"nbr/pack 或 edi/rbm/access_summary/process_tree/thread"
          f"/runner_isolation 任一失配):没有一致 Run Evidence 的材料"
          f"不得进入正式考试(EXAM_INVALID;阶段 2.6.0g 收尾 E4 + "
          f"2.6.0h F + 2.6.0i D3)")
    committed_policy = dict(commitment.builder_attempt_policy or {})
    request_policy = dict((commitment.builder_build_request or {}).get(
        "attempt_policy") or {})
    if committed_mode == "mock_payload_assembly":
        policy_ok = (
            committed_policy == request_policy
            and committed_policy.get("policy") == "assembly"
            and committed_policy.get("max_attempts") == 0)
    else:
        policy_ok = (
            committed_policy == request_policy
            and committed_policy.get("policy") == "first_pass"
            and isinstance(committed_policy.get("max_attempts"), int)
            and not isinstance(committed_policy.get("max_attempts"), bool)
            and int(committed_policy.get("max_attempts") or 0) >= 1)
    check("builder_attempt_policy_binding", policy_ok,
          f"builder attempt 选择策略绑定失败:承诺 {committed_policy!r}"
          f"必须与冻结构建请求的 attempt_policy 完全一致且为 "
          f"first_pass/正整数上限(E1:策略进入请求/manifest/evidence/"
          f"承诺且四方一致;EXAM_INVALID;阶段 2.6.0h)")
