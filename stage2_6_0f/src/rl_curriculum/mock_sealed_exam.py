...
def _validate_pack_ephemeral(pack: ExamPack, cfg, schema, *,
    ...
    identity = (builder_identity if builder_identity is not None
                else MockBuilderIdentityProvider().builder_identity())
...
def build_mock_commitment(
    *, pack: ExamPack, charter: dict[str, Any], schema,
    ...
    profile = sandbox_profile or default_sandbox_profile()
    # ---- 工作包 A:builder 身份显式来自 Provider(mock 流程显式构造
    #      MockBuilderIdentityProvider;formal API 本身不硬编码任何
    #      mock provider——正式私有流程由评估方传入自己的 Provider)
    ...
    return SealedExamCommitment(
        ...
        notes={"mock": True, "声明": "公开 mock 承诺 v6,验证密封基础设施用"},
    )
