...
def write_private_builder(root: Path, files: dict | None = None,
                          label: str = "private-builder-a") -> Path:
    ...
    (root / "provider_config.json").write_text(
        __import__("json").dumps({
            "entrypoint_module": "builder_a",
            "entrypoint_qualname": "assemble_private_hidden_pack",
            "attempt_loop_module": "builder_a",
            "attempt_loop_qualname": "assemble_private_hidden_pack",
            "params_spec": {
                "base_params": {
                    "episode_bars": 96,
...
def private_provider_from_root(root: Path):
    ...
    pcfg = _json.loads(
        (Path(root) / "provider_config.json").read_text(encoding="utf-8"))
    return PrivateBuilderIdentityProvider(
        ...
        pair_count_per_family=int(pcfg.get("pair_count_per_family", 32)),
        max_attempts=int(pcfg.get("max_attempts", 8)),
...
def private_builder_b(tmp_path):
    """内容不同的第二个私有 builder(替换攻击用)。"""
    files = dict(PRIVATE_BUILDER_A_FILES)
    files["builder_a.py"] = files["builder_a.py"].replace(
        "def assemble_private_hidden_pack", "def assemble_private_hidden_pack_v2")
    ...
    return PrivateBuilderIdentityProvider(
        root, entrypoint_module="builder_a",
        entrypoint_qualname="assemble_private_hidden_pack_v2",
        params_spec={
            "base_params": {
                "episode_bars": 96,
                "drift_bps_range": [18.0, 30.0],
