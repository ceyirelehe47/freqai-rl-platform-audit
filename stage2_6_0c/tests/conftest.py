...
def null_qual_reports(schema, cfg):
    """三族严格 Null 的真实资格审查报告(v2,含全部对账材料)。"""
...
@pytest.fixture(scope="session")
def null_qual_bindings(null_qual_reports):
    from rl_curriculum.null_qualification import (
        build_null_qualification_bindings,
    )

    return build_null_qualification_bindings(null_qual_reports)


@pytest.fixture(scope="session")
def sealed_exam_env(null_qual_bindings, schema, cfg, mock_trusted_issuer,
                    sandbox_profile):
    """mock 密封考试环境:pack + commitment v3(含 runtime manifest)。"""
    ...
    commitment = build_mock_commitment(
        ...
        null_qualification_bindings=null_qual_bindings)
    return {
        "pack": pack,
        "charter": charter,
        "schema": schema,
